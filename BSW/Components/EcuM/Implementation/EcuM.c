...
#if((ECUM_CHANNELOFCOMMCHANNELREFS == STD_ON) || (ECUM_PNCOFCOMMPNCREFS == STD_ON) || (ECUM_SUPPORT_COMM == STD_ON)) /* COV_ECUM_PNCOFCOMMPNCREFS_QM */
# include "ComM.h"
...
#if (  (ECUM_SW_MAJOR_VERSION != (0x15u)) \
    || (ECUM_SW_MINOR_VERSION != (0x00u)) \
    || (ECUM_SW_PATCH_VERSION != (0x01u)) )
...
/* vendor specific version information is BCD coded */
#if (  (ECUM_CBK_SW_MAJOR_VERSION != (0x15u)) \
    || (ECUM_CBK_SW_MINOR_VERSION != (0x00u)) \
    || (ECUM_CBK_SW_PATCH_VERSION != (0x01u)) )
...
#define EcuM_EnterCriticalArea_3()         SchM_Enter_EcuM_ECUM_EXCLUSIVE_AREA_3()   /* PRQA S 3453 */ /* MD_MSR_FctLikeMacro */
#define EcuM_LeaveCriticalArea_3()         SchM_Exit_EcuM_ECUM_EXCLUSIVE_AREA_3()    /* PRQA S 3453 */ /* MD_MSR_FctLikeMacro */
...
#if((ECUM_NUMBER_OF_CORES > 1u) && (ECUM_RESOURCEOFPARTITIONDATA == STD_ON))
# define ECUM_SLEEP_RESOURCE(partitionIdx)                    EcuM_GetResourceOfPartitionData(partitionIdx)
#else
# define ECUM_SLEEP_RESOURCE(partitionIdx)                    ECUM_OS_RESOURCE
#endif
...
ECUM_LOCAL VAR(uint8, ECUM_VAR_NO_INIT) EcuM_CurrentShutdownMode;

/* Selected shutdown target for shutdown phase */
ECUM_LOCAL VAR(EcuM_StateType, ECUM_VAR_NO_INIT) EcuM_CurrentShutdownTarget;

/* Selected shutdown target for shutdown phase */
ECUM_LOCAL VAR(EcuM_StateType, ECUM_VAR_NO_INIT) EcuM_LastShutdownTarget; /* PRQA S 3218 */ /* MD_EcuM_3218 */

/* Get the last sleep/reset shutdownmode */
ECUM_LOCAL VAR(uint8, ECUM_VAR_NO_INIT) EcuM_LastShutdownMode; /* PRQA S 3218 */ /* MD_EcuM_3218 */

#if (ECUM_SHUTDOWNCAUSECOUNT != 0u)
/* Selected shutdown cause for next shutdown */
ECUM_LOCAL VAR(uint8, ECUM_VAR_NO_INIT) EcuM_ShutdownCause;
#endif

#if (ECUM_MODE_HANDLING == STD_ON)
/* Current Rte State to notify during MainFunction */
ECUM_LOCAL VAR(Rte_ModeType_EcuM_Mode, ECUM_VAR_NO_INIT) EcuM_RteStateBuffer;
#endif

/* Select the BootTarget for the next BootSequence */
ECUM_LOCAL VAR(EcuM_BootTargetType, ECUM_VAR_NO_INIT) EcuM_CurrentBootTarget;

#if(ECUM_ALARM_CLOCK_PRESENT == STD_ON)
/* Two variables to indicate the reason of a restart */
ECUM_LOCAL VAR(uint8, ECUM_VAR_NO_INIT) EcuM_IntendedRestartID_One;
ECUM_LOCAL VAR(uint8, ECUM_VAR_NO_INIT) EcuM_IntendedRestartID_Two;
#endif

#define ECUM_STOP_SEC_VAR_NO_INIT_8
#include "EcuM_MemMap.h" /* PRQA S 5087 */ /* MD_MSR_MemMap */

#define ECUM_START_SEC_VAR_NO_INIT_32
#include "EcuM_MemMap.h" /* PRQA S 5087 */ /* MD_MSR_MemMap */

/* Variable to store the validated wakeup event which was responsible for wakeup up the ECU */
ECUM_LOCAL VAR(EcuM_WakeupSourceType, ECUM_VAR_NO_INIT) EcuM_ValidatedWakeups;

#if(ECUM_SLEEPMODELIST == STD_ON)
/* Variable which stores information about enabled wakeup interrupts */
ECUM_LOCAL VAR(EcuM_WakeupSourceType, ECUM_VAR_NO_INIT) EcuM_EnabledWakeups;
#endif

/* Variable to store wakeups that occur during the startup before BswM is initialized */
ECUM_LOCAL VAR(EcuM_WakeupSourceType, ECUM_VAR_NO_INIT) EcuM_BswM_BufferedWakeups;

#if (ECUM_VALIDATIONTIMEOFWAKEUPSOURCELIST == STD_ON)

/* Saves all wakeup events which are not validated yet */
ECUM_LOCAL VAR(EcuM_WakeupSourceType, ECUM_VAR_NO_INIT) EcuM_PendingWakeups;

/* Variable to store the cleared wakeup event with still active wakeup sources */
ECUM_LOCAL VAR(EcuM_WakeupSourceType, ECUM_VAR_NO_INIT) EcuM_ClearedPendingWakeups;
...
ECUM_LOCAL VAR(EcuM_WakeupSourceType, ECUM_VAR_NO_INIT) EcuM_ExpiredWakeups;
...
ECUM_LOCAL VAR(EcuM_WakeupSourceType, ECUM_VAR_NO_INIT) EcuM_PendingCheckWakeups;

# if(ECUM_DEFERRED_BSWM_NOTIFICATION == STD_ON)
ECUM_LOCAL VAR(EcuM_WakeupSourceType, ECUM_VAR_NO_INIT) EcuM_ExpiredCheckWakeups;
# endif

#endif

#if((ECUM_CHANNELOFCOMMCHANNELREFS == STD_ON) || (ECUM_PNCOFCOMMPNCREFS == STD_ON)) /* COV_ECUM_PNCOFCOMMPNCREFS_QM */

/* Variable to store wakeups that occur during the startup before ComM is initialized */
ECUM_LOCAL VAR(EcuM_WakeupSourceType, ECUM_VAR_NO_INIT) EcuM_ComM_BufferedWakeups;

#endif

#if(ECUM_ALARM_CLOCK_PRESENT == STD_ON)
/* This is the absolute time since the battery connect */
ECUM_LOCAL VAR(uint32, ECUM_VAR_NO_INIT) EcuM_Clock;

/* This is the time of the next occurring alarm timer */
ECUM_LOCAL VAR(uint32, ECUM_VAR_NO_INIT) EcuM_MasterAlarm;
#endif

#define ECUM_STOP_SEC_VAR_NO_INIT_32
#include "EcuM_MemMap.h" /* PRQA S 5087 */ /* MD_MSR_MemMap */
...
P2CONST(EcuM_PbConfigType, ECUM_VAR_NO_INIT, ECUM_PBCFG) EcuM_ConfigDataPtr; /* PRQA S 1514 */ /* MD_EcuM_1514 */
# endif

# if (ECUM_CONFIGURATION_VARIANT == ECUM_CONFIGURATION_VARIANT_POSTBUILD_LOADABLE)
P2CONST(EcuM_GlobalConfigType, ECUM_VAR_NO_INIT, ECUM_PBCFG) EcuM_GlobalPBConfig_Ptr;
#  if(ECUM_POSTBUILD_VARIANT_SUPPORT == STD_ON)
P2CONST(EcuM_GlobalPCConfigType, ECUM_VAR_NO_INIT, ECUM_CONST) EcuM_GlobalPCConfig_Ptr;
#  endif
# else
P2CONST(EcuM_GlobalConfigType, ECUM_VAR_NO_INIT, ECUM_CONST) EcuM_GlobalPCConfig_Ptr;
# endif
...
#include "EcuM_MemMap.h"      /* PRQA S 5087 */ /* MD_MSR_MemMap */
 ...
 /***********************************************************************************************************************
 *  EcuM_InternalGetShutdownTargetAndMode
 **********************************************************************************************************************/
/*! \brief      Returns EcuM_CurrentShutdownTarget and EcuM_CurrentShutdownMode
 *  \details    Encapsulates EcuM_CurrentShutdownTarget and EcuM_CurrentShutdownMode into an exclusive area
                for concurrent access
 *  \param[out] target Current shutdown target
 *  \param[out] mode Current shutdown mode
 *  \pre        -
 *  \context    TASK
 *  \reentrant  TRUE
 **********************************************************************************************************************/
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalGetShutdownTargetAndMode(EcuM_StateType* target, uint8* mode);

/***********************************************************************************************************************
*  EcuM_InternalSetShutdownTargetAndMode()
**********************************************************************************************************************/
/*! \brief     Sets EcuM_CurrentShutdownTarget and EcuM_CurrentShutdownMode
*  \details    Encapsulates EcuM_CurrentShutdownTarget and EcuM_CurrentShutdownMode into an exclusive area
               for concurrent access
*  \param[in]  target New shutdown target
*  \param[in]  mode New shutdown mode
*  \pre        -
*  \context    TASK
*  \reentrant  TRUE
**********************************************************************************************************************/
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalSetShutdownTargetAndMode(EcuM_StateType target, uint8 mode);
 ...
 *  \reentrant  FALSE
 **********************************************************************************************************************/
ECUM_LOCAL FUNC(void, ECUM_CODE) EcuM_InternalComMNotifications(void);
#endif

#if(ECUM_CHANNELOFCOMMCHANNELREFS == STD_ON)
/***********************************************************************************************************************
 *  EcuM_InternalComMChannelNotification
 **********************************************************************************************************************/
  ...
  *  \reentrant  FALSE
  **********************************************************************************************************************/
ECUM_LOCAL FUNC(void, ECUM_CODE) EcuM_InternalComMChannelNotification(uint8_least sourceIDx);
#endif

#if(ECUM_PNCOFCOMMPNCREFS == STD_ON) /* COV_ECUM_PNCOFCOMMPNCREFS_QM */
/***********************************************************************************************************************
 ...
 **********************************************************************************************************************/
ECUM_LOCAL FUNC(EcuM_SizeOfPartitionDataType, ECUM_CODE) EcuM_InternalGetPartitionIdx(void);
#endif
...
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalGetShutdownTargetAndMode(EcuM_StateType* target, uint8* mode)
{
  if (EcuM_GetModuleState() != ECUM_STATE_GO_OFF_TWO)
  {
    EcuM_EnterCriticalArea_3();
  }

  *target = EcuM_CurrentShutdownTarget; /* SBSW_ECUM_POINTERACCESS_SHUTDOWNTARGET */
  *mode = EcuM_CurrentShutdownMode; /* SBSW_ECUM_POINTERACCESS_SHUTDOWNTARGET */
  
  if (EcuM_GetModuleState() != ECUM_STATE_GO_OFF_TWO)
  {
    EcuM_LeaveCriticalArea_3();
  }
}
...
{
  EcuM_EnterCriticalArea_3();
  EcuM_CurrentShutdownTarget = target;
  EcuM_CurrentShutdownMode = mode;
  EcuM_LeaveCriticalArea_3();
}
...
# endif
  ...
  {
#if((ECUM_NUMBER_OF_CORES > 1u) && (ECUM_RESOURCEOFPARTITIONDATA == STD_ON))
    EcuM_SizeOfPartitionDataType partitionIdx = EcuM_InternalGetPartitionIdx();
#endif

# if(ECUM_FIXED_BEHAVIOR == STD_OFF)
    EcuM_SetModuleState(ECUM_STATE_GO_SLEEP); /* SBSW_ECUM_CSL_VAR_ACCESS */
# endif
...
# if (ECUM_ISDEF_WAKEUPSOURCEOFSLEEPMODELIST == STD_OFF)
    ...
    if ((EcuM_EnabledWakeups & (~((uint32)EcuM_GetWakeupSourceOfSleepModeList(localShutdownMode)))) != 0u)
    {
      EcuM_DisableWakeupSources(EcuM_EnabledWakeups & (~((uint32)EcuM_GetWakeupSourceOfSleepModeList(localShutdownMode))));
    }
# endif
    ...
    EcuM_EnableWakeupSources(EcuM_GetWakeupSourceOfSleepModeList(localShutdownMode) & (~EcuM_EnabledWakeups));
    EcuM_EnabledWakeups = EcuM_GetWakeupSourceOfSleepModeList(localShutdownMode);
    ...
    (void)GetResource(ECUM_SLEEP_RESOURCE(partitionIdx));
...
# if(ECUM_FIXED_BEHAVIOR == STD_OFF)
      ...
      EcuM_SetModuleState(ECUM_STATE_SLEEP); /* SBSW_ECUM_CSL_VAR_ACCESS */
# endif

      /* Store the LastShutdownTarget and LastShutdownMode. */
      EcuM_LastShutdownTarget = localShutdownTarget;
      EcuM_LastShutdownMode = localShutdownMode;
...
# if(ECUM_FIXED_BEHAVIOR == STD_OFF)
    EcuM_SetModuleState(ECUM_STATE_WAKEUP_ONE); /* SBSW_ECUM_CSL_VAR_ACCESS */
# endif
...
# if(ECUM_FIXED_BEHAVIOR == STD_OFF)
      ...
      EcuM_SetModuleState(ECUM_STATE_RUN); /* SBSW_ECUM_CSL_VAR_ACCESS */
# endif
    }
# if(ECUM_VALIDATIONTIMEOFWAKEUPSOURCELIST == STD_ON)
...
#  endif
#  if(ECUM_FIXED_BEHAVIOR == STD_OFF)
      EcuM_SetModuleState(ECUM_STATE_WAKEUP_VALIDATION); /* SBSW_ECUM_CSL_VAR_ACCESS */
# endif
    }
# endif
    ...
    (void)ReleaseResource(ECUM_SLEEP_RESOURCE(partitionIdx));
...
#  if(ECUM_RESOURCEOFPARTITIONDATA == STD_ON)
  EcuM_SizeOfPartitionDataType partitionIdx = EcuM_InternalGetPartitionIdx();
  /* #60 Block the scheduler to avoid interruption of the sleep phase by other tasks. */
  if ((partitionIdx < EcuM_GetSizeOfPartitionData() )
    /* In case of a non-EcuM partition, Resource handling is not existent */
#   if(ECUM_ISDEF_ECUMPARTITIONOFPARTITIONDATA == STD_OFF)
    && (EcuM_IsEcuMPartitionOfPartitionData(partitionIdx))
#   endif
    )
  {
    (void)GetResource(ECUM_SLEEP_RESOURCE(partitionIdx));
...
#if ((ECUM_RESOURCEOFPARTITIONDATA == STD_OFF) && (ECUM_SLAVE_CORE_HANDLING == STD_OFF))
...
ECUM_LOCAL FUNC(void, ECUM_CODE) EcuM_ShutdownSynchronization(EcuM_CoreStatusType ShutdownState)
{
  ...
  do
  {
    ...
    {
      if (EcuM_GetCoreStatusValue(loopCount) != ShutdownState) /* PRQA S 0404 */ /* MD_EcuM_0404 */
      {
...
ECUM_LOCAL FUNC(void, ECUM_CODE) EcuM_InternalComMNotifications(void)
{
  ...
  if (ComMStatus == COMM_INIT)
  {
    ...
    {
      sourceID--;
      wakeupSource = (EcuM_WakeupSourceType)1u << (EcuM_WakeupSourceType)sourceID;

      /* #31 Notify all buffered wakeup sources to ComM. */
      if ((wakeupSource & EcuM_ComM_BufferedWakeups) != 0u) /* COV_ECUM_04 */
      {
# if (ECUM_CHANNELOFCOMMCHANNELREFS == STD_ON) /* COV_ECUM_PNCOFCOMMPNCREFS_QM_DEPENDENCY */
        EcuM_InternalComMChannelNotification(sourceID);
# endif

# if (ECUM_PNCOFCOMMPNCREFS == STD_ON) /* COV_ECUM_PNCOFCOMMPNCREFS_QM */
        EcuM_InternalComMPNCNotification(sourceID);
# endif
        /* Notification to ComM done, remove current wakeup source from the buffer. */
        EcuM_ComM_BufferedWakeups &= ~wakeupSource;
      }
    }
  }
}
...
{
  /* ----- Local Variables ---------------------------------------------- */
  EcuM_ComMChannelRefsIterType idxChRef;

  /* ----- Implementation ----------------------------------------------- */
  /* #50 Notify all Channels which belong to a wakeup source to ComM. */
  for (idxChRef = EcuM_GetComMChannelRefsStartIdxOfWakeupSourceList(sourceIDx); idxChRef < EcuM_GetComMChannelRefsEndIdxOfWakeupSourceList(sourceIDx); idxChRef++)
  {
    ComM_EcuM_WakeUpIndication(EcuM_GetChannelOfComMChannelRefs(idxChRef));
  }
}
#endif
...
{
  /* ----- Local Variables ---------------------------------------------- */
  EcuM_ComMPNCRefsIterType idxPncRef;

  /* ----- Implementation ----------------------------------------------- */
  /* #30 Notify all PNCs which belong to a wakeup source to ComM. */
  for(idxPncRef = EcuM_GetComMPNCRefsStartIdxOfWakeupSourceList(sourceIDx); idxPncRef < EcuM_GetComMPNCRefsEndIdxOfWakeupSourceList(sourceIDx); idxPncRef++)
  {
    ComM_EcuM_PNCWakeUpIndication(EcuM_GetPNCOfComMPNCRefs(idxPncRef));
  }
}
#endif
...
{
  /* ----- Local Variables ---------------------------------------------- */
  EcuM_SizeOfPartitionDataType partitionIndex = EcuM_GetSizeOfPartitionData();
  ApplicationType applicationId = GetCurrentApplicationID();

    /* ----- Development Error Checks ------------------------------------- */
  /* #20 Check if the returned applicationId is a valid one. */
  if (applicationId >= EcuM_GetSizeOfPartitionData())
  {
  ...
  /* ----- Implementation ----------------------------------------------- */
    ...
    for(partitionIndex = 0u; partitionIndex < EcuM_GetSizeOfPartitionData(); partitionIndex++) /* COV_ECUM_PARTITIONID */
    {
      if(applicationId == EcuM_GetApplicationOfPartitionData(partitionIndex))
      {
        break;
      }
    }
  }
  /* #22 Return EcuM_GetSizeOfPartitionData() in case that no partition was found, if the partition was found, return the partitionIndex. */
 ...
 *
 */
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalWakeupValidation(void) /* PRQA S 6030, 6080 */ /* MD_MSR_STCYC, MD_MSR_STMIF */
{
  /* ----- Local Variables ---------------------------------------------- */
...
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalEnterPollingMode(void)
{
  uint8_least loopCount;
  uint8 localShutdownMode;
  EcuM_StateType unused;

  EcuM_InternalGetShutdownTargetAndMode(&unused, &localShutdownMode); /* SBSW_ECUM_POINTER_TO_LOCAL_VARIABLES */

  /* #20 Set the McuMode for the current sleep phase */
  EcuM_McuSetMode(EcuM_GetMcuModeOfSleepModeList(localShutdownMode));
  ...
  while (ECUM_COMBINEDWAKEUPS == ECUM_WKSOURCE_NONE)
  {
    ...
    for (loopCount = 0u; loopCount < EcuM_GetSizeOfWakeupSourceList(); loopCount++)
    {
      ...
      if ((((EcuM_WakeupSourceType)1u << (EcuM_WakeupSourceType)loopCount) &
           (EcuM_GetWakeupSourceOfSleepModeList(localShutdownMode))) != 0u)
      {
...
# endif
  ...
  EcuM_SetModuleState(ECUM_STATE_APP_RUN); /* SBSW_ECUM_CSL_VAR_ACCESS */
...
# endif
  ...
  EcuM_SetTimer((uint16)EcuM_GetSelfRunRequestTimeout()); /* SBSW_ECUM_CSL_VAR_ACCESS */
...
ECUM_LOCAL boolean EcuM_WaitOnNvMWriteAll(void)
{
  ...
  if ( EcuM_GetNvMWriteAllStatus() == ECUM_NVM_WRITEALL_IDLE)
  {
    ...
    EcuM_SetTimer(ECUM_TIMER_NOT_SET); /* SBSW_ECUM_CSL_VAR_ACCESS */
  }
  ...
  else if ( EcuM_GetNvMWriteAllStatus() == ECUM_NVM_WRITEALL_REQUIRED)
  {
    ...
    EcuM_SetNvMWriteAllStatus(ECUM_NVM_WRITEALL_STARTED); /* SBSW_ECUM_CSL_VAR_ACCESS */
    /* #42 Set the internal timer to monitor a WriteAll timeout. */
    EcuM_SetTimer(EcuM_GetNvMWriteAllTimeout()); /* SBSW_ECUM_CSL_VAR_ACCESS */
  ...
  else if ( EcuM_GetTimer() == ECUM_TIMER_EXPIRED)
  {
    ...
    EcuM_SetNvMWriteAllStatus(ECUM_NVM_WRITEALL_IDLE); /* SBSW_ECUM_CSL_VAR_ACCESS */
  }
...
#   endif
  {
    ...
    EcuM_SetNvMWriteAllStatus(ECUM_NVM_WRITEALL_CANCELED); /* SBSW_ECUM_CSL_VAR_ACCESS */

    /* #42 Set the internal timer to monitor a CancelWriteAll timeout. */
#   if(ECUM_NVMCANCELWRITEALLTIMEOUT == STD_ON)
    EcuM_SetTimer(EcuM_GetNvMCancelWriteAllTimeout()); /* SBSW_ECUM_CSL_VAR_ACCESS */
    retVal = TRUE;
#   else
    EcuM_SetTimer(ECUM_TIMER_NOT_SET); /* SBSW_ECUM_CSL_VAR_ACCESS */
#   endif
...
#   if(ECUM_NVMCANCELWRITEALLTIMEOUT == STD_ON)
  else if ( EcuM_GetTimer() == ECUM_TIMER_EXPIRED)
  {
    ...
    EcuM_SetNvMWriteAllStatus(ECUM_NVM_WRITEALL_REQUIRED); /* SBSW_ECUM_CSL_VAR_ACCESS */
  }
 ...
 *
 */
ECUM_LOCAL FUNC(void, ECUM_CODE) EcuM_Statemachine(void) /* PRQA S 6030 */ /* MD_MSR_STCYC */
{
# if(ECUM_COMM_COMALLOWEDLIST == STD_ON)
  uint8_least loopCount;
# endif
...
# endif
      ...
      /* Transit to PostRun */
      EcuM_SetModuleState(ECUM_STATE_APP_POST_RUN); /* SBSW_ECUM_CSL_VAR_ACCESS */
...
# endif
        ...
        EcuM_SetModuleState(ECUM_STATE_PREP_SHUTDOWN); /* SBSW_ECUM_CSL_VAR_ACCESS */
...
# if (ECUM_SUPPORT_DEM == STD_ON)
    Dem_Shutdown();
    EcuM_SetDemStatus(ECUM_DEM_SHUT_DOWN); /* SBSW_ECUM_CSL_VAR_ACCESS */
# endif
# if (ECUM_SLEEPMODELIST == STD_ON)
...
# endif
    {
      /* Transit to Go Off */
      EcuM_SetModuleState(ECUM_STATE_GO_OFF_ONE); /* SBSW_ECUM_CSL_VAR_ACCESS */
      EcuM_OnGoOffOne();
...
#  endif
    ...
    EcuM_SetKillAllInProgress(0u); /* SBSW_ECUM_CSL_VAR_ACCESS */
...
# endif
    {
      ...
      EcuM_SetModuleState(ECUM_STATE_WAKEUP_REACTION); /* SBSW_ECUM_CSL_VAR_ACCESS */
...
# if (ECUM_SUPPORT_DEM == STD_ON)
        ...
        EcuM_SetDemStatus(ECUM_DEM_INITIALIZED); /* SBSW_ECUM_CSL_VAR_ACCESS */
# endif
      ...
      else
      {
        ...
        EcuM_SetModuleState(ECUM_STATE_WAKEUP_WAKESLEEP); /* SBSW_ECUM_CSL_VAR_ACCESS */
...
# endif
      {
        ...
        EcuM_SetModuleState(ECUM_STATE_ERROR); /* SBSW_ECUM_CSL_VAR_ACCESS */
      }
    }
    break;
...
ECUM_LOCAL FUNC(void, ECUM_CODE) EcuM_InternalKillAllRequests(uint8 mask)
{
  ...
  EcuM_KillAllInProgressType maskedKillAllInProgress = 0u;

  /* ----- Implementation ----------------------------------------------- */
  /* #30 Set the internal flag-variable to indicate that KillAll is in progress. */
  maskedKillAllInProgress = (EcuM_KillAllInProgressType)(EcuM_GetKillAllInProgress() | mask);
  EcuM_SetKillAllInProgress(maskedKillAllInProgress); /* SBSW_ECUM_CSL_VAR_ACCESS */
  ...
  {
    EcuM_SetRunRequestCounter(0u); /* SBSW_ECUM_CSL_VAR_ACCESS */
    ...
    if ( EcuM_GetModuleState() == ECUM_STATE_APP_RUN)
    {
      ...
      EcuM_SetTimer((uint16)ECUM_TIMER_EXPIRED); /* SBSW_ECUM_CSL_VAR_ACCESS */
    }
...
# endif
  ...
  {
    EcuM_SetPostRunRequestCounter(0u); /* SBSW_ECUM_CSL_VAR_ACCESS */
  }
 ...
 *
 */
ECUM_LOCAL FUNC(Std_ReturnType, ECUM_CODE) EcuM_InternalRequestRUN(uint8 ApiId, EcuM_UserType User, uint8 mask) /* PRQA S 6080 */ /* MD_MSR_STMIF */
{
  /* ----- Local Variables ---------------------------------------------- */
...
#  endif
        ...
        EcuM_IncRunRequestCounter(); /* SBSW_ECUM_CSL_VAR_ACCESS */
      }
      else
      {
#  if(ECUM_MODE_HANDLING == STD_ON)
...
# endif
        ...
        EcuM_IncPostRunRequestCounter(); /* SBSW_ECUM_CSL_VAR_ACCESS */
      }
      retval = E_OK;
 ...
 *
 */
ECUM_LOCAL FUNC(Std_ReturnType, ECUM_CODE) EcuM_InternalReleaseRUN(uint8 ApiId, EcuM_UserType User, uint8 mask) /* PRQA S 6080 */ /* MD_MSR_STMIF */
{
  /* ----- Local Variables ---------------------------------------------- */
  Std_ReturnType retval = E_NOT_OK;
  uint8 errorId = ECUM_E_INVALID_PAR; /* PRQA S 2981 */ /* MD_MSR_RetVal */
...
#  endif
        EcuM_DecRunRequestCounter(); /* SBSW_ECUM_CSL_VAR_ACCESS */
      }
      else
      {
#  if(ECUM_MODE_HANDLING == STD_ON)
...
#  endif
        EcuM_DecPostRunRequestCounter(); /* SBSW_ECUM_CSL_VAR_ACCESS */
      }
      retval = E_OK;
      errorId = ECUM_E_NO_ERROR;
    }
...
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalRteFeedbackCheck(void)
{
  ...
  if (Rte_SwitchAck_EcuM_currentMode_currentMode() != RTE_E_NO_DATA)
  {
    EcuM_SetBswMCurrentModuleState(EcuM_GetBswMRequestedModuleState()); /* SBSW_ECUM_CSL_VAR_ACCESS */
...
# endif
  {
    ...
    if (errorId == ECUM_E_NO_ERROR)
    {
      ...
      EcuM_SetBswMRequestedModuleState(state); /* SBSW_ECUM_CSL_VAR_ACCESS */
    }
  }
...
# if(ECUM_ECUMPARTITIONOFPARTITIONDATA == STD_ON)
#  if(ECUM_ISDEF_ECUMPARTITIONOFPARTITIONDATA == STD_OFF)
  EcuM_SizeOfPartitionDataType partitionIdx = EcuM_InternalGetPartitionIdx();
#  endif
# endif
...
# if(ECUM_ECUMPARTITIONOFPARTITIONDATA == STD_ON)
#  if(ECUM_ISDEF_ECUMPARTITIONOFPARTITIONDATA == STD_OFF)
      && (partitionIdx < EcuM_GetSizeOfPartitionData())
      && (EcuM_IsEcuMPartitionOfPartitionData(partitionIdx))
#  endif
# endif
      )
...
# endif /* (ECUM_FIXED_BEHAVIOR != STD_ON) */
      ...
      {
        EcuM_DecTimer(); /* SBSW_ECUM_CSL_VAR_ACCESS */
      }
 ...
 *
 */
FUNC(void, ECUM_CODE) EcuM_Init(void) /* PRQA S 6010, 6030, 6080 */ /* MD_MSR_STPTH, MD_MSR_STCYC, MD_MSR_STMIF */
{
  /* ----- Local Variables ---------------------------------------------- */
  uint8 errorId = ECUM_E_NO_ERROR;
  EcuM_WakeupSourceType wakeupSource = ECUM_WKSOURCE_RESET;
...
# endif
      ...
      EcuM_SetBswMCurrentModuleState(ECUM_STATE_SHUTDOWN); /* SBSW_ECUM_CSL_VAR_ACCESS */
      /* Initialize the Requested state with startup to begin RTE synchronization directly after initialization. */
      EcuM_SetBswMRequestedModuleState(ECUM_STATE_STARTUP); /* SBSW_ECUM_CSL_VAR_ACCESS */
      ...
      ECUM_DRIVERINITLIST_ONE(); /* SBSW_ECUM_FUNCTIONPOINTER_CALL */ /* PRQA S 2812 */ /* MD_EcuM_2812 */
      ...
      /* Try to detect a specific wakeup source. 
       * If the hardware can not detect a specific wakeup source, ECUM_WKSOURCE_RESET will be reported as default source then. */
      ...
      /* #50 Map the Mcu reset reason from the last occurred shutdown. */
      {
        ...
        for (loopCount = 0u; loopCount < EcuM_GetSizeOfWakeupSourceList(); loopCount++)
        {
          ...
          {
            /* Assign the current wakeup source */
            wakeupSource = (((EcuM_WakeupSourceType)1u << (EcuM_WakeupSourceType)loopCount));
            break;
          }
        }
      }
      ...
      EcuM_ValidatedWakeups |= wakeupSource;

      /* Store this source in the BufferedWakeups to validate it at the end of EcuM_StartupTwo */
      EcuM_BswM_BufferedWakeups |= wakeupSource;

#if((ECUM_CHANNELOFCOMMCHANNELREFS == STD_ON) || (ECUM_PNCOFCOMMPNCREFS == STD_ON)) /* COV_ECUM_PNCOFCOMMPNCREFS_QM */
      {
        uint8 sourceID = EcuM_GetShiftCount(wakeupSource);
# if(ECUM_CHANNELOFCOMMCHANNELREFS == STD_ON) /* COV_ECUM_PNCOFCOMMPNCREFS_QM_DEPENDENCY */
        if (EcuM_GetComMChannelRefsStartIdxOfWakeupSourceList(sourceID) != ECUM_NO_COMMCHANNELREFSSTARTIDXOFWAKEUPSOURCELIST) /* COV_ECUM_06 */
        {
          EcuM_ComM_BufferedWakeups |= wakeupSource;
        }
# endif
# if(ECUM_PNCOFCOMMPNCREFS == STD_ON) /* COV_ECUM_PNCOFCOMMPNCREFS_QM */
        if (EcuM_GetComMPNCRefsStartIdxOfWakeupSourceList(sourceID) != ECUM_NO_COMMPNCREFSSTARTIDXOFWAKEUPSOURCELIST)
        {
          EcuM_ComM_BufferedWakeups |= wakeupSource;
        }
# endif
      ...
      /* Select the shutdown target to the configured default shutdowntarget
       ...
       Direct access to shutdown target and mode has to be used instead of EcuM_InternalSetShutdownTargetAndMode,
       because the OS is not initialized yet so the exclusive areas will not yet function. */
...
FUNC(void, ECUM_CODE) EcuM_Shutdown(void)
{
...
#endif
  {
    ...
    EcuM_SetModuleState(ECUM_STATE_GO_OFF_TWO); /* SBSW_ECUM_CSL_VAR_ACCESS */
...
P2VAR(EcuM_ModeType, AUTOMATIC, ECUM_APPL_DATA) resetSleepMode)
{
  ...
  else
  {
    ...
    /* ----- Implementation ----------------------------------------------- */
    EcuM_InternalGetShutdownTargetAndMode(target, resetSleepMode); /* SBSW_ECUM_POINTER_TO_LOCAL_VARIABLES */
    retval = E_OK;
  }
...
#if (((ECUM_PARTITIONDATA == STD_ON)) || (ECUM_NUMBER_OF_CORES > 1u))
# if ((ECUM_BSWMPARTITIONOFPARTITIONDATA == STD_ON) || (ECUM_ECUMPARTITIONOFPARTITIONDATA == STD_ON))
  EcuM_SizeOfPartitionDataType partitionIdx = EcuM_InternalGetPartitionIdx();
# endif
...
#if(ECUM_SCHM_START_INITIALIZATION == STD_ON)
...
#if(ECUM_FIXED_BEHAVIOR == STD_ON)
      EcuM_SetModuleState(ECUM_STATE_STARTUP_TWO); /* SBSW_ECUM_CSL_VAR_ACCESS */
...
# endif
      ...
      /* #50 Check if there are wakeups stored in the EcuM_ComM_BufferedWakeups variable and if so, notify them to ComM. */
#if((ECUM_CHANNELOFCOMMCHANNELREFS == STD_ON) || (ECUM_PNCOFCOMMPNCREFS == STD_ON)) /* COV_ECUM_PNCOFCOMMPNCREFS_QM */
      if (EcuM_ComM_BufferedWakeups != 0u)
      {
        EcuM_InternalComMNotifications();
...
#if ((ECUM_BSWMPARTITIONOFPARTITIONDATA == STD_ON) || (ECUM_ECUMPARTITIONOFPARTITIONDATA == STD_ON) || (ECUM_NUMBER_OF_CORES > 1))
  else
  {
# if ((ECUM_BSWMPARTITIONOFPARTITIONDATA == STD_ON) || (ECUM_ECUMPARTITIONOFPARTITIONDATA == STD_ON))
    if(partitionIdx < EcuM_GetSizeOfPartitionData())
...
#  if(ECUM_ISDEF_ECUMPARTITIONOFPARTITIONDATA == STD_OFF)
      if(EcuM_IsEcuMPartitionOfPartitionData(partitionIdx))
#  endif
      {
        /* #32 Initialization of the schedule Manager. */
#  if(ECUM_SCHM_START_INITIALIZATION == STD_ON)
...
#  if(ECUM_BSWMPARTITIONOFPARTITIONDATA == STD_ON)
#   if(ECUM_ISDEF_BSWMPARTITIONOFPARTITIONDATA == STD_OFF)
      if(EcuM_IsBswMPartitionOfPartitionData(partitionIdx))
#   endif
      {
        /* #33 Initialization of the BswM. */
        BswM_Init( ECUM_BSWM_CONFIG_POINTER); /* SBSW_ECUM_FUNCTIONCALL_POINTER */
...
# if (ECUM_CHANNELOFCOMMCHANNELREFS == STD_ON)
        || (EcuM_GetComMChannelRefsStartIdxOfWakeupSourceList(Shiftcount) != ECUM_NO_COMMCHANNELREFSSTARTIDXOFWAKEUPSOURCELIST)
# endif

# if (ECUM_PNCOFCOMMPNCREFS == STD_ON) /* COV_ECUM_PNCOFCOMMPNCREFS_QM */
        || (EcuM_GetComMPNCRefsStartIdxOfWakeupSourceList(Shiftcount) != ECUM_NO_COMMPNCREFSSTARTIDXOFWAKEUPSOURCELIST)
# endif
    )
#endif
    {
...
#if ((ECUM_CHANNELOFCOMMCHANNELREFS == STD_ON) || (ECUM_PNCOFCOMMPNCREFS == STD_ON)) /* COV_ECUM_PNCOFCOMMPNCREFS_QM */
# if(((ECUM_CHANNELOFCOMMCHANNELREFS == STD_ON) && (ECUM_PNCOFCOMMPNCREFS == STD_ON))) /* COV_ECUM_PNCOFCOMMPNCREFS_QM */
      if ((EcuM_GetComMChannelRefsStartIdxOfWakeupSourceList(Shiftcount) != ECUM_NO_COMMCHANNELREFSSTARTIDXOFWAKEUPSOURCELIST)
          || (EcuM_GetComMPNCRefsStartIdxOfWakeupSourceList(Shiftcount) != ECUM_NO_COMMPNCREFSSTARTIDXOFWAKEUPSOURCELIST))
# elif (ECUM_CHANNELOFCOMMCHANNELREFS == STD_ON) /* COV_ECUM_PNCOFCOMMPNCREFS_QM_DEPENDENCY */
      if (EcuM_GetComMChannelRefsStartIdxOfWakeupSourceList(Shiftcount) != ECUM_NO_COMMCHANNELREFSSTARTIDXOFWAKEUPSOURCELIST)
# else
      if (EcuM_GetComMPNCRefsStartIdxOfWakeupSourceList(Shiftcount) != ECUM_NO_COMMPNCREFSSTARTIDXOFWAKEUPSOURCELIST)
# endif
      {
        ...
        {
# if (ECUM_CHANNELOFCOMMCHANNELREFS == STD_ON) /* COV_ECUM_PNCOFCOMMPNCREFS_QM_DEPENDENCY */
            EcuM_InternalComMChannelNotification(Shiftcount);
# endif
# if(ECUM_PNCOFCOMMPNCREFS == STD_ON) /* COV_ECUM_PNCOFCOMMPNCREFS_QM */
...
# if(ECUM_SLEEPMODELIST == STD_ON) && (ECUM_ECUMPARTITIONOFPARTITIONDATA == STD_ON)
#  if(ECUM_ISDEF_ECUMPARTITIONOFPARTITIONDATA == STD_OFF)
  EcuM_SizeOfPartitionDataType partitionIdx = EcuM_InternalGetPartitionIdx();
#  endif
# endif
...
# if(ECUM_SLEEPMODELIST == STD_ON) && (ECUM_POLLINGOFSLEEPMODELIST == STD_ON) && (ECUM_ECUMPARTITIONOFPARTITIONDATA == STD_ON)
#  if(ECUM_ISDEF_ECUMPARTITIONOFPARTITIONDATA == STD_OFF)
  EcuM_SizeOfPartitionDataType partitionIdx = EcuM_InternalGetPartitionIdx();
#  endif
# endif
...
# endif /* ECUM_GODOWNALLOWEDOFFLEXUSER == STD_ON) */
...
#endif /* (ECUM_FIXED_BEHAVIOR != STD_ON */
      {
        ...
        /* 31 Synchronize different cores with EcuM instances in case of Multicore. */
#if(ECUM_SLAVE_CORE_HANDLING == STD_ON)
        ...
        /* #32 Deinitialize the BswM and the SchM. */
        BswM_Deinit();
        SchM_Deinit();
        ...
        if (EcuM_CurrentShutdownTarget == ECUM_STATE_OFF)
        {
          ...
          if (ECUM_COMBINEDWAKEUPS != ECUM_WKSOURCE_NONE)
          {
            ...
            EcuM_InternalSetShutdownTargetAndMode(ECUM_STATE_RESET, EcuMConf_EcuMResetMode_ECUM_RESET_WAKEUP);
#else
            /* EcuMConf_EcuMResetMode_ECUM_RESET_WAKEUP is not configured. Set EcuM_CurrentShutdownMode to a not used mode.*/
            EcuM_InternalSetShutdownTargetAndMode(ECUM_STATE_RESET, 0xFFu);
#endif
          }
  ...
  {
    /* #40 Check if the current partition is configured for EcuM or BswM. */
    if(partitionIdx < EcuM_GetSizeOfPartitionData())
    {
#  if(ECUM_ISDEF_ECUMPARTITIONOFPARTITIONDATA == STD_OFF)
      if(EcuM_IsEcuMPartitionOfPartitionData(partitionIdx))
...
*
     MD_EcuM_0404:
     Reason:     Comparision of two volatile variables.
     Risk:       No risk, only one can actually change.
     ...
     MD_EcuM_2812:
     Reason:     EcuM_ConfigDataPtr is initiaized in a non trivial way.
...
\ID SBSW_ECUM_POINTER_TO_LOCAL_VARIABLES
 \DESCRIPTION The function is called with pointers to local variables as output data.
 \COUNTERMEASURE \N The caller has to guarantee that the pointer is valid.
...
\ID COV_ECUM_06
  \ACCEPT  XF
  \REASON  No wakeup source which is relevant (configured reset reason or ECUM_WKSOURCE_RESET used as fallback) for reset reason translation has a
           reference(s) to a ComM channel while other sources have a reference(s).
...
\ID COV_ECUM_PARTITIONID
  \ACCEPT  TX
  \REASON  The condition in the for loop is never met since there always is a matching entry to a valid application ID, which triggers the break.
           Invalid application IDs are not handled in this loop.

\ID COV_ECUM_PNCOFCOMMPNCREFS_QM
  \ACCEPT  TF tf xf tf
  \ACCEPT  XF tx xf
  \ACCEPT  TF tf xf
  \REASON  The feature switch ECUM_PNCOFCOMMPNCREFS is only valid for QM configurations.

\ID COV_ECUM_PNCOFCOMMPNCREFS_QM_DEPENDENCY
  \ACCEPT  TX
  \REASON  Due to the QM feature ECUM_PNCOFCOMMPNCREFS this switch cannot be covered completely.
...
\ID FETA_EcuM_001
  \DESCRIPTION      EcuM should only recover the ECU from sleep in case of a valid wake-up events. In case of unwanted 
                    events e.g. EMC spikes, the ECU should return to sleep mode as fast as possible.
  \COUNTERMEASURE   \N Loop is terminating in case of valid wake-up event.

\ID FETA_EcuM_002
  \DESCRIPTION      According to ASR, the slave instances should wait after wake-up from sleep until the master core instance
                    has finished the wake-up and is running again.
  \COUNTERMEASURE   \N Loop is terminating if master instance has finished the wake-up handling.

\ID FETA_EcuM_003
  \DESCRIPTION      According to ASR, the master instances should wait prior the transition to sleep until all slave core 
                    instances have finished their transition to sleep.
  \COUNTERMEASURE   \N Loop is terminating if master instance has finished the wake-up handling.

\ID FETA_EcuM_004
  \DESCRIPTION      At the end of the start-up procedure buffered wake-up events needs to be indicated to ComM. Therefore a iteration
                    over all wake-up sources is necessary.
  \COUNTERMEASURE   \N Loop is terminating at the latest after a constant time, depending of the size of configured wake-up sources.
