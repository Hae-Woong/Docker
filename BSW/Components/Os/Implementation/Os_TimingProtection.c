...
/***********************************************************************************************************************
 *  Os_TpAddMaxAllowedValue()
 **********************************************************************************************************************/
/*! \brief        Calculate the sum of Value and the MaxAllowedValue of Config->Timer.
 *  \details      --no details--
 *
 ...
 *
 *  \return       The sum of Value and the MaxAllowedValue.
 *
 *  \context      OS_INTERNAL
 *
 *  \reentrant    TRUE
 ...
 *  \pre          -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE Os_TickType, OS_CODE, OS_ALWAYS_INLINE, Os_TpAddMaxAllowedValue,
(
 ...
 **********************************************************************************************************************/
FUNC(void, OS_CODE) Os_TpThreadInitInternal                                                                             /* PRQA S 3449, 3451 */ /* MD_Os_Rule8.5_3449, MD_Os_Rule8.5_3451 */
(
  P2CONST(Os_TpConfigType, AUTOMATIC, OS_CONST) Config,
 ...
 *  \pre          The timing protection of the owner core is initialized.
 **********************************************************************************************************************/
FUNC(void, OS_CODE) Os_TpThreadKillInternal                                                                             /* PRQA S 3449, 3451 */ /* MD_Os_Rule8.5_3449, MD_Os_Rule8.5_3451 */
(
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE Os_TickType, OS_CODE, OS_ALWAYS_INLINE, Os_TpAddMaxAllowedValue,           /* COV_OS_HALPLATFORMTPUNSUPPORTED_XX */
(
  P2CONST(Os_TpConfigType, AUTOMATIC, OS_CONST) Config,
  Os_TickType Value
))                                                                                                                      /* COV_OS_HALPLATFORMTPUNSUPPORTED_XX */
{
  ...
  return Os_TpAdd(Config, Value, maxAllowedValue);                                                                      /* SBSW_OS_FC_PRECONDITION */
}
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_TpThreadReInitInternal,                /* COV_OS_HALPLATFORMTPUNSUPPORTED_XX */
(
  P2CONST(Os_TpOwnerThreadConfigType, AUTOMATIC, OS_CONST) Owner,
  P2VAR(  Os_TpOwnerThreadType,       AUTOMATIC, OS_CONST) Dyn
))                                                                                                                      /* COV_OS_HALPLATFORMTPUNSUPPORTED_XX */
...
)
{
  ...
  if(Os_TpIsEnabledByCoreTpConfig(Config) != 0u)                                                                        /* PRQA S 2992, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2996 */ /* SBSW_OS_FC_PRECONDITION */ /* COV_OS_HALPLATFORMTPUNSUPPORTED_XF */
  {
    ...
    /* #30 Initialize data to 0 so that arrival is allowed from now on. */
    now = (Os_TickType) 0u;
    ...
    /* #40 Initialize compare value in the timer into far future (changes with initialization of threads TP). */
    compareValue = Os_TpAddMaxAllowedValue(Config, now);                                                                /* SBSW_OS_FC_PRECONDITION_AND_CHECK_IF_NEEDED */
    Os_TimerHrtInitCompareValue(Config->Timer, compareValue);                                                           /* SBSW_OS_TP_TIMERHRTINITCOMPAREVALUE_001 */
  }
}
    ...
    if(current->Dyn->MonitoredBudgets != NULL_PTR)
    {
      ...
      if(OS_UNLIKELY(Os_CounterIsFutureValue(counter, deadline, now) == 0u))                                            /* SBSW_OS_TP_COUNTERISFUTUREVALUE_001 */
      {
        ...
        if(current->Dyn->DelayLevel > 0u)
        {
          ...
          /* #105 Set compare value into far future to avoid frequent activation of TP ISR. */
          compareValue = Os_TpAddMaxAllowedValue(config, deadline);                                                     /* SBSW_OS_TP_TPADDMAXALLOWEDVALUE_001 */
...
\ID SBSW_OS_TP_TIMERHRTINITCOMPAREVALUE_001
 \DESCRIPTION    Os_TimerHrtInitCompareValue is called with a Timer derived from a Config object. The Config object is
                 passed as argument to the caller function. Precondition ensures that the pointer is valid.
                 The compliance of the precondition is checked during review.
 \COUNTERMEASURE \M [CM_OS_TP_TIMER_M]
...
\ID SBSW_OS_TP_TPADDMAXALLOWEDVALUE_001
 \DESCRIPTION    Os_TpAddMaxAllowedValue is called with the return value of Os_CoreAsrGetTimingProtection.
