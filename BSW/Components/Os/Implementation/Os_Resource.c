...
{
   return (Os_StdReturnType)((uint16)OS_RESOURCEID_COUNT > 0u);                                                         /* PRQA S 4304, 2995 */ /* MD_Os_C90BooleanCompatibility, MD_Os_Rule2.2_2995 */
}
...
))
{
  ...
  /* #05 If the previous priority is lower than the current priority of the task: */
  if(Os_SchedulerPriorityIsHigher(currentPrio, previousPrio) != 0u)
  {
    ...
    localScheduler = Os_TaskGetScheduler(Task);                                                                           /* SBSW_OS_FC_PRECONDITION */

    /* #10 If the given task is the currently running task: */
    if(Os_SchedulerGetCurrentTask(localScheduler) == Task)                                                              /* SBSW_OS_RES_SCHEDULERGETCURRENTTASK_001 */
    {
      /* #20 Restore the previous priority. */
      Os_SchedulerDecreasePrio(localScheduler, previousPrio);                                                           /* SBSW_OS_RES_SCHEDULERDECREASEPRIO_001 */ /* SBSW_OS_FC_PRECONDITION */
    }
    /* #30 Otherwise: */
    else
    {
      /* #40 Remove the entry of the given task from the task queue on the current priority of this task. */
      Os_SchedulerDeleteTask(localScheduler, currentPrio, Task);                                                        /* SBSW_OS_RES_SCHEDULERDELETETASK_001 */ /* SBSW_OS_FC_PRECONDITION */

      /* #50 Restore the previous priority of the task. */
      Os_TaskSetPriority(Task, previousPrio);                                                                           /* SBSW_OS_FC_PRECONDITION */
    }
  }
}
...
/***********************************************************************************************************************
 *  Os_ResourceForceReleaseCleanup()
 **********************************************************************************************************************/
/*!
 * Internal comment removed.
 *
 ...
 */
/* PRQA S 6050 1 */ /* MD_MSR_STCAL */
FUNC(void, OS_CODE) Os_ResourceForceReleaseCleanup                                                                         /* PRQA S 3673 */ /* MD_MSR_Rule8.13 */
(
  P2CONST(Os_LockConfigType, AUTOMATIC, OS_CONST) Lock,
...
)
{
  ...
  Os_Assert((Os_StdReturnType)(Os_LockIsResource(Lock) != 0u));                                                         /* PRQA S 4304 */ /* MD_Os_C90BooleanCompatibility */ /* SBSW_OS_FC_PRECONDITION */

  localCore = Os_ThreadGetCore(Thread);                                                                                 /* SBSW_OS_FC_PRECONDITION */
  resource = Os_ResourceLock2Resource(Lock);                                                                            /* SBSW_OS_FC_PRECONDITION */
  ...
  /* #30 Stop blocking time measurement. */
...
)
{
  ...
  else
  {
    ...
    if(OS_UNLIKELY(Os_CoreAsrCheckIsCoreLocal(me->Core) == OS_CHECK_FAILED))                                            /* SBSW_OS_RES_COREASRCHECKISCORELOCAL_001 */ /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */ /* PRQA S 2992, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2996 */
    {
    ...
    else
    {
      ...
      {
        P2CONST(Os_ResourceInterruptConfigType, TYPEDEF, OS_CONST) intResource;
        P2VAR(Os_ResourceInterruptType, AUTOMATIC, OS_VAR_NOINIT) resourceDyn;
        Os_IntStateType intStateResource = { 0, 0 };

        intResource = Os_Resource2ResourceInterrupt(me);                                                                /* SBSW_OS_RES_RESOURCE2RESOURCEINTERRUPT_001 */
        resourceDyn = Os_ResourceInterruptGetDyn(intResource);                                                          /* SBSW_OS_RES_RESOURCEINTERRUPTGETDYN_002 */

        /* #30 Read back and update the interrupt state according to the resources interrupt level. */
        Os_InterruptSaveAndIncrease(&intStateResource, intResource->InterruptLevel);                                    /* SBSW_OS_RES_INTERRUPTSAVEANDINCREASE_001 */

        /* #40 Enter critical section (core local). */
        Os_IntSuspend(&intState);                                                                                       /* SBSW_OS_FC_POINTER2LOCAL */

        /* #50 Store the previous interrupt state inside the critical section to prevent race conditions. */
        Os_InterruptStateCopy(&intStateResource, &(resourceDyn->PreviousInterruptState));                               /* SBSW_OS_RES_RESOURCEINTERRUPTGETDYN_001 */
      }
      /* #60 Otherwise */
      else
      {
        /* #70 Enter critical section (core local). */
        Os_IntSuspend(&intState);                                                                                       /* SBSW_OS_FC_POINTER2LOCAL */
      }

      /* #80 Start blocking time measurement. */
      Os_TpStartResource(configTp, ResID);                                                                              /* SBSW_OS_RES_TPSTARTRESOURCE_001 */

      /* #90 Insert resource to thread's list of occupied locks. */
      Os_LockListPush(Os_ThreadGetLocks(currentThread), &(me->Lock));                                                   /* SBSW_OS_RES_LOCKLISTPUSH_001 */ /* SBSW_OS_RES_THREADGETLOCKS_001 */

      /* #95 Inform the trace module */
      Os_TraceResourceTaken(ResID);

      /* #100 If the current thread is a task: */
      if(Os_ThreadIsTask(currentThread) != 0u)                                                                          /* SBSW_OS_RES_THREADISTASK_001 */
      {
        /* #110 Increase scheduling priority. */
      ...
      /* #120 Leave critical section (core local). */
...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      Os_StdReturnType isIntResource;
      P2CONST(Os_TpConfigType, TYPEDEF, OS_CONST) configTp;
      P2CONST(Os_TpThreadConfigType, TYPEDEF, OS_CONST) configTpThread;
      P2CONST(Os_CoreAsrConfigType, AUTOMATIC, OS_CONST) localCore;

      isIntResource = Os_ResourceIsIntResource(me);                                                                     /* SBSW_OS_RES_RESOURCEISINTRESOURCE_001 */
      localCore = Os_ThreadGetCore(currentThread);                                                                      /* SBSW_OS_RES_THREADGETCORE_001 */
      configTp = Os_CoreAsrGetTimingProtection(localCore);                                                              /* SBSW_OS_RES_COREASRGETTIMINGPROTECTION_001 */
      ...
      if(Os_ThreadIsTask(currentThread) != 0u)                                                                          /* SBSW_OS_RES_THREADISTASK_001 */
      {
        ...
        /*  #85 Perform round robin scheduling (if needed). */
        Os_SchedulerRobRound(scheduler);                                                                                /* SBSW_OS_RES_SCHEDULER_ROBROUND_001 */
      ...
      if(isIntResource != 0u)
      {
        ...
        Os_InterruptRestore(&previousInterruptState);                                                                   /* SBSW_OS_FC_POINTER2LOCAL */
      }
...
\ID SBSW_OS_RES_SCHEDULERDELETETASK_001
 \DESCRIPTION    Os_SchedulerDeleteTask is called with the return value of Os_TaskGetScheduler.
 \COUNTERMEASURE \M [CM_OS_TASKGETSCHEDULER_M]
...
\ID SBSW_OS_RES_INTERRUPTSAVEANDINCREASE_001
 \DESCRIPTION    Os_InterruptSaveAndIncrease is called with the return value of Os_Resource2ResourceInterrupt.
 \COUNTERMEASURE \R [CM_RESOURCE2RESOURCEINTERRUPT_R]
...
\ID SBSW_OS_RES_SCHEDULER_ROBROUND_001
...
\ID SBSW_OS_RES_SCHEDULERGETCURRENTTASK_001
