...
#   include "Os_Hal_Compiler_DiabInt.h"
...
/* module specific MISRA deviations:
  ...
  MD_Os_Hal_Rule2.2_2985:
      ...
      Prevention: The code contains macros with neutral values, e.g. 0 is added or subtracted. There may be cases where
                  the value has an effect, or is not zero (derivative dependent). Component logic is tested carefully 
  ...
  MD_Os_Hal_Rule8.13_3679:
      Reason:     The type of the variable is defined with a typedef. This way the type of all variables with
                  that type can be changed easily leading to more maintainable code. However, it is not
                  possible to change the pointer to a const type without stopping to use the typedef and thus
                  loosing its advantage. For this reason the pointer is not changed to a constant type.
      Risk:       As the type of the pointer is not constant, unintentional modification of the
                  referenced data is possible.
 ...
 MD_Os_Hal_Rule11.6_0326:
  ...
  MD_Os_Hal_Dir1.1_0286:
      Reason:     The string literal describes assembly code. The non-standard character is part of the
                  syntax and will be recognized correctly.
      Risk:       None.
      Prevention: None.
 */
