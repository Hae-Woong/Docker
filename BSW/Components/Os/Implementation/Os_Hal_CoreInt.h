...
struct Os_Hal_CoreConfigType_Tag
{
  ...
  /*! The global address of the SYSCON or the CORECON register on this core */
  uint32 SYSCON_CORECON_Register;

  /*! The global address of the BOOTCON register on this core */
  uint32 BOOTCONRegister;
};
...
{
  uint32 CoreId;
  uint32 AddressOfExceptionTable;
  uint32 AddressOfInterruptTable;
};
...
/***********************************************************************************************************************
 *  Os_Hal_LoadAcquire16()
 **********************************************************************************************************************/
/*! \brief          Perform a load-acquire operation on a 16-bit value.
 *  \details        A load acquire perfoms an atomic load operation with at least the following memory fences:
 *
 *                  Move  | back to front | front to back | of load-acquire
 *                  ------+---------------+---------------+----------------
 *                  Load  | Disallowed    | Allowed       |
 *                  Store | Disallowed    | Allowed       |
 * 
 *                  The implementation can use stricter fences depending on plattform support.
 * 
 *  \param[in]      Address The address of the 16-bit variable.
 *
 *  \return         The value of the 16-bit variable.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE uint16, OS_CODE, OS_ALWAYS_INLINE,
Os_Hal_LoadAcquire16,
(
  P2CONST(uint16 volatile, AUTOMATIC, AUTOMATIC) Address
));


/***********************************************************************************************************************
 *  Os_Hal_StoreRelease16()
 **********************************************************************************************************************/
/*! \brief          Perform a store-release operation on a 16-bit value.
 *  \details        A load acquire perfoms an atomic store operation with at least the following memory fences:
 *
 *                  Move  | back to front | front to back | of store-release
 *                  ------+---------------+---------------+-----------------
 *                  Load  | Allowed       | Disallowed    |
 *                  Store | Allowed       | Disallowed    |
 * 
 *                  The implementation can use stricter fences depending on plattform support.
 * 
 *  \param[out]     Address  The address of the 16-bit variable.
 *  \param[in]      Value    The new value for the 16-bit variable.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
...
Os_Hal_StoreRelease16,
(
  P2VAR(uint16 volatile, AUTOMATIC, AUTOMATIC) Address,
  uint16 Value
));
