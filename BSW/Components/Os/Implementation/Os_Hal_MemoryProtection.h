...
))
{
  ...
  /* #90 Activate the core MPU unit and set U1IED bit in SYSCON. */
  Os_Hal_Mtcr(OS_HAL_SYSCON_CORECON_OFFSET, Os_Hal_Mfcr(OS_HAL_SYSCON_CORECON_OFFSET) | OS_HAL_SYSCON_CORECON_U1IED_MASK | OS_HAL_SYSCON_CORECON_MP_MASK);
...
Os_Hal_MpAppSwitch,
(
  P2CONST(Os_Hal_MpAppConfigType, TYPEDEF, OS_CONST) CurrentAppConfig,
  P2CONST(Os_Hal_MpAppConfigType, TYPEDEF, OS_CONST) NextAppConfig
))
{
  OS_IGNORE_UNREF_PARAM(CurrentAppConfig);                                                                              /* PRQA S 3112 */ /* MD_MSR_DummyStmt */

  /* #10 Invoke Os_Hal_MpAppSwitchTo() to switch to the next App MPU settings. */
  Os_Hal_MpAppSwitchTo(NextAppConfig);                                                                                  /* SBSW_OS_HAL_FC_CALLER */
}


/***********************************************************************************************************************
 *  Os_Hal_MpThreadSwitchTo()
 **********************************************************************************************************************/
/*!
 * Internal comment removed.
 *
...
))
{
  ...
  /* #80 Instruction sync barrier. */
  Os_Hal_Isync();                                                                                                       /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
}
...
Os_Hal_MpThreadSwitch,
(
  P2CONST(Os_Hal_MpThreadConfigType, TYPEDEF, OS_CONST) CurrentThreadConfig,
  P2CONST(Os_Hal_MpThreadConfigType, TYPEDEF, OS_CONST) NextThreadConfig
))
{
   OS_IGNORE_UNREF_PARAM(CurrentThreadConfig);                                                                          /* PRQA S 3112 */ /* MD_MSR_DummyStmt */

   /* #10 Invoke Os_Hal_MpThreadSwitchTo() to switch to the next thread MPU settings. */
   Os_Hal_MpThreadSwitchTo(NextThreadConfig);                                                                           /* SBSW_OS_HAL_FC_CALLER */
}
...
))
{
  ...
  for(Index=0; Index<Count; Index++)                                                                                    /* FETA_OS_HAL_MPU_REGIONS_NBR */
  {
