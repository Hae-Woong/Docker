...
struct Os_Hal_ContextConfigType_Tag
{
  ...
  /*! The PSW register configuration containing
      - protection set for AURIX and AURIX2G (corresponding to memory protection identifier),
      - accessing privilege (2 for privileged thread, 1 for non-privileged thread),
      - IS_MASK (Interrupt Stack Control bit: set to 1 by OS i.e. Shared Global Stack pointer is used on interrupt entry),
      - CDE_MASK (always set). */
  uint32 ProgramStatus;
  
  /*! the PPRS register of the AURIX3G contains the previous valid protection set and is stored here */
  uint32 ProtectionSet;
