...
/***********************************************************************************************************************
 *  Os_ThreadSetComElement()
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE void, OS_CODE,                                                             /* PRQA S 3219 */ /* MD_Os_Rule2.1_3219 */
OS_ALWAYS_INLINE, Os_ThreadSetComElement,
(
  P2CONST(Os_ThreadConfigType, AUTOMATIC, OS_CONST) Thread,
  P2CONST(Os_XSigComDataConfigType, TYPEDEF, OS_CONST) ComElement
))                                                                                                                      /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
{
  Thread->Dyn->ComElement = ComElement;                                                                                 /* SBSW_OS_TRD_THREADGETDYN_001 */
}


/***********************************************************************************************************************
 *  Os_ThreadReleaseComElement()
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE void, OS_CODE,                                                             /* PRQA S 3219 */ /* MD_Os_Rule2.1_3219 */
OS_ALWAYS_INLINE, Os_ThreadReleaseComElement,
(
  P2CONST(Os_ThreadConfigType, AUTOMATIC, OS_CONST) Thread
))                                                                                                                      /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
{
...
))
{
  ...
  while(OS_UNLIKELY(Os_LockListIsEmpty(&it) == 0u))                                                                     /* SBSW_OS_TRD_LOCKLISTISEMPTY_001 */ /* FETA_OS_LOCKLIST */
  {
...
))
{
  ...
  /* #10 Search the complete lock list */
  while(OS_UNLIKELY(Os_LockListIsEmpty(&it) == 0u))                                                                     /* SBSW_OS_TRD_LOCKLISTISEMPTY_001 */ /* COV_OS_HALPLATFORMTESTEDASMULTICORE */ /* FETA_OS_LOCKLIST */
  {
...
{
  P2VAR(Os_LockListType, AUTOMATIC, OS_VAR_NOINIT) it = &(Thread->Dyn->Locks);

  /* #10 Iterate over the lock list. */
  while(Os_LockListIsEmpty(it) == 0u)                                                                                   /* SBSW_OS_TRD_LOCKLISTISEMPTY_001 */ /* FETA_OS_LOCKLIST */
  {
    /* #20 Release the top lock. */
    P2CONST(Os_LockConfigType, AUTOMATIC, OS_VAR_NOINIT) lock = Os_LockListGetTop(it);                                  /* SBSW_OS_TRD_LOCKLISTGETTOP_001 */
    Os_LockListPop(it);                                                                                                 /* SBSW_OS_TRD_LOCKLISTPOP_001 */

    /* #30 If the lock is a resource, clean up the resource. */
    if(Os_LockIsResource(lock) != 0u)                                                                                   /* SBSW_OS_TRD_LOCKISRESOURCE_001 */ /* COV_OS_HALPLATFORMTESTEDASMULTICORE */
    {
      Os_ResourceForceReleaseCleanup(lock, Thread);                                                                     /* SBSW_OS_TRD_RESOURCEFORCERELEASECLEANUP_001 */
    }
    /* #40 Otherwise, forcibly release the spinlock. */
    else
    {
      Os_SpinlockForceReleaseLock(lock);                                                                                /* SBSW_OS_TRD_SPINLOCKFORCERELEASELOCK_001 */
    }
  }

  /* #50 If the thread is killed, release the remaining spinlocks via the locks owner field. */
  if (TerminationReason != OS_VOLUNTARY_TERMINATION)
  {
    Os_SpinlockForciblyReleaseThreadLocksViaOwnerThread(Thread);                                                        /* SBSW_OS_FC_PRECONDITION */
  }

  /* #60 Clear the thread's spinlock list. */
  Os_SpinlockClearThreadSpinlockList(Thread);                                                                           /* SBSW_OS_FC_PRECONDITION */
}
...
))
{
  ...
  /* #25 Initialize the communication element. */
  Thread->Dyn->ComElement = NULL_PTR;                                                                                   /* SBSW_OS_TRD_THREADGETDYN_001 */
...
))
{
  ...
  /* Never reached */
  Os_ErrKernelPanic(OS_STATUS_PANIC_UNEXPECTED_CONTROL_FLOW); /* function never returns! */                             /* PRQA S 2880 */ /* MD_Os_Rule2.1_2880_NonReturning */
}
...
))
{
  ...
  if(Os_ThreadIsFpuContextEnabledForThread(Thread) != 0u)                                                               /* SBSW_OS_FC_PRECONDITION */ /* PRQA S 2992, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2996 */
  {
    ...
    P2CONST(Os_ThreadClassType, AUTOMATIC, OS_VAR_NOINIT) threadClassObject;
...
))
{
  ...
  if(Os_ThreadIsFpuContextEnabled() != 0u)                                                                              /* PRQA S 2992, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2996 */
  {
    ...
    {
      result = (Os_StdReturnType)Thread->UsesFpu;                                                                       /* PRQA S 4304 */  /* MD_Os_C90BooleanCompatibility */
    }
  }
