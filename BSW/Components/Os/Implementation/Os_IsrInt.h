...
#define OS_ISR_CASTDYN_ISR_2_THREAD(isr)    (&((isr).Thread))



/*! Type cast from Os_IsrConfigType to Os_IsrConfigType by use of base element addressing. */
#define OS_ISR_CASTCONFIG_ISR_2_ISR(isr)               (&(isr))
...
/*! Hardware specific ISR and ISR mapping configuration. */
struct Os_IsrHwConfigType_Tag
{
  /*! Hardware specific ISR configuration. */
  P2CONST(Os_Hal_IntIsrConfigType, TYPEDEF, OS_CONST) HwConfig;

  /*! Hardware specific ISR mapping configuration. */
  P2CONST(Os_Hal_IntIsrMapConfigType, TYPEDEF, OS_CONST) MapConfig;

  /*! Tells whether the ISR has mapping information. */
  boolean IsMapped;

  /*! Tells whether the ISR needs a post action. */
  boolean IsPostActionRequired;
};
...
struct Os_IsrConfigType_Tag
{
  ...
  * This attribute must come first! */
  ...
  /*! Hardware specific ISR configuration. */
  P2CONST(Os_IsrHwConfigType, TYPEDEF, OS_CONST) SourceConfig;
 ...
 *  \param[in]      TerminationReason The reason why the ISR is killed.
 *
 *  \context        ISR2
 *
 *  \reentrant      TRUE for different ISRs.
 ...
 *  Os_IsrDisableSource()
 **********************************************************************************************************************/
/*! \brief          Disable the interrupt source of the given ISR.
 *  \details        --no details--
 *
 *  \param[in]      Isr   ISR whose source shall be disabled. Parameter must not be NULL.
 *
 *  \context        TASK|ISR2|ERRHOOK
 *
 *  \reentrant      TRUE for different ISRs.
 ...
 **********************************************************************************************************************/
/*! \brief          Initialize the interrupt source of the given ISR.
 *  \details        --no details--
 *
 *  \param[in]      Isr   ISR whose source shall be initialized. Parameter must not be NULL.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE for different ISRs.
 ...
 *  Os_IsrClearPending()
 **********************************************************************************************************************/
/*! \brief          Clear pending interrupts of the given ISR.
 *  \details        --no details--
 *
