...
))
{
  ...
  /* #10 Get current physical counter value. */
  now = Os_CounterGetPhysicalValue(Job->Counter);                                                                       /* SBSW_OS_JOB_COUNTERGETPHYSICALVALUE_001 */

  /* #20 If the TimeStamp is a future value: */
  if(OS_LIKELY(Os_CounterIsFutureValue(Job->Counter, TimeStamp, now) != 0u))                                            /* SBSW_OS_JOB_COUNTERISFUTUREVALUE_001 */ /* COV_OS_NOFUNCCALL */
  {
    Os_TickType timeStampMod;

    /* #30 Calculate now % MaxAllowedValue. */
    now = Os_CounterGetModulo(Job->Counter, now);                                                                       /* SBSW_OS_JOB_COUNTERGETMODULO_001 */

    /* #40 Calculate TimeStamp % MaxAllowedValue. */
    timeStampMod = Os_CounterGetModulo(Job->Counter, TimeStamp);                                                        /* SBSW_OS_JOB_COUNTERGETMODULO_001 */

    /* #50 Calculate the distance between now and timeStampMod in the range from 1..MaxAllowedValue */
    result = Os_CounterSub(Job->Counter, timeStampMod, now);                                                            /* SBSW_OS_JOB_COUNTERSUB_001 */
  }
  /* #60 Otherwise: */
  else
  {
    /* #70 Job expiration time already passed, job will be handled with the next timer interrupt. */
    result = 0u;
  }
