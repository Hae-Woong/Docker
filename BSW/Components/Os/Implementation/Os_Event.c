 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE Os_StdReturnType, OS_CODE, OS_ALWAYS_INLINE, Os_EventIsEnabled,
(
  void
));
...
OS_ALWAYS_INLINE, Os_EventIsEnabled,
(
  void
))
{
 ...
 */
/* PRQA S 6030, 6050, 6080 1 */ /* MD_MSR_STCYC, MD_MSR_STCAL, MD_MSR_STMIF */
FUNC(Os_StatusType, OS_CODE) Os_Api_SetEventAsyn
(
  TaskType TaskID,
  EventMaskType Mask
)
{
  ...
  else if(Os_XSigIsEnabled() == 0u)                                                                                     /* PRQA S 2991, 2992, 2995, 2996 */ /*  MD_Os_Rule14.3_2991, MD_Os_Rule14.3_2992,  MD_Os_Rule2.2_2995, MD_Os_Rule2.2_2996 */ /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
  {
    status = Os_ErrSetOkInStdStatus(OS_STATUS_XSIGNAL_DISABLED_BY_CONFIG);                                              /* PRQA S 2880 */ /* MD_Os_Rule2.1_2880 */
  }
  else if(OS_UNLIKELY(Os_TaskCheckId(TaskID) == OS_CHECK_FAILED))                                                       /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
  {
    status = OS_STATUS_ID_1;
  }
  else
  {
    ...
    else if(OS_UNLIKELY(Os_TaskIsCoreLocal(task) != 0u))                                                                /* SBSW_OS_EV_TASKISCORELOCAL_001 */ /* PRQA S 2991, 2995 */ /* MD_Os_Rule14.3_2991, MD_Os_Rule2.2_2995 */
    {
      status = OS_STATUS_ID_1;
    }
    /* #20 No errors are detected: */
    else
    {
      P2CONST(Os_CoreAsrConfigType, AUTOMATIC, OS_CONST) core;
      core = Os_TaskGetCore(task);                                                                                      /* SBSW_OS_EV_TASKGETCORE_001 */

      /* #30 Signal the responsible core to set events asynchronously. */
      status = Os_XSigSend_SetEventAsync(core, currentThread, TaskID, Mask);                                            /* SBSW_OS_EV_XSIGSEND_001 */
    }
  }
...
)
{
  ...
  {
    /* #50 Signal the responsible core to set the event. */
...
)
{
  ...
  {
    P2CONST(Os_EventStateType, AUTOMATIC, OS_CONST) event;
    event = Os_TaskGetEvent(Task);                                                                                      /* SBSW_OS_FC_PRECONDITION */

    do                                                                                                                  /* FETA_OS_EVENTGETLOCAL */
    {
