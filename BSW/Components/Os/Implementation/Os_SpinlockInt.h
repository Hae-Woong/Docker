 ...
 * \details
 *
 * The maximal nestable size of spinlocks is limited to 255 by Os_SpinlockOrderType.
 *
...
#define OS_SPINLOCK_CASTDYN_SPINLOCK_2_LOCK(spinlock)  (&((spinlock).Lock))
...
/*! Defines whether the spinlock is an OPTIMIZED or an AUTOSAR standard spinlock. */
typedef enum
{
  /*! \brief   Spinlock is an AUTOSAR standard spinlock.
   *  \details Owner tracing, API checks and lock list tracing are enabled. Spinlock is always supervisor mode. */
  OS_SPINLOCKTYPE_STANDARD,
  /*! \brief   Spinlock is an OPTIMIZED spinlock.
   *  \details Owner tracing is enabled when forcible termination is enabled. API checks and lock list tracing are
   *  disabled. */
  OS_SPINLOCKTYPE_OPTIMIZED
}Os_SpinlockTypeType;
...
struct Os_SpinlockConfigType_Tag
{
  ...
  /*! Tells whether the spinlock is an AUTOSAR or OPTIMIZED spinlock. */
  Os_SpinlockTypeType Type;
 ...
 **********************************************************************************************************************/
/*! \brief          Release all spinlocks which are owned by the given thread, as indicated by the locks' owner fields.
 *  \details        This function iterates over all configured spinlocks and checks each owner field. This function is
 *                  used during forcible termination of a thread because all spinlocks, including optimized spinlocks,
 *                  must be released. If killing is enabled, optimized spinlocks trace their owner but are not stored in
 *                  the thread's lock list.
 *
 ...
 *  \synchronous    TRUE
 *
 *  \pre            The thread's lock list must not contain spinlocks.
 *  \pre            Concurrent access to given object is prevented by caller.
 **********************************************************************************************************************/
 ...
 **********************************************************************************************************************/
/*! \brief          Releases the given lock as a spinlock.
 *  \details        --no details--
 *
 *  \param[in]      Lock      The lock to release. Parameter must not be NULL.
 *
 *  \context        OS_INTERNAL
 *
 ...
 *  \pre            No lock list contains the given lock.
 *  \pre            Concurrent access to given objects is prevented by caller.
 **********************************************************************************************************************/
FUNC(void, OS_CODE) Os_SpinlockForceReleaseLock
(
 ...
 *  \details        --no details--
 *
 *  \param[in,out]  Spinlock  The spinlock to be released. Parameter must not be NULL.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
...
/***********************************************************************************************************************
 *  Os_SpinlockClearThreadSpinlockList()
 **********************************************************************************************************************/
/*! \brief          Clear the given thread's spinlock list.
 *  \details        --no details--
 *
 *  \param[in]      Thread      The thread. Parameter must not be NULL.
 *
 *  \context        OS_INTERNAL
 *
