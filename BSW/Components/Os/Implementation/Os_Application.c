 ...
 *  \param[in]      TerminationReason The reason why the application terminates.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      FALSE
 ...
 *  \pre            If killing is disabled by configuration, TerminationReason must not be OS_FORCIBLE_TERMINATION.
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE void, OS_CODE,
OS_ALWAYS_INLINE, Os_AppKillMembers,
(
  P2CONST(Os_AppConfigType, AUTOMATIC, OS_CONST) App,
  Os_TerminationType TerminationReason
));
 ...
 *  \param[in]      TerminationReason The reason why the thread is killed.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      FALSE
 ...
 *
 *  \pre            Concurrent access to given object is prevented by caller.
 *  \pre            If killing is disabled by configuration, TerminationReason must not be FORCIBLE_TERMINATION.
 **********************************************************************************************************************/
 ...
 *  \param[in]      TerminationReason The reason why the application is killed.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      FALSE
...
{
  return Os_ErrExtendedCheck(((Os_AppIsTrusted(TerminatorApp)) || (TerminatorApp == TargetApp)));                       /* PRQA S 4115, 4404, 4558 */  /* MD_Os_Rule10.1_4115, MD_Os_C90BooleanCompatibility, MD_Os_Rule10.1_4558 */ /* SBSW_OS_FC_PRECONDITION */
}
...
))
{
  ...
  if (accessingApplications != 0u)
  {
    ret = Os_AppIsAccessAllowed(ApplID, OS_APPID2MASK(ApplID), accessingApplications);
  }
...
)
{
  ...
  /* #10 Initialize tasks. */
  for(index = 0; index < (Os_ObjIdx_IteratorType) App->TaskCount; index++)                                              /* FETA_OS_CONSTANTASFUNCTIONPARAMETER */
  {
    Os_TaskInit(App->TaskRefs[index], AppMode);                                                                         /* SBSW_OS_APP_TASKINIT_001 */
  }

  /* #20 Initialize ISRs. */
  for(index = 0; index < (Os_ObjIdx_IteratorType) App->IsrCat2Count; index++)                                           /* FETA_OS_CONSTANTASFUNCTIONPARAMETER */
  {
    Os_IsrInit(App->IsrCat2Refs[index]);                                                                                /* SBSW_OS_APP_ISRINIT_001 */
  }

  /* #30 Initialize Counters. */
  for(index = 0; index < (Os_ObjIdx_IteratorType) App->CounterCount; index++)                                           /* FETA_OS_CONSTANTASFUNCTIONPARAMETER */
  {
    Os_CounterInit(App->CounterRefs[index]);                                                                            /* SBSW_OS_APP_COUNTERINIT_001 */
  }

  /* #40 Initialize Alarms. */
  for(index = 0; index < (Os_ObjIdx_IteratorType) App->AlarmCount; index++)                                             /* FETA_OS_CONSTANTASFUNCTIONPARAMETER */
  {
    Os_AlarmInit(App->AlarmRefs[index], AppMode);                                                                       /* SBSW_OS_APP_ALARMINIT_001 */
  }

  /* #50 Initialize Schedule Tables. */
  for(index = 0; index < (Os_ObjIdx_IteratorType) App->SchTCount; index++)                                              /* FETA_OS_CONSTANTASFUNCTIONPARAMETER */
  {
  ...
  /* #90 Inform the trace module about the application start. */
  Os_TraceAppStart(App->TraceAppl);                                                                                     /* SBSW_OS_APP_TRACEAPPSTART_001 */
...
)
{
  ...
  for(index = 0; index < (Os_ObjIdx_IteratorType) App->ServiceFunctionCount; index++)                                   /* FETA_OS_CONSTANTASFUNCTIONPARAMETER */
  {
...
)
{
  ...
  for(index = 0; index < (Os_ObjIdx_IteratorType) App->HookCount; index++)                                              /* FETA_OS_CONSTANTASFUNCTIONPARAMETER */
  {
...
))
{
  ...
  for(it = 0; it < (Os_ObjIdx_IteratorType) App->IsrCat2Count; it++)                                                    /* FETA_OS_CONSTANTASFUNCTIONPARAMETER */
  {
    Os_IsrDisable(App->IsrCat2Refs[it]);                                                                                /* SBSW_OS_APP_ISRDISABLE_001 */
  }

  for(it = 0; it < (Os_ObjIdx_IteratorType) App->IsrCat1Count; it++)                                                    /* FETA_OS_CONSTANTASFUNCTIONPARAMETER */
  {
    Os_IsrCat1Disable(App->IsrCat1Refs[it]);                                                                            /* SBSW_OS_APP_ISR1DISABLE_001 */
  }

  /* #20 Kill application's alarms. */
  for(it = 0; it < (Os_ObjIdx_IteratorType) App->AlarmCount; it++)                                                      /* FETA_OS_CONSTANTASFUNCTIONPARAMETER */
  {
    Os_AlarmKill(App->AlarmRefs[it]);                                                                                   /* SBSW_OS_APP_ALARMKILL_001 */
  }

  /* #30 Kill application's schedule tables. */
  for(it = 0; it < (Os_ObjIdx_IteratorType) App->SchTCount; it++)                                                       /* FETA_OS_CONSTANTASFUNCTIONPARAMETER */
  {
    Os_SchTKill(App->SchTRefs[it]);                                                                                     /* SBSW_OS_APP_Os_SCHTKILL_001 */
  }

  /* #40 Kill application's tasks (and all it outstanding activations). */
  for(it = 0; it < (Os_ObjIdx_IteratorType) App->TaskCount; it++)                                                       /* FETA_OS_CONSTANTASFUNCTIONPARAMETER */
  {
    Os_TaskKillAll(App->TaskRefs[it], TerminationReason);                                                                                  /* SBSW_OS_APP_TASKKILLALL_001 */
  }

  /* #50 Kill interrupted threads, if they belong to the given application. */
  Os_AppKillInterruptedMemberThreads(App, TerminationReason);                                                           /* SBSW_OS_FC_PRECONDITION */

  /* #60 Kill current thread, if it belongs to the given application. */
  Os_AppKillMemberThread(Os_CoreGetThread(), App, TerminationReason);                                                   /* SBSW_OS_FC_PRECONDITION */
...
))
{
  ...
  while(iterator != iteratorEnd)                                                                                        /* FETA_OS_MAXINTERRUPTEDTHREADS */
  {
    ...
    Os_AppKillMemberThread(interruptedThread, App, TerminationReason);                                                  /* SBSW_OS_APP_APPKILLMEMBERTHREAD_001 */
    ...
    switch(Os_ThreadGetInitialType(Thread))                                                                             /* SBSW_OS_FC_PRECONDITION */
    {
      ...
      case OS_CALLCONTEXT_ISR2:
        Os_IsrKill(                                                                                                     /* SBSW_OS_APP_ISRKILL_001 */
            Os_IsrThread2Isr(Thread), TerminationReason);                                                               /* SBSW_OS_FC_PRECONDITION */
        break;

      case OS_CALLCONTEXT_ERRORHOOK:
      case OS_CALLCONTEXT_STARTUPHOOK:
      case OS_CALLCONTEXT_IOCCALLBACK:
        Os_HookKill(Os_HookThread2Hook(Thread), TerminationReason);                                                     /* SBSW_OS_APP_HOOKKILL_001 */ /* SBSW_OS_FC_PRECONDITION */
        break;
...
)
{
  ...
  Os_AppKillMembers(App, TerminationReason);                                                                                               /* SBSW_OS_FC_PRECONDITION */
  ...
  if(RestartOption == RESTART)
  {
    ...
    {
      Os_ErrKernelPanic(OS_STATUS_PANIC_FAILED_RESTARTTASK_ACTIVATION);
    }
  }
  else
  {
    ...
    restartOptionSet = FALSE;
  }

  /* #90 Inform the trace module about the application killing. */
  Os_TraceAppKill(App->TraceAppl, restartOptionSet);                                                                    /* SBSW_OS_APP_TRACEAPPKILL_001 */
...
)
{
  ...
  if(Os_ErrIsKillingEnabled() == 0u)                                                                                    /* COV_OS_INVSTATE */ /* PRQA S 2992, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2996 */
  {
    Os_ErrKernelPanic(OS_STATUS_PANIC_FUNCTIONALITY_NOT_AVAILABLE);
  }

  /* #15 Call internal kill application behavior. */
  Os_AppKillInternal(App, RestartOption, OS_FORCIBLE_TERMINATION);                                                      /* SBSW_OS_FC_PRECONDITION */
...
)
{
  ...
  Os_AppKillInternal(App, NO_RESTART, TerminationReason);                                                               /* SBSW_OS_FC_PRECONDITION */
}
...
)
{
  ...
  else
  {
    ...
    if(OS_LIKELY(Os_AppIsCoreLocal(targetApp) != 0u))                                                                   /* SBSW_OS_APP_APPISCORELOCAL_001 */ /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */ /* PRQA S 2991, 2995 */ /* MD_Os_Rule14.3_2991, MD_Os_Rule2.2_2995 */
    {
...
)
{
  ...
  /* #10 Perform error checks. */
  if(Os_ErrIsServiceProtectionEnabled() == 0u)                                                                          /* PRQA S 2992, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2996 */
  {
    result = ACCESS;
  }
...
)
{
  ...
  else
  {
    ...
    /* #30 Inform the trace module about the application access allowance. */
    Os_TraceAppAllowAccess(currentApp->TraceAppl);                                                                      /* SBSW_OS_APP_TRACEAPPALLOWACCESS_001 */
  }
...
\ID SBSW_OS_APP_TRACEAPPSTART_001
 \DESCRIPTION    Os_TraceAppStart is called with the Trace member of an App instance. Precondition ensures that the
                 App pointer is valid. The compliance of the precondition is check during review.
 \COUNTERMEASURE \M [CM_OS_APPTRACE_M]

\ID SBSW_OS_APP_TRACEAPPKILL_001
 \DESCRIPTION    Os_TraceAppKill is called with the Trace member of an App instance. Precondition ensures that the
                 App pointer is valid. The compliance of the precondition is check during review.
 \COUNTERMEASURE \M [CM_OS_APPTRACE_M]

\ID SBSW_OS_APP_TRACEAPPALLOWACCESS_001
 \DESCRIPTION    Os_TraceAppAllowAccess is called with the Trace member of an App instance which is returned from
                 Os_ThreadGetCurrentApplication.
 \COUNTERMEASURE \M [CM_OS_APPTRACE_M]
