...
OS_FUNC_ATTRIBUTE_DEFINITION(void, OS_CODE, OS_NORETURN, Os_ThreadSuicide, (void))
{
  ...
  switch(Os_ThreadGetInitialType(currentThread))                                                                        /* SBSW_OS_TRD_THREADGETCORE_001 */
  {
    ...
    default:                                                                                                            /* COV_OS_INVSTATE */
      /* Invalid OS state. */
      Os_ErrKernelPanic(OS_STATUS_PANIC_INVALID_SUICIDE); /* function never returns! */
      break;                                                                                                            /* PRQA S 2880 */ /* MD_Os_Rule2.1_2880_NonReturning */
  }

  /* Invalid OS state. */
  Os_ErrKernelPanic(OS_STATUS_PANIC_UNEXPECTED_CONTROL_FLOW); /* function never returns! */                             /* PRQA S 2880 */ /* MD_Os_Rule2.1_2880_NonReturning */ /* COMP_WARN_OS_UNREACHABLE_CODE */
}
...
FUNC(void, OS_CODE) Os_ThreadKill
(
...
)
{
  ...
  /* #35 Clean up the interrupt state. */
  ...
  Os_ThreadLocksForceRelease(Thread, TerminationReason);                                                                /* SBSW_OS_FC_PRECONDITION */

  /* #45 Release communication data element, if occupied by the thread. */
  if (Thread->Dyn->ComElement != NULL_PTR)                                                                              /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
  {
    Os_XSigChannelAcknowledgeResult(Thread->Dyn->ComElement);                                                           /* SBSW_OS_TRD_XSIGCHANNELACKNOWLEDGERESULT_001 */
    Thread->Dyn->ComElement = NULL_PTR;                                                                                 /* SBSW_OS_TRD_THREADGETDYN_001 */
  }
...
\ID SBSW_OS_TRD_THREADGETDYN_001
 \DESCRIPTION    Write access to the Dyn pointer which is derived from the Thread pointer. The Thread pointer is passed
                 as an argument to the caller function. Precondition ensures that the argument pointer is valid.
...
\ID SBSW_OS_TRD_LOCKLISTINIT_001
 \DESCRIPTION    Os_LockListInit is called with the Locks argument derived from the Dyn pointer of a Thread. The Thread
                 is passed as argument to the caller.
...
\ID SBSW_OS_TRD_LOCKLISTISEMPTY_001
 \DESCRIPTION    Os_LockListIsEmpty is called with an iterator pointer which was initialized with the Locks value of the
                 dynamic object of a thread. Thread is passed as argument to the caller. Precondition ensures that the
                 argument pointer is valid. The compliance of the precondition is check during review.
...
\ID SBSW_OS_TRD_LOCKLISTPOP_001
 \DESCRIPTION    Os_LockListPop is called with an iterator.
...
\ID SBSW_OS_TRD_LOCKLISTGETTOP_001
 \DESCRIPTION    Os_LockListGetTop is called with an iterator.
 \COUNTERMEASURE \R Loop condition ensures that the passed pointer is valid.
...
\ID SBSW_OS_TRD_LOCKISRESOURCE_001
 \DESCRIPTION    Os_LockIsResource is called with the return value of Os_LockListGetTop.
 \COUNTERMEASURE \R Loop condition ensures that the passed pointer is valid.

\ID SBSW_OS_TRD_RESOURCEFORCERELEASECLEANUP_001
 \DESCRIPTION    Os_ResourceForceReleaseCleanup is called with the return value of Os_LockListGetTop and a Thread
                 pointer passed as argument to the caller. Precondition ensures that the Thread pointer is valid.
 \COUNTERMEASURE \R Loop condition ensures that the passed Lock pointer is valid.

\ID SBSW_OS_TRD_SPINLOCKFORCERELEASELOCK_001
 \DESCRIPTION    Os_SpinlockForceReleaseLock is called with the return value of Os_LockListGetTop.
 \COUNTERMEASURE \R Loop condition ensures that the passed Lock pointer is valid.
...
\ID SBSW_OS_TRD_XSIGCHANNELACKNOWLEDGERESULT_001
 \DESCRIPTION    Os_XSigChannelAcknowledgeResult is called with ComElement which is derived from the Dyn pointer of a
                 thread. The thread pointer is passed as an argument to the caller function. Precondition ensures that
                 the argument thread pointer is valid. The compliance of the precondition is checked during review.
                 As each thread object of an abstract class is always part of a concrete class, the concrete class
                 guarantees, that the thread Dyn pointer is valid.
 \COUNTERMEASURE \R [CM_OS_THREADCOMELEM_R]
...
\CM CM_OS_THREADGETLOCKS_N
      ...
      class, the concrete class guarantees, that the Dyn pointer is valid. Precondition ensures that the Locks pointer
...
\CM CM_OS_THREADCOMELEM_R
      Implementation ensures that the ComElement is not NULL_PTR before using it.
