 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE Os_StdReturnType, OS_CODE, OS_ALWAYS_INLINE,                               /* COV_OS_HALPLATFORMCLEARPENDINGAVAILABILITY */
Os_IsrIsInterruptPending,
(
  P2CONST(Os_IsrHwConfigType, AUTOMATIC, OS_CONST) Isr
...
OS_FUNC_ATTRIBUTE_DEFINITION(void, OS_CODE, OS_NORETURN, Os_IsrEpilogue, (void))
{
  ...
  else
  {
    ...
    Os_StdReturnType resourcesReleased = Os_ThreadCheckResourcesReleased(isrThread);                                    /* SBSW_OS_ISR_THREADCHECKRESOURCESRELEASED_001 */
    Os_StdReturnType spinlocksReleased = Os_ThreadCheckSpinlocksReleased(isrThread);                                    /* SBSW_OS_ISR_THREADCHECKSPINLOCKSRELEASED_001 */

    /* Forcibly release all locks. That should do nothing if lock list is empty. */
    Os_ThreadLocksForceRelease(isrThread, OS_VOLUNTARY_TERMINATION);                                                    /* SBSW_OS_ISR_THREADLOCKSFORCERELEASE_001 */

    if(OS_UNLIKELY(resourcesReleased == OS_CHECK_FAILED))                                                               /* COMP_WARN_OS_CW_005 */
    {
      Os_ErrReportIsrEpilogue(OS_STATUS_RESOURCE);
    }

    if(OS_UNLIKELY(spinlocksReleased == OS_CHECK_FAILED))                                                               /* COV_OS_HALPLATFORMTESTEDASMULTICORE */ /* COMP_WARN_OS_CW_005 */
    {
...
OS_FUNC_ATTRIBUTE_DEFINITION(void, OS_CODE, OS_NORETURN, Os_IsrEpilogueTp, (void))                                      /* COV_OS_HALPLATFORMTPUNSUPPORTED_XX */
{
  ...
  if(OS_UNLIKELY(Os_IsrGetDyn(isr)->IsKilled == TRUE))                                                                  /* SBSW_OS_ISR_ISRGETDYN_001 */ /* COV_OS_INVSTATE */
  {
    ...
    Os_ErrKernelPanic(OS_STATUS_PANIC_TP_ISR_KILLED);
  }
...
FUNC(Os_StatusType, OS_CODE) Os_Api_EnableInterruptSource(ISRType ISRID, boolean ClearPending)
{
  ...
  /* #10 Perform checks. */
  if(OS_UNLIKELY(Os_ErrCheckIsBoolean(ClearPending) == OS_CHECK_FAILED))                                                /* PRQA S 2992, 2996 */ /* MD_Os_BoolOnlyInStaticAnalysis, MD_Os_BoolOnlyInStaticAnalysis */
  {
...
FUNC(Os_StatusType, OS_CODE) Os_Api_InitialEnableInterruptSources(boolean ClearPending)
{
  ...
  else if(OS_UNLIKELY(Os_ErrCheckIsBoolean(ClearPending) == OS_CHECK_FAILED))                                           /* PRQA S 2992, 2996 */ /* MD_Os_BoolOnlyInStaticAnalysis, MD_Os_BoolOnlyInStaticAnalysis */
  {
    status = OS_STATUS_VALUE_IS_NOT_BOOLEAN;
  }
  else if(OS_UNLIKELY(Os_IsrCheckClearPendingSupported(ClearPending) == OS_CHECK_FAILED))                               /* PRQA S 2992, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2996 */ /* COV_OS_NOFUNCCALL */
...
\ID SBSW_OS_ISR_THREADLOCKSFORCERELEASE_001
 \DESCRIPTION    Os_ThreadLocksForceRelease is called with the return value of Os_CoreGetThread.
 \COUNTERMEASURE \N [CM_OS_COREGETTHREAD_N]
