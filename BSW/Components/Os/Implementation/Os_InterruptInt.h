...
typedef uint16 Os_IntApiCntType;

/*! Tells whether the user is inside of an interrupt API sequence. */
typedef struct
{
  boolean InDisableAll; /*!< Inside a DisableAllInterrupts() API sequence. */
 ...
 *  \details      --no details--
 *
 *  \param[in,out]    Source        The interrupt state to be copied. Parameter must not be NULL.
 ...
 *  \pre          -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_InterruptStateCopy,
(
  P2CONST(Os_IntStateType, AUTOMATIC, OS_VAR_NOINIT) Source,
  P2VAR(Os_IntStateType, AUTOMATIC, OS_VAR_NOINIT) Destination
));
 ...
 **********************************************************************************************************************/
/*! \brief        OS service \ref Os_EnableInterruptsPreStart().
 *  \details      For further details see Os_EnableInterruptsPreStart().
 *
 *  \context      INIT
 *
 *  \reentrant    FALSE
 *  \synchronous  TRUE
 *
 *  \pre          See Os_EnableInterruptsPreStart().
 *
