...
{
  Os_Hal_SpinlockResultType retVal = Os_Hal_CmpSwap(Spinlock, Os_Hal_CoreGetId(), OS_HAL_UNLOCKED);   /* SBSW_OS_HAL_FC_CALLER */
  Os_Hal_Dsync();
  return retVal;
}
 ...
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE uint16, OS_CODE, OS_ALWAYS_INLINE,
Os_Hal_LoadAcquire16,
(
  P2CONST(uint16 volatile, AUTOMATIC, AUTOMATIC) Address
))
{
  uint16 Loaded_Val = *Address;
  Os_Hal_Dsync();
  return Loaded_Val;
}
...
{
  Os_Hal_Dsync();
  *Address = Value;                                                                                                     /* SBSW_OS_HAL_PWA_CALLER */
}
