...
))
{
  ...
  OS_IGNORE_UNREF_PARAM(CurrentConfig); 
}
...
{
  Os_Hal_Mtcr(OS_HAL_PPRS_OFFSET, PprsValue);
}
...
{
  return (uint32)Os_Hal_Mfcr(OS_HAL_PPRS_OFFSET);
}
...
{
  Os_Hal_Mtcr(OS_HAL_BTV_REGISTER, Config->AddressOfExceptionTable);
  Os_Hal_Mtcr(OS_HAL_BIV_REGISTER, Config->AddressOfInterruptTable);
  Os_Hal_Isync();
}
