...
#define OS_RESOURCE_CASTCONFIG_RESOURCE_2_RESOURCE(resource)           (&(resource))


/*! Type cast from Os_ResourceConfigType to Os_ResourceInterruptConfigType by use of base element addressing. */
#define OS_RESOURCE_CASTCONFIG_RESOURCEINTERRUPT_2_RESOURCE(resource)  (&((resource).Resource))


/*! Type cast from Os_ResourceType to Os_LockType by use of base element addressing. */
#define OS_RESOURCE_CASTDYN_RESOURCE_2_LOCK(resource)                  (&((resource).Lock))


/*! Type cast from Os_ResourceInterruptType to Os_LockType by use of base element addressing. */
#define OS_RESOURCE_CASTDYN_RESOURCEINTERRUPT_2_LOCK(resource)         (&((resource).Resource.Lock))
 ...
 **********************************************************************************************************************/
/*! \brief          Cleanup a forcibly released resource lock.
 *  \details        Restores the scheduling priority of the occupying task (if applicable) and stops the resource
 *                  budget monitoring.
 *
 *  \param[in]      Lock      The resource lock to clean up. Parameter must not be NULL.
 *  \param[in,out]  Thread    The thread which is forcibly terminated. Parameter must not be NULL.
 *
 *  \context        OS_INTERNAL
 *
 ...
 *  \synchronous    TRUE
 *
 *  \pre            The given lock is a resource.
 *  \pre            Concurrent access to given objects is prevented by caller.
 **********************************************************************************************************************/
FUNC(void, OS_CODE) Os_ResourceForceReleaseCleanup
(
 ...
 *  \retval       OS_STATUS_DISABLEDINT     (EXTENDED status:) Caller is in interrupt API sequence.
 *  \retval       OS_STATUS_ACCESSRIGHTS_1  (Service Protection:) Caller's access rights are not sufficient.
 *
 *  \context      TASK|ISR2
 *
