...
)
{
  ...
  if(Os_CoreIsInterruptOnlyOs() == 0u )                                                                                 /* PRQA S 2991, 2992, 2995, 2996 */ /* MD_Os_Rule14.3_2991, MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2995, MD_Os_Rule2.2_2996 */ /* COV_OS_INVSTATE */
  {
    /* #10 Switch from global interrupt lock to level lock if needed. */
    Os_IntDisable();

    if(Os_InterruptLockOnLevelIsNeeded() != 0u)                                                                         /* PRQA S 2991, 2992, 2995, 2996 */ /* MD_Os_Rule14.3_2991, MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2995, MD_Os_Rule2.2_2996 */ /* COV_OS_HALPLATFORMLEVELSUPPORT */
    {
      if(Os_IntIsGlobal2LevelSwitchSupported() != 0u)                                                                   /* PRQA S 2991, 2992, 2995, 2996 */ /* MD_Os_Rule14.3_2991, MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2995, MD_Os_Rule2.2_2996 */ /* COV_OS_HALPLATFORMGLOBAL2LEVELSUPPORTED */
      {
        Os_IntEnableGlobal();
      }
    }
    ...
    /* #40 Otherwise, enter Kernel Panic. */
    Os_ErrKernelPanic(OS_STATUS_PANIC_MEMFAULT);
  }
}
...
\ID SBSW_OS_MP_Hal_MPAPPSWITCH_001
 \DESCRIPTION    Os_Hal_MpAppSwitch is called with AppAccessRights objects derived from the caller arguments.
                 Precondition ensures that the pointer arguments are valid. The compliance of the precondition is
                 checked during review.
 \COUNTERMEASURE \M [CM_OS_MPACCESSRIGHTS_APPACCESSRIGHTS_M]

\ID SBSW_OS_MP_Hal_MPAPPSWITCHTO_001
 \DESCRIPTION    Os_Hal_MpAppSwitchTo is called with an AppAccessRights object derived from the caller argument.
                 Precondition ensures that the pointer is valid. The compliance of the precondition is checked during
                 review.
 \COUNTERMEASURE \M [CM_OS_MPACCESSRIGHTS_APPACCESSRIGHTS_M]

\ID SBSW_OS_MP_Hal_MPTHREADSWITCH_001
 \DESCRIPTION    Os_Hal_MpThreadSwitch is called with ThreadAccessRights object derived from the caller arguments.
                 Precondition ensures that the pointer arguments are valid. The compliance of the precondition is
                 checked during review.
 \COUNTERMEASURE \M [CM_OS_MPACCESSRIGHTS_THREADACCESSRIGHTS_M]

\ID SBSW_OS_MP_Hal_MPTHREADSWITCHTO_001
 \DESCRIPTION    Os_Hal_MpThreadSwitchTo is called with a ThreadAccessRights object derived from the caller argument.
                 Precondition ensures that the pointer is valid. The compliance of the precondition is checked during
                 review.
 \COUNTERMEASURE \M [CM_OS_MPACCESSRIGHTS_THREADACCESSRIGHTS_M]
