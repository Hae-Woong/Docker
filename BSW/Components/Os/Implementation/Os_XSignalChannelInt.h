...
typedef enum {
  OS_XSIG_WRITE_LOCAL,
  OS_XSIG_WRITE_SYNCRONIZED
} Os_XSigWriteVisibilityType;
 ...
 */
typedef Os_XSigBufferIdxType Os_XSigState_Type;

#define OS_XSIGSTATE_ASYNC_NACK   ((Os_XSigBufferIdxType)0u) /*!< The request is asynchronous and the result was not handled on sender side. */
#define OS_XSIGSTATE_SYNC_NACK    ((Os_XSigBufferIdxType)1u) /*!< The request is synchronous and the result was not handled on sender side. */
#define OS_XSIGSTATE_SYNC_PENDING ((Os_XSigBufferIdxType)2u) /*!< The request is synchronous and the result was handled on receiver side. */
#define OS_XSIGSTATE_ACK          ((Os_XSigBufferIdxType)3u) /*!< The result of the request was handled on sender side. */
...
/*! Data representation of cross core communication port. */
struct Os_XSigComDataConfigType_Tag
{
...
/*! Pointer to a XSigComData configuration. */
 ...
 **********************************************************************************************************************/
/*! \brief          Checks if a result is available and returns it. If no result is available, NULL_PTR is returned.
 *  \details        --no details--
 *
 *  \param[in,out]  Channel    The Channel instance. Parameter must not be NULL.
 *
 *  \return         The next result element element, if it is present.
 *
 *  \retval         NULL_PTR    If there is no result element available.
 *
 *  \context        ANY
 *
 *  \reentrant      TRUE for different Channels
 ...
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE P2CONST(Os_XSigComDataConfigType, AUTOMATIC, OS_CONST),
OS_CODE, OS_ALWAYS_INLINE, Os_XSigChannelTryGetResult,
(
  P2CONST(Os_XSigChannelSendConfigType, AUTOMATIC, OS_APPL_DATA) Channel
));
 ...
 **********************************************************************************************************************/
/*! \brief          Acknowledges the given communication data element.
 *  \details        --no details--
 *
 *  \param[in,out]  ComData    The communication data element. Parameter must not be NULL.
 *
 *  \context        ANY
 *
 *  \reentrant      TRUE for different communication data elements.
 *  \synchronous    TRUE
 *
 *  \pre            -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_XSigChannelAcknowledgeResult,
(
