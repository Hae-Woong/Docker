...
#define OS_TASK_CASTDYN_TASK_2_THREAD(task)                      (&((task).Thread))


/*! Type cast from Os_TaskConfigType to Os_TaskConfigType by use of base element addressing. */
#define OS_TASK_CASTCONFIG_TASK_2_TASK(task)                     (&(task))


/***********************************************************************************************************************
 *  GLOBAL DATA TYPES AND STRUCTURES
 **********************************************************************************************************************/
typedef uint8 Os_ActivationCntType;
...
struct Os_TaskType_Tag
{
  ...
  /*! Tells the remaining number of round robin scheduling events until rescheduling takes place */
  uint32 NumRemainScheduleEventsRoundRobin;
};
 ...
 *  \param[in]      TerminationReason The reason why the task is terminated.
 *
 ...
 *  \pre            Killing is enabled by configuration.
 *
 *  \trace          CREQ-70
 ...
 **********************************************************************************************************************/
/*! \brief          Set the remaining schedule events for round robin scheduling to the initial value.
 *  \details        Initial value is given in configuration.
 *
 *  \param[in,out]  Task        Task to modify. Parameter must not be NULL.
 *
 *  \context        ANY
 ...
 *  \pre            After initialization, this function shall only be called by the scheduler.
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_TaskInitRoundRobinCounter,
(
  P2CONST(Os_TaskConfigType, AUTOMATIC, OS_CONST) Task
));
 ...
 **********************************************************************************************************************/
/*! \brief          Counts round robin events. After a configured number of round robin events, round robin
 *                  scheduling will take place.
 *
 *  \param[in,out]  Task        The task to query. Parameter must not be NULL.
 *
 *  \context        ANY
 *
 *  \reentrant      TRUE
 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_TaskRoundRobinCount,
(
  P2CONST(Os_TaskConfigType, AUTOMATIC, OS_CONST) Task
));
 ...
 **********************************************************************************************************************/
/*! \brief          Returns whether the given task shall be moved from head of queue to tail (!0) or not (0).
 *
 *  \param[in]      Task        The task to query. Parameter must not be NULL.
 *
 *  \retval         0    If the given task shall be moved to the queue tail.
 *  \retval         !0   If the given task shall stay at queue head.
 *
 *  \context        ANY
 *
 *  \reentrant      TRUE
 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE Os_StdReturnType, OS_CODE, OS_ALWAYS_INLINE, Os_TaskIsRoundRobinNeeded,
(
  P2CONST(Os_TaskConfigType, AUTOMATIC, OS_CONST) Task
));
 ...
 *  \details      For further details see ActivateTaskAsyn().
 *
 *  \param[in]    TaskID            See ActivateTaskAsyn()
 *
 *  \retval       OS_STATUS_OK              No error.
 *  \retval       OS_STATUS_ID_1            (EXTENDED status:) Invalid TaskID.
 *  \retval       OS_STATUS_CALLEVEL        (EXTENDED status:) Called from invalid context.
 *  \retval       OS_STATUS_DISABLEDINT     (Service Protection:) Caller is in interrupt API sequence.
 *  \retval       OS_STATUS_ACCESSRIGHTS_1  (Service Protection:) Caller's access rights are not sufficient.
 *  \retval       OS_STATUS_ID_1            Asynchronous Local Task activation.
 *
 *  \context      TASK|ISR2
 *
 *  \reentrant    TRUE
 *  \synchronous  FALSE
 *
 *  \pre          See ActivateTaskAsyn().
 *
 *  \trace        CREQ-Os-AsynCrossCoreTaskActivation, CREQ-40
 **********************************************************************************************************************/
FUNC(Os_StatusType, OS_CODE) Os_Api_ActivateTaskAsyn(TaskType TaskID);
