...
FUNC(void, OS_CODE) Os_Hal_CoreInit(P2CONST(Os_Hal_CoreAsrConfigType, AUTOMATIC, OS_CONST) Config)
{
  ...
  Os_Hal_Mtcr(OS_HAL_SYSCON_CORECON_OFFSET, Os_Hal_Mfcr(OS_HAL_SYSCON_CORECON_OFFSET) | OS_HAL_SYSCON_CORECON_IS_MASK | OS_HAL_SYSCON_CORECON_TS_MASK);
  Os_Hal_Isync();

  /* #30 Initialize BTV and BIV */
  Os_Hal_InitBtvBiv(Config);                                                                                            /* SBSW_OS_HAL_FC_CALLER */

  /* #40 Restore the previous interrupt global state. */
...
{
  while( (1 == 1) )                                                                                                     /* COV_OS_HALCOREFREEZE */ /* FETA_OS_HAL_COREFREEZE */
  {
...
\ID COV_OS_HAL_MFD15
   \ACCEPT XX
   \REASON This inline function is used within macros in the file "Os_Hal_Entry_Lcfg.c". The file is not analyzed in our
           testsuite. Nevertheless it is assured (by review of the assembler output) that the macro is used.
...
\ID COV_OS_HALPLATFORMISRMAPPINGAVAILABILITY
  \ACCEPT XX
  \ACCEPT XF
  \REASON As the interrupt mapping functionality is not supported on the TriCore platform, conditions are always false and
          functions will never be called. The unnecessary code parts should be removed by compiler optimization.
          As the tests run on each platform the OS is to be released for, we always just test the code for the
          respective platform. No risk as the remaining code shall be optimized away.
...
\ID SBSW_OS_HAL_PWA_CORECONFIG_BOOTCON
 \DESCRIPTION    A write access is performed on the BOOTCON register of a foreign core. The address is generated in
                 the corresponding core configuration of type Os_Hal_CoreConfigType.
 \COUNTERMEASURE \S Safety manual should state that generated constant variable of Os_Hal_CoreConfigType in
                 Os_Hal_Core_Lcfg.c needs to be checked. More specifically, the attribute ".BOOTCONRegister"
                 must be OS_HAL_CORE<X>_BOOTCON, where <X> is the core ID. SMI-

\ID SBSW_OS_HAL_PWA_CORECONFIG_SYSCON_CORECON
 \DESCRIPTION    A write access is performed on the SYSCON register of a foreign core. The address is generated in
                 the corresponding core configuration of type Os_Hal_CoreConfigType.
 \COUNTERMEASURE \S Safety manual should state that generated constant variable of Os_Hal_CoreConfigType in
                 Os_Hal_Core_Lcfg.c needs to be checked. More specifically, the attribute ".SYSCON_CORECON_Register"
...
\ID FETA_OS_HAL_COREFREEZE
 \DESCRIPTION    The loop is the OS core freeze loop. It is used in case of a catastrophic failure.
 \COUNTERMEASURE \N The loop is endless by design. The system is intended to be reset by a watchdog at this point.

\ID FETA_OS_HAL_DUMMYREAD
 \DESCRIPTION    The loop is used by static code analysis tools.
 \COUNTERMEASURE \N The loop is never used in a productive system and is only active for static code analysis tools.

\ID FETA_OS_HAL_MPU_REGIONS_NBR
 \DESCRIPTION    The loop iterates over all configured MPU regions.
 \COUNTERMEASURE \N The loop finite index, increases till it reaches its maximum finite value :
                    the number of configured MPU regions.
                    The loop index is not manipulated by the loop. So, the number of iterations is finite.

\ID FETA_OS_HAL_STARTUP_CSA_CLEANUP
 \DESCRIPTION    The loop cleans up all CSAs created by the startup code.
 \COUNTERMEASURE \N The number of startup CSAs is finite.
                    The loop is performed until the PCX field of PCXI register is equal to 0.
                    Once all startup CSAs are cleared, PCX field contains its reset value 0
                    and the loop is left.
