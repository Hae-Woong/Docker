...
))
{
  ...
  /* #20 Clear the bits for priority, VM, cyber security and core assignment. */
  tempSRC &= (~OS_HAL_INT_SRC_SRPN_MASK & ~OS_HAL_INT_SRC_VM_MASK & ~OS_HAL_SRC_CS_MASK & ~OS_HAL_INT_SRC_TOS_MASK );

  /* #30 Set the required priority, cyber security and core assignment. */
  tempSRC |= (InterruptSource->Level | OS_HAL_SRC_CS_MASK | (((uint32)(InterruptSource->CoreAssignment)) << OS_HAL_INT_SRC_TOS_SHIFTVALUE));  /* PRQA S 2985 */ /* MD_Os_Hal_Rule2.2_2985 */

  /* #40 Set the current virtual machine number */
  OS_HAL_INT_SETCURRENTVM(tempSRC);

  /* #50 Write back to the register. */
