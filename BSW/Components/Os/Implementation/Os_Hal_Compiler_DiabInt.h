 ...
 * "WIND RIVER DIAB COMPILER USER'S GUIDE FOR TRICORE"
 *********************************************************************************************************************/
/***********************************************************************************************************************
 *  REVISION HISTORY
 *  --------------------------------------------------------------------------------------------------------------------
...
#ifndef OS_HAL_COMPILER_DIABINT_H
# define OS_HAL_COMPILER_DIABINT_H
...
/*! \brief include compiler header for using the intrisics */
...
/*! \brief  Declaration of code section for assembler */
# define OS_HAL_CODE_SECTION(x)     Os_Hal_Asm("   .section   \"."OS_HAL_EXPAND(x)"\"");

/*! \brief  Declaration of code section for assembler */
# define OS_HAL_OS_CODE_SECTION()   Os_Hal_Asm("   .section   \".OS_CODE\"");

/*! \brief   Prevent usage of keywords: "static inline" for functions called from Os_TrapHandler().
 *  \details The Windriver DiabData compiler uses too much stack in case of inlining into Os_TrapHandler().
 *           Therefore, we prevent inlining into Os_TrapHandler().*/
# define OS_TRAP_CALLINLINE1

/*! \brief   Prevent inlining into Os_TrapHandler().
 *  \details The Windriver DiabData compiler uses too much stack in case of inlining into Os_TrapHandler().
 *           Therefore, we prevent inlining into Os_TrapHandler().*/
# define OS_TRAP_CALLINLINE2 __attribute__((noinline))
...
# define Os_Hal_ExceptionSectionDeclaration(core)
# define Os_Hal_InterruptSectionDeclaration(core)
...
# define Os_Hal_UnhandledTrapEntry(core, class) \
  __asm ("  .section .OS_EXCVEC_CORE"#core"_CODE,,c"); \
  __asm ("  .alignn 32"); \
  ...
  __asm ("  movh.a %a14, Os_Hal_UnhandledExceptionHandler@ha");       /* load address of Os_Hal_UnhandledExceptionHandler */ \
  __asm ("  lea %a14,[%a14]Os_Hal_UnhandledExceptionHandler@l");      /* load address of Os_Hal_UnhandledExceptionHandler */ \
...
# define Os_Hal_MemoryTrapEntry(core, class) \
  ...
  __asm ("  movh %d4, " #class);                                      /* prepare parameters for C-Function; D4 = Exception class */\
  __asm ("  or %d4, %d15");                                           /* prepare parameters for C-Function; D4 |= TIN (D15) */\
  __asm ("  movh.a %a14, Os_Hal_MemFaultExceptionHandler@ha");        /* load address of Os_Hal_MemFaultExceptionHandler */ \
  __asm ("  lea %a14,[%a14]Os_Hal_MemFaultExceptionHandler@l");       /* load address of Os_Hal_MemFaultExceptionHandler */ \
  __asm ("  ji %a14 ");                                               /* jump to the handler */\
  __asm ("  .section .text");
...
/*! \brief         Abstraction of user defined exception entry for class 3 (context management) exception.
 *  \details       This macro function is used for user configured class 3 exception. User has to define his
 *                 own exception handler.
 *  \param[in]     core       On which core the entry is registered. It must be in range [0, 6].
 *  \param[in]     class      For which class the entry is registered. It must be in range [0, 7].
 *  \param[in]     UserFunc   The user function, which is to be called.
 ...
 *  \pre           -
 */
# define Os_Hal_UserTrapEntryClass3(core, class, UserFunc) \
  __asm ("  .section .OS_EXCVEC_CORE"#core"_CODE,,c"); \
  __asm ("  .alignn 32"); \
  __asm ("  .global osTrap_" #class "_Core" #core); \
  __asm ("osTrap_" #class "_Core" #core ":"); \
  __asm ("  movh.a  %a15, " #UserFunc "@ha"); \
  __asm ("  lea  %a15, [%a15]" #UserFunc "@l"); \
  __asm ("  ji %a15");\
  __asm ("  .section .text");
...
# define Os_Hal_Cat2InterruptEntry(core, level, systemlevel, isrconfig) \
  __asm ("  .section .OS_INTVEC_CORE"#core"_CODE,,c"); \
  __asm ("  .alignn 32"); \
  __asm ("  .global osIsrLevel_" #level "_Core" #core); \
  __asm ("osIsrLevel_" #level "_Core" #core ": svlcx"); \
  __asm ("  movh.a  %a4, "#isrconfig"@ha"); \
  __asm ("  lea  %a4, [%a4]"#isrconfig"@l"); \
 ...
 *  \pre           -
 */
asm volatile void Os_Hal_Mta11(uint32 x)                                                                                /* COMP_WARN_OS_HAL_UNUSED_FUNCTION */
{
%reg x
  mov.a %a11, x
}
 ...
 *  \pre           -
 */
asm volatile uint32 Os_Hal_Mfa10(void)                                                                                  /* COMP_WARN_OS_HAL_UNUSED_FUNCTION */
{
! "%d2"
  mov.d %d2, %a10
}
 ...
 *  \pre           -
 */
asm volatile uint32 Os_Hal_Mfa11(void)                                                                                  /* COMP_WARN_OS_HAL_UNUSED_FUNCTION */
{
! "%d2"
  mov.d %d2, %a11
}
 ...
 *  \pre           -
 */
asm volatile uint32 Os_Hal_Mfa8(void)                                                                                   /* COMP_WARN_OS_HAL_UNUSED_FUNCTION */
{
! "%d2"
  mov.d %d2, %a8
}
 ...
 *  \pre           -
 */
asm volatile uint32 Os_Hal_Mfa9(void)
{
! "%d2"
  mov.d %d2, %a9
}
 ...
 *  \pre           -
 */
asm volatile uint32 Os_Hal_Mfa0(void)
{
! "%d2"
  mov.d %d2, %a0
}
 ...
 *  \pre           -
 */
asm volatile uint32 Os_Hal_Mfa1(void)
{
! "%d2"
  mov.d %d2, %a1
}
 ...
 *  \pre           -
 */
asm volatile uint32 Os_Hal_Mfd15(void)
{
! "%d2"
  mov %d2, %d15
}
 ...
 *  \pre           -
 */
asm volatile uint32 Os_Hal_CmpSwap(volatile void* Spinlock, uint32 CoreId, uint32 SpinlockValue)                        /* COMP_WARN_OS_HAL_UNUSED_FUNCTION */
{
% reg Spinlock, CoreId, SpinlockValue;
! "%d2", "%d3"
  mov        %e2, SpinlockValue, CoreId    /* D2 = CoreId, D3 = SpinlockValue */
  cmpswap.w  [Spinlock]0,%e2               /* swaps into D2 -> which is automatically the return value */
}

# define OS_STOP_SEC_CODE
# include "Os_MemMap_OsCode.h"                                                                                          /* PRQA S 5087 */ /* MD_MSR_MemMap */

#endif /* OS_HAL_COMPILER_DIABINT_H */
