...
#define OS_COUNTER_CASTCONFIG_TIMERSW_2_COUNTER(timer)                (&((timer).Counter))


/*! Type cast from Os_TimerPitConfigType to Os_CounterConfigType by use of base element addressing. */
#define OS_COUNTER_CASTCONFIG_TIMERPIT_2_COUNTER(timer)               (&((timer).SwCounter.Counter))


/*! Type cast from Os_TimerHrtConfigType to Os_CounterConfigType by use of base element addressing. */
#define OS_COUNTER_CASTCONFIG_TIMERHRT_2_COUNTER(timer)               (&((timer).Counter))


/*! Type cast from Os_TimerPfrtConfigType to Os_CounterConfigType by use of base element addressing. */
#define OS_COUNTER_CASTCONFIG_TIMERPFRT_2_COUNTER(timer)              (&((timer).SwCounter.Counter))


/*! Type cast from Os_PriorityQueueConfigType to Os_CollectionConfigType by use of base element addressing. */
#define OS_COUNTER_CASTCONFIG_PRIORITYQUEUE_2_COLLECTION(queue)       (P2CONST(Os_CollectionConfigType, AUTOMATIC, \
                                                                       OS_CONST))(&queue)


/*! Type cast from Os_ArrayListConfigType to Os_CollectionConfigType by use of base element addressing. */
#define OS_COUNTER_CASTCONFIG_ARRAYLIST_2_COLLECTION(list)            (&(list.Collection))
...
struct Os_CounterConfigType_Tag
{
  ...
  /*! Jobs which are currently mapped to the counter. */
  P2CONST(Os_CollectionConfigType, TYPEDEF, OS_CONST) Collection;

  /*! Counter's collection type. */
  Os_CollectionTypeType CollectionType;
 ...
 *  \pre          -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE Os_StdReturnType, OS_CODE, OS_ALWAYS_INLINE, Os_CounterHasQueue,
(
  void
));
 ...
 *  Os_CounterHasList()
 **********************************************************************************************************************/
/*! \brief        Returns whether any array list is configured or not.
 *  \details      --no details--
 *
 *  \retval       !0   If any array list is configured.
 *  \retval       0    If no array list is configured.
 *
 *  \context      ANY
 *
 *  \reentrant    TRUE
 ...
 *  \pre          -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE Os_StdReturnType, OS_CODE, OS_ALWAYS_INLINE, Os_CounterHasList,
(
  void
));
...
/***********************************************************************************************************************
 *  Os_CounterCollectionIsList()
 **********************************************************************************************************************/
/*! \brief        Returns whether the collection type of the counter is an array list or not.
 *  \details      --no details--
 *
 * \param[in]     Counter   The counter to query. Parameter must not be NULL.
 *
 *  \retval       !0   If the collection type is an array list.
 *  \retval       0    If the collection type is not an array list.
 *
 *  \context      ANY
 *
 *  \reentrant    TRUE
 ...
 *  \pre          -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE Os_StdReturnType, OS_CODE, OS_ALWAYS_INLINE, Os_CounterCollectionIsList,
(
...
/***********************************************************************************************************************
 *  Os_CounterCollectionIsQueue()
 **********************************************************************************************************************/
/*! \brief        Returns whether the collection type of the counter is a priority queue or not.
 *  \details      --no details--
 *
 * \param[in]     Counter   The counter to query. Parameter must not be NULL.
 *
 *  \retval       !0   If the collection type is a priority queue.
 *  \retval       0    If the collection type is not a priority queue.
 *
 *  \context      ANY
 *
 *  \reentrant    TRUE
 ...
 *  \pre          -
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE Os_StdReturnType, OS_CODE, OS_ALWAYS_INLINE, Os_CounterCollectionIsQueue,
(
  P2CONST(Os_CounterConfigType, AUTOMATIC, OS_CONST) Counter
));
 ...
 *  \details      see Os_CounterGetPhysicalValueInternal().
 *
 *  \param[in]    Counter           The counter to query. Parameter must not be NULL.
 *
 *  \context      OS_INTERNAL
 ...
 *                  If the collection type is a priority queue and if the counter is a HRT counter, it will update the
 *                  compare value of the hardware timer for the next future job.
 *
 ...
 *  Os_CounterAddRelJob()
 **********************************************************************************************************************/
/*! \brief          Add a job to counter's job collection with given expiration time (relative to now).
 *  \details        --no details--
 *
 *  \param[in,out]  Counter    The counter, which shall work of the job. Parameter must not be NULL.
 *  \param[in,out]  Job        Job to add. Parameter must not be NULL.
 ...
 *  Os_CounterAddAbsJob()
 **********************************************************************************************************************/
/*! \brief          Add a given job to counter's job collection with given absolute expiration time.
 *  \details        --no details--
 *
 *  \param[in,out]  Counter    The counter, which shall work of the job. Parameter must not be NULL.
 *  \param[in,out]  Job        Job to add. Parameter must not be NULL.
 ...
 *  Os_CounterReloadJob()
 **********************************************************************************************************************/
/*! \brief          Add a given job to counter's job collection relative to its current expiration time with given
 *                  offset.
 *  \details        --no details--
 *
 *  \param[in,out]  Counter    The counter, which shall work of the job. Parameter must not be NULL.
 *  \param[in,out]  Job        Job to reload. Parameter must not be NULL.
 ...
 *  Os_CounterDeleteJob()
 **********************************************************************************************************************/
/*! \brief          Delete a given job from counter's job collection.
 *  \details        If the collection type is a priority queue, the deleted job was the new highest prio job and the
 *                  counter is a HRT counter, it will update the compare value of the hardware timer.
 *
 *  \param[in,out]  Counter  The counter, which shall delete the job from its collection. Parameter must not be NULL.
