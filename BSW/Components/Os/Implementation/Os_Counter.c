...
/*! The maximum number of job executions per interrupt lock */
# define MAX_JOB_EXECS_PER_LOCK     4u
...
/***********************************************************************************************************************
 *  Os_CounterGetPhysicalValueInternal()
 **********************************************************************************************************************/
 ...
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE TickType, OS_CODE, OS_ALWAYS_INLINE, Os_CounterGetPhysicalValueInternal,
(
  P2CONST(Os_CounterConfigType, AUTOMATIC, OS_CONST) Counter
));
...
/***********************************************************************************************************************
 *  Os_CounterCollection2Queue()
 **********************************************************************************************************************/
/*! \brief          Returns the priority queue corresponding to the given collection.
 *  \details        --no details--
 *
 *  \param[in,out]  Collection         The collection to query. Parameter must not be NULL.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 *  \synchronous    TRUE
 *
 *  \pre            Collection type of counter is a priority queue.
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE P2CONST(Os_PriorityQueueConfigType, AUTOMATIC, OS_CONST), OS_CODE,
OS_ALWAYS_INLINE, Os_CounterCollection2Queue,
(
  P2CONST(Os_CollectionConfigType, AUTOMATIC, OS_CONST) Collection
));


/***********************************************************************************************************************
 *  Os_CounterCollection2List()
 **********************************************************************************************************************/
/*! \brief          Returns the array list corresponding to the given collection.
 *  \details        --no details--
 *
 *  \param[in,out]  Collection         The collection to query. Parameter must not be NULL.
 *
 ...
 *  \synchronous    TRUE
 *
 *  \pre            Collection type of counter is an array list.
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE P2CONST(Os_ArrayListConfigType, AUTOMATIC, OS_CONST), OS_CODE,
OS_ALWAYS_INLINE, Os_CounterCollection2List,
(
  P2CONST(Os_CollectionConfigType, AUTOMATIC, OS_CONST) Collection
));
...
/***********************************************************************************************************************
 *  Os_CounterWorkJobsQueue()
 **********************************************************************************************************************/
/*! \brief          Works off all jobs until now in case counter's collection type is a priority queue.
 *  \details        This function works off all jobs, which are queued to the given counter and expired until now.
                    ...
                    next future job.
 *
 *  \param[in,out]  Counter         The counter, which shall work off its jobs. Parameter must not be NULL.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *  \pre            Concurrent access to given object is prevented by caller.
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_CounterWorkJobsQueue,
(
  P2CONST(Os_CounterConfigType, AUTOMATIC, OS_CONST) Counter
));
...
/***********************************************************************************************************************
 *  Os_CounterWorkJobsList()
 **********************************************************************************************************************/
/*! \brief          Works off all jobs until now in case counter's collection type is an array list.
 *  \details        This function works off all jobs, which are mapped to the given counter and expired until now.
 *
 *  \param[in,out]  Counter         The counter, which shall work off its jobs. Parameter must not be NULL.
 *
 *  \context        OS_INTERNAL
 ...
 *  \pre            Concurrent access to given object is prevented by caller.
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_CounterWorkJobsList,
(
  P2CONST(Os_CounterConfigType, AUTOMATIC, OS_CONST) Counter
));
...
/***********************************************************************************************************************
 *  Os_CounterCollectionInsert()
 **********************************************************************************************************************/
/*! \brief          Add a job to counter's job collection.
 *  \details        If the collection type is a priority queue, the job is the new highest prio job and the counter is
 *                  a HRT counter, it will update the compare value of the hardware timer.
 *
 *  \param[in,out]  Counter         The counter, which shall work of the job. Parameter must not be NULL.
 *  \param[in]      Job             Job to add. Parameter must not be NULL.
 *  \param[in]      CompareValue    The compare value to set.
 *                                  CompareValue is in range 0 <= CompareValue <= MaxCountingValue.
 *                                  CompareValue-Now is in range -MaxAllowedValue to +MaxAllowedValue.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_CounterCollectionInsert,
(
  P2CONST(Os_CounterConfigType, AUTOMATIC, OS_CONST) Counter,
  P2CONST(Os_JobConfigType, AUTOMATIC, OS_APPL_DATA) Job,
...
{
  return (Os_StdReturnType)(OS_CFG_COUNTER_SW_USED == STD_ON);                                                          /* PRQA S 4304, 2995, 2996 */ /* MD_Os_C90BooleanCompatibility, MD_Os_Rule2.2_2995, MD_Os_Rule2.2_2996 */
}
...
))
{
  ...
  if(OS_LIKELY(Os_CoreAsrIsCoreLocal(Counter->Core) != 0u))                                                             /* SBSW_OS_FC_PRECONDITION */ /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */ /* PRQA S 2991, 2995 */ /* MD_Os_Rule14.3_2991, MD_Os_Rule2.2_2995 */
  {
...
/* PRQA S 6050 1 */ /* MD_MSR_STCAL */
OS_FUNC_ATTRIBUTE_DEFINITION(
OS_LOCAL_INLINE TickType, OS_CODE, OS_ALWAYS_INLINE, Os_CounterGetPhysicalValueInternal,
(
  P2CONST(Os_CounterConfigType, AUTOMATIC, OS_CONST) Counter
))
{
...
))
{
  ...
  Os_TickType counterValue = Os_CounterGetPhysicalValueInternal(Counter);                                               /* SBSW_OS_FC_PRECONDITION */
 ...
 **********************************************************************************************************************/
FUNC(Os_TickType, OS_CODE) Os_CounterGetPhysicalValue
(
  P2CONST(Os_CounterConfigType, AUTOMATIC, OS_CONST) Counter
)
{
  return Os_CounterGetPhysicalValueInternal(Counter);                                                                   /* SBSW_OS_FC_PRECONDITION */
}
...
)
{
  ...
  /* #20 Perform modulo. */
  if(Value < Sub)                                                                                                       /* COV_OS_NOFUNCCALL_XF */
  {
 ...
 *  Os_CounterCollection2Queue()
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE P2CONST(Os_PriorityQueueConfigType, AUTOMATIC, OS_CONST), OS_CODE,
OS_ALWAYS_INLINE, Os_CounterCollection2Queue,
(
  P2CONST(Os_CollectionConfigType, AUTOMATIC, OS_CONST) Collection
))
{
  return (P2CONST(Os_PriorityQueueConfigType, AUTOMATIC, OS_CONST))Collection;
}
 ...
 *  Os_CounterCollection2List()
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE P2CONST(Os_ArrayListConfigType, AUTOMATIC, OS_CONST), OS_CODE,
OS_ALWAYS_INLINE, Os_CounterCollection2List,
(
  P2CONST(Os_CollectionConfigType, AUTOMATIC, OS_CONST) Collection
))
{
  return (P2CONST(Os_ArrayListConfigType, AUTOMATIC, OS_CONST))Collection;                                              /* PRQA S 0310 */ /* MD_Os_Rule11.3_0310 */
}
 ...
 */
/* PRQA S 6050 1 */ /* MD_MSR_STCAL */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_CounterWorkJobsQueue,
(
  P2CONST(Os_CounterConfigType, AUTOMATIC, OS_CONST) Counter
))
{
  P2CONST(Os_JobConfigType, AUTOMATIC, OS_CONST) job;
  P2CONST(Os_PriorityQueueConfigType, AUTOMATIC, OS_CONST) jobQueue = Os_CounterCollection2Queue(Counter->Collection);  /* SBSW_OS_CNT_COLLECTION2QUEUE_001 */
  Os_IntStateType interruptState;
  uint8 jobCounter = 0u;
  ...
  job = Os_PriorityQueueTopGet(jobQueue);                                                                               /* SBSW_OS_CNT_PRIORITYQUEUEFUNCTION_001 */

  /* #20 Repeat until queue is empty OR there are no expired jobs in the queue: */
  while(OS_LIKELY(job != NULL_PTR))                                                                                     /* FETA_OS_COUNTERWORKJOBSQUEUE */
  {
    /* #30 Get counter's current value. (We have a continuously changing FRT here.) */
    Os_TickType now = Os_CounterGetPhysicalValueInternal(Counter);                                                      /* SBSW_OS_FC_PRECONDITION */
    ...
    {
      /* #50 Dequeue the job. */
      Os_PriorityQueueDeleteTop(jobQueue);                                                                              /* SBSW_OS_CNT_PRIORITYQUEUEFUNCTION_001 */

      /* #60 Work the job off. */
      Os_JobDo(job);                                                                                                    /* SBSW_OS_CNT_JOBDO_001 */
 ...
 */
/* PRQA S 6050 1 */ /* MD_MSR_STCAL */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_CounterWorkJobsList,
(
  P2CONST(Os_CounterConfigType, AUTOMATIC, OS_CONST) Counter
))
{
  P2CONST(Os_JobConfigType, AUTOMATIC, OS_CONST) job;
  P2CONST(Os_ArrayListConfigType, AUTOMATIC, OS_CONST) jobList = Os_CounterCollection2List(Counter->Collection);        /* SBSW_OS_CNT_COLLECTION2LIST_001 */
  Os_IntStateType interruptState;
  uint8 jobCounter = 0u;
  Os_TickType now;
  ...
  /* #20 Get counter's current value. */
  now = Os_CounterGetPhysicalValueInternal(Counter);                                                                    /* SBSW_OS_FC_PRECONDITION */

  job = Os_ArrayListGetIterNode(jobList);                                                                               /* SBSW_OS_CNT_ARRAYLISTFUNCTION_001 */

  /* #30 Repeat until the end of the list: */
  while(OS_LIKELY(job != NULL_PTR))                                                                                     /* FETA_OS_COUNTERWORKJOBSLIST*/
  {
    /* #40 If the job is expired: */
    if(job->Dyn->ExpirationTimestamp == now)
    {
      /* #50 Delete the job. */
      Os_ArrayListDelIterNode(jobList);                                                                                 /* SBSW_OS_CNT_ARRAYLISTFUNCTION_001 */

      /* #60 Work the job off. */
      Os_JobDo(job);                                                                                                    /* SBSW_OS_CNT_JOBDO_002 */
    ...
    /* #100 Otherwise increment the list iterator. */
    else
    {
      Os_ArrayListIncrementIter(jobList);                                                                               /* SBSW_OS_CNT_ARRAYLISTFUNCTION_001 */
    }

    /* #110 Get next job from list. */
    job = Os_ArrayListGetIterNode(jobList);                                                                             /* SBSW_OS_CNT_ARRAYLISTFUNCTION_001 */
  }

  /* #120 Reset the list iterator for next counter cycle. */
  Os_ArrayListResetIter(jobList);                                                                                       /* SBSW_OS_CNT_ARRAYLISTFUNCTION_001 */

  /* #130 Resume all interrupts. */
  Os_IntResume(&interruptState);                                                                                        /* SBSW_OS_FC_POINTER2LOCAL */
}
...
)
{
  ...
  /* #20 Initialize the respective collection. */
  if ((Os_CounterHasQueue() != 0u) && (Counter->CollectionType == OS_COLLECTIONTYPE_PRIORITYQUEUE))                     /* PRQA S 2992, 2995, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2995, MD_Os_Rule2.2_2996 */
  {
    Os_PriorityQueueInit(Os_CounterCollection2Queue(Counter->Collection));                                              /* SBSW_OS_CNT_PRIORITYQUEUEFUNCTION_001 */ /* SBSW_OS_CNT_COLLECTION2QUEUE_001 */
  }
  else if ((Os_CounterHasList() != 0u) && (Counter->CollectionType == OS_COLLECTIONTYPE_ARRAYLIST))                     /* PRQA S 2992, 2995, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2995, MD_Os_Rule2.2_2996 */
  {
    Os_ArrayListInit(Os_CounterCollection2List(Counter->Collection));                                                   /* SBSW_OS_CNT_ARRAYLISTFUNCTION_001 */ /* SBSW_OS_CNT_COLLECTION2LIST_001 */
  }
  else
  {
...
{
  /* #10 Work off jobs on the respective collection. */
  if ((Os_CounterHasQueue() != 0u) && (Counter->CollectionType == OS_COLLECTIONTYPE_PRIORITYQUEUE))                     /* PRQA S 2992, 2995, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2995, MD_Os_Rule2.2_2996 */
  {
    Os_CounterWorkJobsQueue(Counter);                                                                                   /* SBSW_OS_FC_PRECONDITION */
  }
  else if ((Os_CounterHasList() != 0u) && (Counter->CollectionType == OS_COLLECTIONTYPE_ARRAYLIST))                     /* PRQA S 2992, 2995, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2995, MD_Os_Rule2.2_2996 */
  {
    Os_CounterWorkJobsList(Counter);                                                                                    /* SBSW_OS_FC_PRECONDITION */
  }
  else
  {
    Os_ErrKernelPanic(OS_STATUS_PANIC_UNEXPECTED_CONTROL_FLOW);
 ...
 *  Os_Counter2TimerPit()
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DEFINITION(                                                                                           /* COV_OS_HALPLATFORMPITUNSUPPORTED_XX */
 ...
 *  Os_Counter2TimerPfrt()
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE P2CONST(Os_TimerPfrtConfigType, AUTOMATIC, OS_CONST), OS_CODE,             /* COV_OS_HALPLATFORMPFRTUNSUPPORTED_XX */
OS_ALWAYS_INLINE,
Os_Counter2TimerPfrt,
(
  P2CONST(Os_CounterConfigType, AUTOMATIC, OS_CONST) Counter
...
)
{
  ...
  /* #30 Add the given job to the respective collection. */
  Os_CounterCollectionInsert(Counter, Job, newExpTime);                                                                 /* SBSW_OS_FC_PRECONDITION */
}
...
)
{
  ...
  /* #30 Add the given job to the respective collection. */
  Os_CounterCollectionInsert(Counter, Job, absStart);                                                                   /* SBSW_OS_FC_PRECONDITION */
}
...
{
  P2CONST(Os_CollectionConfigType, AUTOMATIC, OS_CONST) jobCollection = Counter->Collection;

  /* #10 If the collection is a priority queue: */
  if ((Os_CounterHasQueue() != 0u) && (Counter->CollectionType == OS_COLLECTIONTYPE_PRIORITYQUEUE))                     /* PRQA S 2992, 2995, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2995, MD_Os_Rule2.2_2996 */
  {
    P2CONST(Os_PriorityQueueConfigType, AUTOMATIC, OS_CONST) queue = Os_CounterCollection2Queue(jobCollection);         /* SBSW_OS_CNT_COLLECTION2QUEUE_001 */
    P2CONST(Os_JobConfigType, AUTOMATIC, OS_CONST) topJob;
    Os_CollectionDelResultType queueResult;

    topJob = Os_PriorityQueueTopGet(queue);                                                                             /* SBSW_OS_CNT_PRIORITYQUEUEFUNCTION_001 */
    Os_Assert((Os_StdReturnType)(topJob != NULL_PTR));                                                                  /* PRQA S 4304 */ /* MD_Os_C90BooleanCompatibility */

    /* #20 Delete the job from the priority queue. */
    queueResult = Os_PriorityQueueDelete(queue, Job);                                                                   /* SBSW_OS_CNT_PRIORITYQUEUEFUNCTION_002 */
    Os_Assert((Os_StdReturnType)(queueResult == OS_COLLECTIONDELETE_OK));                                               /* PRQA S 4304 */ /* MD_Os_C90BooleanCompatibility */
 ...
 * */
  ...
  /* #70 Otherwise, delete the job from the array list. */
  else if ((Os_CounterHasList() != 0u) && (Counter->CollectionType == OS_COLLECTIONTYPE_ARRAYLIST))                     /* PRQA S 2992, 2995, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2995, MD_Os_Rule2.2_2996 */
  {
    Os_CollectionDelResultType listResult = Os_ArrayListDelete(Os_CounterCollection2List(jobCollection), Job);          /* SBSW_OS_CNT_ARRAYLISTFUNCTION_002 */ /* SBSW_OS_CNT_COLLECTION2LIST_001 */
    Os_Assert((Os_StdReturnType)(listResult == OS_COLLECTIONDELETE_OK));                                                /* PRQA S 4304 */ /* MD_Os_C90BooleanCompatibility */
  }
  else
  {
    Os_ErrKernelPanic(OS_STATUS_PANIC_UNEXPECTED_CONTROL_FLOW);
 ...
 */
/* PRQA S 6050 1 */ /* MD_MSR_STCAL */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_CounterCollectionInsert,
(
  P2CONST(Os_CounterConfigType, AUTOMATIC, OS_CONST) Counter,
  P2CONST(Os_JobConfigType, AUTOMATIC, OS_APPL_DATA) Job,
  Os_TickType CompareValue
...
{
  P2CONST(Os_CollectionConfigType, AUTOMATIC, OS_CONST) jobCollection;

  jobCollection = Counter->Collection;
  ...
  if ((Os_CounterHasQueue() != 0u) && (Counter->CollectionType == OS_COLLECTIONTYPE_PRIORITYQUEUE))                     /* PRQA S 2992, 2995, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2995, MD_Os_Rule2.2_2996 */
  {
    ...
    /* #20 Add the given job to the priority queue. */
    Os_PriorityQueueInsert(queue, Job);                                                                                 /* SBSW_OS_CNT_PRIORITYQUEUEFUNCTION_002 */

    /* #30 If the top element of the queue has changed: */
    if(Job == Os_PriorityQueueTopGet(queue))                                                                            /* SBSW_OS_CNT_PRIORITYQUEUEFUNCTION_001 */
    {
      /* #40 Update timer's compare value. */
      Os_CounterSetCompareValue(Counter, CompareValue);                                                                 /* SBSW_OS_FC_PRECONDITION */
    }
  }
  /* #50 Otherwise, if the collection is an array list: */
  else if ((Os_CounterHasList() != 0u) && (Counter->CollectionType == OS_COLLECTIONTYPE_ARRAYLIST))                     /* PRQA S 2992, 2995, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2995, MD_Os_Rule2.2_2996 */
  {
    /* #60 Add the given job to the array list. */
    Os_ArrayListInsert(Os_CounterCollection2List(jobCollection), Job);                                                  /* SBSW_OS_CNT_ARRAYLISTFUNCTION_002 */ /* SBSW_OS_CNT_COLLECTION2LIST_001 */
  }
  /* #70 Otherwise: */
  else
  {
    /* #80 Enter KernelPanic. */
    Os_ErrKernelPanic(OS_STATUS_PANIC_UNEXPECTED_CONTROL_FLOW);
  }
}
...
)
{
  ...
  else
  {
    ...
    else if(OS_UNLIKELY(Os_CoreAsrCheckIsCoreLocal(me->Core) == OS_CHECK_FAILED))                                       /* SBSW_OS_CNT_COREASRISCORELOCAL_001 */ /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */ /* PRQA S 2992, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2996 */
    {
...
\ID SBSW_OS_CNT_COLLECTION2QUEUE_001
 \DESCRIPTION     Os_CounterCollection2Queue is called with the collection of a counter object which is passed
                  as argument to the caller. Correctness of the argument is ensured by preconditions. The compliance
                  of the precondition is check during review.
 \COUNTERMEASURE  \M [CM_OS_COUNTERGETCOLLECTION_M]

\ID SBSW_OS_CNT_PRIORITYQUEUEFUNCTION_001
 \DESCRIPTION     An Os_PriorityQueue function is called with the return value of Os_CounterCollection2Queue.
                  Precondition ensures that collection type of counter is a priority queue. The compliance of the
                  precondition is check during review.
 \COUNTERMEASURE  \M [CM_OS_COUNTERCOLLECTION2QUEUE_01_02_M]
                  \R [CM_OS_COUNTERCOLLECTION2QUEUE_02_02_R]

\ID SBSW_OS_CNT_JOBDO_001
 \DESCRIPTION     Os_JobDo is called with the return value of Os_PriorityQueueTopGet.
 \COUNTERMEASURE  \R [CM_OS_PRIORITYQUEUETOPGET_R]

\ID SBSW_OS_CNT_JOBDO_002
 \DESCRIPTION     Os_JobDo is called with the return value of Os_ArrayListGetIterNode.
 \COUNTERMEASURE  \R [CM_OS_ARRAYLISTGETITERNODE_R]

 \ID SBSW_OS_CNT_COLLECTION2LIST_001
 \DESCRIPTION     Os_CounterCollection2List is called with the collection of a counter object which is passed
                  as argument to the caller. Correctness of the argument is ensured by preconditions. The compliance
                  of the precondition is check during review.
 \COUNTERMEASURE  \M [CM_OS_COUNTERGETCOLLECTION_M]

\ID SBSW_OS_CNT_ARRAYLISTFUNCTION_001
 \DESCRIPTION     An Os_ArrayList function is called with the return value of Os_CounterCollection2List.
                  Precondition ensures, that collection type of counter is an array list. The compliance of the
                  precondition is check during review.
 \COUNTERMEASURE  \M [CM_OS_COUNTERCOLLECTION2LIST_01_02_M]
                  \R [CM_OS_COUNTERCOLLECTION2LIST_02_02_R]

\ID SBSW_OS_CNT_PRIORITYQUEUEFUNCTION_002
 \DESCRIPTION     An Os_PriorityQueue function is called with two values:
                  1) The return value of Os_CounterCollection2Queue
                     Precondition ensures that collection type of counter is a priority queue.
                  2) A value passed as pointer to the caller function
                     Precondition ensures that the pointer is valid.
                  The compliance of the preconditions is check during review.
 \COUNTERMEASURE  \M [CM_OS_COUNTERCOLLECTION2QUEUE_01_02_M]
                  \R [CM_OS_COUNTERCOLLECTION2QUEUE_02_02_R]

\ID SBSW_OS_CNT_ARRAYLISTFUNCTION_002
 \DESCRIPTION     An Os_ArrayList function is called with two values:
                  1) The return value of Os_CounterCollection2List
                     Precondition ensures that collection type of counter is an array list.
                  2) A value passed as pointer to the caller function
                     Precondition ensures that the pointer is valid.
                  The compliance of the preconditions is check during review.
 \COUNTERMEASURE  \M [CM_OS_COUNTERCOLLECTION2LIST_01_02_M]
...
\CM CM_OS_COUNTERCOLLECTION2QUEUE_02_02_R
      Implementation ensures that converted object is an Os_PriorityQueueConfigType.
...
\CM CM_OS_COUNTERCOLLECTION2LIST_02_02_R
      Implementation ensures that converted object is an Os_ArrayListConfigType.
