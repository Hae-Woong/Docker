...
{
  const Os_FifoIdxType oldValue = *Index;
  Os_FifoIdxType newValue;
  ...
  /* #20  Store new value. */
  if(Visibility == OS_FIFO_WRITE_LOCAL)
  {
...
))
{
  ...
  Os_FifoIdxType nextWriteIdx = WriteIdx;
  Os_FifoIdxIncrement(&nextWriteIdx, Size, OS_FIFO_WRITE_LOCAL);                                                        /* SBSW_OS_FC_POINTER2ARGUMENT */

  /* #20  If the next write index wasn't read, then the buffer is full. */
  return (nextWriteIdx == ReadIdx);                                                                                     /* PRQA S 4404 */ /*  MD_Os_C90BooleanCompatibility */
}
