...
OS_LOCAL_INLINE void, OS_CODE, OS_NORETURN OS_ALWAYS_INLINE, Os_HookSuicide, (void))
{
  ...
  {
    /* #20 Panic! */
    Os_ErrKernelPanic(OS_STATUS_PANIC_INVALID_SUICIDE);
  }
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE void, OS_CODE, OS_ALWAYS_INLINE, Os_HookKill,                              /* PRQA S 3219 */ /* MD_Os_Rule2.1_3219 */
(
  P2CONST(Os_HookConfigType, AUTOMATIC, OS_CONST) Hook,
...
))
{
  ...
  if(Hook != NULL_PTR)
  {
    Os_ThreadKill(&(Hook->Thread), TerminationReason);                                                                  /* SBSW_OS_FC_PRECONDITION_AND_CHECK */
  }
}
