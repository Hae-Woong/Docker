...
# define OS_PANICHOOK(Status)        Os_PanicHook(Status)
#else
# define OS_PANICHOOK(Status)        Os_Hal_CoreFreeze()
#endif
...
{
  return (Os_StdReturnType)(OS_CFG_STARTUPHOOKS == STD_ON);                                                             /* PRQA S 4304, 2995 */ /* MD_Os_C90BooleanCompatibility, MD_Os_Rule2.2_2995 */
}
...
OS_FUNC_ATTRIBUTE_DEFINITION(void, OS_CODE, OS_NORETURN, Os_HookReturn, (void))
{
  ...
  activeHooks->InHookFlags = Os_HookGetDyn(currentHook)->HookTypeFlag;                                                  /* SBSW_OS_HK_COREGETHOOKINFO_002 */ /* SBSW_OS_HK_HOOKGETDYN_005 */
...
))
{
  ...
  {
    /* #30 Kernel Panic. */
    Os_ErrKernelPanic(OS_STATUS_PANIC_HOOK_NESTING);
  }

  /* - #40 Update current hook flag state. */
  Os_HookGetDyn(Hook)->HookTypeFlag = activeHooks->InHookFlags;                                                         /* SBSW_OS_FC_PRECONDITION */ /* SBSW_OS_HK_HOOKGETDYN_003 */
 ...
 *
 */
FUNC(StatusType, OS_CODE) Os_HookGetStatus
(
  P2CONST(Os_HookConfigType, AUTOMATIC, OS_CONST) Hook
)
{
  /* #10 Return the status parameter of the hook. */
  return Os_HookGetDyn(Hook)->Parameter.Status;                                                                         /* SBSW_OS_FC_PRECONDITION */
}
...
)
{
  ...
  {
    Os_ErrKernelPanic(OS_STATUS_PANIC_FUNCTIONALITY_NOT_AVAILABLE);                                                     /* PRQA S 2880 */ /* MD_Os_Rule2.1_2880 */
  }
  /* #20 Otherwise: */
  else
  {
    ...
    if(Hook != NULL_PTR)
    {
      ...
      Os_HookGetDyn(&(Hook->Hook))->Parameter.Status = Error;                                                           /* SBSW_OS_FC_PRECONDITION */ /* SBSW_OS_HK_HOOKGETDYN_003 */
...
)
{
  ...
  else
  {
    ...
    status = Os_TrapHookGetStatus(hook);                                                                                /* SBSW_OS_HK_TRAPHOOKGETSTATUS_001 */
...
)
{
  ...
  {
    /* #20 Kernel panic. */
    Os_ErrKernelPanic(OS_STATUS_PANIC_CORE_INIT_HOOK_NULL);
  }
  /* #30 Otherwise: */
  else
  {
    ...
    hookDyn->HookTypeFlag = OS_HOOKFLAG_NOHOOKS;                                                                        /* SBSW_OS_HK_HOOKGETDYN_003 */

    /* #60 Bypass mode parameter.                 */
    hookDyn->Parameter.Mode = Mode;                                                                                     /* SBSW_OS_HK_HOOKGETDYN_003 */
...
)
{
  ...
  mode = Os_HookGetDyn(hook)->Parameter.Mode;                                                                           /* SBSW_OS_HK_HOOKGETDYN_004 */ /* SBSW_OS_HK_HOOKGETDYN_003 */
...
)
{
  ...
  else
  {
    ...
    hookDyn->ReturnValue.ProtectionReaction = protectionReaction;                                                       /* SBSW_OS_HK_HOOKGETDYN_003 */
...
)
{
  ...
  if (Os_HookIsProtectionHookEnabled() != 0u)                                                                           /* PRQA S 2991, 2995 */ /* MD_Os_Rule14.3_2991, MD_Os_Rule2.2_2995 */ /* COV_OS_INVSTATE */
  {
    if(Hook != NULL_PTR)                                                                                                /* COV_OS_INVSTATE */
    {
      ...
      Os_HookGetDyn(&(Hook->Hook))->Parameter.Status = Fatalerror;                                                      /* SBSW_OS_FC_PRECONDITION */ /* SBSW_OS_HK_HOOKGETDYN_003 */
...
)
{
  ...
  if(Os_HookIsCallbackEnabled() != 0u)                                                                                  /* PRQA S 2991, 2995 */ /* MD_Os_Rule14.3_2991, MD_Os_Rule2.2_2995 */
  {
    if(Hook != NULL_PTR)
    {
      ...
      if(Os_CoreAsrIsCoreLocal(targetCore) != 0u)                                                                       /* SBSW_OS_HK_COREASRISCORELOCAL_001 */ /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */ /* PRQA S 2991, 2995 */ /* MD_Os_Rule14.3_2991, MD_Os_Rule2.2_2995 */
      {
 ...
 *
 */
FUNC(void, OS_CODE) Os_HookCallPanicHook(Os_PanicStatusType Status)
{
...
( void ))
{
  return (Os_StdReturnType)(OS_CFG_PANICHOOK == STD_ON);                                                                /* PRQA S 4304, 2995 */ /* MD_Os_C90BooleanCompatibility, MD_Os_Rule2.2_2995 */
}
...
\ID SBSW_OS_HK_HOOK2STATUSHOOK_001
 \DESCRIPTION    Os_Hook2StatusHook is called with the return value of Os_HookThread2Hook which is called with an
                 argument passed to the caller. As the type correctness of the argument is configuration dependent,
                 it cannot be guaranteed by means of implementation. Therefore, type correctness of the object is
                 checked by MSSV.
 \COUNTERMEASURE \M [CM_OS_HOOK2STATUSHOOK_M]

\ID SBSW_OS_HK_HOOK2PROTECTIONHOOK_001
 \DESCRIPTION    Os_Hook2ProtectionHook is called with the return value of Os_HookThread2Hook which is called with an
                 argument passed to the caller. As the type correctness of the argument is configuration dependent,
                 it cannot be guaranteed by means of implementation. Therefore, type correctness is checked by MSSV.
 \COUNTERMEASURE \M [CM_OS_HOOK2PROTECTIONHOOK_M]

\ID SBSW_OS_HK_HOOK2INITHOOK_001
 \DESCRIPTION    Os_Hook2InitHook is called with the return value of Os_HookThread2Hook which is called with the
                 return value of Os_CoreGetThread. As type correctness of the returned hook object is configuration
                 dependent, it cannot be guaranteed by means of implementation. Therefore, type correctness of the
                 object is checked by MSSV.
 \COUNTERMEASURE \M [CM_OS_HOOK2INITHOOK_M]

\ID SBSW_OS_HK_HOOK2CALLBACKHOOK_001
 \DESCRIPTION    Os_Hook2CallbackHook is called with the return value of Os_HookThread2Hook which is called with an
                 argument passed to the caller. As the type correctness of the argument is configuration dependent,
                 it cannot be guaranteed by means of implementation. Therefore, type correctness of the object is
                 checked by MSSV.
 \COUNTERMEASURE \M [CM_OS_HOOK2CALLBACK_M]

\ID SBSW_OS_HK_HOOKGETDYN_001
 \DESCRIPTION    Os_HookGetDyn is called with the return value of Os_HookThread2Hook which is called with an
                 argument passed to the caller. As the type correctness of the argument is configuration dependent,
                 it cannot be guaranteed by means of implementation. Therefore, type correctness of the object is
                 checked by MSSV.
 \COUNTERMEASURE \M [CM_OS_HOOK2PROTECTIONHOOK_M]

\ID SBSW_OS_HK_HOOKGETDYN_003
 \DESCRIPTION    Write access to the return value of Os_HookGetDyn.
 \COUNTERMEASURE \M [CM_OS_HOOKGETDYN_M]

\ID SBSW_OS_HK_HOOKGETDYN_004
 \DESCRIPTION    Os_HookGetDyn is called with the return value of Os_HookThread2Hook which is called with the
                 return value of Os_CoreGetThread. As the type correctness of the current running thread is
                 configuration dependent, it cannot be guaranteed by means of implementation. Therefore, it is checked
                 by MSSV that the only allowed context is an InitHook context.
 \COUNTERMEASURE \M [CM_OS_HOOK2INITHOOK_M]

\ID SBSW_OS_HK_HOOKGETDYN_005
 \DESCRIPTION    Os_HookGetDyn is called with the return value of Os_HookThread2Hook which is called with the
                 return value of Os_CoreGetThread. As the type correctness of the current running thread is
                 configuration dependent, it cannot be guaranteed by means of implementation. Therefore, it is checked
                 by MSSV that the only allowed context is a hook context.
 \COUNTERMEASURE \M [CM_OS_HOOKRETURNADDRESS_M]
...
\ID SBSW_OS_HK_TRAPHOOKGETSTATUS_001
 \DESCRIPTION    Os_TrapHookGetStatus is called with the return value of Os_HookThread2Hook which is called
                 with an argument passed to the caller. As the type correctness of the argument is configuration
                 dependent, it cannot be guaranteed by means of implementation. Therefore, type correctness of
                 the object is checked by MSSV.
 \COUNTERMEASURE \M [CM_OS_HOOK2STATUSHOOK_M]
...
\ID SBSW_OS_HK_STATUS_CALLBACK_001
 \DESCRIPTION    Function pointer is called, derived from the return value of Os_Hook2StatusHook.
 \COUNTERMEASURE \M [CM_OS_HOOK_STATUS_CALLBACK_M]

\ID SBSW_OS_HK_PROTECTIONHOOK_CALLBACK_001
 \DESCRIPTION    Function pointer is called, derived from the return value of Os_Hook2ProtectionHook.
 \COUNTERMEASURE \M [CM_OS_HOOK_PROTECTIONHOOK_CALLBACK_M]

\ID SBSW_OS_HK_CALLBACK_CALLBACK_001
 \DESCRIPTION    Function pointer is called, derived from an argument passed to the caller.
...
\ID SBSW_OS_HK_INITHOOK_CALLBACK_001
 \DESCRIPTION    Function pointer is called, derived from the return value of Os_Hook2InitHook.
...
\CM CM_OS_HOOK2PROTECTIONHOOK_M
      Verify that the thread context entry of each ProtectionHook refers to Os_HookWrapperProtectionHook and that there are
      no references to Os_HookWrapperProtectionHook from other objects.
