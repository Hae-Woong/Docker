 ...
 *  7.00.00   2018-08-17  vispkn  STORYC-6138   Update to MISRA2012
 *                                ESCAN00100665 Compiler Warning: conversion to 'uint8' from 'unsigned int' may alter its value [-Wconversion]
 *  8.00.00   2019-05-07  vispkn  STORYC-8146   Extend PNC bits in NM message layout (Support of NM Pdus greater 8 bytes)
 *                                ESCAN00103466 Compiler warning: "conditional expression is constant"
 *  9.00.00   2021-09-27  vispkn  ESCAN00110299 Channels in coordinator cluster fall asleep unexpectedly
 *            2021-12-15  vispkn  NMM-1451      Support variance for SNI
 *                                NMM-1440      Updated to MISRA Helix 2021
 *  9.01.00   2022-03-15  vispkn  NMM-1847      Synchronized PNC Shutdown feature.
 *  9.01.01   2022-06-10  vispkn  ESCAN00112073 Synchronized PNC Shutdown does not work as expected
 *                                ESCAN00112072 Compiler error C2166: l-value specifies const object
 *********************************************************************************************************************/
...
# define NM_ASR4NMFR_VERSION                                 (0x0901u)
...
# define FRNM_SID_REQUESTSYNCHRONIZEDPNCSHUTDOWN              (0xF3u) /*!< Service ID: FrNm_RequestSynchronizedPncShutdown() */
...
# define FRNM_E_INVALID_PN_SYNC_SHUTDOWN_REQUEST              (0x13u)  /*!< Error code: A PNSR message was received on a channel that is actively coordinated. */
# define FRNM_E_TRANSMISSION_OF_PN_SHUTDOWN_MESSAGE_FAILED    (0x14u)  /*!< Error code: The PNSR message could not be transmitted in the configured time. */
# define FRNM_E_REQUEST_PN_SHUTDOWN_MESSAGE_INVALID_STATE     (0x15u)  /*!< Error code: The Request of a synchronized Pn Shutdown cannot be accepted due to an invalid state of the Nm state machine. */
 ...
 *  \pre         -
 *  \note        Called from NM Interface
 *  \config      FRNM_SOURCENODEIDENTIFIERENABLED = STD_ON
 *  \context     TASK|ISR1|ISR2
 *  \reentrant   TRUE
 *  \synchronous TRUE
 ...
 *  \pre         -
 *  \note        Called from NM Interface
 *  \config      FRNM_SOURCENODEIDENTIFIERENABLED = STD_ON || FRNM_USER_DATA_ENABLED = STD_ON || FRNM_CONTROL_BIT_VECTOR_ENABLED = STD_ON
 *  \context     TASK|ISR1|ISR2
 *  \reentrant   TRUE
 *  \synchronous FALSE
...
/***********************************************************************************************************************
 *  FrNm_RequestSynchronizationPncShutdown
 **********************************************************************************************************************/
 ...
 *  \note        Called from NM Interface
 *  \config      FRNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG == STD_ON
 *  \context     TASK|ISR1|ISR2
 *  \reentrant   TRUE for different channels
 *  \synchronous TRUE
 *  \trace       DSGN-FrNm-SynchronizedPncShutdown
 **********************************************************************************************************************/
extern FUNC( Std_ReturnType, FRNM_CODE ) FrNm_RequestSynchronizedPncShutdown( CONST( NetworkHandleType, AUTOMATIC ) NetworkHandle,
                                                                              CONST( PNCHandleType, AUTOMATIC ) pncId );
# endif
 ...
 * \endexclusivearea
 *
 * \exclusivearea FRNM_EXCLUSIVE_AREA_7
 * Ensures that the FrNm_SyncPncShutdownReqNew is not written while data is modified.
 * Note: This critical section can be left empty when ComM cannot interrupt the FrNm_MainFunction.
 * \length SHORT At max 128 bytes are copied within this exclusive area. But usually it is a lot less, due to the PN settings.
 * \usedin FrNm_MainFunction
 * \exclude FrNm_RequestSynchronizedPncShutdown
 * \protects FrNm_SyncPncShutdownReqNew
 * \endexclusivearea
 */
