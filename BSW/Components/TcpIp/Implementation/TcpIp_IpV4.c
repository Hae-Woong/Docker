 ...
 *                                   \spec requires UnicastAddrV4Idx < TcpIp_GetSizeOfUnicastAddrV4(); \endspec
 ...
 *                                 \spec requires LocalAddrV4Idx < TcpIp_GetSizeOfLocalAddr(); \endspec
 *  \pre            -
 *  \context        TASK|ISR2
 *  \reentrant      TRUE for different controllers.
 *  \synchronous    TRUE
 ...
 *                                    \spec requires LlAddrStateIdx < TcpIp_GetSizeOfLlAddrState(); \endspec
 ...
 *  \param[in]      WasFragmented  Indicate if the IP packet was fragmented
 *  \pre            -
 *  \context        TASK|ISR2
 *  \reentrant      FALSE
 *  \synchronous    TRUE
...
IPV4_LOCAL_INLINE FUNC(boolean, TCPIP_CODE) IpV4_Ip_VReassemblyRxIndication(
  TcpIp_IpV4CtrlIterType   IpV4CtrlIdx,
  IPV4_P2C(uint8)          DataPtr,
  IPV4_P2V(TcpIp_Uint8P2V) FragPayloadPtrPtr,
 ...
 *  \param[in]      IpV4CtrlIdx     Index the IP controller/instance.
 *                  CONSTRAINT      [0..TcpIp_GetSizeOfIpV4Ctrl()-1]
 *  \param[in]      SocketIpV4Idx   Index of the upper layer socket.
...
IPV4_LOCAL_INLINE FUNC(BufReq_ReturnType, TCPIP_CODE) IpV4_Ip_VFragmentProvideTxBuffers(
  TcpIp_IpV4CtrlIterType                    IpV4CtrlIdx,
  TcpIp_IpV4SocketDynIterType               SocketIpV4Idx,
  uint16                                    IpPayloadLen,
 ...
 *                                    TCPIP_TCP_HDR_CHECKSUM_OFFSET = 16u
 *                                    \spec requires (Protocol != TCPIP_PROTOCOL_TCP) || ($lengthOf(ProtocolHeaderPtr) >= (16u + 2u)); \endspec
 *                                    TCPIP_UDP_HDR_OFS_CHECKSUM = 8u
 *                                    \spec requires (Protocol != TCPIP_PROTOCOL_UDP) || ($lengthOf(ProtocolHeaderPtr) >= (8u + 2u)); \endspec
 *                                    ICMPV4_HDR_START_CHECKSUM_OFS = 2u
 *                                    \spec requires (Protocol != IPV4_IP_PROTOCOL_ICMP) || ($lengthOf(ProtocolHeaderPtr) >= (2u + 2u)); \endspec
 *  \pre            -
 *  \context        TASK|ISR2
 *  \reentrant      FALSE
 *  \synchronous    TRUE
 ...
 *                  the parameter IpV4CtrlIdx can NOT be removed!
 *  \param[in]      IpV4CtrlIdx       Index the IP controller/instance.
 *                  CONSTRAINT        \spec requires IpV4CtrlIdx < TcpIp_GetSizeOfIpV4Ctrl(); \endspec
 ...
 *                  CONSTRAINT         [0..TcpIp_GetSizeOfIpV4Ctrl()-1]
 *  \param[in]      DestAddr           Destination IP address.
 *  \param[in]      IpV4SocketIdx      Upper layer socket index. (used for optimized/cached lookup)
 ...
 *  \param[out]     AddrInfoPtr          IP address information (IP address, Netmask, Default Router) tuple.
 *                                       \spec requires AddrInfoPtr != NULL_PTR; \endspec
 ...
 *                                 \spec requires LocalAddrV4Idx < TcpIp_GetSizeOfLocalAddrV4(); \endspec
...
/**********************************************************************************************************************
 *  IpV4_Ip_CheckIfChecksumCalcIsRequired()
 *********************************************************************************************************************/
/*! \brief            Check if the hardware can verify the checksum for the transmit
 *  \details          -
 ...
 *  \reentrant        FALSE
 *********************************************************************************************************************/
IPV4_LOCAL_INLINE FUNC(TcpIp_IpProtocolChecksumType, TCPIP_CODE) IpV4_Ip_CheckIfChecksumCalcIsRequired(
  IPV4_P2C(TcpIp_IpTxRequestDescriptorType) IpTxReqDescrPtr
);

/**********************************************************************************************************************
 *  IpV4_Ip_VCalcFragmentsChecksum()
 *********************************************************************************************************************/
/*! \brief            Calculate the partial checksum of at fragment
 *  \details          For the first fragement the pseudo header is added, for the last fragment the crc will be
                      finalized and written into the IP header
 *  \param[in]        IpV4CtrlIdx             IP Controller Index
 *                                            CONSTRAINT: [0...TcpIp_GetSizeOfIpV4ctrl()-1]
 *  \param[in]        FragParamPtr            Pointer to the struct that store the current fragment informations.
 *  \param[in]        ProtocolChecksumPtr     Pointer to the current checksum,that will be updated.
 *  \param[in]        IpTxReqDescrPtr         Pointer to TX request descriptor
 *  \param[in]        IpProtocol              Upper layer protocol type of the IPv4 packet
 *  \pre              -
 *  \context          TASK
 *  \reentrant        FALSE
 *********************************************************************************************************************/
IPV4_LOCAL_INLINE FUNC(void, TCPIP_CODE) IpV4_Ip_VCalcFragmentsChecksum(
  TcpIp_IpV4CtrlIterType IpV4CtrlIdx,
  IPV4_P2C(TcpIp_IpV4FragmentType) FragParamPtr,
  IPV4_P2V(uint32) ProtocolChecksumPtr,
  IPV4_P2C(TcpIp_IpTxRequestDescriptorType) IpTxReqDescrPtr,
  uint8                          IpProtocol
);
    ...
    for(ipV4CtrlIdx = 0; ipV4CtrlIdx < TcpIp_GetSizeOfIpV4Ctrl(); ipV4CtrlIdx++)
    {
      ...
      if(TcpIp_IsDemEventUsedOfIpV4Ctrl(ipV4CtrlIdx) == TRUE)
      {
        ...
        if(TcpIp_IsLastDuplicateDhcpAddrUsedOfDemEvent(demEventIdx) == TRUE)
        {
          ...
          /*@ assert lastDuplicateDhcpAddrIdx < TcpIp_GetSizeOfLastDuplicateDhcpAddr(); */                              /* VCA_TCPIP_CSL03_IS_USEDOF */
          ...
          TcpIp_VCopyPhysAddr(&TcpIp_GetAddrPhysAddrOfLastDuplicateDhcpAddr(lastDuplicateDhcpAddrIdx)->data[0],
                              initPhysAddr);
          TcpIp_SetNetAddrOfLastDuplicateDhcpAddr(lastDuplicateDhcpAddrIdx, 0u);
        }
      }
...
IPV4_LOCAL_INLINE FUNC(void, TCPIP_CODE) IpV4_Ip_Init(void)
{
  ...
  for (ipV4CtrlIdx = 0; ipV4CtrlIdx < TcpIp_GetSizeOfIpV4Ctrl(); ipV4CtrlIdx++)
  {
    ...
    TcpIp_SetStateOfIpV4CtrlDyn(ipV4CtrlIdx, IPV4_IP_OFFLINE);
    TcpIp_SetRequestedStateOfIpV4CtrlDyn(ipV4CtrlIdx, IPV4_IP_OFFLINE);
    TcpIp_SetAddrAssignmentReadyChangedOfIpV4CtrlDyn(ipV4CtrlIdx, FALSE);
    TcpIp_SetActiveAddrAssignmentMethodOfIpV4CtrlDyn(ipV4CtrlIdx, TCPIP_IPADDR_ASSIGNMENT_NONE);
    TcpIp_SetAddrAssignmentTriggeredFlagsOfIpV4CtrlDyn(ipV4CtrlIdx, 0);
# if (TCPIP_SUPPORT_IP_ADDR_NVM_STORAGE == STD_ON)
    TcpIp_SetAddrAssignmentMethodPersistOfIpV4CtrlDyn(ipV4CtrlIdx, TCPIP_IPADDR_ASSIGNMENT_NONE);
# endif

    TcpIp_SetActiveNetAddrOfIpV4CtrlDyn(ipV4CtrlIdx, IPV4_ADDR_ANY);
    TcpIp_SetActiveNetMaskOfIpV4CtrlDyn(ipV4CtrlIdx, 0);
    TcpIp_SetActiveDefGwAddrOfIpV4CtrlDyn(ipV4CtrlIdx, IPV4_ADDR_ANY);
    TcpIp_SetLastBcAddrOfIpV4CtrlDyn(ipV4CtrlIdx, IPV4_ADDR_ANY);

    /* #30 Reset LinkLocal address assignment. */
    if (TcpIp_IsLlAddrStateUsedOfIpV4Ctrl(ipV4CtrlIdx) == TRUE)
    {
      TcpIp_SizeOfLlAddrStateType llAddrStateIdx = TcpIp_GetLlAddrStateIdxOfIpV4Ctrl(ipV4CtrlIdx);
      /*@ assert llAddrStateIdx < TcpIp_GetSizeOfLlAddrState(); */                                                      /* VCA_TCPIP_CSL03_IS_USEDOF */
      TCPIP_ASSERT(IpV4_Ip_VCfgIsAddrAssignmentEnabled(ipV4CtrlIdx, TCPIP_IPADDR_ASSIGNMENT_LINKLOCAL));

      IpV4_Ip_VLl_Reset(llAddrStateIdx);
    }
  } /* for each controller */

  {
      ...
      multicastAddrV4Idx++)
    {
      ...
      TcpIp_SetNetAddrOfMulticastAddrV4Requested(multicastAddrV4Idx,
        TcpIp_GetNetAddrOfDefaultMulticastAddrV4(defaultMulticastAddrV4Idx));
    }
  }

  {
    TcpIp_IpV4SocketDynIterType   ipV4SocketIdx;   /* IpV4_Ip_TransferBlock / socket index */
    ...
    {
      IpV4_Ip_ResetSocket(ipV4SocketIdx);
    }
  }
...

  {
    ...
    {
      TcpIp_SetEthBufPtrOfBuf2TxReqMap(buf2TxReqMapIdx, NULL_PTR);
    }
  }
} /* IpV4_Ip_Init */
...
)
{
  ...
  if (WriteAccess == TRUE)
  {
    ...
    if (nvMStatus != NVM_REQ_PENDING)
    {
      ...
      /*@ assert nvmDataIdx <= TcpIp_GetSizeOfNvmData(); */                                                             /* VCA_TCPIP_NVM_DATA_IDX */
      /* PRQA S 0310 2 */ /* MD_TCPIP_Rule11.3_0310_Uint8ToStruct */
      *IpAddrSetPtrPtr = (IPV4_P2V(IpV4_IpAddrSetType))TcpIp_GetAddrNvmData(nvmDataIdx);
      retVal = E_OK;
    }
  }
  else
...
{
  /* ----- Local Variables ---------------------------------------------- */
  /* Initialize the ipAddrSetPtr to the start position of the TcpIp_NvmData[] array.
     The pointer will be updated in context of IpV4_Ip_NvmGetAddrSet(). */
  /* PRQA S 0310 1 */ /* MD_TCPIP_Rule11.3_0310_Uint8ToStruct */
  IPV4_P2V(IpV4_IpAddrSetType) ipAddrSetPtr = (IPV4_P2V(IpV4_IpAddrSetType))TcpIp_GetAddrNvmData(0u);

  /* ----- Implementation ----------------------------------------------- */
  /* #10 Continue only in case the address set in NvM of this IpV4 controller can be accessed */
  if(IpV4_Ip_NvmGetAddrSet(IpV4CtrlIdx, &ipAddrSetPtr, TRUE) == E_OK)
  {
  ...
  TcpIp_IpV4CtrlIterType IpV4CtrlIdx)
{
  ...
  TcpIp_IpAddrAssignmentType bestAvailableAddrAssignmentMethod =
    IpV4_Ip_VGetBestAvailableAddrAssignment(IpV4CtrlIdx, &bestAvailableAddr);
  ...
  if(TcpIp_GetActiveAddrAssignmentMethodOfIpV4CtrlDyn(IpV4CtrlIdx) == TCPIP_IPADDR_ASSIGNMENT_NONE)
  {
    ...
    {
      /* IP address ready. */
      TcpIp_SetActiveDefGwAddrOfIpV4CtrlDyn(IpV4CtrlIdx, bestAvailableAddr.DefGwAddr);
      TcpIp_SetActiveNetAddrOfIpV4CtrlDyn(IpV4CtrlIdx, bestAvailableAddr.NetAddr);
      TcpIp_SetActiveNetMaskOfIpV4CtrlDyn(IpV4CtrlIdx, bestAvailableAddr.NetMask);
      TcpIp_SetActiveAddrAssignmentMethodOfIpV4CtrlDyn(IpV4CtrlIdx, bestAvailableAddrAssignmentMethod);
  ...
  /* #40 Otherwise an assignment method is active, already, for the given controller. */
  {
    ...
    if(bestAvailableAddrAssignmentMethod != TCPIP_IPADDR_ASSIGNMENT_NONE)
    {
      ...
      {
        /* Store Ip state. */
        TcpIp_SizeOfLocalAddrV4Type localAddrIdx = TcpIp_GetLocalAddrV4UnicastIdxOfIpV4Ctrl(IpV4CtrlIdx);
        ...
        IpV4_VLocalIpAddrAssignmentChange(localAddrIdx, TCPIP_IPADDR_STATE_UNASSIGNED);
        ...
        IpV4_VLocalIpAddrAssignmentChange(localAddrIdx, state);
      }
...
# endif
    {
      ...
      else if (TcpIp_GetStateOfIpV4CtrlDyn(ipV4CtrlIdx) == IPV4_IP_ONHOLD)
      {
                 ...
                 || (TcpIp_GetRequestedStateOfIpV4CtrlDyn(ipV4CtrlIdx) == IPV4_IP_ONLINE_PASSIVE))
        {
          ...
          TcpIp_SetRequestedStateOfIpV4CtrlDyn(ipV4CtrlIdx, TcpIp_GetStateOfIpV4CtrlDyn(ipV4CtrlIdx));
        }
        else
        {
...
# if (TCPIP_SUPPORT_DEM == STD_ON)
    if(TcpIp_IsDemEventUsedOfIpV4Ctrl(IpV4CtrlIdx) == TRUE)
    {
      ...
      if (TcpIp_IsLastDuplicateDhcpAddrUsedOfDemEvent(demEventIdx) == TRUE)
      {
        ...
        TcpIp_VCopyPhysAddr(&TcpIp_GetAddrPhysAddrOfLastDuplicateDhcpAddr(lastDuplicateDhcpAddrIdx)->data[0],
                            PhysAddrPtr);
        TcpIp_SetNetAddrOfLastDuplicateDhcpAddr(lastDuplicateDhcpAddrIdx, SrcAddr);
  ...
  IpBase_AddrInType         DestAddr)
{
  ...
  else if(TcpIp_IsReceiveAllMulticastsOfIpV4Ctrl(IpV4CtrlIdx) == TRUE)
  {
    ...
    TcpIp_SetLastBcAddrOfIpV4CtrlDyn(IpV4CtrlIdx, DestAddr);
  }
  else
  {
  ...
  TcpIp_LocalAddrV4IterType                LocalAddrV4Idx)
{
  ...
  boolean isFragmented     = (boolean)((flagsOffset & IPV4_IP_FRAGMENT_OFFSET_MASK) != 0u);
  ...
  if (ipPayloadLenByte > 0u)
  {
    ...
    else
    {
      ...
      checksumOk = (boolean)(IpBase_TcpIpChecksumAdd(DataPtr, ipHdrLenByte, 0, TRUE) == 0u);                            /* PRQA S 4304 */ /* MD_MSR_AutosarBoolean */
    }
    ...
    if (checksumOk == TRUE)
    {
      ...
      if((0u != fragmentOffset) || (0u != (flagsOffset & IPV4_IP_FLAGS_MORE_FRAGMENTS)))
      {
        ...
        {
          if(FALSE == IpV4_Ip_VReassemblyRxIndication(IpV4CtrlIdx, DataPtr, &ipPayloadPtr, &ipPayloadLenByte))
          {
      ...
      {
        IpV4_Ip_VForwardPacketToUpperLayer(IpV4CtrlIdx, LocalAddrV4Idx, DataPtr, ipPayloadPtr, ipPayloadLenByte,
                                           isFragmented);
      }
        ...
        && (IPV4_IP_DEF_HEADER_LENGTH_BYTE <= LenByte))
    {
      ...
      if (    (ipHdrLenByte >= IPV4_IP_DEF_HEADER_LENGTH_BYTE)                                                          /* PRQA S 3415 28 */ /* MD_IPV4_Rule12.4_3415 */
          ...
          && (   (TcpIp_IsHwChecksumIpOfIpV4Ctrl(ipV4CtrlIdx) == TRUE)
              || (IpBase_TcpIpChecksumAdd(DataPtr, ipHdrLenByte, 0, TRUE) == 0u))
          ...
          && (IpV4_Ip_RxIsDuplicateDhcpAddr(ipV4CtrlIdx, srcAddr, PhysAddrPtr) == FALSE)
          ...
          && (!IPV4_ADDR_IN_SUBNET(srcAddr, IPV4_LOCALNET_PREFIX, IPV4_LOCALNET_PREFIX_MASK)))
      {
        ...
        {
          /*@ assert localAddrV4Idx < TcpIp_GetSizeOfLocalAddrV4(); */                                                  /* VCA_TCPIP_IPV4_LOC_ADDR_IDX */
          IpV4_Ip_ReassembleAndForwardToUpperLayer(DataPtr, ipV4CtrlIdx, localAddrV4Idx);
        }
...
FUNC(BufReq_ReturnType, TCPIP_CODE) IpV4_Ip_ProvideTxBuffer(
  ...
  IPV4_P2V(TcpIp_Uint8P2V)                  IpBufferPtr,
  ...
  boolean                                   ForceProvideBuffer)
{
  /* ----- Local Variables ---------------------------------------------- */
# if (TCPIP_SUPPORT_IPSEC == STD_ON)
  Std_ReturnType            retIpSec = E_NOT_OK;
# endif
  TcpIp_IpV4CtrlIterType    ipV4CtrlIdx;
  TcpIp_ReturnType          arpRetVal = TCPIP_E_NOT_OK;
  ...
  ipV4CtrlIdx = IpV4_Ip_VGetLocalAddrIdxAndCtrlIdx(LocalAddrV4IdxPtr, destAddr);

# if (TCPIP_SUPPORT_IPSEC == STD_ON)
  /* Store the default reserved trailer length. For IPsec it will be changed later. */
  IpTxReqDescrPtr->ReservedTrailer = 0;
# endif
  IpTxReqDescrPtr->EthBufferCnt = 0;
...
# if (TCPIP_SUPPORT_IPSEC == STD_ON)
      ...
      && (TcpIp_IsIpSecConfigSetUsedOfIpV4Ctrl(ipV4CtrlIdx) == TRUE))
  {
    ...
    retIpSec = TcpIp_IpSec_PreparePacketforAuthentication(ipV4CtrlIdx, IpV4SocketIdx, IpTxReqDescrPtr, *DestAddrPtr);
  }
...
# endif
  ...
  if (   (ipV4CtrlIdx < TcpIp_GetSizeOfIpV4Ctrl())                                                                      /* PRQA S 3415 5 */ /* MD_TCPIP_Rule13.5_3415_CslReadOnly */
...
# endif
  ...
  {
    IpTxReqDescrPtr->SrcAddrIdx    = (TcpIp_SizeOfLocalAddrV4Type)(*LocalAddrV4IdxPtr);
    IpTxReqDescrPtr->CtrlIdx       = TcpIp_GetEthIfCtrlIdxOfIpV4Ctrl(ipV4CtrlIdx);
    IpTxReqDescrPtr->SockIdx       = (TcpIp_SizeOfIpV4SocketDynType)IpV4SocketIdx;
    IpTxReqDescrPtr->DestAddrV4    = destAddr;
    IpTxReqDescrPtr->AddressFamily = TCPIP_AF_INET;
      ...
      )
    {
      ...
      /* SWS_TCPIP_00179 */
      *LocalAddrV4IdxPtr = TcpIp_GetLocalAddrV4UnicastIdxOfIpV4Ctrl(ipV4CtrlIdx);
    }
    ...
    else
    {
      ...
      {
        IpBase_EthPhysAddrType destMacAddr = { 0 };
        arpRetVal = IpV4_Ip_VGetNextHopPhysAddr(ipV4CtrlIdx, destAddr, IpV4SocketIdx, &destMacAddr[0]);
      }
...
# endif
        ...
        if(retVal == BUFREQ_OK)
        {
          *IpBufferPtr    = &IpTxReqDescrPtr->EthPayloadPtr[ipTotHdrLenBytes];
          *IpBufferLenPtr = (uint16)(IpTxReqDescrPtr->EthPayloadLen - ipTotHdrLenBytes);
          /* Prepare the checksum calculation */
          IpTxReqDescrPtr->ChecksumHwCalced = IpV4_Ip_CheckIfChecksumCalcIsRequired(IpTxReqDescrPtr);
          IpTxReqDescrPtr->TmpChecksum = 0u;
          /*@ assert ($external(*IpBufferPtr) && ($lengthOf(*IpBufferPtr) >= (*IpBufferLenPtr))); */                    /* VCA_TCPIP_IP_BUFFER_PTR */
        }
      }
      else
      {
  ...
  IPV4_P2V(uint8)             NextHopPhysAddrPtr)
{
  ...
  {
    /* retVal is TCPIP_E_NOT_OK */
  }

  if (retVal == TCPIP_OK)
  {
    retVal = IpV4_Arp_GetPhysicalAddress(IpV4CtrlIdx, (TcpIp_SizeOfIpV4SocketDynType)IpV4SocketIdx, dstAddr,
...
{
  /* ----- Local Variables ---------------------------------------------- */
  TcpIp_IpV4CtrlIterType ipV4CtrlIdx = TcpIp_GetSizeOfIpV4Ctrl();
  ...
  else
  {
    ...
    if(candidateIpV4CtrlIdx < TcpIp_GetSizeOfIpV4Ctrl())
    {
      ...
      *LocalAddrV4IdxPtr = TcpIp_GetLocalAddrV4BroadcastIdxOfIpV4Ctrl(ipV4CtrlIdx);
    }
  ...
  IPV4_P2V(TcpIp_IpTxRequestDescriptorType) IpTxReqDescrPtr)
{
  ...
  while (payloadLenLocal > 0u)
  {
    ...
    retVal = TCPIP_LL_ProvideTxBuffer(TcpIp_GetEthIfCtrlIdxOfIpV4Ctrl(IpV4CtrlIdx), IPV4_IP_FRAME_TYPE,
                                      IPV4_GET_SOCKET_ETHIF_FRAME_PRIO(SocketIpV4Idx, IpV4CtrlIdx), &ethBufIdx,
                                      &ethBufPtr, &ethBufLen);
    ...
    if (BUFREQ_OK == retVal)
    {
      /*@ assert $external(ethBufPtr) && ($lengthOf(ethBufPtr) >= (ethBufLen)); */                                      /* VCA_TCPIP_LL_PROVIDE_TX_BUFFER */
      ...
      if ((buf2TxReqMapStartIdx + ethBufIdx) < TcpIp_GetBuf2TxReqMapEndIdxOfIpV4Ctrl(IpV4CtrlIdx))
      {
        ...
        if (prevEthBufIdx == IPV4_IP_TX_BUF_IDX_INV)
        {
          ...
          IpTxReqDescrPtr->CurBufIdxV4 = ethBufIdx;
        }
        else
        {
          ...
          TcpIp_SetEthBufIdxNextOfBuf2TxReqMap(nextBuf2TxReqMapIdx, ethBufIdx);
        }
        ...
        TcpIp_SetEthBufPtrOfBuf2TxReqMap(nextBuf2TxReqMapIdx, (IPV4_P2V(uint8))ethBufPtr);
        TcpIp_SetEthBufLenOfBuf2TxReqMap(nextBuf2TxReqMapIdx, ethBufLen);
        TcpIp_SetEthBufIdxNextOfBuf2TxReqMap(nextBuf2TxReqMapIdx, IPV4_IP_TX_BUF_IDX_INV);
...
# if (TCPIP_SUPPORT_IPSEC == STD_ON)
    IpTxReqDescrPtr->IpPayloadOfs  = IPV4_IP_DEF_HEADER_LENGTH_BYTE + IpTxReqDescrPtr->ExtHdrLenBytes;
# else
...
FUNC(Std_ReturnType, TCPIP_CODE) IpV4_Ip_FragmentProvideTxBuffer(
  IPV4_P2V(TcpIp_IpTxRequestDescriptorType)  IpTxReqDescrPtr,
  IPV4_P2V(TcpIp_Uint8P2V)                   BufPtrPtr,
  IPV4_P2V(uint16)                           BufLenPtr)
{
  ...
  if (IpTxReqDescrPtr->CurBufIdxV4 != IPV4_IP_TX_BUF_IDX_INV)
  {
    ...
    if (nxtBufIdx != IPV4_IP_TX_BUF_IDX_INV)
    {
      ...
      (*BufPtrPtr) = &TcpIp_GetEthBufPtrOfBuf2TxReqMap(buf2TxReqMapIdx)[IPV4_IP_DEF_HEADER_LENGTH_BYTE];
      (*BufLenPtr) =  TcpIp_GetEthBufLenOfBuf2TxReqMap(buf2TxReqMapIdx) - IPV4_IP_DEF_HEADER_LENGTH_BYTE;

      retVal = E_OK;
    }

    IpTxReqDescrPtr->CurBufIdxV4 = nxtBufIdx;
  }
...
{
  TcpIp_IpProtocolChecksumType   checkType = IpTxReqDescrPtr->ChecksumHwCalced;

  /* #10 If this is the last fragment, finalize the checksum and write to hdr. Else add crc to tmp checksum */
  if (FragParamPtr->IsLast == TRUE)
  {
    IPV4_P2V(uint8) protocolHdrPtr =
      &TcpIp_GetEthBufPtrOfBuf2TxReqMap(TcpIp_GetBuf2TxReqMapStartIdxOfIpV4Ctrl(IpV4CtrlIdx)
        + IpTxReqDescrPtr->BufIdx)[FragParamPtr->HeaderLen];
    /*@ assert $external(protocolHdrPtr); */                                                                            /* VCA_TCPIP_PROT_HDR_PTR */

    if (checkType == TCPIP_IP_PROTOCOL_CHECKSUM_CHECK_COMPLETE)
    {
      *ProtocolChecksumPtr = IpBase_TcpIpChecksumAdd(FragParamPtr->PayloadPtr, FragParamPtr->PayloadLen, *ProtocolChecksumPtr, TRUE);
    }
    else /* checkType == Copy */
    {
      *ProtocolChecksumPtr = TcpIp_AddPartialChecksums(*ProtocolChecksumPtr, IpTxReqDescrPtr->TmpChecksum);
      *ProtocolChecksumPtr = IpBase_TcpIpChecksumAdd(FragParamPtr->PayloadPtr, 0, *ProtocolChecksumPtr, TRUE); /* Finalize checksum */
    }

    IpV4_Ip_VStoreProtocolChecksum(IpProtocol, (uint16)*ProtocolChecksumPtr, protocolHdrPtr);                           /* VCA_TCPIP_PROT_HDR_PTR */
  }
  else if (checkType == TCPIP_IP_PROTOCOL_CHECKSUM_CHECK_COMPLETE)
  {
    *ProtocolChecksumPtr = IpBase_TcpIpChecksumAdd(FragParamPtr->PayloadPtr, FragParamPtr->PayloadLen, *ProtocolChecksumPtr, FALSE);
  }
  else
  {
    /* The checksum will be calculated while copy */
  }
} /* IpV4_Ip_VCalcFragmentsChecksum */
    ...
    uint16                                    Identification)
{
  ...
  TcpIp_IpProtocolChecksumType   checkType          = IpTxReqDescrPtr->ChecksumHwCalced;
  boolean                        chkSumFlag         = (checkType != TCPIP_IP_PROTOCOL_CHECKSUM_CHECK_HW_OR_NOT_REQ);
# if (TCPIP_SUPPORT_IPSEC == STD_ON)
...
# endif
  ...
  while (ethBufIdx != IPV4_IP_TX_BUF_IDX_INV)
  {
    ...
    {
      IpBase_AddrInType srcAddr = TcpIp_GetActiveNetAddrOfIpV4CtrlDyn(IpV4CtrlIdx);
      ...
      /*@ assert $external(ethPayloadPtr) && $lengthOf(ethPayloadPtr) >= IPV4_IP_DEF_HEADER_LENGTH_BYTE; */             /* VCA_TCPIP_ETH_PAYLOAD_PTR */
      uint16            ethPayloadLen  = TcpIp_GetEthBufLenOfBuf2TxReqMap(
                                           TcpIp_GetBuf2TxReqMapStartIdxOfIpV4Ctrl(IpV4CtrlIdx) + ethBufIdx);
      uint16            fragBlockOfs   = nextFragBlockOfs;

      TcpIp_IpV4FragmentType fragParam = { 0 };
# if (TCPIP_SUPPORT_IPSEC == STD_ON)
      /* Subtract the IpSec header length from fragment payload length */
      fragParam.PayloadLen = (uint16)(ethPayloadLen - IPV4_IP_DEF_HEADER_LENGTH_BYTE - extHdrLen);
# else
      fragParam.PayloadLen = (uint16)(ethPayloadLen - IPV4_IP_DEF_HEADER_LENGTH_BYTE);
# endif
      fragParam.IsFirst = (boolean)(fragBlockOfs == 0u);                                                                /* PRQA S 4304 */ /* MD_MSR_AutosarBoolean */
      fragParam.IsLast = FALSE;
      fragParam.HeaderLen = 0;
...
# if (TCPIP_SUPPORT_DHCPV4 == STD_ON)
       ...
       */
      if (IPV4_GET_SOCKET_FORCE_ZERO_SRC_ADDR(TCPIP_SOCKET_TO_IPV4_SOCKET(IpTxReqDescrPtr->SockIdx)))
      {
        srcAddr = 0u;
      }
# endif
      ...
      if (ipPayloadLenRemain > fragParam.PayloadLen)
      {
        /* remaining data does not fit into one IP packet */
        /* IsLast is already set to FALSE */
...
# if (TCPIP_SUPPORT_IPSEC == STD_ON)
        ...
        TCPIP_ASSERT(0u == ((fragParam.PayloadLen + extHdrLen) % 8u));
# else
        TCPIP_ASSERT(0u == (fragParam.PayloadLen % 8));
# endif
        nextFragBlockOfs   += (fragParam.PayloadLen / 8u);
        ipPayloadLenRemain -= fragParam.PayloadLen;
      }
      else
      {
        /* this is the last fragment */
        fragParam.IsLast = TRUE;

        fragParam.PayloadLen = ipPayloadLenRemain;
        ipPayloadLenRemain  = 0;
      }

# if (TCPIP_SUPPORT_IPSEC == STD_ON)
      /* #50 if IpSec header is inserted, update payload length to include IpSec header length */
      if (   (fragParam.IsFirst == TRUE)                                                                                /* PRQA S 3415 2 */ /* MD_TCPIP_Rule13.5_3415_CslReadOnly */
          && (IpTxReqDescrPtr->SaEntryIdx < TcpIp_GetSizeOfSaEntry()))
      {
...
# endif
      ...
      if (fragParam.IsFirst == TRUE)
      {
        fragParam.HeaderLen = IPV4_IP_DEF_HEADER_LENGTH_BYTE;
# if (TCPIP_SUPPORT_IPSEC == STD_ON)
       /* #60 Update payload length to include IpSec header length */
        fragParam.HeaderLen += extHdrLen;
# endif
      }

      fragParam.PayloadPtr = &ethPayloadPtr[fragParam.HeaderLen];

      /* #70 In case checksum shall be calculated do calculate both pseudo header and packet checksum */
      if (chkSumFlag == TRUE)
      {
        /* #80 Call the upper layer checksum calculation function
               in case its the first fragment and the protocol is TCP or UDP. */
        if ((fragParam.IsFirst == TRUE) && ((IpProtocol == TCPIP_PROTOCOL_TCP) || (IpProtocol == TCPIP_PROTOCOL_UDP)))
        {
        ...
        /* #90 In case of a 'last' fragment, calculate payload checksum of last fragment
        ...
        IpV4_Ip_VCalcFragmentsChecksum(IpV4CtrlIdx, &fragParam, &protocolChecksum, IpTxReqDescrPtr, IpProtocol);
      }
      else
      {
        /* #100 In pass 2 of the iteration of all IP fragments, trigger the transmission of each fragment. */
        uint16 flagsOffset;
        TcpIp_IpV4SocketDynIterType ipV4SocketIdx = TCPIP_SOCKET_TO_IPV4_SOCKET(IpTxReqDescrPtr->SockIdx);
        /*@ assert ipV4SocketIdx < TcpIp_GetSizeOfIpV4SocketDyn(); */                                                   /* VCA_TCPIP_IPV4_SOCKET_AND_LOCAL_ADDR_IDX */

# if (TCPIP_SUPPORT_IPSEC == STD_ON)
        Std_ReturnType ipSecRetVal;
        /* Total fragment length includes the extension header length (if inserted) */
        uint16 fragTotalLen = fragParam.PayloadLen + IPV4_IP_DEF_HEADER_LENGTH_BYTE + extHdrLen;
# else
        uint16 fragTotalLen = fragParam.PayloadLen + IPV4_IP_DEF_HEADER_LENGTH_BYTE;
# endif
         ...
         */
        TCPIP_ASSERT(buf2TxReqMapIdx < TcpIp_GetBuf2TxReqMapEndIdxOfIpV4Ctrl(IpV4CtrlIdx));
        /*@ assert buf2TxReqMapIdx < TcpIp_GetSizeOfBuf2TxReqMap(); */                                                  /* VCA_TCPIP_TX_REQ_MAP */

        /* #110 Update the fields of IP header */
        TCPIP_PUT_UINT8(ethPayloadPtr, TCPIP_IPV4_HDR_OFS_VER_IHL, IPV4_HDR_FIELD_CREATE_VER_IHL(IPV4_IP_DEF_HEADER_LENGTH_BYTE));
        TCPIP_PUT_UINT8(ethPayloadPtr, TCPIP_IPV4_HDR_OFS_PROT, nextHdr);
        TCPIP_PUT_UINT16(ethPayloadPtr, TCPIP_IPV4_HDR_OFS_TOT_LEN, fragTotalLen);
        TCPIP_PUT_UINT16(ethPayloadPtr, TCPIP_IPV4_HDR_OFS_IDENT, Identification);
        TCPIP_PUT_UINT32_RAW(ethPayloadPtr, TCPIP_IPV4_HDR_OFS_SRC_ADDR, srcAddr);
        TCPIP_PUT_UINT32_RAW(ethPayloadPtr, TCPIP_IPV4_HDR_OFS_DEST_ADDR, IpTxReqDescrPtr->DestAddrV4);
        TCPIP_PUT_UINT8(ethPayloadPtr, TCPIP_IPV4_HDR_OFS_TOS, IPV4_GET_SOCKET_TYPE_OF_SERVICE(ipV4SocketIdx, IpV4CtrlIdx));
        TCPIP_PUT_UINT8(ethPayloadPtr, TCPIP_IPV4_HDR_OFS_TTL, IPV4_GET_SOCKET_TIME_TO_LIVE(ipV4SocketIdx, IpV4CtrlIdx));
        flagsOffset = (uint16)(IPV4_IP_FLAGS_DONT_FRAGMENT | fragBlockOfs);

        if (fragParam.IsLast == FALSE)
        {
          flagsOffset |= IPV4_IP_FLAGS_MORE_FRAGMENTS;
        }

        /* Update the flag offset field */
        TCPIP_PUT_UINT16(ethPayloadPtr, TCPIP_IPV4_HDR_OFS_FLAGS_OFFSET, flagsOffset);

# if (TCPIP_SUPPORT_IPSEC == STD_ON)
        /* #120 If valid IpSecConfigset is present, perform IpSec processing of the packet */
        if (TcpIp_IsIpSecConfigSetUsedOfIpV4Ctrl(IpV4CtrlIdx) == TRUE)
        {
          ipSecRetVal = TcpIp_IpSec_ProcessTxPacket(IpV4CtrlIdx, IpTxReqDescrPtr, ethPayloadPtr, fragTotalLen, IpProtocol);
        }
        /* #130 Otherwise, proceed with packet Tx */
        else
        {
          ipSecRetVal = E_OK;
        }
# endif
        ...
        TCPIP_PUT_UINT16_RAW(ethPayloadPtr, TCPIP_IPV4_HDR_OFS_CHECKSUM, TCPIP_CHECKSUM_UNSET);

        /* #140 In case HW based checksum is configured: Calculate IP header checksum and insert into header. */
        if (TcpIp_IsHwChecksumIpOfIpV4Ctrl(IpV4CtrlIdx) == TRUE)
        {
        ...
        {
          TCPIP_PUT_UINT16_RAW(ethPayloadPtr, TCPIP_IPV4_HDR_OFS_CHECKSUM, (uint16)IpBase_TcpIpChecksumAdd(ethPayloadPtr, IPV4_IP_DEF_HEADER_LENGTH_BYTE, 0, TRUE));
        }

        /* #150 Store information that may be required for TX-confirmation */
        TcpIp_SetProtocolOfIpV4SocketDyn(ipV4SocketIdx, IpProtocol);
        TcpIp_SetIpV4SocketDynIdxOfBuf2TxReqMap(buf2TxReqMapIdx, (TcpIp_SizeOfIpV4SocketDynType)ipV4SocketIdx);
        TcpIp_SetUlTxReqIdxOfBuf2TxReqMap(buf2TxReqMapIdx, UlReqIdx);

# if (TCPIP_SUPPORT_IPSEC == STD_ON)
        /* #160 Trigger transmission of packet if packet was authenticated successfully */
        if (ipSecRetVal == E_OK)
        {
# endif
          /* #170 Trigger the transmission of all fragments: Transmission of the last n fragments is aborted */
          ...
          if (E_OK == TCPIP_LL_Transmit(TcpIp_GetEthIfCtrlIdxOfIpV4Ctrl(IpV4CtrlIdx), ethBufIdx, IPV4_IP_FRAME_TYPE,
            ReqTxConfirmation, fragTotalLen, DestPhysAddrPtr))
          {
            retVal = E_OK;
          }
          else
...
# if (TCPIP_SUPPORT_IPSEC == STD_ON)
        ...
        /* #180 Drop packet if packet not authenticated successfully */
        else
        {
          retVal = E_NOT_OK;
          /* #190 Cancel transmit to clear the buffer */
...
# endif
    ...
    /* #200 In case there is no more data to send, cancel the transmission of this (last) fragment.
    ...
    /* #210 In case the next Eth buffer index is still set, validly set this index with the next index to come.
...
{
  /* ----- Local Variables ---------------------------------------------- */
  TcpIp_IpProtocolChecksumType calculateProtocolChecksum = TCPIP_IP_PROTOCOL_CHECKSUM_CHECK_COPY;
  TcpIp_EthIfCtrlIterType ethIfCtrlIdx = IpTxReqDescrPtr->CtrlIdx;
  TcpIp_SizeOfIpV4CtrlType ipV4CtrlIdx = TcpIp_GetIpV4CtrlIdxOfEthIfCtrl(ethIfCtrlIdx);

  /* ----- Implementation ----------------------------------------------- */
  /* #10 In case hardware calculation of checksums is possible:
       Determine, if this can be done for the given protocol and set the calculation flag accordingly. */
  if (IpTxReqDescrPtr->EthBufferCnt > 1u)
  {
    /* #20 This packet needs fragmentation: Checksum calculation for IP protocols by hardware is not supported
                                            for fragmented packets set the checksum calculation flag if required. */
    if ((IpTxReqDescrPtr->SkipUdpChecksum == TRUE) && (IpTxReqDescrPtr->XptProtocol == TCPIP_PROTOCOL_UDP))
    {
      calculateProtocolChecksum = TCPIP_IP_PROTOCOL_CHECKSUM_CHECK_HW_OR_NOT_REQ;
    }
  }
  else
  {
    /* #30 According to the configuration, check if the checksum of the upper layer (TCP, UDP or ICMPv4)
           can be calculated by the lower layer (HW) or has to be calculated by the upper layer itself. */
    switch (IpTxReqDescrPtr->XptProtocol)
    {
# if (TCPIP_SUPPORT_TCP == STD_ON)
    case TCPIP_PROTOCOL_TCP:
#  if (TCPIP_SUPPORT_IPSEC == STD_ON)
      ...
      {
        /* calculateProtocolChecksum is already set to 'checkByCopy' */
      }
      else
#  endif
      if(TcpIp_IsHwChecksumTcpOfIpV4Ctrl(ipV4CtrlIdx) == TRUE)
      {
        calculateProtocolChecksum = TCPIP_IP_PROTOCOL_CHECKSUM_CHECK_HW_OR_NOT_REQ;
      }
      else
...
# if (TCPIP_SUPPORT_IPSEC == STD_ON)
      else if (IpTxReqDescrPtr->SaEntryIdx != TcpIp_GetSizeOfSaEntry())
      {
        /* calculateProtocolChecksum is already set to 'checkByCopy' */
      }
# endif
      else if(TcpIp_IsHwChecksumUdpOfIpV4Ctrl(ipV4CtrlIdx) == TRUE)
      {
        calculateProtocolChecksum = TCPIP_IP_PROTOCOL_CHECKSUM_CHECK_HW_OR_NOT_REQ;
      }
      else
...
# if (TCPIP_SUPPORT_IPSEC == STD_ON)
      if (IpTxReqDescrPtr->SaEntryIdx != TcpIp_GetSizeOfSaEntry())
      {
        /* calculateProtocolChecksum is already set to true */
      }
      else
# endif
      if(TcpIp_IsHwChecksumIcmpOfIpV4Ctrl(ipV4CtrlIdx) == TRUE)
      {
        calculateProtocolChecksum = TCPIP_IP_PROTOCOL_CHECKSUM_CHECK_HW_OR_NOT_REQ;
      }
      else
  ...
  return calculateProtocolChecksum;
} /* IpV4_Ip_CheckIfChecksumCalcIsRequired */
  ...
  boolean                                   RetryOnPhysAddrMiss)
{
  ...
  uint16 payloadLenLocal = IpPayloadLen;
  TcpIp_LocalAddrV4IterType localAddrV4Idx = IpTxReqDescrPtr->SrcAddrIdx;
  /*@ assert localAddrV4Idx < TcpIp_GetSizeOfLocalAddrV4(); */                                                          /* VCA_TCPIP_IPV4_SOCKET_AND_LOCAL_ADDR_IDX */
  TcpIp_IpV4SocketDynIterType ipV4SocketIdx = TCPIP_SOCKET_TO_IPV4_SOCKET(IpTxReqDescrPtr->SockIdx);
  /*@ assert ipV4SocketIdx < TcpIp_GetSizeOfIpV4SocketDyn(); */                                                         /* VCA_TCPIP_IPV4_SOCKET_AND_LOCAL_ADDR_IDX */
                                                                  ...
                                                                  IPV4_IP_TRANSFER_BLOCK_FLAG_PHYS_DEST_ADDR))
  {
    ...
    TcpIp_VCopyPhysAddr(destMacAddr, transferBlockPtr->PhysDestAddr);
  }
  else
  {
    retVal = IpV4_Ip_VGetNextHopPhysAddr(ipV4CtrlIdx, IpTxReqDescrPtr->DestAddrV4, ipV4SocketIdx, &destMacAddr[0]);
  ...
  if ((retVal != TCPIP_E_PHYS_ADDR_MISS) || (RetryOnPhysAddrMiss == FALSE))
  {
    ...
    /* #50 Trigger transmission of all fragments. */
    retVal |= IpV4_Ip_VTransmitFragments(ipV4CtrlIdx, IpTxReqDescrPtr, IpProtocol, payloadLenLocal, &destMacAddr[0],
                                         ReqTxConfirmation, UlTxReqIdx, identification);
  }
 ...
 *
 */
FUNC(void, TCPIP_CODE) IpV4_Ip_CancelTransmit(
  IPV4_P2C(TcpIp_IpTxRequestDescriptorType) IpTxReqDescrPtr,                                                            /* PRQA S 3673 */ /* MD_IPV4_Rule8.3 */
  uint8                                     UlTxReqIdx
  )
{
  ...
  /* ----- Implementation ----------------------------------------------- */
  /* #10 Release all requested fragments */
  (void)IpV4_Ip_VTransmitFragments(ipV4CtrlIdx, IpTxReqDescrPtr, 0u, 0u, &destMacAddr[0], FALSE, UlTxReqIdx, 0u);
}
  ...
  IPV4_P2V(uint8)           PhysAddrPtr)
{
  /* ----- Local Variables ---------------------------------------------- */
  TcpIp_IpV4CtrlIterType    ipV4CtrlIdx  = TcpIp_GetSizeOfIpV4Ctrl();
  TcpIp_SizeOfEthIfCtrlType ethIfCtrlIdx = TcpIp_GetSizeOfEthIfCtrl();
  ...
  boolean           InitRes)
{
  ...
  if (TcpIp_IsIpV4CtrlUsedOfEthIfCtrl(EthIfCtrlIdx) == TRUE)
  {
    ...
    {
      /* address is inside the local subnet */
      retVal = IpV4_Arp_GetPhysicalAddress(ipV4CtrlIdx, IPV4_INV_SOCK_IDX, IpAddr, PhysAddrPtr, InitRes);
    }
...
# endif
  {
    ...
    if (errorId == TCPIP_E_NO_ERROR)
    {
      ...
      TcpIp_IpV4SocketDynIterType ipV4SocketIdx   = TcpIp_GetIpV4SocketDynIdxOfBuf2TxReqMap(buf2TxReqMapIdx);
      ...
      if (ipV4SocketIdx < TcpIp_GetSizeOfSocketUdpDyn())
      {
        ...
        {
          /* #50 Check whether there has been a preceding ProvideTxBuffer call at the LL:
          ...
          {
            uint8 txReqElemIdx = TcpIp_GetUlTxReqIdxOfBuf2TxReqMap(buf2TxReqMapIdx);
           /*@ assert txReqElemIdx < TcpIp_GetSizeOfTxReqElem(); */                                                     /* VCA_TCPIP_TX_REQ_ELEM_IDX */
            TcpIp_Udp_TxConfirmation(TCPIP_SOCKET_IDX_TO_UDP_IDX(ipV4SocketIdx), txReqElemIdx);
          }
        }
        else
        {
...
TCPIP_LOCAL FUNC(void, TCPIP_CODE) IpV4_Ip_VCtrlStateOnHold(uint8 IpV4CtrlIdx)
{
  ...
  TcpIp_SetRequestedStateOfIpV4CtrlDyn(IpV4CtrlIdx, IPV4_IP_ONHOLD);
...
TCPIP_LOCAL FUNC(void, TCPIP_CODE) IpV4_Ip_VCtrlStateStartup(uint8 IpV4CtrlIdx)
{
  ...
  TcpIp_SetRequestedStateOfIpV4CtrlDyn(IpV4CtrlIdx, IPV4_IP_ONLINE_PASSIVE);
  ...
  P2VAR(IpBase_AddrInType, AUTOMATIC, TCPIP_APPL_DATA) NetAddrPtr)
{
  ...
  if (!TCPIP_IPV4_ADDR_IDX_IS_ASBC(LocalAddrV4Idx))                                                                     /* PRQA S 3415 2 */ /* MD_TCPIP_Rule13.5_3415_CslReadOnly */
  {
    ...
    if(TcpIp_IsMulticastAddrV4UsedOfLocalAddrV4(LocalAddrV4Idx) == TRUE)
    {
      ...
      {
        *NetAddrPtr = TcpIp_GetNetAddrOfMulticastAddrV4Requested(multicastAddrV4Idx);
        retVal = E_OK;
      }
    }
    else
    {
      ...
      *NetAddrPtr = TcpIp_GetActiveNetAddrOfIpV4CtrlDyn(ipV4CtrlIdx);
  ...
  {
    *NetAddrPtr = TcpIp_GetLastBcAddrOfIpV4CtrlDyn(ipV4CtrlIdx);
    retVal = E_OK;
  }
...
{
  /* ----- Local Variables ---------------------------------------------- */
  TcpIp_SizeOfIpV4CtrlType ipV4CtrlIdx = TcpIp_GetSizeOfIpV4Ctrl();
  ...
  *DefRouterNetAddrPtr = TcpIp_GetActiveDefGwAddrOfIpV4CtrlDyn(ipV4CtrlIdx);
}
  ...
  P2VAR(IpBase_AddrInType, AUTOMATIC, TCPIP_APPL_DATA) NetMaskPtr)
{
  ...
  *NetMaskPtr = 0;
  ...
  {
    *NetMaskPtr = TcpIp_GetActiveNetMaskOfIpV4CtrlDyn(ipV4CtrlIdx);
    retVal = E_OK;
  }
...
{
  /* ----- Local Variables ---------------------------------------------- */
  uint8 retVal = TCPIP_IPADDR_ASSIGNMENT_NONE;
  ...
  else
  {
    ...
    {
      /* retVal is TCPIP_IPADDR_ASSIGNMENT_NONE */
    }
  }
  ...
  IPV4_P2V(uint8)           MatchTypePtr)
{
  ...
  {
    *MatchTypePtr = TCPIP_ADDR_MATCH_ANYIP;
    retVal = TRUE;
  }
  else
  {
    ...
    {
      *MatchTypePtr = TCPIP_ADDR_MATCH_EXACT;
      retVal = TRUE;
    }
    else
    {
      ...
      if (TCPIP_IPV4_ADDR_IDX_IS_ASBC(BindLocalAddrV4Idx) || TCPIP_IPV4_ADDR_IDX_IS_ASBC(RecvLocalAddrV4Idx))           /* PRQA S 3415 */ /* MD_TCPIP_Rule13.5_3415_CslReadOnly */
      {
        ...
        {
          *MatchTypePtr = TCPIP_ADDR_MATCH_INTERFACE;
          retVal = TRUE;
        }
  ...
  if (retVal == FALSE)
  {
    *MatchTypePtr = TCPIP_ADDR_MATCH_NONE;
  }
  ...
  IpBase_AddrInType           DefaultRouter)
{
  ...
  if (LocalIpAddr == IPV4_ADDR_ANY)
  {
    if (TcpIp_IsDefaultUnicastAddrV4UsedOfUnicastAddrV4(UnicastAddrV4Idx) == TRUE)
    {
      ...
      /*@ assert staticUnicastAddrV4RequestedIdx < TcpIp_GetSizeOfStaticUnicastAddrV4Requested(); */                    /* VCA_TCPIP_STATIC_UNICAST_ADDR_V4_REQUESTED */

      TcpIp_SetNetAddrOfStaticUnicastAddrV4Requested(staticUnicastAddrV4RequestedIdx,
                                                     TcpIp_GetNetAddrOfDefaultUnicastAddrV4(defaultUnicastAddrV4Idx));
      TcpIp_SetNetMaskOfStaticUnicastAddrV4Requested(staticUnicastAddrV4RequestedIdx,
                                                     TcpIp_GetNetMaskOfDefaultUnicastAddrV4(defaultUnicastAddrV4Idx));
      TcpIp_SetDefGwAddrOfStaticUnicastAddrV4Requested(staticUnicastAddrV4RequestedIdx,
...
# endif
  ...
  else
  {
    if (!(IPV4_ADDR_IS_MULTICAST(LocalIpAddr) || IPV4_ADDR_IS_DIRECTED_BROADCAST(IpV4CtrlIdx, LocalIpAddr)))
    {
      ...
      TcpIp_SetNetAddrOfStaticUnicastAddrV4Requested(staticUnicastAddrV4RequestedIdx, LocalIpAddr);
      ...
      {
        TcpIp_SetNetMaskOfStaticUnicastAddrV4Requested(staticUnicastAddrV4RequestedIdx, 0u);
      }

      TcpIp_SetDefGwAddrOfStaticUnicastAddrV4Requested(staticUnicastAddrV4RequestedIdx, DefaultRouter);
  ...
  IpBase_AddrInType          DefaultRouter)
{
  ...
  if(TcpIp_GetStateOfIpV4CtrlDyn(ipV4CtrlIdx) != IPV4_IP_OFFLINE)
  {
    ...
    {
      TcpIp_UnicastAddrV4IterType unicastAddrV4Idx = TcpIp_GetUnicastAddrV4IdxOfLocalAddrV4(LocalAddrV4Idx);
      /*@ assert unicastAddrV4Idx < TcpIp_GetSizeOfUnicastAddrV4(); */                                                  /* VCA_TCPIP_CSL03_IS_USEDOF */
      ...
      if(addrAssignmentPrio != IPV4_IP_ADDR_ASSIGNMENT_PRIO_NONE)
      {
        ...
        TcpIp_AddrAssignmentCfgByPrioType addrAssignmentCfg = 0u;
        ...
        if(!IPV4_CHK_FLAG_U8(addrAssignmentCfg, TCPIP_IPV4_ADDR_ASSIGNMENT_FLAG_AUTO_TRIGGER))
        {
          ...
          {
            retVal = IpV4_Ip_StoreRequestedAddress(ipV4CtrlIdx, unicastAddrV4Idx, LocalIpAddr, NetmaskCidr, DefaultRouter);
  ...
  TcpIp_IpV4SocketDynIterType IpV4SocketIdx)
{
  ...
  transferBlockPtr->PhysDestAddr[0] = 0;
  transferBlockPtr->PhysDestAddr[1] = 0;
  transferBlockPtr->PhysDestAddr[2] = 0;
  transferBlockPtr->PhysDestAddr[3] = 0;
  transferBlockPtr->PhysDestAddr[4] = 0;
  transferBlockPtr->PhysDestAddr[5] = 0;
  transferBlockPtr->EthIfFramePrio  = 0;
  transferBlockPtr->TimeToLive      = 0;
  transferBlockPtr->TypeOfService   = 0;
  transferBlockPtr->Flags           = 0;
} /* IpV4_Ip_ResetSocket */
  ...
  uint8                       Value)
{
  ...
  TcpIp_GetAddrTransferBlockOfIpV4SocketDyn(IpV4SocketIdx)->TimeToLive = Value;
  TcpIp_GetAddrTransferBlockOfIpV4SocketDyn(IpV4SocketIdx)->Flags |= IPV4_IP_TRANSFER_BLOCK_FLAG_TIME_TO_LIVE;
} /* IpV4_Ip_SetTimeToLive */
  ...
  uint8                   Value)
{
  ...
  TcpIp_GetAddrTransferBlockOfIpV4SocketDyn(IpV4SocketIdx)->TypeOfService = Value;
  TcpIp_GetAddrTransferBlockOfIpV4SocketDyn(IpV4SocketIdx)->Flags |= IPV4_IP_TRANSFER_BLOCK_FLAG_TYPE_OF_SERVICE;
} /* IpV4_Ip_SetTypeOfService */
  ...
  IPV4_P2C(uint8)         PhysDestAddrPtr)
{
  ...
  if(PhysDestAddrPtr != NULL_PTR)
  {
    ...
    {
      transferBlockPtr->PhysDestAddr[physAddrByteIdx] = PhysDestAddrPtr[physAddrByteIdx];
    }

    IPV4_SET_FLAG_U8(transferBlockPtr->Flags, IPV4_IP_TRANSFER_BLOCK_FLAG_PHYS_DEST_ADDR);
  }
  else
  /* #20 Otherwise clear the 'valid' flag. */
  {
    IPV4_CLR_FLAG_U8(transferBlockPtr->Flags, IPV4_IP_TRANSFER_BLOCK_FLAG_PHYS_DEST_ADDR);
  }
} /* IpV4_Ip_SetPhysDestAddr */
  ...
  uint8                   Value)
{
  ...
  {
    TcpIp_GetAddrTransferBlockOfIpV4SocketDyn(IpV4SocketIdx)->EthIfFramePrio = Value;
    TcpIp_GetAddrTransferBlockOfIpV4SocketDyn(IpV4SocketIdx)->Flags |= IPV4_IP_TRANSFER_BLOCK_FLAG_ETHIF_FRAME_PRIO;
    retVal = E_OK;
  }
  ...
  TcpIp_LocalAddrV4IterType LocalAddrV4Idx)
{
  ...
  if (TcpIp_IsLlAddrStateUsedOfIpV4Ctrl(ipV4CtrlIdx) == TRUE)
  {
    ...
    {
      TcpIp_SetStateOfLlAddrState(llAddrStateIdx, IPV4_IP_LL_STATE_CONFLICT);
    ...
    {
      TcpIp_AutoIpConfigIdxOfIpV4CtrlType autoIpConfigIdx = TcpIp_GetSizeOfAutoIpConfig();
      ...
      TcpIp_SetAddrConflictTimeoutOfLlAddrState(llAddrStateIdx,
    ...
    IpBase_AddrInType      IpAddr)
{
  ...
  {
    /*@ assert llAddrStateIdx < TcpIp_GetSizeOfLlAddrState(); */                                                        /* VCA_TCPIP_LINKLOCAL_ADDRESS_IDX */
    ...
    {
      TcpIp_SetAddrConflictOfLlAddrState(llAddrStateIdx, TRUE);
    }
  }
} /* IpV4_Ip_LlAddrInUse */
...
#  endif
  ...
  {
    TcpIp_DemEventIdxOfIpV4CtrlType demEventIdx = TcpIp_GetDemEventIdxOfIpV4Ctrl(IpV4CtrlIdx);
    /*@ assert demEventIdx < TcpIp_GetSizeOfDemEvent(); */                                                              /* VCA_TCPIP_CSL03_IS_USEDOF */
    ...
    else
    {
      ...
      TcpIp_SizeOfLastDuplicateDhcpAddrType lastDuplicateDhcpAddrIdx =
        TcpIp_GetLastDuplicateDhcpAddrIdxOfDemEvent(demEventIdx);
      /*@ assert lastDuplicateDhcpAddrIdx < TcpIp_GetSizeOfLastDuplicateDhcpAddr(); */                                  /* VCA_TCPIP_DUPLICATE_DHCP_ADDR_EXISTS */
      TCPIP_P2C(uint8) srcAddrPtr = &TcpIp_GetAddrPhysAddrOfLastDuplicateDhcpAddr(lastDuplicateDhcpAddrIdx)->data[0];

      TCPIP_ASSERT(TcpIp_IsLastDuplicateDhcpAddrUsedOfDemEvent(demEventIdx) == TRUE);

      TcpIp_VCopyPhysAddr(PhysAddrPtr, srcAddrPtr);

      *IpAddrPtr = TcpIp_GetNetAddrOfLastDuplicateDhcpAddr(lastDuplicateDhcpAddrIdx);
      retVal = E_OK;
    }
  }
...
{
  /* ----- Local Variables ---------------------------------------------- */
    IpBase_EthPhysAddrType physMulticastAddr = { 0u };
  ...
  if(IPV4_ADDR_IS_MULTICAST(MulticastAddr))
  {
    ...
    if(TCPIP_LL_UpdatePhysAddrFilter(TcpIp_GetEthIfCtrlIdxOfIpV4Ctrl(IpV4CtrlIdx),
                                     &physMulticastAddr[0],
  ...
  boolean                Join)
{
  ...
  if(TcpIp_IsMulticastAddrV4UsedOfIpV4Ctrl(IpV4CtrlIdx) == TRUE)
  {
        ...
        multicastAddrV4Idx++)
    {
      ...
      if(TcpIp_GetNetAddrOfMulticastAddrV4Requested(multicastAddrV4Idx) != IPV4_ADDR_ANY)
      {
        TcpIp_SizeOfLocalAddrV4Type localAddrV4Idx = TcpIp_GetLocalAddrV4IdxOfMulticastAddrV4(multicastAddrV4Idx);
        /*@ assert localAddrV4Idx < TcpIp_GetSizeOfLocalAddrV4(); */                                                    /* VCA_TCPIP_IPV4_LOC_ADDR_IDX */
...
# endif
        IpV4_VLocalIpAddrAssignmentChange(localAddrV4Idx, AddrState);
      }
    }
  }
...
# if (TCPIP_SUPPORT_IP_ADDR_NVM_STORAGE == STD_ON)
      ...
      if (IPV4_CHK_FLAG_U8(addrAssignmentCfg, TCPIP_IPV4_ADDR_ASSIGNMENT_FLAG_LIFETIME_STORE))
      {
        ...
        if (IpV4_Ip_NvmGetAddrSet(IpV4CtrlIdx, &ipAddrSetPtr, FALSE) == E_OK)
        {
...
# endif
      {
        if (IpV4_Ip_VGetActiveAddressOfAssignmentMethod(IpV4CtrlIdx, addrAssignmentMethod, AddrInfoPtr) == TRUE)
        {
          addrAssignmentMethodReady = addrAssignmentMethod;
        }
      }
...
# if (TCPIP_SUPPORT_DHCPV4 == STD_ON)
  case TCPIP_IPADDR_ASSIGNMENT_DHCP:
    if(TcpIp_IsDhcpConfigUsedOfIpV4Ctrl(IpV4CtrlIdx) == TRUE)                                                           /* PRQA S 3415 */ /* MD_TCPIP_Rule13.5_3415_CslReadOnly */
    {
      ...
      retVal = TcpIp_DhcpV4_GetActiveAddress(IpV4CtrlIdx, AddrInfoPtr);
    }
    break;
# endif
  ...
  case TCPIP_IPADDR_ASSIGNMENT_LINKLOCAL:
    {
      ...
      {
        AddrInfoPtr->NetAddr = TcpIp_GetAddrFoundOfLlAddrState(llAddrStateIdx);
        AddrInfoPtr->NetMask = IPV4_IP_LL_NET_ADDR_MASK;
        AddrInfoPtr->DefGwAddr = TCPIP_INADDR_ANY;
        retVal = TRUE;
      }
    }
    break;
  ...
  case TCPIP_IPADDR_ASSIGNMENT_STATIC:
  {
    ...
    AddrInfoPtr->NetAddr   = TcpIp_GetNetAddrOfStaticUnicastAddrV4Requested(staticUnicastAddrV4RequestedIdx);
    AddrInfoPtr->NetMask   = TcpIp_GetNetMaskOfStaticUnicastAddrV4Requested(staticUnicastAddrV4RequestedIdx);
    AddrInfoPtr->DefGwAddr = TcpIp_GetDefGwAddrOfStaticUnicastAddrV4Requested(staticUnicastAddrV4RequestedIdx);
    retVal = TRUE;
    break;
  }
  ...
  TcpIp_AddrAssignmentCfgByPrioType AddrAssignmentCfg)
{
  ...
  if(!IPV4_CHK_BIT_U8(TcpIp_GetAddrAssignmentTriggeredFlagsOfIpV4CtrlDyn(IpV4CtrlIdx), AddrAssignmentMethod))
  {
    ...
    TcpIp_SetAddrAssignmentTriggeredFlagsOfIpV4CtrlDyn(IpV4CtrlIdx, addrAssignmentTriggeredFlagsOfIpV4CtrlDyn);
...
# endif
        case TCPIP_IPADDR_ASSIGNMENT_LINKLOCAL:
          if (TcpIp_IsLlAddrStateUsedOfIpV4Ctrl(IpV4CtrlIdx) == TRUE)
          {
            ...
            IpV4_Ip_VLl_Start(llAddrStateIdx);
            retVal = E_OK;
          }
          break;
  ...
  TcpIp_IpV4CtrlIterType IpV4CtrlIdx)
{
       ...
       addrAssignmentCfgByPrioIdx++)
  {
        ...
        && ((addrAssignmentCfg & TCPIP_IPV4_ADDR_ASSIGNMENT_FLAG_AUTO_TRIGGER) != 0u))
    {
      ...
      if(addrAssignmentMethod == TCPIP_IPADDR_ASSIGNMENT_STATIC)
      {
        TcpIp_DefaultUnicastAddrV4IdxOfUnicastAddrV4Type defaultUnicastAddrV4Idx = TcpIp_GetSizeOfDefaultUnicastAddrV4();
        ...
        TcpIp_SizeOfStaticUnicastAddrV4RequestedType staticUnicastAddrV4RequestedIdx = TcpIp_GetStaticUnicastAddrV4RequestedIdxOfUnicastAddrV4(unicastAddrV4Idx);
...
{
  /* ----- Local Variables ---------------------------------------------- */
  /* IpV4Ctrl is not in IPV4_IP_ONLINE_ACTIVE state yet */
  /* ----- Implementation ----------------------------------------------- */
  /* #10 Prepare state offline from current state. */
  boolean isOnlineActive = IpV4_Ip_PrepareCtrlStateOffline(IpV4CtrlIdx);
  ...
  if(TcpIp_GetStateOfIpV4CtrlDyn(IpV4CtrlIdx) == IPV4_IP_ONLINE_PASSIVE)
  {
    ...
    if (TcpIp_IsMulticastAddrV4UsedOfIpV4Ctrl(IpV4CtrlIdx) == TRUE)
    {
      ...
      for (multicastAddrV4Idx = TcpIp_GetMulticastAddrV4StartIdxOfIpV4Ctrl(IpV4CtrlIdx);
           ...
           multicastAddrV4Idx++)
      {
        /*@ assert multicastAddrV4Idx < TcpIp_GetSizeOfMulticastAddrV4(); */                                            /* VCA_TCPIP_CSL03_START_END */
        TcpIp_SizeOfDefaultMulticastAddrV4Type defaultMulticastAddrV4Idx =
          TcpIp_GetDefaultMulticastAddrV4IdxOfMulticastAddrV4(multicastAddrV4Idx);
        /*@ assert defaultMulticastAddrV4Idx < TcpIp_GetSizeOfMulticastAddrV4(); */                                     /* VCA_TCPIP_IPV4_LOC_ADDR_IDX */
  ...
  TcpIp_IpV4CtrlIterType IpV4CtrlIdx)
{
  ...
  if (TcpIp_GetStateOfIpV4CtrlDyn(IpV4CtrlIdx) == IPV4_IP_ONHOLD)
  {
    ...
    TcpIp_SetStateOfIpV4CtrlDyn(IpV4CtrlIdx, IPV4_IP_ONLINE_PASSIVE);
  }
  ...
  TcpIp_IpV4CtrlIterType IpV4CtrlIdx)
{
         ...
         && (TcpIp_GetPreOnHoldOfIpV4CtrlDyn(IpV4CtrlIdx) == IPV4_IP_ONLINE_ACTIVE)))
  {
    ...
    else
    {
      ...
      TcpIp_SetPreOnHoldOfIpV4CtrlDyn(IpV4CtrlIdx, IPV4_IP_ONLINE_PASSIVE);
    }
  ...
  TcpIp_IpV4CtrlIterType IpV4CtrlIdx)
{
  ...
  if(TcpIp_GetStateOfIpV4CtrlDyn(IpV4CtrlIdx) == IPV4_IP_ONLINE_PASSIVE)
  {
    ...
    TcpIp_SetStateOfIpV4CtrlDyn(IpV4CtrlIdx, IPV4_IP_ONLINE_ACTIVE);
  ...
  TcpIp_IpV4CtrlIterType IpV4CtrlIdx)
{
      ...
      || (TcpIp_GetStateOfIpV4CtrlDyn(IpV4CtrlIdx) == IPV4_IP_ONLINE_ACTIVE))
  {
    ...
    TcpIp_SetPreOnHoldOfIpV4CtrlDyn(IpV4CtrlIdx, TcpIp_GetStateOfIpV4CtrlDyn(IpV4CtrlIdx));
    ...
    /* Ctrl to OnHold */
    TcpIp_SetStateOfIpV4CtrlDyn(IpV4CtrlIdx, IPV4_IP_ONHOLD);
  ...
  TcpIp_IpV4CtrlIterType IpV4CtrlIdx)
{
  ...
  {
    /* Set previous state */
    TcpIp_SetStateOfIpV4CtrlDyn(IpV4CtrlIdx, TcpIp_GetPreOnHoldOfIpV4CtrlDyn(IpV4CtrlIdx));
  ...
  IpBase_AddrInType         NetAddr)
{
  /* ----- Local Variables ---------------------------------------------- */
  Std_ReturnType retVal = E_NOT_OK;
  IpBase_AddrInType prevMcNetAddr = IPV4_ADDR_ANY;
  TcpIp_SizeOfMulticastAddrV4Type multicastAddrV4Idx = 0u;
  TcpIp_SizeOfIpV4CtrlType ipV4CtrlIdx = TcpIp_GetIpV4CtrlIdxOfLocalAddrV4(LocalAddrV4Idx);
  ...
  /*@ assert multicastAddrV4Idx < TcpIp_GetSizeOfMulticastAddrV4(); */                                                  /* VCA_TCPIP_CSL03_IS_USEDOF */
...
# endif
      ...
      if(NetAddr != IPV4_ADDR_ANY)
      {
        ...
        TcpIp_SetNetAddrOfMulticastAddrV4Requested(multicastAddrV4Idx, NetAddr);
      }
      else
      {
  ...
  TcpIp_ConflictsNumOfLlAddrStateType ConflictCount)
{
  ...
  if(TcpIp_IsAutoIpConfigUsedOfIpV4Ctrl(IpV4CtrlIdx) == TRUE)
  {
    ...
    /*@ assert autoIpConfigIdx < TcpIp_GetSizeOfAutoIpConfig(); */                                                      /* VCA_TCPIP_CSL03_IS_USEDOF */
    ...
    {
      addrCalloutFunc(TCPIP_LOCAL_ADDR_IDX_TO_ID(localAddrV4UnicastIdx), (uint8)ConflictCount, &ipAddr);
    }
  }
  ...
  {
    ipAddr = IPV4_IP_LL_NET_ADDR_START_NBO + ((uint32) TcpIp_GetRandomNumber() % IPV4_IP_LL_NET_ADDR_COUNT);
  }
  ...
  TcpIp_LlAddrStateIdxOfIpV4CtrlType LlAddrStateIdx)
{
  ...
  TcpIp_SetAddrConflictOfLlAddrState(LlAddrStateIdx, FALSE);
  TcpIp_SetAddrFoundOfLlAddrState(LlAddrStateIdx, 0);
  TcpIp_SetNetAddrCandOfLlAddrState(LlAddrStateIdx, 0);
  TcpIp_SetMsecCntOfLlAddrState(LlAddrStateIdx, 0);
  TcpIp_SetProbeNumSinceConfOfLlAddrState(LlAddrStateIdx, 0);
  TcpIp_SetAnnounceNumOfLlAddrState(LlAddrStateIdx, 0);
  TcpIp_SetConflictsNumOfLlAddrState(LlAddrStateIdx, 0);

  TcpIp_SetStateOfLlAddrState(LlAddrStateIdx, IPV4_IP_LL_STATE_INACTIVE);
  TcpIp_SetProbeStateOfLlAddrState(LlAddrStateIdx, IPV4_IP_LL_PROBE_STATE_CHOOSE);
  TcpIp_SetAnnStateOfLlAddrState(LlAddrStateIdx, IPV4_IP_LL_ANN_STATE_PROBE);

  TcpIp_SetWaitUntilTimeOfLlAddrState(LlAddrStateIdx, 0);
} /* IpV4_Ip_VLl_Reset */
  ...
  TcpIp_LlAddrStateIdxOfIpV4CtrlType LlAddrStateIdx)
{
  ...
  {
    TcpIp_SetStateOfLlAddrState(LlAddrStateIdx, IPV4_IP_LL_STATE_INIT);
  }
  else
  {
    TCPIP_ASSERT_UNREACHABLE();
  ...
  TcpIp_IpV4CtrlIterType IpV4CtrlIdx)
{
  ...
  if(TcpIp_GetStateOfLlAddrState(llAddrStateIdx) != IPV4_IP_LL_STATE_INACTIVE)
  {
    if(TcpIp_GetAddrConflictTimeoutOfLlAddrState(llAddrStateIdx) > 0u)
    {
      ...
      TcpIp_DecAddrConflictTimeoutOfLlAddrState(llAddrStateIdx);
    }
  ...
  TcpIp_AutoIpConfigIdxOfIpV4CtrlType AutoIpConfigIdx)
{
  ...
  /* #20 Otherwise process the sub-state related to the state PROBE. */
  {
    ...
    {
      TcpIp_SetNetAddrCandOfLlAddrState(LlAddrStateIdx,
                                        IpV4_Ip_VLl_GenerateIpAddr(IpV4CtrlIdx,
                                                                   TcpIp_GetConflictsNumOfLlAddrState(LlAddrStateIdx)));
      TcpIp_SetProbeStateOfLlAddrState(LlAddrStateIdx, IPV4_IP_LL_PROBE_STATE_PROBE);
    }
    ...
    if(TcpIp_GetProbeStateOfLlAddrState(LlAddrStateIdx) == IPV4_IP_LL_PROBE_STATE_PROBE)
    {
      ...
      {
        TcpIp_IncProbeNumSinceConfOfLlAddrState(LlAddrStateIdx);
        TcpIp_SetProbeStateOfLlAddrState(LlAddrStateIdx, IPV4_IP_LL_PROBE_STATE_WAIT);
        ...
        if(TcpIp_GetProbeNumSinceConfOfLlAddrState(LlAddrStateIdx) == TcpIp_GetProbeNumOfAutoIpConfig(AutoIpConfigIdx))
        {
          ...
          TcpIp_SetWaitUntilTimeOfLlAddrState(LlAddrStateIdx,
        ...
        else
        {
          ...
          if(   TcpIp_GetProbeMaxMsOfAutoIpConfig(AutoIpConfigIdx)
              > TcpIp_GetProbeMinMsOfAutoIpConfig(AutoIpConfigIdx))
          {
            uint16 RandNo = TcpIp_GetRandomNumber();
            TcpIp_SetWaitUntilTimeOfLlAddrState(LlAddrStateIdx,
    ...
    else if(TcpIp_GetProbeStateOfLlAddrState(LlAddrStateIdx) == IPV4_IP_LL_PROBE_STATE_WAIT)
    {
      if(TcpIp_GetWaitUntilTimeOfLlAddrState(LlAddrStateIdx) < TcpIp_GetMsecCntOfLlAddrState(LlAddrStateIdx))
      {
        ...
        {
          /* goto announce */
          TcpIp_SetStateOfLlAddrState(LlAddrStateIdx, IPV4_IP_LL_STATE_ANNOUNCE);
          TcpIp_SetAnnStateOfLlAddrState(LlAddrStateIdx, IPV4_IP_LL_ANN_STATE_PROBE);
          ...
          TcpIp_SetAddrFoundOfLlAddrState(LlAddrStateIdx, TcpIp_GetNetAddrCandOfLlAddrState(LlAddrStateIdx));
  ...
  TcpIp_AutoIpConfigIdxOfIpV4CtrlType AutoIpConfigIdx)
{
  ...
  TcpIp_SetAddrConflictOfLlAddrState(LlAddrStateIdx, FALSE);
  TcpIp_SetProbeNumSinceConfOfLlAddrState(LlAddrStateIdx, 0);
  TcpIp_IncConflictsNumOfLlAddrState(LlAddrStateIdx);
  ...
  if(TcpIp_GetMaxConflictsOfAutoIpConfig(AutoIpConfigIdx) > TcpIp_GetConflictsNumOfLlAddrState(LlAddrStateIdx))
  {
    ...
    TcpIp_SetWaitUntilTimeOfLlAddrState(LlAddrStateIdx, TcpIp_GetMsecCntOfLlAddrState(LlAddrStateIdx));
  }
  else
  {
  ...
  TcpIp_SetStateOfLlAddrState(LlAddrStateIdx, IPV4_IP_LL_STATE_WAIT);
} /* IpV4_Ip_VLl_HandleStateConflict */
  ...
  TcpIp_AutoIpConfigIdxOfIpV4CtrlType AutoIpConfigIdx)
{
  ...
  if(TcpIp_GetAnnStateOfLlAddrState(LlAddrStateIdx) == IPV4_IP_LL_ANN_STATE_PROBE)
  {
    ...
    if(IpV4_Arp_SendArpAnnouncement(IpV4CtrlIdx, TcpIp_GetAddrFoundOfLlAddrState(LlAddrStateIdx)) == E_OK)
    {
      ...
      TcpIp_IncAnnounceNumOfLlAddrState(LlAddrStateIdx);
      TcpIp_SetAnnStateOfLlAddrState(LlAddrStateIdx, IPV4_IP_LL_ANN_STATE_WAIT);
  ...
  else if(TcpIp_GetAnnStateOfLlAddrState(LlAddrStateIdx) == IPV4_IP_LL_ANN_STATE_WAIT)
  {
    if(TcpIp_GetWaitUntilTimeOfLlAddrState(LlAddrStateIdx) < TcpIp_GetMsecCntOfLlAddrState(LlAddrStateIdx))
    {
      ...
      if(TcpIp_GetAnnounceNumOfLlAddrState(LlAddrStateIdx) >= TcpIp_GetAnnounceNumOfAutoIpConfig(AutoIpConfigIdx))
      {
        ...
        TcpIp_SetConflictsNumOfLlAddrState(LlAddrStateIdx, 0);
        /* Goto ready */
        TcpIp_SetStateOfLlAddrState(LlAddrStateIdx, IPV4_IP_LL_STATE_READY);
      }
      else
      {
        /* next announcement */
  ...
  TcpIp_IpV4CtrlIterType IpV4CtrlIdx)
{
  ...
  case IPV4_IP_LL_STATE_WAIT:
    {
      ...
      if(TcpIp_GetWaitUntilTimeOfLlAddrState(llAddrStateIdx) <= TcpIp_GetMsecCntOfLlAddrState(llAddrStateIdx))
      {
        TcpIp_SetStateOfLlAddrState(     llAddrStateIdx, IPV4_IP_LL_STATE_PROBE);
        TcpIp_SetProbeStateOfLlAddrState(llAddrStateIdx, IPV4_IP_LL_PROBE_STATE_CHOOSE);
      }
      else
      {
  ...
  TcpIp_SetMsecCntOfLlAddrState(llAddrStateIdx,
...
# if (TCPIP_SUPPORT_IPSEC == STD_ON)
  isIPsecUsed = (ipProtocol == TCPIP_PROTOCOL_AH);

  /* #10 If valid IpSecConfigset is present, perform IpSec processing of the packet */
  if (TcpIp_IsIpSecConfigSetUsedOfIpV4Ctrl(IpV4CtrlIdx) == TRUE)
  {
    ipSecRetVal = TcpIp_IpSec_ProcessRxPacket(IpV4CtrlIdx, DataPtr, upperLayerPayloadLen, &ipProtocol, &ipSecHdrLen);
  }
...
# if (TCPIP_SUPPORT_IPSEC == STD_ON)
        /* HW can not validate checksum if IPsec is in use. */
        if (isIPsecUsed == TRUE)
        {
          checksumValidated = FALSE;
        }
# endif
        if(WasFragmented == TRUE)
        {
          checksumValidated = FALSE;
        }
...
# if (TCPIP_SUPPORT_DHCPV4 == STD_ON)
        ...
        if(TcpIp_DhcpV4_RxIndication(IpV4CtrlIdx, upperLayerPayloadPtr, upperLayerPayloadLen,
...
# endif
        ...
        {
          if (TcpIp_Udp_RxIndication(TCPIP_IPV4_TO_LOCAL_ADDR_IDX(LocalAddrV4Idx), &IpRxSockAddrRem,
...
# if (TCPIP_SUPPORT_ICMPV4_DST_UNREACHABLE_MSG == STD_ON)
            ...
            {
              IpV4_Icmp_SendDestUnreachable(LocalAddrV4Idx, DataPtr, IPV4_ICMP_CODE_PORT_UNREACHABLE);
            }
# endif
          }
        }
...
#  endif
        ...
        if(IPV4_IP_ONLINE_ACTIVE == TcpIp_GetStateOfIpV4CtrlDyn(IpV4CtrlIdx))
        {
          ...
          if (!(IPV4_ADDR_IS_MULTICAST(destAddr) || IPV4_ADDR_IS_BROADCAST(IpV4CtrlIdx, destAddr)))
          {
            ...
            TcpIp_Tcp_RxIndication((TcpIp_SizeOfLocalAddrType)LocalAddrV4Idx, &IpRxSockAddrRem, upperLayerPayloadPtr, upperLayerPayloadLen, checksumValidated);
          }
        }
      }
      break;
...
# if (TCPIP_SUPPORT_ICMPV4 == STD_ON)                                                                                   /* COV_TCPIP_ICMPV4_ALWAYS_ON */
      ...
      case IPV4_IP_PROTOCOL_ICMP:
      {
        ...
        boolean checksumValidated = TcpIp_IsHwChecksumIcmpOfIpV4Ctrl(IpV4CtrlIdx);
#  if (TCPIP_SUPPORT_IPSEC == STD_ON)
        /* HW can not validate checksum if IPsec is in use. */
        if (isIPsecUsed == TRUE)
        {
...
#  endif
        ...
        IpV4_Icmp_RxIndication((TcpIp_SizeOfLocalAddrType)LocalAddrV4Idx, srcAddr, TCPIP_GET_UINT8(DataPtr, TCPIP_IPV4_HDR_OFS_TTL), upperLayerPayloadPtr, upperLayerPayloadLen,
...
# if (TCPIP_SUPPORT_ICMPV4_DST_UNREACHABLE_MSG == STD_ON)
      ...
      if (!(IPV4_ADDR_IS_MULTICAST(destAddr) || IPV4_ADDR_IS_BROADCAST(IpV4CtrlIdx, destAddr)))
      {
        IpV4_Icmp_SendDestUnreachable((TcpIp_SizeOfLocalAddrType)LocalAddrV4Idx, DataPtr, IPV4_ICMP_CODE_PROTOCOL_UNREACHABLE);
      }
# endif
      break;
    }
...
IPV4_LOCAL_INLINE FUNC(void, TCPIP_CODE) IpV4_Ip_VReassemblyInitBuffers(void)
{
       ...
       reassemblyBufferDescIdx++)
  {
    ...
    reassemblyBufDescPtr->ReassemblyBufDescIdx = reassemblyBufferDescIdx;
  }
} /* IpV4_Ip_VReassemblyInitBuffers */
  ...
  IPV4_P2V(TcpIp_SizeOfReassemblyBufferDescType) UnusedDescIdxPtr)
{
  ...
  /*@ assert fragmentationConfigIdx < TcpIp_GetSizeOfFragmentationConfig(); */                                          /* VCA_TCPIP_CSL03_IS_USEDOF */
  ...
  *UnusedDescIdxPtr = IPV4_IP_REASSEMBLY_BUF_DESC_IDX_INV; /* will be used if no matching descriptor is found */
  ...
  for ( reassemblyBufferDescIdx = TcpIp_GetReassemblyBufferDescStartIdxOfFragmentationConfig(fragmentationConfigIdx);
        ...
        reassemblyBufferDescIdx++)
  {
    /*@ assert reassemblyBufferDescIdx < TcpIp_GetSizeOfReassemblyBufferDesc(); */                                      /* VCA_TCPIP_CSL03_IS_USEDOF */
    IPV4_P2V(IpV4_Ip_ReassemblyBufferDescType) reassemblyBufDescPtr =
      TcpIp_GetAddrReassemblyBufferDescDyn(reassemblyBufferDescIdx);
    ...
    else if (*UnusedDescIdxPtr == IPV4_IP_REASSEMBLY_BUF_DESC_IDX_INV)
    {
      ...
      *UnusedDescIdxPtr = reassemblyBufferDescIdx;
    }
    else
    {
  ...
  IPV4_P2C(uint8)        DataPtr)
{
  ...
  TcpIp_SizeOfReassemblyBufferDescType reassemblyBufferDescIdx =
    IpV4_Ip_VReassemblyFindBufferDesc(IpV4CtrlIdx, DataPtr, &unusedDescIdx);
  TcpIp_FragmentationConfigIdxOfIpV4CtrlType fragmentationConfigIdx =
    TcpIp_GetFragmentationConfigIdxOfIpV4Ctrl(IpV4CtrlIdx);

  /* ----- Implementation ----------------------------------------------- */
  /* #20 Prepare unused assembly buffer descriptor if no matching reassembly buffer was found. */
  if(reassemblyBufferDescIdx == IPV4_IP_REASSEMBLY_BUF_DESC_IDX_INV)
  {
    ...
    {
      /*@ assert unusedDescIdx < TcpIp_GetSizeOfReassemblyBufferDescDyn(); */                                           /* VCA_TCPIP_REASSEMBLY_DESC_IDX */
      IPV4_P2V(IpV4_Ip_ReassemblyBufferDescType) reassemblyBufDescPtr;
      IPV4_P2V(uint8) reassemblyBufPtr;
      ...
      reassemblyBufferDescIdx = unusedDescIdx;

      /* #40 In case a free descriptor is available: Initialize its values with the info from the IP header pointer. */
      reassemblyBufDescPtr = TcpIp_GetAddrReassemblyBufferDescDyn(reassemblyBufferDescIdx);
      reassemblyBufPtr = IpV4_Ip_VReassemblyGetBufferPtr(reassemblyBufDescPtr);
      /*@ assert $lengthOf(reassemblyBufPtr) >= (IPV4_IP_REASSEMBLY_BUF_GAP_SIZE_OFS + 2); */                           /* VCA_TCPIP_FUNC_CALL_WRITE_REASSEMBLY_BUF_INFO */

      reassemblyBufDescPtr->MatchIpSrcAddr       = TCPIP_GET_UINT32_RAW(DataPtr, TCPIP_IPV4_HDR_OFS_SRC_ADDR );
      reassemblyBufDescPtr->MatchIpDstAddr       = TCPIP_GET_UINT32_RAW(DataPtr, TCPIP_IPV4_HDR_OFS_DEST_ADDR);
      reassemblyBufDescPtr->MatchIpProtocol      = TCPIP_GET_UINT8     (DataPtr, TCPIP_IPV4_HDR_OFS_PROT     );
      reassemblyBufDescPtr->MatchIdentification  = TCPIP_GET_UINT16_RAW(DataPtr, TCPIP_IPV4_HDR_OFS_IDENT    );
      reassemblyBufDescPtr->NextGapOfs           = 0u;

      TCPIP_PUT_UINT16(reassemblyBufPtr, IPV4_IP_REASSEMBLY_BUF_NEXT_GAP_OFS, firstGapOfs);
      TCPIP_PUT_UINT16(reassemblyBufPtr, IPV4_IP_REASSEMBLY_BUF_GAP_SIZE_OFS, firstGapSize);
  ...
  return reassemblyBufferDescIdx;
} /* IpV4_Ip_VReassemblyGetBufferDesc */
  ...
  IPV4_P2C(IpV4_Ip_ReassemblyBufferDescType) ReassemblyBufDescPtr)
{
  ...
  TcpIp_ReassemblyBufferStartIdxOfReassemblyBufferDescType startReassemblyBufferIdx =
    TcpIp_GetReassemblyBufferStartIdxOfReassemblyBufferDesc(reassemblyBufferDescIdx);
  return TcpIp_GetAddrReassemblyBuffer(startReassemblyBufferIdx);
}
  ...
  uint16                                     NextGapOfs)
{
  ...
  if(FragEndOfs == ThisGapEndOfs)
  {
    ...
    {
      TCPIP_PUT_UINT16(reassemblyBufPtr, PrevGapOfs, NextGapOfs);                                                       /* VCA_TCPIP_FUNC_CALL_WRITE_REASSEMBLY_BUF_INFO */
    }
    else
    {
      /* Set the very first gap offset to next gap */
      ReassemblyBufDescPtr->NextGapOfs = NextGapOfs;
    }
  }
  else
  {
    ...
    TCPIP_PUT_UINT16(reassemblyBufPtr, newGapOfs + IPV4_IP_REASSEMBLY_BUF_GAP_SIZE_OFS, (ThisGapEndOfs - FragEndOfs));  /* VCA_TCPIP_FUNC_CALL_WRITE_REASSEMBLY_BUF_INFO */

    /* If there is a previous gap, set next pointer to new gap */
    if(IPV4_IP_REASSEMBLY_BUF_NO_NEXT_GAP_OFS != PrevGapOfs)
    {
      TCPIP_PUT_UINT16(reassemblyBufPtr, PrevGapOfs, newGapOfs);                                                        /* VCA_TCPIP_FUNC_CALL_WRITE_REASSEMBLY_BUF_INFO */
    }
    else
    {
      ReassemblyBufDescPtr->NextGapOfs = newGapOfs;
    }

    /* Set next pointer of new gap to next gap (or INV) */
    TCPIP_PUT_UINT16(reassemblyBufPtr, newGapOfs, NextGapOfs);                                                          /* VCA_TCPIP_FUNC_CALL_WRITE_REASSEMBLY_BUF_INFO */
  }
} /* IpV4_Ip_VReassemblyAdaptGapBeginning */
  ...
  uint16          NextGapOfs)
{
  ...
  TCPIP_PUT_UINT16(ReassemblyBufPtr, ThisGapOfs + IPV4_IP_REASSEMBLY_BUF_GAP_SIZE_OFS, (FragOfs - ThisGapOfs));         /* VCA_TCPIP_FUNC_CALL_WRITE_REASSEMBLY_BUF_INFO */
  ...
  if(FragEndOfs != ThisGapEndOfs)
  {
    ...
    TCPIP_PUT_UINT16(ReassemblyBufPtr, ThisGapOfs, newGapOfs);                                                          /* VCA_TCPIP_FUNC_CALL_WRITE_REASSEMBLY_BUF_INFO */
  ...
  uint16                                     NextGapOfs)
{
  ...
  if(FragOfs == ThisGapOfs)
  {
    ...
    IpV4_Ip_VReassemblyAdaptGapBeginning(ReassemblyBufDescPtr, FragEndOfs, ThisGapEndOfs, PrevGapOfs, NextGapOfs);
  }
  else
  {
    ...
    IpV4_Ip_VReassemblyAdaptGapNonBeginning(reassemblyBufPtr, FragOfs, FragEndOfs,
           ...
           Shrink the gap to end behind this very last fragment (if required). Adapt gap, if matching gap was found. */
    {
      ...
      if(TRUE == FragIsLast)
      {
        ...
        if(fragEndOfs < gapEndOfs)
        {
          ...
          TCPIP_PUT_UINT16(reassemblyBufPtr, gapOfs + IPV4_IP_REASSEMBLY_BUF_GAP_SIZE_OFS, gapSize);                    /* VCA_TCPIP_FUNC_CALL_WRITE_REASSEMBLY_BUF_INFO */
        }

        ReassemblyBufDescPtr->FragmentLen = (uint16)gapEndOfs;
      }
      ...
      IpV4_Ip_VReassemblyAdaptGap(ReassemblyBufDescPtr, FragOfs, (uint16)fragEndOfs, gapOfs,
  ...
  boolean                                    FragIsLast)
{
  ...
  if(IpV4_Ip_VReassemblyHandleGaps(ReassemblyBufDescPtr, FragAbsByteOfs, FragPayloadLen, FragIsLast, &gapOfs) == E_OK)
  {
    ...
    IpBase_Copy((IpBase_CopyDataType*)&reassemblyBufPtr[FragAbsByteOfs],                                                /* VCA_TCPIP_FUNC_CALL_COPY_INTO_REASSEMBLY_BUF */
...
# endif
    ...
    else
    {
      ...
      IpBase_Copy((IpBase_CopyDataType*)&reassemblyBufPtr[FragAbsByteOfs],                                              /* VCA_TCPIP_FUNC_CALL_COPY_INTO_REASSEMBLY_BUF */
                  (const IpBase_CopyDataType*)&(FragPayloadPtr)[0], FragPayloadLen);
      ReassemblyBufDescPtr->NextGapOfs = FragAbsByteOfs + FragPayloadLen;

      /* #40 Update reassembly status to COMPLETE, if this was the last fragment. */
      if(FragIsLast == TRUE)
      {
        ReassemblyBufDescPtr->Status = IPV4_IP_REASSEMBLY_BUF_STATUS_COMPLETE;
        ReassemblyBufDescPtr->FragmentLen = ReassemblyBufDescPtr->NextGapOfs;
      }
    }
  }
  else
  ...
  IPV4_P2V(uint16)         FragPayloadLenPtr)
{
  ...
  TcpIp_ReassemblyBufferDescIterType  reassemblyBufferDescIdx = IPV4_IP_REASSEMBLY_BUF_DESC_IDX_INV;
  ...
  reassemblyBufferDescIdx = IpV4_Ip_VReassemblyGetBufferDesc(IpV4CtrlIdx, DataPtr);
  if (reassemblyBufferDescIdx == IPV4_IP_REASSEMBLY_BUF_DESC_IDX_INV)
  {
  ...
  {
    /*@ assert reassemblyBufferDescIdx < TcpIp_GetSizeOfReassemblyBufferDesc(); */                                      /* VCA_TCPIP_REASSEMBLY_DESC_IDX */
    IpV4_Ip_ReassemblyBufferDescType *reassemblyBufDescPtr = TcpIp_GetAddrReassemblyBufferDescDyn(reassemblyBufferDescIdx);
    uint16                            flagsOffset          = TCPIP_GET_UINT16(DataPtr, TCPIP_IPV4_HDR_OFS_FLAGS_OFFSET);
    ...
    if(acceptFragment == TRUE)
    {
      ...
      if((fragIsLast == TRUE) || (((*FragPayloadLenPtr) % IPV4_IP_FRAGMENT_BLOCK_SIZE) == 0u))
      {
        ...
        {
          IpV4_Ip_VReassemblyRxFragment(reassemblyBufDescPtr, fragAbsByteOfs,
                                        *FragPayloadPtrPtr, *FragPayloadLenPtr, fragIsLast);
        }
        else
        {
          IpV4_Ip_VReassemblyRxFragmentInOrder(reassemblyBufDescPtr, fragAbsByteOfs,
                                               *FragPayloadPtrPtr, *FragPayloadLenPtr, fragIsLast,
        ...
        {
          *FragPayloadPtrPtr = IpV4_Ip_VReassemblyGetBufferPtr(reassemblyBufDescPtr);
          *FragPayloadLenPtr = reassemblyBufDescPtr->FragmentLen;
        }
      }
  ...
  TCPIP_P2C(uint8)          ParameterValuePtr)
{
  ...
  if (socketIsIpV4 == TRUE)
  {
    ...
    case TCPIP_PARAMID_V_PHYS_DEST_ADDR:
      IpV4_Ip_SetPhysDestAddr(SocketIdx, ParameterValuePtr);
      retVal = E_OK;
      break;
...
/* VCA_JUSTIFICATION_START
  ...
  \ID VCA_TCPIP_IPV4_LOC_ADDR_IDX
    \DESCRIPTION       A local variable localAddrV4Idx is derived via CSL indirection.
                       (Qualified use-case CSL03 of ComStackLib.)
    \COUNTERMEASURE \N The local variable localAddrV4Idx valid due to the CONCEPT described in
                       TCPIP_CONCEPT_IPV4_SPECIFIC_LOC_ADDR_IDX.

  \ID VCA_TCPIP_REASSEMBLY_DESC_IDX
    \DESCRIPTION       Index reassemblyBufferDescIdx/reassemblyBufferDescIdx/unusedDescIdx is derived via function
                       IpV4_Ip_VReassemblyGetBufferDesc()/IpV4_Ip_VReassemblyFindBufferDesc().
    \COUNTERMEASURE \N The function IpV4_Ip_VReassemblyFindBufferDesc() ensures that either a valid descriptor index or
                       the invalid value IPV4_IP_REASSEMBLY_BUF_DESC_IDX_INV is returned. Within the actual function,
                       the index is checked against IPV4_IP_REASSEMBLY_BUF_DESC_IDX_INV.
                       See CONCEPT TCPIP_CONCEPT_FRAGM_REASSEMBLY_DESC_ARRAY_USAGE.

  \ID VCA_TCPIP_FUNC_CALL_WRITE_REASSEMBLY_BUF_INFO
    \DESCRIPTION       Memory write access to reassembly buffer management structure.
    \COUNTERMEASURE \N The write access is valid as described below:
                       - Macro TCPIP_PUT_UINT16(Buffer, Offset, Value) maps to the function TcpIp_WriteUInt16() that
                         writes an uint16 value to the given 'Buffer' memory location.
                       - The functions Buffer parameter is valid as described in
                         CONCEPT TCPIP_CONCEPT_REASSEMBLY_BUF_PARTITIONING, for these cases:
                         - return value of TcpIp_GetAddrReassemblyBuffer() is directly used as parameter Buffer
                         - a local variable reassemblyBufPtr is initialized using the function
                           TcpIp_GetAddrReassemblyBuffer() with a parameter that comes valid from caller.
                         - a parameter that comes valid from caller (and there has been initialized using
                           TcpIp_GetAddrReassemblyBuffer()) is used
                       - Actually 2 calls to the function write two 16-bit values into the segments "admin area" which
                         are described in CONCEPTs:
                         - TCPIP_CONCEPT_RX_FRAGMENT_ADMIN_DATA (because offsets IPV4_IP_REASSEMBLY_BUF_NEXT_GAP_OFS and
                           IPV4_IP_REASSEMBLY_BUF_GAP_SIZE_OFS are used) and
                         - TCPIP_CONCEPT_RX_FRAGMENT_REASSEMBLY.

  \ID  VCA_TCPIP_NVM_DATA_IDX
    \DESCRIPTION       A local variable nvmDataIdx is derived via CSL indirection.
    \COUNTERMEASURE \N The function IpV4_Ip_NvMBasedAddressHandling() is only called for IPv4 controllers, which
                       configure a valid AddrAssignmentMethodPersist. This ensures, that the indirection is present
                       and in a valid range. (Qualified use-case CSL03 of ComStackLib.)

  \ID VCA_TCPIP_LINKLOCAL_ADDRESS_IDX
    \DESCRIPTION       A local variable llAddrStateIdx is derived via CSL indirection.
    \COUNTERMEASURE \N The function IpV4_Ip_LlAddrInUse() is only called in context of link local address conflict
                       detection. Within function IpV4_Arp_VDetectLinkLocalAddressConflict() it is ensured, that the
                       IPv4 controller uses a link local address. Hence, the sub function IpV4_Ip_LlAddrInUse() can use
                       the optional llAddrStateIdx value without further check.

  \ID VCA_TCPIP_STATIC_UNICAST_ADDR_V4_REQUESTED
    \DESCRIPTION       A local variable staticUnicastAddrV4RequestedIdx is derived via CSL indirection.
                       (Qualified use-case CSL03 of ComStackLib.)
    \COUNTERMEASURE \R The optional indirection is checked to be present by one of the following checks:
                       - addrAssignmentMethod == TCPIP_IPADDR_ASSIGNMENT_STATIC
                       - TcpIp_IsStaticUnicastAddrV4RequestedUsedOfUnicastAddrV4()
                       - TcpIp_IsDefaultUnicastAddrV4UsedOfUnicastAddrV4()
                       Thus, the derived index is within the valid range.

  \ID VCA_TCPIP_TX_REQ_MAP
    \DESCRIPTION       A local variable buf2TxReqMapIdx is derived via CSL indirection and offset ethBufIdx.
    \COUNTERMEASURE \N The local variable buf2TxReqMapIdx is valid due to the CONCEPT described in
                       TCPIP_CONCEPT_BUF_TO_TX_REQ_MAP_FRAGM_PREPARATION.

  \ID VCA_TCPIP_TX_REQ_ELEM_IDX
    \DESCRIPTION       A local variable txReqElemIdx is derived via CSL indirection.
    \COUNTERMEASURE \N The local variable txReqElemIdx is valid due to the CONCEPT described in
                       TCPIP_CONCEPT_BUF_TO_TX_REQ_MAP_FRAGM_PREPARATION.

  \ID VCA_TCPIP_LL_PROVIDE_TX_BUFFER
    \DESCRIPTION       The size of the requested Ethernet buffer has to be consistent with the reported length.
    \COUNTERMEASURE \S The Ethernet buffer is requested via function TCPIP_LL_ProvideTxBuffer(). If the function returns
                       BUFREQ_OK, the combination of ethBufPtr and ethBufLen is consistent.
                       This is ensured by safety manual entry SMI-242896.

  \ID VCA_TCPIP_IP_BUFFER_PTR
    \DESCRIPTION       The size of the requested IP buffer has to be consistent with the reported length.
    \COUNTERMEASURE \N The entire Ethernet buffer is requested and valid as described in VCA_TCPIP_LL_PROVIDE_TX_BUFFER.
                       The calculated IpBuffer as well as the corresponding IpBufferLen are ensured to be valid because
                       it is checked that function IpV4_Ip_VFragmentProvideTxBuffers() returns a valid Ethernet buffer
                       and the calculation of the offset of the IP payload is checked against the available buffer
                       size. The provided buffer comes from extern.

  \ID VCA_TCPIP_ETH_PAYLOAD_PTR
    \DESCRIPTION       The pointer ethPayloadPtr is used to write to memory.
                       The size of the array pointed to by ethPayloadPtr is at least the size of an IPv4 packet's header
                       structure respectively IPV4_IP_DEF_HEADER_LENGTH_BYTE.
    \COUNTERMEASURE \N Validity of the pointer is ensured as stated in TCPIP_CONCEPT_ETH_BUF_CONCAT_LIST_FOR_FRAGM
                       and in SMI-242896.

  \ID VCA_TCPIP_PROT_HDR_PTR
    \DESCRIPTION       Local pointer is defined and passed to the function to write at the memory referenced by it.
    \COUNTERMEASURE \R Variable protocolHdrPtr points to the IPV4_IP_DEF_HEADER_LENGTH_BYTE-th position of the
                       Ethernet buffer.
                       The Ethernet buffer is a memory area that is handled as described in
                       TCPIP_CONCEPT_ETH_BUF_CONCAT_LIST_FOR_FRAGM.
                       The size of each ethernet buffer depends on the size of the IP packet and the fact if the packet
                       has to be fragmented or not.
                       - If the entire IP packet fits into the configured TcpIp_GetMtuTxOfIpV4Ctrl(IpV4CtrlIdx):
                         The resulting size of the ethernet buffer is equal to the requested buffer size.
                         This includes the IPV4_IP_DEF_HEADER_LENGTH_BYTE as well as the header and the payload of the
                         inner protocol (TCP, UDP, ICMPv4).
                       - If the entire IP packet does NOT fit into the configured TcpIp_GetMtuTxOfIpV4Ctrl(IpV4CtrlIdx):
                         Each fragment has the size of the configured TcpIp_GetMtuTxOfIpV4Ctrl(IpV4CtrlIdx).
                       Hence, it is ensured, that the protocol checksum of the inner protocol (TCP, UDP, ICMP) can be
                       written without additional check.
                       The Ethernet buffer is read using CSL-macro:
                       TcpIp_GetEthBufPtrOfBuf2TxReqMap(
                           TcpIp_GetBuf2TxReqMapStartIdxOfIpV4Ctrl(IpV4CtrlIdx) + IpTxReqDescrPtr->BufIdx)
                       This access is valid as described in TCPIP_CONCEPT_ETH_BUF_CONCAT_LIST_FOR_FRAGM because
                       ...
                       See also concept TCPIP_CONCEPT_ETH_TX_BUF_IDX.

  \ID VCA_TCPIP_IPV4_SOCKET_AND_LOCAL_ADDR_IDX
    \DESCRIPTION       Local variables are used without runtime check.
    \COUNTERMEASURE \N Variables ipV4SocketIdx and localAddrV4Idx are local variables, initialized via function
                       parameter IpTxReqDescrPtr->SockIdx and IpTxReqDescrPtr->SrcAddrIdx.
                       Caller makes it sure that IpTxReqDescrPtr->SockIdx contains a valid value which is smaller or
                       equal (<=) TcpIp_GetSizeOfIpV4SocketDyn() and IpTxReqDescrPtr->SrcAddrIdx contains a valid
                       value smaller or equal (<=) TcpIp_GetSizeOfLocalAddrV4().
                       - IpTxReqDescrPtr comes valid from these callers:
                         1. TcpIp_DhcpV4_VSendDhcpMsg() calls this function after initializing its local variable
                            ipTxReqDescrPtr by a call to IpV4_Ip_ProvideTxBuffer() whereat the SockIdx member
                            of the ipTxReqDescrPtr is initialized from the configuration item TcpIp_IpV4General's member
                            IpV4SocketDynDhcpIdx. The existence of a DHCP socket (0:1) is checked in
                            TcpIp_DhcpV4_MainFunction():
                            TcpIp_IsDhcpConfigUsedOfIpV4Ctrl(ipV4CtrlIdx) whereat ipV4CtrlIdx follows
                            CONSTRAINT: [0...TcpIp_GetSizeOfIpV4Ctrl[Dyn]()-1]
                            This ensures that TcpIp_DhcpV4_VSendDhcpMsg() is called only in case DHCP is configured for
                            at least one IPv4 controller address.
                         2. IpV4_Icmp_VSendRawMessage() calls this function after initializing its local variable
                            ipTxReqDescrPtr by a call to IpV4_Ip_ProvideTxBuffer() or IpV4_Ip_FragmentProvideTxBuffer()
                            whereat the SockIdx member of the ipTxReqDescrPtr is initialized from the configuration item
                            TcpIp_IpV4General's member IpV4SocketDynIcmpIdx.
                         3. TcpIp_VIpTransmit() calls this function whereat parameter IpTxReqDescrPtr comes with valid
                            from these callers:
                            - TcpIp_Udp_SendRetryQueueIfPossible() and TcpIp_Udp_VTransmit() whereat the UDP
                              subcomponent uses IpV4_Ip_FragmentProvideTxBuffer() and IpV4_Ip_ProvideTxBuffer()
                              to initialize IpTxReqDescrPtr->SockIdx with a SockIdx from configuration which is looked
                              up in UL calls to TcpIp_UdpTransmit() using the given, DET checked SocketId.

  \ID VCA_TCPIP_DUPLICATE_DHCP_ADDR_EXISTS
    \DESCRIPTION       Index lastDuplicateDhcpAddrIdx is used for memory write access.
    \COUNTERMEASURE \R It is checked by usage of TcpIp_GetE_DUPLICATE_DHCP_ADDROfDemEvent(), that this functionality
                       is configured. If this check evaluates to true, the index obtained from
                       TcpIp_GetLastDuplicateDhcpAddrIdxOfDemEvent() is valid.

  \ID VCA_TCPIP_FUNC_CALL_COPY_INTO_REASSEMBLY_BUF
    \DESCRIPTION       Function call with valid parameters as described below:
                       - TgtDataPtr is based on
                         - local variable reassemblyBufPtr, whose validly is derived using a call to
                           IpV4_Ip_VReassemblyGetBufferPtr() as described in TCPIP_CONCEPT_FUNC_CALL_GET_REASSEMBLY_BUF_PTR.
                         - the access index FragAbsByteOfs that is used to write to reassemblyBufPtr whereat
                           FragAbsByteOfs comes valid from caller that derived them as described in
                           TCPIP_CONCEPT_BUF_DESC_PTR_BASED_CHECK_AND_REASSEMBLY.
                       - SrcDataPtr and
                       - LenByte both come valid from caller as described in TCPIP_CONCEPT_FUNC_CALL_REASSEMBLY_RX_IND.
                       Actually here the payload that belongs into the gap has been received and is now written into
                       the gap to complete the IPv4 packet. As described in concept TCPIP_CONCEPT_FUNC_CALL_ADAPT_GAP
                       this overwrites the admin data for this gap after the admin data have been updated in a way that
                       this gap is not referenced anymore (because it was possible to fill the gap).
    \COUNTERMEASURE \N All parameters of the function call come directly or indirectly from caller.

  \ID VCA_TCPIP_UPDATE_PTR
    \DESCRIPTION       Buffer Pointer is updated inside previous function call with checked return value
    \COUNTERMEASURE \N A Local pointer is created inside a local function. The address is returned by
                       IpV4_Ip_VFragmentProvideTxBuffers, IpV4_Ip_ProvideTxBuffer or IpV6_ProvideTxBuffer.
                       In all cases it is verified that a buffer is provided by the return value.
                       All functions also only provide buffer that are at least as long as requested.
                       Thats why it is always valid to assert a valid bufPtr <-> bufLenByte combination here.
                       The functions IpV6_ProvideTxBuffer and IpV4_Ip_ProvideTxBuffer also provide external buffer.

  VCA_JUSTIFICATION_END */

  /* TCPIP_CONCEPT_START

  \CONCEPT TCPIP_CONCEPT_IPV4_SPECIFIC_LOC_ADDR_IDX
    All over the code of TcpIp_IpV4.c the values of variable localAddrV4Idx follow the following convention:
    ...
    - invalid, it is initialized with the value TCPIP_LOCAL_ADDR_V4_IDX_ANY.
      Note: The value of TCPIP_LOCAL_ADDR_V4_IDX_ANY is currently constantly defined as 0xFF.
            As TCPIP_LOCAL_ADDR_V4_IDX_ANY has a special meaning distinct from other localAddrV4Idx values that range
            from [0..TcpIp_GetSizeOfLocalAddrV4()-1], TcpIp_GetSizeOfLocalAddrV4()
            needs to be <= TCPIP_LOCAL_ADDR_V4_IDX_ANY.
            This check is done by CM_TCPIP_LOC_ADDR_IDX_ANY.
    - valid, it is initialized with a value that complies with the qualified use-case CSL03 of ComStackLib which
      complies to the following: CONSTRAINT [0..TcpIp_GetSizeOfLocalAddrV4()-1]
    Result: All assignments do either use TCPIP_LOCAL_ADDR_V4_IDX_ANY or use the following CSL-macros that
            fulfill the CSL03 use-case:
    - TcpIp_GetLocalAddrV4IdxOfMulticastAddrV4(multicastAddrV4Idx): multicastAddrV4Idx is determined using the
      following CSL-macros due to 0:n relation:
      - TcpIp_GetMulticastAddrV4StartIdxOfIpV4Ctrl(IpV4CtrlIdx)
      - TcpIp_GetMulticastAddrV4EndIdxOfIpV4Ctrl(IpV4CtrlIdx)
      whereat IpV4CtrlIdx is checked by the caller to comply to VCA requires [0..TcpIp_GetSizeOfIpV4Ctrl()-1]
      The indirection index LocalAddrIdx is determined using one of the following CSL-macros due to 1:1 relation:
    - TcpIp_GetLocalAddrV4BroadcastIdxOfIpV4Ctrl(ipV4CtrlIdx)
    - TcpIp_GetLocalAddrV4UnicastIdxOfIpV4Ctrl(ipV4CtrlIdx)
      whereat these ipV4CtrlIdx values are valid as they base on CSL03 lookups.

  \CONCEPT TCPIP_CONCEPT_FRAGM_REASSEMBLY_DESC_ARRAY_USAGE
    All over the code of TcpIp_IpV4.c the runtime admin data for reassembly of received IP packets is stored in
    an array of type IpV4_Ip_ReassemblyBufferDescType.
    The array contains buffer descriptor C-structs. The index to a position in that array is like a handle for a
    fragmentation-buffer that is in use.
    The so called buffer descriptor itself is a structure storing the information about
    - the current usage of the related reassembly buffer and
    - the information about the IPv4 packet who's reassembly is currently in progress.
    Each IPv4 has a FragmentationConfigIdx as member of the array. Which can have the value:
    - TCPIP_NO_FRAGMENTATIONCONFIGIDXOFIPV4CTRL in case this IPv4 controller does not support fragmentation, or value:
    - [FragmentationConfigIdx < TcpIp_GetSizeOfFragmentationConfig()] in case the controller supports fragmentation
      and thus has a fragmentation config and thus may use buffer descriptors for reassembly.
    Each fragmentation config referenced uses its members ReassemblyBufferDescStartIdx and ReassemblyBufferDescEndIdx
    to reference a range of "buffer descriptors" of type IpV4_Ip_ReassemblyBufferDescType which are
    ...
    Each reassembly buffer descriptor can be marked used or unused by its member variable 'Status',
    which has one of these values:
    ...
    From this array of buffer descriptors, each IPv4 controller may use the range it is associated with according
    to the IPv4 controllers configuration.
    The actual buffer is:
    - a part from one linear buffer that is
    - divided into parts using start and end index values in the configuration array that has one C-struct of type
      TcpIp_ReassemblyBufferDescType that is related 1:1 to each
    - buffer descriptor of the IpV4_Ip_ReassemblyBufferDescType, mentioned above.
    Reassembly buffers or reassembly buffer descriptors are arrays of the same size and thus these are CSL02-compliant
    lookups. Any buffer descriptor is identified and accessed by the buffer descriptor index which is determined by
    function IpV4_Ip_VReassemblyFindBufferDesc().
    In case no free buffer descriptor (and thus no free buffer for reassembly) is available, this is indicated by
    the value IPV4_IP_REASSEMBLY_BUF_DESC_IDX_INV in the particular index variable.
    IpV4_Ip_VReassemblyFindBufferDesc() is able to properly find a free buffer descriptor (or not) and follows
    the above convention because:
    - IpV4_Ip_RxIndication() calls IpV4_Ip_ReassembleAndForwardToUpperLayer() in any case
      (IPv4 controller supports fragmentation or not)
    - IpV4_Ip_ReassembleAndForwardToUpperLayer() calls IpV4_Ip_VReassemblyRxIndication() only in case the affected
      IPv4 controller ACTUALLY supports Rx-fragmentation (refer to CONSTRAINT of the given IpV4CtrlIdx).
      This is done by:
      - the following boolean check which is a CSL03-compliant check for the 0:1 relation expressing the presence
        (or absence) of fragmentation support for an IPv4 controller:
      ...
      - a valid reassembly buffer configuration for the given IPv4 controller with at least one reassembly buffer
        [descriptor] configured.
      - a valid fragmentation config in a CSL03-compliant way for a given IpV4CtrlIdx which comes valid from caller
        following CSL01-compliant CONSTRAINT [0..TcpIp_GetSizeOfIpV4Ctrl()-1]
      - the sub-array of all reassembly buffer descriptors belonging to that fragmentation config.
        Using the fragment the part of the array.
      That sub-array is identified by IpV4_Ip_VReassemblyFindBufferDesc() by the CSL03-compliant 1:N indirection using
      these index values of the fragmentation config:
      - TcpIp_GetReassemblyBufferDescStartIdxOfFragmentationConfig()
      - TcpIp_GetReassemblyBufferDescEndIdxOfFragmentationConfig()
      The modelled CSL03-compliant 1:N indirection points to the buffer descriptor which is accessed via
      TcpIp_GetAddrReassemblyBufferDescDyn() and the contained index is returned if
      ...
      - UnusedDescIdxPtr.
      These [out] parameters are used by the function IpV4_Ip_VReassemblyGetBufferDesc() which returns at least one
      reassembly buffer [descriptor] that is available for use or IPV4_IP_REASSEMBLY_BUF_DESC_IDX_INV, otherwise.
      It uses a runtime-check descIdx != IPV4_IP_REASSEMBLY_BUF_DESC_IDX_INV and only if this is TRUE it reads the
      buffer descriptor CSL03-compliant using:
      - TcpIp_GetAddrReassemblyBufferDescDyn( usedReassemblyBufferDescIdx )
      The resulting pointer reassemblyBufDescPtr is valid thus.

  \CONCEPT TCPIP_CONCEPT_BUF_TO_TX_REQ_MAP_FRAGM_PREPARATION
    Array write access using a CSL-indirection based index as basis but adding a local variable as index offset.
    The CSL-macro TcpIp_SetEthBufIdxNextOfBuf2TxReqMap() is used to access a CSL-generated array of runtime-data
    structs in RAM. The Buf2TxReqMap array models a linked list to administer Tx-buffers provided by EthIf.
    Linking is done only in case of fragmentation.
    There is only one Buf2TxReqMap array for all Ip V4 controllers.
    For each Ip V4 controller there is a start- and an end-index to track Buffers provided by EthIf for Tx-requests
    on that controller.
    ...
    - TcpIp_GetBuf2TxReqMapEndIdxOfIpV4Ctrl(IpV4CtrlIdx) (called 'endIdx' in the subsequent paragraph)
    Thus reading start- and end-index values fulfills VCA_TCPIP_CSL03_START_END.
    The buffer index value of the EthIf 'ethBufIdx' is used as offset in the section of the Buf2TxReqMap that is
    assigned to a particular IPv4 controller.
    These local variables are used to administer and store the offsets:
    - ethBufIdx and
    - prevEthBufIdx
    Variable prevEthBufIdx is only initialized with values of ethBufIdx. Variable prevEthBufIdx is only set
    and used after ethBufIdx has a valid value.
    Variable ethBufIdx is used only after the call to TCPIP_LL_ProvideTxBuffer() has returned BUFREQ_OK.
    --> This is checked by runtime check:
        if (BUFREQ_OK == retVal)
    ...
    The linking from the first Tx-buffer reserved at the EthIf can start only in case at least "the first fragment"
    has a provided Tx-buffer.
    This fact is tracked by prevEthBufIdx being != IPV4_IP_TX_BUF_IDX_INV or not.
    --> This is checked by runtime check:
        if (prevEthBufIdx == IPV4_IP_TX_BUF_IDX_INV)
    In case prevEthBufIdx == IPV4_IP_TX_BUF_IDX_INV prevEthBufIdx is not used to add it to startIdx and thus,
    no invalid array access happens.
    When prevEthBufIdx != IPV4_IP_TX_BUF_IDX_INV this means that a valid ethBufIdx has been assigned to prevEthBufIdx,
    because there is only one assignment to prevEthBufIdx.

  \CONCEPT TCPIP_CONCEPT_REASSEMBLY_BUF_PARTITIONING
    Every buffer descriptor of an IPv4 controller has assigned its own reassembly buffer.
    This reassembly buffer is a segment of an one-piece, flat buffer, allocated for all reassembly activities.
    The associated buffer segment is defined by a start- and an end-index which both are stored
    in the array of C-structs named:
    - TcpIp_ReassemblyBufferDesc[] in the configuration. This array has the same size as the array of buffer
      descriptors, which, due to the same size, yields a CSL02-compliant lookup.
    The start of the buffer segment to be used by a particular buffer descriptor is retrieved by an array access
    into the flat, overall buffer using the index:
    - startReassemblyBufferIdx which is read from the config using macro:
    - TcpIp_GetReassemblyBufferStartIdxOfReassemblyBufferDesc(usedReassemblyBufferDescIdx)
    - Index reassemblyBufferDescIdx is returned by function IpV4_Ip_VReassemblyFindBufferDesc() and is valid as soon as
      it is != IPV4_IP_REASSEMBLY_BUF_DESC_IDX_INV according to the CONCEPT described in
      TCPIP_CONCEPT_FRAGM_REASSEMBLY_DESC_ARRAY_USAGE.

  \CONCEPT TCPIP_CONCEPT_RX_FRAGMENT_ADMIN_DATA
    The assembly of IPv4 packet fragments that come in out of order is done in a flat, linear buffer.
    In the out of order case, gaps remain in between the fragments. These gaps are used to store admin data regarding
    ...
    As there is no gap in front of the first fragment (because the fragment starts at buffer position '0') the buffer
    descriptor as described in CONCEPTs
    - TCPIP_CONCEPT_RX_FRAGMENT_REASSEMBLY and
    - TCPIP_CONCEPT_REASSEMBLY_BUF_PARTITIONING
    have gotten a member NextGapOfs that points to the first admin data couple of 16-bit values.
    To write the 2 admin data values the following offset constants are used which are to be added
    to the start-position of the gap-area in the buffer-segment of a buffer descriptor:
    - IPV4_IP_REASSEMBLY_BUF_NEXT_GAP_OFS (0)
      is valid to write the 16-bit offset of the next gap into the buffers gap-area or using
    - IPV4_IP_REASSEMBLY_BUF_GAP_SIZE_OFS (2)
      is valid to write the size of the current gap into the buffers gap-area.
    ...
    The minimum value in the description file of the configuration value related to MaxPacketSize of all
    FragmentationConfig is 4.
    Thus the MaxPacketSize values of all FragmentationConfigs have the value of 4 at least.

  \CONCEPT TCPIP_CONCEPT_FUNC_CALL_ADAPT_GAP
    Function call with valid parameters as described below:
    - ReassemblyBufDescPtr is a pointer to a C-struct of type IpV4_Ip_ReassemblyBufferDescType which comes valid
      (as described in CONCEPT TCPIP_CONCEPT_RX_FRAGMENT_REASSEMBLY)
      and non-NULL from caller of the function calling IpV4_Ip_VReassemblyHandleGaps().
    - FragOfs comes valid from caller.
    - FragEndOfs is calculated in local variable fragEndOfs as sum of the function parameters FragOfs and FragLen
      which both come valid from caller.
    - ThisGapOfs is given by local variable gapOfs which is initially initialized from the member NextGapOfs of the
      ReassemblyBufDescPtr which comes valid from caller (including its members).
      NextGapOfs contains the offset of the next gap.
    - ThisGapEndOfs is given by local variable gapEndOfs which is calculated as gapOfs + gapSize from 2 local variables,
      whereat:
      - gapOfs is valid as stated in the above description of parameter ThisGapOfs.
      - gapSize is a local variable that is initialized by reading the admin data of the current gap using the buffer
        segment pointed to by local variable reassemblyBufPtr:
        - the admin data is read by reading an uint16 at array offset
          <gap [start] offset> + IPV4_IP_REASSEMBLY_BUF_GAP_SIZE_OFS in the given reassemblyBufPtr,
          as described by CONCEPT TCPIP_CONCEPT_RX_FRAGMENT_ADMIN_DATA.
        - reassemblyBufPtr is initialized by the return value of function call IpV4_Ip_VReassemblyGetBufferPtr()
          with argument ReassemblyBufDescPtr which comes valid from caller.
    - PrevGapOfs is given by local variable prevGapOfs which is initialized:
      - in the first run of the while-loop with IPV4_IP_REASSEMBLY_BUF_NO_NEXT_GAP_OFS which
        stands for "no previous gap" and
      - in all further runs of the while-loop with gapOfs which is valid as described for parameter ThisGapOfs, above.
    - NextGapOfs is given by local variable nextGapOfs which is read from the admin data of the current gap
      using the buffer segment pointed to by local variable reassemblyBufPtr:
      - the admin data is read by reading an uint16 at array offset
        <gap [start] offset> + IPV4_IP_REASSEMBLY_BUF_NEXT_GAP_OFS in the given reassemblyBufPtr,
        as described by CONCEPT TCPIP_CONCEPT_RX_FRAGMENT_ADMIN_DATA.
      - reassemblyBufPtr is valid as described for parameter ThisGapEndOfs, above.
    All parameters of the function call come directly or indirectly from caller.

  \CONCEPT TCPIP_CONCEPT_RX_FRAGMENT_REASSEMBLY
    Each IPv4 packet fragment has a fragment offset in the overall IPv4 packet.
    It is stored in a flat buffer as described in CONCEPT TCPIP_CONCEPT_FRAGM_REASSEMBLY_DESC_ARRAY_USAGE.
    Gaps occur in case packet fragments arrive out of order and are stored at their particular offset in a flat buffer.
    The administration of these gaps is described in CONCEPT TCPIP_CONCEPT_RX_FRAGMENT_ADMIN_DATA.
    As soon as the first IPv4 packet fragment has been received, 2 things are done:
    1. in function IpV4_Ip_VReassemblyGetBufferDesc() a free buffer descriptor and a suitable buffer belonging
       to the descriptor is found using IpV4_Ip_VReassemblyFindBufferDesc().
       After that in the first function:
       - the offset of the first gap is set to IPV4_IP_REASSEMBLY_BUF_NO_NEXT_GAP_OFS (65535, invalid value, because
         at the max. size of an IPv4 packet no gap can occur anymore) when variable firstGapOfs is written to the admin
         area of the assigned buffer segment.
       - the size of the first gap is set to the size of the reassembly buffer of the descriptor (e.g. 10000) when
         variable firstGapSize is written to the admin area of the assigned buffer segment.
      Writing these 2 admin values is done as described in CONCEPT TCPIP_CONCEPT_RX_FRAGMENT_ADMIN_DATA and thus valid.
    2. in function IpV4_Ip_VReassemblyHandleGaps(), when handling the first fragment received
       - the size of the reassembly buffer that is used for this reception read out again (as initially written by
         IpV4_Ip_VReassemblyGetBufferDesc()) and is used as initial value for the upper limit of all gaps
         that can be handled,
       - so the reassembly buffers size can never be exceeded.
    EXAMPLE: In case a 10000 byte reassembly buffer has been configured and the transfer consists of 2 fragmented IP
    packets then after the first (e.g.) 16 byte packet the gap-size is calculated as
    <gap size> = <buffer size> - <fragment size>
    i.e.
    9984       = 10000         - 16
    The buffer size of 10000 is re-calculated (respectively "persists"), each time a new fragment of this reception
    was received because these items are stored:
    - <next gap offset> (e.g. 16 as soon as the first 16-byte fragment has been received)
    - <gap size> (e.g. 9984)
    As the <next gap offset> of the very last gap is also the <end offset of uppermost fragment received> the IP
    payload length of any newly received fragment can be used to calculate the buffer space that would be consumed
    to store the fragment, like this:
    ...
    In case the possibly consumed length is larger than the buffer size the search for a matching gap in the
    reassembly buffer is ended and thus the fragmentation process is ended by letting IpV4_Ip_VReassemblyHandleGaps()
    indicate:
    ...
    to the caller IpV4_Ip_VReassemblyRxFragment() which will then end the re-assembly process for this reception
    with failure.
    The described concept ensures that fragments that are larger than the buffer segment assigned to a buffer
    descriptor are detected and not stored in the buffer segment.

  \CONCEPT TCPIP_CONCEPT_ETH_BUF_CONCAT_LIST_FOR_FRAGM
    All over the code of TcpIp_IpV4.c the runtime admin data of particular IP packets to be transmitted
    (i.e. EthIf-buffers) consist of a set of 5 values: BufPtr, BufLen, BufIdxNext, SocketDynIdx and UlTxReqIdx.
    ...
    (The last 2 items are written after valid initialization of at least one element of the (possibly concatenated)
     IP packet administration data as described in TCPIP_CONCEPT_BUF_TO_TX_REQ_MAP_FRAGM_PREPARATION.)
    ...
    - follows the constraints described in the below concept:
      ...
      - When the lower layer answers the request for an EthIf-buffer positively by returning BUFREQ_OK,
        it is assumed that:
        ...
        - The memory the returned buffer pointer points to has at least the buffer size requested, before, i.e.:
          ethBufLen = fragPayloadLen + IPV4_IP_DEF_HEADER_LENGTH_BYTE
    Thus it is assumed that, when one item of the described 5-item-data-set is read out, it will fit to the other
    2-items of the set. All 5-item-data-sets are initialized in function IpV4_Ip_VFragmentProvideTxBuffers()
    where the validity of the conditions listed above is prepared like this:
    1. ethBufLen being IPV4_IP_DEF_HEADER_LENGTH_BYTE larger than the payload to transfer:
       ethBufLen = (fragPayloadLen + IPV4_IP_DEF_HEADER_LENGTH_BYTE);
    2. EthBufIdxNext + TcpIp_GetBuf2TxReqMapStartIdxOfIpV4Ctrl(IpV4CtrlIdx) will never exceed
       TcpIp_GetBuf2TxReqMapEndIdxOfIpV4Ctrl(IpV4CtrlIdx) as described in TCPIP_CONCEPT_BUF_TO_TX_REQ_MAP_FRAGM_PREPARATION.
    3. When looking at a valid element in the Buf2TxReqMap-RAM-array at index ethBufIdx then the EthBufIdxNext-member
       of that element is either
       - IPV4_IP_TX_BUF_IDX_INV for a non fragmented transmission or
       - a valid ethBufIdx for a fragmented transmission.
    This is ensured in function IpV4_Ip_VFragmentProvideTxBuffers() as described in
    TCPIP_CONCEPT_BUF_TO_TX_REQ_MAP_FRAGM_PREPARATION.
    IpV4_Ip_VFragmentProvideTxBuffers() which builds up the concatenated list of Tx-buffers requested from the EthIf
                                       has to be called prior to
    IpV4_Ip_VTransmitFragments()        which actually uses the concatenated list of requested Tx-buffers.
    IpV4_Ip_VTransmitFragments()        ensures this by rejecting calls whose parameters to not point to a valid entry
                                        point in the concatenated list of admin data.
                                        This is done by check of the while-loop: (ethBufIdx != IPV4_IP_TX_BUF_IDX_INV).

  \CONCEPT TCPIP_CONCEPT_ETH_TX_BUF_IDX
    All over the code of TcpIp_IpV4.c the values of variable ethBufIdx follow the following convention:
    If any IPv4-specific ethBufIdx variable or parameter is:
    - invalid, it is initialized with the value IPV4_IP_TX_BUF_IDX_INV.
    - valid, it is initialized with a value that complies with the qualified use-case CSL03 of ComStackLib which
      complies to the following: CONSTRAINT [0..TcpIp_GetSizeOfBuf2TxReqMap()-1]
  ...
  \CONCEPT TCPIP_CONCEPT_FUNC_CALL_GET_REASSEMBLY_BUF_PTR
    Function call with valid parameters as described below:
    - IpV4_Ip_VReassemblyGetBufferPtr() is called to retrieve the buffer pointer of the buffer segment that is
      associated to the given buffer descriptor pointer.
    - ReassemblyBufDescPtr is:
      - Either given by local variable reassemblyBufDescPtr which is initialized:
        ...
        - reassemblyBufferDescIdx (in one function also 'reassemblyBufferDescIdx') which is initialized:
      ...
      - Or given by a parameter ReassemblyBufDescPtr from the caller which is initialized by the caller in the same
        way as described, above.
        ReassemblyBufDescPtr might be passed far down along the callstack as there are many internal functions
        that handle the fragmentation/reassembly.
      As described in CONCEPT TCPIP_CONCEPT_FRAGM_REASSEMBLY_DESC_ARRAY_USAGE:
      - each IPv4 controller optionally has assigned an reassembly buffer.
      - IpV4_Ip_VReassemblyRxIndication() is only called by caller IpV4_Ip_ReassembleAndForwardToUpperLayer()
        in case the affected IPv4 controller has assigned a buffer descriptor for reassembly.
      ...
      2. the index a buffer descriptor that was already assigned to an IPv4 reception fitting to the contents
         of the given DataPtr.
      3. no index of a free buffer descriptor at all because all configured buffer descriptors of the fragmentation
         config of the given IPv4 controller are in use, already:
      ...
      In the first 2 cases reassemblyBufferDescIdx is valid and can be used to lookup the call-parameter
      ReassemblyBufDescPtr using CSL-macro TcpIp_GetAddrReassemblyBufferDescDyn() which
      ...
      Only modelled indirections as described in CONCEPT TCPIP_CONCEPT_FRAGM_REASSEMBLY_DESC_ARRAY_USAGE are used
      to determine valid parameter 'reassemblyBufDescPtr' for the call to IpV4_Ip_VReassemblyGetBufferPtr()

  \CONCEPT TCPIP_CONCEPT_BUF_DESC_PTR_BASED_CHECK_AND_REASSEMBLY
    Function call with valid parameters as described below:
    Functions:
    ...
    - ReassemblyBufDescPtr is given by local variable reassemblyBufDescPtr which is looked up from reassemblyBufferDescIdx
      as described in TCPIP_CONCEPT_FUNC_CALL_GET_REASSEMBLY_BUF_PTR and thus valid.
    - FragIsLast, FragAbsByteOfs are given by local variables fragIsLast and fragAbsByteOfs which are all
      - initialized from local variable flagsOffset that is read as uint16 word at offset
        TCPIP_IPV4_HDR_OFS_FLAGS_OFFSET in the IPv4 packet that comes valid from caller via DataPtr whereat:
      ...
      - calculated by masking the IPv4 protocol specific status bits/values from local variable flagsOffset
        by using the masks:
    ...
    - FragBlockOfs is given by local variable fragBlockOfs which is initialized from the local variable flagsOffset
      in the same way as fragIsLast, described in the section immediately above.
    ...
    - FragmentationConfigIdx is given by local variable fragmentationConfigIdx which is:
      - initialized as described in CONCEPT TCPIP_CONCEPT_FRAGM_REASSEMBLY_DESC_ARRAY_USAGE,
      - using the IpV4CtrlIdx that comes valid from caller.

  \CONCEPT TCPIP_CONCEPT_VALID_PTR_CALLER
    A pointer that was checked to be valid by the caller of this function is dereferenced and written or
    passed to a function in a function-call in order to be dereferenced and written. The caller ensures the
    validity of the pointer-parameter(s).

  \CONCEPT TCPIP_CONCEPT_FUNC_CALL_REASSEMBLY_RX_IND
    Function-call with valid parameters as described below:
    - IpV4CtrlIdx: Valid as described in VCA_TCPIP_CSL01.
    - DataPtr: Valid as described in TCPIP_CONCEPT_VALID_PTR_CALLER.
    - &ipPayloadPtr: This local pointer variable is initialized by an array access to the array DataPtr[]
                     ...
                     - according to TCPIP_CONCEPT_VALID_PTR_CALLER and due
                     - the runtime check:
                       if (ipPayloadLenByte > 0)
                       ensures that at least one byte of payload is present beyond the IPv4 header structure
                       at the beginning of the IPv4 packet received.
      The array access to initialize ipPayloadPtr is valid because the array-position is local variable ipHdrLenByte
      which is valid due to CM_TCPIP_IP_HDR_LEN_BYTE .
...
/* \CM CM_TCPIP_IP_HDR_PTR_RX_IND           The given pointer variable comes valid from caller as described in TCPIP_CONCEPT_VALID_PTR_CALLER.
...
/* \CM CM_TCPIP_IPV4_SPECIFIC_LOC_ADDR_IDX  CONCEPT: All over the code of TcpIp_IpV4.c the values of variable localAddrV4Idx follow the following convention:
                                              ...
                                              - TcpIp_GetLocalAddrV4BroadcastIdxOfIpV4Ctrl(ipV4CtrlIdx)
                                              - TcpIp_GetLocalAddrV4UnicastIdxOfIpV4Ctrl(ipV4CtrlIdx)
                                                whereat these ipV4CtrlIdx values are valid as ensured by VCA assertions or requires statements.
...
/* \CM CM_TCPIP_IPV4_CTRL_IDX_CHECK         The local variable ipV4CtrlIdx is initialized using the return value of one of these TcpIp-internal functions:
                                              ...
                                              Only then ipV4CtrlIdx is used as parameter for a succeeding function call. */
