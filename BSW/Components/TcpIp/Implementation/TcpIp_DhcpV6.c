 ...
 *                                      \spec requires DhcpV6DataIdx < TcpIp_GetSizeOfDhcpV6Data(); \endspec
 ...
 *                                 \spec requires DhcpV6DataIdx < TcpIp_GetSizeOfDhcpV6Config(); \endspec
 *  \pre          -
 *  \context      TASK
 *  \reentrant    TRUE
 *********************************************************************************************************************/
...
STATIC FUNC(void, TCPIP_CODE) TcpIp_DhcpV6_VAddRawOpt(
  ...
  uint16                                                 OptDataLen)
{
  TcpIp_SizeOfDhcpV6TxOption39InfoType dhcpV6TxOption39InfoIdx = TcpIp_GetDhcpV6TxOption39InfoIdxOfIpV6Ctrl(IpCtrlIdx);
...
#  endif
  ...
  if (OptType == TCPIP_DHCPV6_OPT_ID_CLIENT_FQDN)
  {
    ...
    */
    ...
    MsgPtr->MsgBufPtr[MsgPtr->MsgBufPos] = (uint8)(TcpIp_GetOptLenOfDhcpV6TxOption39Info(dhcpV6TxOption39InfoIdx) & 0xFFu);
    MsgPtr->MsgBufPos++;
  }
...
#   endif
  ...
  if (0u < TcpIp_GetOptLenOfDhcpV6TxOption39Info(dhcpV6TxOption39InfoIdx))
  {
    TcpIp_SizeOfDhcpV6TxOption39Type dhcpV6TxOption39Idx;
    IPV6_BEGIN_CRITICAL_SECTION();

    dhcpV6TxOption39Idx = TcpIp_GetDhcpV6TxOption39StartIdxOfIpV6Ctrl(IpCtrlIdx);
    TcpIp_DhcpV6_VAddRawOpt(IpCtrlIdx, MsgPtr,
      TCPIP_DHCPV6_OPT_ID_CLIENT_FQDN, TcpIp_GetAddrDhcpV6TxOption39(dhcpV6TxOption39Idx),
      TcpIp_GetOptLenOfDhcpV6TxOption39Info(dhcpV6TxOption39InfoIdx));
  ...
  for (dhcpUserOptionIdx = TcpIp_GetDhcpUserOptionStartIdxOfIpV6Ctrl(IpCtrlIdx); dhcpUserOptionIdx < TcpIp_GetDhcpUserOptionEndIdxOfIpV6Ctrl(IpCtrlIdx); dhcpUserOptionIdx++)
  {
    ...
    {
      TcpIp_DhcpUserOptionBufferIterType dhcpUserOptionBufferIdx;
      IPV6_BEGIN_CRITICAL_SECTION();

      dhcpUserOptionBufferIdx = TcpIp_GetDhcpUserOptionBufferStartIdxOfDhcpUserOption(dhcpUserOptionIdx);

      TcpIp_DhcpV6_VAddRawOpt(IpCtrlIdx, MsgPtr,
        TcpIp_GetCodeOfDhcpUserOption(dhcpUserOptionIdx), TcpIp_GetAddrDhcpUserOptionBuffer(dhcpUserOptionBufferIdx),
...
#  endif
  ...
  if (BUFREQ_OK == IpV6_ProvideTxBuffer(&ipAddrIdx, TcpIp_GetIpV6SocketDynDhcpIdxOfIpV6General(TCPIP_IPV6GENERAL_IDX),
                                        &TcpIp_DhcpV6_Addr_All_DHCP_Relay_Agents_and_Servers,
                                        &bufPtr, &bufLenByte, &ipTxReqDesc, TCPIP_PROTOCOL_UDP, FALSE))
  {
    ...
    ipTxReqDesc.TmpChecksum = 0u;
    if (ipTxReqDesc.ChecksumHwCalced == TCPIP_IP_PROTOCOL_CHECKSUM_CHECK_COPY)
    {
      ipTxReqDesc.ChecksumHwCalced = TCPIP_IP_PROTOCOL_CHECKSUM_CHECK_COMPLETE; /* For DHCPv6 the checksum will be calculated complete. */
    }
...
)
{
      ...
      && (TcpIp_DhcpV6_VIsTxUserOption(dhcpUserOptionIdx) == FALSE))
  {
    ...
    if (OptLen <= TcpIp_GetDhcpUserOptionBufferLengthOfDhcpUserOption(dhcpUserOptionIdx))
    {
      ...
      /* PRQA S 0315 1 */ /* MD_MSR_VStdLibCopy */
      IpBase_Copy(TcpIp_GetAddrDhcpUserOptionBuffer(dhcpUserOptionBufferIdx), OptPtr, OptLen);
    ...
    else
    {
      ...
      /* PRQA S 0315 1 */ /* MD_MSR_VStdLibCopy */
      IpBase_Copy(TcpIp_GetAddrDhcpUserOptionBuffer(dhcpUserOptionBufferIdx), OptPtr, TcpIp_GetDhcpUserOptionBufferLengthOfDhcpUserOption(dhcpUserOptionIdx));
...
STATIC FUNC(void, TCPIP_CODE) TcpIp_DhcpV6_VRestartAddressAssignment(TcpIp_IpV6CtrlIterType IpCtrlIdx, uint32 DelayMs)
{
  ...
  if (TRUE == TcpIp_GetAddrIaNaLeaseOfDhcpV6Data(dhcpV6DataIdx)->Valid)
  {
    ...
    TcpIp_IpV6SourceAddressIterType ipV6SourceAddressIdx = TcpIp_GetIpV6SourceAddressDhcpIdxOfIpV6Ctrl(IpCtrlIdx);

    (void)IpV6_SetAddress(TcpIp_GetLocalAddrV6IdxOfIpV6SourceAddress(ipV6SourceAddressIdx),
      NULL_PTR, 0, 0, FALSE);
...
FUNC(Std_ReturnType, TCPIP_CODE) TcpIp_DhcpV6_StartAddressAssignmentForAddr(TcpIp_SizeOfLocalAddrV6Type LocalAddrV6Idx)
{
  ...
  {
    TcpIp_IpV6SourceAddressIterType ipV6SourceAddressIdx = TcpIp_GetIpV6SourceAddressIdxOfLocalAddrV6(LocalAddrV6Idx);
    /* #10 Trigger address assignment only if the Ip address index is Dhcp configurable */
    if (TcpIp_GetAddressAssignVariantOfIpV6SourceAddress(ipV6SourceAddressIdx) == TCPIP_IPADDR_ASSIGNMENT_DHCP)
    {
      TCPIP_ASSERT(TcpIp_IsDhcpV6ConfigUsedOfIpV6Ctrl(ipCtrlIdx) == TRUE); /* Assert that DhcpV6 is configured for the IpCtrlIdx */
...
{
  TcpIp_SizeOfDhcpV6DataType dhcpV6DataIdx = TcpIp_GetDhcpV6DataIdxOfIpV6Ctrl(IpCtrlIdx);
  TcpIp_SizeOfDhcpV6MsgTxBufferType dhcpV6MsgTxBufferIdx = TcpIp_GetDhcpV6MsgTxBufferStartIdxOfIpV6Ctrl(IpCtrlIdx);
  ...
  if (TcpIp_IsPrepareRestartOfDhcpV6Data(dhcpV6DataIdx) == TRUE)
  {
    TcpIp_SetPrepareRestartOfDhcpV6Data(dhcpV6DataIdx, FALSE);
    TcpIp_DhcpV6_VChangeState(dhcpV6DataIdx, TCPIP_DHCPV6_STATE_TX_SOL);
  }
  else
  {
  ...
  TcpIp_GetAddrMsgOfDhcpV6Data(dhcpV6DataIdx)->MsgBufPtr = TcpIp_GetAddrDhcpV6MsgTxBuffer(dhcpV6MsgTxBufferIdx);
...
#  if (TCPIP_SUPPORT_DHCPV6_OPTIONS == STD_ON)
  {
    TcpIp_DhcpUserOptionIterType dhcpUserOptionIdx;
    TcpIp_SizeOfDhcpV6TxOption39InfoType dhcpV6TxOption39InfoIdx = TcpIp_GetDhcpV6TxOption39InfoIdxOfIpV6Ctrl(IpCtrlIdx);
    TcpIp_SetOptLenOfDhcpV6TxOption39Info(dhcpV6TxOption39InfoIdx, 0);
 ...
 *
 */
FUNC(void, TCPIP_CODE) TcpIp_DhcpV6_Stop(TcpIp_IpV6CtrlIterType IpCtrlIdx)
{
  /* ----- Local Variables ---------------------------------------------- */
  TcpIp_SizeOfDhcpV6DataType dhcpV6DataIdx = TcpIp_GetDhcpV6DataIdxOfIpV6Ctrl(IpCtrlIdx);
  ...
  /* ----- Implementation ----------------------------------------------- */
  /* #10 Reset all variables */
  TcpIp_DhcpV6_Reset(IpCtrlIdx);
  /* #20 Set state to TCPIP_DHCPV6_STATE_UNBOUND */
  TcpIp_DhcpV6_VChangeState(dhcpV6DataIdx, TCPIP_DHCPV6_STATE_UNBOUND);

} /* End of TcpIp_DhcpV6_Stop() */
    ...
    P2VAR(uint8, AUTOMATIC, TCPIP_APPL_DATA)  OptPtr)
{
  ...
  if (TcpIp_GetIpV6SourceAddressDhcpIdxOfIpV6Ctrl(ipCtrlIdx) == srcAddrIdx)
  {
    ...
    if ((OptId == TCPIP_DHCPV6_OPT_ID_CLIENT_FQDN) && (TcpIp_IsDhcpV6TxOption39InfoUsedOfIpV6Ctrl(ipCtrlIdx) == TRUE))  /* PRQA S 3415 */ /* MD_TCPIP_Rule13.5_3415_CslReadOnly */
    {
      ...
      if (*OptLenPtr >= TcpIp_GetOptLenOfDhcpV6TxOption39Info(dhcpV6TxOption39InfoIdx))
      {
        /* #30 If there is valid data, copy length and host name */
        *OptLenPtr = TcpIp_GetOptLenOfDhcpV6TxOption39Info(dhcpV6TxOption39InfoIdx);
        if (*OptLenPtr > 0u)
        {
          TcpIp_SizeOfDhcpV6TxOption39Type dhcpV6TxOption39Idx = TcpIp_GetDhcpV6TxOption39StartIdxOfIpV6Ctrl(ipCtrlIdx);
          /* PRQA S 0315 1 */ /* MD_MSR_VStdLibCopy */
          IpBase_Copy(OptPtr, TcpIp_GetAddrDhcpV6TxOption39(dhcpV6TxOption39Idx), *OptLenPtr);
        }
          ...
          && (*OptLenPtr >= TcpIp_GetLengthOfDhcpUserOptionDyn(dhcpUserOptionIdx)))
      {
        *OptLenPtr = TcpIp_GetLengthOfDhcpUserOptionDyn(dhcpUserOptionIdx);
        ...
        if (*OptLenPtr > 0u)
        {
          ...
          /* PRQA S 0315 1 */ /* MD_MSR_VStdLibCopy */
          IpBase_Copy(OptPtr, TcpIp_GetAddrDhcpUserOptionBuffer(dhcpUserOptionBufferIdx), *OptLenPtr);
        }
    ...
    uint16                                     MsgTypes)
{
  ...
  Std_ReturnType             retValue = E_NOT_OK;
  uint8                      errorId  = TCPIP_DHCPV6_E_NO_ERROR;
  TcpIp_SizeOfDhcpV6TxOption39InfoType dhcpV6TxOption39InfoIdx;
  TcpIp_IpV6SourceAddressIterType srcAddrIdx;

  /* ----- Implementation ----------------------------------------------- */
  srcAddrIdx = TcpIp_GetIpV6SourceAddressIdxOfLocalAddrV6(LocalAddrV6Idx);
  ipCtrlIdx = TcpIp_GetIpV6CtrlIdxOfLocalAddrV6(LocalAddrV6Idx);
  dhcpV6TxOption39InfoIdx = TcpIp_GetDhcpV6TxOption39InfoIdxOfIpV6Ctrl(ipCtrlIdx);
  ...
  if (TcpIp_GetIpV6SourceAddressDhcpIdxOfIpV6Ctrl(ipCtrlIdx) == srcAddrIdx)
  {
    ...
    if ((OptId == TCPIP_DHCPV6_OPT_ID_CLIENT_FQDN) && (TcpIp_IsDhcpV6TxOption39InfoUsedOfIpV6Ctrl(ipCtrlIdx) == TRUE))  /* PRQA S 3415 */ /* MD_TCPIP_Rule13.5_3415_CslReadOnly */
    {
      ...
      if (OptLen == 0u)
      {
        ...
        /* PRQA S 0315 1 */ /* MD_MSR_VStdLibCopy */
        IpBase_Fill(TcpIp_GetAddrDhcpV6TxOption39(dhcpV6TxOption39Idx), 0, TcpIp_GetOptLenOfDhcpV6TxOption39Info(dhcpV6TxOption39InfoIdx));
        TcpIp_SetOptLenOfDhcpV6TxOption39Info(dhcpV6TxOption39InfoIdx, 0);
        TcpIp_SetMsgFlagsOfDhcpV6TxOption39Info(dhcpV6TxOption39InfoIdx, 0);
      }
      else if (OptLen <= txOption39_OptLen)
      {
        /* #30 If the data provided by the user fits into the option buffer, Set tx option parameters */
        TcpIp_SetOptLenOfDhcpV6TxOption39Info(dhcpV6TxOption39InfoIdx, OptLen);
        TcpIp_SetMsgFlagsOfDhcpV6TxOption39Info(dhcpV6TxOption39InfoIdx, MsgTypes);
        /* PRQA S 0315 1 */ /* MD_MSR_VStdLibCopy */
        IpBase_Copy(TcpIp_GetAddrDhcpV6TxOption39(dhcpV6TxOption39Idx), OptPtr, OptLen);
      }
      else
      {
  ...
  TcpIp_SizeOfDhcpV6DataType DhcpV6DataIdx)
{
  ...
  {
    TcpIp_SizeOfDhcpV6DataType dhcpV6ConfigIdx = TcpIp_GetDhcpV6ConfigIdxOfIpV6Ctrl(IpCtrlIdx);

    /* #20 Restart dhcpv6 address assignment if maximum retries reached */
    if (TcpIp_IsRestartOnFailureOfDhcpV6Config(dhcpV6ConfigIdx) == FALSE)
    {
      TcpIp_DhcpV6_StartAddressAssignment(IpCtrlIdx);
    }
  ...
  for (ipCtrlIdx = 0; ipCtrlIdx < TcpIp_GetSizeOfIpV6Ctrl(); ipCtrlIdx++)
  {
    ...
    if (IPV6_CTRL_STATE_LLADDR_DAD_FAIL != TcpIp_GetCtrlStateOfIpV6CtrlDyn(ipCtrlIdx))
    {
      ...
      if ((TcpIp_IsDhcpV6ConfigUsedOfIpV6Ctrl(ipCtrlIdx) == TRUE) && TCPIP_DHCPV6_IS_INITIALIZED(dhcpV6DataIdx))
      {
        ...
        /* The state Fail does not need to wait for a timeout, it can be handled directly. */
        if (TcpIp_GetStateOfDhcpV6Data(dhcpV6DataIdx) == TCPIP_DHCPV6_STATE_FAIL)
          /* #120 [Fail] Restart DHCP process if RestartOnFailure is active */
        {
          if (TcpIp_IsRestartOnFailureOfDhcpV6Config(dhcpV6ConfigIdx) == TRUE)
          {
            TcpIp_SetPrepareRestartOfDhcpV6Data(dhcpV6DataIdx, TRUE);

            TcpIp_DhcpV6_VRestartAddressAssignment(ipCtrlIdx, 0);
          }
          /* Else: stay in state Fail */
        }
      }
    }
  }
  ...
  TCPIP_P2C(IpBase_AddrIn6Type) DestinationPtr)
{
  ...
  else if (DataLen < (sizeof(TcpIp_Udp_HdrType) + TCPIP_DHCPV6_MIN_HDR_LEN))
  {
    result = E_NOT_OK;
  }
