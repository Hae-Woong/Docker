...
#  define IPV6_ADDRESS_LEN_BIT                      (IPV6_ADDRESS_LEN_BYTE * TCPIP_BITS_IN_BYTE) /*!< length of an IPv6 address in bits (128) */
...
typedef struct
{
  ...
  uint32                                 TmpChecksum;       /*!< the current/partial checksum */
  TcpIp_IpProtocolChecksumType           ChecksumCalcType;  /*!< indicate when the checksum shall be calculated. */
...
#  endif
  ...
  boolean                    Triggered;        /*!< address is triggered */
