...
STATIC FUNC(Std_ReturnType, TCPIP_CODE) IpV6_Icmp_VMultipartTimeExdMessage(
  ...
  IPV6_P2V(TcpIp_Uint8P2V)                    OrigPktDataPtr,
 ...
 *                             \spec requires IpAddrIdx < TcpIp_GetSizeOfLocalAddr(); \endspec or TCPIP_LOCAL_ADDR_V6_IDX_ANY
  ...
  uint16                      LenByte)
{
  ...
  {
    TcpIp_SizeOfIcmpV6EchoDataBufferType icmpV6EchoDataBufferIdx = 0u;
    ...
    icmpV6EchoDataBufferIdx = (uint16) sizeof(IpV6_IcmpEchoReplyType);
    /* PRQA S 0315 1 */ /* MD_MSR_VStdLibCopy */
    IpV6_MemCpy(TcpIp_GetAddrIcmpV6EchoDataBuffer(icmpV6EchoDataBufferIdx),
  ...
  uint32                      MtuNbo)
{
  ...
  if (E_OK == IpV6_Ndp_VDCLookup(IpCtrlIdx, DstAddrPtr, &ipV6DestinationCacheEntryIdx))
  {
    /* destination cache entry found */
    TcpIp_GetAddrIpV6DestinationCacheEntry(ipV6DestinationCacheEntryIdx)->MtuReported = (uint16)(TCPIP_NTOHL(MtuNbo));

    /* #20 Check that the reported Mtu is valid (greater than the Minimum Mtu) */
    if (   (IPV6_MINIMUM_MTU < TcpIp_GetAddrIpV6DestinationCacheEntry(ipV6DestinationCacheEntryIdx)->Mtu)               /* PRQA S 3415 2 */ /* MD_TCPIP_Rule13.5_3415_CslReadOnly */
        && (TcpIp_GetAddrIpV6DestinationCacheEntry(ipV6DestinationCacheEntryIdx)->MtuReported < TcpIp_GetAddrIpV6DestinationCacheEntry(ipV6DestinationCacheEntryIdx)->Mtu))
    {
      if (IPV6_MINIMUM_MTU <= TcpIp_GetAddrIpV6DestinationCacheEntry(ipV6DestinationCacheEntryIdx)->MtuReported)
      {
        TcpIp_GetAddrIpV6DestinationCacheEntry(ipV6DestinationCacheEntryIdx)->Mtu =
          TcpIp_GetAddrIpV6DestinationCacheEntry(ipV6DestinationCacheEntryIdx)->MtuReported;
      }
      else
      {
        TcpIp_GetAddrIpV6DestinationCacheEntry(ipV6DestinationCacheEntryIdx)->Mtu = IPV6_MINIMUM_MTU;
      }

      IPV6_SET_LIFETIME_S(TcpIp_GetAddrIpV6DestinationCacheEntry(ipV6DestinationCacheEntryIdx)->MtuTimeout,
        TcpIp_GetPathMtuTimeoutOfIpV6Ctrl(IpCtrlIdx), (IPV6_P2V(IpV6_TimeType))NULL_PTR);

      {
        uint16 UpperLayerPayloadLen = (uint16)(TcpIp_GetAddrIpV6DestinationCacheEntry(ipV6DestinationCacheEntryIdx)->Mtu - IPV6_HDR_LEN);
        TcpIp_SockAddrBaseType sockAddr;                                                                                /* PRQA S 0759 */ /* MD_MSR_Union */

        if (TcpIp_GetAddrIpV6DestinationCacheEntry(ipV6DestinationCacheEntryIdx)->MtuReported < IPV6_MINIMUM_MTU)
        {
        ...
        /* PRQA S 0315 1 */ /* MD_MSR_VStdLibCopy */
        IPV6_ADDR_COPY(sockAddr.SockAddrIn6.sin6_addr.addr[0], TcpIp_GetAddrIpV6DestinationCacheEntry(ipV6DestinationCacheEntryIdx)->DestinationAddress);
  ...
  uint16                      LenByte)
{
  ...
  else if (   (LenByte < (sizeof(IpV6_IcmpPacketTooBigMsgType) + IPV6_HDR_OFF_DSTADDR + IPV6_ADDRESS_LEN_BYTE))
           || (IPV6_ICMP_ERR_MSG_MAX_PAYLOAD_SIZE < LenByte))
  {
    ...
    if (multiPartExtLen >= (uint16) sizeof(IpV6_IcmpMultiPartExtHdr))
    {
      ...
      if (((multiPartExtHdr.VerReserved >> 4) == 2u) && (multiPartExtHdr.ChecksumNbo != 0u))
      {
        ...
        uint16 calculatedChecksumNbo = (uint16)IpBase_TcpIpChecksumAdd(&icmpPayloadPtr[multiPartDataOfsTmp],
                    multiPartDataLenTmp, (((uint32)multiPartExtHdr.VerReserved) << TCPIP_BITS_IN_BYTE), TRUE);
  ...
  P2CONST(IpV6_Icmp_MsgInstanceType, AUTOMATIC, TCPIP_APPL_DATA) PendingMsgPtr)
{
  ...
  TcpIp_IpTxRequestDescriptorType IpTxReqDesc = { 0 };
  IPV6_P2V(uint8)                 IcmpBufPtr = NULL_PTR;
  uint16                          IcmpBufLen = PendingMsgPtr->PayloadLen;
  TcpIp_LocalAddrIterType         ipAddrIdx  = PendingMsgPtr->IpAddrIdx;

  /* ----- Implementation ----------------------------------------------- */
  /* #10 Request transmission buffer */
  if (IpV6_Icmp_ProvideTxBuffer(&ipAddrIdx, TcpIp_GetIpV6SocketDynIcmpIdxOfIpV6General(TCPIP_IPV6GENERAL_IDX),
    &IpTxReqDesc, &PendingMsgPtr->DstAddr, &IcmpBufPtr, &IcmpBufLen) == BUFREQ_OK)
  {
    if (IpTxReqDesc.ChecksumHwCalced == TCPIP_IP_PROTOCOL_CHECKSUM_CHECK_COPY)
    {
      IpTxReqDesc.TmpChecksum = IpBase_TcpIpChecksumCopyAdd(&IcmpBufPtr[0], &PendingMsgPtr->PayloadPtr[0],
                                      PendingMsgPtr->PayloadLen, IpTxReqDesc.TmpChecksum, FALSE);
    }
    else
    {
      /* The checksum will be calculated at the end. */
      /* PRQA S 0315 1 */ /* MD_MSR_VStdLibCopy */
  ...
  /* Else: result is already set to E_NOT_OK */
  ...
  uint16                                      IpV6PktDataOffs)
{
  ...
  uint16                                      MaxOrgPktLen = TcpIp_GetSizeOfIcmpV6TxMsgBuffer();
  TcpIp_SizeOfIcmpV6TxMsgBufferType           icmpV6TxMsgBufferIdx = (uint16)sizeof(IpV6_IcmpParameterProblemErrMsgType);
  P2VAR(uint8, AUTOMATIC, TCPIP_VAR_NO_INIT)  MsgPtr = TcpIp_GetAddrIcmpV6TxMsgBuffer(0);
  IpV6_AddrType                               destAddr;                                                                 /* PRQA S 0759 */ /* MD_MSR_Union */
  ...
  IpV6_MemCpy(TcpIp_GetAddrIcmpV6TxMsgBuffer(icmpV6TxMsgBufferIdx), IpV6PktDataPtr, pktDataLen);
...
{
  /* ----- Local Variables ---------------------------------------------- */
  uint16          maxOrigPktLen         = TcpIp_GetSizeOfIcmpV6TxMsgBuffer();
  ...
  else
  {
    ...
    TcpIp_SizeOfIcmpV6TxMsgBufferType multiPartExtHdrOfsIcmpV6TxMsgBufferIdx = 0u;
    IPV6_P2V(uint8) MultiPartExtHdrPtr = NULL_PTR;
    ...
    multiPartExtHdrOfsIcmpV6TxMsgBufferIdx = (*IcmpPayloadOfsPtr);

    MultiPartExtHdrPtr = TcpIp_GetAddrIcmpV6TxMsgBuffer(multiPartExtHdrOfsIcmpV6TxMsgBufferIdx);
    ...
    TCPIP_PUT_UINT16_RAW(MultiPartExtHdrPtr, IPV6_ICMP_MULTI_EXT_HDR_OFF_CHECKSUMNBO,
      (uint16)IpBase_TcpIpChecksumAdd(TcpIp_GetAddrIcmpV6TxMsgBuffer(multiPartExtHdrOfsIcmpV6TxMsgBufferIdx),
  ...
  uint16                       MultiPartExtLen)
{
  ...
  uint16                            MaxOrigPktLen         = TcpIp_GetSizeOfIcmpV6TxMsgBuffer();
  IPV6_P2V(uint8)                   TimeExceededMsgHdrPtr = TcpIp_GetAddrIcmpV6TxMsgBuffer(0);
  TcpIp_SizeOfIcmpV6TxMsgBufferType icmpV6TxMsgBufferIdx        = (uint16)sizeof(IpV6_IcmpTimeExceededMsgType);
  uint16                            OrigPktDataLen        = (*OrigPktDataLenPtr);
  ...
  {
    retVal = IpV6_Icmp_VMultipartTimeExdMessage(MultiPartExtLen, &icmpV6TxMsgBufferIdx,
  ...
  else
  {
    ...
    /* return buffer and length for original packet payload. Caller will copy data directly into the buffer */
    (*OrigPktDataPtr)    = TcpIp_GetAddrIcmpV6TxMsgBuffer(icmpV6TxMsgBufferIdx);
    (*OrigPktDataLenPtr) = OrigPktDataLen;

    icmpV6TxMsgBufferIdx += OrigPktDataLen;
  }
  ...
  if(retVal == E_OK)
  {
    ...
    IpV6_Icmp_PendingMsg.Code       = Code;
    IpV6_Icmp_PendingMsg.PayloadPtr = TcpIp_GetAddrIcmpV6TxMsgBuffer(0);
    IpV6_Icmp_PendingMsg.PayloadLen = icmpV6TxMsgBufferIdx;
    IpV6_Icmp_PendingMsg.Pending    = TRUE;
  }
...
# if (TCPIP_SUPPORT_IPV6_PATH_MTU == STD_ON) || (TCPIP_SUPPORT_NDP_INV_NA == STD_ON) || (TCPIP_SUPPORT_NDP_INV_NS == STD_ON) || (IPV6_ICMP_ENABLE_NDP == STD_ON)
...
# if (IPV6_ICMP_ENABLE_NDP == STD_ON)
  ...
  case IPV6_ICMP_MSG_TYPE_NDP_RA:           /* Type 134 */
  {
    ...
    if (   (IPV6_NDP_MSG_HOP_LIMIT == ipV6Hdr.HopLimit)
        ...
        && (TcpIp_IsDropRouterAdvertisementOfNdpConfig(ndpConfigIdx) == FALSE))
    {
  ...
  case IPV6_ICMP_MSG_TYPE_NDP_REDIR:        /* Type 137 */
  {
    ...
    if ((IPV6_NDP_MSG_HOP_LIMIT == ipV6Hdr.HopLimit)
      ...
      && (TcpIp_IsDropRedirectOfNdpConfig(ndpConfigIdx) == FALSE))
    {
    ...
    TCPIP_P2C(uint8)                   DataPtr)
{
  ...
  TcpIp_SizeOfIpV6SocketDynType     icmpDynIpV6SocketIdx = TcpIp_GetIpV6SocketDynIcmpIdxOfIpV6General(TCPIP_IPV6GENERAL_IDX);
  ...
  if (retVal == BUFREQ_OK)
  {
    ...
    if (ipTxReqDesc.ChecksumHwCalced == TCPIP_IP_PROTOCOL_CHECKSUM_CHECK_COPY)
    {
      ipTxReqDesc.TmpChecksum = IpBase_TcpIpChecksumCopyAdd(icmpBufferPtr, DataPtr, DataLength, ipTxReqDesc.TmpChecksum, FALSE);
    }
    else
    {
      /* The checksum will be calculated at the end. */
...
FUNC(Std_ReturnType, TCPIP_CODE) IpV6_Icmp_Transmit(
  TcpIp_SizeOfIpV6SocketDynType             SocketIdx,
  ...
  uint16                                    PayloadLen)
{
  ...
  P2VAR(uint8, AUTOMATIC, TCPIP_APPL_VAR) IcmpV6HdrPtr = IpV6_Icmp_EthBufPtr;
  ...
  if (IpTxReqDescPtr->ChecksumHwCalced == TCPIP_IP_PROTOCOL_CHECKSUM_CHECK_COPY)
  {
    IpTxReqDescPtr->TmpChecksum = IpBase_TcpIpChecksumAdd(IcmpV6HdrPtr, sizeof(IpV6_IcmpHeaderType),
                                                          IpTxReqDescPtr->TmpChecksum, FALSE);
  }
...
FUNC(BufReq_ReturnType, TCPIP_CODE) IpV6_Icmp_ProvideTxBuffer(
    ...
    IPV6_P2C(IpV6_AddrType)                   DstAddrPtr,
    IPV6_P2V(TcpIp_Uint8P2V)                  BufPtr,
    IPV6_P2V(uint16)                          BufLenPtr)
{
  /* ----- Local Variables ---------------------------------------------- */
  BufReq_ReturnType BufReqRes = BUFREQ_E_NOT_OK;
  ...
  *BufLenPtr += (uint16) sizeof(IpV6_IcmpHeaderType);
  ...
  BufReqRes = IpV6_ProvideTxBuffer(IpAddrIdxPtr, SocketIdx, DstAddrPtr, BufPtr, BufLenPtr, TxReqDescPtr, TCPIP_PROTOCOL_ICMP, FALSE);

  if (BufReqRes == BUFREQ_OK)
  {
