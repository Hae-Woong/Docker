...
/**        \file  Dcm_Svc28.c
 *         \unit  Svc28
 *        \brief  Contains the implementation of Service 0x28 unit.
 *
...
{
  DCM_IGNORE_UNREF_PARAM(networkRef);
...
# if (DCM_NET_COMCTRL_ALLNET_SUPPORT_ENABLED == STD_ON)
    ...
    for (lChannelIter = Dcm_NetPbCfgGetComCtrlChannelListAll(0); lChannelIter != 0u; --lChannelIter)
    {
      Dcm_ModeSwitchCommControl(Dcm_NetPbCfgGetComCtrlChannelListAll(lChannelIter), mode);
    }
# endif
  }
# if (DCM_SVC_28_CURNET_SUPPORT_ENABLED == STD_ON) && \
...
{
  uint8_least channelIter = Dcm_NetPbCfgGetNumAllComMChannels();

  while (channelIter != 0u) /* Use top-down counting to be consistent with the Dcm_Svc28ApplyCommControl all-channel calls */
  {
    --channelIter;
    if (Dcm_NetPbCfgGetComCtrlChannelState(channelIter) != DCM_ENABLE_RX_TX_NORM_NM)
    {
  ...
  )
{
  ...
  Dcm_Svc28RepeaterProxyContextPtrType pRepContext = Dcm_RepeaterGetSvc28Context(pContext->ThreadId);
...
# if (DCM_SVC_28_CURNET_SUPPORT_ENABLED == STD_ON)
      ...
      {
        pRepContext->NetworkRef = Dcm_NetPbCfgGetNetworkRefByConnHdl(Dcm_DiagGetTranspObj(pContext->ThreadId)->ConnHdl);                             /* SBSW_DCM_GLOBAL_PTR_WRITE */
      }
# endif
    }
    else
  ...
  )
{
  ...
  {
    Dcm_Svc28RepeaterProxyContextConstPtrType pRepContext = Dcm_RepeaterGetSvc28Context(pContext->ThreadId);

    if ((pRepContext->SubServiceId == 0x01u)
      || (pRepContext->SubServiceId == 0x02u)
      || (pRepContext->SubServiceId == 0x03u))
    {
      Dcm_DebugReportSecurityEvent(pContext->ThreadId, DCM_DEBUG_SEV_COMMUNICATION_CONTROL_SWITCHED_OFF);
    }

    if (pRepContext->IsInternallyProcessed == TRUE)
    {
      Dcm_Svc28ApplyCommControl(pRepContext->NetworkRef,
        pRepContext->ComMode);
    }
  }
}
