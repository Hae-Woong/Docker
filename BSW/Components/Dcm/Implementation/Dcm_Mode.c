...
/**        \file  Dcm_Mode.c
 *         \unit  Mode
 *        \brief  Contains the implementation of Mode Manager unit.
 *
...
# if (DCM_MODE_COMMCTRL_ENABLED == STD_ON)
  /* Initialize CommunicationControl internal mode */
  {
    ...
    for (channelIter = 0; channelIter < Dcm_NetPbCfgGetNumAllComMChannels(); ++channelIter)
    {
  ...
  )
{
  ...
  DCM_IGNORE_UNREF_PARAM(DTCGroup);
  DCM_IGNORE_UNREF_PARAM(DemClientId);
...
{
  NetworkHandleType lNetwork = Dcm_NetPbCfgGetAllComMChannelMap(networkRef);
