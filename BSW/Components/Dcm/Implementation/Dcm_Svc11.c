...
/**        \file  Dcm_Svc11.c
 *         \unit  Svc11
 *        \brief  Contains the implementation of Service 0x11 unit.
 *
...
#  if (DCM_SVC_11_RST2BOOT_HIS_ENABLED == STD_ON)
    if(Dcm_CfgWrapSvc11SubFuncInfo(pRepContext->SubSvcRef)->ResponseType == DCM_SVC11_RESPONSE_AFTER_RESET)
    {
      ...
      if(Dcm_NetPbCfgGetHasRespPendOnBoot(Dcm_DiagGetTranspObj(pContext->ThreadId)) == TRUE)                                                         /* SBSW_DCM_PARAM_PTR_FORWARD */
      {
...
#  endif
  ...
  if(Dcm_ModeSwitchEcuReset(mode) == DCM_E_OK)
  {
  ...
  )
{
  ...
  /* Try performing mode switch on ECU reset */
  if(Dcm_ModeSwitchRapidShutDown(mode) == DCM_E_OK)
  {
    /* If succeeded, go on with waiting for acknowledge */
  ...
  )
{
  ...
  Dcm_Svc11RepeaterProxyContextPtrType pRepContext = Dcm_RepeaterGetSvc11Context(pContext->ThreadId);
...
# if (DCM_SVC_11_ECU_RESET_ENABLED == STD_ON)
  ...
  {
    if(Dcm_RepeaterGetSvc11Context(pContext->ThreadId)->SubSvcId < 0x04u)
    {
