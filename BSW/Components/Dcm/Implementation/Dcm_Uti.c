...
/**        \file  Dcm_Uti.c
 *         \unit  Uti
 *        \brief  Contains the implementation of Utilities unit.
 *
  ...
  )
{
  ...
  Dcm_DebugAssert(Dcm_UtiIsPowerOfTwo(uint32, lBitSet), DCM_SID_INTERNAL, DCM_E_PARAM);                                                              /* COV_DCM_RTM_DEV_DEBUG TX */
  ...
  )
{
  ...
  if (opStatus == DCM_INITIAL)
  {
    ...
    else
    {
      ...
      Dcm_DebugAssert((opStatus == DCM_PENDING), DCM_SID_INTERNAL, DCM_E_ILLEGAL_STATE);                                                             /* COV_DCM_RTM_DEV_DEBUG TX */
    }
  ...
  )
{
  ...
  if (Dcm_UtiRuntimeLimiterEnabled())
  {
...
{
  DCM_IGNORE_UNREF_PARAM(targetSize);
   ...
   */
  if (Dcm_DebugDetectRuntimeError((targetSize < targetPos)
  ...
  )
{
   ...
   */
  if(Dcm_DebugDetectRuntimeError(pDataContext->AvailLen < length))
  {
    Dcm_DebugReportError(DCM_SID_INTERNAL, DCM_E_CRITICAL_ERROR);
  }
  else
  ...
  )
{
  ...
  if (Dcm_DebugDetectRuntimeError(index >= pMsgContext->resBufSize))
  {
    Dcm_DebugReportError(DCM_SID_INTERNAL, DCM_E_CRITICAL_ERROR);
