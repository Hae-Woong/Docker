...
# endif /* ((DCM_AUTHMGR_APCE_INTERNAL_ENABLED == STD_ON) || (DCM_SVC_29_04_SUPPORT_ENABLED == STD_ON)) */
# if ((DCM_AUTHMGR_APCE_INTERNAL_ENABLED == STD_ON) || (DCM_RIDMGR_SECURE_CODING_ENABLED == STD_ON))                                                 /* COV_DCM_SUPPORT_PARTIALLY TF tf xf */
#  include "Csm.h"
# endif /* ((DCM_AUTHMGR_APCE_INTERNAL_ENABLED == STD_ON) || (DCM_RIDMGR_SECURE_CODING_ENABLED == STD_ON)) */
 ...
 *  \reentrant      TRUE
 *  \synchronous    TRUE
 *  \config         This function is only available if service 0x29 is handled internally
 *  \pre            -
*********************************************************************************************************************/
 ...
 *  Dcm_SetChannelReady()
 *********************************************************************************************************************/
/*! \brief          ComM callback for setting channel availability to "Ready".
 *  \details        This call informs the DCM module that a specific ComM channel is "Ready". This is necessary in 
 *                  case of DoIP, when the DCM needs to wait until a tester is connected, to send an unsolicited 
 *                  final response for a service 0x10 or service 0x11 after a reset.
 *  \param[in]      networkId    The communication channel on which this state is now active.
 *  \context        ISR1|ISR2
 *  \reentrant      TRUE
 *  \synchronous    TRUE
