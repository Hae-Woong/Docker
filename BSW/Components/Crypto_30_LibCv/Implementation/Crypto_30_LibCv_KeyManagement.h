 ...
 *                 The derived key is set at the target key element (CRYPTO_KE_KEYDERIVATION_PASSWORD).
 *               - For CRYPTO_30_LIBCV_KDF_ALGO_KDF_ISO_15118_20_CERTIFICATE_HANDLING:
 *                 The KDF reads the algorithm configuration from the cryptoKeyId key elements.
 *                 The following key element need to be set at the cryptoKeyId:
 ...
 *                 the PCID and SKI (CRYPTO_KE_CUSTOM_SCC_CONTRACT_AAD) and
 *                 the public key needed for ECDH (CRYPTO_KE_CUSTOM_KEYEXCHANGE_PARTNER_PUB_KEY).
 *                 The derived key is set at the target key element (CRYPTO_KE_KEYDERIVATION_PASSWORD).
 ...
 *                 The write access right for the destination key element must be less than or equal to WA_INTERNAL_COPY.
 *               - For CRYPTO_KDF_ALGO_HKDF_EXPAND_HMAC_SHA256 and CRYPTO_KDF_ALGO_HKDF_EXPAND_HMAC_SHA384:
 *                 The given key must contain the key elements for the password (CRYPTO_KE_KEYDERIVATION_PASSWORD) and optionally
 *                 CRYPTO_KE_CUSTOM_KEYDERIVATION_ADDITIONAL_INFO (additional information).
 *                 The derived key is stored in the key element with id 1 of the given key identified by targetCryptoKeyId.
 *                 The write access right for the destination key element must be less than or equal to WA_INTERNAL_COPY.
 *               - For CRYPTO_30_LIBCV_KDF_ALGO_HKDF_OPTION1_SHA256 and CRYPTO_30_LIBCV_KDF_ALGO_HKDF_OPTION1_SHA512:
 *                 The given key must contain the key elements for the password (CRYPTO_KE_KEYDERIVATION_PASSWORD),
