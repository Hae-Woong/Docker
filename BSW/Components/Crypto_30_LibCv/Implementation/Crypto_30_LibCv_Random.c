 ...
 *  \param[in]      partitionIdx                  The index of the currently active partition.
 *  \param[in]      cryptoKeyId                   Identifier of the crypto key.
 ...
 *  \param[in]      currentPartitionIdx      The partition Idx of the currently running partition.
 ...
 *  \synchronous    TRUE
 *********************************************************************************************************************/
CRYPTO_30_LIBCV_LOCAL_INLINE FUNC(boolean, CRYPTO_30_LIBCV_CODE) Crypto_30_LibCv_IsRngCalculationValid(uint16 localCount, uint32 currentPartitionIdx);
...
# if (CRYPTO_30_LIBCV_NVBLOCK == STD_ON)
          ...
          {
            Crypto_30_LibCv_SetProcessJob_Trigger_Write(objectId, TRUE); /* SBSW_CRYPTO_30_LIBCV_CSL01_OBJECTID */
          }
# endif
        }
        else
  ...
  P2VAR(boolean, AUTOMATIC, AUTOMATIC) persist) /* PRQA S 3673 */ /* MD_CRYPTO_30_LIBCV_3673_1 */
{
  ...
  if (retVal == E_OK)
  {
    ...
    {
      /* # Copy counter and RNG paramater to local buffers. */
      localCount = Crypto_30_LibCv_GetRandomSourceGenerateCount(partitionIdx);
      ...
      if ((Crypto_30_LibCv_IsRngCalculationValid(localCount, partitionIdx)) && (retValCv == ESL_ERC_NO_ERROR))
      {
        retVal = Crypto_30_LibCv_Local_Get_And_Set_NistDrbgAesState(
          seedStateElementIndex,
  ...
  P2VAR(Crypto_JobType, AUTOMATIC, CRYPTO_30_LIBCV_APPL_VAR) job) /* PRQA S 3673 */ /* MD_CRYPTO_30_LIBCV_3673_1 */
{
  ...
  {
    Crypto_30_LibCv_SetProcessJob_Trigger_Write(objectId, persist); /* SBSW_CRYPTO_30_LIBCV_CSL01_OBJECTID */
# if(CRYPTO_30_LIBCV_KEYVALUECHANGEDCALLOUTFCTNAMEOFCONFIGURABLECALLOUTS == STD_ON)
  ...
  P2VAR(eslt_WorkSpaceCTRDRBG, AUTOMATIC, CRYPTO_30_LIBCV_CRYPTOCV_APPL_VAR) wsDRBG) /* PRQA S 3673 */ /* MD_CRYPTO_30_LIBCV_3673_1 */
{
  ...
  eslt_DRBGSeedStatusType seedStatus;
  eslt_DRBGMode modeDRBG;
  eslt_Length seedLength = Crypto_30_LibCv_GetKeyElementLength(seedStateElementIndex);
  ...
  P2VAR(eslt_Length, AUTOMATIC, CRYPTO_30_LIBCV_APPL_VAR) seedLength) /* PRQA S 3673 */ /* MD_CRYPTO_30_LIBCV_3673_1 */
{
  ...
  {
    /* #20 Set correct mode and seed length */
  ...
  eslt_DRBGSeedStatusType seedStatus) /* PRQA S 3673 */ /* MD_CRYPTO_30_LIBCV_3673_1 */
{
  ...
  {
    Crypto_30_LibCv_SetKeyElementWrittenLength(seedStateElementIndex, seedLength);
...
CRYPTO_30_LIBCV_LOCAL_INLINE FUNC(boolean, CRYPTO_30_LIBCV_CODE) Crypto_30_LibCv_IsRngCalculationValid(uint16 localCount, uint32 currentPartitionIdx)
{
  ...
  if (localCount == Crypto_30_LibCv_GetRandomSourceGenerateCount(currentPartitionIdx))
  {
    validCalculation = TRUE;
    Crypto_30_LibCv_SetRandomSourceGenerateCount((Crypto_30_LibCv_RandomSourceGenerateCountType)(localCount + 1u), currentPartitionIdx); /* SBSW_CRYPTO_30_LIBCV_WRITE_PARTITION_DATA */
  }
  else
  {
...
# if (CRYPTO_30_LIBCV_NUMBER_OF_PARTITIONS < 2u)
  CRYPTO_30_LIBCV_DUMMY_STATEMENT(currentPartitionIdx); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint !e438 */
# endif
  ...
  Crypto_OperationModeType mode)
{
  ...
  if (mode == CRYPTO_OPERATIONMODE_FINISH)
  {
    if (Crypto_30_LibCv_Local_KeyElementSearch(job->cryptoKeyId, CRYPTO_KE_RANDOM_SEED_STATE, &elementIndex) == E_OK) /* SBSW_CRYPTO_30_LIBCV_STACK_VARIABLE_AS_PTR */
    {
      ...
      Crypto_30_LibCv_SetProcessJob_Trigger_Write(objectId, writeBlock); /* SBSW_CRYPTO_30_LIBCV_CSL01_OBJECTID */
    }
    else
    {
      retVal = E_NOT_OK;
...
# if (CRYPTO_30_LIBCV_DEV_ERROR_DETECT == STD_ON)
    if ((Crypto_30_LibCv_IsModulePartitionInitialized(partitionIdx) && Crypto_30_LibCv_IsModuleSharedInitialized()) && (target != NULL_PTR))
# endif
    {
# if(CRYPTO_30_LIBCV_RANDOM_SOURCE_DRBG_AES == STD_ON)
#  if(CRYPTO_30_LIBCV_RANDOM_SOURCE_DRBG_HASH == STD_ON)
      if(Crypto_30_LibCv_IsAesOfPartitionInfo(partitionIdx))
#  endif
      {
        /* # Generate Drbg Aes Random Number */
        rngResult = Crypto_30_LibCv_GetRandomNistDrbgAes(partitionIdx, defaultRandomKey, target, targetLength, &triggerPersist); /* SBSW_CRYPTO_30_LIBCV_API_CHECKED_SIZE_ACCESS */
      }
# endif
# if(CRYPTO_30_LIBCV_RANDOM_SOURCE_DRBG_HASH == STD_ON)
#  if(CRYPTO_30_LIBCV_RANDOM_SOURCE_DRBG_AES == STD_ON)
      if(Crypto_30_LibCv_IsHashOfPartitionInfo(partitionIdx))
#  endif
      {
        /* # Generate Drbg Hash Random Number */
        rngResult = Crypto_30_LibCv_GetRandomNistDrbgHash(partitionIdx, defaultRandomKey, target, targetLength, &triggerPersist); /* SBSW_CRYPTO_30_LIBCV_API_CHECKED_SIZE_ACCESS */
      }
# endif
...
# if (CRYPTO_30_LIBCV_NVBLOCK == STD_ON)
      if (triggerPersist)
      {
        Crypto_30_LibCv_NvBlock_Write_Req(Crypto_30_LibCv_GetNvBlockIdxOfKey(defaultRandomKey));
      }
# endif
    }
...
# if(CRYPTO_30_LIBCV_KEYVALUECHANGEDCALLOUTFCTNAMEOFCONFIGURABLECALLOUTS == STD_ON)
        /* Call the callout if the functionality is enabled and trigger that the key value is changed */
        Crypto_30_LibCv_KeyValueChangedCallout(defaultRandomKey);
# endif
        /* Call the callout if the functionality is enabled and trigger that the key is set to valid */
# if(CRYPTO_30_LIBCV_KEYVALIDITYSETCALLOUTFCTNAMEOFCONFIGURABLECALLOUTS == STD_ON)
        Crypto_30_LibCv_GetKeyValiditySetCalloutFctNameOfConfigurableCallouts()(defaultRandomKey, TRUE);
# endif
    }
