 ...
 *                                      ESCAN00066316   Removed excluded code (by preprocessor switches) from Fee_Partition.c
 ...
 * 8.02.00      2017-10-18   Jwa        FEAT-2503       Implemented lookup table support to decrease chunk search overhead
 * 8.03.00      2017-12-04   Jwa        ESCAN00097636   Reach 100 percent mc/dc coverage for lookup table feature
 *              2017-12-21   Jwa        ESCAN00097610   Potentially non initialized variable might lead to invalid pointer write access
 *              2018-06-27   Jwa        STORY-5868      Reach quality status ASIL D for lookup table feature
 *              2018-06-27   Jwa        ESCAN00094523   Add macros identifying current compilation unit
 *              2018-06-27   Jwa        STORY-6501      Support asynchronous cancel operation of underlying module
 * 8.04.00      2018-09-20   Jwa        STORY-6248      MISRA-C:2012 Compliance
 *              2018-10-29   Jwa        ESCAN00100222   Fee Read operation might deliver corrupted data
 * 8.04.01      2019-01-30   Jwa        ESCAN00101715   Fee loses data of several blocks at once
 * 8.04.02      2019-03-12   Jwa        -               Trivial Changes in order to comply with vector coding rules and doxygen rules
 * 8.04.03      2019-07-16   Jwa        ESCAN00103359   Fee loses data of several blocks at once
 * 8.05.00      2019-10-09   Jwa        MWDG-2045       Recognize unreliable sector header in order to increase performance in case of an erase abort
 * 8.05.01      2020-01-08   Jwa        ESCAN00105010   FEE might deliver outdated data due to unnecessary copy operation
 *                                      ESCAN00105078   FEE might lose data of blocks that do not fit to link table
 * 8.06.00      2020-06-24   Jwa        MWDG-2865       Extend instance trailer page with status informations to ease dump analysis
 * 9.00.00      2020-09-07   Jwa        MWDG-3512       FEE shall optimize flash driver read job handling
 * 9.00.01      2020-11-23   Jwa        ESCAN00107873   All write jobs might fail due to partly written flash pages
 * 9.00.02      2020-12-15   Jwa        ESCAN00108063   Fee Read operation might deliver corrupted data
 * 9.00.03      2021-03-15   Jwa        MWDG-4977       Fee: Reliably distinguish write aborted from invalid or inconsistent instances
 * 9.00.04      2021-05-28   Jwa        MWDG-5426       Fee shall write chunk links even if a broken link was found before in the chunk chain.
 * 9.00.05      2021-09-01   Jwa        MWDG-5558       Fee: Fix QAC rule 9.1 found violated during products MISRA analysis
 * 10.00.00     2021-11-10   Jwa        ESCAN00110182   Bad Performance due to unexpected erase procedure on Cypress Traveo and Cypress Traveo-II devices
 * 10.00.01     2022-01-18   Jwa        ESCAN00111054   Unintentionally overwriting the erase start marker lets the following user jobs fail
 * 10.00.02     2022-02-25   Eno        MWDG-6669       Remove mapping of FEE_CODE to FEE_API_CODE
 * 10.00.03     2022-03-09   Eno        MWDG-6669       Close comment in implementation
 * 10.00.04     2022-03-09   Jwa        MWDG-6754       Remove statemachine tracer
 *********************************************************************************************************************/
 ...
 * Fee_SyncLookUpTable
 *********************************************************************************************************************/
/*! \brief      Synchronizes RAM LookUpTable to flash memory
 *  \details    Writes content of all RAM LookUpTable blocks to corresponding FEE blocks
 *  \return     E_OK                Job accepted.
 *  \return     E_NOT_OK            Job rejected.
