...
/* CRC32PJ193976 polynomial */
#define CRC_INITIAL_VALUE32PJ193976 (uint32)(0xFFFFFFFFuL)
#define CRC_POLYNOMIAL_32PJ193976   (uint32)(0x6938392DuL)
...
#include "Crc_MemMap.h"                                                                                                     /* PRQA S 5087 */ /* MD_MSR_MemMap */
 ...
 * \details     This service is used by Crc_CalculateCRC32PJ193976 routine.
 * \param[in]   Crc_Length    Length of data to be processed.
 * \param[in]   Crc_DataPtr   Data to be processed. 
 *                            Must not be NULL and must be valid for the duration of the CRC calculation.
 * \param[in]   Crc_Value     Start value for processing.
 * \param[in]   Crc_Poly      Polynomial to use for CRC32 calculation.
...
/**********************************************************************************************************************
 *  Crc_CalculateCRC32RuntimeReflexion()
 *********************************************************************************************************************/
/*!
 * \brief       Performs runtime calculation of 32-bit CRC values using the reflected crc polynomial.
 * \details     This service is used by Crc_CalculateCRC32 and Crc_CalculateCRC32P4 routines.
 * \param[in]   Crc_Length          Length of data to be processed.
 * \param[in]   Crc_DataPtr         Data to be processed. 
 *                                  Must not be NULL and must be valid for the duration of the CRC calculation.
 * \param[in]   Crc_Value           Start value for processing.
 * \param[in]   Crc_PolyReflected   Reflected polynomial to use for CRC32 calculation.
 * \return      Calculated 32-bit CRC value
 * \pre         -
 ...
 * \synchronous TRUE
 */
CRC_LOCAL FUNC(uint32, CRC_PRIVATE_CODE) Crc_CalculateCRC32RuntimeReflexion(
  uint32 Crc_Length,
  Crc_DataRefType Crc_DataPtr,
  uint32 Crc_Value,
  ...
  uint8 Crc_Poly)
{
  ...
  for (Crc_DataPtrAddr = 0; Crc_DataPtrAddr < Crc_Length; Crc_DataPtrAddr++)                                            /* FETA_CRC_LENGTH */
  {
  ...
  uint16 Crc_Value)
{
  ...
  while(Crc_DataPtrAddr < Crc_Length)                                                                                   /* FETA_CRC_LENGTH */
  {
    /* Process 4 bytes as long as possible */
    if ((Crc_DataPtrAddr + CRC_SB4_CHUNKSIZE) <= Crc_Length)
    {
        /* Note for the future: Shifting the CRC value to get the index of the CRC table didn't work on the big endian 
        tests for PowerPC. Using the addition we currently have fixed this problem. 
        Not exactly sure why, it seems that shifting doesn't always seem to work as expected on that platform. */
      /* Calculate chunks:
  ...
  uint32 Crc_Poly)
{
  ...
  for (Crc_DataPtrAddr = 0; Crc_DataPtrAddr < Crc_Length; Crc_DataPtrAddr++)                                            /* FETA_CRC_LENGTH */
  {
    /* XOR next byte of Crc_DataPtr with current CRC value. This is equivalent to calculating CRC value of 
     * concatenated bytes */
    Crc_CalcValue ^= ((uint32)Crc_DataPtr[Crc_DataPtrAddr] << 24u);

    /* Perform modulo-2 division, a bit at a time */
    for (Crc_LoopCounter = 0u; Crc_LoopCounter < 8u; Crc_LoopCounter++)
    {
      /* If MSB is 1, CRC value is XORed with polynomial */
      if ( (Crc_CalcValue & (uint32)0x80000000u) > 0u )
      {
        Crc_CalcValue = (Crc_CalcValue << 1u) ^ Crc_Poly;
      }
      else
      {
        Crc_CalcValue <<= 1u;
  ...
  uint32 Crc_Value)
{
  ...
  for ( Crc_LoopCounter = 0; Crc_LoopCounter < Crc_Length; Crc_LoopCounter++ )                                          /* FETA_CRC_LENGTH */
  {
  ...
  uint64 Crc_PolyReflected)
{
  ...
  for (index = 0; index < Crc_Length; index++)                                                                          /* FETA_CRC_LENGTH */
  {
  ...
  boolean Crc_IsFirstCall)
{
  ...
  Crc_Value = Crc_CalculateCRC32RuntimeReflexion(Crc_Length, Crc_DataPtr, Crc_Value, CRC_POLYNOMIAL_32_REFLECT);        /* SBSW_CRC_02 */
#endif
...
#else
  /*********** RUNTIME CALCULATION *****************************************************************************/
  Crc_Value = Crc_CalculateCRC32RuntimeReflexion(Crc_Length, Crc_DataPtr, Crc_Value, CRC_POLYNOMIAL_32P4_REFLECT);      /* SBSW_CRC_02 */
#endif
 ...
 *
 */
FUNC(uint32, CRC_CODE) Crc_CalculateCRC32PJ193976(
    Crc_DataRefType Crc_DataPtr, 
    uint32 Crc_Length, 
    uint32 Crc_StartValue32, 
    boolean Crc_IsFirstCall)
{
  ...
  if( Crc_IsFirstCall == TRUE )
  {
    Crc_Value = CRC_INITIAL_VALUE32PJ193976;
  }
  else
  {
  ...
  /*********** RUNTIME CALCULATION *****************************************************************************/
  /* Calculate CRC value via CRC32PJ193976 routine */
  Crc_Value = Crc_CalculateCRC32Runtime(Crc_Length, Crc_DataPtr, Crc_Value, CRC_POLYNOMIAL_32PJ193976);                 /* SBSW_CRC_02 */
