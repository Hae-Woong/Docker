  ...
  )
{
  ...
  Dem_EventIdType lEventId = Dem_DebounceBase_GetEventId(&(DebounceInfoData->Base));                                             /* SBSW_DEM_DEBOUNCECOUNTERBASED_INFO_BASE_PTR */
  sint16 lJumpToValue = Dem_Cfg_EventJumpDownValue(lEventId);
...
{
  Dem_EventIdType lEventId = Dem_DebounceBase_GetEventId(&(DebounceInfoData->Base));                                             /* SBSW_DEM_DEBOUNCECOUNTERBASED_INFO_BASE_PTR */
  sint16 lJumpToValue = Dem_Cfg_EventJumpUpValue(lEventId);
  boolean lEnableConditionsSatisfied = Dem_GlobalDiagnostics_TestEventEnableConditionsSatisfied(lEventId);
  ...
  )
{
  ...
  if (Dem_DebounceCounterBased_CheckDebounceInfoPtr((Dem_DebounceBase_ConstInfoPtrType)DebounceInfoPtr) == TRUE)                 /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  {
    Dem_DebounceCounterBased_InfoPtrType lDebounceInfo = Dem_DebounceCounterBased_Base2CounterBased(DebounceInfoPtr);            /* SBSW_DEM_CALL_DEBOUNCECOUNTERBASED_INFOPTR_INIT */
  ...
  )
{
  ...
  Dem_EventIdType lEventId = Dem_DebounceBase_GetEventId(DebounceInfoPtr);                                                       /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  sint16 lNewDebounceValue = Dem_DebounceCounterBased_GetNewDebounceValue(DebounceInfoPtr);                                      /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  boolean lEnableConditionsSatisfied = Dem_GlobalDiagnostics_TestEventEnableConditionsSatisfied(lEventId);
  ...
  )
{
    ...
    || ( (Dem_Cfg_EventPassedTarget(lEventId) == Value) && (ExpectedThreshold == FALSE)) )                                       
  {
    lReturnValue = Dem_SatelliteData_SetDebounceData(
                      SatelliteData,
                      (Dem_Debounce_DataType) Value);                                                                            /* PRQA S 2897 */ /* MD_DEM_2897 */
  }
  else
  {
  ...
  )
{
  ...
  else
  {
    ...
    { /* current debounce counter value is somewhere in range of passed threshold and failed threshold but not 0 */
      uint32 lIntermediateResult;
      if (DebounceValue > 0)
      {
        /* Cast to uint32 introduced to guarantee division in positive value space  - see qac rule 3103 */
        lIntermediateResult = ((uint32)DebounceValue * (uint32)127) / (uint32)lFailedTarget;
      ...
      {
        /* Cast to uint32 and negation introduced to perform division in positive value space  - see qac rule 3103 */
        lIntermediateResult = ((uint32)(-DebounceValue) * (uint32)128)  / (uint32)(-lPassedTarget);
        lReturnedFdc = -(sint8)lIntermediateResult;
