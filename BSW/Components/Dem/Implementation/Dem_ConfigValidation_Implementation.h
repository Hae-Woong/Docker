...
#define DEM_START_SEC_VAR_NO_INIT_UNSPECIFIED_RESTRICTED
 ...
 *  because pointer will only be initialized in this case.
 ...
 */
P2CONST(Dem_ConfigType, DEM_VAR_NO_INIT, DEM_INIT_DATA)  Dem_GlobalConfigPtr;                                                    /* PRQA S 3406 */ /* MD_DEM_8.6 */
#endif

#define DEM_STOP_SEC_VAR_NO_INIT_UNSPECIFIED_RESTRICTED
#include "Dem_MemMap.h"                                                                                                          /* PRQA S 5087 */ /* MD_MSR_MemMap */


#define DEM_START_SEC_VAR_INIT_8_RESTRICTED
#include "Dem_MemMap.h"                                                                                                          /* PRQA S 5087 */ /* MD_MSR_MemMap */
...
#define DEM_STOP_SEC_VAR_INIT_8_RESTRICTED
#include "Dem_MemMap.h"                                                                                                          /* PRQA S 5087 */ /* MD_MSR_MemMap */
...
/* ****************************************************************************
 * Dem_Infrastructure_ConfigValidation_InitPtrHashCode
 *****************************************************************************/
/*!
 * \brief         Get the hash code of a Dem configuration set
 *
 * \details       Get the hash code of a Dem configuration set.
 *                This hash code can be used to prevent initializing the Dem
 *                with an incompatible configuration.
 *
 * \param[in]     ConfigPtr
 *                Pointer to a Dem configuration set
 *
 * \return        Hash code of the configuration set
 *
 * \pre           -
 *
 * \context       TASK|ISR2
 ...
 */
DEM_LOCAL_INLINE FUNC(sint32, DEM_CODE)
Dem_Infrastructure_ConfigValidation_InitPtrHashCode(                                                                             /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  CONSTP2CONST(Dem_ConfigType, AUTOMATIC, DEM_INIT_DATA) ConfigPtr
)
{
  DEM_IGNORE_UNUSED_CONST_ARGUMENT(ConfigPtr)                                                                                    /* PRQA S 3112 */ /* MD_DEM_14.2 */
  return (sint32)(Dem_Cfg_GetInitDataHashCode(ConfigPtr));
}
...
# if (DEM_CFG_VARIANT_POSTBUILD_LOADABLE == STD_ON)
  ...
  if (Dem_Infrastructure_ConfigValidation_InitPtrHashCode(ConfigPtr) != DEM_CFG_INIT_DATA_HASH_CODE)                             /* SBSW_DEM_POINTER_FORWARD_API */
  {
    EcuM_BswErrorHook((uint16) DEM_MODULE_ID, (uint8) ECUM_BSWERROR_MAGICNUMBER);
  }
  else
