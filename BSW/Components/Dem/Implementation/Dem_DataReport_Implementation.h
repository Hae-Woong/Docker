...
#if ((DEM_FEATURE_NEED_TIME_SERIES == STD_ON) || (DEM_FEATURE_NEED_CUSTOMTRIGGERSTORAGE == STD_ON))
/* ****************************************************************************
 ...
 *                DEM_FEATURE_NEED_CUSTOMTRIGGERSTORAGE == STD_ON
 *
 * \context       TASK|ISR2
 * \synchronous   TRUE
 * \reentrant     TRUE
 ...
 *****************************************************************************/
/*!
 * \brief         Retrieves the user data stored in the event entry or
 *                time series entry
 *
 * \details       Retrieves the user data stored in the event entry or
 *                time series entry
 *
...
Dem_DataReport_WriteTimeSeriesPowertrainSequentialHeader(
  ...
  Dem_TimeSeriesEntry_PtrToConstBaseType SourceEntry,
  CONSTP2CONST(Dem_Memory_TimeSeries_PowertrainTimeSeriesInfoType, AUTOMATIC, AUTOMATIC) TimeSeriesInfo
);
#endif
  ...
  )
{
  ...
  case DEM_CFG_DATA_FROM_IUMPR:
  case DEM_CFG_DATA_FROM_DTR:
  case DEM_CFG_DATA_FROM_MONITOR_ACTIVITY_DATA:
...
#endif /* (DEM_CFG_SUPPORT_SRECS == STD_ON) || (DEM_CFG_SUPPORT_GLOBAL_SRECS == STD_ON)|| (DEM_CFG_SUPPORT_OBDII_OR_OBDONUDS == STD_ON) || (DEM_CFG_SUPPORT_WWHOBD == STD_ON) || (DEM_FEATURE_NEED_TIME_SERIES == STD_ON) */
...
{
  if (Dem_Data_CheckAvailableSizeDestinationBuffer(DestinationBuffer, DataSize) == E_OK)                                         /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  {
    ...
    Dem_MemSet(lDestinationBufferWritePtr,                                                                                       /* PRQA S 0315, 0602 */ /* MD_MSR_VStdLibCopy, MD_DEM_20.2 */  /* SBSW_DEM_MEMSET_DESTINATIONBUFFER_INVALID_DATAPATTERN */
                (uint8)DEM_DATAREPORT_INVALID_DATA_PATTERN,
                DataSize);

    Dem_Data_IncrementWriteIndexDestinationBuffer(DestinationBuffer, DataSize);                                                  /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  }
 ...
 */
DEM_LOCAL FUNC(boolean, DEM_CODE)
Dem_DataReport_CopyDid_NoInternalData(                                                                                           /* PRQA S 3219 */ /* MD_DEM_14.1_CONFIGURATION_SWITCH */
  CONST(Dem_DataReport_DidCopyInfoBasePtrType, AUTOMATIC)  DidCopyInfoBasePtr,
  CONST(Dem_DataReport_DidSourceBufferReadInfoPtrType, AUTOMATIC) DidSourceBufferReadInfoPtr ,
  CONST(Dem_Cfg_DidIndexType, AUTOMATIC)  DidIndex
  )
...
)
{
  ...
  Dem_TimeSeriesEntry_PtrToConstBaseType lTimeSeriesEntryPtr;
  ...
  Dem_Memory_TimeSeries_PowertrainTimeSeriesInfoType lTimeSeriesInfo = {0};
  ...
  lTimeSeriesEntryPtr = Dem_MemoryEntry_TimeSeriesEntry_GetBaseEntryPtrSafe(Dem_DTCReadoutBuffer_GetTimeSeriesEntryIndex(ReadoutBufferId));

  lDidCount = 0;
  lDidSourceBufferIndex = 0x00u;

  /* Get meta information about the time series record */
  Dem_Memories_TimeSeriesMemory_GetTimeSeriesPowertrainSequentialInfo(lTimeSeriesEntryIndex, lEventId, &lTimeSeriesInfo);        /* SBSW_DEM_POINTER_FORWARD_STACK */
  ...
  Dem_DataReport_WriteTimeSeriesPowertrainSequentialHeader(&lSourceHeaderBuffer,                                                 /* SBSW_DEM_POINTER_LOCAL_DESTINATIONBUFFER */ /* SBSW_DEM_POINTER_FORWARD_STACK */
    lTimeSeriesEntryPtr, &lTimeSeriesInfo);

  for (Dem_Memories_TimeSeriesMemory_EventDidIter_Init(lEventId, &lDidIter);                                                     /* SBSW_DEM_CALL_ITERATOR_POINTER */
        Dem_Memories_TimeSeriesMemory_EventDidIter_Exists(&lDidIter) == TRUE;                                                    /* PRQA S 2994 */ /* MD_DEM_2994 */ /* SBSW_DEM_CALL_ITERATOR_POINTER */
        Dem_Memories_TimeSeriesMemory_EventDidIter_Next(&lDidIter))                                                              /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    Dem_Cfg_DidIndexType lDidIndex;
    uint16 lDidSize;
    uint16 lSequentialDidId;
  
    lDidIndex = Dem_Memories_TimeSeriesMemory_EventDidIter_Get(&lDidIter);                                                       /* SBSW_DEM_CALL_ITERATOR_POINTER */
    lDidSize  = Dem_StorageManager_DataStorage_GetDidNVSize(lDidIndex);

    lSequentialDidId = lTimeSeriesInfo.StartingSequentialDIDIdentifier + lDidCount;
    Dem_Data_WriteDestinationBufferUint16(DestinationBuffer, lSequentialDidId);                                                  /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */

    Dem_Data_WriteDestinationBuffer(DestinationBuffer, lSourceHeaderBuffer.Buffer, lSourceHeaderBuffer.BufferSize);              /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */ /* SBSW_DEM_POINTER_DESTINATIONBUFFER_SOURCEBUFFER */

    Dem_Internal_AssertContinue(lDidSourceBufferIndex < Dem_Cfg_GetMaxSnapShotSizeOfTimeSeriesEntryTable(lTimeSeriesEntryIndex), DEM_E_INCONSISTENT_STATE);
    Dem_DataReport_WriteTimeSeriesPowertrainSequentialDid(DestinationBuffer,                                                     /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */ /* SBSW_DEM_POINTER_FORWARD_STACK */
      lTimeSeriesEntryIndex, &lTimeSeriesInfo, lDidIndex, lDidSourceBufferIndex);
    lDidCount = lDidCount + 1u;
    lDidSourceBufferIndex += lDidSize;
  }

  *DidCount = lDidCount;                                                                                                         /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
...
)
{
  ...
  Dem_Data_WriteDestinationBufferUint16(DestinationHeaderBuffer, Dem_MemoryEntry_TimeSeriesEntry_GetTimeSinceLastSample(SourceEntry)); /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */ /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
}
#endif
...
)
{
  ...
  lDidSize = Dem_StorageManager_DataStorage_GetDidNVSize(DidIndex);
  ...
  for (lSampleIndex = 0x00u; lSampleIndex < TimeSeriesInfo->NumberOfConfiguredSamples; lSampleIndex++)                           /* PRQA S 2994 */ /* MD_DEM_2994 */
  {
    ...
    else
    {
      ...
      lSourceBuffer = Dem_MemoryEntry_TimeSeriesEntry_GetSnapshotDataPtr(TimeSeriesEntryIndex,  0, 0, lSampleIndex);
...
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_DataReport_InitDidCopyInfo(
  Dem_DataReport_DidCopyInfoPtrType DidCopyInfoPtr,
  ...
  )
{
  ...
  DidCopyInfoPtr->EventEntryIndex = EventEntryIndex;                                                                             /* SBSW_DEM_CALL_DIDCOPYINFO_INIT */
#if (DEM_CFG_SUPPORT_TYPE2_COMBINATION == STD_OFF)
  DidCopyInfoPtr->ReadoutBufferId = ReadoutBufferId;                                                                             /* SBSW_DEM_CALL_DIDCOPYINFO_INIT */
#else
  DidCopyInfoPtr->ReadoutBufferId = DEM_CFG_READOUTBUFFER_INVALID;                                                               /* SBSW_DEM_CALL_DIDCOPYINFO_INIT */
...
Dem_DataReport_SRecCopyData(
  ...
  Dem_EventIdType EventId,
  Dem_Cfg_EntryIndexType ReadoutBufferEventEntryIndex,
  Dem_DTCReadoutBuffer_HandleType ReadoutBufferId
)
{
...
# if ((DEM_FEATURE_NEED_TIME_SERIES == STD_ON) || (DEM_FEATURE_NEED_CUSTOMTRIGGERSTORAGE == STD_ON))
  /* Time series data or custom trigger data can be stored even if the DTC is not stored into event memory entry.
     The internal data elements are padded since they are't stored in event entry */
  boolean lStoredInEventEntry = TRUE;
  if((Dem_MemoryEntry_EventEntry_GetEventId(ReadoutBufferEventEntryIndex) == DEM_EVENT_INVALID)
  || (Dem_MemoryEntry_EventEntry_TestAgingOnlyEntry(ReadoutBufferEventEntryIndex) == TRUE))                                      /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
  {    
    lStoredInEventEntry = FALSE;
  }
# endif
  ...
  Dem_DataReport_InitDidCopyInfo(&lDidCopyInfo, DestinationBuffer, EventId , ReadoutBufferEventEntryIndex, ReadoutBufferId);     /* SBSW_DEM_CALL_DIDCOPYINFO_INIT */
...
# if (DEM_CFG_SUPPORT_DID_LENGTH_FOR_SVC19_04 == STD_ON)
...
# endif
  ...
  Dem_Data_WriteDestinationBufferAtIndex(lDidCopyInfo.Base.DestinationBuffer, lDidCountWriteIndex, Dem_GetLoByte(lDidCount));    /* SBSW_DEM_POINTER_LOCAL_COPYDID_DESTINATIONBUFFER */
}                                                                                                                                /* PRQA S 6050 */ /* MD_MSR_STCAL */
  ...
  )
{
  ...
  Dem_DataReport_InitDidCopyInfo(&lDidCopyInfo, DestinationBuffer, lEventId, EventEntryIndex, ReadoutBufferId);                  /* SBSW_DEM_CALL_DIDCOPYINFO_INIT */
  Dem_DataReport_InitDidSourceBufferReadInfo(&lDidSourceBufferReadInfo, SourceBuffer);                                           /* SBSW_DEM_CALL_DIDSOURCEBUFFERREADINFO_INIT */
  ...
  )
{
  ...
  Dem_DataReport_InitDidCopyInfo(&lDidCopyInfo, DestinationBuffer, EventId, EventEntryIndex, ReadoutBufferId);                   /* SBSW_DEM_CALL_DIDCOPYINFO_INIT */
  Dem_DataReport_InitDidSourceBufferReadInfo(&lDidSourceBufferReadInfo, SourceBuffer);                                           /* SBSW_DEM_CALL_DIDSOURCEBUFFERREADINFO_INIT */
...
/* ****************************************************************************
 % Dem_RecordReader_DataReport_ObdOnUdsFreezeFrameCopyData
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
  ...
  )
{
  ...
  lPtrToSource = (Dem_ConstSharedDataPtrType)Dem_RecordReader_DTCReadoutBuffer_GetPtrToObdOnUdsFreezeFrameBuffer(ReadoutBufferId, FreezeFrameType);
      ...
      { /* look for the requested data element of the PID */
        ...
        if (DataElementIndex == lDataElementIndex)
        {
          ...
          if (lElementSize <= *BufSize)
          {
            ...
            lObdFreezeFrameDataPtr = Dem_MemoryEntry_ObdFreezeFrameEntry_GetObdIIDataPtr(FreezeFrameIndex);

            Dem_MemCpy(Buffer, &lObdFreezeFrameDataPtr[lSrcBufferOffset], lElementSize);                                         /* PRQA S 0311, 0315, 0602 */ /* MD_DEM_11.8, MD_MSR_VStdLibCopy, MD_DEM_20.2 */ /* SBSW_DEM_POINTER_WRITE_ARGUMENT_BUFFER */
...
# if(DEM_CFG_SUPPORT_OBDII_FREEZEFRAME_IN_SVC19 == STD_ON)
  ...
  {
    lPtrToSource = (Dem_ConstSharedDataPtrType)Dem_DTCReadoutBuffer_GetPtrToOBDIIFreezeFrameBuffer(BufferIdx.ReadoutBufferId);
  }
  else
# endif
  {
    lPtrToSource = (Dem_ConstSharedDataPtrType)Dem_MemoryEntry_ObdFreezeFrameEntry_GetObdIIDataPtr(BufferIdx.FreezeFrameIndex);
  }
...
/* ****************************************************************************
 % Dem_RecordReader_DataReport_CopyJ1939ObdFreezeFrameData
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_RecordReader_DataReport_CopyJ1939ObdFreezeFrameData(
  Dem_DataReport_BufferIndexType  BufferIdx,
  ...
  )
{
  ...
  Dem_ConstSharedDataPtrType lPtrToSource = (Dem_ConstSharedDataPtrType)
                                              Dem_MemoryEntry_ObdFreezeFrameEntry_GetObdIIDataPtr(
                                              BufferIdx.FreezeFrameIndex);
  ...
  for (Dem_Cfg_GlobalPidIterInit(&lPidIter);                                                                                     /* SBSW_DEM_CALL_ITERATOR_POINTER */
        ...
        Dem_Cfg_GlobalPidIterNext(&lPidIter))                                                                                    /* SBSW_DEM_CALL_ITERATOR_POINTER */
  { 
    Dem_Cfg_DidIndexType lDidIndex = Dem_Cfg_GlobalPidIterGet(&lPidIter);                                                        /* SBSW_DEM_CALL_ITERATOR_POINTER */

    /* Copy the PID data into the destination buffer */
    (void)Dem_DataReport_CopyDid(&lDidCopyInfo, &lDidSourceBufferReadInfo, lDidIndex);                                           /* SBSW_DEM_POINTER_LOCAL_DIDCOPYINFO */
  }
#else
  DEM_IGNORE_UNUSED_CONST_ARGUMENT(BufferIdx)                                                                                    /* PRQA S 3112 */ /* MD_DEM_14.2 */
...
# if (DEM_CFG_SUPPORT_USER_ERECS == STD_ON)
  ...
  {
    lSourceBuffer = Dem_MemoryEntry_ExtendedEntry_IteratorGetExtendedRecordDataConstPtr(DidCopyInfoPtr->EventEntryIndex, ExtendedEntryIndex);
  }
  ...
  )
{
    ...
    Dem_Cfg_EventObdDidIterNext(&lDidIter))                                                                                      /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    {
      if (Dem_Cfg_DidSize(lDidIndex) <= Dem_Data_GetRemainingSizeOfDestinationBuffer(lDidCopyInfo.Base.DestinationBuffer))       /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
      {
        (void)Dem_DataReport_CopyDid(&lDidCopyInfo, &lDidSourceBufferReadInfo, lDidIndex);                                       /* SBSW_DEM_POINTER_LOCAL_DIDCOPYINFO */
        lReturnValue = E_OK;
      }
...
/* ****************************************************************************
 % Dem_RecordReader_DataReport_CopyDidFromObdOnUdsFreezeFrame
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
)
{
  ...
  for (Dem_Cfg_EventObdDidIterInit(lDidCopyInfo.Base.EventId, &lDidIter);                                                        /* SBSW_DEM_CALL_ITERATOR_POINTER */
       Dem_Cfg_EventObdDidIterExists(&lDidIter) == TRUE;                                                                         /* SBSW_DEM_CALL_ITERATOR_POINTER */
       Dem_Cfg_EventObdDidIterNext(&lDidIter))                                                                                   /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    Dem_Cfg_DidIndexType lDidIndex;
    ...
    { /* skip to next since not the requested DID */
      Dem_DataReport_SkipDid(&lDidSourceBufferReadInfo, lDidIndex);                                                              /* SBSW_DEM_POINTER_LOCAL_DIDCOPYINFO */
    }
  }
...
#if (((DEM_FEATURE_NEED_TIME_SERIES == STD_ON) && (DEM_CFG_SUPPORT_POWERTRAIN == STD_OFF)) || (DEM_FEATURE_NEED_CUSTOMTRIGGERSTORAGE == STD_ON))
/* ****************************************************************************
 % Dem_DataReport_CopyDidForSRecMayBeWithoutEventStorage
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
  ...
  )
{
  ...
  Dem_DataReport_InitDidCopyInfo(&lDidCopyInfo                                                                                   /* SBSW_DEM_CALL_DIDCOPYINFO_INIT */
                                 , DestinationBuffer
                                 , Dem_MemoryEntry_EventEntry_GetEventId(EventEntryIndex)
                                 , EventEntryIndex
                                 , DEM_CFG_READOUTBUFFER_INVALID);
  Dem_DataReport_InitDidSourceBufferReadInfo(&lDidSourceBufferReadInfo, SourceBuffer);                                           /* SBSW_DEM_CALL_DIDSOURCEBUFFERREADINFO_INIT */
       ...
       Dem_Cfg_EventDidIterNext(&lDidIter))                                                                                      /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    if (Dem_Cfg_DidNumber(lDidIndex) == DataId)
    {
      uint16 DataSize;
      DataSize = Dem_Cfg_DidSize(lDidIndex);
      if (DataSize <= Dem_Data_GetRemainingSizeOfDestinationBuffer(lDidCopyInfo.Base.DestinationBuffer))                         /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
      {
        (void)Dem_DataReport_CopyDid(&lDidCopyInfo, &lDidSourceBufferReadInfo, lDidIndex);                                       /* SBSW_DEM_POINTER_LOCAL_DIDCOPYINFO */
        lReturnValue = E_OK;
      }
