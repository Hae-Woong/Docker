 ...
 *  \details    Encapsulates the list of extended data record buffers stored in the event entry
 *  \entity     ExtendedEntry
 *********************************************************************************************************************/
...
Dem_MemoryEntry_ExtendedEntry_ERecIsSupportedByAvailableCombinedEvent(
 ...
 */
DEM_LOCAL FUNC(boolean, DEM_CODE)
Dem_MemoryEntry_ExtendedEntry_TestDTCHasERecConfigured(                                                                          /* PRQA S 3219 */ /* MD_DEM_14.1_CONFIGURATION_SWITCH */
  CONST(uint8, AUTOMATIC)  ERecId,
  CONST(Dem_Cfg_CombinedGroupIndexType, AUTOMATIC)  CombinedDTCGroup,
  ...
  )
{
       ...
       Dem_Cfg_DTCExtendedDataRecordIterNext(&lDTCExtendedDataRecordIter))                                                       /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    {
      if (Dem_MemoryEntry_ExtendedEntry_ERecIsSupportedByAvailableCombinedEvent(CombinedDTCGroup, ERecId) == TRUE)
      {
...
Dem_MemoryEntry_ExtendedEntry_IterInit(                                                                                          /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
 ...
 */
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_MemoryEntry_ExtendedEntry_IterNext(
  CONSTP2VAR(Dem_ExtendedEntry_IterType, AUTOMATIC, AUTOMATIC)  IterPtr
  )
{
...
Dem_MemoryEntry_ExtendedEntry_IterExists(
...
Dem_MemoryEntry_ExtendedEntry_IterGet(
  CONSTP2CONST(Dem_ExtendedEntry_IterType, AUTOMATIC, AUTOMATIC)  IterPtr
  )
{
...
Dem_MemoryEntry_ExtendedEntry_IteratorGetExtendedRecordDataConstPtr(
  ...
  )
{
  ...
  return Dem_MemoryEntry_EventEntry_GetExtendedRecordDataConstPtr(EventEntryIndex, ExtendedEntryIndex);
}
 ...
 */
DEM_LOCAL_INLINE FUNC(boolean, DEM_CODE)
Dem_MemoryEntry_ExtendedEntry_IteratorIsExtendedRecordStored(
  Dem_Cfg_EntryIndexType EventEntryIndex,
  CONST(uint8, AUTOMATIC)  ExtendedEntryIndex
  )
{
  ...
  if (EventEntryIndex != DEM_CFG_ENTRYINDEX_INVALID)
  {
    ...
    if ((Dem_MemoryEntry_EventEntry_GetExtendedHeader(EventEntryIndex) & lMask) != 0u)
    {
 ...
 */
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_MemoryEntry_ExtendedEntry_IteratorUpdateExtendedEntry(
  Dem_Cfg_EntryIndexType EventEntryIndex,
  CONST(uint8, AUTOMATIC)  ExtendedEntryIndex
  )
{
  if (EventEntryIndex != DEM_CFG_ENTRYINDEX_INVALID)
  {
    ...
    lMask = ((uint8)1 << ExtendedEntryIndex);
    Dem_MemoryEntry_EventEntry_SetExtendedHeader(EventEntryIndex, (Dem_MemoryEntry_EventEntry_GetExtendedHeader(EventEntryIndex) | lMask));
  }
  else
  {
    Dem_Internal_AssertAlways(DEM_E_INCONSISTENT_STATE);
 ...
 */
DEM_LOCAL FUNC(boolean, DEM_CODE)
Dem_MemoryEntry_ExtendedEntry_GetUserERecStorageStatus(                                                                          /* PRQA S 3219 */ /* MD_DEM_14.1_CONFIGURATION_SWITCH */
  ...
  )
{
  ...
  if (UserERecStorageIndex < DEM_EXTENDEDENTRY_USEREREC_STORAGEINDEX_INVALID)
  {
    ...
    if ((Dem_MemoryEntry_EventEntry_GetCombinedDTCExtendedHeader(EventEntryIndex) & lMask) != 0u)
    {
  ...
  )
{
  if (EventEntryIndex != DEM_CFG_ENTRYINDEX_INVALID)
  {
    ...
    {
      lMask = (uint32)1 << UserERecStorageIndex;
      Dem_MemoryEntry_EventEntry_SetCombinedDTCExtendedHeader(EventEntryIndex,
        (Dem_MemoryEntry_EventEntry_GetCombinedDTCExtendedHeader(EventEntryIndex) | lMask));
    }
    else
    {
      Dem_Internal_AssertAlways(DEM_E_INCONSISTENT_STATE);
