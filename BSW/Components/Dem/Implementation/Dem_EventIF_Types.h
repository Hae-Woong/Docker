...
/*! Values used for the event availability bit in event availability data array */
#define DEM_EVENT_AVAILABILITYDATA_AVAILABLE            (0u)
#define DEM_EVENT_AVAILABILITYDATA_UNAVAILABLE          (1u)
...
struct Dem_Event_Context_s
{
  ...
  Dem_Cfg_CombinedGroupIndexType GroupIndex; /*!< [in] Combined group index of event, if available */
  Dem_UDSStatus_StatusType OldEventStatus;  /*!< [in] event status before processing */
  Dem_UDSStatus_StatusType OldExternalEventStatus;  /*!< [in] external event status before processing */
  Dem_UDSStatus_StatusType NewEventStatus;  /*!< [in,out] event status after processing */
  Dem_Event_InternalStatusType QualifyStatus;  /*!< [in,out] event qualify status */
  Dem_Cfg_StorageTriggerType EventEffects; /*!< [in,out] status change triggers resulting from event status changes */
  boolean DataModified;  /*!< [in,out] processing has modified environmental data */
  boolean QualifyEvent;  /*!< [in,out] Qualify single event without driving cycle qualification, due to OBD-II FF visibility */
};
