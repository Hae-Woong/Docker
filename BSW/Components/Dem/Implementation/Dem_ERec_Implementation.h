 ...
 *********************************************************************************************************************/
/*! Dem_Dcm_ERecExtraSizeType is used to define whether of not the output of a specific service includes an ERecId. */
typedef enum
{
  OutputWithoutERecId = 0,    /*!< Output does not contain an ERecId */
  OutputContainsERecId = 1    /*!< Output contains an ERecId */
} Dem_Dcm_ERecExtraSizeType;
 ...
 *                Feature Service 0x19 16 is enabled and extended data records are
 *                configured.
 *
 * \config        -
 *
 ...
 *
 * \pre           IterPtr has been initialized
 *                Feature Extended Data Record and Service 0x19 16 is enabled
 * \config        -
 * \context       TASK|ISR2
 * \synchronous   TRUE
 * \reentrant     TRUE
 ...
 * \param[out]    SizeOfFilteredRecords
 *                Pointer to receive the size of all filtered records.
 *
 * \pre           Feature Service 0x19 16 is enabled and extended data records are
 *                configured.
 *
...
Dem_Dcm_ERec_0x1916_CalcNumberofGlobalERecs(
  ...
  P2VAR(uint16, AUTOMATIC, DEM_DCM_DATA)  SizeOfFilteredRecords
);
...
/* ****************************************************************************
 * Dem_Dcm_ERec_CopyERec_FromSubEventEntry
 *****************************************************************************/
/*!
 * \brief         Copies the requested extended data record from a selected
 *                event entry to the destination buffer.
 *
 * \details       Copies the requested extended data record from a selected
 *                event entry to the destination buffer. Subevent entry
 ...
 *                  extended data records from a subevent entry
 ...
 *
 * \return        The number of bytes of the given ERec
 *
 * \pre           -
 * \config        DEM_CFG_SUPPORT_DCM == STD_ON &&
 *                DEM_CFG_SUPPORT_ERECS == STD_ON
...
/* ****************************************************************************
 * Dem_Dcm_ERec_FitsIntoBuffer
 *****************************************************************************/
/*!
 * \brief         Check whether or not the given ERec of the given event fits
 *                into the given destination buffer.
 *
 * \details       Check whether or not the given ERec of the given event fits
 *                into the given destination buffer.
 *
 * \param[in]     EventId
 ...
 *                The buffer to be checked for sufficient size.
 * \param[in]     Dem_Dcm_ERecSizeForERecId
 *                Extra size (in bytes), needed store the ERecId:
 *                  - OutputWithoutERecId  (0): No ERecId contained in output
 *                  - OutputContainsERecId (1): ERecId contained in output
 *
 * \return        TRUE ERec fits into given buffer.
 *
 * \return        FALSE ERec does not fit into given buffer.
 *
 * \pre           -
 * \config        DEM_CFG_SUPPORT_DCM == STD_ON &&
 *                DEM_CFG_SUPPORT_ERECS == STD_ON
...
CONST(Dem_Dcm_ERecExtraSizeType, AUTOMATIC) Dem_Dcm_ERecSizeForERecId
);
...
) 
{
  ...
  ExtendedDataFilterPtr->StoredEventEntryIndex = DEM_CFG_ENTRYINDEX_INVALID;                                                     /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
}
...
)
{
  ...
  Dem_Cfg_ComplexIter32BitInit(&lPrimaryEventEntryIter,                                                                          /* SBSW_DEM_CALL_ITERATOR_POINTER */
  ...
  ExtendedDataFilterPtr->StoredEventEntryIndex = Dem_Cfg_ComplexIter32BitGet(&lPrimaryEventEntryIter);                           /* SBSW_DEM_POINTER_WRITE_ARGUMENT */ /* SBSW_DEM_CALL_ITERATOR_POINTER */

  Dem_Dcm_ERec_ERecIter_Invalidate(&(ExtendedDataFilterPtr->ERecIterator));                                                      /* SBSW_DEM_POINTER_FORWARD_ARGUMENT_STRUCT_MEMBER */
...
)
{
  ...
  Dem_Cfg_DidIndexType lERecId;
  boolean lSizeOfERecIsCalculated;

  lERecId = Dem_Cfg_GetIdNumberOfDataCollectionTable(ERecDataCollectionTableIdx);
  lERecCount = 0u;
  *SizeOfFilteredRecords = 0u;                                                                                                   /* SBSW_DEM_POINTER_WRITE_ARGUMENT */

  /* In order to prevent code duplication, we have a decision in the loop below which could be placed outside (see CCL_02).*/
  /* We use the variable lSizeOfERecIsCalculated in order to keep the consequences acceptable. */
  if ((Dem_Cfg_IsObdOnUdsSupportedInVariant()) && (lERecId == DEM_EREC_OBDONUDS_ERECNUM_DTR))
  {
    lSizeOfERecIsCalculated = TRUE;
  }
  else
  {
    lSizeOfERecIsCalculated = FALSE;
  }
       ...
       Dem_Dcm_ERec_ERecIter_Next(&lERecIter))                                                                                   /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    if ((lEventId == Dem_Cfg_GetMasterEvent(lEventId))
      ...
      && (Dem_Core_Event_GetDestination(lEventId) == DEM_CFG_MEMORYID_PRIMARY)
      && (Dem_DTC_TestSuppressionOnDtcRetrieval(lEventId) == FALSE))                                                             /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
    {
      lERecCount++;
      if (lSizeOfERecIsCalculated == TRUE)
      {
        (*SizeOfFilteredRecords) += Dem_Dtr_GetERec0x92SizeOfEvent(lEventId);                                                    /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
      }
    }
  }

  *NumberOfFilteredRecords = lERecCount;                                                                                         /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
  if (lSizeOfERecIsCalculated == FALSE)
  {
    *SizeOfFilteredRecords = ((*NumberOfFilteredRecords) * Dem_Cfg_DidSize(ERecDataCollectionTableIdx));                         /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
  }
...
)
{
  ...
  {
    lEventEntryIndex = Dem_Cfg_ComplexIter32BitGet(&lPrimaryEventEntryIter);                                                     /* SBSW_DEM_CALL_ITERATOR_POINTER */
    ...
    if ( (lEventId != DEM_EVENT_INVALID)
      ...
      && (Dem_MemoryEntry_EventEntry_TestAgingOnlyEntry(lEventEntryIndex) == FALSE))                                             /* PRQA S 3415 */ /* MD_DEM_13.5_volatile */
    {
      ...
      for (Dem_MemoryEntry_ExtendedEntry_IterInit(lEventId, &lExtendedEntryIter);                                                /* SBSW_DEM_CALL_ITERATOR_POINTER */
           Dem_MemoryEntry_ExtendedEntry_IterExists(&lExtendedEntryIter) == TRUE;                                                /* PRQA S 2994 */ /* MD_DEM_2994 */ /* SBSW_DEM_CALL_ITERATOR_POINTER */
           Dem_MemoryEntry_ExtendedEntry_IterNext(&lExtendedEntryIter))                                                          /* SBSW_DEM_CALL_ITERATOR_POINTER */
      {
        if (ERecDataCollectionTableIdx == Dem_MemoryEntry_ExtendedEntry_IterGet(&lExtendedEntryIter))                            /* SBSW_DEM_CALL_ITERATOR_POINTER */
        {
          if ((Dem_Cfg_ERecType(Dem_MemoryEntry_ExtendedEntry_IterGet(&lExtendedEntryIter)) != DEM_CFG_EREC_TYPE_USER)           /* SBSW_DEM_CALL_ITERATOR_POINTER */
            || (Dem_MemoryEntry_ExtendedEntry_IteratorIsExtendedRecordStored(lEventEntryIndex, lExtendedEntryIndex) == TRUE))    /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
          {
            lERecCount++;
          }
          break;
    ...
    if ( (lEventId == Dem_Cfg_GetMasterEvent(lEventId))
      ...
      && (Dem_DTC_TestSuppressionOnDtcRetrieval(lEventId) == FALSE))                                                             /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
      {
        ...
        lReturnValue = Dem_Dcm_ERec_0x1916_CopyERecs(lERecNumberFilter.DataCollectionTableIdx, 
          DEM_CFG_ENTRYINDEX_INVALID /* unused parameter*/, 0 /* unused parameter*/, lEventId, &lDestinationBuffer);             /* SBSW_DEM_POINTER_LOCAL_DESTINATIONBUFFER */
...
)
{
  Std_ReturnType lReturnValue;
  Dem_Cfg_EntryIterType lPrimaryEventEntryIter;
  Dem_Cfg_EntryIndexType lStoredEntryIndex;
  Dem_Cfg_ExtendedDataRecordIterator_FilterType lERecNumberFilter;

  lReturnValue = DEM_NO_SUCH_ELEMENT;

  lERecNumberFilter = Dem_Dcm_ERec_0x1916_GetRecordNumberFilter(ExtendedDataFilterId);
  lStoredEntryIndex = lERecNumberFilter.StoredEventEntryIndex;
  
  for (Dem_Cfg_ComplexIter32BitInit(&lPrimaryEventEntryIter, lStoredEntryIndex, Dem_FaultMemory_Memories_GetEndEntryIndex(DEM_CFG_MEMORYID_PRIMARY)); /* SBSW_DEM_CALL_ITERATOR_POINTER */
      (Dem_Cfg_ComplexIter32BitExists(&lPrimaryEventEntryIter) == TRUE);                                                         /* SBSW_DEM_CALL_ITERATOR_POINTER */
       Dem_Cfg_ComplexIter32BitNext(&lPrimaryEventEntryIter))                                                                    /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    Dem_Cfg_EntryIndexType lEntryIndex = Dem_Cfg_ComplexIter32BitGet(&lPrimaryEventEntryIter);                                   /* SBSW_DEM_CALL_ITERATOR_POINTER */
    Dem_EventIdType lEventId = Dem_MemoryEntry_EventEntry_GetEventId(lEntryIndex);
    
    if ( (lEventId != DEM_EVENT_INVALID)
      && (Dem_DTC_TestDtcSuppression(lEventId) == FALSE)                                                                         /* PRQA S 3415 */ /* MD_DEM_13.5_volatile */
      && (Dem_DTC_TestDtcAvailableInVariant(lEventId) == TRUE)
      && (Dem_MemoryEntry_EventEntry_TestAgingOnlyEntry(lEntryIndex) == FALSE))                                                  /* PRQA S 3415 */ /* MD_DEM_13.5_volatile */
    {
      Dem_ExtendedEntry_IterType lExtendedEntryIter;
      uint8 lExtendedEntryIndex;
      lExtendedEntryIndex = 0u; /*Iterator over 'user' extended data records in event entry*/
      ...
      for (Dem_MemoryEntry_ExtendedEntry_IterInit(lEventId, &lExtendedEntryIter);                                                /* SBSW_DEM_CALL_ITERATOR_POINTER */
           ...
           Dem_MemoryEntry_ExtendedEntry_IterNext(&lExtendedEntryIter))                                                          /* SBSW_DEM_CALL_ITERATOR_POINTER */
      {
        if (lERecNumberFilter.DataCollectionTableIdx == Dem_MemoryEntry_ExtendedEntry_IterGet(&lExtendedEntryIter))              /* SBSW_DEM_CALL_ITERATOR_POINTER */
        {
          if ( (Dem_Cfg_ERecType(Dem_MemoryEntry_ExtendedEntry_IterGet(&lExtendedEntryIter)) != DEM_CFG_EREC_TYPE_USER)          /* SBSW_DEM_CALL_ITERATOR_POINTER */
           || (Dem_MemoryEntry_ExtendedEntry_IteratorIsExtendedRecordStored(lEntryIndex, lExtendedEntryIndex) == TRUE))          /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
          {
            /* Record is stored */
            ...
            lReturnValue = Dem_Dcm_ERec_0x1916_CopyERecs(lERecNumberFilter.DataCollectionTableIdx,
              lEntryIndex, lExtendedEntryIndex, lEventId, &lDestinationBuffer);                                                  /* SBSW_DEM_POINTER_LOCAL_DESTINATIONBUFFER */

            if (lReturnValue == E_OK)
            {
              *EventId = lEventId;                                                                                               /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
              *BufSize = lDestinationBuffer.WriteIndex;                                                                          /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
              Dem_Cfg_ComplexIter32BitNext(&lPrimaryEventEntryIter);                                                             /* PRQA S 2469 */ /* MD_DEM_14.2_LoopControlHandledInternally */ /* SBSW_DEM_CALL_ITERATOR_POINTER */
            }
          }
...
# endif
  ...
  lERecNumberFilter.StoredEventEntryIndex = Dem_Cfg_ComplexIter32BitGet(&lPrimaryEventEntryIter);                                /* SBSW_DEM_CALL_ITERATOR_POINTER */
  Dem_Dcm_ERec_0x1916_SetRecordNumberFilter(ExtendedDataFilterId, &lERecNumberFilter);                                           /* SBSW_DEM_POINTER_FORWARD_STACK */
  return lReturnValue;
}                                                                                                                                /* PRQA S 6050, 6080 */ /* MD_MSR_STCAL, MD_MSR_STMIF */
...
Dem_Dcm_ERec_0x1916_CopyERecs(
  Dem_Cfg_ERecIndexType ERecDataCollectionTableIdx,
  Dem_Cfg_EntryIndexType EventEntryIndex,                                                                                        /* PRQA S 3206 */ /* MD_DEM_3206 */
  uint8 ExtendedEntryIndex,
  Dem_EventIdType EventId,
  CONST(Dem_Data_DestinationBufferPtrType, AUTOMATIC)  DestinationBuffer
  )
{
  ...
  if (Dem_Dcm_ERec_FitsIntoBuffer(EventId, ERecDataCollectionTableIdx, DestinationBuffer, OutputWithoutERecId) != TRUE)          /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  {
  ...
  else
  {
    do
    {
      ...
      Dem_Internal_AssertReturnValue((Dem_Cfg_ERecType(ERecDataCollectionTableIdx) == DEM_CFG_EREC_TYPE_GLOBAL) || (EventEntryIndex != DEM_CFG_ENTRYINDEX_INVALID), DEM_E_INCONSISTENT_STATE, E_NOT_OK)
      ...
      if (Dem_Event_TestDataUpdateInProgress(lUpdateState) == FALSE)
      {
        ...
        Dem_DataReport_InitDidCopyInfo(&lDidCopyInfo, DestinationBuffer, EventId, EventEntryIndex, DEM_CFG_READOUTBUFFER_INVALID); /* SBSW_DEM_CALL_DIDCOPYINFO_INIT */
...
{
  Dem_ExtendedEntry_IterType lExtendedEntryIter;
  boolean lERecProcessed = FALSE;
  uint8 lExtendedEntryIndex = Dem_DTCReadoutBuffer_GetExtendedEntryIndex(
                          ReadoutBufferId,
  ...
  while (Dem_MemoryEntry_ExtendedEntry_IterExists(&lExtendedEntryIter) == TRUE)                                                  /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    if (ERecId == (uint8)Dem_Cfg_DidNumber(Dem_Cfg_ERecDid(lCfgERecIndex)))
    {
      ...
      lEventEntryIndex = Dem_DTCReadoutBuffer_GetEventEntryIndex(ReadoutBufferId, ReadoutBufferDataIndex);

      if ( ((Dem_Cfg_ERecType(lCfgERecIndex) == DEM_CFG_EREC_TYPE_INTERNAL)
        && (Dem_RecordReader_ERec_IsInternalERecReadable(lCfgERecIndex, lEventEntryIndex) == TRUE))                              /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
          || (Dem_MemoryEntry_ExtendedEntry_IteratorIsExtendedRecordStored(lEventEntryIndex, lExtendedEntryIndex) == TRUE)       /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
         )
      {
        Dem_DataReport_DidCopyInfoType lDidCopyInfo;
...
# endif
    if (lERecProcessed == TRUE)
    {
      break;
    }
  }

  Dem_Internal_AssertContinue((lERecProcessed == TRUE), DEM_E_INCONSISTENT_STATE)
  ...
  )
{
  ...
  if (Dem_Data_GetRemainingSizeOfDestinationBuffer(DestinationBuffer)                                                            /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
       < Dem_Dcm_ERec_CalculateSizeSingleERec_CombinedDTC(
  ...
  else
  {
    ...
    /* Return E_OK wherein:
      - Internal ERec has no event entry
    ...
    switch (Dem_Cfg_ERecType(ERecEntryIndex))
    {
      ...
      case DEM_CFG_EREC_TYPE_INTERNAL:
      {
             ...
             Dem_Cfg_ExtendedDataRecordEventIterNext(&lExtendedDataRecordEventIter))                                             /* SBSW_DEM_CALL_ITERATOR_POINTER */
        {
          ...
          /* Event has an entry, try to copy content */
          if (lReadoutBufferEventDataIndex != Dem_Cfg_SizeOfReadOutBufferData())
          {
            Dem_Dcm_ERec_CopyERec_FromSubEventEntry(ERecId,                                                                      /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
                                                          ReadoutBufferId,
...
# if (DEM_CFG_SUPPORT_USER_ERECS == STD_ON)
      case DEM_CFG_EREC_TYPE_USER:
      {
        ...
        lUserERecStorageIndex = Dem_MemoryEntry_ExtendedEntry_CalculateUserERecStorageIndex(ERecId, lCombinedDTCGroupId);
            ...
            lReadoutBufferEventDataIndex++)
        {
          ...
          lReadoutBufferEventEntryIndex = Dem_DTCReadoutBuffer_GetEventEntryIndex(ReadoutBufferId, lReadoutBufferEventDataIndex);
          if (Dem_MemoryEntry_ExtendedEntry_GetUserERecStorageStatus(lUserERecStorageIndex, lReadoutBufferEventEntryIndex) == TRUE)
          {
            Dem_Dcm_ERec_CopyERec_FromSubEventEntry(ERecId,                                                                      /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
                                                          ReadoutBufferId,
                                                          lReadoutBufferEventDataIndex,
...
# if (DEM_CFG_SUPPORT_ERECS == STD_ON)
  {
    ...
    {
      Dem_Cfg_ERecIndexType lCfgERecIndex;
      uint8 lCurrentRecordNumber;

      lCfgERecIndex = Dem_MemoryEntry_ExtendedEntry_IterGet(&lExtendedEntryIter);                                                /* SBSW_DEM_CALL_ITERATOR_POINTER */
      lCurrentRecordNumber = (uint8) Dem_Cfg_DidNumber(Dem_Cfg_ERecDid(lCfgERecIndex));

      /* If the extended data number matches */
      if (Dem_Dcm_ERec_TestRecordMatch(lReadoutBufferId, lCurrentRecordNumber) == TRUE)                                          /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
      {
...
# if (DEM_CFG_SUPPORT_ERECS == STD_ON)
  {
    ...
    if (lRecordNumber < 0xf0U)
    {
      ...
      lEventId = Dem_DTCReadoutBuffer_GetEventId(lReadoutBufferId, lReadoutBufferDataIndex);
      lReadoutBufferEventEntryIndex = Dem_DTCReadoutBuffer_GetEventEntryIndex(lReadoutBufferId, lReadoutBufferDataIndex);
...
# endif
    ...
    else
    {
      ...
      else if ( (Dem_Cfg_IsEmissionObdSupportedInVariant() == TRUE)
             && (lRecordNumber == 0xfeU))
      {
      ...
      if (lEventId != DEM_EVENT_INVALID)
      {
        Dem_Cfg_EntryIndexType lReadoutBufferEventEntryIndex = Dem_DTCReadoutBuffer_GetEventEntryIndex(lReadoutBufferId, lReadoutBufferDataIndex);
        uint8 lExtendedEntryIndex = 0u;
             ...
             Dem_MemoryEntry_ExtendedEntry_IterNext(&lExtendedEntryIter))                                                        /* SBSW_DEM_CALL_ITERATOR_POINTER */
        {
          ...
          if (lCurrentRecordNumber > lERecMinNumber)
...
# if (DEM_CFG_SUPPORT_ERECS == STD_ON)
  {
    ...
    { /* Process single record selection */
      if (Dem_MemoryEntry_ExtendedEntry_TestDTCHasERecConfigured(lSelectedRecordNumber, lCombinedDTCGroup, &lERecEntryIndex) == TRUE) /* SBSW_DEM_POINTER_FORWARD_STACK */
      {
...
# if (DEM_CFG_SUPPORT_TYPE2_COMBINATION == STD_ON)
    if (Dem_DTCReadoutBuffer_TestCombinedEventSelected(ReadoutBufferId) == TRUE)
    {
      ...
      lCombinedDTCGroupId = Dem_Cfg_EventCombinedGroup(Dem_DTCReadoutBuffer_GetMasterEventId(ReadoutBufferId));
      lMatch = Dem_MemoryEntry_ExtendedEntry_ERecIsSupportedByAvailableCombinedEvent(lCombinedDTCGroupId, RecordNumber);
    }
    else
# endif
    {
  ...
  if (Dem_MemoryEntry_ExtendedEntry_IterExists(&lExtendedEntryIter) == TRUE)                                                     /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    if ( (lERecType == DEM_CFG_EREC_TYPE_GLOBAL)
        || ( (Dem_DTCReadoutBuffer_TestStoredDataAvailable(ReadoutBufferId) == TRUE)                                                
             ...
             && (Dem_RecordReader_ERec_IsInternalERecReadable(lCfgERecIndex, lReadoutBufferEventEntryIndex) == TRUE))            /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
                || (Dem_MemoryEntry_ExtendedEntry_IteratorIsExtendedRecordStored(lReadoutBufferEventEntryIndex, lExtendedEntryIndex) == TRUE) /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
                )
           )
       )
    {
      if (Dem_Dcm_ERec_FitsIntoBuffer(lEventId, lCfgERecIndex, DestinationBuffer, OutputContainsERecId) == TRUE)                 /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
      {
        Dem_DataReport_DidCopyInfoType lDidCopyInfo;
        Dem_Data_WriteDestinationBufferUint8(DestinationBuffer, (uint8)Dem_Cfg_DidNumber(Dem_Cfg_ERecDid(lCfgERecIndex)));       /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  ...
  switch (Dem_Cfg_ERecType(ERecEntryIndex))
  {
    ...
    case DEM_CFG_EREC_TYPE_INTERNAL:
    {
      for (Dem_Cfg_ExtendedDataRecordEventIterInit(&lExtendedDataRecordEventIter, CombinedGroupId, ERecId);                      /* SBSW_DEM_CALL_ITERATOR_POINTER */
        ...
        Dem_Cfg_ExtendedDataRecordEventIterNext(&lExtendedDataRecordEventIter))                                                  /* SBSW_DEM_CALL_ITERATOR_POINTER */
      {
        Dem_EventIdType lEventId = Dem_Cfg_ExtendedDataRecordEventIterGet(&lExtendedDataRecordEventIter);                        /* SBSW_DEM_CALL_ITERATOR_POINTER */
        Dem_DTCReadoutBuffer_ReadoutBufferDataIndexType lReadoutBufferDataIndex = Dem_DTCReadoutBuffer_GetEventDataIndex(ReadoutBufferId, lEventId);
        ...
        /* Only count size of internal extended data records if the event has an event entry */
        if ((lReadoutBufferDataIndex != Dem_Cfg_SizeOfReadOutBufferData())
          && (Dem_RecordReader_ERec_IsInternalERecReadable(Dem_Cfg_ERecDid(ERecEntryIndex), lReadoutBufferEventEntryIndex) == TRUE) /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
           )
        { /* Add one byte for RecordId */
          lSizeOfExtRec += (uint16)(Dem_Cfg_DidSize(Dem_Cfg_ERecDid(ERecEntryIndex)) + 1u);
        }
...
# if (DEM_CFG_SUPPORT_USER_ERECS == STD_ON)
    case DEM_CFG_EREC_TYPE_USER:
    {
      ...
      lReadoutBufferDataIndex = DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT;
      lNumOccupiedReadoutBufferSlots = Dem_DTCReadoutBuffer_GetNumberOfOccupiedSlots(ReadoutBufferId);
      lUserERecStorageIndex = Dem_MemoryEntry_ExtendedEntry_CalculateUserERecStorageIndex(ERecId, CombinedGroupId);
  ...
  )
{
  ...
  /* Set the output size to default value 0 */
  lSizeOfExtRec = 0u;
  lERecType = Dem_Cfg_ERecType(CfgERecIndex);

  /* If the record is of storage type GLOBAL */
  if (lERecType != DEM_CFG_EREC_TYPE_GLOBAL)
  {
    /* Test if event has an entry */
    if (Dem_DTCReadoutBuffer_TestStoredDataAvailable(ReadoutBufferId) == TRUE)
    {
        if ( ((lERecType == DEM_CFG_EREC_TYPE_INTERNAL) 
          && (Dem_RecordReader_ERec_IsInternalERecReadable(Dem_Cfg_ERecDid(CfgERecIndex), EventEntryIndex) == TRUE))             /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
          || (Dem_MemoryEntry_ExtendedEntry_IteratorIsExtendedRecordStored(EventEntryIndex, ExtendedEntryIndex) == TRUE))        /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
      {
  ...
  { /* Currently statistic data is always available, add record id to data size */
    /* Unfortunately, ERec 0x92 in OBD on UDS configurations needs extra care as its size is DTC dependent */
    if ((Dem_Cfg_GetIdNumberOfDataCollectionTable(CfgERecIndex) != DEM_EREC_OBDONUDS_ERECNUM_DTR) ||
        (Dem_Cfg_IsObdOnUdsSupported() == FALSE))
    {
      /* Normal case, fixed size */
      lSizeOfExtRec = (uint16)(Dem_Cfg_DidSize(Dem_Cfg_ERecDid(CfgERecIndex)) + 1u);
    }
    else
    {
      /* 0x92 case, get the size from Dtr unit */
      lSizeOfExtRec = Dem_Dtr_GetERec0x92SizeOfEvent(
        Dem_DTCReadoutBuffer_GetEventId(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT)
        );
      lSizeOfExtRec++;/* One more byte to store the ERec number */
    }
  }
...
{
  boolean lFitsIntoBuffer;
  uint16 lSize;

  /* If OBD on UDS supported in variant and the ERec is 0x92 */
  if ((Dem_Cfg_IsObdOnUdsSupportedInVariant()) && (Dem_Cfg_GetIdNumberOfDataCollectionTable(CfgERecIndex) == DEM_EREC_OBDONUDS_ERECNUM_DTR))
  {
    /* Get the event dependent size of ERec */
    lSize = Dem_Dtr_GetERec0x92SizeOfEvent(EventId);
  }
  else
  {
    /* Get the event independent size of ERec */
    lSize = Dem_Cfg_DidSize(Dem_Cfg_ERecDid(CfgERecIndex));
  }
  /* If destination buffer size is big enough to store the ERec and extra size */
  if ((lSize + (uint8)Dem_Dcm_ERecSizeForERecId) <= Dem_Data_GetRemainingSizeOfDestinationBuffer(DestinationBuffer))             /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  {
    /* Set Return value TRUE */
    lFitsIntoBuffer = TRUE;
  }
  else
  {
    /* Set Return value FALSE*/
    lFitsIntoBuffer = FALSE;
  }
  return lFitsIntoBuffer;
}
...
/* ****************************************************************************
 % Dem_RecordReader_ERec_IsAgingThroughReallocationEnabled()
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
{
  return (boolean)(DEM_FEATURE_NEED_AGING_REALLOCATION == STD_ON);
}
...
)
{
  ...
  lTempRecordFilter.StoredEventEntryIndex = DEM_CFG_ENTRYINDEX_INVALID;
...
/* ****************************************************************************
 % Dem_Dcm_ERec_0x1916_CalcSizeOfERecs
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
)
{
  ...
  {
    Dem_Dcm_ERec_0x1916_CalcNumberofGlobalERecs(lDataCollectionTableIdx, NumberOfFilteredRecords, SizeOfRecords);                /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  }
  else
  {
    ...
    *SizeOfRecords = ((*NumberOfFilteredRecords) * Dem_Cfg_DidSize(lDataCollectionTableIdx));                                    /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
  }
}
...
)
{
  ...
  if(lReturnValue == E_OK)
  {
    ...
    *DTCStatus = Dem_DTC_GetExternalUDSStatus(lEventId) & Dem_Cfg_GlobalStatusMask();                                            /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
  }
  else
  {
...
/* ****************************************************************************
 % Dem_RecordReader_ERec_IsInternalERecReadable
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
Dem_RecordReader_ERec_IsInternalERecReadable(                                                                                    /* PRQA S 3219 */ /* MD_DEM_14.1_CONFIGURATION_SWITCH */
  Dem_Cfg_DidIndexType  ERecIndex,
  Dem_Cfg_EntryIndexType EventEntryIndex
)
{
  boolean lReturnValue = TRUE;

  if (Dem_RecordReader_ERec_IsAgingThroughReallocationEnabled() == TRUE)
  {
    if (DEM_EVENTENTRY_TEST_AGING_ONLY(Dem_MemoryEntry_EventEntry_GetState(EventEntryIndex)) == TRUE)
    {
      ...
      for (Dem_Cfg_DidDataIterInit(ERecIndex, &lDataIter);                                                                       /* SBSW_DEM_CALL_ITERATOR_POINTER */
      ...
      {
        Dem_Cfg_DataIndexType lDataIndex = Dem_Cfg_DidDataIterGet(&lDataIter);                                                   /* SBSW_DEM_CALL_ITERATOR_POINTER */
        uint8 lCallbackType = Dem_Cfg_DataCallbackType(lDataIndex);

        if ((lCallbackType != DEM_CFG_DATA_FROM_AGINGCTR)
            && (lCallbackType != DEM_CFG_DATA_FROM_AGINGCTR_INVERTED))
        {
          lReturnValue = FALSE;
          break;
        }
...
/* ****************************************************************************
 % Dem_RecordReader_ERec_IsERecNumberAllowedInVariant
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
)
{
  ...
  {
    if ( (RecordNumber >= DEM_DCM_EXTENDEDDATARECORD_FIRST_OBD)
    ...
    {
      /* UDS format must be used for non-OBD relevant ERecs */
 ...
 */
/* ********************************************************************************************************************
 *  END OF FILE: Dem_ERec_Implementation.h
 *********************************************************************************************************************/
