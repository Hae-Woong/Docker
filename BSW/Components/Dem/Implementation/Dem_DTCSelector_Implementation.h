...
Dem_DTCSelector_GetDTCFormat(
  Dem_DTCSelector_HandleType DTCSelectorId
  )
{
  Dem_DTCFormatType lReturnValue = DEM_DTC_FORMAT_INVALID;

  if (Dem_DTCSelector_GetSelectionData(DTCSelectorId)->SelectorState != DEM_DTCSELECTOR_SELECTION_NO_MATCH)                      /* COV_DEM_ROBUSTNESS TX */
  {
    lReturnValue = Dem_DTCSelector_GetSelectionData(DTCSelectorId)->Request.DTCFormat;
  }
 ...
 * \return        DEM_DTCSELECTOR_GROUP_OBD_ALL_DTC
 *                The DTC matches the WWH-OBD or OBD on UDS ALL DTC group
...
# endif
  { 
    ...
    lDTCSelector->MemoryId = TranslatedMemoryId;                                                                                 /* SBSW_DEM_ARRAY_POINTER_DTCSELECTORTABLE */
...
# endif
  {
    Dem_DTCSelector_DataPtrType lDTCSelector;
    lDTCSelector = Dem_Cfg_GetAddrDTCSelectorTable(DTCSelectorId);                                                               /* PRQA S 2931 */ /* MD_DEM_18.1_ComputingOfInvalidPointer */
  ...
  )
{
    ...
    || (DTCFormat == DEM_DTC_FORMAT_OBD) )
  {
    ...
    else if (DTC == DEM_DTC_GROUP_OBD_DTCS)
    {
      lReturnValue = DEM_DTCSELECTOR_GROUP_OBD_ALL_DTC;
    }
    else
    {
      lReturnValue = DEM_DTCSELECTOR_GROUP_INVALID;
  ...
  {
    /* Other DTC formats are not supported */
    lReturnValue = DEM_DTCSELECTOR_GROUP_INVALID;
  }
  ...
  )
{
  ...
  if (DTC == 0u) /* '0' is not a valid DTC number */
  {
  ...
  {
    lEventId = Dem_Cfg_GetMasterEvent(Dem_Core_DTC_FindEventFromDtc(DTC, DTCFormat, TranslatedMemoryId));
  }
  ...
  )
{
  Dem_DTCSelector_InitInvalid(DTCSelectorId, 
                              (uint32)DEM_CFG_DTC_UDS_INVALID,
...
{
  /* Must be initialized due to MISRA */
  uint16 lTranslatedMemoryId = DEM_CFG_MEMORYID_PRIMARY;
  if (Dem_FaultMemory_Memories_TranslateDTCOrigin(DTCOrigin, &lTranslatedMemoryId) == E_NOT_OK)                                  /* SBSW_DEM_POINTER_FORWARD_STACK */
  {
    Dem_DTCSelector_InitInvalid(DTCSelectorId, DTCNumber, DTCFormat, DTCOrigin, DEM_DTCSELECTOR_SELECTION_WRONG_ORIGIN);
  }
  else
  {
    Dem_DTCSelector_GroupIdType lGroupId = Dem_DTCSelector_TestForAllDtc(DTCNumber, DTCFormat);
    if (lGroupId != DEM_DTCSELECTOR_GROUP_INVALID)
    {
      Dem_DTCSelector_InitGroupId(DTCSelectorId, DTCNumber, DTCFormat, DTCOrigin, lTranslatedMemoryId, lGroupId);
    }
    else
    {
      uint8 lGroupMask = Dem_DTCSelector_TestForGroupDtc(DTCNumber, DTCFormat);
      if (lGroupMask != DEM_CFG_GROUPMASK_NONE)
      {
        Dem_DTCSelector_InitGroupMask(DTCSelectorId, DTCNumber, DTCFormat, DTCOrigin, lTranslatedMemoryId, lGroupMask);
      }
      else
      {
        Dem_EventIdType lEventId;
        lEventId = Dem_DTCSelector_TestForSingleDtc(DTCNumber, DTCFormat, lTranslatedMemoryId);
        if (lEventId != DEM_EVENT_INVALID)
        {
          Dem_DTCSelector_InitEventId(DTCSelectorId, DTCNumber, DTCFormat, DTCOrigin, lTranslatedMemoryId, lEventId);
        }
        else
        {
