 ...
 * \brief         Copy DID data into the given buffer
 *
 * \details       The stored OBD on UDS FF data of an event is written to
 *                DestinationBuffer in no particular format (raw hex).
 *
 * \param[in]     EventId
 *                Unique handle of the Event.
 *                The EventId must be relevant for OBD on UDS.
 * \param[in]     DataId
 *                Return only the data of this DID.
 * \param[in]     SourceBuffer
 *                Source buffer of OBD on UDS freeze frame data
 * \param[in,out] DestinationBuffer
 *                Pointer to the destination buffer structure.
 * \return        E_OK
 *                The requested data was copied to the destination buffer.
 ...
 *****************************************************************************/
/*!
 * \brief         Copy DID data for a SRec, which may be stored without the
 *                event stored in event entry
 *
 * \details       Copy DID data for a time series SRec or custom triggered SRec:
 *                These records are stored in a separate memory. It may happen
 *                that the SRecs are stored even if the event is not stored in event
 *                entry. The internal data elements will be read or padded depending
 *                on event storage.
 *
 * \param[in]     DataId
 *                Return only the data of this DID.
 * \param[in]     EventEntryIndex
 *                Index of the event entry in Dem_Cfg_EventEntryPtr.
 *                DEM_CFG_ENTRYINDEX_INVALID, if event is not stored yet
 * \param[in]     EventId
 *                Unique handle of the event.
 ...
 *                Memory Identifier of the Readout Buffer
 *
 * \pre           An entry must have been copied to the intermediate
 ...
 *                Unique handle of the event.
 *                The EventId must be in range [1..Dem_Cfg_GlobalEventCount()[.
 * \param[in]     ReadoutBufferEventEntryIndex
 *                Index of the readoutbuffer event entry in Dem_Cfg_EventEntryPtr.
 *                Must be in range [0..Dem_Cfg_GetSizeOfEventEntryPtr()[.
 * \param[in]     ReadoutBufferId
 *                Memory Identifier of the Readout Buffer
 *
 ...
 *****************************************************************************/
/*!
 * \brief         Copies the contents of the OBD on UDS freeze frame
 *
 * \details       This function formats the OBD on UDS freeze frame data stored in
 *                the source according to the UDS snapshot record specification
 *                and copies the result into the given destination buffer.
 *                The function will abort copying data if the destination is
 *                too small.
 ...
 * \pre           Existence of the record must be verified before calling this
 *                function, otherwise random data will be copied.
 *                DestinationBuffer write index has to be within the buffer size.
 * \config        DEM_CFG_SUPPORT_DCM == STD_ON &&
 ...
 *                source according to the UDS snapshot record specification and
 *                copies the result into the given destination buffer.
 *                The function will abort copying data if the destination is
 *                too small.
 *
 ...
 *****************************************************************************/
/*!
 * \brief         Copies the J1939 OBD freeze frame if supported
 *
 * \details       Copies the J1939 OBD freeze frame if supported
 *
 * \param[in]     BufferIdx
 *                Index of the event's OBD freeze frame
 * \param[in,out] DestinationBuffer
 *                Pointer to DestinationBuffer structure
 *
 * \pre           Existence of the OBD FreezeFrame must be verified before calling
 *                this function.
 * \config        -
 *
 * \context       TASK|ISR2
 ...
 *
 * \details       The stored snapshot data of an event entry is written
 ...
 *                The requested data was copied to the destination buffer.
 * \return        DEM_NO_SUCH_ELEMENT
 *                The requested data identifier is undefined within the
 *                requested record.
 * \return        DEM_BUFFER_TOO_SMALL
 *                The provided buffer size is too small.
 *
