 ...
 *  \{
 *  \file       Dem_DiagnosticObserverIF_SvcImplementation.h
...
#if !defined (DEM_DIAGNOSTICOBSERVERIF_SVCIMPLEMENTATION_H)
#define DEM_DIAGNOSTICOBSERVERIF_SVCIMPLEMENTATION_H
...
#define DEM_DIAGNOSTICOBSERVERIF_SVCIMPLEMENTATION_FILENAME "Dem_DiagnosticObserverIF_SvcImplementation.h"
  ...
  )
{
  ...
  {
    /* ----- Implementation ------------------------------------------------ */
    lReturnValue = Dem_GlobalDiagnostics_ProcessEventDisabled(EventId);
  }
...
# endif
  ...
  else
  {
    ...
    if ( (Dem_Event_TestEventAvailable(RatioID) == TRUE)
      && (Dem_GlobalDiagnostics_TestPtoActiveAndEventAffected(RatioID) == FALSE) )
    {
  ...
  )
{
  ...
  /* ----- Development Error Checks ---------------------------------------- */
# if (DEM_DEV_ERROR_DETECT == STD_ON)
  if ((Dem_Iumpr_IsEnabled() == FALSE) && (Dem_Mar_IsEnabled() == FALSE))
  {
    Dem_Det_ErrorRegisterSet(DEM_E_WRONG_CONDITION)
  }
  else
...
FUNC(Std_ReturnType, DEM_CODE)
Dem_SetIUMPRFilter(
  Dem_IumprReadinessGroupType IumprReadinessGroup,                                                                               /* PRQA S 1330 */ /* MD_DEM_8.3_rte */
...
FUNC(Std_ReturnType, DEM_CODE)
Dem_GetIUMPRGeneralData(
  P2VAR(uint16, AUTOMATIC, DEM_APPL_DATA)  GeneralDenominator,                                                                   /* PRQA S 1330 */ /* MD_DEM_8.3_rte */
...
FUNC(Std_ReturnType, DEM_CODE)
Dem_GetNextIUMPRRatioDataAndDTC(
  P2VAR(uint32, AUTOMATIC, DEM_APPL_DATA)  UdsDtcNumber,                                                                         /* PRQA S 1330 */ /* MD_DEM_8.3_rte */
...
)
{
  ...
  if (Dem_APIChecks_NullPointer(DataValueBuffer) == FALSE)                                                                       /* SBSW_DEM_POINTER_NULLPTR_CHECKING */  /* PRQA S 0315 */ /* MD_DEM_Dir1.1 */
  {
    Dem_Det_ErrorRegisterSet(DEM_E_PARAM_POINTER)
  }
  else
  if (Dem_APIChecks_NullPointer(DataValueBufferSize) == FALSE)                                                                   /* SBSW_DEM_POINTER_NULLPTR_CHECKING */  /* PRQA S 0315 */ /* MD_DEM_Dir1.1 */
  {
    Dem_Det_ErrorRegisterSet(DEM_E_PARAM_POINTER)
  }
  else
  ...
  {
    if (*DataValueBufferSize < DEM_MAD_SIZE)
    {
      lReturnValue = DEM_BUFFER_TOO_SMALL;
    }
    else
    {
      DataValueBuffer[0] = (uint8)Dem_Mar_Mad_GetDenominator();                                                                  /* SBSW_DEM_POINTER_WRITE_ARGUMENT_BUFFER */
      *DataValueBufferSize = DEM_MAD_SIZE;                                                                                       /* SBSW_DEM_POINTER_WRITE_API */
      lReturnValue = E_OK;
    }
  }
  ...
  if (Dem_Det_ErrorRegisterGet() != DEM_E_NO_ERROR)
  {
    Dem_Error_ReportError(DEM_DCMGETINFOTYPEVALUE79_APID, Dem_Det_ErrorRegisterGet());
  }

  DEM_IGNORE_UNUSED_CONST_ARGUMENT(OpStatus)
...
#endif /* DEM_DIAGNOSTICOBSERVERIF_SVCIMPLEMENTATION_H */

/*!
 * \}
 */
/* ********************************************************************************************************************
 *  END OF FILE: Dem_DiagnosticObserverIF_SvcImplementation.h
 *********************************************************************************************************************/
