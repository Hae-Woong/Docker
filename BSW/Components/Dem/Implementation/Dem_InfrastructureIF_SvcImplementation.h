...
#if !defined (DEM_INFRASTRUCTUREIF_SVCIMPLEMENTATION_H)
#define DEM_INFRASTRUCTUREIF_SVCIMPLEMENTATION_H
...
#define DEM_INFRASTRUCTUREIF_SVCIMPLEMENTATION_FILENAME "Dem_InfrastructureIF_SvcImplementation.h"
...
# endif
#endif
  {
...
#endif
    {
      ...
      /* In Dem_GlobalDiagnostics_Init event indicator counter and activation mode are set according to the number of events with WIR bit set
      -> As WIR bit of event can be recovered during restoration of permanent memory,
         this function has to be called first. */
      Dem_GlobalDiagnostics_Init();
      /* In Dem_OperationCycle_Init OBD DCY may be restarted checking array 
      Dem_GlobalDiagnostics_ActivationMode_DtcClassMILCount initialized in Dem_GlobalDiagnostics_Init. */
      ...
      if (Dem_Cfg_IsPermanentSupportedInVariant() == TRUE)
      {
        ...
        if ( (Dem_Cfg_IsObdIIOrObdOnUdsSupportedInVariant() == TRUE)
          && (Dem_Memories_PermanentMemory_TestEnabled() == TRUE) )
#endif
        {
  ...
  )
{
  ...
  else
  {
    ...
    Dem_Satellite_CopyDataDebounceNv(DEM_NVM_BLOCKSTATE_DIRTY);
...
FUNC(Std_ReturnType, DEM_CODE)
Dem_PostRunRequested(
  P2VAR(boolean, AUTOMATIC, DEM_APPL_DATA)  IsRequested                                                                          /* PRQA S 1330 */ /* MD_DEM_8.3_rte */
  )
{
  Std_ReturnType lReturnValue;
  Dem_Det_ErrorRegisterDefine()
  ...
  else
  {
    ...
    { /* Currently waiting for NvM JobResult, and the same block is still in the process
         ...
         NvM_SetRamBlockStatus -> return TRUE to request more time until the situation
         has resolved itself */
  ...
  )
{
  ...
  else
  {
    ...
    /* ----- Implementation ------------------------------------------------ */
    Dem_FaultMemory_Memories_SetInitAdminDataRequested(TRUE);
    ...
    Dem_Memories_PermanentMemory_SetEnabled(FALSE);

# if (DEM_CFG_DATA_MAX_FDC_SINCE_LAST_CLEAR == STD_ON) && (DEM_CFG_SUPPORT_MEM_INDEPENDENT_MAX_FDC_SLC == STD_ON)
    {
      uint16 lMemIndependentMaxFdcIndex = 0u;
      for (; lMemIndependentMaxFdcIndex < DEM_CFG_MAX_NUMBER_MAX_FDC_SLC_DTCS; lMemIndependentMaxFdcIndex++)
      {
        Dem_Cfg_AdminData.MaxFDCSinceLastClear[lMemIndependentMaxFdcIndex] = 0;                                                  /* SBSW_DEM_ARRAY_WRITE_ADMINDATA_MAXFDCSINCELASTCLEAR */
      }
    }
# endif
  }
...
#endif /* DEM_INFRASTRUCTUREIF_SVCIMPLEMENTATION_H */

/*!
 * \}
 */
/* ********************************************************************************************************************
 *  END OF FILE: Dem_InfrastructureIF_SvcImplementation.h
 *********************************************************************************************************************/
