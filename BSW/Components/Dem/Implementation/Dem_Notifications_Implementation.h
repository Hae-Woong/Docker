...
/* ****************************************************************************
 * Dem_DiagnosticObserver_Notifications_IsTriggerJ1939DcmReportsSupported
 *****************************************************************************/
/*!
 * \brief         Tests if triggering J1939 Dcm reports about DTC status
 *                changes is supported.
 *
 * \details       Tests if notifying the J1939 Dcm module about DTC status
 *                changes in primary memory is supported.
 *
 * \return        TRUE
 *                Triggering J1939 Dcm reports is supported.
 * \return        FALSE
 *                Triggering J1939 Dcm reports is not supported
 *
 * \pre           -
 *
 * \context       TASK|ISR2
...
{
  return (boolean)(DEM_CFG_NOTIFY_J1939DCM == STD_ON);
}
...
#else
  DEM_IGNORE_UNUSED_CONST_ARGUMENT(J1939DTC)                                                                                     /* PRQA S 3112 */ /* MD_DEM_14.2 */
...
{
  uint32 lJ1939Dtc = Dem_Cfg_EventJ1939Dtc(EventId);
  ...
  if (lJ1939Dtc != DEM_CFG_DTC_J1939_INVALID)
  {
    ...
    {
      Dem_DtcStatusChangedFPtrType lpDtcStatusFunc = Dem_Cfg_GlobalCbkJ1939DtcIterGet(&lIter);                                   /* SBSW_DEM_CALL_FUNCPTR_ITERATOR_POINTER */
    ...
    if ( (Dem_DiagnosticObserver_Notifications_IsTriggerJ1939DcmReportsSupported() == TRUE)
      && (Dem_Core_Event_GetDestination(EventId) == DEM_CFG_MEMORYID_PRIMARY)
      && (Dem_Event_UDSStatus_TestBit((Dem_UDSStatus_StatusType) (DtcStatusOld ^ DtcStatusNew), Dem_UdsStatus_TF) == TRUE) )
    { /* J1939Dcm is only triggered for events in primary memory if testFailed bit changes */
...
# else
  DEM_IGNORE_UNUSED_CONST_ARGUMENT(EventId)                                                                                      /* PRQA S 3112 */ /* MD_DEM_14.2 */
  DEM_IGNORE_UNUSED_CONST_ARGUMENT(EventAvailable)                                                                               /* PRQA S 3112 */ /* MD_DEM_14.2 */
# endif
}
#endif
