...
/*!
* \addtogroup Dem_0x1904_PublicProperties
* \{
*/
 ...
 *                The requested record is not available
 *
 * \pre           An entry has to be locked for Dcm readout
 * \config        DEM_CFG_SUPPORT_DCM == STD_ON &&
 ...
 *
 * \pre           The relevant subevent event entry has to be locked for 
 *                Dcm readout
 * \config        DEM_CFG_SUPPORT_DCM == STD_ON &&
 *                DEM_CFG_SUPPORT_TYPE2_COMBINATION == STD_ON
 *                ( DEM_CFG_SUPPORT_SRECS == STD_ON ||
 ...
 *                to the destination buffer. Subevent entry is identified
...
Dem_RecordReader_0x1904_CopySRec_CombinedDTC(
  uint8 SRecId,
  uint8 SnapshotHeaderBitMaskIndex,
  Dem_DTCReadoutBuffer_HandleType ReadoutBufferId,
  Dem_Data_DestinationBufferPtrType  DestinationBuffer
  );
#endif
 ...
 *
 * \details       Move the source iterator to the next source depending
...
#if (DEM_CFG_SUPPORT_DCM == STD_ON)
/* ****************************************************************************
 * Dem_RecordReader_0x1904_ReadNextRecord_CustomTriggerSrec
 *****************************************************************************/
/*!
 * \brief         Reads a custom triggered snapshot record from readout buffer
 *
 * \details       -
 *
 * \param[in]     ReadoutBufferId
 ...
 *
 * \return        E_OK
 *                Custom triggered snapshot record is read successfully
 * \return        DEM_BUFFER_TOO_SMALL
 *                Provided buffer size too small.
 * \return        DEM_NO_SUCH_ELEMENT
 *                Event has no custom triggered snapshot record configured
 *
 * \pre           -
 * \config        DEM_CFG_SUPPORT_DCM == STD_ON
 *
...
#if (DEM_CFG_SUPPORT_DCM == STD_ON)
/* ****************************************************************************
 * Dem_RecordReader_0x1904_ReadNextRecord_CustomTriggerTssr
 *****************************************************************************/
/*!
 * \brief         Reads a custom triggered time series snapshot record 
 *                from readout buffer
 *
 * \details       -
 *
 ...
 *                Provided buffer size too small.
 * \return        DEM_NO_SUCH_ELEMENT
 *                The requested custom triggered time series snapshot record
 *                not available
 *
 * \pre           -
 * \config        DEM_CFG_SUPPORT_DCM == STD_ON
 ...
 *
 * \pre           An entry has to be locked for Dcm readout
 *                RecordNumber must be a valid time series srec number of the event
 * \config        DEM_CFG_SUPPORT_DCM == STD_ON &&
 *                DEM_FEATURE_NEED_TIME_SERIES == STD_ON
 *                && DEM_CFG_SUPPORT_TIME_SERIES_POWERTRAIN == STD_OFF
 *
...
Dem_RecordReader_0x1904_CalculateSize_TimeSeriesPowertrain(
  Dem_DTCReadoutBuffer_HandleType ReadoutBufferId,
  Dem_EventIdType EventId,
  CONSTP2VAR(uint16, AUTOMATIC, DEM_SHARED_DATA)  SizeOfFreezeFrame
  );
#endif

/* ****************************************************************************
 * Dem_RecordReader_0x1904_CalculateSize_ConfiguredOrCalculatedSRecs
 *****************************************************************************/
/*!
 * \brief         Calculates the size of configured or calculated
 *                snapshot records
 *
 * \details       Calculates the size of all configured or calculated
 *                snapshot records stored for the event.
 *
 * \param[in]     ReadoutBufferId
 *                Index of the ReadoutBuffer
 * \param[in]     EventId
...
Dem_RecordReader_0x1904_CalculateSize_ConfiguredOrCalculatedSRecs(
  Dem_DTCReadoutBuffer_HandleType ReadoutBufferId,
  Dem_EventIdType EventId,
  CONSTP2VAR(uint16, AUTOMATIC, AUTOMATIC) SizeOfFreezeFrame
);

/* ****************************************************************************
 * Dem_RecordReader_0x1904_CalculateSize_TimeSeriesSRecs
 *****************************************************************************/
/*!
 * \brief         Calculates the size of time series snapshot records
 *
 * \details       Calculates the size of all time series snapshot records
 ...
 *                Snapshot record number
 * \param[in]     SnapshotHeaderBitMaskIndex
 *                Index of the SRec inside the DTC specific SRec list
 *
 ...
 *
 * \return        E_OK
 *                The requested record was found. If stored in memory, data is returned
 * \return        DEM_NO_SUCH_ELEMENT
 *                The requested record is not OBD related
 * \return        DEM_BUFFER_TOO_SMALL
 *                The destination buffer is too small
 *
 * \pre           An entry has to be locked for Dcm readout
...
#if (DEM_CFG_SUPPORT_DCM == STD_ON)
/* ****************************************************************************
 * Dem_RecordReader_0x1904_CalculateSize_CustomTriggerRecord
 *****************************************************************************/
/*!
 * \brief         Calculates size of the requested custom triggered record,
 *                (snapshot or time series sample) formatted as UDS 
 *                snapshot record (19 04)
 *
 * \details       -
 *
 * \param[in]     ReadoutBufferId
 ...
 *                The EventId must be in range [1..Dem_Cfg_GlobalEventCount()[.
 * \param[in]     RecordNumber
 *                A valid record number of custom triggered snapshot or time series
 *                snapshot
 * \param[out]    SizeOfFreezeFrame
 *                Number of bytes.
 *
 ...
 *                Data was found and size returned
 * \return        DEM_NO_SUCH_ELEMENT
 *                Requested custom triggered record is not available
 *
 * \pre           Record number has to be verified by caller using
...
/* ****************************************************************************
 * Dem_RecordReader_0x1904_CalculateSize_AllCustomTriggeredRecords
 *****************************************************************************/
/*!
 * \brief         Calculates the total size of all custom triggered snapshot records
 *                and time series samples formatted as UDS snapshot record (19 04)
 *
 * \details       -
 *
 * \param[in]     ReadoutBufferId
 ...
 *                Data was found and size returned
 * \return        DEM_NO_SUCH_ELEMENT
 *                No custom triggered data is available
 *
 * \pre           An entry has to be locked for Dcm readout
 *
 * \context       TASK|ISR2
 ...
 *
 * \pre           The EventId referenced event needs at least one TimeSeries Snapshot assigned.
 * \config        -
 *
 * \context       TASK|ISR2
 * \synchronous   TRUE
...
/* ****************************************************************************
 * Dem_RecordReader_0x1904_GlobalSnapshotRecordsEnabled
 *****************************************************************************/
/*!
 * \brief         Returns true if feature Global Snapshot Records is enabled.
 *
 * \details       -
 *
 * \return        TRUE
 *                Global Snapshot Records is enabled
 * \return        FALSE
 *                Global Snapshot Records is not enabled
 *
 * \pre           -
 *
 * \context       TASK|ISR2
...
# endif
      break;
    case Dem_DTCReadoutBuffer_SnapshotSource_TimeSeries:
...
# endif
      break;
    case Dem_DTCReadoutBuffer_SnapshotSource_CustomTriggerSrec:
      lReadNextReturnValue = Dem_RecordReader_0x1904_ReadNextRecord_CustomTriggerSrec(lReadoutBufferId, DestinationBuffer);      /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
      break;
    case Dem_DTCReadoutBuffer_SnapshotSource_CustomTriggerTssr:
      lReadNextReturnValue = Dem_RecordReader_0x1904_ReadNextRecord_CustomTriggerTssr(lReadoutBufferId, DestinationBuffer);      /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
      break;
...
# endif
    ...
    default:                                                                                                                     /* PRQA S 2016 */ /* MD_MSR_EmptyClause */
      ...
      /* Dem_DTCReadoutBuffer_SnapshotSource_TimeSeries */
...
{
  Dem_DTCReadoutBuffer_SnapshotSourceType lSource;
  Dem_DTCReadoutBuffer_HandleType lReadoutBufferId = Dem_Client_ClientAccess_GetReadoutBuffer(ClientId);
  Dem_EventIdType lEventId = Dem_DTCReadoutBuffer_GetEventId(lReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT);
  ...
  if (Dem_Memories_CustomTriggerMemory_TestRecordNumberIsSRec(lEventId, RecordNumber))
  {
    lSource = Dem_DTCReadoutBuffer_SnapshotSource_CustomTriggerSrec;
  }
  else
  if (Dem_Memories_CustomTriggerMemory_TestRecordNumberIsTSSRec(lEventId, RecordNumber))
  {
    lSource = Dem_DTCReadoutBuffer_SnapshotSource_CustomTriggerTssr;
  }
  else
# if (DEM_FEATURE_NEED_TIME_SERIES == STD_ON)
#  if (DEM_CFG_SUPPORT_TIME_SERIES_POWERTRAIN == STD_OFF)
  if(Dem_Memories_TimeSeriesMemory_TestRecordNumberIsValid(lEventId, RecordNumber))
  {
    lSource = Dem_DTCReadoutBuffer_SnapshotSource_TimeSeries;
    Dem_DTCReadoutBuffer_SnapshotSource_SetCurrentTimeSeriesId(lReadoutBufferId, Dem_Memories_TimeSeriesMemory_GetNextOrLastSeriesByRecordNumber(lEventId, RecordNumber));
  }
  else
#  else
...
# if ( (DEM_CFG_SUPPORT_SRECS == STD_ON) \
    ...
    || ((lRecordNumber == 0xF0u) && (Dem_Cfg_IsObdOnUdsSupportedInVariant() == TRUE)))
  {
    if (lMemoryId == DEM_CFG_MEMORYID_PRIMARY)
    {
      if (Dem_Cfg_IsObdOnUdsSupportedInVariant() == TRUE)
      {
        boolean lIsOBDDataStored = Dem_DTCReadoutBuffer_GetOBDData_IsStored(lReadoutBufferId);
        lReturnValue = Dem_RecordReader_0x1904_CalculateSize_ObdOnUds(lEventId,
                         lIsOBDDataStored, SizeOfFreezeFrame);                                                                   /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
      }
      else
      if (Dem_RecordReader_0x1904_IsObdIIFreezeFrameInService19EnabledInVariant() == TRUE)
      {
  ...
  if(Dem_Memories_CustomTriggerMemory_TestRecordNumberIsValid(lEventId, lRecordNumber))
  {
    lReturnValue = Dem_RecordReader_0x1904_CalculateSize_CustomTriggerRecord(lReadoutBufferId, lEventId, lRecordNumber, SizeOfFreezeFrame); /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  }
  else
#  if (DEM_FEATURE_NEED_TIME_SERIES == STD_ON) && (DEM_CFG_SUPPORT_TIME_SERIES_POWERTRAIN == STD_OFF)
  if (Dem_Memories_TimeSeriesMemory_TestRecordNumberIsValid(lEventId, lRecordNumber) == TRUE)
  {
  ...
  )
{
  ...
  lSRecNumerationType = Dem_FaultMemory_Memories_GetSRecNumerationType(Dem_Core_Event_GetDestination(lMasterEventId));
  lSnapshotHeaderBitMaskIndex = Dem_MemoryEntry_SnapshotEntry_FindSRecIndexInCombinedDTCSRecList(RecordNumber, lSRecNumerationType, lCombinedDTCGroup);

  if ((lSnapshotHeaderBitMaskIndex == DEM_SNAPSHOTENTRY_CONFIGURED_SREC_INDEX_INVALID)
    || (Dem_MemoryEntry_SnapshotEntry_SRecIsSupportedByAvailableCombinedEvent(lCombinedDTCGroup, RecordNumber) == FALSE))        /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
  {
...
# if (DEM_CFG_SUPPORT_SRECS == STD_ON)
  {
    ...
    Dem_Cfg_DTCFreezeFrameRecordIterInit(&lRecordNumberIterator,                                                                 /* SBSW_DEM_CALL_ITERATOR_POINTER */
    ...
    if (Dem_Cfg_DTCFreezeFrameRecordIterExists(&lRecordNumberIterator) == TRUE)                                                  /* SBSW_DEM_CALL_ITERATOR_POINTER */
    {
      ...
      uint8 lNumOccupiedReadoutBufferEntries = Dem_DTCReadoutBuffer_GetNumberOfOccupiedSlots(ReadoutBufferId);
      lSizeOfFreezeFrame = 0x00U;
           ...
           lReadoutBufferDataIndex++)
      {
        Dem_Cfg_EntryIndexType lReadoutBufferEventEntryIndex
          = Dem_DTCReadoutBuffer_GetEventEntryIndex(ReadoutBufferId, lReadoutBufferDataIndex);
        Dem_EventIdType lEventId = Dem_DTCReadoutBuffer_GetEventId(ReadoutBufferId, lReadoutBufferDataIndex);
        lSizeOfFreezeFrame += ((uint16)(Dem_MemoryEntry_SnapshotEntry_GetNumberOfStoredSnapshotRecords(lReadoutBufferEventEntryIndex)
  ...
  )
{
  ...
  while (Dem_MemoryEntry_SnapshotEntry_IteratorExists(&lSnapshotEntryIter) == TRUE)                                              /* SBSW_DEM_POINTER_FORWARD_STACK */
  {
    lSRecIndex = Dem_MemoryEntry_SnapshotEntry_IteratorGetSnapshotRecordIndex(&lSnapshotEntryIter);                              /* SBSW_DEM_POINTER_FORWARD_STACK */
    if (SRecId == Dem_Cfg_SRecId(lSRecIndex))
    {
        ...
        Dem_DataReport_SRecCopyData(DestinationBuffer,                                                                           /* SBSW_DEM_POINTER_DESTINATIONBUFFER_SOURCEBUFFER */
                                      Dem_MemoryEntry_SnapshotEntry_IteratorGetSnapshotRecordDataConstPtr(&lSnapshotEntryIter),  /* SBSW_DEM_POINTER_FORWARD_STACK */
                                      Dem_DTCReadoutBuffer_GetEventId(ReadoutBufferId, ReadoutBufferDataIndex),
                                      Dem_DTCReadoutBuffer_GetEventEntryIndex(ReadoutBufferId, ReadoutBufferDataIndex),
                                      ReadoutBufferId);
    ...
    Dem_MemoryEntry_SnapshotEntry_IteratorNext(&lSnapshotEntryIter);                                                             /* SBSW_DEM_POINTER_FORWARD_STACK */
  }
  ...
  */
  Dem_Internal_AssertContinue(Dem_MemoryEntry_SnapshotEntry_IteratorExists(&lSnapshotEntryIter), DEM_E_INCONSISTENT_STATE)       /* SBSW_DEM_POINTER_FORWARD_STACK */
  ...
  )
{
  ...
  if (Dem_MemoryEntry_SnapshotEntry_SRecIsSupportedByAvailableCombinedEvent(lCombinedDTCGroupId, SRecId) == FALSE)
  {
    lReturnValue = DEM_NO_SUCH_ELEMENT;
  }
  ...
  else
  {
    ...
    for (; lReadoutBufferDataIndex < lNumOccupiedReadoutBufferEntries; lReadoutBufferDataIndex++)
    {
      lReadoutBufferEventEntryIndex = Dem_DTCReadoutBuffer_GetEventEntryIndex(ReadoutBufferId, lReadoutBufferDataIndex);
      if (Dem_MemoryEntry_SnapshotEntry_GetSRecStorageStatus(SRecId, SnapshotHeaderBitMaskIndex, lReadoutBufferEventEntryIndex) == TRUE)
      {
        Dem_RecordReader_0x1904_CopySRec_CombinedSubEvent(SRecId, ReadoutBufferId, lReadoutBufferDataIndex, DestinationBuffer);  /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
      }
    }
    lReturnValue = E_OK;
  }
  ...
  )
{
  ...
  /* Increment the iterator in case of buffer is not too small */
  if (lReturnValue != DEM_BUFFER_TOO_SMALL)
  {
    Dem_Cfg_DTCFreezeFrameRecordIterNext(&lRecordNumberIterator);                                                                /* SBSW_DEM_CALL_ITERATOR_POINTER */
 ...
 *
 */
DEM_LOCAL_INLINE FUNC(boolean, DEM_CODE) Dem_RecordReader_0x1904_TestRecordMatch(                                                /* PRQA S 3219 */ /*  MD_DEM_14.1_CONFIGURATION_SWITCH  */
  Dem_DTCReadoutBuffer_HandleType ReadoutBufferId,
  uint8 RecordNumber
  )
{
  ...
  )
{
  ...
  uint16_least lSourceIteratorStartId = 0;
  uint16_least lSourceIteratorEndId = 0;

  Dem_DTCReadoutBuffer_SnapshotSourceType lSource = Dem_DTCReadoutBuffer_GetSnapshotRecordSource(ReadoutBufferId);
...
# if (DEM_CFG_SUPPORT_CUSTOMTRIGGER_SREC == STD_ON)
    {
      ...
      {
        lSourceIteratorStartId = Dem_Memories_CustomTriggerMemory_GetSRecNumber(lEventId);
        lSourceIteratorEndId = (uint16_least)Dem_Memories_CustomTriggerMemory_GetSRecNumber(lEventId) + 1u;
      }
    }
# endif
    break;
...
# if (DEM_CFG_SUPPORT_CUSTOMTRIGGER_TSSR == STD_ON)
      {
        ...
        {
          lSourceIteratorStartId = Dem_Memories_CustomTriggerMemory_FirstTSSRecNum(lEventId);
          lSourceIteratorEndId = (uint16_least)Dem_Memories_CustomTriggerMemory_LastTSSRecNum(lEventId) + 1u;
        }
      }
# endif
      break;
...
# if (DEM_FEATURE_NEED_TIME_SERIES == STD_ON)
    {
      ...
      if (Dem_Memories_TimeSeriesMemory_TestEventSupportTimeSeries(lEventId) == TRUE)
      {
        Dem_TimeSeriesIdType lSeriesId = Dem_DTCReadoutBuffer_SnapshotSource_GetCurrentTimeSeriesId(ReadoutBufferId);
        Dem_TimeSeriesIdType lNumberOfSeries = Dem_Memories_TimeSeriesMemory_GetNumberOfSeriesOfEvent(lEventId);
        if(lSeriesId < lNumberOfSeries)                                                                                          /* COV_DEM_ROBUSTNESS TX */
        {
          lSourceIteratorStartId = Dem_Cfg_GetFirstRecordNumberOfTimeSeriesFreezeFrameTable(Dem_Memories_TimeSeriesMemory_GetTimeSeriesTableId(lEventId, lSeriesId));
          lSourceIteratorEndId = (uint8)(Dem_Memories_TimeSeriesMemory_GetLastRecordNumberOfSeries(lEventId, lSeriesId) + 1u);
        }
      }
    }
# endif
  ...
  Dem_Cfg_ComplexIterInit(&lSourceIterator, lSourceIteratorStartId, lSourceIteratorEndId);                                       /* SBSW_DEM_CALL_ITERATOR_POINTER */
...
# else
        ...
        Dem_DTCReadoutBuffer_SetSnapshotRecordSource(ReadoutBufferId, Dem_DTCReadoutBuffer_SnapshotSource_TimeSeries);
        Dem_DTCReadoutBuffer_SnapshotSource_SetCurrentTimeSeriesId(ReadoutBufferId, Dem_Memories_TimeSeriesMemory_GetNextOrLastSeriesByRecordNumber(lEventId, 0));
# endif
      }
      lReturnValue = E_OK;
      break;
    ...
    case Dem_DTCReadoutBuffer_SnapshotSource_TimeSeries:
      {
        ...
        uint8 lLastRecordNumber;
        lIter = Dem_Memories_TimeSeriesMemory_InitializeTimeSeriesRecordIteratorBySeriesId(lEventId, Dem_DTCReadoutBuffer_SnapshotSource_GetCurrentTimeSeriesId(ReadoutBufferId));
        lLastRecordNumber = lIter.LastRecordNumber;

        /* Check if there are later record numbers belonging to a time series of the event */
        Dem_Memories_TimeSeriesMemory_TimeSeriesRecordIteratorNextSeriesByRecordNumber(&lIter);                                  /* SBSW_DEM_POINTER_FORWARD_STACK */
        if(lIter.LastRecordNumber > lLastRecordNumber)
        {
          Dem_DTCReadoutBuffer_SnapshotSource_SetCurrentTimeSeriesId(ReadoutBufferId, lIter.SeriesId);
        }
        else
        {
          Dem_DTCReadoutBuffer_SetSnapshotRecordSource(ReadoutBufferId, Dem_DTCReadoutBuffer_SnapshotSource_CustomTriggerSrec);
        }
        lReturnValue = E_OK;
      }
      break;
    ...
    case Dem_DTCReadoutBuffer_SnapshotSource_Srec_GreaterThan_0x30:
      ...
      break;
    case Dem_DTCReadoutBuffer_SnapshotSource_CustomTriggerSrec:
      Dem_DTCReadoutBuffer_SetSnapshotRecordSource(ReadoutBufferId, Dem_DTCReadoutBuffer_SnapshotSource_CustomTriggerTssr);
      lReturnValue = E_OK;
      break;
    case Dem_DTCReadoutBuffer_SnapshotSource_CustomTriggerTssr:
      Dem_DTCReadoutBuffer_SetSnapshotRecordSource(ReadoutBufferId, Dem_DTCReadoutBuffer_SnapshotSource_ObdOnUds_Updateable);
...
# if (DEM_CFG_SUPPORT_WWHOBD == STD_ON)
      if (Dem_Cfg_IsWwhObdSupportedInVariant() == TRUE)                                                                          /* COV_DEM_MISRA_BRANCH */
      {
  ...
  )
{
  ...
  Dem_Cfg_ComplexIterType lSourceIterator;
  for(lSourceIterator = Dem_DTCReadoutBuffer_GetSnapshotSourceIterator(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT);
      Dem_Cfg_ComplexIterExists(&lSourceIterator) == TRUE;                                                                       /* SBSW_DEM_CALL_ITERATOR_POINTER */
      Dem_Cfg_ComplexIterNext(&lSourceIterator))                                                                                 /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    uint8 lRecordNumber = (uint8)Dem_Cfg_ComplexIterGet(&lSourceIterator);                                                       /* SBSW_DEM_CALL_ITERATOR_POINTER */
    if (Dem_RecordReader_0x1904_TestRecordMatch(ReadoutBufferId, lRecordNumber) == TRUE)
    {
      Dem_EventIdType lEventId = Dem_DTCReadoutBuffer_GetEventId(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT);
      if(Dem_Memories_TimeSeriesMemory_TestEventSupportTimeSeries(lEventId))                                                     /* COV_DEM_ROBUSTNESS TX */
      {
        Dem_TimeSeriesIdType lSeriesId = Dem_Memories_TimeSeriesMemory_GetNextOrLastSeriesByRecordNumber(lEventId, lRecordNumber);
        if(Dem_Memories_TimeSeriesMemory_TestRecordNumberIsInSeries(lEventId, lSeriesId, lRecordNumber))
        {
          lReturnValue = E_OK;
          if (Dem_RecordReader_0x1904_TimeSeriesSRecIsReadableInVirtualBuffer(ReadoutBufferId, lRecordNumber) == TRUE)
          {
            ...
            else
            {
              ...
              Dem_Cfg_EntryIndexType lTimeSeriesEntryIndex = Dem_DTCReadoutBuffer_GetTimeSeriesEntryIndex(ReadoutBufferId);

              Dem_Cfg_EntryIndexType lReadoutBufferEventEntryIndex =
                Dem_DTCReadoutBuffer_GetEventEntryIndex(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT);

              uint8 lSetId = Dem_Memories_TimeSeriesMemory_GetSetIdOfRecordNumber(lTimeSeriesEntryIndex, lSeriesId, lRecordNumber);
              lpSourceBuffer = Dem_MemoryEntry_TimeSeriesEntry_GetSnapshotDataPtr(lTimeSeriesEntryIndex, lSeriesId, lSetId,
                Dem_Memories_TimeSeriesMemory_GetSnapshotOffsetFromFirstOneInSet(lEventId, lSeriesId, lRecordNumber));
  ...
  if (Dem_Cfg_ComplexIterExists(&lSourceIterator) && (lReturnValue != DEM_BUFFER_TOO_SMALL))                                     /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    Dem_Cfg_ComplexIterNext(&lSourceIterator);                                                                                   /* SBSW_DEM_CALL_ITERATOR_POINTER */
  }

  Dem_DTCReadoutBuffer_SetSnapshotSourceIterator(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT, lSourceIterator);
  ...
  )
{
  ...
  else if (Dem_Memories_TimeSeriesMemory_TestEventSupportTimeSeries(lEventId) == FALSE)
  {
  ...
  {
    lReturnValue = E_OK;
    if (Dem_DTCReadoutBuffer_GetTimeSeriesEntryIndex(ReadoutBufferId) != DEM_CFG_ENTRYINDEX_INVALID)
    {
  ...
  )
{
  ...
  while (Dem_MemoryEntry_SnapshotEntry_IteratorExists(&lSnapshotEntryIter) == TRUE)                                              /* PRQA S 0771 */ /* MD_DEM_15.4_opt */ /* SBSW_DEM_POINTER_FORWARD_STACK */
  {
    lSRecIndex = Dem_MemoryEntry_SnapshotEntry_IteratorGetSnapshotRecordIndex(&lSnapshotEntryIter);                              /* SBSW_DEM_POINTER_FORWARD_STACK */
      ...
      if (Dem_MemoryEntry_SnapshotEntry_IteratorIsSnapshotRecordStored(&lSnapshotEntryIter) == TRUE)                             /* SBSW_DEM_POINTER_FORWARD_STACK */
      {
        ...
        else
        {
          ...
          Dem_DataReport_SRecCopyData(DestinationBuffer,                                                                         /* SBSW_DEM_POINTER_DESTINATIONBUFFER_SOURCEBUFFER */
                                      Dem_MemoryEntry_SnapshotEntry_IteratorGetSnapshotRecordDataConstPtr(&lSnapshotEntryIter),  /* SBSW_DEM_POINTER_FORWARD_STACK */
                                      lEventId,
                                      Dem_DTCReadoutBuffer_GetEventEntryIndex(ReadoutBufferId,
                                      DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT),
                                      ReadoutBufferId);
        }
      }
  ...
  else
  {
    ...
    if (Dem_DTCReadoutBuffer_TestGlobalSnapshotStored(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT) == TRUE)
    {
      ...
      else
      {
        ...
        lReadoutBufferEventEntryIndex = Dem_DTCReadoutBuffer_GetEventEntryIndex(ReadoutBufferId,
                                                                            DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT);
 ...
 */
DEM_LOCAL_INLINE FUNC(Std_ReturnType, DEM_CODE)
Dem_RecordReader_0x1904_ReadNextRecord_CustomTriggerSrec(                                                                        /* PRQA S 3219 */ /*  MD_DEM_14.1_CONFIGURATION_SWITCH  */
  Dem_DTCReadoutBuffer_HandleType ReadoutBufferId,
  Dem_Data_DestinationBufferPtrType  DestinationBuffer                                                                           /* PRQA S 3673 */ /* MD_DEM_8.13_fp */
  )
{
...
# if (DEM_CFG_SUPPORT_CUSTOMTRIGGER_SREC == STD_ON)
  ...
  if ((Dem_Core_TestCustomTriggeredSrecConfigured(lEventId) == TRUE)
      && (Dem_Cfg_ComplexIterExists(&lSourceIterator) == TRUE))                                                                  /* PRQA S 3415 */ /* MD_DEM_13.5_cf */ /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {/* Custom trigger snapshot not yet read with previous read operation */
    uint8 lRecordNumber = (uint8)Dem_Cfg_ComplexIterGet(&lSourceIterator);                                                       /* SBSW_DEM_CALL_ITERATOR_POINTER */

    if (Dem_RecordReader_0x1904_TestRecordMatch(ReadoutBufferId, lRecordNumber) == TRUE)
    {
      if (Dem_Memories_CustomTriggerMemory_TestRecordNumberIsSRec(lEventId, lRecordNumber) == TRUE)
      {
        lReturnValue = E_OK;
        if (Dem_RecordReader_DTCReadoutBuffer_CustomTriggeredRecordIsStored(ReadoutBufferId, lRecordNumber) == TRUE)
        {
          ...
          {
            Dem_ConstDataPtrType lSourceBuffer = Dem_RecordReader_DTCReadoutBuffer_GetCustomTriggerSnapshotConstPtr(ReadoutBufferId);
 ...
 */
DEM_LOCAL_INLINE FUNC(Std_ReturnType, DEM_CODE)
Dem_RecordReader_0x1904_ReadNextRecord_CustomTriggerTssr(                                                                        /* PRQA S 3219 */ /*  MD_DEM_14.1_CONFIGURATION_SWITCH  */
  Dem_DTCReadoutBuffer_HandleType ReadoutBufferId,
  Dem_Data_DestinationBufferPtrType  DestinationBuffer                                                                           /* PRQA S 3673 */ /* MD_DEM_8.13_fp */
  )
{
...
# if (DEM_CFG_SUPPORT_CUSTOMTRIGGER_TSSR == STD_ON)
  ...
  if (Dem_Core_TestCustomTriggeredTssrConfigured(lEventId) == TRUE)
  {
         ...
         Dem_Cfg_ComplexIterNext(&lSourceIterator))                                                                              /* SBSW_DEM_CALL_ITERATOR_POINTER */
    {
      ...
      if (Dem_RecordReader_0x1904_TestRecordMatch(ReadoutBufferId, lRecordNumber) == TRUE)
      {
        if (Dem_Memories_CustomTriggerMemory_TestRecordNumberIsTSSRec(lEventId, lRecordNumber) == TRUE)
        {
          lReturnValue = E_OK;
          if (Dem_RecordReader_DTCReadoutBuffer_CustomTriggeredRecordIsStored(ReadoutBufferId, lRecordNumber) == TRUE)
          {
            ...
            {
              uint8 lTimeSeriesSampleIndex = Dem_Memories_CustomTriggerMemory_GetTimeSeriesSampleIndex(lEventId, lRecordNumber);
              Dem_ConstDataPtrType lSourceBuffer = Dem_RecordReader_DTCReadoutBuffer_GetCustomTriggerTSSnapshotConstPtr(ReadoutBufferId, lTimeSeriesSampleIndex);
              Dem_Cfg_EntryIndexType lReadoutBufferEventEntryIndex = Dem_DTCReadoutBuffer_GetEventEntryIndex(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT);
 ...
 */
DEM_LOCAL FUNC(Std_ReturnType, DEM_CODE)
Dem_RecordReader_0x1904_CalculateSize_ConfiguredOrCalculatedSRecs(                                                               /* PRQA S 3219 */ /*  MD_DEM_14.1_CONFIGURATION_SWITCH  */
  Dem_DTCReadoutBuffer_HandleType ReadoutBufferId,
  Dem_EventIdType EventId,
  CONSTP2VAR(uint16, AUTOMATIC, AUTOMATIC) SizeOfFreezeFrame                                                                     /* PRQA S 3673 */ /* MD_DEM_8.13_fp */
)
{
  Std_ReturnType lReturnValue = DEM_NO_SUCH_ELEMENT;

  if (Dem_Core_Event_HasConfiguredOrCalculatedSRecs(EventId) == TRUE)
  {
#if (DEM_CFG_SUPPORT_DCM == STD_ON) && (DEM_CFG_SUPPORT_SRECS == STD_ON)
    Dem_Cfg_EntryIndexType lReadoutBufferEventEntryIndex =
      Dem_DTCReadoutBuffer_GetEventEntryIndex(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT);

    *SizeOfFreezeFrame = (uint16)(Dem_MemoryEntry_SnapshotEntry_GetNumberOfStoredSnapshotRecords(lReadoutBufferEventEntryIndex)  /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
                                   * Dem_Cfg_EventSRecUdsSize(EventId));  
    lReturnValue = E_OK;
#else
 ...
 */
DEM_LOCAL FUNC(Std_ReturnType, DEM_CODE)
Dem_RecordReader_0x1904_CalculateSize_TimeSeriesSRecs(                                                                           /* PRQA S 3219 */ /* MD_DEM_14.1_CONFIGURATION_SWITCH */
  Dem_DTCReadoutBuffer_HandleType ReadoutBufferId,
  Dem_EventIdType EventId,
  CONSTP2VAR(uint16, AUTOMATIC, AUTOMATIC) SizeOfFreezeFrame                                                                     /* PRQA S 3673 */ /* MD_DEM_8.13_fp */
)
{
  ...
  {
#if (DEM_CFG_SUPPORT_DCM == STD_ON) && (DEM_CFG_SUPPORT_SRECS == STD_ON) && (DEM_FEATURE_NEED_TIME_SERIES == STD_ON)
    *SizeOfFreezeFrame = (uint16) (Dem_RecordReader_0x1904_TimeSeriesSRecCountOfVirtualBuffer(ReadoutBufferId)                   /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
                                     * Dem_Cfg_EventSRecUdsSize(EventId));
    lReturnValue = E_OK;
#else
    DEM_IGNORE_UNUSED_ARGUMENT(EventId)                                                                                          /* PRQA S 1338, 3112 */ /* MD_MSR_DummyStmt, MD_DEM_14.2 */
  ...
  )
{
  ...
  lReturnValue = E_OK;
  lEventId = Dem_DTCReadoutBuffer_GetEventId(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT);
  lReadoutBufferEventEntryIndex = Dem_DTCReadoutBuffer_GetEventEntryIndex(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT);

  for (Dem_MemoryEntry_SnapshotEntry_IteratorInit(lEventId, lReadoutBufferEventEntryIndex, &lSnapshotEntryIter);                 /* SBSW_DEM_POINTER_FORWARD_STACK */
        Dem_MemoryEntry_SnapshotEntry_IteratorExists(&lSnapshotEntryIter) == TRUE;                                               /* SBSW_DEM_POINTER_FORWARD_STACK */
        Dem_MemoryEntry_SnapshotEntry_IteratorNext(&lSnapshotEntryIter))                                                         /* SBSW_DEM_POINTER_FORWARD_STACK */
  {
    Dem_Cfg_SRecIndexType lCfgSRecIndex;

    lCfgSRecIndex = Dem_MemoryEntry_SnapshotEntry_IteratorGetSnapshotRecordIndex(&lSnapshotEntryIter);                           /* SBSW_DEM_POINTER_FORWARD_STACK */
  ...
  if (Dem_MemoryEntry_SnapshotEntry_IteratorExists(&lSnapshotEntryIter) == FALSE)                                                /* SBSW_DEM_POINTER_FORWARD_STACK */
  {
    lReturnValue = DEM_NO_SUCH_ELEMENT;
  }
  ...
  )
{
  ...
  for (ReadoutBufferDataIndex = DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT;
       ...
       ReadoutBufferDataIndex++)
  {
    lReadoutBufferEventEntryIndex = Dem_DTCReadoutBuffer_GetEventEntryIndex(ReadoutBufferId, ReadoutBufferDataIndex);
    if (Dem_MemoryEntry_SnapshotEntry_GetSRecStorageStatus(RecordNumber, SnapshotHeaderBitMaskIndex, lReadoutBufferEventEntryIndex) == TRUE)
    {
...
{
  Std_ReturnType lReturnValue = DEM_NO_SUCH_ELEMENT;
  uint16 lAccumulatedSizeOfFreezeFrame = 0U;
  uint16 lSizeOfFreezeFrame = 0U;
  Dem_EventIdType lEventId = Dem_DTCReadoutBuffer_GetEventId(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT);

  if (Dem_RecordReader_0x1904_CalculateSize_ConfiguredOrCalculatedSRecs(ReadoutBufferId, lEventId, &lSizeOfFreezeFrame) == E_OK) /* SBSW_DEM_POINTER_FORWARD_STACK */                                                                                         /* SBSW_DEM_POINTER_FORWARD_STACK */
  {
    lReturnValue = E_OK;
    lAccumulatedSizeOfFreezeFrame += lSizeOfFreezeFrame;
  }

  if (Dem_RecordReader_0x1904_CalculateSize_TimeSeriesSRecs(ReadoutBufferId, lEventId, &lSizeOfFreezeFrame) == E_OK)             /* SBSW_DEM_POINTER_FORWARD_STACK */                                                                           /* SBSW_DEM_POINTER_FORWARD_STACK */
  {
    lReturnValue = E_OK;
    lAccumulatedSizeOfFreezeFrame += lSizeOfFreezeFrame;
  }

  if (Dem_RecordReader_0x1904_CalculateSize_TimeSeriesPowertrain(ReadoutBufferId, lEventId, &lSizeOfFreezeFrame) == E_OK)        /* SBSW_DEM_POINTER_FORWARD_STACK */
  {
    lReturnValue = E_OK;
    lAccumulatedSizeOfFreezeFrame += lSizeOfFreezeFrame;
  }

  if(Dem_RecordReader_0x1904_CalculateSize_AllCustomTriggeredRecords(ReadoutBufferId, lEventId, &lSizeOfFreezeFrame) == E_OK)    /* SBSW_DEM_POINTER_FORWARD_STACK */
  {
    lReturnValue = E_OK;
    lAccumulatedSizeOfFreezeFrame += lSizeOfFreezeFrame;
  }

  if (Dem_RecordReader_0x1904_GlobalSnapshotRecordsEnabled() == TRUE)
  {
    if (Dem_DTCReadoutBuffer_TestGlobalSnapshotStored(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT) == TRUE)
    {
      lAccumulatedSizeOfFreezeFrame += Dem_Cfg_GlobalSRecUdsSize();
    }
    lReturnValue = E_OK;
  }
  ...
  if (Dem_Cfg_IsObdOnUdsSupportedInVariant() == TRUE)
  {
    if (Dem_RecordReader_0x1904_CalculateSize_ObdOnUds(lEventId,
      Dem_DTCReadoutBuffer_GetOBDData_IsStored(ReadoutBufferId), &lSizeOfFreezeFrame) == E_OK)                                   /* SBSW_DEM_POINTER_FORWARD_STACK */
    {
      lReturnValue = E_OK;

      /* Obd on UDS, always contains two FFs 0x00, 0xF0 */
      lAccumulatedSizeOfFreezeFrame += (uint16)(lSizeOfFreezeFrame * 2u); 
    }
  }
  ...
  {
    if (Dem_RecordReader_0x1904_CalculateSize_WwhObd(lEventId, &lSizeOfFreezeFrame) == E_OK)                                     /* SBSW_DEM_POINTER_FORWARD_STACK */
    {
      lReturnValue = E_OK;
      lAccumulatedSizeOfFreezeFrame += lSizeOfFreezeFrame;
    }
  ...
  if (lReturnValue == E_OK)
  {
    ...
    *SizeOfFreezeFrame = lAccumulatedSizeOfFreezeFrame;                                                                          /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
  }
  ...
  else
  {
    if ( (Dem_DTCReadoutBuffer_GetOBDData_IsStored(ReadoutBufferId) == TRUE))
    {
      ...
      else
      {
        ...
        /* Read Freeze Frame */
        SRecNumber = Dem_Memories_ObdFreezeFrameMemory_GetObdOnUdsSRecNumberByFreezeFrameType(FreezeFrameType);
  ...
  )
{
  ...
  {
    Dem_Cfg_EntryIndexType lEventEntryIndex = Dem_Memories_EventMemory_FindEntryOfEventAtEventDestination(lEventId, FALSE);
    lReturnValue = E_OK;
...
# if (DEM_FEATURE_NEED_AGING_REALLOCATION == STD_ON)                                                                             /* COV_DEM_UNSUPPORTED_FEATURE TX */
      if (DEM_EVENTENTRY_TEST_AGING_ONLY(Dem_MemoryEntry_EventEntry_GetState(lEventEntryIndex)) == FALSE)
# endif
      {
        ...
        {
          Dem_Cfg_EntryIndexType lReadoutBufferEventEntryIndex = Dem_DTCReadoutBuffer_GetEventEntryIndex(
            ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT);
          Dem_Data_WriteDestinationBufferUint8(DestinationBuffer, 0x00u);                                                        /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
          
          Dem_DataReport_WWHOBDFreezeFrameCopyData(DestinationBuffer,                                                            /* SBSW_DEM_POINTER_DESTINATIONBUFFER_SOURCEBUFFER */
            (Dem_ConstSharedDataPtrType) Dem_MemoryEntry_EventEntry_GetWwhObdFreezeFrameDataPtr(lReadoutBufferEventEntryIndex),
            lEventId,
            lReadoutBufferEventEntryIndex,
            ReadoutBufferId);
        }
  ...
  )
{
  ...
  else
  {
    ...
    { /* process request if Freeze Frame is stored */
# if (DEM_FEATURE_NEED_AGING_REALLOCATION == STD_ON)                                                                             /* COV_DEM_UNSUPPORTED_FEATURE TX */
      if (DEM_EVENTENTRY_TEST_AGING_ONLY(Dem_MemoryEntry_EventEntry_GetState(lEventEntryIndex)) == TRUE)
      {
        *SizeOfFreezeFrame = 0u;                                                                                                 /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
      }
      else
 ...
 */
DEM_LOCAL_INLINE FUNC(Std_ReturnType, DEM_CODE)
Dem_RecordReader_0x1904_CalculateSize_CustomTriggerRecord(                                                                       /* PRQA S 3219 */ /* MD_DEM_14.1_CONFIGURATION_SWITCH */
  Dem_DTCReadoutBuffer_HandleType ReadoutBufferId,
  Dem_EventIdType  EventId,
  uint8  RecordNumber,
  CONSTP2VAR(uint16, AUTOMATIC, DEM_SHARED_DATA)  SizeOfFreezeFrame                                                              /* PRQA S 3673 */ /* MD_DEM_8.13_fp */
  )
{
  Std_ReturnType lReturnValue = E_OK;
  uint16 lSizeOfFreezeFrame = 0u;
# if (DEM_FEATURE_NEED_CUSTOMTRIGGERSTORAGE == STD_ON)
  if (Dem_RecordReader_DTCReadoutBuffer_CustomTriggeredRecordIsStored(ReadoutBufferId, RecordNumber) == TRUE)
  { /* process request if Freeze Frame is stored */
    lSizeOfFreezeFrame = Dem_Cfg_EventSRecUdsSize(EventId);
  }
# else
 ...
 */
DEM_LOCAL_INLINE FUNC(Std_ReturnType, DEM_CODE)
Dem_RecordReader_0x1904_CalculateSize_AllCustomTriggeredRecords(                                                                 /* PRQA S 3219 */ /* MD_DEM_14.1_CONFIGURATION_SWITCH */
  Dem_DTCReadoutBuffer_HandleType ReadoutBufferId,
  Dem_EventIdType  EventId,
  CONSTP2VAR(uint16, AUTOMATIC, DEM_SHARED_DATA)  SizeOfFreezeFrame                                                              /* PRQA S 3673 */ /* MD_DEM_8.13_fp */
  )
...
# if (DEM_FEATURE_NEED_CUSTOMTRIGGERSTORAGE == STD_ON)
  ...
  {
    lReturnValue = E_OK;
    *SizeOfFreezeFrame = (uint16)(Dem_RecordReader_DTCReadoutBuffer_GetNumberofStoredCustomTriggeredRecords(ReadoutBufferId)     /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
      * Dem_Cfg_EventSRecUdsSize(EventId));
  }
# endif
  DEM_IGNORE_UNUSED_CONST_ARGUMENT(ReadoutBufferId)                                                                              /* PRQA S 3112 */ /* MD_DEM_14.2 */
...
{
  uint16 lSRecUdsSize = (uint16)0x00;

#if ((DEM_FEATURE_NEED_TIME_SERIES == STD_ON) && (DEM_CFG_SUPPORT_TIME_SERIES_POWERTRAIN == STD_ON))
  Dem_Cfg_SamplingProfileIterType lSamplingProfileId = Dem_Memories_TimeSeriesMemory_GetSamplingProfileId(EventId, 0);
  uint16 freezeFrameTableIndex = Dem_Cfg_GetFreezeFrameTableDevFFBIdxOfEventTable(EventId);
  uint16 i;
  for(i = Dem_Cfg_GetPowerTrainTimeSeriesUdsSizeTableStartIdxOfFreezeFrameTable(freezeFrameTableIndex);                          /* COV_DEM_ROBUSTNESS TX */
      i < Dem_Cfg_GetPowerTrainTimeSeriesUdsSizeTableEndIdxOfFreezeFrameTable(freezeFrameTableIndex);
      i++)
  {
    if(lSamplingProfileId == Dem_Cfg_GetTimeSeriesSamplingProfileTableIdxOfPowerTrainTimeSeriesUdsSizeTable(i))
    {
      lSRecUdsSize = Dem_Cfg_GetUdsRecordSizeOfPowerTrainTimeSeriesUdsSizeTable(i);
      break;
    }
  }
#else 
...
)
{
  ...
  if (Dem_DTCReadoutBuffer_GetTimeSeriesEntryIndex(ReadoutBufferId) != DEM_CFG_ENTRYINDEX_INVALID)
  {
    ...
    Dem_Cfg_EntryIndexType lTimeSeriesEntryIndex = Dem_DTCReadoutBuffer_GetTimeSeriesEntryIndex(ReadoutBufferId);
    Dem_TimeSeriesEntry_PtrToConstBaseType lTimeSeriesEntry = Dem_MemoryEntry_TimeSeriesEntry_GetBaseEntryPtrSafe(lTimeSeriesEntryIndex);
    Dem_EventIdType lEventId = Dem_DTCReadoutBuffer_GetEventId(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT);
    ...
    for(lSeriesId = 0; lSeriesId < lNumberOfSeries; lSeriesId++)
    {
      ...
      for (Dem_Memories_TimeSeriesMemory_TssSetIter_Init(lTimeSeriesEntryIndex, lSeriesId, &lIter);                              /* SBSW_DEM_POINTER_FORWARD_STACK */
      ...
      {
        lReturnValue += Dem_Cfg_GetFutureSamplesOfTimeSeriesSamplingProfileTable(lSamplingProfileId) + Dem_Cfg_GetPastSamplesOfTimeSeriesSamplingProfileTable(lSamplingProfileId);
        lReturnValue -= 
          (Dem_MemoryEntry_TimeSeriesEntry_GetUnusedPastSamples(lTimeSeriesEntry, lSeriesId, lIter.TssSetId) +                   /* SBSW_DEM_POINTER_READ_TIMESERIES_BASEENTRY */
            Dem_DTCReadoutBuffer_GetUnusedFutureSamples(ReadoutBufferId, lSeriesId, lIter.TssSetId));
      }
    }
  }
...
)
{
  ...
  if (Dem_DTCReadoutBuffer_GetTimeSeriesEntryIndex(ReadoutBufferId) != DEM_CFG_ENTRYINDEX_INVALID)
  {
    ...
    lIter = Dem_Memories_TimeSeriesMemory_InitializeTimeSeriesRecordIteratorByRecordNumber(lEventId, RecordNumber);

    if(Dem_Memories_TimeSeriesMemory_TestEventSupportTimeSeries(lEventId) && Dem_Memories_TimeSeriesMemory_TestRecordNumberIsInSeries(lEventId, lIter.SeriesId, lIter.RecordNumber))
    {
      if (lIter.ChronologicalId < Dem_Memories_TimeSeriesMemory_GetNumberOfStoredTimeSeriesSets(lTimeSeriesEntryIndex, lIter.SeriesId))
      {
        uint8 lSnapshotPositionInSet = Dem_Memories_TimeSeriesMemory_GetSnapshotOffsetFromFirstOneInSet(lIter.EventId, lIter.SeriesId, lIter.RecordNumber);

        Dem_Cfg_SamplingProfileIterType lSamplingProfileId = Dem_Memories_TimeSeriesMemory_GetSamplingProfileId(lIter.EventId, lIter.SeriesId);
        ...
        DEM_IGNORE_UNUSED_VARIABLE(lSamplingProfileId);                                                                          /* PRQA S 1338, 3112 */ /* MD_MSR_DummyStmt, MD_DEM_14.2 */ 
        if (lSnapshotPositionInSet >= lPastSamples)
        {
          /* Future Sample */
          lSnapshotPositionInSet -= lPastSamples;
          lIsStored = (lSnapshotPositionInSet < (lFutureSamples - Dem_DTCReadoutBuffer_GetUnusedFutureSamples(ReadoutBufferId, lIter.SeriesId, lSetId)));
        }
        else
        {
          /* Past Sample */
          lIsStored = (lSnapshotPositionInSet >= Dem_MemoryEntry_TimeSeriesEntry_GetUnusedPastSamples(lTimeSeriesEntry, lIter.SeriesId, lSetId)); /* SBSW_DEM_POINTER_READ_TIMESERIES_BASEENTRY */
        }
      }
    }
  }
 ...
 */
DEM_LOCAL_INLINE FUNC(boolean, DEM_CODE)
Dem_RecordReader_0x1904_GlobalSnapshotRecordsEnabled(                                                                            /* PRQA S 3219 */ /*  MD_DEM_14.1_CONFIGURATION_SWITCH */
  void
  )
{
  return (boolean)(DEM_CFG_SUPPORT_GLOBAL_SRECS == STD_ON);
}
...
)
{
  ...
  {
    if ( (RecordNumber == DEM_RECORDREADER_SNAPSHOTDATARECORD_OBD)
      || (RecordNumber == DEM_RECORDREADER_SNAPSHOTDATARECORD_OBDONUDS_UPDATEABLE) )
    {
    ...
    {
      /* UDS format must be used for non-OBD relevant SRecs */
