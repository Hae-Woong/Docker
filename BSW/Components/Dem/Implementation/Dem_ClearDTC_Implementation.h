 ...
 *****************************************************************************/
/*!
 * \brief         Test if clearing of all WWH-OBD or OBD on UDS or ZEV on UDS
 *                related DTCs is allowed.
 *
 * \details       Test if clearing of all WWH-OBD or OBD on UDS or ZEV on UDS
 *                related DTCs is allowed.
 *
 * \return        TRUE
...
/* ****************************************************************************
 * Dem_ClearDTC_TestClearObdIIAllDTCsAllowed()
 *****************************************************************************/
/*!
 * \brief         Test if clearing of all OBDII related DTCs is allowed.
 *
 * \details       Test if clearing of all OBDII related DTCs is allowed.
 *
 * \return        TRUE
 *                Clearing of OBDII DTCs is allowed
 * \return        FALSE
 *                Clearing of OBDII DTCs is not allowed
 *
 * \pre           -
 *
 * \context       TASK|ISR2
 ...
 * \reentrant     TRUE, for different ClearDTCHandle
 *****************************************************************************/
DEM_LOCAL_INLINE FUNC(boolean, DEM_CODE) Dem_ClearDTC_TestClearObdIIAllDTCsAllowed(
  void
  );
...
#if (DEM_CFG_CLEAR_EMISSIONS_SYSTEM_GROUP_DTCS == STD_ON)
  if ( (Dem_Cfg_IsWwhObdSupportedInVariant() == TRUE)
    || (Dem_Cfg_IsObdOnUdsSupportedInVariant() == TRUE)
    || (Dem_Cfg_IsZevOnUdsSupportedInVariant() == TRUE) )
  {
    lReturnValue = TRUE;
  }
  else
 ...
 *                DEM_DTCSELECTOR_GROUP_OBD_ALL_DTC
 *                  - All WWH-OBD or OBD on UDS or or ZEV on UDS DTCs.
  ...
  )
{
  ...
  if (Dem_DTCSelector_GetGroupId(DTCSelector) == DEM_DTCSELECTOR_GROUP_OBD_ALL_DTC)
  {
  ...
  {
    if ((Dem_ClearDTC_TestClearObdIIAllDTCsAllowed() == TRUE)
      && (ClearRequest->ClearMemoryId == DEM_CFG_MEMORYID_PRIMARY))
    {
      lReturnValue = E_OK;
    }
...
/* ****************************************************************************
 % Dem_Client_ClearDTC_StartClearSingleDtcJ1939
 *****************************************************************************/
 /*!
  * Internal comment removed.
 *
 ...
 *
  *****************************************************************************/
DEM_LOCAL_INLINE FUNC(Std_ReturnType, DEM_CODE) Dem_Client_ClearDTC_StartClearSingleDtcJ1939(                                    /* PRQA S 3219 */ /* MD_DEM_14.1_CONFIGURATION_SWITCH */
  Dem_ClearDTC_HandleType ClearDTCHandle,
  Dem_DTCSelector_HandleType DTCSelector,
  uint8 DTCTypeFilter,
  uint8 Node
)
{
  Dem_EventIdType lSelectedEventId = Dem_DTCSelector_GetEventId(DTCSelector);
  ...
  if (Dem_DTC_TestSuppressionOnDtcRetrieval(lSelectedEventId) == FALSE)
  {
    lClearRequest.ClearEventId = lSelectedEventId;
...
/* ****************************************************************************
 % Dem_Client_ClearDTC_StartClearAllDtcsJ1939
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
 *
 *****************************************************************************/
DEM_LOCAL_INLINE FUNC(Std_ReturnType, DEM_CODE) Dem_Client_ClearDTC_StartClearAllDtcsJ1939(                                      /* PRQA S 3219 */ /* MD_DEM_14.1_CONFIGURATION_SWITCH */
  Dem_ClearDTC_HandleType ClearDTCHandle,
  Dem_DTCSelector_HandleType DTCSelector,
  uint8 DTCTypeFilter,
  uint8 Node
  ...
  )
{
  ...
  lClearRequest.ClearMemoryId = Dem_DTCSelector_GetMemoryId(DTCSelector);
  lClearRequest.ClearNode = Dem_Cfg_GetInvalidJ1939NodeIndex();
