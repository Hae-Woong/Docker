...
DEM_LOCAL VAR(uint16, DEM_VAR_NO_INIT) Dem_Mar_NumOfPendingEvents;
  ...
  if (Dem_Mar_IsEnabled() == TRUE)
  {
         ...
         lEventId++)
    {
      if ((Dem_Core_DTC_IsObdRelated(lEventId) == TRUE)
        && (Dem_GlobalDiagnostics_TestEventHasMilAssigned(lEventId) == TRUE)
        && (Dem_Event_UDSStatus_TestBit(Dem_Event_UDSStatus_GetInternalEventUdsStatus(lEventId), Dem_UdsStatus_PDTC) == TRUE))
      {
        ++lEventCounter;
      }
    }
...
)
{
  if (Dem_Mar_IsEnabled() == TRUE)
  {
    if ((Dem_Core_DTC_IsObdRelated(EventId) == TRUE)
      ...
      && (Dem_Event_UDSStatus_TestBit(Dem_Event_UDSStatus_GetInternalEventUdsStatus(EventId), Dem_UdsStatus_PDTC) == FALSE))
    {
      Dem_Mar_IncrementPendingEvents();
    }
  }
...
)
{
  if (Dem_Mar_IsEnabled() == TRUE)
  {
    if ((Dem_Core_DTC_IsObdRelated(EventId) == TRUE)
      && (Dem_GlobalDiagnostics_TestEventHasMilAssigned(EventId) == TRUE)
      && (Dem_Event_UDSStatus_TestBit(Dem_Event_UDSStatus_GetInternalEventUdsStatus(EventId), Dem_UdsStatus_PDTC) == TRUE))
    {
      Dem_Mar_DecrementPendingEvents();
    }
  }
