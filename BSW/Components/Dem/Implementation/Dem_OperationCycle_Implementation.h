...
DEM_LOCAL VAR(uint16, DEM_VAR_NO_INIT)               Dem_OperationCycle_CurrentCycleState;
/*! Pending operating cycle states - protected by critical section DEM_EXCLUSIVE_AREA_1 (StateManager) */
DEM_LOCAL VAR(uint16, DEM_VAR_NO_INIT)               Dem_OperationCycle_PendingCycleState;
/*! Operating cycle states update flags - protected by critical section DEM_EXCLUSIVE_AREA_1 (StateManager) */
DEM_LOCAL volatile VAR(uint16, DEM_VAR_NO_INIT)      Dem_OperationCycle_CycleFlag;
/*! DCY qualification flag */
DEM_LOCAL VAR(boolean, DEM_VAR_NO_INIT)              Dem_OperationCycle_DcyCycleFlag;
...
/*!
 * \defgroup Dem_OperationCycle_PrivateProperties Private Properties
 * \{
 */

/* ****************************************************************************
 * Dem_OperationCycle_GetAgingCycleId
 *****************************************************************************/
/*!
...
Dem_OperationCycle_GetCurrentAgingCycleCount(
  Dem_EventIdType EventId
)
{
  return Dem_OperationCycle_GetCycleCounter(Dem_OperationCycle_GetAgingCycleId(EventId));
}
...
/* ****************************************************************************
 * Dem_OperationCycle_CycleUpdateRdyThisDcy
 *****************************************************************************/
/*!
 * \brief         Updates readiness this driving cycle when cycle is started.
 *
 * \details       Processes readiness this driving cycle (PID41, DM26) when
 *                cycle is started.
 *
 * \param[in]     CycleId
 *                Unique Id of the cycle
...
#if ((DEM_FEATURE_NEED_EMISSIONOBD == STD_ON) && (DEM_FEATURE_NEED_CYCLEAUTOMATICEND == STD_ON))
  ...
  )
{
        ...
        Dem_Cfg_CombinedGroupIterNext(&lCombinedIter))                                                                           /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    {
      Dem_FaultMemory_StorageManager_UpdateCycleCounterOfEvent(lSubEventId, CycleStatus);
    }
  }
...
)
{
  ...
  lUDSStatus = Dem_DTC_ApplyCombinedStatus(Dem_Core_Event_GetInternalUdsStatus(EventId));

  if (((CycleStatus & DEM_OPERATIONCYCLE_CYCLE_STOPPED) != 0u)
    && (Dem_Event_UDSStatus_TestBit(lUDSStatus, Dem_UdsStatus_TNCTOC) == FALSE))                                                                         
  {
    if (Dem_Event_UDSStatus_TestBit(lUDSStatus, Dem_UdsStatus_TFTOC) == FALSE)
  ...
  )
{
...

  {
...
#endif
    {
      ...
      {
        Dem_FaultMemory_StorageManager_UpdateCycleCounterOfEvent(EventId, CycleStatus);
      }
    }
  }

  lOldDtcStatus = Dem_DTC_GetInternaUDSStatus(EventId);
  lOldExternalDtcStatus = Dem_DTC_GenerateExternalOnlyStatus(EventId, lOldDtcStatus);
  Dem_DTC_OpCycle(EventId, CycleStatus);
  lNewDtcStatus = Dem_DTC_GetInternaUDSStatus(EventId);
  ...
  if (Dem_DTC_TestEventStoresPermanentDTC(EventId) == TRUE)
  {
    ...
    {
      Dem_Memories_PermanentMemory_UpdatePermanentEntryOBD(EventId, lOldDtcStatus, lNewDtcStatus);
    }
    else if ((Dem_Cfg_IsWwhObdSupportedInVariant() == TRUE))
    {
      Dem_Memories_PermanentMemory_UpdatePermanentEntryWWHOBD(EventId, lOldDtcStatus);
    }
    else
    {
      /* MISRA case */
  ...
  )
{
  ...
  boolean lCycleStopped = (CycleStatus & DEM_OPERATIONCYCLE_CYCLE_STOPPED) != 0U;

  /* Perform transitions due to cycle end or tick */
  if (Dem_Cfg_IsPermanentSupportedInVariant() == TRUE)
  {
    lOldPermanentCount = Dem_FaultMemory_Memories_GetNumAllocatedMemoryEntry(DEM_CFG_MEMORYID_PERMANENT);
  }
  else
  {
  ...
  {
    if ((CycleId == Dem_OperationCycle_GetAgingCycleId(lEventId)) && (lCycleStopped == TRUE))
    {
      Dem_Core_Aging_ProcessAgingCycleEnd(lEventId);
    }
...
# if (DEM_FEATURE_NEED_MIL_GROUPS == STD_ON)
  /* ----- MIL group handling at DCY restart ----- */
  {
    ...
    while (MilGroupIndex != DEM_CFG_MIL_GROUP_INVALID)
    {
      ...
      if (Dem_Event_InternalStatus_GetMilGroupPendingCount(MilGroupIndex) == 0u)
      {
...
{
#if (DEM_CFG_SUPPORT_DELAY_WUC_TO_DCY == STD_OFF)
  ...
  )
{
  ...
  if (Dem_Cfg_IsEmissionObdSupportedInVariant() == TRUE)
  {
    if ((CycleStatus & DEM_OPERATIONCYCLE_CYCLE_STOPPED) != 0U)
    {
      Dem_DiagnosticObserver_ProcessOBDCycleEnd(CycleId);
      Dem_Core_Event_ProcessOBDCycleEnd(CycleId);
    }
  ...
  )
{
  ...
  if ((CycleStatus & DEM_OPERATIONCYCLE_CYCLE_STARTED) != 0U)
  {
    Dem_OperationCycle_CycleUpdateRdyThisDcy(CycleId);
  }
  ...
  )
{
  ...
  if (Dem_Cfg_IsEmissionObdSupportedInVariant() == TRUE)
  {
    /* In EmissionOBD configurations, make sure the OBD DCY is in state 'started' */
    Dem_OperationCycle_CurrentCycleState = Dem_OperationCycle_CurrentCycleState | ((uint16)1 << Dem_Cfg_GlobalObdDrivingCycleId());
...
# endif
      ...
      for (lEventId = DEM_EVENT_FIRST; lEventId < Dem_Cfg_GlobalEventCount(); ++lEventId)
      {
        if (Dem_Cfg_EventOperationCycle(lEventId) == Dem_Cfg_GlobalObdDrivingCycleId())
        {
          Dem_Event_ExtendedStatus_SetEventStatus(lEventId,
            Dem_Event_ExtendedStatus_ResetBit(Dem_Event_ExtendedStatus_GetEventStatus(lEventId), Dem_ExtendedStatus_SimilarConditions)
          );
        }
      }
    }
