...
/* ****************************************************************************
 * Dem_Iumpr_Denominator_TestIncrementDisabledByPto
 *****************************************************************************/
/*!
 * \brief         Test whether the denominator increment is disabled by PTO
 *
 * \details       Test whether the denominator increment is disabled by PTO
 *
 * \param[in]     RatioIndex
 *                Unique handle of the Ratio.
 *
 * \return        TRUE
 *                The ratio is affected by PTO and the global denominator
 *                conditions have only been reached while PTO was active.
 * \return        FALSE
 *                The ratio is not affected by PTO or the global denominator
 *                conditions have been reached while PTO was inactive.
 *
 * \pre           -
 *
 * \context       TASK|ISR2
  ...
  )
{
  ...
  if ((Dem_Iumpr_Ratio_Denominator_TestIncremented(RatioIndex) == FALSE)
    ...
    && (Dem_Iumpr_Denominator_TestIncrementDisabledByPto(RatioIndex) == FALSE))
  {
...
)
{
  ...
  Dem_EventIdType lEventId = Dem_Iumpr_Ratio_GetRelatedEvent(RatioIndex);

  if ( (Dem_Iumpr_Ratio_Denominator_TestIncremented(RatioIndex) == FALSE)
    && ( (Dem_Core_Event_TestEventAffectedByPto(lEventId) == FALSE)
  ...
  )
{
  ...
  if (Dem_Core_Event_TestEventAffectedByPto(Dem_Iumpr_Ratio_GetRelatedEvent(RatioIndex)))
  {
    switch (Dem_Iumpr_Ratio_Denominator_GetDenominatorGroup(RatioIndex))
    {
    case DEM_IUMPR_DEN_NONE:
    case DEM_IUMPR_DEN_PHYS_API:
      lReturnValue = (Dem_Iumpr_ConditionStatus_TestReachedWhilePtoInactive(DEM_IUMPR_GENERAL_INDIVIDUAL_DENOMINATOR) == FALSE);
      break;
    case DEM_IUMPR_DEN_COLDSTART:
      lReturnValue = ((Dem_Iumpr_ConditionStatus_TestReachedWhilePtoInactive(DEM_IUMPR_GENERAL_INDIVIDUAL_DENOMINATOR) == FALSE) ||
                       (Dem_Iumpr_ConditionStatus_TestReachedWhilePtoInactive(DEM_IUMPR_DEN_COND_COLDSTART) == FALSE));
      break;
    case DEM_IUMPR_DEN_EVAP:
      lReturnValue = ((Dem_Iumpr_ConditionStatus_TestReachedWhilePtoInactive(DEM_IUMPR_GENERAL_INDIVIDUAL_DENOMINATOR) == FALSE) ||
                       (Dem_Iumpr_ConditionStatus_TestReachedWhilePtoInactive(DEM_IUMPR_DEN_COND_EVAP) == FALSE));
      break;
    case DEM_IUMPR_DEN_INDEPENDENT:
      lReturnValue = (Dem_Iumpr_ConditionStatus_TestReachedWhilePtoInactive(DEM_IUMPR_DEN_COND_INDEPENDENT) == FALSE);
      break;
    default:                                                                                                                     /* COV_DEM_MISRA_BRANCH */
      /* Calls for denominator group 500mi not necessary. Instead of the group specific state, a ratio specific reached state is checked. */
      /* The state can only be set if the related event is not affected by PTO or the conditions were reached while PTO was inactive. */
      /* MISRA Case */
      break;
    }
  }
