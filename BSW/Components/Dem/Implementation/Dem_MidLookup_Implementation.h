...
/*! Visibility change of DTR(s) is pending. Used to serialize recalculation of MID visibility when TIDs change visibility. */
DEM_LOCAL volatile VAR(boolean, DEM_VAR_NO_INIT)  Dem_MidLookup_VisibilityChanged;
...
DEM_LOCAL VAR(uint32, DEM_VAR_NO_INIT)  Dem_MidLookup_SupportedMidMask[DEM_MIDLOOKUP_MAX_NUMBER_SUPPORTED_MIDS];
#endif
...
DEM_LOCAL VAR(Dem_MidLookup_CacheType, DEM_VAR_NO_INIT)  Dem_MidLookup_Cache;                                                    /* PRQA S 3218 */ /* MD_DEM_8.7 */
  ...
  )
{
  if (Dem_DtrData_IsDtrEnabled())
  {
    ...
    {
      uint8 lMid = 0u;
      boolean lTidVisibleInCurrentMid = FALSE;
      uint8 lSupportedMidIndex = 0u;
      uint16 lMaxSupportedMidIndex = 0;
      Dem_ComplexIterType lMidIter = { 0 };

      Dem_MidLookup_ResetVisibilityChanged();

      for (; lSupportedMidIndex < DEM_MIDLOOKUP_MAX_NUMBER_SUPPORTED_MIDS;
        ++lSupportedMidIndex)
      {
        ...
        Dem_MidLookup_MidIterNext(&lMidIter))                                                                                    /* SBSW_DEM_CALL_ITERATOR_POINTER */
      {
        ...
        if (lMid != lLastMid)
        {
          lTidVisibleInCurrentMid = FALSE;
        }

        if (lTidVisibleInCurrentMid == FALSE)
        {
          DTRIdType lDtrId = Dem_MidLookup_GetMidDtrId(lMidIndex);
          /* >>>> -------------------------------- Enter Critical Section: DiagMonitor */
          Dem_EnterCritical_DiagMonitor();
          if (Dem_DtrData_TestDtrVisibility(lDtrId) == TRUE)
          {
            uint32 lSupportedMid = 0u;
            uint8 lSupportedMidBitIndex = Dem_MidLookup_GetSupportedMidBitPositionOfMid(lMid);
            lSupportedMidIndex = Dem_MidLookup_GetSupportedMidIndexOfMid(lMid);
            lTidVisibleInCurrentMid = TRUE;
