  ...
  )
{
  ...
  if ((Dem_SatelliteSat_IsLRTimeDebouncingEnabled() == TRUE) || (Dem_SatelliteSat_IsHRTimeDebouncingEnabled() == TRUE))          /* COV_DEM_ROBUSTNESS TX tf tx */
  {
    lReturnValue = (uint16)(Dem_Cfg_GetFailTimerCyclesOfDebounceTable(Dem_Cfg_GetDebounceTableIdxOfDebounceIndexTable(EventId)));
  }
  else
  {
  lReturnValue = 1u;
  ...
  )
{
  ...
  if((Dem_SatelliteSat_IsLRTimeDebouncingEnabled() == TRUE) || (Dem_SatelliteSat_IsHRTimeDebouncingEnabled() == TRUE))           /* COV_DEM_ROBUSTNESS TX tf tx */
  {
    lReturnValue = (uint16)(Dem_Cfg_GetPassTimerCyclesOfDebounceTable(Dem_Cfg_GetDebounceTableIdxOfDebounceIndexTable(EventId)));
  }
  else
  {
    lReturnValue = 1u;                                                                                                    
  ...
  )
{
  ...
  if ((Dem_SatelliteSat_IsLRTimeDebouncingEnabled() == TRUE) || (Dem_SatelliteSat_IsHRTimeDebouncingEnabled() == TRUE))          /* COV_DEM_ROBUSTNESS TX tf tx */
  {
    lReturnValue = (uint16)(Dem_Cfg_GetStorageTimerCyclesOfDebounceTable(Dem_Cfg_GetDebounceTableIdxOfDebounceIndexTable(EventId)));
  }
  else
  {
    lReturnValue = 0u;
  ...
  )
{
  ...
  case DEM_DEBOUNCE_TIMEBASED_DONE_PASSED:
    if (Dem_Cfg_EventDebounceContinuous(DebounceInfoData->Base.EventId) == TRUE)
    {
      ...
      Allow for processing the passed result qualification with next 'Prepassed'*/
      if (Dem_GlobalDiagnostics_TestEventEnableConditionsSatisfied(DebounceInfoData->Base.EventId) == TRUE)
      {
  ...
  case DEM_DEBOUNCE_TIMEBASED_DONE_FAILED:
    if (Dem_Cfg_EventDebounceContinuous(DebounceInfoData->Base.EventId) == TRUE)
    {
      /*Rare case with continuous debouncing in which a qualified failed result should
        ...
        Allow for processing the failed result qualification with next 'Prefailed'*/
      if (Dem_GlobalDiagnostics_TestEventEnableConditionsSatisfied(DebounceInfoData->Base.EventId) == TRUE)
      {
...
{
  Dem_EventIdType lEventId = Dem_DebounceBase_GetEventId(&(DebounceInfoData->Base));                                             /* SBSW_DEM_DEBOUNCETIMEBASED_INFO_BASE_PTR */
  boolean lEnableConditionsSatisfied = Dem_GlobalDiagnostics_TestEventEnableConditionsSatisfied(lEventId);
...
{
  Dem_EventIdType lEventId = Dem_DebounceBase_GetEventId(DebounceInfoPtr);                                                       /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  boolean lReturnValue;

  if (Dem_DebounceTimeBased_CheckDebounceInfoPtr((Dem_DebounceBase_ConstInfoPtrType)DebounceInfoPtr) == TRUE)                    /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  {
    Dem_DebounceTimeBased_InfoPtrType lDebounceInfo = Dem_DebounceTimeBased_Base2TimeBased(DebounceInfoPtr);                     /* SBSW_DEM_CALL_DEBOUNCETIMEBASED_INFOPTR_INIT */
    boolean lEnableConditionsSatisfied = Dem_GlobalDiagnostics_TestEventEnableConditionsSatisfied(lEventId);
...
{
  Dem_DebouncingStateType lDebouncingState = Dem_DebouncingState_GetInitValue();
  Dem_EventIdType lEventId = Dem_DebounceBase_GetEventId(DebounceInfoPtr);                                                       /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  boolean lEnableConditionsSatisfied = Dem_GlobalDiagnostics_TestEventEnableConditionsSatisfied(lEventId);
  boolean lStorageConditionsSatisfied = Dem_GlobalDiagnostics_TestEventStorageConditionsSatisfied(lEventId);
  ...
  case DEM_DEBOUNCE_TIMEBASED_DONE_FAILED:
    ...
    if ((lEnableConditionsSatisfied == TRUE) && (lStorageConditionsSatisfied == TRUE))
    {
