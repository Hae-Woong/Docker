...
DEM_LOCAL VAR(Dem_IumprFilter_InfoType, DEM_VAR_NO_INIT)  Dem_Iumpr_FilterInfo;
 ...
 *
 * \return        TRUE
 *                Hybrid ignition cycle counter was incremented
 * \return        FALSE
 *                Hybrid ignition cycle counter was not incremented
 *
 * \pre           -
 *
 * \context       TASK|ISR2
 ...
 *
 * \pre           Feature IUMPR and hybrid ignition cycle is supported
 *
 * \context       TASK|ISR2
 * \synchronous   TRUE
 * \reentrant     TRUE
...
/* ****************************************************************************
 * Dem_Iumpr_CalculateCombinedDtcWorstObdRatio
 *****************************************************************************/
/*!
 * \brief         Function retrieves IUMPR numerator and denominator of the
 *                worst ratio of a combined Dtc.
 *
 * \details       Function retrieves IUMPR numerator and denominator of the
 *                worst ratio of all sub-events of a combined Dtc.
 *
 * \param[in]     GroupId
...
/* ****************************************************************************
 * Dem_Iumpr_ConditionStatus_SetReachedWhilePtoInactive
 *****************************************************************************/
/*!
 * \brief         Set global denominator conditions reached while PTO inactive
 *                state
 *
 * \details       Set the state that the global denominator conditions were
 *                reached while PTO was inactive
 *
 * \param[in]     ConditionId
 *                The denominator condition id.
 *
 * \pre           -
 *
 * \context       TASK|ISR2
...
/* ****************************************************************************
 * Dem_Iumpr_ConditionStatus_ResetReachedWhilePtoInactive
 *****************************************************************************/
/*!
 * \brief         Reset global denominator conditions reached while PTO inactive
 *                state
 *
 * \details       Reset global denominator conditions reached while PTO inactive
 *                state to indicate that the conditions were reached while
 *                PTO was inactive
 *
 * \param[in]     ConditionId
 *                The denominator condition id.
...
)
{
  ...
  {
    case DEM_IUMPR_GENERAL_OBDCOND:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) || (DEM_CFG_DATA_MONITOR_ACTIVITY_DATA == STD_ON))
      /* For general OBD denominator this bit means that denominator is incremented */
...
)
{
  ...
  if (ConditionId == DEM_IUMPR_GENERAL_INDIVIDUAL_DENOMINATOR)
  {
    ...
    if ( (Dem_GlobalDiagnostics_IsPtoStatusSupportEnabled() == TRUE)
      && (Dem_Iumpr_ConditionStatus_TestReachedWhilePtoInactive(ConditionId) == TRUE) )
    {
      Dem_Iumpr_ConditionStatus_ResetReachedWhilePtoInactive(ConditionId);
    }
...
{
  Dem_Cfg_ComplexIterType lCombinedIter;
  uint16 lNumerator = 0x00U;
  uint16 lDenominator = 0x00U;
  Dem_RatioIdIndexType lWorstRatioIndex = DEM_CFG_RATIOINDEX_INVALID;

  for (Dem_Cfg_CombinedGroupIterInit(GroupId, &lCombinedIter);                                                                   /* SBSW_DEM_CALL_ITERATOR_POINTER */
       Dem_Cfg_ComplexIterExists(&lCombinedIter) == TRUE;                                                                        /* PRQA S 2994 */ /* MD_DEM_2994 */ /* SBSW_DEM_CALL_ITERATOR_POINTER */
       Dem_Cfg_ComplexIterNext(&lCombinedIter))                                                                                  /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    Dem_EventIdType lSubEventId = Dem_Cfg_CombinedGroupIterGet(&lCombinedIter);                                                  /* SBSW_DEM_CALL_ITERATOR_POINTER */
    Dem_RatioIdIndexType lRatioIndex = Dem_Event_GetRatioIdx(lSubEventId);

    if (Dem_Event_TestEventUnavailable(lSubEventId) == FALSE)
    {
      if (lWorstRatioIndex == DEM_CFG_RATIOINDEX_INVALID)
      {
        /* First available ratio of the combined group was found.No need to compare it. */
        lWorstRatioIndex = lRatioIndex;
      }
      else
      {
        lWorstRatioIndex = Dem_DiagnosticObserver_Iumpr_CompareAndGetWorseObdRatio(
          lWorstRatioIndex,
          lRatioIndex);
      }

      Dem_Iumpr_Ratio_GetRatio(lWorstRatioIndex, &lNumerator, &lDenominator);                                                    /* SBSW_DEM_POINTER_FORWARD_STACK */
  ...
  *Numerator = lNumerator;                                                                                                       /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
  *Denominator = lDenominator;                                                                                                   /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
 ...
 */
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_Iumpr_CycleCounter_ResetHybridCycleCounter(                                                                                  /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  void
  )
{
 ...
 */
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_Iumpr_CycleCounter_ResetIncrementedHybridCycleCounter(                                                                       /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  void
  )
{
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) || (DEM_CFG_DATA_MONITOR_ACTIVITY_DATA == STD_ON))
...
{
  /* Overflow handling is done with a reset to 0, which is the expected c behaviour  */
 ...
 */
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_Iumpr_CycleCounter_ResetIgnitionCycleCounter(                                                                                /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  void
  )           
{
#if (DEM_CFG_SUPPORT_IUMPR == STD_ON)
 ...
 */
DEM_LOCAL_INLINE FUNC(uint16, DEM_CODE)
Dem_Iumpr_CycleCounter_GetHybridCycleCounter(                                                                                    /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  void
  )
{
#if (DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_SUPPORT_CYCLE_IGNITION_HYBRID)
  ...
  )
{
  ...
  {
  case DEM_IUMPR_GENERAL_INDIVIDUAL_DENOMINATOR:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive = ((uint8)(Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive | DEM_IUMPR_CONDITIONSTATUS_GENIND));
#endif
    break;

  case DEM_IUMPR_DEN_COND_COLDSTART:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive = ((uint8)(Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive | DEM_IUMPR_CONDITIONSTATUS_COLDSTART));
#endif
    break;

  case DEM_IUMPR_DEN_COND_EVAP:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive = ((uint8)(Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive | DEM_IUMPR_CONDITIONSTATUS_EVAP));
#endif
    break;

  case DEM_IUMPR_DEN_COND_500MI:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive = ((uint8)(Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive | DEM_IUMPR_CONDITIONSTATUS_500MI));
#endif
    break;

  case DEM_IUMPR_DEN_COND_INDEPENDENT:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive = ((uint8)(Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive | DEM_IUMPR_CONDITIONSTATUS_INDEPENDENT));
#endif
    break;
...
)
{
  ...
  case DEM_IUMPR_GENERAL_INDIVIDUAL_DENOMINATOR:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive = ((uint8)(Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive & DEM_IUMPR_CONDITIONSTATUS_GENIND_MASK));
#endif
    break;

  case DEM_IUMPR_DEN_COND_COLDSTART:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive = ((uint8)(Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive & DEM_IUMPR_CONDITIONSTATUS_COLDSTART_MASK));
#endif
    break;

  case DEM_IUMPR_DEN_COND_EVAP:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive = ((uint8)(Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive & DEM_IUMPR_CONDITIONSTATUS_EVAP_MASK));
#endif
    break;

  case DEM_IUMPR_DEN_COND_500MI:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive = ((uint8)(Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive & DEM_IUMPR_CONDITIONSTATUS_500MI_MASK));
#endif
    break;

  case DEM_IUMPR_DEN_COND_INDEPENDENT:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive = ((uint8)(Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive & DEM_IUMPR_CONDITIONSTATUS_INDEPENDENT_MASK));
#endif
    break;
...
/* ****************************************************************************
 % Dem_Iumpr_ConditionStatus_TestReachedWhilePtoInactive
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
{
  boolean lReachedWhilePtoInactive = FALSE;

  switch (ConditionId)
  {
  case DEM_IUMPR_GENERAL_INDIVIDUAL_DENOMINATOR:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    lReachedWhilePtoInactive = ((Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive & DEM_IUMPR_CONDITIONSTATUS_GENIND) != 0u);
#endif
    break;
  case DEM_IUMPR_DEN_COND_COLDSTART:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    lReachedWhilePtoInactive = ((Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive & DEM_IUMPR_CONDITIONSTATUS_COLDSTART) != 0u);
#endif
    break;
  case DEM_IUMPR_DEN_COND_EVAP:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    lReachedWhilePtoInactive = ((Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive & DEM_IUMPR_CONDITIONSTATUS_EVAP) != 0u);
#endif
    break;
  case DEM_IUMPR_DEN_COND_500MI:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    lReachedWhilePtoInactive = ((Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive & DEM_IUMPR_CONDITIONSTATUS_500MI) != 0u);
#endif
    break;
  case DEM_IUMPR_DEN_COND_INDEPENDENT:
#if ((DEM_CFG_SUPPORT_IUMPR == STD_ON) && (DEM_CFG_PTO_STATUS_SUPPORT == STD_ON))
    lReachedWhilePtoInactive = ((Dem_Cfg_ObdIumprData.DenominatorConditionStatusReachedWhilePtoInactive & DEM_IUMPR_CONDITIONSTATUS_INDEPENDENT) != 0u);
#endif
    break;
  default:                                                                                                                       /* COV_DEM_MISRA_BRANCH */
    /* MISRA Case */
  ...
  return lReachedWhilePtoInactive;
}
/*!
 * \}
 */
 ...
 *                Only the general denominator can be inhibited.
 *
 ...
 *****************************************************************************/
/*!
 * \brief         Initialize general OBD denominator
 *
 * \details       Initialize general OBD denominator
 *
 *
 * \pre           Feature IUMPR is supported
 *
...
/* ****************************************************************************
 * Dem_Iumpr_ConditionStatus_UpdateReachedWhilePtoInactive
 *****************************************************************************/
/*!
 * \brief         Update the global denominator conditions reached while PTO
 *                inactive state
 *
 * \details       Update whether global denominator conditions were reached
 *                while PTO was inactive
 *
 * \param[in]     ConditionId
 *                The denominator condition id.
 *
 * \param[in]     PtoStatus
 *                The PTO activation status
 *
 * \param[out]    TRUE
  ...
  )
{
  ...
  boolean lUpdateRatio = FALSE;
  boolean lPtoStatus = Dem_GlobalDiagnostics_GetPtoStatus();
  ...
  case DEM_IUMPR_DEN_STATUS_REACHED:
    lUpdateNvM = Dem_Iumpr_ConditionStatus_UpdateReachedWhilePtoInactive(ConditionId, lPtoStatus);
    ...
    /* Update ratio unconditionally to increment denominators that were locked by FID at the last condition update or by active PTO */
    lUpdateRatio = TRUE;
    break;
    ...
    Dem_Iumpr_RatioIter_Next(&lRatioIter))                                                                                       /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    /* Queue relevant ratios for an update  */
    if ( ((Dem_Iumpr_Ratio_Denominator_GetDenominatorGroup(lRatioIndex) == ConditionId)
      ...
      && ((lPtoStatus == FALSE)
       || (Dem_Core_Event_TestEventAffectedByPto(Dem_Iumpr_Ratio_GetRelatedEvent(lRatioIndex)) == FALSE)) )
    {
...
)
{
  ...
  {
    /* General Individual Denominator is already inhibited, nothing to do */
    lReturnValue = E_OK;
  }
  else
  {
...
*/
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_Iumpr_ProcessIgnCycleCounterIncrement(                                                                                       /* PRQA S 3219 */ /* MD_DEM_14.1_CONFIGURATION_SWITCH */
  void
  )
{
 ...
 */
DEM_LOCAL_INLINE FUNC(uint16, DEM_CODE)
Dem_Iumpr_CycleCounter_GetValue(                                                                                                 /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  Dem_Iumpr_CycleCounter_CycleCounterType CycleCounter
  )
{
 ...
 */
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_Iumpr_CycleCounter_Increment(                                                                                                /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  Dem_Iumpr_CycleCounter_CycleCounterType CycleCounter
  )          
{
 ...
 */
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_Iumpr_CycleCounter_InitStatus(                                                                                               /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  void
  )
{
  Dem_Iumpr_CycleCounter_ResetIncrementedHybridCycleCounter();
 ...
 */
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_Iumpr_CycleCounter_InitMemory(                                                                                               /* PRQA S 3219 */ /* MD_DEM_14.1_CONFIGURATION_SWITCH */
  void
  )
{
 ...
 */
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_Iumpr_OBDDenominator_InitStatus(                                                                                             /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  void
  )          
{
...
)
{
  switch (ConditionId)
  {
    case DEM_IUMPR_GENERAL_INDIVIDUAL_DENOMINATOR:
      ...
      Dem_Iumpr_ConditionStatus_ResetReachedWhilePtoInactive(DEM_IUMPR_GENERAL_INDIVIDUAL_DENOMINATOR);
      break;
    case DEM_IUMPR_DEN_COND_COLDSTART:
      Dem_Iumpr_ConditionStatus_ResetReached(DEM_IUMPR_DEN_COND_COLDSTART);
      Dem_Iumpr_ConditionStatus_ResetReachedWhilePtoInactive(DEM_IUMPR_DEN_COND_COLDSTART);
      break;
    case DEM_IUMPR_DEN_COND_EVAP:
      Dem_Iumpr_ConditionStatus_ResetReached(DEM_IUMPR_DEN_COND_EVAP);
      Dem_Iumpr_ConditionStatus_ResetReachedWhilePtoInactive(DEM_IUMPR_DEN_COND_EVAP);
      break;
    case DEM_IUMPR_DEN_COND_500MI:
      Dem_Iumpr_ConditionStatus_ResetReached(DEM_IUMPR_DEN_COND_500MI);
      Dem_Iumpr_ConditionStatus_ResetReachedWhilePtoInactive(DEM_IUMPR_DEN_COND_500MI);
      break;
    case DEM_IUMPR_DEN_COND_INDEPENDENT:
      Dem_Iumpr_ConditionStatus_ResetReached(DEM_IUMPR_DEN_COND_INDEPENDENT);
      Dem_Iumpr_ConditionStatus_ResetReachedWhilePtoInactive(DEM_IUMPR_DEN_COND_INDEPENDENT);
      break;
    default:
      /* MISRA Case */
      break;
 ...
 */
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_Iumpr_RestartDcy(                                                                                                            /* PRQA S 3219 */ /* MD_DEM_14.1_CONFIGURATION_SWITCH */
  void
  )
{
  Dem_Cfg_ObdRatioIdIterType    lRatioIter;
...
{
  boolean lStatusChanged = FALSE;

  if (Dem_GlobalDiagnostics_IsPtoStatusSupportEnabled() == TRUE)
  {
    if ( (PtoStatus == FALSE)
      && (Dem_Iumpr_ConditionStatus_TestReachedWhilePtoInactive(ConditionId) == FALSE) )
    {
      Dem_Iumpr_ConditionStatus_SetReachedWhilePtoInactive(ConditionId);
      lStatusChanged = TRUE;
    }
  }

  return lStatusChanged;
}
...
{
  if ((Dem_Iumpr_IsEnabled() == TRUE) || (Dem_Mar_IsEnabled() == TRUE))
  {
    Dem_Cfg_ObdRatioIdIterType     lRatioIter;
    ...
    /* initialize denominator group status */
    Dem_Iumpr_ConditionStatus_InitStatus(DEM_IUMPR_DEN_COND_COLDSTART);
    Dem_Iumpr_ConditionStatus_InitStatus(DEM_IUMPR_DEN_COND_EVAP);
    Dem_Iumpr_ConditionStatus_InitStatus(DEM_IUMPR_DEN_COND_500MI);
    Dem_Iumpr_ConditionStatus_InitStatus(DEM_IUMPR_DEN_COND_INDEPENDENT);
    ...
    for (Dem_Iumpr_RatioIter_Init(&lRatioIter);                                                                                  /* SBSW_DEM_CALL_ITERATOR_POINTER */
      Dem_Iumpr_RatioIter_Exists(&lRatioIter) == TRUE;                                                                           /* SBSW_DEM_CALL_ITERATOR_POINTER */ /* PRQA S 2994 */ /* MD_DEM_2994 */
      Dem_Iumpr_RatioIter_Next(&lRatioIter))                                                                                     /* SBSW_DEM_CALL_ITERATOR_POINTER */
    {
...
/* ****************************************************************************
 % Dem_DiagnosticObserver_Iumpr_GetGeneralDenominator
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
{
  if (Dem_Iumpr_IsEnabled() == TRUE)                                                                                             /* COV_DEM_UNIT_ENTRY_CONFIGURATION_CHECK */
  {
    *GeneralDenominator = Dem_Iumpr_OBDDenominator_GetValue();                                                                   /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
  }
}
...
/* ****************************************************************************
 % Dem_DiagnosticObserver_Iumpr_GetIgnitionCycleCounter
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_DiagnosticObserver_Iumpr_GetIgnitionCycleCounter(
  CONSTP2VAR(uint16, AUTOMATIC, AUTOMATIC) IgnitionCycles
)
{
  if (Dem_Iumpr_IsEnabled() == TRUE)                                                                                             /* COV_DEM_UNIT_ENTRY_CONFIGURATION_CHECK */
  {
 ...
 */
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_Iumpr_ResetPendingFids(                                                                                                      /* PRQA S 3219 */ /* MD_DEM_14.1_CONFIGURATION_SWITCH */
  void
  )
{
  if (Dem_Iumpr_IsEnabled() == TRUE)
...
/* ****************************************************************************
 % Dem_Iumpr_CalculateDtcWorstObdRatio
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
 ...
 */
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_Iumpr_CalculateDtcWorstObdRatio(                                                                                             /* PRQA S 3219 */ /* MD_DEM_14.1_LIB */
  ...
  )
{
  ...
  {
    Dem_Iumpr_CalculateCombinedDtcWorstObdRatio(lGroupId, Numerator, Denominator);                                               /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  }
  else
  {
    Dem_RatioIdIndexType lRatioIndex;
...
/* ****************************************************************************
 % Dem_DiagnosticObserver_Iumpr_CompareAndGetWorseObdRatio()
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
DEM_LOCAL_INLINE FUNC(Dem_RatioIdIndexType, DEM_CODE)
Dem_DiagnosticObserver_Iumpr_CompareAndGetWorseObdRatio(
  Dem_RatioIdIndexType RatioIndex1,
  Dem_RatioIdIndexType RatioIndex2
)
{
  Dem_RatioIdIndexType lWorseRatio = 0x00;

  if (Dem_Iumpr_IsEnabled() == TRUE)                                                                                             /* COV_DEM_UNIT_ENTRY_CONFIGURATION_CHECK */
  {
    uint16 lNumeratorRatio1;
    uint16 lDenominatorRatio1;

    uint16 lNumeratorRatio2;
    uint16 lDenominatorRatio2;

    Dem_Iumpr_Ratio_GetRatio(RatioIndex1, &lNumeratorRatio1, &lDenominatorRatio1);                                               /* SBSW_DEM_POINTER_FORWARD_STACK */
    Dem_Iumpr_Ratio_GetRatio(RatioIndex2, &lNumeratorRatio2, &lDenominatorRatio2);                                               /* SBSW_DEM_POINTER_FORWARD_STACK */

    if (lDenominatorRatio1 == 0x00u)
    {
      lWorseRatio = RatioIndex1;
    }
    else
    if (lDenominatorRatio2 == 0x00u)
    {
      lWorseRatio = RatioIndex2;
    }
    else
    {
      lWorseRatio = RatioIndex1;

      /* Expand the ratios so that their denominators are the same and their
          numerators can be compared directly. */
      if ( ( ( (uint32) lNumeratorRatio2 ) * lDenominatorRatio1 ) ==
           ( ( (uint32) lNumeratorRatio1 ) * lDenominatorRatio2 ) )
      {
        /* If two ratios have the same numerical value the ratio with
          the bigger denominator is worse. */
        if (lDenominatorRatio2 > lDenominatorRatio1)
        {
          lWorseRatio = RatioIndex2;
        }
      }
      ...
      {
        /* Nothing to do, first ratio is worse. */
      }
    }
  }
...
)
{
  ...
  if (Dem_Cfg_IsEmissionObdSupportedInVariant() == TRUE)
  {
    ...
    {
      if (lTranslatedMemoryId != DEM_CFG_MEMORYID_PERMANENT)
      {
        Dem_Iumpr_FilterInfo.FirstEvent = Dem_FaultMemory_Memories_GetFirstEvent(lTranslatedMemoryId);
        Dem_Iumpr_FilterInfo.LastEvent = Dem_FaultMemory_Memories_GetLastEvent(lTranslatedMemoryId);
        lReturnValue = E_OK;
      }
    }
  }
