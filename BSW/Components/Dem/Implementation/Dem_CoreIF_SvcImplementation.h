 ...
 *  \{
 *  \file       Dem_CoreIF_SvcImplementation.h
...
#if !defined (DEM_COREIF_SVCIMPLEMENTATION_H)
#define DEM_COREIF_SVCIMPLEMENTATION_H
...
#define DEM_COREIF_SVCIMPLEMENTATION_FILENAME "Dem_CoreIF_SvcImplementation.h"
...
#  endif
  ...
  if (Dem_GlobalDiagnostics_TestPtoActiveAndEventAffected(EventId) == TRUE)
  {
    lReturnValue = E_NOT_OK;
  }
  else
  ...
  )
{
  ...
  else
  {
    ...
    if (Dem_Event_TestEventAvailable(EventId) == TRUE)
    {
      if (Dem_DTC_GetFormatedDTCByEvent(EventId, DTCFormat, DTCOfEvent) == E_OK)                                                 /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
      {
        lReturnValue = E_OK;
      }
      else
  ...
  else
  {
...
#if (DEM_FEATURE_NEED_MIL_GROUPS == STD_ON)
    {
      ...
      while (lMilGroupIndex != DEM_CFG_MIL_GROUP_INVALID)
      {
        ...
        Dem_Event_InternalStatus_SetMilGroupPendingCount(lMilGroupIndex, 0u);
      }
    }
#endif
  ...
  )
{
  ...
  {
    /* ----- Implementation ------------------------------------------------ */
    Dem_MemSet((Dem_NvDataPtrType)&Dem_Cfg_EventAvailableData[0], 0x00, Dem_Cfg_GlobalEventAvailableDataSize() * sizeof(Dem_Cfg_EventAvailableData[0])); /* PRQA S 0310, 0315, 0602 */ /* MD_DEM_11.4_cpy, MD_MSR_VStdLibCopy, MD_DEM_20.2 */ /* SBSW_DEM_MEMSET_EVENTAVAILABLEDATA */
  }
...
# if (DEM_CFG_SUPPORT_AGING_FOR_ALL_DTCS == STD_ON)
    ...
    Dem_MemSet((Dem_DataPtrType)&(Dem_Cfg_GetAgingData().AgingTargetCycle[0]),                                                   /* PRQA S 0310, 0315, 0602 */ /* MD_DEM_11.4_cpy, MD_MSR_VStdLibCopy, MD_DEM_20.2 */ /* SBSW_DEM_MEMSET_AGINGDATA_AGINGTARGETCYCLE */
      0xFF,
      DEM_CFG_MAX_NUMBER_AGING_DTCS * sizeof(Dem_Cfg_GetAgingData().AgingTargetCycle[0]));
# endif

# if (DEM_CFG_DATA_AGED_COUNTER == STD_ON)
    /* Set all aged counter array entries to the initial value */
    Dem_MemSet((Dem_DataPtrType)&(Dem_Cfg_GetAgingData().AgedCounter[0]),                                                        /* PRQA S 0310, 0315, 0602 */ /* MD_DEM_11.4_cpy, MD_MSR_VStdLibCopy, MD_DEM_20.2 */ /* SBSW_DEM_MEMSET_AGINGDATA_AGEDCOUNTER */
  ...
  )
{
  ...
  else
  {
    ...
    if (Dem_Event_TestEventAvailable(EventId) == TRUE)
    {
      lReturnValue = Dem_Core_StatusIndicator_GetStatusIndicator(EventId, Status);                                               /* SBSW_DEM_POINTER_FORWARD_API */
    }
  }
...
#endif /* DEM_COREIF_SVCIMPLEMENTATION_H */

/*!
 * \}
 */
/* ********************************************************************************************************************
 *  END OF FILE: Dem_CoreIF_SvcImplementation.h
 *********************************************************************************************************************/
