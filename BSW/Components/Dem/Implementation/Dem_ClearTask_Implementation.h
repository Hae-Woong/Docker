...
DEM_LOCAL volatile VAR(uint8, DEM_VAR_NO_INIT)    Dem_ClearTask_ClearState;

/*! Flag to indicate write errors during clear */
DEM_LOCAL VAR(boolean, DEM_VAR_NO_INIT)           Dem_ClearTask_ClearWriteErrors;                                                /* PRQA S 3218 */ /* MD_DEM_8.7 */

/*! Flag to indicate that the clear operation is finished */
DEM_LOCAL VAR(boolean, DEM_VAR_NO_INIT)           Dem_ClearTask_ClearSuccessful;
...
DEM_LOCAL VAR(uint16, DEM_VAR_NO_INIT)            Dem_ClearTask_ClearEventIterator;
...
/*! ClearRequest data */
DEM_LOCAL VAR(Dem_ClearTask_ClearRequestType, DEM_VAR_NO_INIT)  Dem_ClearTask_ClearRequest;
  ...
  )
{
  ...
  {
    lRetVal = (boolean)(Dem_ClearTask_ClearEventIterator <= Dem_FaultMemory_Memories_GetLastEvent(TranslatedMemoryId));
  }
  else
  {
    lRetVal = TRUE;
...
{
  Dem_EventIdType lClearEventId = Dem_ClearTask_GetClearEvent();
#if (DEM_DEV_RUNTIME_CHECKS == STD_ON)
  if (Dem_ClearTask_GetClearMemoryId() != Dem_Core_Event_GetDestination(lClearEventId))
  {
  ...
  if ((Dem_ClearTask_GetClearNodeIndex() == Dem_Cfg_GetInvalidJ1939NodeIndex())
    || (Dem_Client_ClientAccess_TestDtcMatchesJ1939ClearFilter(lClearEventId, Dem_ClearTask_GetClearFilter())))                  /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
  {
    Dem_ClearTask_ClearSuccessful = Dem_DTC_Cleared(lClearEventId);
  }

  if (Dem_ClearTask_ClearSuccessful == FALSE)
  {
  ...
  )
{
  ...
  {
    /*Remember that a suitable DTC was cleared -> give positive response to clear request*/
  ...
  )
{
  if (FirstEventId != DEM_EVENT_INVALID) 
  {
    ...
    { /* Event must be a J1939 related and match the ClearCode */
      ...
      /* If EmissionOBD is supported, non-J1939 DTCs shall also be removed when clear is requested */
      if ( (Dem_Cfg_IsEmissionObdSupportedInVariant() == FALSE)
        && (Dem_Core_DTC_HasJ1939DTC(lEventIterator) == FALSE) )
...
# if (DEM_FEATURE_SUPPORT_EVENTCOMBINATION == STD_ON)
      if (lEventIterator != Dem_Cfg_GetMasterEvent(lEventIterator))
      { /* do not match dependent combined events */
        continue;                                                                                                                
      }
# endif
      if (Dem_Client_ClientAccess_TestDtcMatchesJ1939ClearFilter(lEventIterator, Dem_ClearTask_GetClearFilter()) == FALSE)
      { /* DTC status needs to match the request */
        if ((Dem_Cfg_IsEmissionObdSupportedInVariant() == FALSE) || (Dem_ClearTask_GetClearFilter() != DEM_J1939DTC_CLEAR_ACTIVE))
        {
          continue;                                                                                                                
        }
      }
      if (Dem_ClearTask_GetClearNodeIndex() != Dem_Cfg_GetInvalidJ1939NodeIndex())
      {
        /* If EmissionOBD is supported, all events in the memory shall be cleared */
        if ( (Dem_Cfg_IsEmissionObdSupportedInVariant() == FALSE)
          && (Dem_Client_ClientAccess_IsEventAssignedToNode(lEventIterator, Dem_ClearTask_GetClearNodeIndex()) == FALSE) )
        {
          continue;
        }
      }
  ...
  )
{
  ...
  Dem_EventIdType FirstEventId = Dem_FaultMemory_Memories_GetFirstEvent(TranslatedMemoryId);
  ...
  {
    Dem_ClearTask_ClearSuccessful = TRUE;
  }
  ...
  )
{
  ...
  {
    LastEventId = Dem_FaultMemory_Memories_GetLastEvent(TranslatedMemoryId);
  }
...
)
{
  ...
  {
    if (FirstClearedEventId == Dem_FaultMemory_Memories_GetFirstEvent(TranslatedMemoryId))
    { /* All events with the memory destination are cleared and update the iterator to started clearing the internal events */
  ...
  )
{
  ...
  Dem_ClearTask_ClearSuccessful = FALSE;
...
# endif
    ...
    {
      Dem_ClearTask_ClearEventIterator = Dem_FaultMemory_Memories_GetLastEvent(lTranslatedMemoryId);
    }
#endif
...
# endif
  ...
  if (Dem_ClearTask_ClearSuccessful == TRUE)
  {
#if (DEM_CFG_SUPPORT_CLEAR_DTC_RUNTIME_LIMIT== STD_ON)
    Dem_ClearTask_ClearEventIterator = DEM_EVENT_INVALID;
#endif
  ...
  )
{
  ...
  case DEM_CLEARTASK_INPROGRESS_DTCFOUND:                                                                                        /* PRQA S 2003 */ /* MD_DEM_15.2 */
    Dem_ClearTask_ProcessClearDtc();
