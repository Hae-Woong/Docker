...
#if (DEM_CFG_SUPPORT_IUMPR == STD_OFF) && (DEM_CFG_DATA_MONITOR_ACTIVITY_DATA == STD_OFF)
# define DEM_CFG_NVBLOCK_IUMPR_INDEX 0U
#endif
...
DEM_LOCAL VAR(uint8, DEM_VAR_NO_INIT)             Dem_Nvm_PendingEventAvailabilityBlockState;
#endif

#if (DEM_CFG_USE_NVM == STD_ON) && (DEM_CFG_SUPPORT_NVM_APPL_SYNCHRONIZE == STD_ON)
/*! Pending request for NV synchronization */
DEM_LOCAL VAR(boolean, DEM_VAR_NO_INIT)             Dem_Nvm_IsSynchronizationRequested;
#endif
...
DEM_LOCAL VAR(Dem_Nvm_CommitDataType, DEM_VAR_NO_INIT)  Dem_Nvm_CommitData;
#endif
...
Dem_Nvm_GetPrivateBlockState(
  CONST(Dem_Cfg_NvBlockIndexType, AUTOMATIC)  BlockIndex
  );
#endif
...
{
  return DEM_CFG_NVBLOCK_IUMPR_INDEX;
}
#endif
...
{
  Dem_MemSet((Dem_DataPtrType)&Dem_Cfg_GetMemoryStatus(0), DEM_NVM_BLOCKSTATE_DIRTY, Dem_Cfg_GetSizeOfMemoryStatus()*sizeof(Dem_Cfg_GetMemoryStatus(0))); /* PRQA S 0315, 0602 */ /* MD_MSR_VStdLibCopy, MD_DEM_20.2 */  /* SBSW_DEM_MEMSET_MEMORYSTATUS */
}
...
{
  Dem_Internal_AssertReturnVoid(Dem_Nvm_CommitData.PendingCount <= DEM_CFG_NVBLOCKINDEX_MAX, DEM_E_INCONSISTENT_STATE)
...
{
  Dem_Internal_AssertReturnVoid(Dem_Nvm_CommitData.PendingClearCount <= DEM_CFG_NVBLOCKINDEX_MAX, DEM_E_INCONSISTENT_STATE)
  ...
  )
{
  ...
  if ( (Dem_Cfg_IsPermanentSupportedInVariant() == TRUE) 
    && (Dem_Nvm_TestBlockStateImmediate(Dem_Nvm_GetPrivateBlockState(Dem_Cfg_NvBlockPermanentIndex())) == TRUE) )
  {
    Dem_Nvm_SetCurrentBlockIndex(Dem_Cfg_NvBlockPermanentIndex());
    lFoundBlock = TRUE;
  }
  else
  /* Freezeframe has next highest prio: check for modification and write it */
  if ( (Dem_Cfg_IsObdIIOrObdOnUdsSupportedInVariant() == TRUE)
    && (Dem_Nvm_TestBlockStateImmediate(Dem_Nvm_GetPrivateBlockState(Dem_Cfg_NvBlockFreezeFrameIndex())) == TRUE) )
  {
    Dem_Nvm_SetCurrentBlockIndex(Dem_Cfg_NvBlockFreezeFrameIndex());
    lFoundBlock = TRUE;
  }
  else
  /* Event states first, they are bound to be modified quickly */
  if (Dem_Nvm_TestBlockStateImmediate(Dem_Nvm_GetPrivateBlockState(Dem_Cfg_NvBlockStatusIndex())) == TRUE)
  {
    Dem_Nvm_SetCurrentBlockIndex(Dem_Cfg_NvBlockStatusIndex());
    lFoundBlock = TRUE;
  }
  else
  {
    ...
    Dem_Cfg_NvBlockIndexType lBlockIndex;
    Dem_Cfg_NvBlockIndexType lEndIndex;
...
# endif
    if (lBlockIndex == Dem_Cfg_NvBlockAdminIndex())
    {
    ...
    {
      Dem_Cfg_NvBlockDataSizeType lDataSize = Dem_Cfg_NvBlockDataSize[lBlockIndex];

# if (DEM_DEV_RUNTIME_CHECKS == STD_ON)
      if ((Dem_Cfg_NvBlockDataSizeType)sizeof(Dem_Cfg_CommitBuffer) < lDataSize)
      {
        Dem_Error_RunTimeCheckFailed(DEM_NVM_IMPLEMENTATION_FILENAME, __LINE__);                                                 /* SBSW_DEM_POINTER_RUNTIME_CHECK */
        lNvmResult = E_NOT_OK;
      }
...
# endif
      {
        Dem_MemCpy((Dem_DataPtrType)&Dem_Cfg_CommitBuffer.mRaw[0],                                                               /* PRQA S 0315, 0602 */ /* MD_MSR_VStdLibCopy, MD_DEM_20.2 */  /* SBSW_DEM_MEMCOPY_NVBLOCK_DATA */
                   Dem_Cfg_NvBlockDataPtr[lBlockIndex],
                   lDataSize);
        ...
        lNvmResult = NvM_WriteBlock((NvM_BlockIdType)Dem_Cfg_NvBlockId[lBlockIndex],                                             /* SBSW_DEM_NVM_WRITEBLOCK */
...
# if (DEM_FEATURE_NEED_EVENTAVAILABLE_NV == STD_ON)
  ...
  {
    Dem_Nvm_SetBlockStateImmediate(Dem_Cfg_NvBlockEventAvailableIndex(), Dem_Nvm_PendingEventAvailabilityBlockState);
...
# if (DEM_CFG_SUPPORT_NVM_APPL_SYNCHRONIZE == STD_ON)
  ...
  if (Dem_Nvm_IsSynchronizationRequested == TRUE)
  {
    ...
    Dem_Satellite_CopyDataDebounceNv(DEM_NVM_BLOCKSTATE_DIRTYIMMEDIATE);
  ...
  )
{
  if (Dem_Nvm_GetCurrentCommitState() == DEM_NVM_COMMIT_WRITING)
  {
    ...
    lCallResult = NvM_GetErrorStatus((NvM_BlockIdType)Dem_Cfg_NvBlockId[Dem_Nvm_GetCurrentBlockIndex()], &lRequestResult);       /* SBSW_DEM_POINTER_FORWARD_STACK */
    if (lCallResult == E_OK)
    {
...
{
  Dem_MemSet((Dem_DataPtrType)&Dem_Cfg_GetMemoryStatus(0), DEM_NVM_BLOCKSTATE_UNMODIFIED, Dem_Cfg_GetSizeOfMemoryStatus()*sizeof(Dem_Cfg_GetMemoryStatus(0))); /* PRQA S 0315, 0602 */ /* MD_MSR_VStdLibCopy, MD_DEM_20.2 */  /* SBSW_DEM_MEMSET_MEMORYSTATUS */
...
/* ****************************************************************************
 % Dem_Nvm_IsDebounceCounterStorageSupported
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
/* ****************************************************************************
 % Dem_Nvm_SetEventEntryNvBlockState
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
 ...
 *****************************************************************************/
DEM_LOCAL FUNC(void, DEM_CODE)
Dem_Nvm_SetEventEntryNvBlockState(                                                                                               /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  Dem_Cfg_EntryIndexType EventEntryIndex,
  uint8 Status
)
{
#if (DEM_CFG_USE_NVM == STD_ON)
  uint8 lBlockState = Status;
  Dem_Cfg_NvBlockIndexType lNvBlockIndex = Dem_MemoryEntry_EventEntry_GetEntryIndexToNvBlockIndex(EventEntryIndex);
#if (DEM_FEATURE_NEED_NVM_IMMEDIATE == STD_ON)
#if (DEM_CFG_SUPPORT_NVM_WRITELIMIT == STD_ON)
  if ((lBlockState == DEM_NVM_BLOCKSTATE_DIRTYIMMEDIATE)
   && (Dem_MemoryEntry_EventEntry_GetOccurrenceCounter(EventEntryIndex) >= Dem_Cfg_GlobalNvStorageLimit()))                      /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
  {
    lBlockState = DEM_NVM_BLOCKSTATE_DIRTY;
  }
#endif /* DEM_CFG_SUPPORT_NVM_WRITELIMIT */
  Dem_Nvm_SetBlockStateImmediate(lNvBlockIndex, lBlockState);
#else /* DEM_FEATURE_NEED_NVM_IMMEDIATE == STD_OFF */
  Dem_Nvm_SetPrivateBlockState(lNvBlockIndex, lBlockState);
#endif
#else
...
# endif
  ...
  for (lBlockIndex = 0u; lBlockIndex < Dem_Cfg_GlobalNvBlockCount(); ++lBlockIndex)
  {
    ...
    {
      (void)NvM_SetRamBlockStatus((NvM_BlockIdType)Dem_Cfg_NvBlockId[lBlockIndex], TRUE);
    }
  }
    ...
    case DEM_NVM_COMMIT_FAILED:
      {
        /* Error handling:
           ...
           changes again */
        (void)NvM_SetRamBlockStatus((NvM_BlockIdType)Dem_Cfg_NvBlockId[Dem_Nvm_GetCurrentBlockIndex()], TRUE);
# if (DEM_CFG_CLEAR_NV_END_RESPONSE == STD_ON)
