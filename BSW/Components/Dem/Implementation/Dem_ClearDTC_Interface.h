 ...
 *****************************************************************************/
/*!
 * \brief         Start clearing a single J1939 DTC.
 *
 * \details       Initializes a clear DTC handler to process clearing of a single
 *                J1939 DTC
 *
 * \param[in]     ClearDTCHandle
 *                The ClearDTC handle to modify.
 ...
 *                Current state of the DTC to be cleared
 * \param[in]     Node
 *                The node to which the DTC to be cleared is assigned
 *
 * \return        DEM_PENDING
 *                The clear operation has been started.
 * \return        DEM_CLEAR_BUSY
 *                No clear operation was started since another clear operation
 *                is already active.
 * \return        DEM_WRONG_DTC
 *                The DTC is not available.
 *
 * \pre           A single DTC was selected to be cleared in the DTCSelector.
 * \pre           DTCTypeFilter filters either for a currently or a previously active DTC,
 *                not both.
 *
 * \context       TASK|ISR2
 * \synchronous   TRUE
 * \reentrant     TRUE, for different ClearDTCHandle
 *****************************************************************************/
DEM_LOCAL_INLINE FUNC(Std_ReturnType, DEM_CODE) Dem_Client_ClearDTC_StartClearSingleDtcJ1939(
 ...
 * \reentrant     TRUE, for different ClearDTCHandle
 *****************************************************************************/
DEM_LOCAL_INLINE FUNC(Std_ReturnType, DEM_CODE) Dem_Client_ClearDTC_StartClearAllDtcsJ1939(
  Dem_ClearDTC_HandleType ClearDTCHandle,
  Dem_DTCSelector_HandleType DTCSelector,
  uint8 DTCTypeFilter,
  uint8 Node
