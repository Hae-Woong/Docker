 ...
 *  \param[in]      targetMode    Mode that the request is targeting for
 ...
 *  \details        When ETHIF_ENABLE_AUTOSAR_FORWARD_COMPATIBILITY is disabled and no previous ACTIVE request for it
 *                  was triggered: The EthIf will forward the call to the API Eth_SetControllerMode() in the
 *                  Eth Driver. The return value of this API will be used as the return value of the
 *                  function EthIf_EnableEthCtrl().
 *                  When ETHIF_ENABLE_AUTOSAR_FORWARD_COMPATIBILITY is enabled and no previous ACTIVE request for it
 *                  was triggered: The EthIf will forward the call to the API Eth_SetControllerMode() in the
 *                  Eth Driver. After that the EthIf will continuously check if the Eth Controller is really set to
 *                  ACTIVE in certain cycles, after that, if it still not set to ACTIVE, EthIf_EnableEthCtrl() will
 *                  return E_NOT_OK.
  ...
  Eth_ModeType             targetMode)
{
  ...
  thresholdCnt = ETHIF_MIN_ETHCTRLMODECNT; /* for targetMode == ETH_MODE_DOWN the threshold is 0 */

  if( targetMode == ETH_MODE_ACTIVE )
  {
...
# if ( ETHIF_ETHSWT_USED == STD_ON )
    if( EthIf_IsPortGroupUsedOfPhysLayerModeElems(physLayerModeElemsIdx) )
    {
      ...
      result = EthIf_IsModeRequestPossibleForEthIfSwtPortGroup(portGroupIdx, targetMode);
    }
# endif /* ETHIF_ETHSWT_USED */
  }
  ...
  Eth_ModeType             targetMode)
{
  ...
  /* #10 Set the pass error criteria for possibility check dependent on the targeted mode */
  thresholdCnt = ETHIF_MIN_ETHSWTPORTMODECNT; /* for targetMode == ETH_MODE_DOWN the threshold is 0 */
 ...
 *
 */
ETHIF_MODE_LOCAL_INLINE FUNC(Std_ReturnType, ETHIF_MODE_INLINE_CODE) EthIf_EnableEthCtrl( /* PRQA S 6080 */ /* MD_MSR_STMIF */
  EthIf_EthCtrlIterType  ethCtrlIdx)
{
  ...
  if( EthIf_GetEthCtrlModeCnt(ethCtrlIdx) == ETHIF_MIN_ETHCTRLMODECNT )
  {
    ...
    uint32 timeoutCnt;
    Eth_ModeType tempControllerMode = ETH_MODE_DOWN;
    ...
    /* #110 Check if driver provides the API */
    if( EthIf_GetSetCtrlModeOfEthDrvApi(ethDrvApiIdx) != NULL_PTR )
    {
    ...
    {
      /* #1210 Set the result to E_NOT_OK */
      retVal = E_NOT_OK;
    }
#if (ETHIF_ENABLE_AUTOSAR_FORWARD_COMPATIBILITY == STD_ON)
    /* #130 If the call to the Ethernet Driver was successfull */
    if ( retVal == E_OK )
    {
      /* #1310 Check if driver provides the API */
      if( EthIf_GetGetCtrlModeOfEthDrvApi(ethDrvApiIdx) != NULL_PTR )
      {
        /* #13110 Loop until the process is done or time out counter is finished */
        for( timeoutCnt = 0u; timeoutCnt < EthIf_GetSetControllerModeTimeout(); timeoutCnt++ )
        {
          (void)EthIf_GetGetCtrlModeOfEthDrvApi(ethDrvApiIdx)((uint8)EthIf_GetSnvOfEthCtrl(ethCtrlIdx),
                                                             &tempControllerMode);                                      /* SBSW_ETHIF_DRVAPI_FCT_PTR_MAN_CSL03 */
          if( tempControllerMode == ETH_MODE_ACTIVE )
          {
        ...
        /* #13120 If timeout occurred */
        if( timeoutOccurred == TRUE)
        {
          /* #131210 Set the result to E_NOT_OK */
          retVal = E_NOT_OK;
        }
      }
      /* #1320 Else... */
      ...
      {
        /* #13210 Set the result to E_NOT_OK */
        retVal = E_NOT_OK;
      }
    }
#endif /* ETHIF_ENABLE_AUTOSAR_FORWARD_COMPATIBILITY */
...
#else
  ETHIF_DUMMY_STATEMENT(physLayerModeElemsIdx); /* PRQA S 1338, 2983, 3122 */ /* MD_MSR_DummyStmt */
#endif /* ETHIF_ETHSWT_USED */
  ...
  EthIf_EthIfCtrlIterType  ethIfCtrlIdx)
{
  ...
  if( retVal == E_OK )
  {
    ...
    EthIf_TryUpdateEthTrcvStatesAfterActive(physLayerModeElemsIdx);
    EthIf_TryUpdateEthIfSwtPortGroupStatesAfterActive(physLayerModeElemsIdx);
  }
...
#else
  ETHIF_DUMMY_STATEMENT(physLayerInitElemsIdx); /* PRQA S 1338, 2983, 3122 */ /* MD_MSR_DummyStmt */
#endif /* ETHIF_ETHSWT_USED */
  ...
  EthIf_EthIfCtrlIterType  ethIfCtrlIdx)
{
  ...
  /* ----- Implementation ----------------------------------------------- */
  /* #10 Trigger disabling of the Ethernet controller */
  retVal = EthIf_DisableEthCtrl(ethCtrlIdx);

  /* #20 Check if disabling the Ethernet controller was successful */
  if( retVal == E_OK )
  {
    EthIf_PhysLayerModeElemsIdxOfEthIfCtrlType physLayerModeElemsIdx = EthIf_GetPhysLayerModeElemsIdxOfEthIfCtrl(ethIfCtrlIdx);

    /* #200 Trigger disabling of Ethernet transceiver and EthIf Switch Port Group */
  ...
  if( retVal == E_OK )
  {
    ...
    /* #300 Trigger adaption of the related hardware initialization and mode states and counters */
    EthIf_UpdateEthIfCtrlStatesAfterDown(ethIfCtrlIdx);
    EthIf_UpdateEthCtrlStatesAfterDown(ethCtrlIdx);
    EthIf_TryUpdateEthTrcvStatesAfterDown(physLayerInitElemsIdx);
    EthIf_TryUpdateEthIfSwtPortGroupStatesAfterDown(physLayerInitElemsIdx);
  }
 ...
 *
 */
ETHIF_MODE_LOCAL_INLINE FUNC(Std_ReturnType, ETHIF_MODE_INLINE_CODE) EthIf_Internal_ControllerInit( /* PRQA S 6050 */ /* MD_MSR_STCAL */
  uint8  ethIfCtrlIdx,
  uint8  cfgIdx)
{
  ...
  if( isInitLocked == TRUE )
  {
    ...
    if( retVal == E_OK )
    {
      ...
      EthIf_TryUpdateEthTrcvStatesAfterInit(physLayerInitElemsIdx);
      EthIf_TryUpdateEthSwtStatesAfterInit(physLayerInitElemsIdx);
    }
 ...
 *
 */
ETHIF_MODE_LOCAL_INLINE FUNC(Std_ReturnType, ETHIF_MODE_INLINE_CODE) EthIf_Internal_StartAllPorts( void ) /* PRQA S 6080 */ /* MD_MSR_STMIF */
{
  Std_ReturnType retVal = E_NOT_OK;
 ...
 *
 */
FUNC(Std_ReturnType, ETHIF_CODE) EthIf_SwitchPortGroupRequestMode( /* PRQA S 6080 */ /* MD_MSR_STMIF */
    ...
    Eth_ModeType CtrlMode)
{
  ...
  ETHIF_DUMMY_STATEMENT(CtrlIdx); /* PRQA S 1338, 2983, 3122 */ /* MD_MSR_DummyStmt */ /*lint !e438 */
  ETHIF_DUMMY_STATEMENT(CtrlMode); /* PRQA S 1338, 2983, 3122 */ /* MD_MSR_DummyStmt */ /*lint !e438 */
}
#endif /* ETHIF_ENABLE_AUTOSAR_FORWARD_COMPATIBILITY */
