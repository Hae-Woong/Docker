 ...
 *  \return         E_OK - Transmission triggered successful
...
# if (ETHIF_ENABLE_AUTOSAR_FORWARD_COMPATIBILITY == STD_ON)
      EthIf_TxConfirmation((uint8)EthIf_GetSnvOfEthCtrl(ethCtrlIdx), txFrameHandling->BufIdx, E_OK);
# else
 ...
 *
 */
ETHIF_TX_LOCAL_INLINE FUNC(BufReq_ReturnType, ETHIF_TX_INLINE_CODE) EthIf_Internal_ProvideTxBuffer( /* PRQA S 6060 */ /* MD_MSR_STPAR */
        uint8                                                                       ethIfCtrlIdx,
        Eth_FrameType                                                               frameType,
        uint8                                                                       priority,
  P2VAR(uint8,                                          AUTOMATIC, ETHIF_APPL_VAR)  bufIdxPtr,
  ...
  P2VAR(uint16,                                         AUTOMATIC, ETHIF_APPL_VAR)  lenBytePtr)
{
  ...
  if( EthIf_GetEthIfCtrlMode(ethIfCtrlIdx) == ETH_MODE_ACTIVE )
  {
    ...
    if( retVal == BUFREQ_OK )
    {
      ...
      {
        /* #1200 Trigger buffer provision finalization if buffer index is in expected range */
 ...
 *
 */
ETHIF_TX_LOCAL_INLINE FUNC(void, ETHIF_TX_INLINE_CODE) EthIf_PrepareTransmission( /* PRQA S 6060 */ /* MD_MSR_STPAR */
          EthIf_TxContextIterType                                txCtxtIdx,
          Eth_FrameType                                          frameType,
          uint16                                                 lenByte,
          uint8                                                  bufIdx,
 ...
 *
 */
ETHIF_TX_LOCAL_INLINE FUNC(Std_ReturnType, ETHIF_TX_INLINE_CODE) EthIf_Internal_VTransmit( /* PRQA S 6060 */ /* MD_MSR_STPAR */
        uint8                                     ethIfCtrlIdx,
        EthIf_UserIterType                        ownerIdx,
        uint8                                     bufIdx,
        Eth_FrameType                             frameType,
 ...
 *
 */
FUNC(BufReq_ReturnType, ETHIF_CODE) EthIf_ProvideTxBuffer( /* PRQA S 6030, 6060, 6080 */ /* MD_MSR_STCYC, MD_MSR_STPAR, MD_MSR_STMIF */
        uint8                                                                       CtrlIdx,
        Eth_FrameType                                                               FrameType,
        uint8                                                                       Priority,
  P2VAR(uint8,                                          AUTOMATIC, ETHIF_APPL_VAR)  BufIdxPtr,
 ...
 *
 */
FUNC(Std_ReturnType, ETHIF_CODE) EthIf_VTransmit( /* PRQA S 6060, 6080 */ /* MD_MSR_STPAR, MD_MSR_STMIF */
        uint8                                     CtrlIdx,
        uint8                                     BufIdx,
        Eth_FrameType                             FrameType,
        boolean                                   TxConfirmation,
 ...
 *
 */
FUNC(Std_ReturnType, ETHIF_CODE) EthIf_Transmit( /* PRQA S 6060 */ /* MD_MSR_STPAR */
        uint8                                     CtrlIdx,
        uint8                                     BufIdx,
        Eth_FrameType                             FrameType,
        boolean                                   TxConfirmation,
 ...
 *
 */
FUNC(void, ETHIF_CODE) EthIf_TxConfirmation( /* PRQA S 6080 */ /* MD_MSR_STMIF */
#if (ETHIF_ENABLE_AUTOSAR_FORWARD_COMPATIBILITY == STD_ON)
    uint8           CtrlIdx,
...
#endif /* ETHIF_ENABLE_AUTOSAR_FORWARD_COMPATIBILITY */
{
...
#endif /* ETHIF_DEV_ERROR_DETECT */
  {
    /* ----- Implementation ----------------------------------------------- */
    EthIf_TxContextIterType txCtxtIdx;
    uint8 localBufIdx = (uint8)BufIdx;

    txCtxtIdx = EthIf_CalcTxCtxtIdx(ethCtrlIdx, localBufIdx);
    ...
    if ( EthIf_IsValidOfTxContext(txCtxtIdx) == TRUE )
    {
           ...
           indIdx++ )
      {
        ...
        EthIf_GetTxConfirmations(txConfIdx)((uint8)EthIf_GetSnvOfEthIfCtrl(EthIf_GetEthIfCtrlIdxOfTxContext(txCtxtIdx)),
                                            localBufIdx);                                                               /* SBSW_ETHIF_USERAPI_FCT_PTR_CSL03 */
      }
