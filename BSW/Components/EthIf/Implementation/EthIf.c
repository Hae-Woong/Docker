 ...
 *
 *      \details  Implementation of main-module of EthIf containing only functionality independent functions like
 *                component initialization and version information.
 *
 *********************************************************************************************************************/
 ...
 *
 */
FUNC(void, ETHIF_CODE) EthIf_Init( /* PRQA S 6050 */ /* MD_MSR_STCAL */
  P2CONST(EthIf_ConfigType, AUTOMATIC, ETHIF_INIT_DATA) CfgPtr)
{
...
#endif /* ETHIF_DEV_ERROR_DETECT */
  {
    ...
    EthIf_ModuleInitialized = ETHIF_STATE_INIT;
  }
...
/* module specific MISRA deviations:
  ...
  MD_EthIf_311_RxIndication:
    Reason:     In AUTOSAR, the datatype of the parameters PhysAddrPtr and DataPtr in the API EthIf_RxIndication is
                pointer to const. But the upper layers' callback functions (<User>_RxIndication) which are called in
                EthIf_RxIndication need the datatype pointer to variable. Therefore is decided to convert the
                datatype of these two parameters in context of EthIf from pointer to const to pointer to variable,
                to be still downward compatible to AUTOSAR version smaller as 4.3.1.
    Risk:       Passing constant data that can't be accessed with write operations and therefore could result in
                an exception.
    Prevention: Covered by code review.
  ...
  MD_EthIf_2987:
    Reason:     QAC issue where it can not properly resolve MACROS and wrongly stating that it is a code with no side effects
                ...
                removed. The MACRO itself is verified to have the same outcome. This issue can be resolved in future QAC
                releases.
    Risk:       None.
  ...
  MD_EthIf_0883:
    Reason:     This is a false positive when there is a cyclic inclusion and it is also acknowledged by Perforce.
                Perforce's issue number (QCT-53274).
    Risk:       There is no risk as this finding is a false positive.
    Prevention: Covered by code review.
*/
...
\ID SBSW_ETHIF_USERAPI_FCT_PTR_CSL03
   ...
   \ID SBSW_ETHIF_PUB_FCT_PUB_PARAM_PTR_PASS
     \DESCRIPTION Passing a pointer provided during a call to a public function as parameter to a public function of
                  another module.
     \COUNTERMEASURE \R [CM_ETHIF_PUB_PARAM_PTR_CHECK]
   ...
   \ID SBSW_ETHIF_STACK_PARAM_PTR
     ...
     \COUNTERMEASURE \N Pointer is generated by allocating it on stack and immediately assigning a target to it.
