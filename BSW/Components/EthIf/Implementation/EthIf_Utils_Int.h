...
/**********************************************************************************************************************
 *  EthIf_GetPortIdxOfEthIfContext
 *********************************************************************************************************************/
 ...
 *  \synchronous    TRUE
 *********************************************************************************************************************/
ETHIF_UTILS_INT_LOCAL_INLINE FUNC(EthIf_EthSwtPortIterType, ETHIF_CODE) EthIf_GetPortIdxOfEthIfContext( /* PRQA S 3219 */ /* MD_EthIf_3219 */
  uint8 switchIdx,
  uint8 switchPortIdx);
  ...
  EthIf_EthCtrlIterType  ethCtrlIdx)
{
   ...
   *     by the EthIf_TxContext indirections mapped by ComStackLib. */
  return (uint8)(EthIf_GetTxContextEndIdxOfEthCtrl(ethCtrlIdx) - EthIf_GetTxContextStartIdxOfEthCtrl(ethCtrlIdx) - 1u);
} /* EthIf_GetMaxTxBufIdxForEthCtrl */
...
{
  /* ----- Local Variables ---------------------------------------------- */
  EthIf_EthSwtPortIterType queriedPortIdx = EthIf_GetSizeOfEthSwtPort();

  /* ----- Implementation ----------------------------------------------- */
  /* #10 If EthSwt is configured */
  if ( EthIf_GetSizeOfEthSwt() > 0u )
  {
    /* #110 Get the indirection for the switch port index */
    EthIf_EthSwtPortInEthSwtContextIterType ethSwtPortInEthSwtContext = EthIf_GetEthSwtPortInEthSwtContextStartIdxOfEthSwt(switchIdx) + (EthIf_EthSwtPortInEthSwtContextIterType)switchPortIdx;
    /* #120 If the indirection is in valid range */
    if ( ethSwtPortInEthSwtContext < EthIf_GetEthSwtPortInEthSwtContextEndIdxOfEthSwt(switchIdx) )
    {
      /* #1210 Get the port index in the context of EthIf */
      queriedPortIdx = EthIf_GetEthSwtPortIdxOfEthSwtPortInEthSwtContext(ethSwtPortInEthSwtContext);
    }
  }
