 ...
 *
 */
FUNC(BufReq_ReturnType, ETHIF_CODE) EthIf_ProvideExtTxBuffer( /* PRQA S 6030, 6060, 6080 */ /* MD_MSR_STCYC, MD_MSR_STPAR, MD_MSR_STMIF */
        uint8                                                                       CtrlIdx,
      ...
      if( EthIf_GetProvideExtTxBufferOfEthDrvApi(ethDrvApiIdx) != NULL_PTR)
      {
        ...
        if( retVal == BUFREQ_OK )
        {
          ...
          { /* PRQA S 0715 */ /* MD_MSR_1.1_715 */
            /* #30100 Trigger buffer provision finalization if buffer index is in expected range */
 ...
 *
 */
FUNC(Std_ReturnType, ETHIF_CODE) EthIf_ReleaseRxBuffer( /* PRQA S 6080 */ /* MD_MSR_STMIF */
        uint8                                     CtrlIdx,
...
# endif /* ETHIF_DEV_ERROR_DETECT */
  {
    ...
    if( EthIf_GetReleaseRxBufferOfEthDrvApi(ethDrvApiIdx) != NULL_PTR )
    {
      ...
      retVal = EthIf_GetReleaseRxBufferOfEthDrvApi(ethDrvApiIdx)((uint8)EthIf_GetSnvOfEthCtrl(ethCtrlIdx),
                                                                 (P2VAR(Eth_DataType, AUTOMATIC, ETHIF_APPL_VAR))tagAwareBufPtr); /* SBSW_ETHIF_DRVAPI_FCT_PTR_OPT_WITH_PUB_PARAM_PTR_CSL03 */
    }
# if ( ETHIF_DEV_ERROR_DETECT == STD_ON )                                                                               /* COV_ETHIF_SAFE_FEAT */
    else
    {
