...
ETHIF_RX_LOCAL_INLINE FUNC(boolean, ETHIF_RX_INLINE_CODE) EthIf_HandleUlRxIndication(
  ...
  P2CONST(EthIf_UlRxIndicationDataType, AUTOMATIC, ETHIF_APPL_DATA)  ulRxIndicationData);
 ...
 *
 */
ETHIF_RX_LOCAL_INLINE FUNC(boolean, ETHIF_RX_INLINE_CODE) EthIf_IsRxAllowed( /* PRQA S 6060 */ /* MD_MSR_STPAR */
          EthIf_EthCtrlIterType                                ethCtrlIdx,
    P2VAR(EthIf_EthIfCtrlIterType, AUTOMATIC, ETHIF_APPL_VAR)  ethIfCtrlIdx,
          boolean                                              doubleVlanTagged,
  P2CONST(EthIf_FrameHdrType,      AUTOMATIC, ETHIF_APPL_DATA) frameHdr,
...
#else
        ETHIF_DUMMY_STATEMENT_CONST(payloadPtr);  /* PRQA S 1338, 2983, 3122 */ /* MD_MSR_DummyStmt */
        ETHIF_DUMMY_STATEMENT(payloadLen);        /* PRQA S 1338, 2983, 3122 */ /* MD_MSR_DummyStmt */
...
#else
    ETHIF_DUMMY_STATEMENT(isReceivedByUser);       /* PRQA S 1338, 2983, 3122 */ /* MD_MSR_DummyStmt */
    ETHIF_DUMMY_STATEMENT(ethIfCtrlIdx);           /* PRQA S 1338, 2983, 3122 */ /* MD_MSR_DummyStmt */
    ETHIF_DUMMY_STATEMENT(ethCtrlIdx);             /* PRQA S 1338, 2983, 3122 */ /* MD_MSR_DummyStmt */
    ETHIF_DUMMY_STATEMENT_CONST(rxFrameHandling);  /* PRQA S 1338, 2983, 3122 */ /* MD_MSR_DummyStmt */
 ...
 *
 */
ETHIF_RX_LOCAL_INLINE FUNC(void, ETHIF_RX_INLINE_CODE) EthIf_Internal_RxIndication( /* PRQA S 6060 */ /* MD_MSR_STPAR */
        uint8                                     ethCtrlIdx,
        Eth_FrameType                             frameType,
        boolean                                   isBroadcast,
  P2VAR(uint8,        AUTOMATIC, ETHIF_APPL_VAR)  physAddrPtr,
        ...
        uint16                                    lenByte)
{
  ...
  {
#if ( ETHIF_EXTENDED_TRAFFIC_HANDLING_TYPE != ETHIF_NO_EXTENDED_TRAFFIC_HANDLING )                                      /* COV_ETHIF_QM_FEAT_CHECK_ON */
    /* #200 Invoke mirroring/gateway sub modules to check if frame is allowed to be passed to the upper layers */
    ...
    if( passFrameToUl == TRUE ) /* PRQA S 2991, 2995 */ /* MD_EthIf_2991, MD_EthIf_2995 */
    {
      ...
      EthIf_UlRxIndicationDataType rxIndData;
  ...
  EthIf_InvokeZeroCopyHandling(isReceivedByUser, ethIfCtrlIdx, ethCtrlIdx, &rxFrameHandling);                           /* SBSW_ETHIF_REF_OF_STACK_VAR */
 ...
 *
 */
FUNC(void, ETHIF_CODE) EthIf_RxIndication( /* PRQA S 6060 */ /* MD_MSR_STPAR */
          uint8                                     CtrlIdx,
          Eth_FrameType                             FrameType,
          boolean                                   IsBroadcast,
  P2CONST(uint8,        AUTOMATIC, ETHIF_APPL_VAR)  PhysAddrPtr,
  P2CONST(Eth_DataType, AUTOMATIC, ETHIF_APPL_VAR)  DataPtr,
          uint16                                    LenByte)
{
  /* ----- Local Variables ---------------------------------------------- */
  ...
  {
    /* ----- Implementation ----------------------------------------------- */
    P2VAR(uint8,        AUTOMATIC, ETHIF_APPL_VAR) physAddrInternalPtr =
                                                    (P2VAR(uint8,        AUTOMATIC, ETHIF_APPL_VAR))PhysAddrPtr;        /* PRQA S 0311 */ /* MD_EthIf_311_RxIndication */
    P2VAR(Eth_DataType, AUTOMATIC, ETHIF_APPL_VAR) dataInternalPtr =
                                                    (P2VAR(Eth_DataType, AUTOMATIC, ETHIF_APPL_VAR))DataPtr;            /* PRQA S 0311 */ /* MD_EthIf_311_RxIndication */
    /* #30 Trigger frame reception */
    EthIf_Internal_RxIndication((uint8)ethCtrlIdx,
                                FrameType, IsBroadcast, physAddrInternalPtr, dataInternalPtr, LenByte);                 /* SBSW_ETHIF_STACK_PARAM_PTR */
  }
 ...
 *
 */
FUNC(void, ETHIF_CODE) EthIf_MainFunctionRx( void ) /* PRQA S 6080 */ /* MD_MSR_STMIF */
{
