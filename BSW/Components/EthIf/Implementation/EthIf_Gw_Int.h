 ...
 **********************************************************************************************************************/
/*! \brief          Enables/disables the promiscuous mode of the Ethernet controller related to the EthIf controller in
 *                  in case EthIf controller is involved in traffic gateway.
 *  \details        Function enables/disables the promiscuous mode of the Ethernet controller related to the EthIf
 ...
 *                                ETH_ADD_TO_FILTER - enables promiscuous mode
 *                                ETH_REMOVE_FROM_FILTER - disables promiscuous mode
 *  \pre            -
 *  \context        TASK
 *  \reentrant      FALSE
 **********************************************************************************************************************/
  ...
  Eth_FilterActionType     filterAction)
{
  ...
  if( EthIf_IsGatewayDestEthIfCtrlIdxUsedOfEthIfCtrl(ethIfCtrlIdx) == TRUE )
  {
    ...
    /* #100 Apply promiscuous mode change by calling the driver */
...
#   if ( ETHIF_DEV_ERROR_REPORT == STD_ON )                                                                             /* COV_ETHIF_QM_FEAT_INNER */
    if( retVal != E_OK )
    {
      /* Promiscuous mode couldn't be enabled -> traffic gateway will not work */
      (void)Det_ReportError(ETHIF_MODULE_ID,
                            ETHIF_INSTANCE_ID_DET,
                            ETHIF_SID_GATEWAY_MANIPULATE_PROMISCUOUS_MODE,
                            ETHIF_TRAFFIC_GATEWAY_E_MANIPULATE_PROMISCUOUS_MODE);
    }
#   else
    ETHIF_DUMMY_STATEMENT(retVal); /* PRQA S 1338, 2983, 3122 */ /* MD_MSR_DummyStmt */
