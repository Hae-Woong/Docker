...
/**********************************************************************************************************************
 * EthIf_TransformSwtIdxFromEthSwtToEthIfContext
 **********************************************************************************************************************/
/*! \brief         This function converts switch index from EthIf context to EthSwt context
 *  \details       -
 *  \param[in]     ethSwtDrvIdx  EthSwt driver index in the context of EthIf
 *  \param[in,out] switchIdxPtr  Pointer to store the switch index:
 *                               [in] - EthSwt index in the context of EthSwt
 *                               [out] - EthSwt index in the context of EthIf
 *  \return        E_OK - EthSwt index in the context of EthIf found
 *  \return        E_NOT_OK - EthSwt index in the context of EthIf not found
 *  \pre           -
 *  \context       ANY
 *  \reentrant     TRUE
 **********************************************************************************************************************/
ETHIF_ETHSWT_LOCAL_INLINE FUNC(Std_ReturnType, ETHIF_LINK_INLINE_CODE) EthIf_TransformSwtIdxFromEthSwtToEthIfContext(
        EthIf_EthSwtDrvApiIterType        ethSwtDrvIdx,
  P2VAR(uint8, AUTOMATIC, ETHIF_APPL_VAR) switchIdxPtr);

/**********************************************************************************************************************
 * EthIf_Internal_GetPortMacAddr
 **********************************************************************************************************************/
/*! \brief         Internal API to retrieve the switch port a MAC address is assigned to.
 *  \details       -
 ...
 *  \reentrant     FALSE
 **********************************************************************************************************************/
ETHIF_ETHSWT_LOCAL_INLINE FUNC(Std_ReturnType, ETHIF_LINK_INLINE_CODE) EthIf_Internal_GetPortMacAddr(
  ...
  P2VAR(uint8, AUTOMATIC, ETHIF_APPL_VAR) switchIdxPtr)
{
  ...
  /* #10 Search for a matching driver local switch index within the EthIf switch index search range */
  for( ethIfSwtIdx = EthIf_GetEthSwtStartIdxOfEthSwtDrv(ethSwtDrvIdx);
        ethIfSwtIdx < EthIf_GetEthSwtEndIdxOfEthSwtDrv(ethSwtDrvIdx);
        ethIfSwtIdx++ )
  {
    /* #110 check if the driver context switch index matches */
    ...
    P2VAR(uint8, AUTOMATIC, ETHIF_APPL_VAR)  portIdxPtr)
{
  ...
  /* #10 For each Ethernet switch driver managed by EthIf */
  for( ethSwtDrvIdx = 0u;
        ...
        ethSwtDrvIdx++ )
  {
    ...
    if( EthIf_GetGetPortMacAddrOfEthSwtDrvApi(ethSwtDrvIdx) != NULL_PTR )
    {
     ...
     /* #1120 Check if call to driver was successful -> port information found */
      if( retVal == E_OK )
      {
        retVal = EthIf_TransformSwtIdxFromEthSwtToEthIfContext(ethSwtDrvIdx, switchIdxPtr);                             /* SBSW_ETHIF_INT_FCT_PARAM_PTR_PASS */
  ...
  uint8 switchPortIdx)
{
  ...
  {
    /* #110 Get the port index within the context of Ethernet interface */
    EthIf_EthSwtPortIterType portIdx = EthIf_GetPortIdxOfEthIfContext(switchIdx, switchPortIdx);

    /* #120 If the PortIdx is valid */
    if (portIdx < EthIf_GetSizeOfEthSwtPort())
    {
      /* #1210 If the switch port is in state ACTIVE */
      if( EthIf_GetEthSwtPortModeCnt(portIdx) > 0U )
      {
        /* #12110 Redirect call to the driver */
 ...
 *
 */
FUNC(Std_ReturnType, ETHIF_CODE) EthIf_GetPortMacAddr( /* PRQA S 6030, 6080 */ /* MD_MSR_STCYC, MD_MSR_STMIF */
  P2CONST(uint8, AUTOMATIC, ETHIF_APPL_DATA)  MacAddrPtr,
    P2VAR(uint8, AUTOMATIC, ETHIF_APPL_VAR)   SwitchIdxPtr,
    P2VAR(uint8, AUTOMATIC, ETHIF_APPL_VAR)   PortIdxPtr)
{
...
# endif /* ETHIF_DEV_ERROR_DETECT */
  {
    /* #310 Call the Internal function to retrieve the port MAC address */
    retVal = EthIf_Internal_GetPortMacAddr(MacAddrPtr, SwitchIdxPtr, PortIdxPtr);                                       /* SBSW_ETHIF_INT_FCT_PUB_PARAM_PTR_PASS */
  }
 ...
 *
 */
FUNC(Std_ReturnType, ETHIF_CODE) EthIf_GetArlTable( /* PRQA S 6080 */ /* MD_MSR_STMIF */
        uint8                                            SwitchIdx,
 ...
 *
 */
FUNC(Std_ReturnType, ETHIF_CODE) EthIf_GetDropCount( /* PRQA S 6080 */ /* MD_MSR_STMIF */
        uint8                               SwitchIdx,
 ...
 *
 */
FUNC(Std_ReturnType, ETHIF_CODE) EthIf_SwitchUpdateMCastPortAssignment( /* PRQA S 6080 */ /* MD_MSR_STMIF */
          uint8                              SwitchIdx,
          uint8                              PortIdx,
 ...
 *
 */
FUNC(Std_ReturnType, ETHIF_CODE) EthIf_SetSwitchMgmtInfo( /* PRQA S 6080 */ /* MD_MSR_STMIF */
        uint8                                             CtrlIdx,
 ...
 *
 */
FUNC(Std_ReturnType, ETHIF_CODE) EthIf_GetRxMgmtObject( /* PRQA S 6080 */ /* MD_MSR_STMIF */
        uint8                                                                               CtrlIdx,
 ...
 *
 */
FUNC(Std_ReturnType, ETHIF_CODE) EthIf_GetTxMgmtObject( /* PRQA S 6080 */ /* MD_MSR_STMIF */
        uint8                                                                               CtrlIdx,
        Eth_BufIdxType                                                                      BufIdx,
  P2VAR(P2VAR(EthSwt_MgmtObjectType, AUTOMATIC, ETHIF_APPL_VAR), AUTOMATIC, ETHIF_APPL_VAR) MgmtObjectPtr)
{
 ...
 *
 */
FUNC(Std_ReturnType, ETHIF_CODE) EthIf_SwitchEnableTimeStamping( /* PRQA S 6080 */ /* MD_MSR_STMIF */
        uint8                                            CtrlIdx,
        Eth_BufIdxType                                   BufIdx,
 ...
 *
 */
FUNC(Std_ReturnType, ETHIF_CODE) EthIf_GetSwitchPortSignalQuality( /* PRQA S 6080 */ /* MD_MSR_STMIF */
        uint8 SwitchIdx,
        uint8 SwitchPortIdx,
...
# endif /* ETHIF_DEV_ERROR_DETECT */
  {
    ...
    portIdx = EthIf_GetPortIdxOfEthIfContext(SwitchIdx, SwitchPortIdx);

    /* #40 Enter exclusive area (Reason: Ensure consistency of signal quality statistics) */
    EthIf_EnterExclusiveArea(SIGNAL_QUALITY); /* PRQA S 3109 */ /* MD_MSR_14.3 */

    /* #50 If the PortIdx is valid */
    if (portIdx < EthIf_GetSizeOfEthSwtPort())
    {
      /* #510 Get the port signal qualities */
...
# endif /* ETHIF_DEV_ERROR_DETECT */
  {
    ...
    /* #30 Get the port index within the context of Ethernet interface */
    portIdx = EthIf_GetPortIdxOfEthIfContext(SwitchIdx, SwitchPortIdx);
    /* #40 If the PortIdx is valid */
    if (portIdx < EthIf_GetSizeOfEthSwtPort())
    {
      /* #410 Clear the port signal qualities which are stored in Ethernet interface */
      EthIf_Reset_PortSignalQualityList(portIdx);
      retVal = E_OK;
    }
  }
...
FUNC(void, ETHIF_CODE) EthIf_PollPortSignalQualities( void )
{
       ...
       portIdx++ )
  {
    ...
    if (EthIf_Internal_GetPortSignalQuality(portIdx, EthIf_GetEthSwtIdxOfEthSwtPort(portIdx), (uint8)EthIf_GetSnvOfEthSwtPort(portIdx)) != E_OK){
