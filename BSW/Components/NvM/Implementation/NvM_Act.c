...
NVM_LOCAL CONST(NvM_CompiledConfigIdType, NVM_PUBLIC_CONST) NvM_CompiledConfigId_t = {(uint16)NVM_COMPILED_CONFIG_ID}; /* PRQA S 0759 */ /* MD_MSR_Union */
...
/**********************************************************************************************************************
 * NvM_IntUpdateCurrentBlockDataIntegrityCompareData
 *********************************************************************************************************************/
/*!
 * \brief       Updates or invalidates the current DataIntegrityRecordCompareMechanism buffer depending on
 *              the given job result.
 * \details     -
 * \param[in]   result - OK=store generated DataIntegrityRecord; NOT_OK=invalidate DataIntegrityRecord
 * \context     TASK
 * \reentrant   FALSE
 * \synchronous TRUE
 * \pre -
 */
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_IntUpdateCurrentBlockDataIntegrityCompareData(
    const NvM_RequestResultType result);
...
/**********************************************************************************************************************
 * NVM_ConvertServiceIdToBlockRequestType
 *********************************************************************************************************************/
/*! \brief Converts Service ID to Block Request Type.
 *  \details Is used if AUTOSAR 4.4.0 callback interface is enabled.
 ...
 *  \pre -
 */
NVM_LOCAL FUNC(NvM_BlockRequestType, NVM_PRIVATE_CODE) NVM_ConvertServiceIdToBlockRequestType(NvM_ServiceIdType ServiceId);

/**********************************************************************************************************************
 * NVM_ConvertServiceIdToMultiBlockRequestType
 *********************************************************************************************************************/
/*! \brief Converts Service ID to MultiBlock Request Type.
 *  \details Is used if AUTOSAR 4.4.0 callback interface is enabled.
 *  \param[in] ServiceId as a request type. Caller has to ensure validity.
 ...
 *  \pre -
 */
NVM_LOCAL FUNC(NvM_MultiBlockRequestType, NVM_PRIVATE_CODE) NVM_ConvertServiceIdToMultiBlockRequestType(NvM_ServiceIdType ServiceId);
 ...
 *
 */
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActFinishMainJob(void) /* PRQA S 6080 */ /* MD_MSR_STMIF */
{
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActInitPreReadAll(void)
{
    ...
    for (currBlockId = (NVM_TOTAL_NUM_OF_NVRAM_BLOCKS - 1u); currBlockId > 1u; currBlockId--)
    {
      if(NvM_IsBooleanBitfieldTrue(NvM_BlockDescriptorTable_at[currBlockId].Flags.SelectBlockForPreReadAllEnabled) == TRUE)
      {
        ...
        NvM_BlockNotification(currBlockId, NVM_SERVICE_ID_PRE_READ_ALL, NVM_REQ_PENDING);
      }
      else
      {
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActFinishReadBlock(void)
{
    ...
    NvM_IntUpdateCurrentBlockDataIntegrityCompareData(NvM_CurrentBlockInfo_t.LastResult_t);
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActFinishWriteBlock(void)
{
    ...
    if((NvM_CurrentBlockInfo_t.Descriptor_pt->MngmtType_t & NVM_BLOCK_REDUNDANT) != 0u)
    {
        ...
        /* one or both blocks defect -> use negative result to invalidate DataIntegrityRecordCompMechanism data,
         * otherwise update */
        NvM_IntUpdateCurrentBlockDataIntegrityCompareData((firstBlockDefect || secondBlockDefect) ?
            NVM_REQ_NOT_OK :
            NVM_REQ_OK);
    }
    else
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActInitReadAll(void)
{
    NvM_BlockIdType currBlockId = NVM_TOTAL_NUM_OF_NVRAM_BLOCKS;
    ...
    for (currBlockId = (NVM_TOTAL_NUM_OF_NVRAM_BLOCKS - 1u); currBlockId > lowestBlockId; currBlockId--)
    {
        ...
        if(NvM_IsBooleanBitfieldTrue(NvM_BlockDescriptorTable_at[currBlockId].Flags.SelectBlockForPreReadAllEnabled) == FALSE)
        {
            ...
            NvM_BlockNotification(currBlockId, NVM_SERVICE_ID_READ_ALL, NVM_REQ_PENDING);
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActFinishEraseBlock(void)
{
    ...
    NvM_IntUpdateCurrentBlockDataIntegrityCompareData(NVM_REQ_NOT_OK);
}
#endif /* (NVM_API_CONFIG_CLASS == NVM_API_CONFIG_CLASS_3) */
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActWriteNvBlock(void)
{
    ...
    /* Invalidate DataIntegrityRecordCompMechanism data to avoid following situation:
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActFinishRestoreRomDefaults(void)
{
  /* Given NULL_PTR from API indicating the use of PermanentRAM/ExplicitSync */
      ...
      ((NvM_CurrentBlockInfo_t.InternalFlags_u8 & NVM_INTFLAG_ROM_RESTORED_SET) == NVM_INTFLAG_ROM_RESTORED_SET))       /* COV_NVM_APICFGCLASS */
  {
    ...
    if (NvM_IsBooleanBitfieldTrue(NvM_CurrentBlockInfo_t.Descriptor_pt->Flags.CalcRamBlockCrcEnabled))                  /* COV_NVM_COVEREDINOTHERCFG */
    {
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActSetBlockPendingWriteAll(void)
{
    ...
    NvM_BlockNotification(NvM_CurrentJob_t.JobBlockId_t, NVM_SERVICE_ID_WRITE_ALL, NVM_REQ_PENDING);
}
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActGetMultiBlockJob(void)
 {
    ...
    if ((multiJobFlags & NVM_APIFLAG_WRITE_ALL_SET) != 0u)
    {
        ...
        NvM_CurrentJob_t.JobBlockId_t = (NVM_TOTAL_NUM_OF_NVRAM_BLOCKS - 1u);
    }
 ...
 *
 */
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_IntUpdateCurrentBlockDataIntegrityCompareData(const NvM_RequestResultType result)
{
  if (NvM_CurrentBlockInfo_t.Descriptor_pt->DataIntegrityRecordCompMechanismBuffer != NULL_PTR)
  {
    ...
    else
    {
      /* Invalidate buffer */
      uint8 i;
      for (i = 0u; i < NvM_DataIntegrity_GetIntegrityCodeLength(NvM_CurrentJob_t.JobBlockId_t); i++)                    /* FETA_NvM_IntUpdateCurrentBlockDataIntegrityCompareData */
      {
        NvM_CurrentBlockInfo_t.Descriptor_pt->DataIntegrityRecordCompMechanismBuffer[i] = 0xFFu; /* SBSW_NvM_AccessArray_CrcBuffers */
      }
    }
  }
}
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE)  NvM_InitializeBlockForReadAll(void)
{
  ...
  if(NvM_CurrentJob_t.JobBlockId_t < NVM_TOTAL_NUM_OF_NVRAM_BLOCKS)
  {
    ...
    /* Reset flags needed for ReadAll DataIntegrityRecordCompareMechanism case of ExplicitSync blocks
...
NVM_LOCAL FUNC(NvM_BlockRequestType, NVM_PRIVATE_CODE)  NVM_ConvertServiceIdToBlockRequestType(NvM_ServiceIdType ServiceId)
{
  ...
  switch (ServiceId)
  {
    ...
    case NVM_SERVICE_ID_RESTORE_BLOCK_DEFAULTS:
        ...
        break;
    case NVM_SERVICE_ID_ERASE_BLOCK:
    ...
    case NVM_SERVICE_ID_READ_ALL:
        ...
        break;
    case NVM_SERVICE_ID_PRE_READ_ALL:                                                                                   /* COV_NVM_COVEREDINOTHERCFG */
        blockRequest = NVM_PRE_READ_ALL_BLOCK;
        break;
    case NVM_SERVICE_ID_WRITE_ALL:
    default:                                                                                                            /* COV_NVM_MISRA */
        blockRequest = NVM_WRITE_ALL_BLOCK;
        break;
  }
...
NVM_LOCAL FUNC(NvM_MultiBlockRequestType, NVM_PRIVATE_CODE) NVM_ConvertServiceIdToMultiBlockRequestType(NvM_ServiceIdType ServiceId)
{
  ...
  switch (ServiceId)
  {
      ...
      case NVM_SERVICE_ID_WRITE_ALL:
          ...
          break;
      case NVM_PRE_READ_ALL:                                                                                            /* COV_NVM_COVEREDINOTHERCFG */
      default:                                                                                                          /* COV_NVM_MISRA */
          multiBlockRequest = NVM_PRE_READ_ALL;
          break;
  }
 ...
 *
 */
FUNC(void, NVM_PRIVATE_CODE) NvM_BlockNotification(NvM_BlockIdType BlockId, NvM_ServiceIdType ServiceId, NvM_RequestResultType JobResult)
{
  ...
  if (orgBlockId == BlockId)
  {
    ...
    if( (JobResult != NVM_REQ_PENDING)
        && ((ServiceId != NVM_SERVICE_ID_WRITE_ALL) || allowWriteAllInvocation)
        && ((ServiceId != NVM_SERVICE_ID_READ_ALL) || allowReadAllInvocation)
#if(NVM_PRE_READALL_API == STD_ON)
        && ((ServiceId != NVM_SERVICE_ID_PRE_READ_ALL) || allowReadAllInvocation)
#endif
      )
    {
#if(NVM_USE_ASR440_CALLBACK_INTERFACE == STD_ON)
        NvM_BlockRequestType requestId = NVM_ConvertServiceIdToBlockRequestType(ServiceId);
#else
        NvM_ServiceIdType requestId = ServiceId;
#endif

      if (blockDescriptorPtr->JobEndCbkFunc_pt != NULL_PTR)
      {
        (void)blockDescriptorPtr->JobEndCbkFunc_pt(requestId, JobResult); /* SBSW_NvM_FuncPtrCall_UserCallbacks */
      }

      if (blockDescriptorPtr->JobEndCbkExtFunc_pt != NULL_PTR)
      {
        (void)blockDescriptorPtr->JobEndCbkExtFunc_pt(BlockId, requestId, JobResult); /* SBSW_NvM_FuncPtrCall_UserCallbacks */
      }
    }
...
#if(NVM_USE_ASR440_CALLBACK_INTERFACE == STD_ON)
  NvM_MultiBlockRequestType requestId = NVM_ConvertServiceIdToMultiBlockRequestType(ServiceId);  /* PRQA S 3205 */ /* MD_NvM_2.3_MultiBlockCbk_UnusedVariablesDueToFunctionMacro */
#else
  NvM_ServiceIdType requestId = ServiceId;  /* PRQA S 3205 */ /* MD_NvM_2.3_MultiBlockCbk_UnusedVariablesDueToFunctionMacro */
#endif

  NvM_invokeCurrentJobMode(requestId, JobResult);  /* SBSW_NvM_Macro_FuncCall */

  if(JobResult != NVM_REQ_PENDING)
  {
    NvM_invokeMultiCbk(requestId, JobResult);  /* SBSW_NvM_Macro_FuncCall */
  }
}
...
{
  NvM_invokeDetectedLossOfRedundancyCallback(BlockId);  /* SBSW_NvM_Macro_FuncCall */
}
