 ...
 *  VERSION CHECK
 *********************************************************************************************************************/
#if ((NVM_SW_MAJOR_VERSION != (11u)) \
        || (NVM_SW_MINOR_VERSION != (3u)))
# error "Version numbers of NvM.c and NvM.h are inconsistent!"
#endif

#if ((NVM_CFG_MAJOR_VERSION != (10u)) \
...
FUNC(void, NVM_PUBLIC_CODE) NvM_Init(void)
{
    ...
    NvM_CrcJob_Create(&NvM_AsyncCrcJob_t, (NvM_BlockIdType)0, NULL_PTR, (uint16)0, NvM_NoOfCrcBytes_u16); /* SBSW_NvM_FuncCall_PtrParam_CrcAsyncJob */
#endif
    ...
    for (i = 0u; i < NVM_TOTAL_NUM_OF_NVRAM_BLOCKS; i++)
    {
      NvM_BlockMngmtArea_at[i].NvDataIndex_t = 0u; /* SBSW_NvM_AccessBlockManagementArea */
    }
 ...
 *
 */
FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_SetDataIndex(NvM_BlockIdType BlockId, uint8 DataIndex) /* PRQA S 6050, 6080 */ /* MD_MSR_STCAL */
{
    ...
    {
        NvM_Errorhook(NVM_SERVICE_ID_SET_DATA_INDEX, detErrorId);
    }
#else
...
FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_GetDataIndex(NvM_BlockIdType BlockId, P2VAR(uint8, AUTOMATIC, NVM_APPL_DATA) DataIndexPtr)
{
    ...
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_GET_DATA_INDEX, detErrorId);
    }
#else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
#endif
 ...
 *
 */
FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_SetBlockProtection(NvM_BlockIdType BlockId, boolean ProtectionEnabled) /* PRQA S 6050, 6080 */ /* MD_MSR_STCAL, MD_MSR_STMIF */
{
  Std_ReturnType returnValue = E_NOT_OK;
  uint8 detErrorId = NVM_E_NO_ERROR;
...
# if (NVM_DEV_ERROR_REPORT == STD_ON)
  if(detErrorId != NVM_E_NO_ERROR)
  {
    NvM_Errorhook(NVM_SERVICE_ID_SET_BLOCK_PROTECTION, detErrorId);
  }
# else
  NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
# endif
    ...
    P2VAR(NvM_RequestResultType, AUTOMATIC, NVM_APPL_DATA) RequestResultPtr)
{
    ...
    else if (NVM_BLOCK_FROM_DCM_ID(BlockId) >= NVM_TOTAL_NUM_OF_NVRAM_BLOCKS)
    {
        detErrorId = NVM_E_PARAM_BLOCK_ID;
    }
    ...
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_GET_ERROR_STATUS, detErrorId);
    }
#else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
#endif
...
# if(NVM_DEV_ERROR_REPORT == STD_ON)
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_GET_VERSION_INFO, detErrorId);
    }
# else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
# endif
 ...
 *
 */
FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_SetRamBlockStatus(NvM_BlockIdType BlockId, boolean BlockChanged) /* PRQA S 6050, 6080 */ /* MD_MSR_STCAL, MD_MSR_STMIF */
{
  Std_ReturnType returnValue = E_NOT_OK;
  uint8 detErrorId = NVM_E_NO_ERROR;
...
# if (NVM_CALC_RAM_CRC_USED == STD_ON)
      /* CalcRamBlockCrc can only be set for blocks that have a valid CRC configured,
       * this is ensured by validation before generating the configuration.*/
...
# if (NVM_DEV_ERROR_REPORT == STD_ON)
  if(detErrorId != NVM_E_NO_ERROR)
  {
    NvM_Errorhook(NVM_SERVICE_ID_SET_RAM_BLOCK_STATUS, detErrorId);
  }
# else
  NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
# endif
 ...
 *
 */
FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_ReadBlock(NvM_BlockIdType BlockId, P2VAR(void, AUTOMATIC, NVM_APPL_DATA) NvM_DstPtr) /* PRQA S 6050, 6080 */ /* MD_MSR_STCAL, MD_MSR_STMIF */
{
    Std_ReturnType returnValue = E_NOT_OK;
    uint8 detErrorId = NVM_E_NO_ERROR;
...
# endif /* ECO_IGNORE_LINE */
    {
        /* PRQA S 0316 1 */ /* MD_NvM_11.5_JobQueue_CastVoidPtrToObjPtr */
...
# if (NVM_DEV_ERROR_REPORT == STD_ON)
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_READ_BLOCK, detErrorId);
    }
# else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
# endif
...
{
    return NvM_ReadBlock(BlockId, NULL_PTR); /* SBSW_NvM_Forward_Api_Call_With_NULL_PTR */
}
 ...
 *
 */
FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_WriteBlock(NvM_BlockIdType BlockId, P2CONST(void, AUTOMATIC, NVM_APPL_DATA) NvM_SrcPtr) /* PRQA S 6050, 6080 */ /* MD_MSR_STCAL, MD_MSR_STMIF */
{
    Std_ReturnType returnValue = E_NOT_OK;
    uint8 detErrorId = NVM_E_NO_ERROR;
...
# if (NVM_DEV_ERROR_REPORT == STD_ON)
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_WRITE_BLOCK, detErrorId);
    }
# else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
# endif
 ...
 *
 */
FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_WritePRAMBlock(NvM_BlockIdType BlockId)
{
    return NvM_WriteBlock(BlockId, NULL_PTR); /* SBSW_NvM_Forward_Api_Call_With_NULL_PTR */
}
 ...
 *
 */
FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_RestoreBlockDefaults(NvM_BlockIdType BlockId, P2VAR(void, AUTOMATIC, NVM_APPL_DATA) NvM_DstPtr) /* PRQA S 6050, 6080 */ /* MD_MSR_STCAL, MD_MSR_STMIF */
{
    Std_ReturnType returnValue = E_NOT_OK;
    uint8 detErrorId = NVM_E_NO_ERROR;
...
# if (NVM_DEV_ERROR_REPORT == STD_ON)
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_RESTORE_BLOCK_DEFAULTS, detErrorId);
    }
# else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
# endif
 ...
 *
 */
FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_RestorePRAMBlockDefaults(NvM_BlockIdType BlockId)
{
    return NvM_RestoreBlockDefaults(BlockId, NULL_PTR); /* SBSW_NvM_Forward_Api_Call_With_NULL_PTR */
}
#endif /* (NVM_API_CONFIG_CLASS > NVM_API_CONFIG_CLASS_1) */
 ...
 *
 */
FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_EraseNvBlock(NvM_BlockIdType BlockId) /* PRQA S 6050, 6080 */ /* MD_MSR_STCAL, MD_MSR_STMIF */
{
    Std_ReturnType returnValue = E_NOT_OK;
    uint8 detErrorId = NVM_E_NO_ERROR;
...
# if (NVM_DEV_ERROR_REPORT == STD_ON)
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_ERASE_BLOCK, detErrorId);
    }
# else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
# endif
 ...
 *
 */
FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_InvalidateNvBlock(NvM_BlockIdType BlockId) /* PRQA S 6050, 6080 */ /* MD_MSR_STCAL, MD_MSR_STMIF */
{
    Std_ReturnType returnValue = E_NOT_OK;
    uint8 detErrorId = NVM_E_NO_ERROR;
...
# if (NVM_DEV_ERROR_REPORT == STD_ON)
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_INVALIDATE_NV_BLOCK, detErrorId);
    }
# else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
# endif
...
# if (NVM_DEV_ERROR_REPORT == STD_ON)
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_CANCEL_JOBS, detErrorId);
    }
# else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
# endif
...
FUNC(void, NVM_PUBLIC_CODE) NvM_ReadAll(void)
{
...
#endif /* ECO_IGNORE_LINE */
    {
        ...
        NvM_MultiBlockCbk(NVM_SERVICE_ID_READ_ALL, NVM_REQ_PENDING);
    }

#if (NVM_DEV_ERROR_REPORT == STD_ON)
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_READ_ALL, detErrorId);
    }
#else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
#endif
 ...
 *
 */
FUNC(void, NVM_PUBLIC_CODE) NvM_PreReadAll(void)
{
    uint8 detErrorId = NVM_E_NO_ERROR;
...
# endif /* ECO_IGNORE_LINE */
    {
        ...
        NvM_BlockMngmtArea_at[0].NvRamErrorStatus_u8 = NVM_REQ_PENDING; /* SBSW_NvM_AccessBlockManagementArea */
        NvM_ApiFlags_u8 |= NVM_APIFLAG_PRE_READ_ALL_SET;

        NvM_ExitCriticalSection();

        NvM_MultiBlockCbk(NVM_SERVICE_ID_PRE_READ_ALL, NVM_REQ_PENDING);
    }

# if (NVM_DEV_ERROR_REPORT == STD_ON)
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_PRE_READ_ALL, detErrorId);
    }
# else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
# endif
...
FUNC(void, NVM_PUBLIC_CODE) NvM_KillReadAll(void)
{
    ...
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_KILL_READ_ALL, detErrorId);
    }
#else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
#endif
...
FUNC(void, NVM_PUBLIC_CODE) NvM_WriteAll(void)
{
...
#endif /* ECO_IGNORE_LINE */
    {
        ...
        NvM_MultiBlockCbk(NVM_SERVICE_ID_WRITE_ALL, NVM_REQ_PENDING);
    }

#if (NVM_DEV_ERROR_REPORT == STD_ON)
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_WRITE_ALL, detErrorId);
    }
#else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
#endif
...
FUNC(void, NVM_PUBLIC_CODE) NvM_CancelWriteAll(void)
{
    ...
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_CANCEL_WRITE_ALL, detErrorId);
    }
#else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
#endif
...
# endif /* ECO_IGNORE_LINE */
    {
        ...
        if((NvM_ApiFlags_u8 & NVM_APIFLAG_WRITE_ALL_SET) == NVM_APIFLAG_WRITE_ALL_SET)
        {
            ...
            {
                NvM_BlockNotification(blockEndNotificationId, NVM_SERVICE_ID_WRITE_ALL, mngmt_pt->NvRamErrorStatus_u8);
            }

            NvM_MultiBlockCbk(NVM_SERVICE_ID_WRITE_ALL, NVM_REQ_CANCELED);
        }
    }
...
# if (NVM_DEV_ERROR_REPORT == STD_ON)
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_KILL_WRITE_ALL, detErrorId);
    }
# else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
# endif
...
# if (NVM_DEV_ERROR_REPORT == STD_ON)
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_REPAIR_REDUNDANT_BLOCKS, detErrorId);
    }
# else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
# endif
...
FUNC(void, NVM_PUBLIC_CODE) NvM_MainFunction(void)
{
...
#endif /* ECO_IGNORE_LINE */
    {
        ...
        do                                                                                                              /* FETA_NVM_MainFunction_TaskLoop */
        {
    ...
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_MAINFUNCTION, detErrorId);
    }
#else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
#endif
...
FUNC(void, NVM_PUBLIC_CODE) NvM_JobErrorNotification(void)
{
    ...
    switch(NvM_MemIfAbstraction_GetJobResult((uint8)NvM_CurrentBlockInfo_t.Descriptor_pt->DeviceId_u8))
    {       
...
FUNC(void, NVM_PUBLIC_CODE) NvM_SetBlockLockStatus(NvM_BlockIdType BlockId, boolean BlockLocked)
{
    ...
    if(detErrorId != NVM_E_NO_ERROR)
    {
        NvM_Errorhook(NVM_SERVICE_ID_SET_BLOCK_LOCK_STATUS, detErrorId);
    }
#else
    NVM_DUMMY_STATEMENT(detErrorId); /* PRQA S 3112 */ /* MD_MSR_DummyStmt */
#endif
...
NVM_LOCAL_INLINE FUNC(boolean, NVM_PRIVATE_CODE) NvM_CheckBlockId(const NvM_BlockIdType BlockId)
{
    return (boolean)((NVM_BLOCK_FROM_DCM_ID(BlockId) > 0u) && (NVM_BLOCK_FROM_DCM_ID(BlockId) < NVM_TOTAL_NUM_OF_NVRAM_BLOCKS)); /* COV_NVM_COVEREDINOTHERCFG */
}
# endif
...
NVM_LOCAL_INLINE FUNC(boolean, NVM_PRIVATE_CODE) NvM_CheckBlockPending(const NvM_BlockIdType BlockId)
{
      ...
      ))
    {
        ...
        /* #120 NvM_(Pre)ReadAll is pending */
        else
        {
            /* (Pre)ReadAll begins with configuration block/first user block and ends with last block -> lower block Id than current is done */
            blockPending = (boolean)(BlockId > NvM_CurrentJob_t.JobBlockId_t);
        }
    }
...
NVM_LOCAL_INLINE FUNC(void, NVM_PRIVATE_CODE) NvM_ProcessBackgroundCrcJobs(void)
{
  if(!NvM_CrcJob_isBusy(&NvM_AsyncCrcJob_t))
  {
    ...
    NvM_CrcJob_Create(&NvM_AsyncCrcJob_t, newCrcBlockId, crcRecalculationPtr, descr_pt->NvBlockLength_u16, NvM_NoOfCrcBytes_u16);             /* SBSW_NvM_FuncCall_PtrParam_CrcAsyncJob */
...
# if (NVM_DATA_INTEGRITY_INT_BUFFER == STD_ON)
    ...
    NvM_CrcJob_ReassignBuffer(&NvM_AsyncCrcJob_t, descr_pt->RamBlockDataIntegrityAddr_t);                                         /* SBSW_NvM_AccessPtr_CrcReassignBuffer */
# endif
  }

  /* Process CRC chunk */
  NvM_CrcJob_Process(&NvM_AsyncCrcJob_t);                                                                               /* SBSW_NvM_FuncCall_PtrParam_CrcAsyncJob */
}
#endif
...
MD_NvM_8.9_DataIntegrity_SubmoduleUsedOnlyInOneFunction:
  Reason:     The data integrity submodule uses redirections to explicit data integrity modules. Instances to it are
              stored at "private" file scope.
  Risk:       No risk.
  Prevention: No prevention, component test and reviews ensure functionality.
...
MD_NvM_11.5_JobQueue_CastVoidPtrToObjPtr:
...
MD_NvM_11.5_DataIntegrityJob_CastVoidPtrToObjPtr:
  Reason:     DataIntegrity acts as a proxy to different data integrity mechanisms. They are processed via "jobs" that
              are again represented as a OOP pointer.
  Risk:       Wrong job structure could be injected to software submodule. This could lead to wrong memory access.
  Prevention: Before usage of each forwarding a certain check is performed to ensure correct job structure usage.
...
MD_NvM_1.1_DataIntegrity_IncompatibleFPtrTypes:
  Reason:     The function pointer of the data integrity module have void pointers for the processed jobs whereas the
              used submodule needs a concrete type.
  Risk:       Incompatible pointer could be injected into software submodule, causing disallowed memory access.
  Prevention: No prevention, review and component test ensure functionality.

MD_NvM_2.3_MultiBlockCbk_UnusedVariablesDueToFunctionMacro:
  Reason:     The MultiBlock callback contains function macros and in some configurations the defined variables
              in the function body are not used because this variables are only used within the function macro.
  Risk:       No risk.
  Prevention: None.
...
\ID SBSW_NvM_Forward_Api_Call_With_NULL_PTR
 \DESCRIPTION The service request is forwarded with a NULL_PTR. 
 \COUNTERMEASURE \R The called function checks the given parameters to be valid, in case development error mode is enabled.
...
\ID SBSW_NvM_AccessMacJobPtr
 \DESCRIPTION To setup and process MAC jobs, NvM uses a job structure storing all relevant information for data processing
              All functions assume the pointer to be valid.
 \COUNTERMEASURE \N Code review will ensure correct usage of Self pointer.

\ID SBSW_NvM_MacInternalFunctions
 \DESCRIPTION NvM MAC module internally uses helper methods to keep the code readable and maintainable.
 \COUNTERMEASURE \N Code review will ensure correct usage of internal functions.

\ID SBSW_NvM_MacInternalFunctionsWithOutParameters
 \DESCRIPTION NvM MAC module internally uses helper methods that use out-parameters for further processing. The pointers
              are always valid as only local and valid stack variables are used performing those internal operations.
 \COUNTERMEASURE \N Code review will ensure correct usage of the out-parameters.

\ID SBSW_NvM_MacExportFunctionByteCopy
 \DESCRIPTION NvM MAC module has an API function to copy buffer bytes to a target array.
 \COUNTERMEASURE \T Tests and code review will ensure correct usage of the CSM API. Tests:
                    TCASE-1785854
                    TCASE-1785855
                    TCASE-1785856
                    TCASE-1785858
...
\ID SBSW_NvM_MacExternalFunctionCallsToCsm
 \DESCRIPTION NvM MAC module used the API of the Csm module to generate and verify MACs.
 \COUNTERMEASURE \T Tests and code review will ensure correct usage of the CSM API. Tests:
                    TCASE-1785854
                    TCASE-1785855
                    TCASE-1785856
...
\ID SBSW_NvM_AccessDataIntegrityJobPtr
 \DESCRIPTION To setup and process data integrity jobs, NvM uses a job structure storing all parameters and
              function pointers to forward job processing. All functions assume the pointer to be valid.
 \COUNTERMEASURE \N Code review will ensure correct wiring of function pointers to submodule is done correctly.
...
\ID SBSW_NvM_FuncPtrCall_DataIntegrityModule
 \DESCRIPTION To do the integrity code computation in a generic way, NvM uses function pointers in the job structure to forward to the correct algorithm.
              The concrete algorithms will set these function pointers when a new job arrives via the functions NvM_DataIntegrity_CreateGenerateJob and
              NvM_DataIntegrity_CreateValidateJob. It is a mandatory precondition to first create the jobs and then do any actions on them.
 \COUNTERMEASURE \T TCASE-1529283, TCASE-1529284.
...
\ID SBSW_NvM_FuncCall_CrcModule
 ...
 \ID SBSW_NvM_FuncCall_DataIntegrityForwarding
 \DESCRIPTION To get the integrity code, DataIntegrity submodule forwards the given parameters to the concrete submodules (e.g. CRC) and does not
              change them on its own.
 \COUNTERMEASURE \N The caller of the DataIntegrity functions is responsible that only valid parameters are given to the API.
...
\ID SBSW_NvM_AccessArray_MemIfRequestState
 \DESCRIPTION In case requests are forwarded cross core wise, NvM stores the current state of all underlying devices within an
              array structure accessible by both cores. The array is accessed via a DeviceIndex parameter or it is accessed
              within a for loop (MEMIF_NUMBER_OF_DEVICES times).
 \COUNTERMEASURE \T Correct NvM behavior is checked during component tests and code reviews. The DeviceIndex parameter must be forwarded correctly
                    by upper layers. The break condition within for loop (MEMIF_NUMBER_OF_DEVICES) must be defined correctly by MemIf.

\ID SBSW_NvM_Macro_FuncCall
 \DESCRIPTION To call user defined functions, function macros are used. They are always invoked with valid parameters.
 \COUNTERMEASURE \T Correct NvM behavior is checked during component tests and code reviews.
...
\ID COV_NVM_NOCRCDUMMYHANDLER
 \ACCEPT X
 \REASON All NvM CRC handlers have the same structure: they provide a function to calculate, compare and copy data.
         For blocks without CRC the NvM uses a dummy CRC handler, which simply does nothing, but exists to simplify CRC
         handling: NvM does not have to check whether the block has a CRC each time, the assigned CRC handler does it.
         The uncovered function is not having any code functionality.
...
\ID COV_NVM_READALL_EXPLICITSYNC_NO_RETRY
 \ACCEPT TF tf tx
 \ACCEPT XX xx xx
 \REASON NvM currently does not support explicit synchronization retry handling. The information ExplSyncAttemptCounter
         and NVM_READALL_MAX_NUM_EXPLICITSYNC_RETRY_ATTEMPTS are kept for easy future extension of the feature usage.
         The counter is currently only available for state machine flow handling as the success flag is not enough for
         safe code.

\ID COV_NVM_ASYNC_REQUEST_ALWAYS_ACCEPTED
 \ACCEPT XF
 \REASON If job forwarding to memory core is enabled the MemIfAbstraction always accepts the job. So the code is 
         unreachable. It is tested in other configurations.

\ID COV_NVM_COPY_ALWAYS
 \ACCEPT TX
 \REASON If job forwarding to memory core is enabled the NvM uses always the intenal buffer as source buffer. For that
         reason the else path is not reachable. The else path is tested in other configurations.
...
\ID FETA_NVM_InternalCopyData
 \DESCRIPTION Copy mechanism between NvM and user data buffer that shall be limited to a configurable amount of
              bytes per main loop.
 \R Processed length is limited by NVM_MAX_DATA_COPY_BYTES.

\ID FETA_NVM_RepairRedBlocksInitNext
 \DESCRIPTION Searching next redundant block to be processed.
 \R Iterations are limited by NVM_TOTAL_NUM_OF_NVRAM_BLOCKS.

\ID FETA_NvM_IntUpdateCurrentBlockDataIntegrityCompareData
 \DESCRIPTION Invalidation of DataIntegrityCompareMechanism buffer.
 \R The used function to determine the iterations of this loop 'NvM_DataIntegrity_GetIntegrityCodeLength' ensures that
    the correct CRC length or a fallback of zero is given.
    The correct MAC buffer size has to be ensured via integrator.

\ID FETA_NVM_NvM_InitializeBlockForReadAll
  \DESCRIPTION Searching next relevant block for ReadAll service to be processed.
  \R Within the while loop a runtime check is implemented to limit iterations to NVM_TOTAL_NUM_OF_NVRAM_BLOCKS.

\ID FETA_NVM_CrcGetQueuedBlockId
 \DESCRIPTION Find a CRC job within the queue.
 \R A runtime check is implemented within the queue to ensure a limited amount of iterations depending on the queue
    size (newBlockId >= queueSizeWords).

\ID FETA_NVM_Mac_ExportAndCompare
 \DESCRIPTION Export/Compare generated MAC to a given buffer.
 \R MAC size is directly given to NvM_Mac_CreateGenerateJob when initializing the job. This again is derived from the
    configuration, which must be ensured to be correct by the integration (SMI-4).

\ID FETA_NVM_QueueInit
 \DESCRIPTION Link queue elements.
 \R Determination of the used variable 'maxQueueIndex' ensures limited iterations of the loop.

\ID FETA_NVM_ActGetNormalPrioJob
 \DESCRIPTION Find queued element with highest priority.
 \R The loops checks for 'currentElem != NvM_NormalPrioQueue.SrvList'. The iterations are therefore limited to the
    amount of elements the queue can hold.

\ID FETA_NVM_QueueFindBlock
 \DESCRIPTION Search for an element with the given block ID.
 \R The loop checks for 'elem != QueueHead'. The iterations are therefore limited to the amount of elements the queue
    can hold.

\ID FETA_NVM_MainFunction_TaskLoop
 \DESCRIPTION Execution of Task/MainState/SubState loop.
 \D By design (DSGN-NvMDiag2138) the IDLE state of NvM will always directly leave the loop. In state ACTIVE each
    designated service FSM is designed to release the loop when necessary or sensible. This is ensured by review of
    design elements and the corresponding implementation of it.
