 ...
 *********************************************************************************************************************/
/*!
 * \brief           Standard CRC job setup handling.
 * \details         -
 * \param[in,out]   Self                  Job pointer to be set up
 ...
 * \synchronous     TRUE
 */
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_Crc_SetupJob(
...
/**********************************************************************************************************************
 *  NvM_Crc_ProcessBlockId
 *********************************************************************************************************************/
/*!
 * \brief           Calculate block ID into CRC job.
 * \details         -
 * \param[in,out]   Self                  Job pointer to be set up
 * \param[in]       BlockId               Corresponding block ID
 * \pre             -
 * \config          NVM_USE_BLOCK_ID_CHECK must be enabled
 * \context         TASK
 * \reentrant       FALSE
 * \synchronous     TRUE
 */
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_Crc_ProcessBlockId(
    NvM_CrcJobPtrType Self,
    NvM_BlockIdType BlockId);
#endif
 ...
 * \param[in,out]   Self      Pointer to CRC job to process.
 ...
 *  \param[in,out] CurrentValue, value not relevant, because dummy function.
 *  \context TASK
 *  \reentrant FALSE
 *  \synchronous TRUE
 *  \pre -
...
/*! \brief Do nothing, function is a dummy (blocks without Crc).
 *  \details Do nothing, function is a dummy (blocks without Crc).
 *  \param[in,out] Dest, value not relevant, because dummy function.
 ...
 *  \param[in,out] Dest to copy to source to. Must not be NULL_PTR. Caller has to ensure correctness.
...
{
  uint16 i;
  for (i = 0u; i < NVM_SIZE_CRC_JOB_QUEUE; i++)
  {
    NvM_CrcQueue_at[i] = 0u;                                                                                          /* SBSW_NvM_AccessCrcQueue */
  }
...
FUNC(NvM_BlockIdType, NVM_PRIVATE_CODE) NvM_CrcGetQueuedBlockId(void)
{
    ...
    while (bitPos >= NVM_CRC_QUEUE_ENTRY_BITS)                                                                          /* FETA_NVM_CrcGetQueuedBlockId */
    {
...
NVM_LOCAL FUNC(uint8, NVM_PRIVATE_CODE) NvM_CrcQueueCountTrailingZeros(NvM_CrcQueueEntryType Word)
{
    ...
    else
    {
        ...
        if((currWord & 0xFFFFu) == 0u)                                                                                  /* COV_NVM_COVEREDINOTHERCFG */
        {
...
FUNC(void, NVM_PRIVATE_CODE) NvM_CrcJob_Create(
    ...
    uint16 BytesPerProcessCycle)
{
  NvM_Crc_SetupJob(Self, BlockId, RamDataPtr, DataLength, BytesPerProcessCycle);                                        /* SBSW_NvM_FuncCall_CrcModule */

  Self->ProcessingStatus = NVM_CRCPROCESSINGSTATUS_COMPATIBILITY;                                                       /* SBSW_NvM_AccessCrcJobPtr */

#if(NVM_USE_BLOCK_ID_CHECK == STD_ON)
  NvM_Crc_ProcessBlockId(Self, BlockId);                                                                                /* SBSW_NvM_FuncCall_CrcModule */
#endif
}
 ...
 *
 */
FUNC(void, NVM_PRIVATE_CODE) NvM_CrcJob_CreateGenerateJob(                                                              /* PRQA S 6060 */ /* MD_MSR_STPAR */
    NvM_CrcJobPtrType Self,
    NvM_BlockIdType BlockId,
    NvM_RamAddressType RamDataPtr,
    uint16 DataLength,
    ...
    uint16 BytesPerProcessCycle)
{
  ...
  Self->ProcessingStatus = NVM_CRCPROCESSINGSTATUS_GENERATE_PENDING;                                                    /* SBSW_NvM_AccessCrcJobPtr */
  Self->CrcBuffer = TargetBuffer;                                                                                       /* SBSW_NvM_AccessCrcJobPtr */
 ...
 *
 */
FUNC(void, NVM_PRIVATE_CODE) NvM_CrcJob_CreateValidateJob(                                                              /* PRQA S 6060 */ /* MD_MSR_STPAR */
    NvM_CrcJobPtrType Self,
    NvM_BlockIdType BlockId,
    NvM_RamAddressType RamDataPtr,
    uint16 DataLength,
    ...
    uint16 BytesPerProcessCycle)
{
  ...
  Self->ProcessingStatus = NVM_CRCPROCESSINGSTATUS_VALIDATE_GENERATION_PENDING;                                         /* SBSW_NvM_AccessCrcJobPtr */
  Self->CrcBuffer = VerifyBuffer;                                                                                       /* SBSW_NvM_AccessCrcJobPtr */
 ...
 *
 */
FUNC(NvM_DataIntegrityStatus, NVM_PRIVATE_CODE) NvM_CrcJob_GetStatus(
    NvM_CrcJobConstPtrType Self)
{
  NvM_DataIntegrityStatus status;

  switch (Self->ProcessingStatus)
  {
  case NVM_CRCPROCESSINGSTATUS_GENERATE_PENDING:
  case NVM_CRCPROCESSINGSTATUS_VALIDATE_GENERATION_PENDING:
    status = NVM_DATAINTEGRITYSTATUS_PENDING;
    break;

  case NVM_CRCPROCESSINGSTATUS_GENERATE_FINISHED:
  case NVM_CRCPROCESSINGSTATUS_VALIDATE_FINISHED_SUCCESSFUL:
    status = NVM_DATAINTEGRITYSTATUS_FINISHED_SUCCESSFUL;
    break;

  case NVM_CRCPROCESSINGSTATUS_VALIDATE_FINISHED_UNSUCCESSFUL:
  default:                                                                                                              /* COV_NVM_MISRA */
    status = NVM_DATAINTEGRITYSTATUS_FINISHED_UNSUCCESSFUL;
    break;
  }
 ...
 *
 */
FUNC(void, NVM_PRIVATE_CODE) NvM_CrcJob_Process(NvM_CrcJobPtrType Self)
{
  if(Self->RemainingLength_u16 > 0u)                                                                                    /* COV_NVM_COVEREDINOTHERCFG */
  {
    ...
    if(currLength > Self->BytesPerProcessCycle)
    {
      currLength = Self->BytesPerProcessCycle;
    }
    ...
    /* Processing of automatic validation/generation job has finished */
    if ((Self->ProcessingStatus != NVM_CRCPROCESSINGSTATUS_COMPATIBILITY) && (Self->RemainingLength_u16 == 0u))
    {
      /* VALIDATE job was generating data to compare */
      if (Self->ProcessingStatus == NVM_CRCPROCESSINGSTATUS_VALIDATE_GENERATION_PENDING)
      {
        if (NvM_CrcJob_Compare(Self))                                                                                   /* SBSW_NvM_FuncCall_CrcModule */
        {
          /* Successful comparison. state transition -> validation success */
          Self->ProcessingStatus = NVM_CRCPROCESSINGSTATUS_VALIDATE_FINISHED_SUCCESSFUL;                                /* SBSW_NvM_AccessCrcJobPtr */
        }
        else
        {
          /* Erroneous comparison. state transition -> validation error */
          Self->ProcessingStatus = NVM_CRCPROCESSINGSTATUS_VALIDATE_FINISHED_UNSUCCESSFUL;                              /* SBSW_NvM_AccessCrcJobPtr */
        }
      }
      else /* GENERATE job running */
      {
        /* Export CRC result to target buffer */
        NvM_CrcJob_CopyToBuffer(Self);                                                                                  /* SBSW_NvM_FuncCall_CrcModule */

        /* state transition -> finished */
        Self->ProcessingStatus = NVM_CRCPROCESSINGSTATUS_GENERATE_FINISHED;                                             /* SBSW_NvM_AccessCrcJobPtr */
      }
    }
  }
}
    ...
    uint16 BytesPerProcessCycle)
{
  ...
  Self->HandlerInstance_pt = NvM_CrcHandlerTable_at[NVM_BLOCK_DATA_INTEGRITY_OFF];                                      /* SBSW_NvM_AccessCrcJobPtr */
  if (descr_pt->DataIntegritySettings < NVM_BLOCK_MAC_ON)                                                               /* COV_NVM_COVEREDINOTHERCFG */
  {
    Self->HandlerInstance_pt = NvM_CrcHandlerTable_at[descr_pt->DataIntegritySettings];                                 /* SBSW_NvM_AccessCrcJobPtr */
  }
   ...
   * HINT: Needed for CRC dummy handling concept within the older parts of NvM */
  Self->RemainingLength_u16 =                                                                                           /* SBSW_NvM_AccessCrcJobPtr */
      ((Self->HandlerInstance_pt->crcLength > 0u) && (RamDataPtr != NULL_PTR)) ? DataLength : 0u;                       /* COV_NVM_COVEREDINOTHERCFG */
  Self->BytesPerProcessCycle = BytesPerProcessCycle;                                                                    /* SBSW_NvM_AccessCrcJobPtr */
 ...
 *
 */
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_Crc_NoCrc_CopyToBuffer(                                                      /* COV_NVM_NOCRCDUMMYHANDLER */
