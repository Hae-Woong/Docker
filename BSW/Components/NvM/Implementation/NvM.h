 ...
 * 8.02.00                virsrl  MWDG-4546     NvM tamper detection: Implement normalized subcomponent to hide data integrity
 *                        virsrl  MWDG-4548     NvM tamper detection: Add CSM MAC handling to data integrity service
 * 8.02.01                virbmz  ESCAN00108918 Compiler error: 'Csm.h': no such file or directory 
 *                        virbmz  ESCAN00108929 Compiler error: Undeclared identifier 'NvM_CsmCryptoJobs'
 *                        virsrl  ESCAN00108944 CSM tamper detection is not functional
 * 9.00.00                virlra  MWDG-3655     Provide and use the generated NvM_MemMap.h
 *                        virsrl  ESCAN00108465 NvM block management data mapped to incorrect MemMap section
 * 9.01.00                virsrl  MWDG-5380     NvM shall provide its JobEnd and JobError notifications compatible to ASR4.4 Fee
 *                        virsrl  ESCAN00109231 Compiler warning: Illegal types of function pointer assignment (NvM_DataIntegrity.c)
 * 9.01.01                virsrl  ESCAN00109565 Compiler error: Incompatible Section Mapping (depends on compiler)
 * 9.01.02                vireno  ESCAN00110782 Compiler error: NvM API service declared but not defined 
 *                        vireno  ESCAN00110784 Compiler error: Incompatible Section Mapping (depends on compiler)
 *                        vireno  ESCAN00109733 NvM completely stucks during processing of a job in non-polling mode (trunk)
 * 10.00.00               vireno  MWDG-5004     NvM: Remove CRC32 compatibility mode
 * 10.01.00               virbka  MWDG-4193     NvM: Split ReadAll into two phases
 * 10.02.00               vireno  MWDG-4194     NvM: Make reading of config block dependent on NvMDynamicConfiguration
 * 10.03.00               virbka  MWDG-7056     NvM: Create MemIf abstraction with configurable usage scenario
 * 10.03.00               vireno  MWDG-7060     NvM: Support synchronous services on MemIf core forwarding feature
 *                        virbmz  MWDG-7169     NvM: Add separate internal buffer for immediate prio jobs
 *                        vireno  MWDG-7224     NvM Init Block Callback Extended may forward NvM internal buffers to the user
 *                        vireno  ESCAN00112064 Compiler error: NvM_InternalBuffer_au8 is undeclared
 * 10.03.01               virsrl  MWDG-6785     Finite Execution Time Analysis on NvM
 * 11.00.00               vireno  MWDG-7388     NvM AR4.3 service interface compliance
 * 11.01.00               virsrl  MWDG-7452     NvM: Support MAC Compare Mechanism
 * 11.01.01               virsrl  ESCAN00114279 Out of bounds write access to NvM_CrcInternalExplicitSyncBuffer
 * 11.02.00               smacht  MWDG-8311     Make NvM_NotifyJobFinished according to 20-11
 * 11.03.00               smacht  ESCAN00115961 Compiler error: Unexpected linkage of NvM_CompiledConfigId_t
 *                        smacht  ESCAN00115650 Compiler warning: Unused variable CompiledConfigId if dynamic configuration is off
 *                        smacht  ESCAN00114088 Compiler error: Invalid attribute in NvM_Crc.h
 **********************************************************************************************************************/
 ...
 *  INCLUDES
 *********************************************************************************************************************/
/* NvM_Cfg.h includes Std_Types.h -> no explicit include here! */
#include "NvM_Types.h"
#include "NvM_Cfg.h"

/* AUTOSAR4.4 Provide callback definitions implicitly via NvM.h.
 * Basically it should be introduced via an extra header "NvM_MemIf.h". SWS_Fee_00098 is inconsistent with this.
 */
#include "NvM_Cbk.h"
...
#define NVM_SW_MAJOR_VERSION       (11u)
#define NVM_SW_MINOR_VERSION       (3u)
 ...
 *  NvM_ReadPRAMBlock
 **********************************************************************************************************************/
/*! \brief Setups a read job for given permanent RAM NvM block
 *  \details Request to copy the data of the NV block to its corresponding permanent RAM block. 
 *  \param[in] BlockId in range [1, (number of blocks - 1)].
 *  \return E_OK job was setup
 *  \return E_NOT_OK otherwise
 *  \context ANY
 ...
 *  \trace CREQ-724
 */
extern FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_ReadPRAMBlock
    (
        NvM_BlockIdType BlockId
    );
 ...
 *  NvM_WritePRAMBlock
 **********************************************************************************************************************/
/*! \brief Setups a write job for given permanent RAM NvM block
 *  \details Request to copy the data of the permanent RAM block to its corresponding NV block.
 *  \param[in] BlockId in range [1, (number of blocks - 1)].
 *  \return E_OK job was setup
 *  \return E_NOT_OK otherwise
 *  \context ANY
 ...
 *  \trace CREQ-725
 */
extern FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_WritePRAMBlock
    (
        NvM_BlockIdType BlockId
    );
 ...
 **********************************************************************************************************************/
/*! \brief Setups a restore block defaults job for given permanent RAM NvM block
 *  \details Request to restore the ROM block default data to its corresponding permanent RAM block.
 *  \param[in] BlockId in range [1, (number of blocks - 1)].
 *  \return E_OK job was setup
 *  \return E_NOT_OK otherwise
 *  \context ANY
 ...
 *  \trace CREQ-728
 */
extern FUNC(Std_ReturnType, NVM_PUBLIC_CODE) NvM_RestorePRAMBlockDefaults
    (
        NvM_BlockIdType BlockId
    );
 ...
 *  NvM_CancelJobs
 **********************************************************************************************************************/
/*! \brief Cancel currently pending job for given NvM block
 *  \details Request to cancel all jobs pending in the queue for the specified NV Block.
 *           Note:
 *           Only jobs that are in the waiting queue can be cancelled. An active job therefore cannot be cancelled.
 *  \param[in] BlockId in range [1, (number of blocks - 1)].
 ...
 *  NvM_ReadAll
 **********************************************************************************************************************/
/*! \brief Setup a read all job.
 *  \details Request to read the data of all "Select Block For ReadAll" configured RAM blocks from their corresponding
 *           NV blocks. Only the data of valid NV blocks is read.
 *  \context ANY
 *  \reentrant FALSE
 *  \synchronous FALSE
 *  \pre -
 ...
 *  NvM_PreReadAll
 **********************************************************************************************************************/
/*! \brief Setup the first phase of a read all job.
 *  \details Request to read the data of all "Select Block For Pre ReadAll" configured RAM blocks from their
 *           corresponding NV blocks. Only the data of valid NV blocks is read. Afterwards, NvM is operable for the
 *           blocks already read out.
 *           This service shall be called before the default read all multi block service is executed. The read all is
 *           then splitted into two separate phases.
 *  \context ANY
 *  \reentrant FALSE
 *  \synchronous FALSE
 *  \pre -
 *  \trace CREQ-312932
 */
extern FUNC(void, NVM_PUBLIC_CODE) NvM_PreReadAll(void);
#endif
 ...
 *           Other write/erase requests shall be rejected. At processing NvM_ReadAll or NvM_PreReadAll a locked block
 *           shall be loaded from NV memory.
 *  \param[in] BlockId in range [1, (number of blocks - 1)].
