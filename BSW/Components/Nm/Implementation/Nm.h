 ...
 *  11.00.00  2018-02-01  visgle      STORYC-3896   Implementation of Nm GW Calibratable
 *  11.00.01  2018-03-13  visgle      STORYC-3896   Review changes: Mask correction in Nm_BusNmSpecificPduRxIndication().
 *                                                  Unnecessary encapsulation for PartEcuToActToNodeId functions removed.
 *  12.00.00  2018-09-10  vispkn      STORYC-6129   Update to MISRA2012
 *  13.00.00  2019-03-25  visgle      STORYC-7950   SDLC Gateway calibratable table expansion
 *  14.00.00  2020-11-27  vispps      NMM-1127      Added MultiPartition Support
 *  14.01.00  2021-09-13  vispps      NMM-1414      Finalize MultiPartition Support
 *                                    ESCAN00109397 Compiler error: l-value specifies const object with NM_DUMMY_STATEMENT
 *  15.00.00  2021-11-18  vispps      NMM-1668      Added support for ASR MemMap include structure
 *  15.01.00  2021-12-21  smarcelin   NMM-1739      Updated AUTOSAR SPEC IDs
 *            2022-01-13  vispps      NMM-1843      Added support for synchronized PNC shutdown
 *                                    ESCAN00111273 Missing DET for services used when not initialized correctly
 *  16.00.00  2022-04-22  vispps      NMM-2081      Finite Execution Time Analysis
 *  16.00.01  2022-08-11  vispps      ESCAN00105822 State Report Signal does not contain the correct signal value
 *  17.00.00  2023-05-26  smarcelin   MISRA compliance
 *  18.00.00  2023-09-26  smarcelin   NMM-2963      Extend number of possible channels for Ford SDLC GwExt Feature
 *********************************************************************************************************************/
...
# if ( (NM_STACKTYPES_MAJOR_VERSION != (18u)) \
...
# define NM_SW_MAJOR_VERSION                      (18u)
...
# define NM_SID_REQUESTSYNCHRONIZEDPNCSHUTDOWN    (0x24u)
# define NM_SID_PREINIT                           (0x30u)
# define NM_SID_POSTINIT                          (0x31u)
/*-----------------------------------------------*/
# define NM_SID_MAINFUNCTION_SATELLITE            (0x80u)
...
/*! API service called from invalid partition context */
# define NM_E_INVALID_PARTITION                   (0x25u)
/*! API Nm_PreInit() was not called prior to Nm_Init() */
# define NM_E_NO_PREINIT                          (0x26u)
/*! BusNm has returned E_NOT_OK when Nm forwarded the API service */
# define NM_E_BUSNM_E_NOT_OK                      (0x27u)
/*! API service used without module initialization of all instances */
# define NM_E_NO_POSTINIT                         (0x28u)
/*! API service used while the module is already initialized on the current instance */
# define NM_E_ALREADY_INITIALIZED                 (0x29u)
/*! Invalid generated data was detected. */
# define NM_E_INVALID_GENDATA                     (0x30u)
/*! No error */
...
#  define Nm_PreInit(ConfigPtr)                                 NM_DUMMY_STATEMENT_CONST(ConfigPtr)
#  define Nm_Init(ConfigPtr)                                    NM_DUMMY_STATEMENT_CONST(ConfigPtr)
#  define Nm_PostInit()
# endif
...
#  define NM_CONCAT_FUNC_MACRO_EXPANSION( prefix, function )    prefix ## function /* PRQA S 0342 */ /* MD_MSR_Rule20.10_0342 */
#  define NM_CONCAT_FUNC( prefix, function )                    NM_CONCAT_FUNC_MACRO_EXPANSION( prefix, function ) /* PRQA S 3453 */ /* MD_MSR_FctLikeMacro */
#  define NM_MAP_CALL_2_BUSNM( function )                       NM_CONCAT_FUNC( NM_BUSNM_TYPE_PREFIX, function ) /* PRQA S 3453 */ /* MD_MSR_FctLikeMacro */
...
/* PRQA S 3453 2 */ /* MD_MSR_FctLikeMacro */
#   define Nm_RequestSynchronizedPncShutdown( nmNetworkHandle, pncId ) \
          (NM_MAP_CALL_2_BUSNM( _RequestSynchronizedPncShutdown)( (nmNetworkHandle), (pncId) ))
#  endif
 ...
 *  Nm_PreInit()
 *********************************************************************************************************************/
/*! \brief       Pre-Initializes the NM Interface.
 *  \details     Initializes the configuration pointer for the current postbuild variant.
 *               Called by EcuM.
 ...
 *  Nm_PostInit()
 *********************************************************************************************************************/
/*! \brief       Post-Initializes the NM Interface.
 *  \details     Displays the overall state of the module initialization over all partitions.
 *               Called by application or BswM.
 *  \param[in]   -
 *  \pre         Module is pre-initialized.
 *  \context     TASK
 *  \reentrant   FALSE
 *  \synchronous TRUE
 *  \trace       CREQ-609
 ...
 *               initialized. Called by application or BswM.
 *  \param[in]   nmConfigPtr      Configuration pointer for initializing the module
 *  \pre         Module is uninitialized.
 *  \context     TASK
 *  \reentrant   FALSE
 ...
 *  \trace       SPEC-4168, SPEC-4144, SPEC-19576, SPEC-4217
 *********************************************************************************************************************/
 ...
 *  \config      (NM_MACRO_LAYER_ENABLED == STD_OFF)
 *  \trace       CREQ-616
 *  \trace       SPEC-4185, SPEC-4189, SPEC-19573, SPEC-4144
 *********************************************************************************************************************/
 ...
 *********************************************************************************************************************/
/*! \brief       Requests the synchronized PNC shutdown at the lower layer by calling the <BusNm>_RequestSynchronizedPncShutdown bus specific function
 *  \details     Called by application (ComM)
 *  \param[in]   nmNetworkHandle       Identification of the NM-channel
 *  \param[in]   pncId                 Identification of the PNC identifier
 *  \return      E_OK       Request for a synchronized PNC shutdown successful
 *  \return      E_NOT_OK   Request for a synchronized PNC shutdown has failed
 *  \pre         -
 *  \context     TASK|ISR1|ISR2
 *  \reentrant   TRUE for the different network handles
 *  \synchronous TRUE
 *  \config      ( NM_SYNCHRONIZED_PNC_SHUTDOWN_ENABLED == STD_ON )
 *  \trace       CREQ-Nm-RequestSyncPncShutdown
 *********************************************************************************************************************/
extern FUNC( Std_ReturnType, NM_CODE ) Nm_RequestSynchronizedPncShutdown( CONST( NetworkHandleType, AUTOMATIC ) nmNetworkHandle, CONST( PNCHandleType, AUTOMATIC ) pncId );
#  endif
 ...
 *  \trace       CREQ-628
 *  \trace       SPEC-4165, SPEC-4183, SPEC-19577
 *********************************************************************************************************************/
 ...
 *  \config      (NM_MACRO_LAYER_ENABLED == STD_OFF) && ( NM_COM_CONTROL_ENABLED == STD_ON )
 *  \trace       CREQ-627
 *  \trace       SPEC-4192, SPEC-4156, SPEC-19570
 *********************************************************************************************************************/
 ...
 *  \trace       CREQ-629
 *  \trace       SPEC-4166, SPEC-4167, SPEC-4250, SPEC-19603
 *********************************************************************************************************************/
 ...
 *  \config      (NM_MACRO_LAYER_ENABLED == STD_OFF) && ( NM_NODE_ID_ENABLED == STD_ON )
 *  \trace       CREQ-636
 *  \trace       SPEC-4197, SPEC-4149, SPEC-19575
 *********************************************************************************************************************/
 ...
 *  \trace       CREQ-637
 *  \trace       SPEC-4181, SPEC-4186, SPEC-19597
 *********************************************************************************************************************/
 ...
 *  \trace       CREQ-612
 *  \trace       SPEC-4115, SPEC-4148, SPEC-19589
 *********************************************************************************************************************/
 ...
 * \endexclusivearea
 *
 * \exclusivearea NM_EXCLUSIVE_AREA_2
 * Protects the synchronization of Nm Interface coordinator related variables from being modified by any interrupt.
 * Ensures data consistency of the coordinator state during data synchronization from satellite partitions.
 * \protects Nm_CoordinatorState, Nm_CoordinatorStateSyncCounter and Nm_CoordinatorStateMasterSyncCounter.
 * \usedin Nm_MainFunction()
 * \exclude -
 * \length SHORT no calls inside and at max 2 bytes are copied within this exclusive area.
 * \endexclusivearea
 *
 * \exclusivearea NM_EXCLUSIVE_AREA_3
 * Protects the synchronization of Nm Interface channel requested state variables from being modified by any interrupt.
 * Ensures data consistency of the channel requested state during data synchronization from satellite partitions.
 * \protects Nm_ChannelRequested and Nm_ChannelRequestedSyncCounter.
 * \usedin Nm_MainFunction(), Nm_PassiveStartUp(), Nm_NetworkRequest() and Nm_NetworkRelease().
 * \exclude -
 * \length SHORT no calls inside and at max 2 bytes are copied within this exclusive area.
 * \endexclusivearea
 */
