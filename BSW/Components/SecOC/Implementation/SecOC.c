...
# include "IpduM.h"
#endif
# if((SECOC_FRESHNESSNOTAVALIBLEEXTERNALINDEX == STD_ON) || (SECOC_MACVERIFICATIONFAILEDEXTERNALINDEX == STD_ON))
# include "IdsM.h"
#endif
 ...
 **********************************************************************************************************************/
#ifndef SECOC_VERIFICATIONSUCCESS /* COV_SECOC_FRESHNESSVALUE_INTERFACE */
#define SECOC_VERIFICATIONSUCCESS 0
#endif
#ifndef SECOC_VERIFICATIONFAILURE /* COV_SECOC_FRESHNESSVALUE_INTERFACE */
#define SECOC_VERIFICATIONFAILURE 1
#endif
#ifndef SECOC_FRESHNESSFAILURE /* COV_SECOC_FRESHNESSVALUE_INTERFACE */
#define SECOC_FRESHNESSFAILURE 2
#endif
#ifndef SECOC_AUTHENTICATIONBUILDFAILURE /* COV_SECOC_FRESHNESSVALUE_INTERFACE */
#define SECOC_AUTHENTICATIONBUILDFAILURE 3
#endif
#ifndef SECOC_NO_VERIFICATION /* COV_SECOC_FRESHNESSVALUE_INTERFACE */
#define SECOC_NO_VERIFICATION 4
#endif  
#ifndef SECOC_VERIFICATIONFAILURE_OVERWRITTEN /* COV_SECOC_FRESHNESSVALUE_INTERFACE */
#define SECOC_VERIFICATIONFAILURE_OVERWRITTEN 5
#endif

#define SECOC_DATAID_SIZE 2u
...
/*! \brief      Handle IF rx PDUs.
    \details    -
    \pre        -
    \param[in]  mainFuncIdx  mainfunction struct index
**********************************************************************************************************************/
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_RxIf_HandlePdus(SecOC_SizeOfMainFunctionRxStructType mainFuncIdx);
#endif
...
**********************************************************************************************************************/
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_TxIf_HandlePdus(SecOC_SizeOfMainFunctionTxStructType mainFuncIdx);
#endif
...
# if (SECOC_GETTXFRESHNESSTRUNCDATAFUNCTABLE == STD_ON)
  if(SecOC_IsGetTxFreshnessTruncDataFuncTableUsedOfTxPduInfo(txPduId)) /* COV_SECOC_FRESHNESS_INTERFACE */
  {
    ...
    {
      SecOCTxFreshnessTruncDataType getTxFreshnessTruncData = SecOC_GetGetTxFreshnessTruncDataFuncTable(SecOC_GetGetTxFreshnessTruncDataFuncTableIdxOfTxPduInfo(txPduId));
      if(getTxFreshnessTruncData != NULL_PTR) /* COV_SECOC_SILENT_CHECK */
      {
        /* *INDENT-OFF* */
        ret = getTxFreshnessTruncData(
...
# endif
# if (SECOC_GETTXFRESHNESSFUNCTABLE == STD_ON)
  if(SecOC_IsGetTxFreshnessFuncTableUsedOfTxPduInfo(txPduId)) /* COV_SECOC_FRESHNESS_INTERFACE */
  {
    if(freshnessValueLength <= ((uint32) (freshnessValueBufferLength) << 3u))   /* COV_SECOC_SAFEBSW_CHECK */
    {
      SecOCTxFreshnessType getTxFreshness = SecOC_GetGetTxFreshnessFuncTable(SecOC_GetGetTxFreshnessFuncTableIdxOfTxPduInfo(txPduId)); 
      if(getTxFreshness != NULL_PTR) /* COV_SECOC_SILENT_CHECK */
      {
        /* *INDENT-OFF* */
        ret = getTxFreshness(
          SecOC_GetExternalFreshnessIdOfTxPduInfo(txPduId),
...
# if(SECOC_FRESHNESSNOTAVALIBLEEXTERNALINDEX == STD_ON)    
    IdsM_SetSecurityEvent(SecOC_GetFreshnessNotAvalibleExternalIndex(0u));
# endif
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_FreshnessValue_CopyVerifyValueToBuffer(SecOC_RxPduInfoIterType rxPduId, P2CONST(uint8, AUTOMATIC, SECOC_APPL_DATA) freshnessVerifyValue)
{
  ...
  for(; destValueidx < SecOC_GetAuthenticPduDataContainerFreshnessVerifyValueEndIdxOfRxPduInfo(rxPduId); destValueidx++) /* FETA_SECOC_01 */
  {
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_FreshnessValue_WriteTruncFreshnessValue(SecOC_TxPduInfoIterType txPduId, P2CONST(uint8, AUTOMATIC, SECOC_APPL_DATA) trucFreshnessValue)
{
  ...
  for(; iterValue > SecOC_GetSecuredPduDataContainerTruncatedFreshnessValueStartIdxOfTxPduInfo(txPduId); iterValue--) /* FETA_SECOC_01 */
  {
  ...
  {
    {
      ...
      /* iterate from the LSB to the MSB. The address of the MSB will then be smaller then the address of the LSB. */
      for(; valueidx < SecOC_GetAuthenticPduDataContainerFreshnessVerifyValueEndIdxOfTxPduInfo(txPduId); valueidx++) /* FETA_SECOC_01 */
      {
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_FreshnessValue_GetParsedFreshnessValueMSBAligned(SecOC_RxPduInfoIterType rxPduId, P2VAR(uint8, AUTOMATIC, SECOC_APPL_DATA) parsedFreshnessValue, uint8 parsedFreshnessValueBufferLength)
{
  if(parsedFreshnessValueBufferLength >= SecOC_GetSecuredPduDataContainerTruncatedFreshnessValueLengthOfRxPduInfo(rxPduId))     /* COV_SECOC_SAFEBSW_CHECK */
  {
    ...
    for(; iterValueidx < SecOC_GetSecuredPduDataContainerTruncatedFreshnessValueEndIdxOfRxPduInfo(rxPduId); iterValueidx++) /* FETA_SECOC_01 */
    {
      uint8 mask = 0xFFu;
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_FreshnessValue_GetParsedFreshnessValue(SecOC_RxPduInfoIterType rxPduId, P2VAR(uint8, AUTOMATIC, SECOC_APPL_DATA) parsedFreshnessValue, uint8 parsedFreshnessValueBufferLength)
{
  if(parsedFreshnessValueBufferLength >= SecOC_GetSecuredPduDataContainerTruncatedFreshnessValueLengthOfRxPduInfo(rxPduId))     /* COV_SECOC_SAFEBSW_CHECK */
  {
    ...
    for(; iterValueidx > SecOC_GetSecuredPduDataContainerTruncatedFreshnessValueStartIdxOfRxPduInfo(rxPduId); iterValueidx--) /* FETA_SECOC_01 */
    {
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_CopyAuthenticationPatternIntoMacAndFvMValue(SecOC_TxPduInfoIterType txPduId)
{
  ...
  for(; destIdx < SecOC_GetSecuredPduDataContainerAuthInfoDataEndIdxOfTxPduInfo(txPduId); destIdx++) /* FETA_SECOC_01 */
  {
...
SECOC_LOCAL_INLINE FUNC(uint8, SECOC_CODE) SecOC_TxIf_CalculatePaddingSize(SecOC_SecuredPduDataContainerAuthenticPduLengthOfTxPduInfoType securedPduLength)
{
  ...
  for(; cmpValueInfoIdx < SecOC_GetSizeOfCanFDLengthCompValueInfo(); cmpValueInfoIdx++) /* FETA_SECOC_01 */
  {
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_TxIf_AddCANFDPadding(SecOC_TxPduInfoIterType txPduId)
{
  ...
  if(paddingSize > 0u)
  {
    ...
    for(; authenticPduBufferIdx < SecOC_GetSecuredPduDataContainerAuthenticPduEndIdxOfTxPduInfo(txPduId); authenticPduBufferIdx++) /* FETA_SECOC_01 */
    {
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_TxIf_HandlePdus(SecOC_SizeOfMainFunctionTxStructType mainFuncIdx)
{
  SecOC_TxPduInfoIterType txPduIdInd = SecOC_GetTxPduInfoIndStartIdxOfMainFunctionTxStruct(mainFuncIdx);
  
  for(; txPduIdInd < SecOC_GetTxPduInfoIndEndIdxOfMainFunctionTxStruct(mainFuncIdx); txPduIdInd++) /* FETA_SECOC_01 */
  {
    SecOC_TxPduInfoIterType txPduId = SecOC_GetTxPduInfoInd(txPduIdInd);
...
# endif
    ...
    if(retVal == E_OK)
    {
# if (SECOC_SPDUTXCONFIRMATIONFUNCTABLE == STD_ON)
      if(SecOC_IsSPduTxConfirmationFuncTableUsedOfTxPduInfo(txPduId))
      {
        SecOCSPduTxConfirmationType sPduTxConfirmation = SecOC_GetSPduTxConfirmationFuncTable(SecOC_GetSPduTxConfirmationFuncTableIdxOfTxPduInfo(txPduId));
        if (sPduTxConfirmation != NULL_PTR) /* COV_SECOC_SILENT_CHECK */
        {
          sPduTxConfirmation(SecOC_GetExternalFreshnessIdOfTxPduInfo(txPduId)); /* SBSW_SECOC_PTR2FUNC */
        }
      }
# endif
      SecOC_SetTransmitRequest(txPduId, SECOC_IDLE_TRANSMITREQUEST);    /* SBSW_SECOC_CSL02 */
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_Verify_PropagateVerificationStatus(SecOC_RxPduInfoIterType rxPduId, SecOC_VerificationResultType verificationStatus)
{
  SecOCRteVerificationStatusType rteVerificationStatus = SecOC_GetRteVerificationStatusFuncTable(SecOC_GetRteVerificationStatusFuncTableIdxOfRxPduInfo(rxPduId));
...
# if (SECOC_VERIFICATIONSTATUSCALLOUT == STD_ON)
  {
    ...
    for(; calloutIdx < SecOC_GetSizeOfVerificationStatusCallout(); calloutIdx++) /* FETA_SECOC_01 */
    {
      SecOC_GetVerificationStatusCallout(calloutIdx) (verificationStatusStruct);        /* SBSW_SECOC_VERIFY_STATUS_CALLOUT */
    }
  }
# endif
  if(rteVerificationStatus != NULL_PTR) /* COV_SECOC_SILENT_CHECK */
  {
    (void) rteVerificationStatus(&verificationStatusStruct);    /* SBSW_SECOC_PTR2FUNC */
  }
}
#endif
...
#  endif
    ...
    for(; securedCalloutIdx < SecOC_GetSizeOfVerificationStatusCalloutWithSecuredPdu(); securedCalloutIdx++) /* FETA_SECOC_01 */
    {
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_Verify_PropagateClientServerVerificationStatus(SecOC_RxPduInfoIterType rxPduId, SecOC_VerificationResultType verificationStatus)
{
  SecOCRteVerificationStatusType rteVerificationStatusNotification = SecOC_GetRteVerificationStatusNotificationFuncTable(SecOC_GetRteVerificationStatusNotificationFuncTableIdxOfRxPduInfo(rxPduId));
  SecOC_VerificationStatusType verificationStatusStruct;
  verificationStatusStruct.verificationStatus = verificationStatus;
  verificationStatusStruct.freshnessValueID = SecOC_GetExternalFreshnessIdOfRxPduInfo(rxPduId);
  verificationStatusStruct.secOCDataId = SecOC_GetDataIdOfRxPduInfo(rxPduId);

  if(rteVerificationStatusNotification != NULL_PTR) /* COV_SECOC_SILENT_CHECK */
  {
    (void) rteVerificationStatusNotification(&verificationStatusStruct);    /* SBSW_SECOC_PTR2FUNC */
  }
}
#endif
...
# if(SECOC_MACVERIFICATIONFAILEDEXTERNALINDEX == STD_ON)
  {
    uint8 dataIDArray[SECOC_DATAID_SIZE];
    dataIDArray[0u] = SecOC_GetDataIdFirstByteOfRxPduInfo(rxPduId);  /* SBSW_SECOC_DATAIDARRAY */
    dataIDArray[1u] = SecOC_GetDataIdSecondByteOfRxPduInfo(rxPduId); /* SBSW_SECOC_DATAIDARRAY */
    IdsM_SetSecurityEventWithContextData(SecOC_GetMACVerificationFailedExternalIndex(0u), dataIDArray, SECOC_DATAID_SIZE); /* SBSW_SECOC_DATAIDARRAY */
  }
# endif
  ...
  if(SecOC_GetVerifyAttemptsCounter(rxPduId) < SecOC_GetVerifyAttemptsOfRxPduInfo(rxPduId))     /* COV_SECOC_IGNORE_VERIFICATION_RESULT_CONFIG */
  {
    ...
    SecOC_SetRxBuildAttemptsCounter(rxPduId, 0u);        /* SBSW_SECOC_CSL02 */
  }
  else
# endif
  {
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_RxIf_RemoveCanFdContainerPadding(SecOC_RxPduInfoIterType rxPduId)
{
  ...
  if(paddingSize > 0u)
  {
    if(actualLength > 0u)
    {
      ...
      for(; endIdx > SecOC_GetStartIdxOfRxSecuredPduDataContainer(rxPduId); endIdx--) /* FETA_SECOC_01 */
      {
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_RxIf_HandlePdus(SecOC_SizeOfMainFunctionRxStructType mainFuncIdx)
{
  SecOC_RxPduInfoIterType rxPduIdInd = SecOC_GetRxPduInfoIndStartIdxOfMainFunctionRxStruct(mainFuncIdx);
  /* Verify the received Secured I-Pdus and hand the Authentic I-Pdus to the UpperLayer. */
  for(; rxPduIdInd < SecOC_GetRxPduInfoIndEndIdxOfMainFunctionRxStruct(mainFuncIdx); rxPduIdInd++) /* FETA_SECOC_01 */
  {
    SecOC_RxPduInfoIterType rxPduId = SecOC_GetRxPduInfoInd(rxPduIdInd);
...
# if (SECOC_PDUSWITHQUEUESIDXOFRXPDUINFO == STD_ON)
        {
          boolean queueElementLoaded = TRUE;
          SecOC_MaxNumberOfProcessIterationsOfRxPduInfoType numberOfIterations = 0u;

          for (;(queueElementLoaded == TRUE) && (numberOfIterations < SecOC_GetMaxNumberOfProcessIterationsOfRxPduInfo(rxPduId)); numberOfIterations++) /* COV_SECOC_SECOND_LOOP_CONDITION */ /* FETA_SECOC_03 */
          {
            SecOC_Verify_MAC(rxPduId);
            queueElementLoaded = SecOC_Queue_LoadNextQueueElement(rxPduId);
          }
        }
# else
        SecOC_Verify_MAC(rxPduId);
...
SECOC_LOCAL_INLINE FUNC(Std_ReturnType, SECOC_CODE) SecOC_FreshnessValue_GetVerifyValueFromCallout(SecOC_RxPduInfoIterType rxPduId, P2VAR(uint8, AUTOMATIC, SECOC_APPL_DATA) freshnessValue, uint8 freshnessValueBufferLength)
{
  ...
  if(freshnessValueLength <= ((uint32) (freshnessValueBufferLength) << 3u))     /* COV_SECOC_SAFEBSW_CHECK */
  {
# if (SECOC_GETRXFRESHNESSAUTHDATAFUNCTABLE == STD_ON)
    if(SecOC_IsGetRxFreshnessAuthDataFuncTableUsedOfRxPduInfo(rxPduId)) /* COV_SECOC_FRESHNESS_INTERFACE */
    {
      uint8 truncatedFreshnessValue[SECOC_TRUNC_FRESHNESS_SIZE_RXPDUS] = { 0u };        /* PRQA S 3678 */ /* MD_MSR_Rule8.13 */
      SecOCGetRxFreshnessAuthDataType getRxFreshnessAuthData = SecOC_GetGetRxFreshnessAuthDataFuncTable(SecOC_GetGetRxFreshnessAuthDataFuncTableIdxOfRxPduInfo(rxPduId));
...
#   endif
#  endif
      if(getRxFreshnessAuthData != NULL_PTR) /* COV_SECOC_SILENT_CHECK */
      {
        /* *INDENT-OFF* */
        ret = getRxFreshnessAuthData(
...
# endif
# if (SECOC_GETRXFRESHNESSFUNCTABLE == STD_ON)
    if(SecOC_IsGetRxFreshnessFuncTableUsedOfRxPduInfo(rxPduId)) /* COV_SECOC_FRESHNESS_INTERFACE */
    {
      uint8 truncatedFreshnessValue[SECOC_TRUNC_FRESHNESS_SIZE_RXPDUS] = { 0u };        /* PRQA S 3678 */ /* MD_MSR_Rule8.13 */
      SecOCGetRxFreshnessType getRxFreshnessFunc = SecOC_GetGetRxFreshnessFuncTable(SecOC_GetGetRxFreshnessFuncTableIdxOfRxPduInfo(rxPduId));
...
#   endif
#  endif
      if(getRxFreshnessFunc != NULL_PTR) /* COV_SECOC_SILENT_CHECK */
      {
        /* *INDENT-OFF* */
        ret = getRxFreshnessFunc(
          SecOC_GetExternalFreshnessIdOfRxPduInfo(rxPduId),
          truncatedFreshnessValue,
          SecOC_GettruncatedFreshnessValueLengthOfRxPduInfo(rxPduId),
          (uint16) SecOC_GetVerifyAttemptsCounter(rxPduId),
...
SECOC_LOCAL_INLINE FUNC(PduLengthType, SECOC_CODE) SecOC_GetLengthFromHeader(P2CONST(uint8, AUTOMATIC, SECOC_APPL_DATA) headerData, uint8 headerLength)
{
  ...
  for(; i < headerLength; i++) /* FETA_SECOC_04 */
  {
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_SerializeLengthToHeader(P2VAR(uint8, AUTOMATIC, SECOC_APPL_DATA) dest, uint8 headerLength, PduLengthType authenticPduLength)
{
  ...
  for(; i > 0u; i--) /* FETA_SECOC_01 */
  {
    uint8 destIdx = (uint8) (i - 1);    /* PRQA S 1860, 4393 */ /* MD_SecOC_1860_4393 */
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_RxIf_InitPdus(void)
{
  ...
  for(; rxPduId < SecOC_GetSizeOfRxPduInfo(); rxPduId++) /* FETA_SECOC_01 */
  {
...
# if (SECOC_EXISTS_AUTHENTIC_SPLIT_MESSAGETYPEOFRXSECUREDPDUINFO == STD_ON)
  {
    ...
    for(; splitCounterIndex < SecOC_GetSizeOfRxSplitMessageCounter(); splitCounterIndex++) /* FETA_SECOC_01 */
    {
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_TxIf_InitPdus(void)
{
  ...
  for(; txPduId < SecOC_GetSizeOfTxPduInfo(); txPduId++) /* FETA_SECOC_01 */
  {
...
SECOC_LOCAL_INLINE FUNC(boolean, SECOC_CODE) SecOC_Split_CheckMessageLinker(P2CONST(uint8, AUTOMATIC, SECOC_APPL_DATA) msgLinkNew, P2VAR(uint8, AUTOMATIC, SECOC_APPL_DATA) msgLinkOld, SecOC_CurrentMessageLinkerLengthOfRxSecuredPduInfoType msgLinkLenInBytes, boolean currentIsNewStatus)
{
  ...
  for(; i < msgLinkLenInBytes; i++) /* FETA_SECOC_01 */
  {
...
SECOC_LOCAL_INLINE FUNC(void, SECOC_CODE) SecOC_GetArrayByteAligned(P2CONST(uint8, AUTOMATIC, SECOC_APPL_DATA) srcBuffer, P2VAR(uint8, AUTOMATIC, SECOC_APPL_DATA) destBuffer, uint16 copyBitLen, uint8 srcStartBitInByte, uint8 destBitOffset, boolean isSrcRightAligned)
{
  ...
  for(; i < msgLinkByte; i++) /* FETA_SECOC_05 */
  {
...
# endif
  ...
  else
  {
...
#if (SECOC_RXPDUSTATE == STD_ON)
    {
      ...
      for(; pduStateIdx < SecOC_GetSizeOfRxPduState(); pduStateIdx++) /* FETA_SECOC_01 */
      {
...
#if (SECOC_TXPDUSTATE == STD_ON)
    {
      ...
      for(; pduStateIdx < SecOC_GetSizeOfTxPduState(); pduStateIdx++) /* FETA_SECOC_01 */
      {
...
#if (SECOC_SENDDEFAULTAUTHENTICATION == STD_ON)
    {
      ...
      for(; sendDefaultAuthenticationIdx < SecOC_GetSizeOfSendDefaultAuthentication(); sendDefaultAuthenticationIdx++) /* FETA_SECOC_01 */
      {
...
#if (SECOC_VERIFYRESULT == STD_ON)
    {
      ...
      for(; verifyResultIdx < SecOC_GetSizeOfVerifyResult(); verifyResultIdx++) /* FETA_SECOC_01 */
      {
 ...
 *
**********************************************************************************************************************/
FUNC(void, SECOC_CODE) SecOC_MainFunctionRxInternal(SecOC_SizeOfMainFunctionRxStructType mainFuncIdx)
{
  /* #10 Check if component is initialized */
  if(SecOC_IsInitialized())
  {
    /* ----- Implementation ----------------------------------------------- */
    SecOC_RxIf_HandlePdus(mainFuncIdx);
  }
}
#endif
 ...
 *
**********************************************************************************************************************/
FUNC(void, SECOC_CODE) SecOC_MainFunctionTxInternal(SecOC_SizeOfMainFunctionTxStructType mainFuncIdx)
{
  /* #10 Check if component is initialized */
  if(SecOC_IsInitialized())
  {
    /* ----- Implementation ----------------------------------------------- */
    SecOC_TxIf_HandlePdus(mainFuncIdx);
  }
}
#endif
...
/* module specific MISRA deviations:
  ...
  MD_SecOC_1860_4393:  Misra Rule 10.4 10.8
...
/* FETA_JUSTIFICATION_BEGIN
    ...
    \ID FETA_SECOC_01
      ...
      \COUNTERMEASURE \N The loop has a static upper limit determined by a ROM value. It will always terminate in finite time.
                         The memory section index is either read from a CSL indirection or checked by the API pattern.
    \ID FETA_SECOC_02
      \DESCRIPTION State processing loop.
      \COUNTERMEASURE \D DSGN-SecOC20836

    \ID FETA_SECOC_03
      \DESCRIPTION Loop has two exit conditions. One of the two exit conditions is a loop interation counter.
      \COUNTERMEASURE \N The loop has a static upper limit determined by a ROM value. It will always terminate in finite time.
                         The memory section index is either read from a CSL indirection or checked by the API pattern.

    \ID FETA_SECOC_04
      \DESCRIPTION Loop over RAM number of elements.
      \COUNTERMEASURE \N The RAM number of elements is checked against a const upper limit. It will always terminate in finite time.

    \ID FETA_SECOC_05
      \DESCRIPTION Loop over calculated number of elements.
      \COUNTERMEASURE \N The calculated value is the result of two const values. It will always terminate in finite time.
...
/* SBSW_JUSTIFICATION_BEGIN
  ...
  \ID SBSW_SECOC_PTR2FUNC
    \DESCRIPTION    A function pointer is called.
    \COUNTERMEASURE \R Function pointer is checked against NULL before calling it.

  \ID SBSW_SECOC_DATAIDARRAY
    \DESCRIPTION    A two byte array is allocated on the stack and passed to a function.
    \COUNTERMEASURE \N The array has a fixed length of two bytes, only the first two bytes are accessed and the array is passed to the function with the length parameter set to two.
...
/* COV_JUSTIFICATION_BEGIN
  ...
  \ID COV_SECOC_FRESHNESSVALUE_INTERFACE
    \ACCEPT TX
    \REASON The state of the switch depends on the used freshness value interface.
...
\ID COV_SECOC_SPLIT_ONLY_CONFIG
  ...
  \ID COV_SECOC_SECOND_LOOP_CONDITION
    \ACCEPT   TF tf tx
    \REASON The extra loop exit condition can never be reached in a component test.
  ...
  \ID COV_SECOC_FRESHNESS_INTERFACE
    \ACCEPT TX
    \REASON Dependents on the used freshness value interface.
