 ...
 *         \unit  String
 *        \brief  IpBase string handling source file
 *      \details  IpBase string handling routines definition. 
 *********************************************************************************************************************/
/**********************************************************************************************************************
 *  REVISION HISTORY
 *  -------------------------------------------------------------------------------------------------------------------
                                      ...
                                      IPBASE_P2CONSTAPPLDATA(uint8) SrcPtr)
{
  ...
  else
  {
    ...
    while ( SrcPtr[byteIdx] != (uint8)'\0' ) /* FETA_IPBASE_STR_ZEROTERM */
    {
                                            ...
                                            IPBASE_P2CONSTAPPLDATA(uint8) SrcPtr, uint32 MaxLen)
{
  ...
  else
  {
    /* ----- Implementation ----------------------------------------------- */
    /* #20 Copy all string characters */
    while ( SrcPtr[byteIdx] != (uint8)'\0' ) /* FETA_IPBASE_STR_LEN_PARAM */
    {
      TgtPtr[byteIdx] = SrcPtr[byteIdx]; /* SBSW_IPBASE_API_ZERO_TERMINATED_STRING */
      byteIdx++;
      if ( byteIdx == MaxLen )
                                     ...
                                     IPBASE_P2CONSTAPPLDATA(uint8) Str2Ptr)
{
  ...
  else
  {
    ...
    while ( (Str1Ptr[byteIdx] != (uint8)'\0') && (Str2Ptr[byteIdx] != (uint8)'\0') && /* FETA_IPBASE_STR_ZEROTERM */
                                        ...
                                        uint16 StrLen)
{
  ...
  else
  {
    ...
    for(byteIdx = 0u; byteIdx < StrLen; byteIdx++) /* FETA_IPBASE_STR_LEN_PARAM */
    {
                                      ...
                                      uint32 MaxLen)
{
  ...
  else
  {
    ...
    for ( byteIdx = 0; byteIdx <= MaxLen; byteIdx++ ) /* FETA_IPBASE_STR_LEN_PARAM */
    {
                                                      ...
                                                      IPBASE_P2VARAPPLDATA(uint8) StrLenPtr)
{
  ...
  /* ----- Development Error Checks ------------------------------------- */
  /* #10 Check for NULL pointer */
  if ( (StrPtr == NULL_PTR) || (*StrPtr == NULL_PTR) || (StrLenPtr == NULL_PTR) )
  {
    errorId = IPBASE_E_INV_POINTER;
  }
  else
  {
    ...
    intStrPtr = *StrPtr;
    while(pos < *StrLenPtr) /* FETA_IPBASE_STR_LEN_PARAM */
    {
      digit = (uint8) (remIntVal % 10u);
      intStrPtr[*StrLenPtr - (pos + 1u)] = (uint8)(digit + IPBASE_STR_INT_ASCII_OFF); /* 0..9 */ /* SBSW_IPBASE_API_STRING_TARGET_FOR_DATA */ /* VCA_IPBASE_API_STRING_TARGET_FOR_DATA */
                                                          ...
                                                          uint8 StrLen)
{
  ...
  else
  {
    ...
    {
      while ( remIntVal > 0u ) /* FETA_IPBASE_STR_DIGIT_TYPE32 */
      {
        digits[digPos] = (uint8) (remIntVal % 10u); /* SBSW_IPBASE_INT_DIGIT_MAX */ /* VCA_IPBASE_INT_DIGIT_MAX */
        remIntVal = remIntVal / 10u;
        digPos++;
      }
    }
    ...
    { /* #30 Convert all digits */
      ...
      for ( strPos = 0u; strPos < maxPos; strPos++ ) /* FETA_IPBASE_STR_DIGIT_TYPE32 */
      {
                                                               ...
                                                               IPBASE_P2VARAPPLDATA(uint8) StrPtr)
{
  ...
  else
  {
    ...
    /* #20 Scan array */
    while(arrayIdx < ArrayLen) /* FETA_IPBASE_STR_LEN_PARAM */
                                                           ...
                                                           IPBASE_P2VARAPPLDATA(uint8) StrLenPtr)
{
  ...
  else
  {
    ...
    { /* #30 Convert all digits */
      ...
      (*StrPtr)[maxPos] = (uint8)'\0'; /* SBSW_IPBASE_API_STRING_TARGET_FOR_DATA */ /* VCA_IPBASE_API_STRING_TARGET_FOR_DATA */
      for ( strPos = 0; strPos < maxPos; strPos++ ) /* FETA_IPBASE_STR_DIGIT_TYPE32 */
      {
        digPos = maxPos - strPos - 1u;
        (*StrPtr)[strPos] = (uint8)(digits[digPos] + IPBASE_STR_INT_ASCII_OFF); /* SBSW_IPBASE_API_STRING_TARGET_FOR_DATA */ /* VCA_IPBASE_API_STRING_TARGET_NULL */
      }
                                                         ...
                                                         IPBASE_P2VARAPPLDATA(uint8) StrLenPtr)
{
  ...
  else
  {
    ...
    while( (strPos < *StrLenPtr) && (remIntVal != 0u) ) /* FETA_IPBASE_STR_DIGIT_TYPE32 */
    { /* #30 Convert digit */
      ...
      if(digit < 10u)
      {
        intStrPtr[intStrPos] = (uint8)(digit + IPBASE_STR_INT_ASCII_OFF); /* 0..9 */ /* SBSW_IPBASE_API_ARGUMENT_LEN */ /* VCA_IPBASE_API_STRING_TARGET_FOR_DATA */
      }
      else
      {
        intStrPtr[intStrPos] = (uint8)(digit + IPBASE_STR_UPP_CASE_ASCII_OFF); /* A..F (Hint: 'A'-10) */ /* SBSW_IPBASE_API_ARGUMENT_LEN */ /* VCA_IPBASE_API_STRING_TARGET_FOR_DATA */
      }
                                                      ...
                                                      IPBASE_P2VARAPPLDATA(uint32) IntValPtr)
{
  ...
  else
  {
    ...
    /* #20 Scan all digits */
    for ( posA = 0; posA < StrLen; posA++ ) /* FETA_IPBASE_STR_LEN_PARAM */
                                                         ...
                                                         IPBASE_P2VARAPPLDATA(uint32) IntValPtr)
{
  ...
  else
  {
    ...
    /* #20 Scan all characters */
    for ( posA = 0; posA < *StrLenPtr; posA++ ) /* FETA_IPBASE_STR_LEN_PARAM */
    {
      digit = (*StrPtr)[posA];
                                                         ...
                                                         IPBASE_P2VARAPPLDATA(uint32) IntValPtr)
{
  ...
  else
  {
    ...
    /* #20 Scan all characters */
    for(pos = 0u; pos < StrLen; pos++) /* FETA_IPBASE_STR_LEN_PARAM */
    {
                                                  ...
                                                  uint8 StrMaxLen)
{
  ...
  else
  {
    ...
    /* #20 Scan all characters */
    for ( posA = 0; posA < StrMaxLen; posA++ ) /* FETA_IPBASE_STR_LEN_PARAM */
    {
                                                               ...
                                                               IPBASE_CONSTP2CONSTAPPLDATA(uint8) StrPtr)
{
  ...
  else
  {
    ...
    for ( arrayIdx = 0; arrayIdx < ArrayLen; arrayIdx++ ) /* FETA_IPBASE_STR_LEN_PARAM */
    {
                                                             ...
                                                             uint16 StrLen, uint16 SubStrLen)
{
  ...
  while ( readPos < StrLen ) /* FETA_IPBASE_STR_LEN_PARAM */
  {
                                                                            ...
                                                                            IPBASE_P2VARAPPLDATA(uint32) IntValPtr)
{
  ...
  for(posA = 0; posA < *StrIntLenPtr; posA++) /* FETA_IPBASE_STR_LEN_PARAM */
  {
