...
/*!        \file  IpBase_PBuf.c
 *         \unit  PBuf
 *        \brief  IpBase buffer handling source file
 *      \details  IpBase buffer handling routines definition. 
 *********************************************************************************************************************/
/**********************************************************************************************************************
 *  REVISION HISTORY
 *  -------------------------------------------------------------------------------------------------------------------
                                         ...
                                         IPBASE_P2VARAPPLDATA(uint32) RestLenBytePtr)
{
  ...
  else
  {
    ...
    while ( (totByteIdx < srcBufTotLen) && (retVal != IPBASE_CMP_EQUAL) ) /* FETA_IPBASE_PBUF_BUF */
    {
                                                         ...
                                                         uint32 StartPos)
{
  ...
  else
  {
    ...
    while ( StartPos >= (pBufElePtr->len + writePosAbs)) /* FETA_IPBASE_PBUF_BUF */
    ...
    while ( copyLen > copiedLen ) /* FETA_IPBASE_PBUF_BUF */
    { /* #60 Copy data into PBuf element */
      ...
      VStdMemCpy((IPBASE_P2VARAPPLDATA(void))&pBufElePtr->payload[writePosLoc],       /* SBSW_IPBASE_PASSING_API_POINTER */ /* VCA_IPBASE_API_PBUF_CONSISTENCY */
                                                       ...
                                                       uint16 StrLen, uint32 StartPos)
{
  ...
  else
  {
    ...
    /* #30 Go to start position in Pbuf */
    while ( StartPos >= (pBufElePtr->len + readPosAbs) ) /* FETA_IPBASE_PBUF_BUF */
    { /* #40 StartPos not within current PBuf segment goto next segment */
    ...
    while ( strPos < StrLen ) /* FETA_IPBASE_PBUF_STR */
    {
                                                         ...
                                                         uint16 StrLen, uint32 StartPos)
{
  ...
  else
  {
    ...
    while ( (pBufElePtr->len + readPosAbs) <= StartPos ) /* FETA_IPBASE_PBUF_BUF */
    { /* #40 StartPos not within current PBuf segment goto next segment */
      readPosAbs += pBufElePtr->len;
      pBufElePtr = &pBufElePtr[1u];
    }
    ...
    while ( locStrPos < StrLen ) /* FETA_IPBASE_PBUF_STR */
                                                      ...
                                                      IPBASE_P2VARAPPLDATA(uint8) SingleBytePtr)
{
  ...
  else
  {
    ...
    /* #30 Go to start position */
    while ( BytePos >= (pBufElePtr->len + readPosAbs) ) /* FETA_IPBASE_PBUF_BUF */
    { /* #40 StartPos not within current PBuf segment goto next segment */
      readPosAbs += pBufElePtr->len;
      pBufElePtr = &pBufElePtr[1u];
    }
                                                                        ...
                                                                        uint16 StrLen)
{
  ...
  while ( (pBufElePtr->len + readPosAbs) <= *StrPosPtr ) /* FETA_IPBASE_PBUF_BUF */
  ...
  while ( (readPosAbs <= (PbufPtr->totLen - StrLen)) && (found == (boolean)FALSE) ) /* FETA_IPBASE_PBUF_BUF */
