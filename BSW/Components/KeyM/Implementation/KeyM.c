...
/**********************************************************************************************************************
 * KeyM_ServiceCertificateByIdInternal()
 *********************************************************************************************************************/
/*! \brief         Handle certificate operation requests.
 *  \details       This function handles processing for KeyM_ServiceCertificateById and
 ...
 *  \param[in]     apiId                                API ID to be reported to DET if an error occurs.
 ...
 *  \synchronous   FALSE
 *********************************************************************************************************************/
KEYM_LOCAL FUNC(Std_ReturnType, KEYM_CODE) KeyM_ServiceCertificateByIdInternal(
...
# endif /* KEYM_REMOTE_REDIRECTION_ENABLED */
    /* # Try to obtain certificate lock, if locked return E_NOT_OK */
...
# endif
  {
    ...
    if (retVal == E_OK)
    {
      /* # Check if provided certificate ID is valid and return an error code otherwise */
      if ((CertId >= KeyM_GetSizeOfCertificate())
        || (KeyM_GetCertFormatTypeOfCertificate((KeyM_CertificateIterType)CertId) == KEYM_AC)) /* AC verification is restricted to KeyM_VerifyCertificate */
      {
        retVal = KEYM_E_PARAMETER_MISMATCH;
      }
      /* # If certificate is configured for remote handling, redirect remote service request */
...
#  endif /* KEYM_REMOTE_REDIRECTION_ENABLED */
    ...
    /* # Call local implementation of KeyM_ServiceCertificateById */
    else
    {
...
#  if (KEYM_DEV_ERROR_REPORT == STD_ON)
  ...
  {
    (void)Det_ReportError(KEYM_MODULE_ID, KEYM_INSTANCE_ID_DET, apiId, errorId);
  }
#  else
  KEYM_DUMMY_STATEMENT(apiId); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
  ...
  uint32 ResponseDataLength)
{
  ...
  /* ----- Implementation ----------------------------------------------- */
  P2CONST(uint8, AUTOMATIC, KEYM_APPL_DATA) CertNameConstPtr = (P2CONST(uint8, AUTOMATIC, KEYM_APPL_DATA))CertNamePtr;  /* PRQA S 0316 */ /* MD_KEYM_0316 */
  P2CONST(uint8, AUTOMATIC, KEYM_APPL_DATA) RequestDataConstPtr = (P2CONST(uint8, AUTOMATIC, KEYM_APPL_DATA))RequestData; /* PRQA S 0316 */ /* MD_KEYM_0316 */
  P2VAR(uint8, AUTOMATIC, KEYM_APPL_VAR) ResponseDataConstPtr = (P2VAR(uint8, AUTOMATIC, KEYM_APPL_VAR))ResponseData; /* PRQA S 0316 */ /* MD_KEYM_0316 */

  /* # Call KeyM_ServiceCertificate */
  retVal = KeyM_ServiceCertificate(
    ...
    RequestDataConstPtr,
    RequestDataLength,
    ResponseDataConstPtr,
    ResponseDataLength);

  /* # Map non-AUTOSAR error codes to E_NOT_OK */
  if ((retVal == KEYM_E_KEY_CERT_EMPTY) || (retVal == KEYM_E_CERT_INVALID_CHAIN_OF_TRUST))
  {
    retVal = E_NOT_OK;
  }
...
#  endif /* KEYM_REMOTE_REDIRECTION_ENABLED */
    ...
    {
      retVal = KeyM_Cert_SetCertificateInGroup(GroupId, RequestData, RequestDataLength, CertId, FALSE);
...
# endif
  {
    ...
    || (KeyM_GetCertFormatTypeOfCertificate((KeyM_CertificateIterType)CertId) == KEYM_AC) /* AC verification is restricted to KeyM_VerifyCertificate */
    || (KeyM_GetCertFormatTypeOfCertificate((KeyM_CertificateIterType)CertUpperId) == KEYM_AC)) /* AC certificates can not issue other certificates */
    {
      retVal = KEYM_E_PARAMETER_MISMATCH;
    }
    /* # If certificate is configured for remote handling, redirect remote service request */
 ...
 *
 */
/* PRQA S 3673 6 */ /* MD_KEYM_8.13_AutosarAPI */
 ...
 *
 */
/* PRQA S 3673 1 */ /* MD_KEYM_8.13_DeactivateableFeature */
...
#  endif
  {
    /* ----- Implementation ----------------------------------------------- */
    /* # Check if provided certificate ID is valid and the service is called only for the certificate format X509,
...
FUNC(Std_ReturnType, KEYM_CODE) KeyM_ServiceCertificateById(
  ...
  uint32 ResponseDataLength)
{
  return KeyM_ServiceCertificateByIdInternal(Service, CertId, RequestData, RequestDataLength, ResponseData, ResponseDataLength, KEYM_SID_SERVICE_CERTIFICATE_BY_ID);
}
# endif /* ifndef KEYM_NOUNIT_CERTIFICATESERVICES */
#endif /* (KEYM_SERVICE_CERTIFICATE_FUNCTION_ENABLED == STD_ON) */
...
FUNC(Std_ReturnType, KEYM_CODE) KeyM_ServiceCertificateByCertId(
  ...
  uint32 ResponseDataLength)
{
  return KeyM_ServiceCertificateByIdInternal(Service, CertId, RequestData, RequestDataLength, ResponseData, ResponseDataLength, KEYM_SID_SERVICE_CERTIFICATE_BY_CERT_ID);
}
# endif /* ifndef KEYM_NOUNIT_CERTIFICATESERVICES */
#endif /* (KEYM_SERVICE_CERTIFICATE_FUNCTION_ENABLED == STD_ON) */
  ...
  uint32 ResponseDataLength)
{
  ...
  /* # Call KeyM_ServiceCertificateByCertId */
  retVal = KeyM_ServiceCertificateByCertId(
    CertId,
    Service,
    RequestDataConstPtr,
    RequestDataLength,
...
#  else
      ...
      {
        /* # Call local implementation of KeyM_SetCRE */
...
# endif /* KEYM_REMOTE_REDIRECTION_ENABLED */
    /* # Call local implementation of KeyM_CertStructureGet */
    else
    {
...
# endif /* KEYM_REMOTE_REDIRECTION_ENABLED */
    ...
    /* # Call local implementation of KeyM_GetIssuerCertId */
    else
    {
...
# if (KEYM_REMOTE_REDIRECTION_ENABLED == STD_ON)
    ...
    {
      retVal = KeyM_Remote_RedirectGetPubKeyHash(CertId, HashData, HashDataLength);
    }
# endif /* KEYM_REMOTE_REDIRECTION_ENABLED */
    else
# if (KEYM_CERTPUBKEYHASHSTORAGEUSEDOFCERTIFICATE == STD_ON)
    {
      /* # Else, call local implementation of KeyM_GetPubKeyHash */
      retVal = KeyM_Cert_GetPubKeyHash(CertId, HashData, HashDataLength);
    }
# else
    {
      KEYM_DUMMY_STATEMENT(HashData); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
...
# if (KEYM_DEV_ERROR_REPORT == STD_ON)
  ...
  if (errorId != KEYM_E_NO_ERROR)
  {
    (void)Det_ReportError(KEYM_MODULE_ID, KEYM_INSTANCE_ID_DET, KEYM_SID_GET_PUB_KEY_HASH, errorId);
  }
# else
  KEYM_DUMMY_STATEMENT(errorId); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
# endif
...
# if (KEYM_REMOTE_REDIRECTION_ENABLED == STD_ON)
    ...
    {
      retVal = KeyM_Remote_RedirectCertDelete(CertId);
    }
# endif /* KEYM_REMOTE_REDIRECTION_ENABLED */
    else
    {
      /* # Get certificate lock */
      if (!KeyM_Cert_GetCertLock(CertId))
      {
        retVal = KEYM_E_BUSY;
      }
      /* # Else, call local implementation of KeyM_CertDelete */
      else
      {
        retVal = KeyM_Cert_CertDelete(CertId);
...
# if (KEYM_DEV_ERROR_REPORT == STD_ON)
  ...
  if (errorId != KEYM_E_NO_ERROR)
  {
    (void)Det_ReportError(KEYM_MODULE_ID, KEYM_INSTANCE_ID_DET, KEYM_SID_CERT_DELETE, errorId);
  }
# else
  KEYM_DUMMY_STATEMENT(errorId); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
# endif
...
# if (KEYM_DEV_ERROR_DETECT == STD_ON)
  ...
  /* # Check if provided pointer is a null pointer */
  else if ((CertInfoData == NULL_PTR)
    || (CertInfoDataLength == NULL_PTR))
  {
    /* [SWS_KeyM_00146] */
    errorId = KEYM_E_PARAM_POINTER;
  }
...
# if (KEYM_REMOTE_REDIRECTION_ENABLED == STD_ON)
    ...
    {
      retVal = KeyM_Remote_RedirectCertInfoGet(CertId, CertInfoType, CertInfoData, CertInfoDataLength);
    }
# endif /* KEYM_REMOTE_REDIRECTION_ENABLED */
    else
    {
      retVal = KeyM_Cert_CertInfoGet(CertId, CertInfoType, CertInfoData, CertInfoDataLength);
    }
  }
...
# if (KEYM_DEV_ERROR_REPORT == STD_ON)
  ...
  if (errorId != KEYM_E_NO_ERROR)
  {
    (void)Det_ReportError(KEYM_MODULE_ID, KEYM_INSTANCE_ID_DET, KEYM_SID_CERT_INFO_GET, errorId);
  }
# else
  KEYM_DUMMY_STATEMENT(errorId); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
# endif
...
#  if (KEYM_DEV_ERROR_DETECT == STD_ON)
  ...
  /* # Check if provided pointer is a null pointer */
  else if (BasicConstraints == NULL_PTR)
  {
    /* [SWS_KeyM_00146] */
    errorId = KEYM_E_PARAM_POINTER;
  }
...
#  endif
  {
    ...
    {
      /* # Else, call local implementation of KeyM_GetCertBasicConstraints */
      retVal = KeyM_Cert_GetCertBasicConstraints(CertId, BasicConstraints);
    }
  }
...
#  if (KEYM_DEV_ERROR_REPORT == STD_ON)
  ...
  if (errorId != KEYM_E_NO_ERROR)
  {
    (void)Det_ReportError(KEYM_MODULE_ID, KEYM_INSTANCE_ID_DET, KEYM_SID_GET_CERT_BASICCONSTRAINTS, errorId);
  }
#  else
  KEYM_DUMMY_STATEMENT(errorId); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
#  endif
...
#  endif
  {
    ...
    {
      /* # Else, call local implementation of KeyM_GetCertKeyUsage */
      retVal = KeyM_Cert_GetCertKeyUsage(CertId, KeyUsage);
    }
  }
...
#  if (KEYM_DEV_ERROR_REPORT == STD_ON)
  ...
  if (errorId != KEYM_E_NO_ERROR)
  {
    (void)Det_ReportError(KEYM_MODULE_ID, KEYM_INSTANCE_ID_DET, KEYM_SID_GET_CERT_KEYUSAGE, errorId);
  }
#  else
  KEYM_DUMMY_STATEMENT(errorId); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
#  endif
...
MD_KEYM_0316:
Reason:       Cast from void pointer to uint8 pointer. Cast is needed since an API used or provided by KeyM expects
              void pointers to be used. KeyM internally operates on uint8 pointers since the bytes behind the buffer
              need to be evaluated and/or manipulated.
Risk:         No risk (cast from void to uint8 always works as void and uint8 use the same alignment).
...
MD_KEYM_16.6:
Description:  Rule 16.6: This 'switch' statement is redundant.
Reason:       Dead code is caused due to specific configuration variant.
Risk:         None.
Prevention:   None
...
Reason:       Invariance is caused due to specific configuration variant.
Risk:         None.
Prevention:   None.
...
MD_KEYM_8.13_ConfigurableInterface:
Description:  Rule 8.13: A pointer should point to a const-qualified type whenever possible
Reason:       Function prototype of configurable interface is specified by AUTOSAR
Risk:         No risk.
Prevention:   Program flow has been verified by component tests and review.
...
MD_KEYM_13.5_Sideeffect:
Description:  Rule 13.5: The right hand operand of a logical && or || operator shall not contain persistent side effects.
Reason:       Read-only operations have no side effects.
Risk:         Erroneous control flow.
Prevention:   Verified by component tests and review.
...
/* COV_JUSTIFICATION_BEGIN
  ...
  \ID COV_KEYM_NON_MICROSAR_CRYPTO_DRIVER
  \ACCEPT XF
  \REASON Ensures the RSA Crypto key element identifier defines exist if the KeyM is
          used with a non-MiCROSAR crypto driver.
  ...
  \ID COV_DEFENSIVE_PROGRAMMING_IDENTIFYINVALIDCHAIN
  \ACCEPT XX
  \REASON This function is called as failsafe to ensure the integrity of the verify stack if construction failed.
          This is something that should never happen if this API is always called in the correct context.
...
/* VCA_JUSTIFICATION_BEGIN
  ...
  \ID  VCA_KEYM_CERT_ATTRIBUTE_CERTIFICATE_ID
    \DESCRIPTION      According to VCA, the following assertion may fail: KeyM_Cert_AttrCertId < KeyM_GetSizeOfCertificate()
    \COUNTERMEASURE   \R The variable in question is global. It is only ever written in
                      the function KeyM_Cert_VerifyAttributeCertificateHolderChain with a certificate identifier, different
                      from the initialization value. Right before the write operation, there is an
                      assertion that ensures that the written value is < KeyM_GetSizeOfCertificate() in KeyM_Cert_VerifyCertificate.
                      The control flow guarantees that KeyM_Cert_AttrCertId is read only after it is been set with a valid value.
  ...
  \ID VCA_KEYM_REMOTE_VERIFYCERTIFICATECHAIN_TERTIARYBUFFERACCESS
    ...
    \COUNTERMEASURE   \R A check that ensures that (offset + KEYM_REMOTE_CERTIFICATE_CHAIN_POS_CERTDATA)
                         ...
                         The check that precedes this call ensures that the buffer location identified by the offset
  ...
  \ID VCA_KEYM_FUNCTION_REQUIREMENT_CHECKED_BY_ASSERTIONS
    \DESCRIPTION      Function is possibly called outside its specification according to VCA
    \COUNTERMEASURE   \N There are assertions in place immediately before the call that check that the function's requirements hold.
  ...
  \ID VCA_KEYM_REMOTE_KEYELEMENTGET_CERTINFOGET
    \DESCRIPTION      Function Csm_KeyElementGet is possibly called outside its specification according to VCA
    \COUNTERMEASURE   \N CertInfoData and CertInfoDataLength are passed through to this function directly from the
                         API KeyM_CertInfoGet without manipulation.
                         KeyM_CertInfoGet's API contract requires that both pointers are valid and
                         CertInfoDataLength's content matches CertInfoData's length. Also, there is a NULL_PTR
                         check for both pointers in the calling function.
  ...
  \ID VCA_KEYM_NVBLOCK_DETERMINESLOTOWNER
    \DESCRIPTION      KeyM_Cert_DetermineSlotOwnerByCertIndexAndAlgo is called for a 2-byte read access on
  ...
  \ID VCA_KEYM_CERT_INITCHECK05
    \DESCRIPTION      Assertion KeyM_IsCertificateGroupUsedOfCertificate(certIdx)
    \COUNTERMEASURE   \R There is a check KeyM_Init() that ensures that the module is only successfully initialized
                         if for all certificates where KeyM_GetCertUpperHierarchicalTypeOfCertificate(certIdx)
                         != KEYM_PRESET_ISSUER) holds, KeyM_IsCertificateGroupUsedOfCertificate(certIdx) also holds.
                         The check is located in the subfunction KeyM_Cert_InitChecks().
                         Since all APIs abort immediately if the module is not initialized and the values are in
                         question are constant, it is ensured that the checked condition still holds when this
                         assertion is checked.
                         There is a runtime check that (KeyM_GetCertUpperHierarchicalTypeOfCertificate(certIdx)
                         != KEYM_PRESET_ISSUER) before this assertion. For the above mentioned reasons, this implies
                         that KeyM_IsCertificateGroupUsedOfCertificate(certIdx) is TRUE.
  ...
  \ID VCA_KEYM_ASN1_PARSEPROCESSING_CHECKPARSEDCERTELEMENTS
    ...
    \COUNTERMEASURE   \N This call occurs in the function KeyM_Asn1_ParseProcessing.
                         ...
                         (elementList + noOfElementList) < KeyM_GetSizeOfCertificateElement().
                         There is only one place in the program where the two values in question are written, and that is KeyM_Asn1_ParseData.
                         KeyM_Asn1_ParseData is not in the call tree of KeyM_Asn1_ParseProcessing.
                         Since the condition was fulfilled when KeyM_Asn1_ParseProcessing was called and no modification of the parameters
  ...
  \ID VCA_KEYM_ASN1_ELEMENTPATH_ELEMENTTABLEIDX
    \DESCRIPTION      Assertion elementTableIdx < KeyM_GetSizeOfCertificateElement()
    \COUNTERMEASURE   \R The surrounding for loop ensures that elementTableIdx at this place has a
                         value between asn1ParseParam->elementList and
                         ((asn1ParseParam->elementList + asn1ParseParam->asn1_element_idx) - 1).
                         In the call tree above in function KeyM_Asn1_MatchCfgElement, it is asserted that
                         (asn1ParseParam->elementList + asn1ParseParam->asn1_element_idx) <= KeyM_GetSizeOfCertificateElement().
                         Therefore, elementTableIdx must have a value < KeyM_GetSizeOfCertificateElement().
  ...
  \ID VCA_KEYM_CERT_GETSUBGROUPID
    \DESCRIPTION      Assertion localGroupId < KeyM_GetSizeOfCertificateGroupExt()
    \COUNTERMEASURE   \N At this point localGroupId, either has its initial value of groupId (which is less than
                         KeyM_GetSizeOfCertificateGroupExt()) or a value that was assigned in the loop in KeyM_Cert_GetSubGroupId 
                         from groupIdx.
                         groupIdx is limited in the loop body by the loop condition to be less than
                         KeyM_GetCertificateGroupEndIdxOfCertificateGroupExt().
                         Qualified use-case CSL03 of ComStackLib guarantees that
                         KeyM_GetCertificateGroupEndIdxOfCertificateGroupExt() <= KeyM_GetSizeOfCertificateGroupExt().

  \ID VCA_KEYM_CERT_CHECKALGORITHMSMATCH
    \DESCRIPTION      Assertion groupIdx < KeyM_GetSizeOfCertificateGroup()
    \COUNTERMEASURE   \N groupIdx is assigned an value from the CSL generated macro KeyM_GetCertificateGroupIdxOfCertificate
                         which takes certIdx as an input. 
                         certIdx is limited in the loop body by the loop condition to be less than
                         KeyM_GetSizeOfCertificate().
                         This is a qualified use-case CSL03 of ComStackLib.

  \ID VCA_KEYM_CERT_DETERMINESLOTOWNERCERTID
    \DESCRIPTION      Assertion determinedCertId < KeyM_GetSizeOfCertificate()
    \COUNTERMEASURE   \N determinedCertId has its value that was assigned in the loop in KeyM_Cert_DetermineSlotOwnerByCertIndexAndAlgo() 
                         from i.
                         i is limited in the loop body by the loop condition to be less than
                         KeyM_GetSizeOfCertificate().

  \ID VCA_KEYM_CERT_GETSLOTREFIDX
    \DESCRIPTION      Assertion slotIdx < KeyM_GetSizeOfCertificateSlot()
    \COUNTERMEASURE   \N slotIdx is assined a value from the CSL macro KeyM_GetSlotRefIdxOfCertificate() which takes localCertId as input.
                      localCertId is asserted to be be valid with a previous VCA assertion (localCertId < KeyM_GetSizeOfCertificate()).

    \ID VCA_KEYM_CERT_GETSLOTOWNER
    \DESCRIPTION      Assertion slotOwnerCertId < KeyM_GetSizeOfCertificate()
    \COUNTERMEASURE   \N slotOwnerCertId is assined a value from the CSL macro KeyM_GetSlotOwner() which takes slotId as input.
                      slotId is assigned a value from the CSL macro KeyM_GetIdOfCertificateSlot() which takes slotIdx as input.
                      slotIdx is asserted to be valid with VCA_KEYM_CERT_GETSLOTREFIDX.
  ...
  \ID  VCA_KEYM_CERT_GETDYNAMICISSUERCERTID_CACHED_ELEMENTS
    \DESCRIPTION      According to VCA, the following assertion may fail: issuerCertIdLocal < KeyM_GetSizeOfCertificate()
    \COUNTERMEASURE   \R The variable in question was read from KeyM_Cert_UpperHierarachicalRefInfo[certId].issuerCertId.
                      certId is a valid index in that array, as has been verified by an assertion in this function.
                      The member issuerCertId of the members of KeyM_Cert_UpperHierarachicalRefInfo is only ever written
                      in this function. Before the value is written, an assertion checks that the value to be written is
                      < KeyM_GetSizeOfCertificate(). Only then is the value written to
                       KeyM_Cert_UpperHierarachicalRefInfo[certId].issuerCertId. At the same time,
                       KeyM_Cert_UpperHierarachicalRefInfo[certId].isIssuerCertIdAvailable is set to TRUE.
                       This is the only place where the isIssuerCertIdAvailable member of a
                       KeyM_Cert_UpperHierarachicalRefInfo[certId] member is set to TRUE.
                       A runtime check is in place that checks that
                       KeyM_Cert_UpperHierarachicalRefInfo[certId].isIssuerCertIdAvailable == TRUE.
                       We can thus be sure that the value of KeyM_Cert_UpperHierarachicalRefInfo[certId].issuerCertId
                       is < KeyM_GetSizeOfCertificate().
