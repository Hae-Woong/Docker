...
# if (KEYM_DELEGATEDRESPONDERREFIDXOFOCSP == STD_ON)
/**********************************************************************************************************************
 * KeyM_Cert_FindValidOCSPResponder()
 *********************************************************************************************************************/
/*! \brief         Find the OCSP Responder certificate in the certificates attached to the OCSP response.
 *  \details       The function searches the certificates extracted from the OCSP response for the OCSP responder.
 *                 If the OCSP responder is found, its certificate chain of trust is validated.
 *  \param[in]     certIds               Holds the certificate identifiers of the certificates extracted from the
 *                                       OCSP response.
 *  \param[in]     numCertIds            number of valid Certificate IDs stored in certIds
 *  \param[in]     requestData           Pointer to OCSP response.
 *  \param[in]     ocspResp              Holds the parsing information of the OCSP response.
 *  \param[in]     ocspId                Holds the identifier of OCSP response configuration instance.
 *  \param[out]    responderCertId       Container for returning the certificate identifier of the OCSP responder.
 *  \return        E_OK                  The OCSP responder's certificate identifier was retrieved
 *                                       successfully.
 *                 E_NOT_OK              No OCSP responder could be found.
 *  \pre           -
 *  \context       TASK
 *  \reentrant     FALSE
 *  \synchronous   TRUE
 *********************************************************************************************************************/
KEYM_LOCAL_INLINE FUNC(Std_ReturnType, KEYM_CODE) KeyM_Cert_FindValidOCSPResponder(
  P2CONST(KeyM_CertificateIdType, AUTOMATIC, AUTOMATIC) certIds,
  uint16 numCertIds,
  P2CONST(uint8, AUTOMATIC, KEYM_APPL_DATA) requestData,
  P2CONST(KeyM_Asn1_OCSPResponseElementInfoType, AUTOMATIC, KEYM_APPL_VAR) ocspResp,
  uint16 ocspId,
...
/**********************************************************************************************************************
 * KeyM_NvBlock_ReadFromBlock_Cert()
 *********************************************************************************************************************/
/*! \brief         Sub routine of KeyM_NvBlock_ReadFromBlock_Copy() that performs the actual copying.
 *  \details       Check if NvmBuffer is empty, do nothing if it is empty
 *                 Else, do the following:
 *                 Determine slot owner if slot sharing is active and used by the given certificate,
 *                 copy data from nvmBuffer to ramBuffer,
 *                 parse certificate,
 *                 verify certificate elements.
 *  \param[in]     certIdx         Holds the identifier of the certificate.
 *  \param[in]     nvMBufferPtr    RAM mirror where Ram block data can be read from.
 *  \param[in]     nvmBlockLength  Length of the NvmBuffer.
 *  \param[in]     ramBuffer       ramBuffer to write the certificate to.
 *  \return        TRUE            if the data was copied or the buffer is empty
 *                 FALSE           if the data was not copied
 *                                 because the slot owner could not be determined
 *                                 or because the data was not valid
 *  \pre           certIdx needs to be valid.
 *                 nvMBufferPtr needs to be a valid pointer.
 *                 The buffer referenced by nvMBufferPtr must contain a valid length value.
 *                 ramBuffer must be a valid ramBuffer of sufficient length.
 *  \context       TASK
 *  \reentrant     FALSE
 *  \synchronous   TRUE
 *  \spec          requires certIdx < KeyM_GetSizeOfCertificate();                                             \endspec
 *********************************************************************************************************************/
KEYM_LOCAL_INLINE FUNC(boolean, KEYM_CODE) KeyM_NvBlock_ReadFromBlock_Cert(
  KeyM_SizeOfCertificateType certIdx,
  P2CONST(uint8, AUTOMATIC, KEYM_APPL_DATA) nvMBufferPtr,
...
/**********************************************************************************************************************
 * KeyM_Cert_SetCsmCertificateSlotOnwersAtStartUp()
 *********************************************************************************************************************/
/*!\brief         Set the slot owner for available CSM certificates at startup.
 * \details       -
 * \param[in]     certAvailable          Contains the information whether RAM data is present for every certificate.
 * \return        E_OK                   if all calls to sub routines succeeded and the slot owners have been set,
 *                E_NOT_OK               if one of the calls to a sub routine failed
 * \pre           certAvailable must point to a boolean buffer of size >= KEYM_NUMBER_OF_CERTIFICATES.
 * \context       TASK
 * \reentrant     FALSE
 * \synchronous   TRUE
 *********************************************************************************************************************/
KEYM_LOCAL_INLINE FUNC(Std_ReturnType, KEYM_CODE) KeyM_Cert_SetCsmCertificateSlotOnwersAtStartUp(
  P2CONST(boolean, AUTOMATIC, KEYM_APPL_DATA) certAvailable);
#endif
...
#if (KEYM_CERTIFICATESLOTUSEDOFCERTIFICATE == STD_ON)
/**********************************************************************************************************************
 * KeyM_Cert_CheckAlgorithmsMatch()
 **********************************************************************************************************************/
/*! \brief          Check whether the configured algorithm for the given certificate index matches the one in the data.
 *  \details        Retrieve the configured primary algorithm for the given certificate index,
 *                  compare it against the primary algorithm from the provided certificate data,
 *                  if a secondary algorithm is used for the provided certificate, compare that against the
 *                  respective configured value as well.
 *                  If all calls to sub routines succeed, return E_OK and use matchOut to indicate whether all compared
 *                  algorithms matched.
 *  \param[in]      certIdx           holds the certificate index to get the configured algorithm elements from
 *  \param[in]      ramBufferPtr      points to the certificate data to get the algorithm(s) from
 *  \param[in]      ramBufferLength   holds the length of the provided certificate data
 *  \param[out]     matchOut          is used to indicate whether the algorithms matched, is set to TRUE then.
 *  \pre            certIdx must be a valid certificate index, i.e. certIdx < KeyM_GetSizeOfCertificates()
 *                  All provided pointers must be valid.
 *  \return         E_OK              if all calls to sub routines succeeded
 *                  E_NOT_OK          if one of the calls to a sub routine failed
 *  \content        TASK
 *  \reentrant      TRUE
 *  \synchronous    TRUE
 *********************************************************************************************************************/
KEYM_LOCAL_INLINE FUNC(Std_ReturnType, KEYM_CODE) KeyM_Cert_CheckAlgorithmsMatch(
  KeyM_CertificateIterType certIdx,
  P2CONST(uint8, AUTOMATIC, KEYM_APPL_DATA) ramBufferPtr,
  const uint32 ramBufferLength,
  P2VAR(boolean, AUTOMATIC, KEYM_APPL_DATA) matchOut);
#endif

#if (KEYM_CERTIFICATESLOTUSEDOFCERTIFICATE == STD_ON)
/**********************************************************************************************************************
 * KeyM_Cert_DetermineSlotOwnerByCertIndexAndAlgo()
 **********************************************************************************************************************/
/*! \brief          Find the certificate ID of the provided certificate data by comparing the algorithm for all
 *                  certificates configured for the certificate slot.
 *  \details        Based on the given index, go through all certificates that are configured for the same slot
 *                  for every certificate, check if the configured algorithm matches the algorithm in the given data
 *  \param[in]      certIdx           holds the certificate index to get the configured algorithm(s) from
 *  \param[in]      ramBufferPtr      points to the certificate data to get the algorithm(s) from
 *  \param[in]      ramBufferLength   holds the length of the provided certificate data
 *  \param[out]     certIdOut         is set to the certificate id of the matching certificate
 *  \pre            certIdx must be a valid certificate index, i.e. certIdx < KeyM_GetSizeOfCertificates().
 *                  Slot sharing must be enabled for the given certIdx.
 *                  All provided pointers must be valid.
 *  \return         E_OK              if all calls to sub routines succeed,
 *                  E_NOT_OK          if one of the calls to a sub routine fails
 *  \context        TASK
 *  \reentrant      TRUE
 *  \synchronous    TRUE
 *********************************************************************************************************************/
KEYM_LOCAL_INLINE FUNC(Std_ReturnType, KEYM_CODE) KeyM_Cert_DetermineSlotOwnerByCertIndexAndAlgo(
  KeyM_SizeOfCertificateType certIdx,
  P2CONST(uint8, AUTOMATIC, KEYM_APPL_DATA) ramBufferPtr,
  const uint32 ramBufferLength,
  P2VAR(KeyM_CertificateIdType, AUTOMATIC, KEYM_APPL_DATA) certIdOut);
#endif
 ...
 *  \param[in]     certId          Holds the certificate identifier.
 *  \pre           certId must be a valid certificate ID, i.e., certId < KeyM_GetSizeOfCertificate()
 *  \return        TRUE            Certificate identifier matches slot owner or certificate is not available.
  ...
  P2VAR(KeyM_CertificateIdType, AUTOMATIC, AUTOMATIC) responderCertId)
{
  ...
  /* # Check if one of the certificates contained in the OCSP response is the valid responder certificate */
  for(uint8_least i = 0u; i < numCertIds; i++)
  {
...
#   if (KEYM_RESPONDERPUBKEYHASHCSMJOBREFOFOCSP == STD_ON)
      ...
      {
        retVal = KeyM_Cert_CheckOCSPResponderByKey(ocspResp, ocspId, requestData, certIds[i], &isResponderCertIdFound);
      }
      else
#   endif
      {
        retVal = KeyM_Cert_CheckOCSPResponderByName(ocspResp, requestData, certIds[i], &isResponderCertIdFound);
      }

      if ((retVal == E_OK) && (isResponderCertIdFound))
      {
        /* # Confirm that the responder is valid */
        retVal = KeyM_Cert_VerifyOCSPResponder(certIds[i]);

        /* # If the OCSP responder is valid, set responderCertId */
        if (retVal == E_OK)
        {
          *responderCertId = certIds[i];
        }
      }
    }
    else
  ...
  /* # If no Responder Cert ID could be found, return E_NOT_OK */
  ...
  P2VAR(KeyM_CertificateIdType, AUTOMATIC, AUTOMATIC) responderCertId)
{
  ...
  uint8_least i = 0u;
  uint16 offset = 0u;
  KeyM_CertificateIdType certIds[KEYM_CERTIFICATE_CHAIN_MAX_DEPTH] = { 0u };
  uint8 numCertIds = 0u;
  ...
  if (retVal == E_OK)
  {
    ...
    /* Iterate through included certs and store them */
    for (i = 0u; i < KEYM_CERTIFICATE_CHAIN_MAX_DEPTH; i++)
    {
      ...
      if (retVal == E_OK)
      {
        ...
        else
        {
          ...
          retVal = KeyM_Cert_SetCertificateInGroup(groupId, &certData[asn1Param.last_tag_base_pos + offset], (uint32)asn1Param.current_pos + (uint32)asn1Param.last_tag_len - (uint32)asn1Param.last_tag_base_pos, &certIds[i], TRUE);
        }
        ...
        if (retVal == E_OK)
        {
          ...
          KeyM_Asn1_NextTag(&asn1Param);
          /* # Increase counter for stored certifictes after each successful call to KeyM_Cert_SetCertificateInGroup */
          numCertIds++;
        }
      }
    ...
    if(retVal == E_OK)
    {
      /* # Check if certificates from OCSP response contain a valid responder */
      retVal = KeyM_Cert_FindValidOCSPResponder(certIds, numCertIds, requestData, ocspResp, ocspId, responderCertId);
    }
  }
  ...
  KeyM_CertificateIdType certId)
{
  ...
  {
    /* # Extract signature from OCSP response */
    if (KeyM_Asn1_GetAlgoType(responderCertId) == KEYM_ASN1_ALGO_X509_ECDSA)
    {
      /* # Apply special handling when extracting ECDSA signature */
  ...
  KeyM_CertificateIdType responderCertId)
{
  ...
  if ((retVal == E_OK) && (certStatus != KEYM_CERTIFICATE_VALID))
  {
    ...
    if (retVal == E_OK)
    {
      ...
      if ((retVal == E_OK) && (localCertId != issuerCertId))
      {
        KeyM_CertificateIdType lastCertId = KEYM_CERT_INVALID_ID;

        /* # Iterate through chain */
        for (i = 0u; i < KEYM_CERTIFICATE_CHAIN_MAX_DEPTH; i++)
        {
          lastCertId = localCertId;
          /* # Verify certificate in OCSP responder chain */
          retVal = KeyM_Cert_VerifyCertInOCSPResponderChain(&localCertId, &issuerCertId);

          /* # Break the loop when the complete certificate chain was verified or the verification failed */
          if ((lastCertId == localCertId) || (retVal != E_OK))
          {
            break;
          }
        }
...
#  endif
    ...
    if (isSubGroupFound || (retVal != E_OK))
    {
      break;
    }
  }
  ...
  P2VAR(KeyM_CertificateIdType, AUTOMATIC, AUTOMATIC) certId)
{
  ...
  for (certIdx = 0u; certIdx < KEYM_CERTIFICATE_CHAIN_MAX_DEPTH; certIdx++)
  {
    ...
    if (isRoot || (retVal != E_OK)) /* PRQA S 2991, 2995 */ /* MD_KEYM_14.3, MD_KEYM_2.2 */
    {
...
#  if (KEYM_CERTIFICATESLOTUSEDOFCERTIFICATE == STD_ON)
  KeyM_IdOfCertificateSlotType slotId = 0u;
  KeyM_CertificateIdType slotOwnerCertId = 0u;
#  endif
...
#  if (KEYM_CERTIFICATESLOTUSEDOFCERTIFICATE == STD_ON)
    else
    {
      if (KeyM_IsCertificateSlotUsedOfCertificate((KeyM_CertificateIterType)localCertId))
      {
        ...
        /*@ assert slotIdx < KeyM_GetSizeOfCertificateSlot(); */ /* VCA_KEYM_CERT_GETSLOTREFIDX */
        ...
        /*@ assert slotOwnerCertId < KeyM_GetSizeOfCertificate(); */ /* VCA_KEYM_CERT_GETSLOTOWNER */
...
#  if (KEYM_CERTIFICATEGROUPUSEDOFCERTIFICATE == STD_ON)
    ...
    if((retVal != E_OK) && (KeyM_GetCertUpperHierarchicalTypeOfCertificate((KeyM_CertificateIterType)certId) == KEYM_DYNAMIC_OPTIONAL_ISSUER))
    {
      upperCertId = certId;
      retVal = E_OK;
    }
...
{
  /* ----- Local Variables ------------------------------------------------ */
  uint16 certLength = 0u;
  boolean retVal = TRUE;
  boolean bufferEmpty = TRUE;

  /* ----- Implementation ------------------------------------------------- */
  /* # Read stored certificate written length */
  certLength = KeyM_Utils_ConvertUint8ArrayToUint16BigEndian(&nvMBufferPtr[KEYM_CERT_POS_NVBLOCK_LENGTH]); /* VCA_KEYM_NVBLOCK_READFROMBLOCK_COPY_READHEADER */

  {
    ...
    {
      /* A byte that is not zero? Then the buffer is not empty */
  ...
  if (bufferEmpty)
  {
    ...
    /* check that NvM block contains the whole claimed certificate data */
    (nvmBlockLength >= (KEYM_CERT_SIZEOF_NVBLOCK_HEADER + (uint32)certLength))
    /* check that RAM buffer can hold the certificate data from the NvM block */
    && (ramBuffer.certDataLength >= certLength))
  {
    KeyM_CertificateIdType determinedCertId = 0xFFu;

#  if (KEYM_CERTIFICATESLOTUSEDOFCERTIFICATE == STD_ON)
    /* If the certificate uses slot sharing set the determined cert id as the slot owner */
    if (KeyM_IsCertificateSlotUsedOfCertificate((KeyM_CertificateIterType) certIdx))
    {
      /* Determine the slot owner by comparing the algorithms of the certificate to the algorithms
       * of certificates that are configured for the slot at hand         */
      if (KeyM_Cert_DetermineSlotOwnerByCertIndexAndAlgo(certIdx,
                                                          &nvMBufferPtr[KEYM_CERT_SIZEOF_NVBLOCK_HEADER],
                                                          nvmBlockLength,
                                                          &determinedCertId)
          == E_OK)
      {
        /* The slot owner was determined successfully, set it */
        /*@ assert determinedCertId < KeyM_GetSizeOfCertificate(); */ /* VCA_KEYM_CERT_DETERMINESLOTOWNERCERTID */
        KeyM_Cert_SetSlotOwner(determinedCertId);
      }
      else
      {
        retVal = FALSE;
    ...
    /* Certificate does not use slot sharing? Then the certIdx is just a certId */
    else
    {
      determinedCertId = certIdx;
    }
#  else
    /* Slot sharing is not enabled in the config, so the determinedCertId is just the certIdx itself */
    determinedCertId = certIdx;
#  endif
...
#  endif
    {
      ...
      KeyM_SetCert_StorageType((KeyM_Cert_StorageTypeIterType)(certIdx),
                              (tKeyMCertStorageType)((KeyM_GetCert_StorageType((KeyM_Cert_StorageTypeIterType)certIdx))
                                                        | (KEYM_CERT_STORAGE_MASK_NVM
                                                        | KEYM_CERT_STORAGE_MASK_DATA_BUFFER)));

      /* Set the certificate status to NOT_PARSED */
      KeyM_Cert_SetStatusInfo(determinedCertId, KEYM_CERTIFICATE_NOT_PARSED);

      /* # Parse certificate */
      (void)KeyM_Cert_StateParseCert(determinedCertId);

      /* # Verify certificate elements */
      (void)KeyM_Cert_StateVerifyCertElements(determinedCertId);
    }
  }
  else
  {
    /* The block is neither empty, nor could the data be copied because of a size mismatch
  ...
  P2CONST(uint8, AUTOMATIC, KEYM_APPL_DATA) NvMBufferPtr)
{
  ...
  if (KeyM_IsNvmBlockUsedOfCertificate(certIdx))
  {
    ...
    if (nvmBlockLength >= KEYM_CERT_SIZEOF_NVBLOCK_HEADER) /* PRQA S 2991, 2995 */ /* MD_KEYM_14.3, MD_KEYM_2.2 */
    {
      ...
      /* # Check header by reading the version byte from it */
      validBlock = KeyM_NvBlock_ReadFromBlock_Header(NvMBufferPtr);

      if (validBlock)
      {
        validBlock = KeyM_NvBlock_ReadFromBlock_Cert(certIdx, NvMBufferPtr, nvmBlockLength, ramBuffer);
      }
    }
    ...
    if (!validBlock)
    {
      ...
      /* The return value of KeyM_NvBlock_Init() is voided because we currently are in the function
       * KeyM_NvBlock_ReadFromBlock_Copy(), which in turn is in the function KeyM_NvBlock_ReadFromBlock(), which
       * is called by the NvM. For the NvM the return value of the function KeyM_NvBlock_ReadFromBlock() indicates
       * wheter the data was copied from the buffer. This is what actually happended. Nevertheless something went
       * wrong, as the data was not valid. But the NvM does not care about the validity of the data or the
       * validity of the certificate, so we don't tell it about that. */
  ...
  P2CONST(boolean, AUTOMATIC, KEYM_APPL_DATA) certAvailable)
{
  ...
  /* ----- Implementation ------------------------------------------------- */
  /* Go through all configured certificates */
  for (certIdx = 0u; certIdx < KeyM_GetSizeOfCertificate(); certIdx++)
  {
    /* If the current certificate does not use slot sharing, skip it */
    if (!KeyM_IsCertificateSlotUsedOfCertificate((KeyM_CertificateIterType) certIdx))
    {
      continue;
    }

    /* Only look at available certificates stored in the CSM */
    if ((KeyM_GetStorageOfCertificate((KeyM_CertificateIterType)certIdx) == KEYM_STORAGE_IN_CSM)
        && (certAvailable[certIdx]))
    {
      KeyM_CertificateIdType determinedCertId = 0xFFu;
      KeyM_CertDataType ramBuffer = KeyM_Cert_GetCertificateWriteBufferData((KeyM_CertificateIdType) certIdx);

      /* Determine the certId of the current slot owner */
      if (KeyM_Cert_DetermineSlotOwnerByCertIndexAndAlgo((KeyM_SizeOfCertificateType) certIdx,
                                                         ramBuffer.certData,
                                                         ramBuffer.certDataLength,
                                                         &determinedCertId)
          == E_OK)
      {
        /*@ assert determinedCertId < KeyM_GetSizeOfCertificate(); */ /* VCA_KEYM_CERT_DETERMINESLOTOWNERCERTID */
      ...
      {
        /* Abort the whole function if the call to KeyM_Cert_DetermineSlotOwnerByCertIndexAndAlgo() fails */
        retVal = E_NOT_OK;
        break;
      }
    }
...
# if (KEYM_CERTIFICATESLOTUSEDOFCERTIFICATE == STD_ON)
  retVal = KeyM_Cert_SetCsmCertificateSlotOnwersAtStartUp(certAvailable);
...
#  if (KEYM_ELEMENTOIDPRIMARYUSEDOFCERTIFICATEGROUP == STD_ON)
  KeyM_CertificateGroupIdxOfCertificateType groupIdx = 0xFFu;
# endif
  uint8 oidData[KEYM_CERTIFICATESUBJECTPUBLICKEYINFO_PUBLICKEYALGORITHM_MAX_LENGTH] = { 0u };
  uint32 oidDataLength = KEYM_CERTIFICATESUBJECTPUBLICKEYINFO_PUBLICKEYALGORITHM_MAX_LENGTH;

  /* ----- Implementation ----------------------------------------------- */
  /* Retrieve public key algorithm from provided certificate data, store it in local buffer */
  retVal = KeyM_Cert_CertElementGetByStructureType(ramBufferPtr,
                                                   ramBufferLength,
                                                   KEYM_CERTIFICATE_SUBJECT_PUBLIC_KEY_INFO_PUBLIC_KEY_ALGORITHM,
                                                   oidData,
                                                   &oidDataLength);

/* Compare primary oid for sub groups */
#  if (KEYM_ELEMENTOIDPRIMARYUSEDOFCERTIFICATEGROUP == STD_ON)
  groupIdx = KeyM_GetCertificateGroupIdxOfCertificate(certIdx);
  /*@ assert groupIdx < KeyM_GetSizeOfCertificateGroup(); */ /* VCA_KEYM_CERT_CHECKALGORITHMSMATCH */
  if (retVal == E_OK)
  {
    /* Retrieve elementOid primary start index of certificate group from config */
    KeyM_ElementOidPrimaryStartIdxOfCertificateGroupType primaryOidStartIdx = KeyM_GetElementOidPrimaryStartIdxOfCertificateGroup(groupIdx);

    /* Compare primary pubKey algorithm in local buffer against configured algorithm */
    match = KeyM_Utils_BufferCmp(KeyM_GetAddrElementOid(primaryOidStartIdx),
                                 KeyM_GetElementOidPrimaryLengthOfCertificateGroup(groupIdx),
                                 oidData,
                                 oidDataLength);
  }
/* No sub groups or no groups at all exist */
#  else
  if (retVal == E_OK)
  {
    KeyM_SizeOfElementOidType oidStartIdx = 0u;
    KeyM_CertificateElementIterType elementIdx = 0u;

    for (elementIdx = KeyM_GetCertificateElementStartIdxOfCertificate(certIdx); elementIdx < KeyM_GetCertificateElementEndIdxOfCertificate(certIdx); elementIdx++)
    {
      if (KeyM_GetOfStructureOfCertificateElement(elementIdx) == KEYM_CERTIFICATE_SUBJECT_PUBLIC_KEY_INFO_PUBLIC_KEY_ALGORITHM)
      {
        oidStartIdx = KeyM_GetElementOidStartIdxOfCertificateElement(elementIdx);

        /* Compare pubKey algorithm in local buffer against configured algorithm */
        /*@ assert elementIdx < KeyM_GetSizeOfCertificateElement(); */
        /*@ assert oidStartIdx < KeyM_GetSizeOfElementOid(); */ /* VCA_KEYM_IDX_FROM_OPTIONAL_CSL_INDIRECTION */
        match = KeyM_Utils_BufferCmp(KeyM_GetAddrElementOid(oidStartIdx),
...
/* The secondary oid only comes into play when ecdsa is used */
#  if (KEYM_ELEMENTOIDSECONDARYUSEDOFCERTIFICATEGROUP == STD_ON)
  /* Additionally compare the ECC curve if the configuration requires it */
  if (match)
  {
    if (KeyM_IsElementOidSecondaryUsedOfCertificateGroup(groupIdx))
    {
      /* Retrieve ecc curve from certificate data, store it in local buffer */
      oidDataLength = KEYM_CERTIFICATESUBJECTPUBLICKEYINFO_PUBLICKEYALGORITHM_MAX_LENGTH;
      retVal = KeyM_Cert_CertElementGetByStructureType(ramBufferPtr,
                                                       ramBufferLength,
                                                       KEYM_CERTIFICATE_SUBJECT_PUBLIC_KEY_INFO_PUBLIC_KEY_ECC_CURVE,
                                                       oidData,
                                                       &oidDataLength);

      if (retVal == E_OK)
      {
        /* Retrieve elementOid secondary start index of certificate group from config */
        KeyM_ElementOidSecondaryStartIdxOfCertificateGroupType secondaryOidStartIdx =
          KeyM_GetElementOidSecondaryStartIdxOfCertificateGroup(groupIdx);
        /*@ assert secondaryOidStartIdx < KeyM_GetSizeOfElementOid(); */ /* VCA_KEYM_IDX_FROM_OPTIONAL_CSL_INDIRECTION */

        /* Compare secondary pubKey algorithm in local buffer against configured algorithm */
        match = KeyM_Utils_BufferCmp(KeyM_GetAddrElementOid(secondaryOidStartIdx),
                                     KeyM_GetElementOidSecondaryLengthOfCertificateGroup(groupIdx),
                                     oidData,
                                     oidDataLength);
      }
...
#  endif
  ...
  /* retVal == E_OK ? Then the retrieval of the data succeeded. This does NOT mean that we have a match */
  if (retVal == E_OK)
  {
    *matchOut = match;
  }
  ...
  P2VAR(KeyM_CertificateIdType, AUTOMATIC, KEYM_APPL_DATA) certIdOut)
{
  ...
  /* ----- Implementation ----------------------------------------------- */
  /* Go through all configured certificates */
  for (KeyM_CertificateIterType i = 0u; i < KeyM_GetSizeOfCertificate(); i++)
  {
    /* Only look at the ones that use the same slot as the given certificate */
    if (KeyM_GetSlotRefIdxOfCertificate(i) == KeyM_GetSlotRefIdxOfCertificate((KeyM_CertificateIterType) certIdx))
    {
      /* Check if the algorithms of the current certificate match the algorithms in the data */
      boolean matchFound = FALSE;
      /*@ assert ramBufferPtr != NULL_PTR; */ /* VCA_KEYM_NVBLOCK_DETERMINESLOTOWNER */
      retVal = KeyM_Cert_CheckAlgorithmsMatch(i, ramBufferPtr, ramBufferLength, &matchFound);
      if (matchFound)
      {
        *certIdOut = (KeyM_CertificateIdType) i;
      }

      /* Break the loop either if something was not ok (retVal != E_OK)
       * or if a match was found (in this case CertIdOut has already been written to) */
      if ((retVal != E_OK) || matchFound)
      {
        break;
      }
    }

    /* If we get to the very end of the loop, we know that the slot owner could not be determined.
     * To signal this we need to return E_NOT_OK.   */
    if (i == (KeyM_GetSizeOfCertificate() - (KeyM_CertificateIterType)1u))
    {
      retVal = E_NOT_OK;
    }
  }
...
#  if (KEYM_ELEMENTOIDPRIMARYUSEDOFCERTIFICATEGROUP == STD_ON)
  if (KeyM_Cert_IsGroupHeterogeneous(localGroupId))
  {
    ...
    /*@ assert localGroupId < KeyM_GetSizeOfCertificateGroupExt(); */ /* VCA_KEYM_CERT_GETSUBGROUPID */
  }
  else
#  endif
  {
