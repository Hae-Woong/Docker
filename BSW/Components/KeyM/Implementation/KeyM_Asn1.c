...
#define KEYM_CERTIFICATE_PUB_KEY_EDDSA_ED25519_ENCODED_LENGTH         (0x21u)
#define KEYM_CERTIFICATE_PUB_KEY_EDDSA_ED448_ENCODED_LENGTH           (0x3Au)
#define KEYM_CERTIFICATE_PUB_KEY_ECDSA_SECP256_ENCODED_LENGTH         (0x42u)
#define KEYM_CERTIFICATE_PUB_KEY_ECDSA_SECP384_ENCODED_LENGTH         (0x62u)
#define KEYM_CERTIFICATE_PUB_KEY_ECDSA_SECP521_ENCODED_LENGTH         (0x86u)
...
/**********************************************************************************************************************
 * KeyM_Asn1_CheckCrlOptionalElements()
 *********************************************************************************************************************/
/*! \brief         Check for the absence of optional elements in a CRL and adapt parsing of a CRL accordingly.
 *  \details       Optional elements can be version, nextUpdate time, revocation list and extenson. 
 *  \param[in]     certId                          Holds the identifier of the certificate.
 *  \param[in,out] asn1ParseParam                  Pointer to the ASN.1 data descriptor.
 *  \param[in,out] asn1Param                       Pointer to the ASN.1 element descriptor.
 *  \pre           -
 *  \context       TASK
 *  \reentrant     TRUE, for the same certificate
 *  \synchronous   TRUE
 *********************************************************************************************************************/
KEYM_LOCAL_INLINE FUNC(void, KEYM_CODE) KeyM_Asn1_CheckCrlOptionalElements(
  KeyM_CertificateIdType certId,
  P2VAR(KeyM_Asn1_ParseType, AUTOMATIC, KEYM_APPL_VAR) asn1ParseParam,
  P2CONST(KeyM_Asn1_ParamType, AUTOMATIC, KEYM_APPL_VAR) asn1Param);
...
/**********************************************************************************************************************
 * KeyM_Asn1_IsEDDSAPubKeyEncoded()
 *********************************************************************************************************************/
/*! \brief         Check if EDDSA is encoded with leading zero byte.
 *  \details       -
 *  \return        TRUE                  EDDSA public key is encoded with a leading zero byte.
 *                 FALSE                 EDDSA public key is not encoded with a leading zero byte.
 *  \pre           -
 *  \context       TASK
 *  \reentrant     FALSE
 *  \synchronous   TRUE
 *********************************************************************************************************************/
KEYM_LOCAL_INLINE FUNC(boolean, KEYM_CODE) KeyM_Asn1_IsEDDSAPubKeyEncoded(void);

/**********************************************************************************************************************
 * KeyM_Asn1_IsECDSAPubKeyEncoded()
 *********************************************************************************************************************/
/*! \brief         Check if ECDSA is encoded with leading zero byte and format byte.
 *  \details       -
 *  \return        TRUE                  ECDSA public key is encoded with a leading zero byte and format byte.
 *                 FALSE                 ECDSA public key is not encoded with a leading zero byte and format byte.
 *  \pre           -
 *  \context       TASK
 *  \reentrant     FALSE
 *  \synchronous   TRUE
 *********************************************************************************************************************/
KEYM_LOCAL_INLINE FUNC(boolean, KEYM_CODE) KeyM_Asn1_IsECDSAPubKeyEncoded(void);
 ...
 *  \return        KEYM_ASN1_RC_LEN_OVERFLOW  Length mismatch.
   ...
   && (asn1Param->last_tag_len > 0u))
  {
    ...
    else
    {
      ...
      if (retVal == KEYM_ASN1_RC_OK)
      {
        ...
        if ((retVal == E_OK)
        && (asn1ParseParam->asn1_nestingLevel > 0u) /* Prevent read out of bounds when asn1_parseParamDerivedBuffers is accessed */
...
{
  /* ----- Implementation ----------------------------------------------- */
  /* Check if certificate type is CRL */
  if ((KeyM_GetCertFormatTypeOfCertificate((KeyM_CertificateIterType)certId) == KEYM_CRL))
  {
    /* Handle missing version element */
    if ((asn1ParseParam->structureTypeIdx <= (KeyM_Asn1_GetSizeOfCertElementDescr(certId) - 2u)) /* Prevent read out of bounds when accessing description table with structure type index */
        && ((KeyM_Asn1_CertElementDescr_CRL[asn1ParseParam->structureTypeIdx].structureType == KEYM_CERTIFICATE_VERSION_NUMBER) && (asn1Param->last_tag != KEYM_ASN1_TYPE_INTEGER)) /* only KEYM_CERTIFICATE_VALIDITY_PERIOD_NOT_AFTER missing */
    ...
    /* Handle missing nextUpdate element */
    if ((asn1ParseParam->structureTypeIdx <= (KeyM_Asn1_GetSizeOfCertElementDescr(certId) - 2u))
        && ((KeyM_Asn1_CertElementDescr_CRL[asn1ParseParam->structureTypeIdx].structureType == KEYM_CERTIFICATE_VALIDITY_PERIOD_NOT_AFTER))
        && (((asn1ParseParam->asn1_nestingLevel == KeyM_Asn1_CertElementDescr_CRL[asn1ParseParam->structureTypeIdx].constructedElementNestingLevel) && ((asn1Param->last_tag != KEYM_ASN1_TYPE_UTC_GENERALIZED_TIME) && (asn1Param->last_tag != KEYM_ASN1_TYPE_UTC_TIME))) /* only KEYM_CERTIFICATE_VALIDITY_PERIOD_NOT_AFTER missing */
            || (asn1ParseParam->asn1_nestingLevel < KeyM_Asn1_CertElementDescr_CRL[asn1ParseParam->structureTypeIdx].constructedElementNestingLevel))) /* all following optimal elements missing */
    {
        asn1ParseParam->structureTypeIdx++;
    }

    /* Handle missing revokedCertificates element */
    if ((asn1ParseParam->structureTypeIdx <= (KeyM_Asn1_GetSizeOfCertElementDescr(certId) - 2u))
        && (KeyM_Asn1_CertElementDescr_CRL[asn1ParseParam->structureTypeIdx].structureType == KEYM_REVOKED_CERTIFICATES)
        && (((asn1ParseParam->asn1_nestingLevel == KeyM_Asn1_CertElementDescr_CRL[asn1ParseParam->structureTypeIdx].constructedElementNestingLevel) && (asn1Param->last_tag != KEYM_ASN1_TYPE_SEQUENCE)) /* only KEYM_REVOKED_CERTIFICATES missing */
            || (asn1ParseParam->asn1_nestingLevel < KeyM_Asn1_CertElementDescr_CRL[asn1ParseParam->structureTypeIdx].constructedElementNestingLevel))) /* all following optimal elements missing */
    {
        asn1ParseParam->structureTypeIdx++;
    }

    /* Handle missing crlExtensions element */
    if ((asn1ParseParam->structureTypeIdx <= (KeyM_Asn1_GetSizeOfCertElementDescr(certId) - 2u))
       && ((KeyM_Asn1_CertElementDescr_CRL[asn1ParseParam->structureTypeIdx].structureType == KEYM_CERTIFICATE_EXTENSION) && ((asn1Param->last_tag & KEYM_ASN1_TYPE_USER_CONTEXT) != KEYM_ASN1_TYPE_USER_CONTEXT)) /* only KEYM_CERTIFICATE_EXTENSION missing */
       && (asn1ParseParam->asn1_nestingLevel == KeyM_Asn1_CertElementDescr_CRL[asn1ParseParam->structureTypeIdx].constructedElementNestingLevel))
    {
        asn1ParseParam->structureTypeIdx++;
    }
  ...
  P2VAR(KeyM_Asn1_ParamType, AUTOMATIC, KEYM_APPL_VAR) asn1ParseParamBuffer)
{
  ...
  {
    /* # Check if an optional element in certificate is not available */
    if (((asn1ParseParam->structureTypeIdx <= (KeyM_Asn1_GetSizeOfCertElementDescr(certId) - 2u))
        && (((KeyM_GetCertFormatTypeOfCertificate((KeyM_CertificateIterType)certId) == KEYM_X509)
    ...
    /* Check if an optional element in a CRL is not available */
    KeyM_Asn1_CheckCrlOptionalElements(certId, asn1ParseParam, asn1Param);
...
KEYM_LOCAL_INLINE FUNC(boolean, KEYM_CODE) KeyM_Asn1_IsEDDSAPubKeyEncoded(void)
{
  ...
  /* ----- Implementation ----------------------------------------------- */
  /* # Check if the first byte of the public key buffer is a leading zero byte */
  boolean isFirstByteLeadingZero = (KeyM_Asn1_PubKeyBuffer[0u] == KEYM_CERTIFICATE_PUB_KEY_BITSTRING_LEADING_ZERO_BYTE);

  /* # Check if the length of the public key buffer is one of the predefined lengths for encoded public keys for different EDDSA curves */
  boolean isPubKeyLengthEncoded = ((KeyM_Asn1_PubKeyLength == KEYM_CERTIFICATE_PUB_KEY_EDDSA_ED25519_ENCODED_LENGTH) || \
                                   (KeyM_Asn1_PubKeyLength == KEYM_CERTIFICATE_PUB_KEY_EDDSA_ED448_ENCODED_LENGTH));

  /* # If all conditions are met, set return value to TRUE */
  if (isFirstByteLeadingZero && isPubKeyLengthEncoded)
  {
    retVal = TRUE;
  }
...
KEYM_LOCAL_INLINE FUNC(boolean, KEYM_CODE) KeyM_Asn1_IsECDSAPubKeyEncoded(void)
{
  ...
  /* # Check if the second byte of the public key buffer represents uncompressed format */
  boolean isSecondByteUncompressedFormat = (KeyM_Asn1_PubKeyBuffer[1u] == KEYM_CERTIFICATE_PUB_KEY_BITSTRING_UNCOMPRESSED_FORMAT_BYTE);

  /* # Check if the length of the public key buffer is one of the predefined lengths for encoded public keys for different ECDSA curves */
  boolean isPubKeyLengthEncoded = ((KeyM_Asn1_PubKeyLength == KEYM_CERTIFICATE_PUB_KEY_ECDSA_SECP256_ENCODED_LENGTH) || \
                                   (KeyM_Asn1_PubKeyLength == KEYM_CERTIFICATE_PUB_KEY_ECDSA_SECP384_ENCODED_LENGTH) || \
                                   (KeyM_Asn1_PubKeyLength == KEYM_CERTIFICATE_PUB_KEY_ECDSA_SECP521_ENCODED_LENGTH));

  /* # If all conditions are met, set return value to TRUE */
  if (isFirstByteLeadingZero && isSecondByteUncompressedFormat && isPubKeyLengthEncoded)
  {
    retVal = TRUE;
  }
  ...
  KeyM_CertificateIdType certId)
{
  ...
  {
    if (!KeyM_Asn1_IsEDDSAPubKeyEncoded())
    {
  ...
  {
    if (!KeyM_Asn1_IsECDSAPubKeyEncoded())
    {
  ...
  KeyM_SizeOfCertificateElementType elementIdx)
{
  /* ----- Local Variables ---------------------------------------------- */
  Std_ReturnType retVal = KEYM_ASN1_RC_OK;
  KeyM_Ans1_Element_Ref_Type elementInfo = {0u};
  ...
  {
    /* Prevent read out of bounds when asn1_parseParamDerivedBuffers is accessed */
    if (asn1ParseParam->asn1_nestingLevel == 0u)
    {
      retVal = KEYM_ASN1_RC_LEN_OVERFLOW;
    }
    else
  ...
  if (retVal == KEYM_ASN1_RC_OK)
  {
    retVal = KeyM_Asn1_SetElementRefInfo(elementIdx, elementInfo, asn1ParseParam->totalDataLength);
  }
