...
typedef uint8           CanTp_PartIdxType; /* CanTp_PCPartitionConfigIdxOfPartitionIdentifiersType */


/* --- Structs ----------------------------------------------------------------------------------------------------- */

/*! */
typedef struct sCanTp_ConnIdType
{
  PduIdType         Hdl;
...
typedef struct sCanTp_RxStateType
{
  ...
  CanTp_ConnIdType                  ConnId;             /*!< ConnId containing the Rx Sdu handle to which this state data is related to and the index of the current CanTp-/OS-partition (which is unused in case CANTP_SUPPORT_MULTI_PARTITION is STD_OFF) and */
  ...
  uint8                             InitialDL;          /*!< DL in bytes of the FF against which the subsequent CFs must be checked */
...
typedef struct sCanTp_TxStateType
{
  ...
  CanTp_BufferStateType             BufferState;        /*!< status of buffer provision */
  CanTp_ConnIdType                  ConnId;             /*!< ConnId containing the Tx Sdu handle to which this state data is related to and the index of the current CanTp-/OS-partition (which is unused in case CANTP_SUPPORT_MULTI_PARTITION is STD_OFF) and */
...
#  if (CANTP_EXISTS_CUSTOM_ADDRESSING_TXADDRESSINGFORMATOFTXSDUCFG == STD_ON)
  uint8                             FcPduMetadata  [CANTP_MAX_PDU_METADATA_LENGTH]; /*!< local buffer for FC-PDU metadata */
#  endif
# endif
#endif
...
/*! MultiPartition specific types */
typedef uint32                                               CanTp_PartitionSNVOfPartitionIdentifiersType;
