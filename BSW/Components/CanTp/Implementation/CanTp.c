...
#if (  (CANTP_CFG_GEN_MAJOR_VERSION != (6u)) \
    || (CANTP_CFG_GEN_MINOR_VERSION != (0u)) )
...
#if (CANTP_APPL_RX_SDU_INDICATION == STD_ON)
# define CanTp_ApplRxIndication(PduRRxSduId, Result)                 (Appl_CanTpSduRxIndication((PduRRxSduId), (Result))); 
#else
# define CanTp_ApplRxIndication(PduRRxSduId, Result)
#endif

#if (CANTP_APPL_TX_SDU_CONFIRMATION == STD_ON)
# define CanTp_ApplTxConfirmation(PduRTxSduId, Result)               (Appl_CanTpSduTxConfirmation((PduRTxSduId), (Result))); 
#else
# define CanTp_ApplTxConfirmation(PduRTxSduId, Result)
#endif
...
# define CanTp_RxTransmitFrame_ISR(RxStatePtr)                       (CanTp_RxTransmitFrame((RxStatePtr)));
#else
/* FC is not transmitted in ISR context but on task level */
# define CanTp_RxTransmitFrame_ISR(RxStatePtr)
#endif
...
/* buffer request is performed immediately within ISR context */
# define CanTp_TxGetBuffer_ISR(TxStatePtr) (CanTp_TxGetBuffer((TxStatePtr)));
#else
/* buffer is not requested in ISR context but on task level */
# define CanTp_TxGetBuffer_ISR(TxStatePtr)
#endif

#if (CANTP_MAX_PDU_METADATA_LENGTH != 0)
/* There's PDU metadata to be assembled. */
# define CanTp_Util_RxAssembleNecessaryMetadata(ConnId, pDataPduPayload, pDataPduMetadata, pSduMetadata, pSduMetadataLength, pFcPduMetadata)    (CanTp_Util_RxAssembleSduMetadataAndFcPduMetadata( (ConnId), (pDataPduPayload), (pDataPduMetadata), (pSduMetadata), (pSduMetadataLength), (pFcPduMetadata) ));
#else
/* There's no PDU metadata to be assembled. */
# define CanTp_Util_RxAssembleNecessaryMetadata(ConnId, pDataPduPayload, pDataPduMetadata, pSduMetadata, pSduMetadataLength, pFcPduMetadata)    (CanTp_Util_RxAssembleSduMetadata( (pDataPduPayload), (pSduMetadata), (pSduMetadataLength) ));
#endif
...
/* --- Strict Length Handling ------------------------------------------------------------------------------------- */
#if (CANTP_NON_STRICT_LENGTHCHECK == STD_ON)
# define CanTp_IsCanDlValid(currentLength, expectedLength)  (currentLength >= expectedLength)
#else
# define CanTp_IsCanDlValid(currentLength, expectedLength)  (currentLength == expectedLength)
#endif
...
/* --- Helper to handle Multi Partition easier --------------------------------------------------------------------- */
#define CanTp_InitConnId(ConnId, Hdl2Set, PartIdx2Set) ConnId.Hdl=(Hdl2Set); ConnId.PartIdx=(PartIdx2Set);
...
#if (CANTP_SUPPORT_MULTI_PARTITION == STD_OFF)
CANTP_LOCAL VAR(boolean, CANTP_VAR_CLEARED) CanTp_Initialized = FALSE;
# define CanTp_IsInitialized(Index, partitionIndex)         (CanTp_Initialized)
# define CanTp_SetInitialized(Index, Value, partitionIndex) (CanTp_Initialized)=(Value)
#endif
...
#include "CanTp_MemMap.h"                                                                                              /* PRQA S 5087 */ /* MD_MSR_MemMap */
CANTP_LOCAL VAR(PduIdType, CANTP_VAR_NO_INIT) CanTp_NumRxChannelsAvail;
CANTP_LOCAL VAR(PduIdType, CANTP_VAR_NO_INIT) CanTp_NumTxChannelsAvail;
 ...
 *  \param[in]     pTpRxState         Pointer to channel data. Has to point to a valid variable of type
 *                                    CanTp_RxStateType or to a position in generated array
 *                                    CanTp_RxState[<OsApplicationName>].
 *  \context       TASK|ISR2
 *  \reentrant     FALSE
 ...
 *  \trace         DSGN-CanTp23076
 *********************************************************************************************************************/
CANTP_LOCAL FUNC(void, CANTP_CODE) CanTp_RxInit(CanTp_RxStatePtrType pTpRxState,
 ...
 *  \trace         DSGN-CanTp23076
 *********************************************************************************************************************/
CANTP_LOCAL FUNC(void, CANTP_CODE) CanTp_TxInit(CanTp_TxStatePtrType pTpTxState,
                                                CanTp_NotificationType NotifyCode,
                                                uint8 DetError);
#endif
...
/**********************************************************************************************************************
  CanTp_ResetChannels()
**********************************************************************************************************************/
/*! \brief         Resets all channels of the CanTp for the current CanTp-/OS-partition.
 *  \details       Initialize all channel related data to default.
 *  \param[in]     PartIdx            Index of the current CanTp-/OS-partition.
 *                                    PartIdx < CanTp_GetSizeOfPartitionIdentifiers();
 *                                    Unused and of value 0u in case CANTP_SUPPORT_MULTI_PARTITION is STD_OFF
 *  \context       TASK|ISR2
 *  \reentrant     FALSE
 *  \pre           -
 *  \trace         DSGN-CanTp23076
 *********************************************************************************************************************/
CANTP_LOCAL FUNC(void, CANTP_CODE) CanTp_ResetChannels(CanTp_PartIdxType PartIdx);
 ...
 *                                    CanTp_RxStateType or to a position in generated array
 *                                    CanTp_RxState[<OsApplicationName>].
 *  \param[in]     pPdu               Pointer to received N-PDU dat
 *  \context       TASK|ISR2
 *  \reentrant     FALSE
 *  \pre           Expects to be called in a critical section
 *  \pre           -
 ...
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(void, CANTP_CODE) CanTp_CanIfRxIndicationFF(
 ...
 *  \trace         DSGN-CanTp23053
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(void, CANTP_CODE) CanTp_CanIfRxIndicationCF(
  CanTp_RxStatePtrType pTpRxState,
  P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pPdu);
#endif
 ...
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(void, CANTP_CODE) CanTp_CanIfRxIndicationFC(
 ...
 *  \param[in]     ConnId.Hdl         Rx or Tx connection handle (depends on frame type)
 *                                    FrameType FC: Valid TxHdl, i.e. < CanTp_Cfg_GetNumTxHdls()
 *                                    Other FrameTypes: Valid RxHdl, i.e. < CanTp_Cfg_GetNumRxHdls()
 *                 ConnId.PartIdx     Index of the current CanTp-/OS-partition.
 *                                    PartIdx < CanTp_GetSizeOfPartitionIdentifiers();
 *                                    Unused and of value 0u in case CANTP_SUPPORT_MULTI_PARTITION is STD_OFF
 ...
 *********************************************************************************************************************/
CANTP_LOCAL FUNC(void, CANTP_CODE) CanTp_CanIfRxIndication(
  CanTp_ConnIdType ConnId,
  uint8 FrameType,
 ...
 *********************************************************************************************************************/
CANTP_LOCAL FUNC(CanTp_TxResultType, CANTP_CODE) CanTp_CanIfTransmitFrame(
  P2CONST(CanTp_PduTxInfoType, AUTOMATIC, CANTP_APPL_DATA) pPduTxData,
 ...
 *  \param[in]     ConnId.Hdl         Valid TxHdl, i.e. < CanTp_Cfg_GetNumTxHdls()
 *                 ConnId.PartIdx     Index of the current CanTp-/OS-partition. Unused in case CANTP_SUPPORT_MULTI_PARTITION is STD_OFF
 ...
 *  \trace         CREQ-102804
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(Std_ReturnType, CANTP_CODE) CanTp_TxStartConnection(
  PduIdType TxSduSnv,
  CanTp_ConnIdType ConnId,
 ...
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(Std_ReturnType, CANTP_CODE) CanTp_LoLayerTransmit(
...
/**********************************************************************************************************************
  CanTp_Util_MemCmp
**********************************************************************************************************************/
/*! \brief      Implements a simple compare loop, used by the CanTp itself
 *  \details    -
 *  \param[in]  mem1     Pointer to one array to compare with other array
 *  \param[in]  mem2     Pointer to other array
 *  \param[in]  length   Number of bytes to compare
 *  \return     0u       The memory areas are the same
 *  \return     1u       The memory areas are not the same
 *  \context    TASK|ISR2
 *  \pre        -
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(uint8, CANTP_CODE) CanTp_Util_MemCmp( P2CONST(uint8, AUTOMATIC, CANTP_APPL_DATA) mem1,
                                                              P2CONST(uint8, AUTOMATIC, CANTP_APPL_DATA) mem2,
 ...
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(boolean, CANTP_CODE) CanTp_Util_CheckConfigValidity(CanTp_PartIdxType PartIdx);
 ...
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(uint8, CANTP_CODE) CanTp_Util_GetPaddingLength(uint8 CurrentLength,
                                                                       boolean PaddingActivation,
                                                                       CanTp_CanType CanType);
#endif
...
/**********************************************************************************************************************
  CanTp_Util_GetFrameType()
**********************************************************************************************************************/
/*! \brief         Returns the type of a frame.
 *  \details       Reads the frame type from the first N_PCI byte, taking addressing offsets into account
 *  \param[in]     Addressing      Frame's addressing format.
 *  \param[in]     pPayloadInfo    PduInfo struct with PDU payload. Shall be at least 2 bytes long.
 *  \return        CANTP_FRAME_SF       SingleFrame
 *  \return        CANTP_FRAME_FF       FirstFrame
 *  \return        CANTP_FRAME_CF       ConsecutiveFrame
 *  \return        CANTP_FRAME_FC       FlowControlFrame
 *  \return        CANTP_FRAME_INVALID  Invalid
 *  \context       TASK|ISR2
 *  \reentrant     TRUE
 *  \pre           -
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(CanTp_FrameType, CANTP_CODE) CanTp_Util_GetFrameType(uint8 Addressing, P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pPayloadInfo);


/**********************************************************************************************************************
  CanTp_Util_GetAdrOffset()
**********************************************************************************************************************/
/*! \brief         Provides the offset due to in-payload N_AI for a given addressing format.
 *  \details       -
 *  \param[in]     Addressing    Addressing format for which the offset will be provided.
 *  \return        1u            For extended, mixed11 and mixed29 addressing formats.
 *  \return        0u            For any other addressing format.
 *  \context       TASK|ISR2
 *  \reentrant     TRUE
 *  \pre           -
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(uint8, CANTP_CODE) CanTp_Util_GetAdrOffset(uint8 Addressing);


#if (CANTP_GENERIC_CONNECTIONS == STD_ON)
# if (CANTP_MAX_PDU_METADATA_LENGTH != 0)
/**********************************************************************************************************************
  CanTp_Util_RxAssembleSduMetadataAndFcPduMetadata()
**********************************************************************************************************************/
/*! \brief         Assembles Rx SDU metadata and fc-PDU metadata.
 *  \details       Assembles Rx SDU metadata and the corresponding fc-PDU metadata based on the configuration
                   of the passed Rx SDU, data-PDU metadata and data-PDU payload. The resulting metadata is copied to
                   the passed destinations.
 *  \param[in]     ConnId.Hdl           Rx connection handle
 *                 ConnId.PartIdx       Index of the current CanTp-/OS-partition.
 *                                      PartIdx < CanTp_GetSizeOfPartitionIdentifiers();
 *                                      Unused and of value 0u in case CANTP_SUPPORT_MULTI_PARTITION is STD_OFF
 *  \param[in]     pDataPduPayload      PduInfo struct with data-PDU payload.
 *  \param[in]     pDataPduMetadata     PduInfo struct with data-PDU metadata.
 *  \param[out]    pSduMetadata         Pointer to a buffer of length CANTP_MAX_SDU_METADATA_LENGTH where the
 *                                      resulting SDU metadata shall be copied.
 *  \param[out]    pSduMetadataLength   Pointer to the variable where the length of the resulting SDU metadata shall
 *                                      be copied.
 *  \param[out]    pFcPduMetadata       Pointer to a buffer of length CANTP_MAX_PDU_METADATA_LENGTH where the
 *                                      resulting fc-PDU metadata shall be copied.
 *  \param[out]    pFcPduMetadataLength Pointer to the variable where the length of the resulting fc-PDU metadata
 *                                      shall be copied.
 *  \return        E_OK                 In case metadata assembly was successful.
 *  \return        E_NOT_OK             In case metadata assembly failed.
 *  \context       TASK|ISR2
 *  \reentrant      TRUE for non-overlapping destination memory areas of both SduMetadata and FcPduMetadata and
 *                       for different pTpRxState->ConnId.Hdl values.
 *  \config        This function is only available if CANTP_GENERIC_CONNECTIONS = STD_ON and
 *                 CANTP_MAX_PDU_METADATA_LENGTH != 0.
 *  \pre           -
 *  \trace         DSGN-CanTpGenericConnections, DSGN-CanTpCustomAddressing
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(Std_ReturnType, CANTP_CODE) CanTp_Util_RxAssembleSduMetadataAndFcPduMetadata(CanTp_ConnIdType ConnId,
                                                                                                     P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pDataPduPayload,
                                                                                                     P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pDataPduMetadata,
                                                                                                     P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA) pSduMetadata,
                                                                                                     P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA) pSduMetadataLength,
                                                                                                     P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA) pFcPduMetadata);

# else
/**********************************************************************************************************************
  CanTp_Util_RxAssembleSduMetadata()
**********************************************************************************************************************/
/*! \brief         Assembles Rx SDU metadata.
 *  \details       Assembles Rx SDU metadata based on data-PDU payload. The resulting metadata is copied to the passed
 *                 destination.
 *  \param[in]     pDataPduPayload        PduInfo struct with data-PDU payload.
 *  \param[out]    pSduMetadata           Pointer to a buffer of length CANTP_MAX_SDU_METADATA_LENGTH where the
 *                                        resulting SDU metadata shall be copied.
 *  \param[out]    pSduMetadataLength     Pointer to the variable where the length of the resulting SDU metadata shall
 *                                        be copied.
 *  \return        E_OK                 In case SDU metadata assembly was successful.
 *  \context       TASK|ISR2
 *  \reentrant     TRUE for different pSduMetadata, pSduMetadataLength.
 *  \config        This function is only available if CANTP_GENERIC_CONNECTIONS = STD_ON and
 *                 CANTP_MAX_PDU_METADATA_LENGTH == 0.
 *  \pre           -
 *  \trace         DSGN-CanTpGenericConnections, DSGN-CanTpCustomAddressing
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(Std_ReturnType, CANTP_CODE) CanTp_Util_RxAssembleSduMetadata(P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pDataPduPayload,
                                                                                     P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA) pSduMetadata,
                                                                                     P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA) pSduMetadataLength);
# endif
 ...
 *  \param[in]     pPayloadInfo           PduInfo struct with data-PDU payload.
 *  \param[in]     pMetadataInfo          PduInfo struct with data-PDU metadata.
 *  \return        E_OK                 Extraction of SDU metadata has worked.
 *  \return        E_NOT_OK             Extraction of SDU metadata has failed.
 *  \context       TASK|ISR2
 *  \reentrant     TRUE
 *  \config        This function is only available if CANTP_GENERIC_CONNECTIONS = STD_ON
 *  \pre           -
 *  \trace         DSGN-CanTpGenericConnections
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(Std_ReturnType, CANTP_CODE) CanTp_Util_RxExtractSduMetadataInfo(CanTp_RxStatePtrType pTpRxState,
                                                                                        P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pPayloadInfo,
                                                                                        P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pMetadataInfo);
 ...
 *  \trace         DSGN-CanTpGenericConnections, DSGN-CanTpCustomAddressing
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(Std_ReturnType, CANTP_CODE) CanTp_Util_TxExtractSduMetadataInfo(CanTp_TxStatePtrType pTpTxState,
                                                                                        P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) CanTpTxInfoPtr);
#endif
...
# else
#  define CanTp_Util_RxGetFCInPayloadAddressInfo(pTpRxState)    CanTp_RxCfg_GetTxFcAddress((pTpRxState)->ConnId)       /* PRQA S 3453 */ /* MD_MSR_FctLikeMacro */
# endif
#endif
...
# if (CANTP_GENERIC_CONNECTIONS == STD_ON)
CANTP_LOCAL_INLINE FUNC(uint8, CANTP_CODE) CanTp_Util_TxGetInPayloadAddressInfo(
  P2CONST(CanTp_TxStateType, AUTOMATIC, CANTP_APPL_DATA) pTpTxState);
# else
#  define CanTp_Util_TxGetInPayloadAddressInfo(pTpTxState) CanTp_TxCfg_GetTxAddress((pTpTxState)->ConnId)              /* PRQA S 3453 */ /* MD_MSR_FctLikeMacro */
# endif
#endif


/**********************************************************************************************************************
  CanTp_Util_GetPartIdx()
**********************************************************************************************************************/
/*! \brief         Provides the CSL memory section index of the given OS Application
 *  \details       -
 *  \param[out]    ErrorIdPtr: Error Id of a DET in case the partIdx could not be determined.
 *  \context       TASK|ISR2
 *  \reentrant     TRUE
 *  \pre           May only be called after call of CanTp_PreInit().
 *  \trace         DSGN-CanTp24875
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(CanTp_PartIdxType, CANTP_CODE) CanTp_Util_GetPartIdx(uint8* ErrorIdPtr);


#if(CANTP_SUPPORT_MULTI_PARTITION == STD_ON)
/**********************************************************************************************************************
  CanTp_Util_GetPartIdxOfRxPduId()
**********************************************************************************************************************/
/*! \brief         Provides the CSL memory section index of the given RxPduId
 *  \details       -
 *  \param[in]     PduId: SNV of a configured CanTpRxPdu
 *  \param[out]    ErrorIdPtr: Error Id of a DET in case the partIdx could not be determined.
 *  \context       TASK|ISR2
 *  \reentrant     TRUE
 *  \pre           May only be called after call of CanTp_PreInit().
 *  \trace         DSGN-CanTp24875
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(CanTp_PartIdxType, CANTP_CODE) CanTp_Util_GetPartIdxOfRxPduId(PduIdType PduId,
                                                                                      uint8* ErrorIdPtr);


/**********************************************************************************************************************
  CanTp_Util_GetPartIdxOfTxPduId()
**********************************************************************************************************************/
/*! \brief         Provides the CSL memory section index of the given TxPduId
 *  \details       -
 *  \param[in]     PduId: SNV of a configured CanTpTxPdu
 *  \param[out]    ErrorIdPtr: Error Id of a DET in case the partIdx could not be determined.
 *  \context       TASK|ISR2
 *  \reentrant     TRUE
 *  \pre           May only be called after call of CanTp_PreInit().
 *  \trace         DSGN-CanTp24875
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(CanTp_PartIdxType, CANTP_CODE) CanTp_Util_GetPartIdxOfTxPduId(PduIdType PduId,
                                                                                      uint8* ErrorIdPtr);
#endif
 ...
 *  \pre           Expects to be called in a critical section
 *  \pre           Expects that ConnId.Hdl is a valid RxHdl (< CanTp_Cfg_GetNumRxHdls())
 *  \pre           Expects that ConnId.PartIdx is a valid CSL partIdx or 0u (CanTpSinglePartitionMemory)
 *  \trace         DSGN-CanTp24842
 *********************************************************************************************************************/
CANTP_LOCAL FUNC(Std_ReturnType, CANTP_CODE) CanTp_RxCheckChannelMode(CanTp_ConnIdType ConnId);
#endif
 ...
 *  \trace         DSGN-CanTp24842
 *********************************************************************************************************************/
CANTP_LOCAL FUNC(Std_ReturnType, CANTP_CODE) CanTp_TxCheckChannelMode(CanTp_ConnIdType ConnId);
#endif
 ...
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(PduIdType, CANTP_CODE) CanTp_Cfg_GetSduHdlByRxPduOnly(PduIdType RxPduId,
                                                                              uint8 FrameType);
#endif
 ...
 *  \trace         DSGN-CanTp23090
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(PduIdType, CANTP_CODE) CanTp_Cfg_GetSduHdlByRxPduAddress(PduIdType RxPduId,
                                                                                 uint8 FrameType,
                                                                                 uint8 Address);
#endif
...
#if (CANTP_GENERIC_CONNECTIONS == STD_ON)
/**********************************************************************************************************************
  CanTp_Cfg_GetSduHdlByRxPduAndMetadata()
**********************************************************************************************************************/
/*! \brief         Gets the handle of an N-SDU associated to a given combination of Rx N-PDU and metadata.
 *  \details       -
 *  \param[in]     RxPduId            Rx N-PDU id passed by the lower layer.
 *  \param[in]     PartIdx            Index of the current CanTp-/OS-partition.
 *                                    PartIdx < CanTp_GetSizeOfPartitionIdentifiers();
 *                                    Unused and of value 0u in case CANTP_SUPPORT_MULTI_PARTITION is STD_OFF
 *  \param[in]     FrameType          SingleFrame, FirstFrame, ConsecutiveFrame or FlowControl.
 *  \param[in]     pPayloadInfo       PduInfo struct with data-PDU payload.
 *  \param[in]     pMetadataInfo      PduInfo struct with data-PDU metadata.
 *  \return        The handle of an N-SDU which is "active" and associated to the passed N-PDU and metadata.
 *  \return        The handle of an N-SDU which is "idle" and associated to the passed N-PDU.
 *  \return        An invalid handle, if no active or idle N-SDU could be found.
 *  \context       TASK|ISR2
 *  \reentrant     TRUE
 *  \config        This function is available if generic connections are enabled.
 *  \pre           -
 *  \trace         DSGN-SimultaneousGenericConnections
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(PduIdType, CANTP_CODE) CanTp_Cfg_GetSduHdlByRxPduAndMetadata(PduIdType RxPduId,
                                                                                     CanTp_PartIdxType PartIdx,
 ...
 *  \param[in]     pPayloadInfo       PduInfo struct with data-PDU payload.
 *  \param[in]     pMetadataInfo      PduInfo struct with data-PDU metadata.
 *  \return        pointer to the acquired state data, or NULL_PTR if there is no free channel for this ConnId.Hdl
 *  \context       TASK|ISR2
 *  \reentrant     FALSE
 *  \pre           Expects to be called in a critical section
 *  \pre           Expects that ConnId.Hdl is a valid RxHdl (< CanTp_Cfg_GetNumRxHdls())
 ...
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(CanTp_RxStatePtrType, CANTP_CODE) CanTp_RxGetFreeChannelPtr(
  CanTp_ConnIdType ConnId,
  P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pPayloadInfo,
  P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pMetadataInfo);
 ...
 *  \details       Get a pointer to the Rx channel state which is used by the given ConnId.Hdl
 *  \param[in]     ConnId.Hdl         Rx connection handle
 *                 ConnId.PartIdx     Index of the current CanTp-/OS-partition.
 *                                    PartIdx < CanTp_GetSizeOfPartitionIdentifiers();
 *                                    Unused and of value 0u in case CANTP_SUPPORT_MULTI_PARTITION is STD_OFF
 *  \return        pointer to the state data, or NULL_PTR if there is no active channel for this ConnId.Hdl
 *  \context       TASK|ISR2
 *  \reentrant     FALSE
 *  \pre           Expects to be called in a critical section
 *  \pre           Expects that ConnId.Hdl is a valid RxHdl (< CanTp_Cfg_GetNumRxHdls())
 ...
 *  \trace         DSGN-CanTp23317
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(CanTp_RxStatePtrType, CANTP_CODE) CanTp_RxGetActiveChannelPtr(CanTp_ConnIdType ConnId);
#endif
 ...
 *  \param[in]     ConnId.Hdl         Tx connection handle
 *                 ConnId.PartIdx     Index of the current CanTp-/OS-partition.
 *                                    PartIdx < CanTp_GetSizeOfPartitionIdentifiers();
 *                                    Unused and of value 0u in case CANTP_SUPPORT_MULTI_PARTITION is STD_OFF
 *  \param[in]     CanTpTxInfoPtr     Pointer to a PduInfo struct. The SduDataPtr member points to metadata if the
 ...
 *  \reentrant     FALSE
 *  \pre           Expects to be called in a critical section
 *  \pre           Expects that ConnId.Hdl is a valid TxHdl (< CanTp_Cfg_GetNumTxHdls())
 ...
 *  \trace         DSGN-CanTp23317
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(CanTp_TxStatePtrType, CANTP_CODE) CanTp_TxGetFreeChannelPtr(
  CanTp_ConnIdType ConnId,
  P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) CanTpTxInfoPtr);
 ...
 *  \trace         DSGN-CanTp23317
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(CanTp_TxStatePtrType, CANTP_CODE) CanTp_TxGetActiveChannelPtr(CanTp_ConnIdType ConnId);
#endif
 ...
 *  \param[in]     ConnId.Hdl         Depending on Direction, either
                                      - a valid Rx handle, i.e. < CanTp_Cfg_GetNumRxHdls() or
 *                                    - a valid Tx handle, i.e. < CanTp_Cfg_GetNumTxHdls()
 *                 ConnId.PartIdx     Index of the current CanTp-/OS-partition.
 *                                    PartIdx < CanTp_GetSizeOfPartitionIdentifiers();
 *                                    Unused and of value 0u in case CANTP_SUPPORT_MULTI_PARTITION is STD_OFF
 ...
 *  \trace         DSGN-CanTp23320
 *********************************************************************************************************************/
CANTP_LOCAL FUNC(void, CANTP_CODE) CanTp_TxQueueCancel(CanTp_ConnIdType ConnId,
                                                       CanTp_DirectionType Direction);
#endif
                                                             ...
                                                             PduLengthType length)
{
  ...
  /*! No DET assertions because callers are only internal functions that assure correct parameters */
  for (i = 0; i < length; i++)
  {
    /* #10 Return 1u in case at least one byte is different, otherwise return 0u. */
    if(mem2[i] != mem1[i])
    {
      result = 1u;
    }
...
# endif
  ...
  CanTp_InitConnId(connId, 0u, PartIdx);                                                                               /* PRQA S 2982, 2983 */ /* MD_CanTp_RedundantAssignment, MD_CanTp_RedundantAssignment */

# if (CANTP_RX_OPTIMIZATION != CANTP_OPT_ZERO_SDU)                                                                     /* COV_CANTP_ROBUSTNESS_CASE TX */
  /* --- Checks that are done on the Rx N-SDU Config Table --- */
  for (lSduId = 0; lSduId < CanTp_Cfg_GetNumRxSduIds(); lSduId++)
  {
    if  (    (CanTp_IsRxNSduHdlUsedOfRxSduSnv2Hdl(lSduId))                                                                       /* PRQA S 2741 */ /* MD_CanTp_ExpressionAlwaysTrue */
#  if (CANTP_SUPPORT_MULTI_PARTITION == STD_ON)
         && (CanTp_RxGetPartIdxBySduId(lSduId) == PartIdx)
#  endif
       )                                                                                                               /* COV_CANTP_POSTBUILD_DELETED */
    {
      connId.Hdl = CanTp_RxGetSduHdlBySduId(lSduId);                                                                   /* PRQA S 2982 */ /* MD_CanTp_RedundantAssignment */

      /* #20 Assert countermeasure CM_CANTP_RXMAXPAYLOADLENGTH */
      lTmp = CanTp_Util_AssertLessEqual(CanTp_RxCfg_GetMaxPayload(connId), CANTP_MAX_PAYLOAD_LENGTH);
      ...
      if (CanTp_RxCfg_GetTAType(connId) == CANTP_TATYPE_PHYSICAL)                                                      /* PRQA S 2741 */ /* MD_CanTp_ExpressionAlwaysTrue */ /* COV_CANTP_ONLY_PHYSICAL TX */
      {
        lTmp = CanTp_Util_AssertLessEqual(CanTp_RxCfg_GetTxFcConfPduId(connId), (CanTp_Cfg_GetNumTxConfPdus(PartIdx) - (uint32)1));
        if (lTmp == FALSE)                                                                                             /* PRQA S 2992, 2996 */ /* MD_CanTp_CheckCfgExpressionAlwaysFalse, MD_CanTp_CheckCfgOperationAlwaysFalse */ /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
        {
...
#  if (CANTP_RX_HAS_CUSTOM_ADDRESSING == STD_ON)
      /* Addressing format custom requires a callout to UL */
      if (CanTp_RxCfg_GetAddressingFormat(connId) == CANTP_ADDRESS_FORMAT_CUSTOM)
      {
        if (!CanTp_IsUpLayerFctsRxFctPtrUsedOfRxSduCfg(connId.Hdl, connId.PartIdx))                                    /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
        {
          CanTp_DetReportError(CANTP_SID_INIT, CANTP_E_PARAM_CONFIG)
          lResult = FALSE;                                                                                             /* PRQA S 2880 */ /* MD_MSR_Unreachable */
        }
...
# endif /* # if (CANTP_RX_OPTIMIZATION != CANTP_OPT_ZERO_SDU) */

# if (CANTP_TX_OPTIMIZATION != CANTP_OPT_ZERO_SDU)                                                                     /* COV_CANTP_ROBUSTNESS_CASE TX */
  /* --- Checks that are done on the Tx N-SDU Config Table --- */
  for (lSduId = 0; lSduId < CanTp_Cfg_GetNumTxSduIds(); lSduId++)
  {
    if (    (CanTp_IsTxNSduHdlUsedOfTxSduSnv2Hdl(lSduId))                                                                        /* PRQA S 2741 */ /* MD_CanTp_ExpressionAlwaysTrue */
#  if (CANTP_SUPPORT_MULTI_PARTITION == STD_ON)
         && (CanTp_TxGetPartIdxBySduId(lSduId) == PartIdx)
#  endif
       )                                                                                                               /* COV_CANTP_POSTBUILD_DELETED */
    {
      connId.Hdl = CanTp_TxGetSduHdlBySduId(lSduId);                                                                   /* PRQA S 2983 */ /* MD_CanTp_RedundantAssignment */

      /* #50 Assert countermeasure CM_CANTP_TXMAXPAYLOADLENGTH */
      lTmp = CanTp_Util_AssertLessEqual(CanTp_TxCfg_GetMaxPayload(connId), CANTP_MAX_PAYLOAD_LENGTH);
      if (lTmp == FALSE)                                                                                               /* PRQA S 2992, 2996 */ /* MD_CanTp_CheckCfgExpressionAlwaysFalse, MD_CanTp_CheckCfgOperationAlwaysFalse */ /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
      {
        lResult = FALSE;                                                                                               /* PRQA S 2880 */ /* MD_MSR_Unreachable */
      }
      ...
      lTmp = CanTp_Util_AssertLessEqual(CanTp_TxCfg_GetTxConfPduId(connId), (CanTp_Cfg_GetNumTxConfPdus(PartIdx) - (uint32)1));
      if (lTmp == FALSE)                                                                                               /* PRQA S 2992, 2996 */ /* MD_CanTp_CheckCfgExpressionAlwaysFalse, MD_CanTp_CheckCfgOperationAlwaysFalse */ /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
      {
        lResult = FALSE;                                                                                               /* PRQA S 2880 */ /* MD_MSR_Unreachable */
      }
      ...
      switch (CanTp_TxCfg_GetAddressingFormat(connId))
      {
#   if ((CANTP_SUPPORT_STANDARD_ADDRESSING == STD_ON) || (CANTP_SUPPORT_NORMALFIXED_ADDRESSING == STD_ON) || (CANTP_SUPPORT_CUSTOM_ADDRESSING == STD_ON))
#    if (CANTP_SUPPORT_STANDARD_ADDRESSING == STD_ON)
        case CANTP_ADDRESS_FORMAT_STANDARD:
#    endif
#    if (CANTP_SUPPORT_NORMALFIXED_ADDRESSING == STD_ON)
        case CANTP_ADDRESS_FORMAT_NORMALFIXED:
#    endif
#    if (CANTP_SUPPORT_CUSTOM_ADDRESSING == STD_ON)
        case CANTP_ADDRESS_FORMAT_CUSTOM:
#    endif
          lTmp = CanTp_Util_AssertLessEqual((CanTp_TxCfg_GetMaxPayload(connId) + (uint32)1), CANTP_MAX_FRAME_LENGTH);
          if (lTmp == FALSE)                                                                                           /* PRQA S 2992, 2996 */ /* MD_CanTp_CheckCfgExpressionAlwaysFalse, MD_CanTp_CheckCfgOperationAlwaysFalse */ /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
          {
            lResult = FALSE;                                                                                           /* PRQA S 2880 */ /* MD_MSR_Unreachable */
          }
...
#   if ((CANTP_SUPPORT_EXTENDED_ADDRESSING == STD_ON) || (CANTP_SUPPORT_MIXED11_ADDRESSING == STD_ON) || (CANTP_SUPPORT_MIXED29_ADDRESSING== STD_ON))
#    if (CANTP_SUPPORT_EXTENDED_ADDRESSING == STD_ON)
        case CANTP_ADDRESS_FORMAT_EXTENDED:
#    endif
#    if (CANTP_SUPPORT_MIXED11_ADDRESSING == STD_ON)
        case CANTP_ADDRESS_FORMAT_MIXED11:
#    endif
#    if (CANTP_SUPPORT_MIXED29_ADDRESSING == STD_ON)
        case CANTP_ADDRESS_FORMAT_MIXED29:
#    endif
          lTmp = CanTp_Util_AssertLessEqual((CanTp_TxCfg_GetMaxPayload(connId) + (uint32)2), CANTP_MAX_FRAME_LENGTH);
          if (lTmp == FALSE)                                                                                           /* PRQA S 2992, 2996 */ /* MD_CanTp_CheckCfgExpressionAlwaysFalse, MD_CanTp_CheckCfgOperationAlwaysFalse */ /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
          {
            lResult = FALSE;                                                                                           /* PRQA S 2880 */ /* MD_MSR_Unreachable */
          }
...
#  if (CANTP_TX_HAS_CUSTOM_ADDRESSING == STD_ON)
      /* Addressing format custom requires a callout to UL */
      if (CanTp_TxCfg_GetAddressingFormat(connId) == CANTP_ADDRESS_FORMAT_CUSTOM)
      {
        if(!CanTp_IsUpLayerFctsTxFctPtrUsedOfTxSduCfg(connId.Hdl, connId.PartIdx))                                               /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
        {
          CanTp_DetReportError(CANTP_SID_INIT, CANTP_E_PARAM_CONFIG)
          lResult = FALSE;                                                                                             /* PRQA S 2880 */ /* MD_MSR_Unreachable */
        }
...
# if (CANTP_CONSTANT_BS == STD_ON)
  lTmp = CanTp_Util_AssertLessEqual(CanTp_GetSizeOfRxSduCfg(PartIdx), CanTp_GetSizeOfCalcBS(PartIdx));
  if (lTmp == FALSE)                                                                                                   /* PRQA S 2992, 2996 */ /* MD_CanTp_CheckCfgExpressionAlwaysFalse, MD_CanTp_CheckCfgOperationAlwaysFalse */ /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
  {
...
# if (CANTP_ENABLE_CHANGE_PARAM == STD_ON)
  lTmp = CanTp_Util_AssertLessEqual(CanTp_GetSizeOfRxSduCfg(PartIdx), CanTp_GetSizeOfDynFCParameters(PartIdx));
  if (lTmp == FALSE)                                                                                                   /* PRQA S 2992, 2996 */ /* MD_CanTp_CheckCfgExpressionAlwaysFalse, MD_CanTp_CheckCfgOperationAlwaysFalse */ /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
  {
    lResult = FALSE;                                                                                                   /* PRQA S 2880, 2982 */ /* MD_MSR_Unreachable, MD_CanTp_RedundantAssignment */
  }
# endif
# if (CANTP_DYN_CHANNEL_ASSIGNMENT == STD_ON)
  lTmp = CanTp_Util_AssertLessEqual(CanTp_GetSizeOfRxSduCfg(PartIdx), CanTp_GetSizeOfRxChannelMap(PartIdx));
  if (lTmp == FALSE)                                                                                                   /* PRQA S 2992, 2996 */ /* MD_CanTp_CheckCfgExpressionAlwaysFalse, MD_CanTp_CheckCfgOperationAlwaysFalse */ /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
  {
    lResult = FALSE;                                                                                                   /* PRQA S 2880 */ /* MD_MSR_Unreachable */
  }
# else
  lTmp = CanTp_Util_AssertLessEqual(CanTp_GetSizeOfRxSduCfg(PartIdx), CanTp_GetSizeOfRxState(PartIdx));
  if (lTmp == FALSE)                                                                                                   /* PRQA S 2992, 2996 */ /* MD_CanTp_CheckCfgExpressionAlwaysFalse, MD_CanTp_CheckCfgOperationAlwaysFalse */ /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
  {
    lResult = FALSE;                                                                                                   /* PRQA S 2880 */ /* MD_MSR_Unreachable */
  }
...
# if (CANTP_DYN_CHANNEL_ASSIGNMENT == STD_ON)
  lTmp = CanTp_Util_AssertLessEqual(CanTp_GetSizeOfTxSduCfg(PartIdx), CanTp_GetSizeOfTxChannelMap(PartIdx));
  if (lTmp == FALSE)                                                                                                   /* PRQA S 2992, 2996 */ /* MD_CanTp_CheckCfgExpressionAlwaysFalse, MD_CanTp_CheckCfgOperationAlwaysFalse */ /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
  {
    lResult = FALSE;                                                                                                   /* PRQA S 2880 */ /* MD_MSR_Unreachable */
  }
# else
  lTmp = CanTp_Util_AssertLessEqual(CanTp_GetSizeOfTxSduCfg(PartIdx), CanTp_GetSizeOfTxState(PartIdx));
  if (lTmp == FALSE)                                                                                                   /* PRQA S 2992, 2996 */ /* MD_CanTp_CheckCfgExpressionAlwaysFalse, MD_CanTp_CheckCfgOperationAlwaysFalse */ /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
  {
    lResult = FALSE;                                                                                                   /* PRQA S 2880 */ /* MD_MSR_Unreachable */
  }
...
# if (CANTP_TRANSMIT_QUEUE == STD_ON)
  lTmp = CanTp_Util_AssertLessEqual(CanTp_GetSizeOfTxSemaphores(PartIdx), CanTp_GetSizeOfTxQueue(PartIdx));
  if (lTmp == FALSE)                                                                                                   /* PRQA S 2992, 2996 */ /* MD_CanTp_CheckCfgExpressionAlwaysFalse, MD_CanTp_CheckCfgOperationAlwaysFalse */ /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
  {
    lResult = FALSE;                                                                                                   /* PRQA S 2880 */ /* MD_MSR_Unreachable */
  }
...
# if (CANTP_LOLAYERFCTS == STD_ON)
  for (lLoLayerIdx = 0; lLoLayerIdx < CanTp_GetSizeOfLoLayerFcts(PartIdx); lLoLayerIdx++)
  {
    if (CanTp_GetTransmitFctOfLoLayerFcts(lLoLayerIdx, PartIdx) == NULL_PTR)
    {
...
# error "CANTP_MAX_PDU_METADATA_LENGTH must be 2 if generic connections and mixed29/normal fixed addressing are enabled."
  ...
  CANTP_DUMMY_STATEMENT(PartIdx);                                                                                      /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */
...
#endif
#if (    (CANTP_SUPPORT_STANDARD_ADDRESSING == STD_ON) \
      || (CANTP_SUPPORT_NORMALFIXED_ADDRESSING == STD_ON) \
      || (CANTP_SUPPORT_CUSTOM_ADDRESSING == STD_ON) \
    )
      lFrameType = (pPayloadInfo->SduDataPtr[0] & CANTP_PCI_TYPE_MASK) >> 4;
      break;
#endif
#if (CANTP_SUPPORT_EXTENDED_ADDRESSING == STD_ON)
    case CANTP_ADDRESS_FORMAT_EXTENDED:
...
#if ((CANTP_SUPPORT_EXTENDED_ADDRESSING == STD_ON) || (CANTP_SUPPORT_MIXED11_ADDRESSING == STD_ON) || (CANTP_SUPPORT_MIXED29_ADDRESSING == STD_ON))
      lFrameType = (pPayloadInfo->SduDataPtr[1] & CANTP_PCI_TYPE_MASK) >> 4;
      break;
#endif
    default:
      lFrameType = CANTP_FRAME_INVALID;
      break;
  }

  if (lFrameType > CANTP_FRAME_FC)
  {
    lFrameType = CANTP_FRAME_INVALID;
  }
                                                                                                     ...
                                                                                                     P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA) pFcPduMetadata)
{
  ...
  CANTP_DUMMY_STATEMENT(pDataPduPayload);                                                                              /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */
  CANTP_DUMMY_STATEMENT(ConnId);                                                                                       /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */

  switch (CanTp_RxCfg_GetAddressingFormat(ConnId))
  {
#  if ((CANTP_RX_HAS_EXTENDED_ADDRESSING == STD_ON) || (CANTP_RX_HAS_MIXED11_ADDRESSING == STD_ON))
    case CANTP_ADDRESS_FORMAT_EXTENDED:
    case CANTP_ADDRESS_FORMAT_MIXED11:
      pSduMetadata[0] = pDataPduPayload->SduDataPtr[0];                                                                /* SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PSDUMETADATA */
      *pSduMetadataLength = 1;                                                                                         /* SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PSDUMETADATALENGTH */
      break;
#  endif

#  if (CANTP_RX_HAS_NORMALFIXED_ADDRESSING == STD_ON)
    case CANTP_ADDRESS_FORMAT_NORMALFIXED:
      pSduMetadata[0] = pDataPduMetadata->SduDataPtr[0];                                                               /* SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PSDUMETADATA */
      pSduMetadata[1] = pDataPduMetadata->SduDataPtr[1];                                                               /* SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PSDUMETADATA */
      *pSduMetadataLength = 2;                                                                                         /* SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PSDUMETADATALENGTH */
      
      pFcPduMetadata[0] = pDataPduMetadata->SduDataPtr[1];                                                             /* SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PFCPDUMETADATA */
      pFcPduMetadata[1] = pDataPduMetadata->SduDataPtr[0];                                                             /* SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PFCPDUMETADATA */
      break;
#  endif
...
#  if (CANTP_RX_HAS_MIXED29_ADDRESSING == STD_ON)
    case CANTP_ADDRESS_FORMAT_MIXED29:
      pSduMetadata[0] = pDataPduMetadata->SduDataPtr[0];                                                               /* SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PSDUMETADATA */
      pSduMetadata[1] = pDataPduMetadata->SduDataPtr[1];                                                               /* SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PSDUMETADATA */
      pSduMetadata[2] = pDataPduPayload->SduDataPtr[0];                                                                /* SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PSDUMETADATA */
      *pSduMetadataLength = 3;                                                                                         /* SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PSDUMETADATALENGTH */
...
#  if (CANTP_RX_HAS_CUSTOM_ADDRESSING == STD_ON)
    /* In case of custom addressing, let the upper layer assemble the NSdu metadata info.
       Otherwise assemble the info according to the configured addressing format. */
    case CANTP_ADDRESS_FORMAT_CUSTOM:
    {
      /* Declaration of parameters of the callout */
      /* PduInfoType lNPduPayload; */       /* Parameter 1 */
      /* PduInfoType lNPduMetaData; */      /* Parameter 2 */
      PduInfoType lResMetaDataOfNSdu;       /* Parameter 3 */
      /* uint8*   lResMetaDataOfFcNPdu; */ /* Parameter 4 */
      PduInfoType lResInPayloadNAiOfFcNPdu; /* Parameter 5 */

      /* Initialization of parameters */
      /* Parameter 1: lNPduPayload - This parameter is equivalent to parameter PduInfoPtr of function CanTp_RxIndication() and to the PduInfoType-variable lPayload prepared in CanTp_CanIfRxIndication() which is provided as parameter pDataPduPayload, here! */
      /* Parameter 2: lNPduMetaData - Contents for this parameter are derived from parameter PduInfoPtr of function CanTp_RxIndication() into PduInfoType-variable lMetadata in CanTp_CanIfRxIndication() which is provided as parameter pDataPduMetadata, here! */
      /* Parameter 3:  */
      lResMetaDataOfNSdu.SduDataPtr       = pSduMetadata;
      lResMetaDataOfNSdu.SduLength        = CanTp_RxCfg_GetSduMetadataLength(ConnId); /* <= CANTP_MAX_SDU_METADATA_LENGTH */
      /* Parameter 4: lResMetaDataOfFcNPdu      - Pointer pFcPduMetadata[] is used as value for this uint8* parameter */
      /* Parameter 5: lResInPayloadNAiOfFcNPdu. - CanTp writes the data provided, here, into the payload of all FC-NPdus of this reception.
                                                  'In' -functionality: SduDataPtr points to CanTp memory. SduLength states the length of that memory area. 
                                                  In the byte-array the UL provides the payload part of N_AI used to assemble FC-NPdus sent in this reception.
                                                  'Out'-functionality: Data that was actually 'used' in the given CanTp memory.
                                                  Due to the unified treatment of N_AI-parts in NPdu payload, independently of PCI bytes,
                                                  CanTp knows from the changed SduLength in the 'Out'-context, how many bytes of the payload of the incoming Data-NPdu are part of N_AI.
                                                  The remaining bytes of the payload are TP-payload, then. */
      lResInPayloadNAiOfFcNPdu.SduDataPtr = (SduDataPtrType)NULL_PTR; /* Unsupported, thus NULL_PTR. */
      lResInPayloadNAiOfFcNPdu.SduLength  = 0u; /* Unsupported, thus 0u, always. */;

      /* --- Actually conduct the callout --- */
      lReturnValue = CanTp_RxCfg_GetCustomAddressingCallout(ConnId)(pDataPduPayload, /* P1: Payload as it comes from CanIf. */ /* SBSW_CANTP_CUSTOMADDRCALLOUT_FCTPTR */
                                                                    pDataPduMetadata, /* P2: MetaData as it comes from CanIf. */
                                                                    &lResMetaDataOfNSdu, /* P3: MetaData to be used for PduR_CanTpStartOfReception(). Freely defined by UL. */
                                                                    pFcPduMetadata, /* P4: MetaData to be used for CanIf_Transmit() of FC NPdus sent in this reception. Length is that of pDataPduMetadata->SduLength */
                                                                    &lResInPayloadNAiOfFcNPdu /* P5: Payload N_AI used to assemble FC NPdus in this reception. */
                                                                   );

      if (lReturnValue == E_OK)
      {
        /* Post-processing of the results coming from the Upper Layer */
        /* Parameter 1: No post-processing req'd because it's purely an In-parameter */
        /* Parameter 2: No post-processing req'd because it's purely an In-parameter */
        /* Parameter 3: Read back the length of the SduMetadata as defined by the functions interface description,
                        even though NSdu-MetaData-length is pre-configured and shall not change. */
        *pSduMetadataLength = CanTp_RxCfg_GetSduMetadataLength(ConnId);                                                /* SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PSDUMETADATALENGTH */
        /* Parameter 4: For FC-NPdu-Metadata the length is the same as for Data-NPdu-Payload, in P2, above. */
        /* Parameter 5: Do not read back the Out-parameter that defines the length of the N_AI in both,
                        data and FC NPdus, because it is unsupported, at the moment. */
      }
                                                                                      ...
                                                                                      P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pMetadataInfo)
{
  ...
  boolean matchResult = TRUE;                                                                                          /* PRQA S 2981 */ /* MD_MSR_RetVal */
  uint8 lSduMetadataLength;
  uint8 lSduMetadata[CANTP_MAX_SDU_METADATA_LENGTH] = { 0 };

# if (CANTP_MAX_PDU_METADATA_LENGTH != 0)
  uint8 lFcPduMetadata[CANTP_MAX_PDU_METADATA_LENGTH];
# endif

  /* #10 Assemble the Rx SDU metadata (and fc-PDU metadata, if necessary) based on the configuration of the passed SDU, data-PDU payload and data-PDU metadata. */
  lReturnValue = CanTp_Util_RxAssembleNecessaryMetadata(pTpRxState->ConnId,                                            /* SBSW_CANTP_RXASSEMBLESDUMETADATA_CALL_PTRS */
                                                        pPayloadInfo,
                                                        pMetadataInfo,
                                                        lSduMetadata,
                                                        &lSduMetadataLength,
  ...
  /* #20 If the metadata could be assembled:
   *       If the assembled metadata doesn't match the metadata assigned to the passed RxState struct, return FALSE.
...
# endif
  {
    matchResult = (CanTp_Util_MemCmp(lSduMetadata, pTpRxState->SduMetadata, pTpRxState->SduMetadataLength) == 1u) ? FALSE : TRUE; /* SBSW_CANTP_MEMCMP_CALL_PTRS */
  }
# if (CANTP_RX_HAS_CUSTOM_ADDRESSING == STD_ON)
  else
  {
...
# endif
  ...
  CANTP_DUMMY_STATEMENT(lReturnValue);                                                                                 /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */
...
CANTP_LOCAL_INLINE FUNC(boolean, CANTP_CODE) CanTp_Util_TxSduMetadataFromFcPduMatch(P2CONST(CanTp_TxStateType, AUTOMATIC, CANTP_APPL_DATA) pTpTxState,
                                                                                    ...
                                                                                    P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pMetadataInfo)
{
  boolean result = TRUE;                                                                                               /* PRQA S 2981 */ /* MD_MSR_RetVal */
...
# endif
  ...
  switch (CanTp_TxCfg_GetAddressingFormat(pTpTxState->ConnId))
  {
...
# if (CANTP_TX_HAS_EXTENDED_ADDRESSING == STD_ON)
   ...
   */
    case CANTP_ADDRESS_FORMAT_EXTENDED:
      if (pPayloadInfo->SduDataPtr[0] != CanTp_TxCfg_GetRxFcAddress(pTpTxState->ConnId))
      {
        result = FALSE;
      }
      break;
...
# if (CANTP_TX_HAS_NORMALFIXED_ADDRESSING == STD_ON)
   ...
   */
    case CANTP_ADDRESS_FORMAT_NORMALFIXED:
      ...
      result = (CanTp_Util_MemCmp(lSduMetadata, pTpTxState->SduMetadata, pTpTxState->SduMetadataLength) == 1u) ? FALSE : TRUE; /* SBSW_CANTP_MEMCMP_CALL_PTRS */
      break;
# endif
...
# if (CANTP_TX_HAS_CUSTOM_ADDRESSING == STD_ON)
    /* #60 Custom Addressing:
     *       If the metadata of the received fc-PDU doesn't match the fc-PDU metadata assigned to the passed TxState:
     *         FALSE will be returned.
   */
    case CANTP_ADDRESS_FORMAT_CUSTOM:
      if(CanTp_Util_MemCmp(pTpTxState->FcPduMetadata, pMetadataInfo->SduDataPtr, pMetadataInfo->SduLength) != 0u)      /* SBSW_CANTP_MEMCMP_CALL_PTRS */
      {
        result = FALSE;
      }
      break;
                                                                                        ...
                                                                                        P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pMetadataInfo)
{
   ...
   *     Assemble and store the Rx SDU metadata based on the configuration of the passed SDU, data-PDU payload and data-PDU metadata. */
  if (CanTp_RxCfg_IsGenericConnection(pTpRxState->ConnId))
  {
    lReturnValue = CanTp_Util_RxAssembleNecessaryMetadata(pTpRxState->ConnId,                                          /* SBSW_CANTP_RXASSEMBLESDUMETADATA_CALL_PTRS */
                                                          pPayloadInfo,
                                                          pMetadataInfo,
                                                          pTpRxState->SduMetadata,
                                                          &pTpRxState->SduMetadataLength,
                                                          pTpRxState->FcPduMetadata);
  }
                                                                                        ...
                                                                                        P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) CanTpTxInfoPtr)
{
  ...
  if (CanTp_TxCfg_GetSduMetadataLength(pTpTxState->ConnId) != 0u)
  {
    /* #15 Store the metadata passed by the upper layer. */
    pTpTxState->SduMetadataLength = CanTp_TxCfg_GetSduMetadataLength(pTpTxState->ConnId);                              /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
...
# if (CANTP_MAX_PDU_METADATA_LENGTH != 0)
    switch (CanTp_TxCfg_GetAddressingFormat(pTpTxState->ConnId))
    {
       ...
       *       No assembly of metadata takes place. */
...
#  endif 
      ...
      /* #50 Custom Addressing:
       *       Retrieve the metadata of both data-PDU and fc-PDU from the upper layer. */
#  if (CANTP_TX_HAS_CUSTOM_ADDRESSING == STD_ON)
      case CANTP_ADDRESS_FORMAT_CUSTOM:
      {
        /* Declaration of return value and parameters of the callout */
        
        PduInfoType lMetaDataOfNSdu;                                  /* Parameter 1 */
        PduInfoType lResMetaDataOfDataNPdu;                           /* Parameter 2 */
        /* uint8* FcPduMetadata - future member of pTpTxState */      /* Parameter 3 */
        PduInfoType lResInPayloadNAiOfDataNPdu;                       /* Parameter 4 */
        /* uint8* InPayloadNAiFcNPdu - future member of pTpTxState */ /* Parameter 5 */
      
        /* Initialization of parameters */
        /* Parameter 1: MetaDataOfNSduPtr - Offer known metadata to the callout: */
        lMetaDataOfNSdu.SduDataPtr = pTpTxState->SduMetadata;
        lMetaDataOfNSdu.SduLength  = pTpTxState->SduMetadataLength;
        /* Parameter 2: ResMetaDataOfDataNPduOutPtr - Offer possibility to the callout to provide N_AI in the payload of data NPdus: */
        lResMetaDataOfDataNPdu.SduDataPtr = pTpTxState->DataPduMetadata;
        lResMetaDataOfDataNPdu.SduLength  = CanTp_TxCfg_GetPduMetadataLength(pTpTxState->ConnId);
        /* Parameter 3: No initialization req'd. */
        /* Parameter 4: Made unusable, because Custom Addressing does not yet support InPayload address extensions. */
        lResInPayloadNAiOfDataNPdu.SduDataPtr =  NULL_PTR;
        lResInPayloadNAiOfDataNPdu.SduLength = 0u;
        /* Parameter 5: No initialization req'd. */

        /* --- Actually conduct the callout --- */
        lReturnValue = CanTp_TxCfg_GetCustomAddressingCallout(pTpTxState->ConnId)(&lMetaDataOfNSdu, /* P1: Known metadata of NSdu from CanTp_Transmit() call. */ /* SBSW_CANTP_CUSTOMADDRCALLOUT_FCTPTR */
                                                                                  &lResMetaDataOfDataNPdu, /* P2: MetaData for CanIf_Transmit() calls: CanTp intermediately stores this in order to assemble Data Frames (SF, FF, CF) */
                                                                                  pTpTxState->FcPduMetadata, /* P3: MetaData to match FCs received in CanTp_RxIndication() calls */
                                                                                  &lResInPayloadNAiOfDataNPdu, /* P4: Payload N_AI used to assemble Data NPdus */
                                                                                  (P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA))NULL_PTR /* P5: Currently unsupported: CanTp could store and use this payload fragment to match the N_AI in the payload of a received FC NPdu. (Refer to e.g. "Extended Addressing"). Would be: pTpTxState->InPayloadNAiFcNPdu */
                                                                                 );
        /* Post-processing of the results coming from the Upper Layer */
        /* Parameter 1: No post-processing req'd because it's purely an In-parameter */
        /* Parameter 2: No post-processing req'd because SduLength is a [configured] in parameter only */
        /* Parameter 3: No post-processing req'd, or rather: For FC-NPdu-MetaData the length is the same as the [configured] Data-NPdu-MetaData, above. */
        /* Parameter 4: Do not read back Out-parameter, pTpTxState->InPayloadNAiLengthNPdu, 
                        because it is unsupported, at the moment. */
        /* Parameter 5: No post-processing req'd, or rather: For FC-NPdu-Payload the length is the same as for Data-NPdu-Payload, above. (Both unsupported, currently) */
      }
      break;
#  endif
      default:                                                                                                         /* COV_CANTP_DEFAULT_BRANCH */
...
#endif /* #if (CANTP_GENERIC_CONNECTIONS == STD_ON) */
 ...
 *
 **********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(uint8, CANTP_CODE) CanTp_Util_RxGetFCInPayloadAddressInfo(
  P2CONST(CanTp_RxStateType, AUTOMATIC, CANTP_APPL_DATA) pTpRxState)
{
  uint8 result;
...
# if (CANTP_RX_HAS_EXTENDED_ADDRESSING)
      && (CanTp_RxCfg_GetAddressingFormat(pTpRxState->ConnId) != CANTP_ADDRESS_FORMAT_EXTENDED)
# endif
     )
  {
  ...
  {
    result = CanTp_RxCfg_GetTxFcAddress(pTpRxState->ConnId);
  }
 ...
 */
  ...
  {
    result = CanTp_TxCfg_GetTxAddress(pTpTxState->ConnId);
  }
 ...
 *
 */
CANTP_LOCAL_INLINE FUNC(CanTp_PartIdxType, CANTP_CODE) CanTp_Util_GetPartIdx(uint8* ErrorIdPtr) /* PRQA S 3673 */ /* MD_CanTp_3673 */
{
#if(CANTP_SUPPORT_MULTI_PARTITION == STD_ON)
  /* #10 For MultiPartition configurations: return the CSL memory section index which is mapped to this OsApplication. */
  CanTp_PartIdxType retVal = 0u;
  /* #20 CSL provides its partIdx for free, in case a partition-independent item indirects to a partition-dependent item. */
  CanTp_PartIdxType partIdx = CanTp_GetInitializedPartitionIdxOfOsApplication2PartitionIdx(GetApplicationID());

  /* #30 Issue an error in case this CanTp is not assigned to this osApplication or is not correctly initialized. */
  if(partIdx == CANTP_NO_INITIALIZEDPARTITIONIDXOFOSAPPLICATION2PARTITIONIDX)
  {
    *ErrorIdPtr = CANTP_E_OSAPP2MEMMAPPING;                                                                            /* SBSW_CANTP_INTERNAL_CALL_ERROR_ID_PTR */
  }
  else
  {
    retVal = partIdx;
  ...
  return retVal;
#else
  CANTP_DUMMY_STATEMENT(ErrorIdPtr);                                                                                   /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */
  /* #40 For SinglePartition configurations: Return a constant which is the only/shared memory section, by default. */
  return CanTpSinglePartitionMemory;
#endif
} /* CanTp_Util_GetPartIdx() */
...
CANTP_LOCAL_INLINE FUNC(CanTp_PartIdxType, CANTP_CODE) CanTp_Util_GetPartIdxOfRxPduId(PduIdType PduId,
                                                                                      uint8* ErrorIdPtr)
{
  CanTp_PartIdxType partIdx = 0u;

# if (CANTP_DEV_ERROR_DETECT == STD_ON)                                                                                /* COV_CANTP_DET_CHECK TX */
  if(PduId < CanTp_GetSizeOfRxPduMap())
# endif
  {
    /* #10 Determine the partIdx based on the partition-independent RxPduId depending on ... */
    /* #20 ... the RxPdu being used by an RxNSdu or ... */
    if(CanTp_GetRxPdu2RxSduPartitionIdxOfRxPduMap(PduId) != CANTP_NO_RXPDU2RXSDUPARTITIONIDXOFRXPDUMAP)                
    {
      partIdx = CanTp_GetRxPdu2RxSduPartitionIdxOfRxPduMap(PduId);                                                     
    }
    /* #30 ... the RxPdu being used by an TxNSdu or ... */
    else if(CanTp_GetRxPdu2TxSduPartitionIdxOfRxPduMap(PduId) != CANTP_NO_RXPDU2TXSDUPARTITIONIDXOFRXPDUMAP)           /* PRQA S 2741 */ /* MD_CanTp_ExpressionAlwaysTrue */
    {
      partIdx = CanTp_GetRxPdu2TxSduPartitionIdxOfRxPduMap(PduId);
    }
    /* #40 ... otherwise, indicate an error */
    else
    { 
      *ErrorIdPtr = CANTP_E_INVALID_RX_ID;                                                                             /* PRQA S 2880 */ /* MD_MSR_Unreachable */ /* SBSW_CANTP_INTERNAL_CALL_ERROR_ID_PTR */
    }
  }
  
  CANTP_DUMMY_STATEMENT(PduId);                                                                                        /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */
...
# if (CANTP_DEV_ERROR_DETECT == STD_ON)                                                                                /* COV_CANTP_DET_CHECK TX */
  if(PduId < CanTp_GetSizeOfTxPduMap())                                                                                /* PRQA S 2004 */ /* MD_MSR_EmptyClause */
# endif
  {
    /* #10 Determine the partIdx based on the partition-independent TxPduId depending on ... */
    /* #20 ... the TxPdu being used by an TxNSdu or ... */
    if(CanTp_GetTxPdu2TxSduPartitionIdxOfTxPduMap(PduId) != CANTP_NO_TXPDU2TXSDUPARTITIONIDXOFTXPDUMAP)                /* PRQA S 2741 */ /* MD_CanTp_ExpressionAlwaysTrue */ /* COV_CANTP_ERROR_ID_VALID */
    {
      partIdx = CanTp_GetTxPdu2TxSduPartitionIdxOfTxPduMap(PduId);                                                     
    }
    /* #30 ... the TxPdu being used by an RxNSdu or ... */
    else if(CanTp_GetTxPdu2RxSduPartitionIdxOfTxPduMap(PduId) != CANTP_NO_TXPDU2RXSDUPARTITIONIDXOFTXPDUMAP)           /* PRQA S 2880 */ /* MD_MSR_Unreachable */ /* COV_CANTP_ERROR_ID_VALID */
    {
      partIdx = CanTp_GetTxPdu2RxSduPartitionIdxOfTxPduMap(PduId);                                                     
    }
    /* #40 ... otherwise, indicate an error */
    else
    {
      *ErrorIdPtr = CANTP_E_INVALID_TX_ID;                                                                             /* SBSW_CANTP_INTERNAL_CALL_ERROR_ID_PTR */
    }
  }
...
#else /* #if(CANTP_SUPPORT_MULTI_PARTITION == STD_ON) */
# define CanTp_Util_GetPartIdxOfRxPduId(RxPduId, ErrorIdPtr) CanTp_Util_GetPartIdx(ErrorIdPtr)
# define CanTp_Util_GetPartIdxOfTxPduId(TxPduId, ErrorIdPtr) CanTp_Util_GetPartIdx(ErrorIdPtr)
#endif
                                                                    ...
                                                                    P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pPdu)
{
  ...
  lAddressingOffset = CanTp_Util_GetAdrOffset(CanTp_RxCfg_GetAddressingFormat(pTpRxState->ConnId));
  ...
  lMinPayloadLength = 1;
  lMaxPayloadLength = CanTp_RxCfg_GetMaxPayloadSF(pTpRxState->ConnId);
...
# if (CANTP_RX_HAS_CANFD == STD_ON)
  ...
  if (pPdu->SduLength > CANTP_CAN20_FRAME_LENGTH)
  {
    if (lReceivedDataLength == 0u)
    {
      ...
      lMinPayloadLength = (uint8)(CanTp_RxCfg_GetMaxPayloadSF(pTpRxState->ConnId) + 1u);
      lMaxPayloadLength = (uint8)CanTp_RxCfg_GetMaxPayloadLSF(pTpRxState->ConnId);
    }
    else
    {
...
# endif
  ...
  if ( (lReceivedDataLength >= lMinPayloadLength) && /* min SF DL check */
       ...
       (CanTp_IsCanDlValid(pPdu->SduLength, CanTp_Util_GetMinFittingCanDl(lPayloadOffset + lReceivedDataLength, CanTp_RxCfg_HasPaddingActivation(pTpRxState->ConnId)))) ) /* CAN_DL check */
  {
    ...
    CanTp_ApplRxSFIndication(CanTp_RxCfg_GetPduRRxSduId(pTpRxState->ConnId), pPdu)                                     /* SBSW_CANTP_APPLNOTIFICATION_CALL_PDUINFO */
    /* prepare for initial buffer request */
    pTpRxState->ProvidedBufferSize = 0;                                                                                /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */

# if (CANTP_GENERIC_CONNECTIONS == STD_ON)
    if (CanTp_RxCfg_GetSduMetadataLength(pTpRxState->ConnId) != 0u)
    {
      PduInfoType lPduInfo;
      lPduInfo.SduDataPtr = pTpRxState->SduMetadata;
      lPduInfo.SduLength  = CanTp_RxCfg_GetSduMetadataLength(pTpRxState->ConnId);

      pTpRxState->BufferState = CanTp_PduRStartOfReception(CanTp_RxCfg_GetPduRRxSduId(pTpRxState->ConnId), /* CanTpRxPduId */ /* SBSW_CANTP_PDURSTARTOFRECEPTION_CALL_RXSTATE */ /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
                                                           &lPduInfo, /* MetaDataPtr */
...
# endif
    ...
    switch(pTpRxState->BufferState)
    {
      ...
      case CANTP_BUFREQ_E_BUSY:
        ...
        pTpRxState->Timer = CanTp_RxCfg_GetNBr(pTpRxState->ConnId); /* CANTP166 */                                     /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
...
# endif
      ...
      case CANTP_BUFREQ_E_OVFL:
        /* #90  Reset connection without a DET error. */
        CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_NO_ERROR, CANTP_RESET_ALL);                           /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
        break;
      default:
        /* #100  Any other (i.e. invalid) buffer state: Reset connection with DET error. */
 ...
 *
 **********************************************************************************************************************/
/* PRQA S 6030, 6050 3 */ /* MD_MSR_STCYC, MD_MSR_STCAL */
...
# if (CANTP_RX_HAS_TATYPE_FUNCTIONAL == STD_ON)
  if (CANTP_TATYPE_FUNCTIONAL == CanTp_RxCfg_GetTAType(pTpRxState->ConnId))
  {
...
#  endif
      ...
      lMaxPayloadLen        = CanTp_RxCfg_GetMaxPayloadLFF(pTpRxState->ConnId);
...
# endif
    {
      ...
      lDataLengthLowerLimit = (uint16)(CanTp_RxCfg_GetMaxSFDataLength(pTpRxState->ConnId, pPdu->SduLength)); /* FF data length must be greater than max single frame length */
      lMaxPayloadLen        = CanTp_RxCfg_GetMaxPayloadFF(pTpRxState->ConnId);
        ...
        (pPdu->SduLength     >= CANTP_CAN20_FRAME_LENGTH))                            /* CAN min DLC check */
    {
      ...
      pTpRxState->WFTCounter = CanTp_RxCfg_GetWFTmax(pTpRxState->ConnId);                                              /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
      ...
      pTpRxState->InitialDL = (uint8)pPdu->SduLength; /* FF DLC is needed for CF length consistency check (SWS_CanTp_00350) */ /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
      CanTp_Util_MemCpy((P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA)) (&pTpRxState->Payload[0]),                          /* SBSW_CANTP_MEMCPY_CALL_PAYLOAD */
                        (P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA)) (&pPdu->SduDataPtr[lPayloadOffset]),
                        (pTpRxState->PayloadLength) );

      CanTp_ApplRxFFIndication(CanTp_RxCfg_GetPduRRxSduId(pTpRxState->ConnId), pPdu)                                   /* SBSW_CANTP_APPLNOTIFICATION_CALL_PDUINFO */
...
# endif
      ...
      switch(pTpRxState->BufferState)
      {
        case CANTP_BUFREQ_OK:
          ...
          pTpRxState->Timer         = CanTp_RxCfg_GetNBr(pTpRxState->ConnId);                                          /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
...
# if (CANTP_CONSTANT_BS == STD_ON)
          ...
          CanTp_Data_CalcBS(pTpRxState->ConnId) = 0;                                               /* SBSW_CANTP_INTERNAL_WRITE_CALCBS */
# endif
          CanTp_RxGetBuffer_ISR(pTpRxState);                                                      /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
          break;
        ...
        case CANTP_BUFREQ_E_BUSY:
          ...
          pTpRxState->Timer         = CanTp_RxCfg_GetNAr(pTpRxState->ConnId);                                          /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
...
# endif
        ...
        case CANTP_BUFREQ_E_NOT_OK:
          /* #120 If buffer state is not ok: UL has not accepted the transfer, at all: Reset connection. Via ApplState CanTp_RxInit() takes care of calling PduR_CanTpRxIndication(), or not! */
          CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_NO_ERROR, CANTP_RESET_ALL);                         /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
          break;
        default:
          /* #130 Any other (i.e. invalid) buffer state: Reset connection with buffer and DET error. */
          CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_INVALID_RX_BUFFER, CANTP_RESET_ALL);                /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
          break;
      } /* switch(BufferState */
    }

    /* #140 If length check failed: ignore frame and release channel if no longer in use. */
    else
    {
      CanTp_DetReportError(CANTP_SID_RXINDICATION, CANTP_E_INVALID_RX_LENGTH)
      if (pTpRxState->ChannelState == CANTP_RXSTATE_IDLE)
...
# if (CANTP_TRANSMIT_QUEUE == STD_ON)
    CanTp_TxQueueCancel(pTpRxState->ConnId, CANTP_DIR_RX);                                         /* SBSW_CANTP_TXQUEUCANCEL_CALL_RX */
# else
    CanTp_Data_TxSemaphore( CanTp_RxCfg_GetTxFcConfPduId(pTpRxState->ConnId), pTpRxState->ConnId.PartIdx).Handle    = CANTP_INVALID_HDL; /* SBSW_CANTP_CANIFRXINDICATIONCF_WRITE_TXSEMAPHORE */
    CanTp_Data_TxSemaphore( CanTp_RxCfg_GetTxFcConfPduId(pTpRxState->ConnId), pTpRxState->ConnId.PartIdx).Direction = CANTP_DIR_NONE; /* SBSW_CANTP_CANIFRXINDICATIONCF_WRITE_TXSEMAPHORE */
    CanTp_CanIfCancelTransmit(CANTP_DIR_RX, pTpRxState->ConnId, CanTp_RxCfg_GetLoLayerTxFcPduId(pTpRxState->ConnId));
# endif
   ...
   */
    ...
    pTpRxState->PayloadLength = (uint8)(pTpRxState->InitialDL - lPayloadOffset); /* assume valid CF DLC which matches the FF DLC */ /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
    ...
    if (pTpRxState->PayloadLength <= (pTpRxState->DataLength - pTpRxState->DataIndex))
    {
      ...
      if (pPdu->SduLength == pTpRxState->InitialDL)
      {
        /* CF DLC check passed */
    ...
    else
    {
      ...
      if (CanTp_IsCanDlValid(pPdu->SduLength, CanTp_Util_GetMinFittingCanDl((pTpRxState->PayloadLength + lPayloadOffset), CanTp_RxCfg_HasPaddingActivation(pTpRxState->ConnId)))) /* max CAN_DL check */
      {
        /* DLC check passed */
        lErrorId = CANTP_E_NO_ERROR;
      }
...
# endif
    ...
    else
    {
      ...
      CanTp_ApplRxCFIndication(CanTp_RxCfg_GetPduRRxSduId(pTpRxState->ConnId), pPdu)                                   /* SBSW_CANTP_APPLNOTIFICATION_CALL_PDUINFO */
...
# endif
      ...
      pTpRxState->Timer = CanTp_RxCfg_GetNCr(pTpRxState->ConnId);                                                      /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
...
CANTP_LOCAL_INLINE FUNC(void, CANTP_CODE) CanTp_CanIfRxIndicationFC(CanTp_TxStatePtrType pTpTxState,
                                                                    P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pPdu)
{
  uint8 lAddressingOffset = CanTp_Util_GetAdrOffset(CanTp_TxCfg_GetAddressingFormat(pTpTxState->ConnId));

  /* #10 If the FC is received prior the TxConfirmation of the FF or prior the last CF in block: */
  if (  (pTpTxState->ChannelState == CANTP_TXSTATE_WAIT_TXCONF_FF ) ||                                        /* FC received prior Tx confirmation of FF */
       ((pTpTxState->ChannelState == CANTP_TXSTATE_WAIT_TXCONF_CF) && (pTpTxState->BlocksizeCounter == 1u)) ) /* FC received prior Tx confirmation of last CF in block */
  {
...
# endif /* CANTP_USE_ONLY_FIRST_FC */
            ...
            pTpTxState->Timer = CanTp_TxCfg_GetNCs(pTpTxState->ConnId); /* CANTP167 */                                 /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
          ...
          case CANTP_FLOWSTATUS_WT:
            ...
            pTpTxState->Timer = CanTp_TxCfg_GetNBs(pTpTxState->ConnId);                                                /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
            break; /* FC.WT */
  ...
  P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) CanTpTxInfoPtr)
{
  ...
  pTpTxState = CanTp_TxGetFreeChannelPtr(ConnId, CanTpTxInfoPtr);                                                      /* SBSW_CANTP_TXGETFREECHANNEL_CALL_PTR */
...
# endif
    ...
    if (SduLength > CanTp_TxCfg_GetMaxSFDataLength(ConnId))
    {
...
# if (CANTP_TX_HAS_TATYPE_FUNCTIONAL == STD_ON)
      if (CanTp_TxCfg_GetTAType(ConnId) == CANTP_TATYPE_FUNCTIONAL)
      {
...
# endif
  ...
  if (lReturnValue == E_OK)
  {
    ...
    pTpTxState->DataIndex = 0u;                                                                                        /* SBSW_CANTP_TXSTART_WRITE_TXSTATE */
    pTpTxState->DataLength = SduLength;                                                                                /* SBSW_CANTP_TXSTART_WRITE_TXSTATE */
    pTpTxState->Timer = CanTp_TxCfg_GetNCs(ConnId); /* CANTP167 */                                                     /* SBSW_CANTP_TXSTART_WRITE_TXSTATE */
...
CANTP_LOCAL_INLINE FUNC(PduLengthType, CANTP_CODE) CanTp_RxGetBytesPerBlock(CanTp_RxStatePtrType pTpRxState)           /* PRQA S 3673 */ /* MD_CanTp_3673 */
{
  ...
  PduLengthType lBytesPerBlock = (PduLengthType)CanTp_RxCfg_GetMaxPayloadCF(pTpRxState->ConnId);

#  if (CANTP_CONSTANT_BS == STD_ON)
  if (pTpRxState->FrameType != CANTP_FRAME_FF)
  {
    /* not the first block: same block size as calculated for first FC.CTS must be used */
    lBytesPerBlock = (PduLengthType)(lBytesPerBlock * CanTp_Data_CalcBS(pTpRxState->ConnId));
  }
#  endif
...
CANTP_LOCAL_INLINE FUNC(void, CANTP_CODE) CanTp_RxGetBufferSF(CanTp_RxStatePtrType pTpRxState)
{
  ...
  if ((pTpRxState->BufferState == CANTP_BUFREQ_OK) && (pTpRxState->ProvidedBufferSize >= pTpRxState->PayloadLength))
  {
    ...
    pTpRxState->BufferState = CanTp_PduRCopyRxData(CanTp_RxCfg_GetPduRRxSduId(pTpRxState->ConnId), /* CanTpRxPduId */  /* SBSW_CANTP_PDURCOPYRXDATA_CALL_PDUINFO */ /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
                                                   &lPduInfo, /* PduInfoPtr */
                                                   &pTpRxState->ProvidedBufferSize); /* RxBufSizePtr */
    ...
    /* #40 Otherwise, reset connection, optionally with buffer error. */
    else
    {
      if (pTpRxState->BufferState > CANTP_BUFREQ_E_OVFL) {
        /* CopyRxData failed with invalid retVal; reset connection, indicate DET error and notify application */
        CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_INVALID_RX_BUFFER, CANTP_RESET_ALL);                  /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
      } 
      else
      {
...
# endif
  ...
  /* #80 If buffer state is invalid: Reset connection with DET error. */
  else if(pTpRxState->BufferState > CANTP_BUFREQ_E_OVFL)
  {
    CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_INVALID_RX_BUFFER, CANTP_RESET_ALL);                      /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
  }
  /* #90 If buffer state is not large enough: Reset connection without DET error */
  else
  {
    CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_NO_ERROR, CANTP_RESET_ALL);                               /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
  }
...
# endif /* CANTP_SINGLE_RX_BUFFER_OPTIMIZED == STD_ON */
    ...
    case CANTP_BUFREQ_E_NOT_OK:
      /* #120 In case buffer state is not ok: Reset connection without sending a flow control and without DET error. */
      CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_NO_ERROR, CANTP_RESET_ALL);                             /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
      break;
    default:
      /* #130 In case of an invalid buffer state: Reset connection without sending a flow control and with DET error. */
      CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_INVALID_RX_BUFFER, CANTP_RESET_ALL);                    /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
      break;
  } /* switch(BufferState) */
} /* CanTp_RxGetBufferFF() */
...
# else
            ...
            else
            {
              ...
              pTpRxState->BufferState = (CanTp_BufferStateType)CanTp_PduRCopyRxData(CanTp_RxCfg_GetPduRRxSduId(pTpRxState->ConnId), /* CanTpRxPduId */ /* SBSW_CANTP_PDURCOPYRXDATA_CALL_PDUINFO */ /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
                                                                                    &lPduInfo, /* PduInfoPtr */
                                                                                    &pTpRxState->ProvidedBufferSize); /* RxBufSizePtr */
              ...
              else
              {
                if (pTpRxState->BufferState != CANTP_BUFREQ_E_BUSY)
                {
                  if (pTpRxState->BufferState > CANTP_BUFREQ_E_OVFL)
                  {
                    /* #140 Any other invalid buffer state: reset connection with DET error (no FC is sent). */
                    CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_INVALID_RX_BUFFER, CANTP_RESET_ALL);      /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
                  }
                  else 
                  {
                    /* #145 Any other valid buffer state: reset connection without DET error (no FC is sent). */
                    CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_NO_ERROR, CANTP_RESET_ALL);               /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
                  }
                }
...
# endif
    ...
    case CANTP_BUFREQ_E_OVFL:  /* CANTP318: no FC.OVFL is transmitted after a CF */
      /* #190 Any other valid buffer state: Reset connection without DET error and without sending a flow control. */
      CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_NO_ERROR, CANTP_RESET_ALL);                             /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
      break;
    default:
      /* #200 Any other invalid buffer state: Reset connection with DET error and without sending a flow control. */
      CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_INVALID_RX_BUFFER, CANTP_RESET_ALL);                    /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
      break;
  } /* switch(BufferState) */
} /* CanTp_RxGetBufferCF() */
...
# endif /* CANTP_SINGLE_RX_BUFFER_OPTIMIZED == STD_ON */
    ...
    case CANTP_BUFREQ_E_BUSY:
      ...
      break;
    case CANTP_BUFREQ_E_NOT_OK:
      /* #110 Any other valid buffer state: Reset connection without DET error. */
      CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_NO_ERROR, CANTP_RESET_ALL);                             /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
      break;
    default:
      /* #120 Any other invalid buffer state: Reset connection with DET error. */
      CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_INVALID_RX_BUFFER, CANTP_RESET_ALL);                    /* SBSW_CANTP_INTERNAL_CALL_RXSTATE */
      break;
  } /* switch(BufferState) */
} /* CanTp_RxGetBufferFCWait() */
...
CANTP_LOCAL_INLINE FUNC(PduIdType, CANTP_CODE) CanTp_Cfg_GetSduHdlByRxPduOnly(PduIdType RxPduId,
                                                                              uint8 FrameType)
{
  PduIdType lSduHdl = CANTP_INVALID_HDL;                                                                               /* PRQA S 2981 */ /* MD_MSR_RetVal */
  ...
  if ( (FrameType == CANTP_FRAME_FC) && (CanTp_RxPduHasTxSduRef(RxPduId)) )                                            /* COV_CANTP_PDU_REF */
  {
    ...
    lSduHdl = (PduIdType)CanTp_GetRxPdu2TxSduInd(CanTp_GetRxPdu2TxSduStartIdxOfRxPduMap(RxPduId),
                                                 CanTp_GetRxPdu2TxSduPartitionIdxOfRxPduMap(RxPduId));
  }
# if (CANTP_RXPDU2RXSDUIND == STD_ON)                                                                                  /* COV_CANTP_ROBUSTNESS_CASE TX */
  else if ( (FrameType != CANTP_FRAME_FC) && (CanTp_RxPduHasRxSduRef(RxPduId)) )                                       /* PRQA S 2991, 2995 */ /* MD_CanTp_ExpressionAlwaysTrue, MD_CanTp_OperationAlwaysTrue */ /* COV_CANTP_PDU_REF */
  {
    /* --- get RxSduId from RxPduId  --- */
    /* for standard addressing, this is a 1:1 reference (validated by GenTool) */
    /* #40 get the configured Rx N-SDU handle for a given RxPduId from the generated tables. */
    lSduHdl = (PduIdType)CanTp_GetRxPdu2RxSduInd(CanTp_GetRxPdu2RxSduStartIdxOfRxPduMap(RxPduId),
                                                 CanTp_GetRxPdu2RxSduPartitionIdxOfRxPduMap(RxPduId));
  }
...
CANTP_LOCAL_INLINE FUNC(PduIdType, CANTP_CODE) CanTp_Cfg_GetSduHdlByRxPduAddress(PduIdType RxPduId,
                                                                                 ...
                                                                                 uint8 Address)
{
  uint8 lDummyErrorId;                                                                                                 /* PRQA S 2981 */ /* MD_MSR_RetVal */
  CanTp_ConnIdType connId;
  PduIdType lSduReturnHdl = CANTP_INVALID_HDL;

  CanTp_InitConnId(connId, 0u, CanTp_Util_GetPartIdxOfRxPduId(RxPduId, &lDummyErrorId));                               /* SBSW_CANTP_INTERNAL_CALL_ERROR_ID_PTR */ /* PRQA S 2982, 2983 */ /* MD_CanTp_RedundantAssignment, MD_CanTp_RedundantAssignment */

  /* due to optimizations, the RxPduId may not be needed */
  CANTP_DUMMY_STATEMENT(RxPduId);                                                                                      /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */

  if (FrameType == CANTP_FRAME_FC)
  {
    /* --- get TxSduId from RxPduId  --- */
    /* with an additional address byte in the payload, there may be multiple references */
#if (CANTP_RXPDU2TXSDUUSEDOFRXPDUMAP == STD_ON)                                                                        /* COV_CANTP_ROBUSTNESS_CASE TX */
    CanTp_RxPdu2TxSduEndIdxOfRxPduMapType lTxSduIndirectionIndex;
    ...
    for (lTxSduIndirectionIndex = (CanTp_RxPdu2TxSduEndIdxOfRxPduMapType)CanTp_GetRxPdu2TxSduStartIdxOfRxPduMap(RxPduId);
         lTxSduIndirectionIndex < (CanTp_RxPdu2TxSduEndIdxOfRxPduMapType)CanTp_GetRxPdu2TxSduEndIdxOfRxPduMap(RxPduId);
         lTxSduIndirectionIndex++)
    {
      connId.Hdl = (PduIdType)CanTp_GetRxPdu2TxSduInd(lTxSduIndirectionIndex, connId.PartIdx);

      if (CanTp_TxCfg_GetRxFcAddress(connId) == Address)
      {
        /* the TxSdu which is referenced by the RxPdu has also the correct FC Address --> bingo! */
        lSduReturnHdl = connId.Hdl;
        break;
      }
    }
#endif
    CANTP_DUMMY_STATEMENT(Address);                                                                                    /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */
  }
  else
  {
    /* --- get RxSduId from RxPduId  --- */
    /* with an additional address byte in the payload, there may be multiple references */
#if (CANTP_RXPDU2RXSDUUSEDOFRXPDUMAP == STD_ON)                                                                        /* COV_CANTP_ROBUSTNESS_CASE TX */ 
    CanTp_RxPdu2RxSduStartIdxOfRxPduMapType lRxSduIndirectionIndex; /* CanTp_RxSduCfgIndIterType */
    ...
    /* Note: if start and end index are equal, no RxSdu is configured */
    for (lRxSduIndirectionIndex = (CanTp_RxPdu2RxSduStartIdxOfRxPduMapType)CanTp_GetRxPdu2RxSduStartIdxOfRxPduMap(RxPduId);
         lRxSduIndirectionIndex < (CanTp_RxPdu2RxSduStartIdxOfRxPduMapType)CanTp_GetRxPdu2RxSduEndIdxOfRxPduMap(RxPduId);
         lRxSduIndirectionIndex++)
    {
      connId.Hdl = (PduIdType)CanTp_GetRxPdu2RxSduInd(lRxSduIndirectionIndex, connId.PartIdx);

      if (CanTp_RxCfg_GetRxAddress(connId) == Address)
      {
...
CANTP_LOCAL_INLINE FUNC(PduIdType, CANTP_CODE) CanTp_Cfg_GetSduHdlByRxPduAndMetadata(PduIdType RxPduId,
                                                                                     ...
                                                                                     P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pMetadataInfo)
{
  /* #10 Initially set the handle-to-be-returned as invalid. */
  PduIdType lSduHdl = CANTP_INVALID_HDL;

  CANTP_DUMMY_STATEMENT(PartIdx);                                                                                      /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */

  /* #20 If a flow control has been received:
   *       For each Tx N-SDU associated to the current Rx N-PDU:
   *         If the Tx N-SDU is active:
   *           If the Tx N-SDU's current metadata matches the metadata of the received N-PDU:
   *             Set the Tx N-SDU's handle as the handle-to-be-returned and stop iterating. */
  if (FrameType == CANTP_FRAME_FC)                                                                                     /* COV_CANTP_PDU_REF */
  {
    CanTp_RxPdu2TxSduEndIdxOfRxPduMapType lTxSduIndirectionIndex;
    for (lTxSduIndirectionIndex = (CanTp_RxPdu2TxSduEndIdxOfRxPduMapType)CanTp_GetRxPdu2TxSduStartIdxOfRxPduMap(RxPduId);
         lTxSduIndirectionIndex < (CanTp_RxPdu2TxSduEndIdxOfRxPduMapType)CanTp_GetRxPdu2TxSduEndIdxOfRxPduMap(RxPduId);
    ...
    {
      CanTp_ConnIdType connId;
      PduIdType lTmpSduHdl = (PduIdType)CanTp_GetRxPdu2TxSduInd(lTxSduIndirectionIndex, PartIdx);
      P2CONST(CanTp_TxStateType, AUTOMATIC, CANTP_APPL_DATA) pTpTxState;
      
      CanTp_InitConnId(connId, lTmpSduHdl, PartIdx);
      pTpTxState = CanTp_TxGetActiveChannelPtr(connId);
      if (pTpTxState != NULL_PTR)
      {
  ...
  /* #30 Otherwise, if any other frame type has been received:
   *       For each Rx N-SDU associated to the current Rx N-PDU:
   *         If the Rx N-SDU is active:
   *           If the Rx N-SDU's current metadata matches the metadata of the received N-PDU:
   *             Set the Rx N-SDU's handle as the handle-to-be-returned and stop iterating.
   *         Otherwise, if no idle Rx N-SDU has been stored yet, store the current one. 
   *       If no active N-SDU was found, but an idle one is available:
   *         Set the handle of the idle N-SDU as the handle-to-be-returned (a new reception could be started). */
  else
  {
    PduIdType lIdleSduHdl = CANTP_INVALID_HDL;
    CanTp_RxPdu2RxSduEndIdxOfRxPduMapType lRxSduIndirectionIndex;
    for (lRxSduIndirectionIndex = (CanTp_RxPdu2RxSduEndIdxOfRxPduMapType)CanTp_GetRxPdu2RxSduStartIdxOfRxPduMap(RxPduId);
         lRxSduIndirectionIndex < (CanTp_RxPdu2RxSduEndIdxOfRxPduMapType)CanTp_GetRxPdu2RxSduEndIdxOfRxPduMap(RxPduId);
         lRxSduIndirectionIndex++)
    {
      CanTp_ConnIdType connId;
      P2CONST(CanTp_RxStateType, AUTOMATIC, CANTP_APPL_DATA) pTpRxState;

      CanTp_InitConnId(connId, (PduIdType)CanTp_GetRxPdu2RxSduInd(lRxSduIndirectionIndex, PartIdx), PartIdx);
      pTpRxState = CanTp_RxGetActiveChannelPtr(connId);
      ...
      else if (lIdleSduHdl == CANTP_INVALID_HDL)
      {
        lIdleSduHdl = connId.Hdl;
      }
      else
      {
        /* Do nothing. */
    ...
    if ((lSduHdl == CANTP_INVALID_HDL) && (lIdleSduHdl != CANTP_INVALID_HDL))
    {
      lSduHdl = lIdleSduHdl;
    }
  }
...
CANTP_LOCAL_INLINE FUNC(void, CANTP_CODE) CanTp_RxInitState(CanTp_RxStatePtrType pTpRxState)
{
  ...
  pTpRxState->ConnId.Hdl         = CANTP_INVALID_HDL;                                                                  /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
  ...
  pTpRxState->Timer              = 0u;                                                                                 /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
  pTpRxState->DataLength         = 0u;                                                                                 /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
  pTpRxState->DataIndex          = 0u;                                                                                 /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
  pTpRxState->ProvidedBufferSize = 0u;                                                                                 /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
  pTpRxState->WFTCounter         = 0u;                                                                                 /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
  pTpRxState->BufferState        = CANTP_BUFREQ_OK;                                                                    /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
  pTpRxState->ConnId.PartIdx     = 0u;                                                                                 /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
  pTpRxState->BlocksizeCounter   = 0u;                                                                                 /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
  pTpRxState->PayloadLength      = 0u;                                                                                 /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
  pTpRxState->InitialDL          = 0u;                                                                                 /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
  pTpRxState->ExpectedSN         = 0u;                                                                                 /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
...
#endif
#if (CANTP_GENERIC_CONNECTIONS == STD_ON)
  pTpRxState->SduMetadataLength    = 0u;                                                                               /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
...
CANTP_LOCAL_INLINE FUNC(void, CANTP_CODE) CanTp_TxInitState(CanTp_TxStatePtrType pTpTxState)
{
  ...
  /* - FCLength      <--> FCData []                                                                    */
  /* - SduMetadataLength                       <--> SduMetadata    []                                  */
  /* - CanTp_GetPduMetadataLengthOfTxSduCfg()  <--> DataPduMetadata[]                                  */
  pTpTxState->ConnId.Hdl        = CANTP_INVALID_HDL;                                                                   /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
#if (CANTP_STMIN_BY_APPL == STD_ON)
  ...
  pTpTxState->Timer             = 0u;                                                                                  /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
  pTpTxState->DataLength        = 0u;                                                                                  /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
  pTpTxState->DataIndex         = 0u;                                                                                  /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
  pTpTxState->BufferState       = CANTP_BUFREQ_OK;                                                                     /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
  pTpTxState->ConnId.PartIdx    = 0u;                                                                                  /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
  pTpTxState->BlocksizeCounter  = 0u;                                                                                  /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
  pTpTxState->STmin             = 0u;                                                                                  /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
  pTpTxState->STminTimer        = 0u;                                                                                  /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
  pTpTxState->PayloadLength     = 0u;                                                                                  /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
  pTpTxState->SeqNumber         = 0u;                                                                                  /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
#if (CANTP_USE_ONLY_FIRST_FC == STD_ON)
  pTpTxState->BS                = 0u;                                                                                  /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
  pTpTxState->FirstFCReceived   = FALSE;                                                                               /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
#endif
  pTpTxState->FCLength          = 0u;                                                                                  /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
#if (CANTP_GENERIC_CONNECTIONS == STD_ON)
  pTpTxState->SduMetadataLength = 0u;                                                                                  /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
#endif
} /* CanTp_TxInitState() */
  ...
  P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pMetadataInfo)
{
  ...
  /* #10 If ConnId.Hdl is currently not active: */
  if (CanTp_RxChannelIsIdle(ConnId))
  {
# if (CANTP_DYN_CHANNEL_ASSIGNMENT == STD_ON)
    PduIdType lChannelHdl = 0u;
    /* #20 If Dynamic Channel Assignment is used, iterate through all the available Rx channels: */
    for (lChannelHdl = 0; lChannelHdl < CanTp_Cfg_GetNumRxChannels(ConnId.PartIdx); lChannelHdl++)
    {
      /* #30 If a free Rx channel is found, map the passed ConnId.Hdl to it. Otherwise, a NULL pointer will be returned. */
      if (CanTp_Data_RxState(lChannelHdl, ConnId.PartIdx).ChannelState == CANTP_RXSTATE_IDLE)
      {
        CanTp_NumRxChannelsAvail--;
        CanTp_Data_RxChannelMap(ConnId) = lChannelHdl; /* mapping handle -> state */                                   /* SBSW_CANTP_RXGETFREECHANNEL_WRITE_CHANNELMAP */
        pTpRxState = &CanTp_Data_RxState(lChannelHdl, ConnId.PartIdx);
        pTpRxState->ConnId = ConnId; /* mapping state -> RxHdl and PartIdx */                                          /* SBSW_CANTP_RXGETFREECHANNEL_WRITE_STATE */
        break;
      }
    }
# else
    /* #40 Otherwise, Return a pointer to RxState[RxHdl] (non-dynamic channel assignment). */
    pTpRxState = &CanTp_Data_RxState(ConnId.Hdl, ConnId.PartIdx);
    pTpRxState->ConnId = ConnId; /* mapping state -> Hdl and PartIdx */                                                /* SBSW_CANTP_RXGETFREECHANNEL_WRITE_STATE */
# endif
  }

  /* #70 If required and a free channel is available:
           Try to extract and store the Rx SDU metadata
           If metadata extraction fails, release the channel without notifying the upper layer.*/
# if (CANTP_GENERIC_CONNECTIONS == STD_ON)
  if (pTpRxState != NULL_PTR)
  {
    Std_ReturnType lReturnValue = CanTp_Util_RxExtractSduMetadataInfo(pTpRxState, pPayloadInfo, pMetadataInfo);        /* SBSW_CANTP_RXEXTRACTSDUMETADATA_CALL_PTRS */
...
CANTP_LOCAL_INLINE FUNC(CanTp_RxStatePtrType, CANTP_CODE) CanTp_RxGetActiveChannelPtr(CanTp_ConnIdType ConnId)
{
  ...
  /* #10 If ConnId.Hdl is not related to an idle channel, return a pointer to the channel state. */
  if (!CanTp_RxChannelIsIdle(ConnId))
  {
    pTpRxState = CanTp_RxGetChannelPtr(ConnId);
  }
  ...
  P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) CanTpTxInfoPtr)
{
  ...
  /* #10 If ConnId.Hdl is currently not active: */
  if (CanTp_TxChannelIsIdle(ConnId))
  {
# if (CANTP_DYN_CHANNEL_ASSIGNMENT == STD_ON)
    PduIdType lChannelHdl = 0u;
    /* #20 If Dynamic Channel Assignment is used, iterate through all the available Tx channels: */
    for (lChannelHdl = 0; lChannelHdl < CanTp_Cfg_GetNumTxChannels(ConnId.PartIdx); lChannelHdl++)
    {
      /* #30 If a free Tx channel is found, map the passed ConnId.Hdl to it. Otherwise, a NULL pointer will be returned. */
      if (CanTp_Data_TxState(lChannelHdl, ConnId.PartIdx).ChannelState == CANTP_TXSTATE_IDLE)
      {
        CanTp_NumTxChannelsAvail--;
        CanTp_Data_TxChannelMap(ConnId) = lChannelHdl; /* mapping handle -> state */                                   /* SBSW_CANTP_TXGETFREECHANNEL_WRITE_CHANNELMAP */
        pTpTxState = &CanTp_Data_TxState(lChannelHdl, ConnId.PartIdx);
        pTpTxState->ConnId = ConnId; /* mapping state -> ConnId including PartIdx and TxHdl */                         /* SBSW_CANTP_TXGETFREECHANNEL_WRITE_STATE */
        break;
      }
    }
# else
    /* #40 Otherwise, Return a pointer to TxState[TxHdl] (non-dynamic channel assignment). */
    pTpTxState = &CanTp_Data_TxState(ConnId.Hdl, ConnId.PartIdx);
    pTpTxState->ConnId = ConnId; /* mapping state -> Hdl and PartIdx */                                                /* SBSW_CANTP_TXGETFREECHANNEL_WRITE_STATE */
# endif
  }
...
# if (CANTP_GENERIC_CONNECTIONS == STD_ON)
  if (pTpTxState != NULL_PTR)
  {
    Std_ReturnType lReturnValue = CanTp_Util_TxExtractSduMetadataInfo(pTpTxState, CanTpTxInfoPtr);                     /* SBSW_CANTP_TXEXTRACTSDUMETADATA_CALL_PTRS */

#  if (CANTP_TX_HAS_CUSTOM_ADDRESSING == STD_ON)
    if(lReturnValue != E_OK)
    {
      CanTp_TxInit(pTpTxState, CANTP_NOTIFY_NORESULT, CANTP_E_NO_ERROR);                                               /* SBSW_CANTP_INTERNAL_CALL_TXSTATE */
      pTpTxState = NULL_PTR;
    }
#  endif
    CANTP_DUMMY_STATEMENT(lReturnValue);                                                                               /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */
...

{
  ...
  if (!CanTp_TxChannelIsIdle(ConnId))
  {
    pTpTxState = CanTp_TxGetChannelPtr(ConnId);
  }
  ...
  P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pTpData)
{
  ...
  {
    lLoLayerIdx = CanTp_TxCfg_GetLoLayerIdx(ConnId);
  }
  else
  {
    lLoLayerIdx = CanTp_RxCfg_GetLoLayerIdx(ConnId);
  }

  /* #20 Request the transmission of the passed data to the lower layer. */
  return CanTp_GetTransmitFctOfLoLayerFcts(lLoLayerIdx, ConnId.PartIdx)(LoLayerTxPduId, pTpData);                      /* SBSW_CANTP_LOLAYERTRANSMIT_FCTPTR */
} /* CanTp_LoLayerTransmit */
#endif
                                                                      ...
                                                                      PduIdType LoLayerTxPduId)
{
  ...
  if (CanTp_GetCancelTransmitFctOfLoLayerFcts(lLoLayerIdx, PartIdx) != NULL_PTR)                                       /* COV_CANTP_LOLAYERFCT_CHECK TX */
  {
    (void)CanTp_GetCancelTransmitFctOfLoLayerFcts(lLoLayerIdx, PartIdx)(LoLayerTxPduId);                               /* SBSW_CANTP_LOLAYERCANCELTRANSMIT_FCTPTR */
  }
} /* CanTp_LoLayerCancelTransmit */
#endif
 ...
 *
 **********************************************************************************************************************/
/* PRQA S 6050, 6030, 6080 5 */ /* MD_MSR_STCAL, MD_MSR_STCYC, MD_MSR_STMIF */
CANTP_LOCAL FUNC(void, CANTP_CODE) CanTp_CanIfRxIndication(
  CanTp_ConnIdType ConnId,
  uint8 FrameType,
  P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pPayloadInfo,
  P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pMetadataInfo)
{
  ...
  if (CANTP_FRAME_FC != FrameType)  /* Rx */
  {
#if (CANTP_RX_OPTIMIZATION != CANTP_OPT_ZERO_SDU)                                                                      /* COV_CANTP_ROBUSTNESS_CASE TX */
    /* --- Rx Padding Check --- */
# if (CANTP_PADDING_SUPPORT_REQUIRED == STD_ON)
    ...
    lPaddingLength = CanTp_Util_GetPaddingLength((uint8)pPayloadInfo->SduLength,
                                                 CanTp_RxCfg_HasPaddingActivation(ConnId),
                                                 CanTp_RxCfg_GetCanType(ConnId));
...
# if (CANTP_RX_HAS_HALF_DUPLEX == STD_ON)
      if (CanTp_RxCheckChannelMode(ConnId) == E_OK)                                                                    /* PRQA S 2991, 2995 */ /* MD_CanTp_ExpressionAlwaysTrue, MD_CanTp_OperationAlwaysTrue */ /* COV_CANTP_POSTBUILD_DELETED */
# endif
      {
        /* --- Process Received Data N-PDU --- */
        CanTp_RxStatePtrType pTpRxState = CanTp_RxGetActiveChannelPtr(ConnId);

        if (pTpRxState == NULL_PTR)
        {
          /* no channel active; maybe we can start a new connection? */
          /* #50 In case of a received SF or FF: allocate new channel, if needed. */
          if ((FrameType == CANTP_FRAME_SF) || (FrameType == CANTP_FRAME_FF))
          {
            pTpRxState = CanTp_RxGetFreeChannelPtr(ConnId, pPayloadInfo, pMetadataInfo);                               /* SBSW_CANTP_RXGETFREECHANNEL_CALL_PTRS */
          }
        ...
        if (pTpRxState != NULL_PTR)
        {
          ...
          switch (FrameType)
          {
            ...
            */
            case CANTP_FRAME_SF:
# if (CANTP_GENERIC_CONNECTIONS == STD_ON)
              if ((!CanTp_RxCfg_IsGenericConnection(ConnId)) ||
                  (pTpRxState->ChannelState == CANTP_RXSTATE_IDLE))
# endif
              {
                pTpRxState->ConnId.PartIdx = ConnId.PartIdx;                                                           /* SBSW_CANTP_RXGETFREECHANNEL_WRITE_STATE */
...
# if (CANTP_PADDING_SUPPORT_REQUIRED == STD_ON)
    ...
    lPaddingLength = CanTp_Util_GetPaddingLength((uint8)pPayloadInfo->SduLength,
                                                 CanTp_TxCfg_HasPaddingActivation(ConnId),
                                                 CanTp_TxCfg_GetCanType(ConnId));
    if (pPayloadInfo->SduLength != lPaddingLength)
    {
 ...
 *
 **********************************************************************************************************************/
/* PRQA S 6080, 6030, 6050 2 */ /* MD_MSR_STMIF, MD_MSR_STCYC, MD_MSR_STCAL */
CANTP_LOCAL FUNC(void, CANTP_CODE) CanTp_CanIfTxConfirmation(CanTp_TxStatePtrType pTpTxState)
{
    ...
    case CANTP_TXSTATE_WAIT_TXCONF_CF:
      if (pTpTxState->DataIndex < pTpTxState->DataLength)
      {
        ...
        if (pTpTxState->ChannelState != CANTP_TXSTATE_WAIT_FC)
        {
          ...
          pTpTxState->Timer = CanTp_TxCfg_GetNCs(pTpTxState->ConnId);                                                  /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
          {
# if (CANTP_STMIN_BY_APPL == STD_ON)
...
# endif
  ...
  if (CanTp_Data_TxSemaphore(pPduTxData->TxConfPduId, pPduTxData->ConnId.PartIdx).Handle == CANTP_INVALID_HDL)
  {
    /* #30 Pass CanTp frame to CanIf and return the result of the transmission request. */
    lCanIfResult = CanTp_CanIfTransmit(pPduTxData->SduDirection, pPduTxData->ConnId, pPduTxData->TxPduId, pTpData);    /* SBSW_CANTP_CANIFTRANSMIT_CALL_TPDATA */

    if (lCanIfResult == CANTP_TX_OK)
    {
      /* #40 If request was successful, also lock the semaphore and call debugging call-out. */
      CanTp_Data_TxSemaphore(pPduTxData->TxConfPduId, pPduTxData->ConnId.PartIdx).Direction = pPduTxData->SduDirection; /* SBSW_CANTP_CANIFTRANSMITFRAME_WRITE_TXSEMAPHORE */
      CanTp_Data_TxSemaphore(pPduTxData->TxConfPduId, pPduTxData->ConnId.PartIdx).Handle = pPduTxData->ConnId.Hdl;     /* SBSW_CANTP_CANIFTRANSMITFRAME_WRITE_TXSEMAPHORE */
  ...
  else
  {
    ...
    if (!CanTp_TxQueueIsFull(pPduTxData->TxConfPduId, pPduTxData->ConnId.PartIdx))
    {
      CanTp_TxQueueEntryWritePos(pPduTxData->TxConfPduId, pPduTxData->ConnId.PartIdx).PduId = pPduTxData->TxPduId;     /* SBSW_CANTP_TXQUEUE_WRITE_ENTRY */
      CanTp_TxQueueEntryWritePos(pPduTxData->TxConfPduId, pPduTxData->ConnId.PartIdx).SduHandle = pPduTxData->ConnId.Hdl; /* SBSW_CANTP_TXQUEUE_WRITE_ENTRY */
      CanTp_TxQueueEntryWritePos(pPduTxData->TxConfPduId, pPduTxData->ConnId.PartIdx).SduDirection = pPduTxData->SduDirection; /* SBSW_CANTP_TXQUEUE_WRITE_ENTRY */
      CanTp_TxQueueEntryWritePos(pPduTxData->TxConfPduId, pPduTxData->ConnId.PartIdx).SduLength = pTpData->SduLength; /* SBSW_CANTP_TXQUEUE_WRITE_ENTRY */
      CanTp_Util_MemCpy((P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA))(CanTp_TxQueueEntryWritePos(pPduTxData->TxConfPduId, pPduTxData->ConnId.PartIdx).SduData), /* SBSW_CANTP_MEMCPY_CALL_TXQUEUE */
                        (P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA))(pTpData->SduDataPtr), (uint16)pTpData->SduLength);

      CanTp_Data_TxQueue(pPduTxData->TxConfPduId, pPduTxData->ConnId.PartIdx).WritePos = (uint8)((CanTp_Data_TxQueue(pPduTxData->TxConfPduId, pPduTxData->ConnId.PartIdx).WritePos + 1u) & CANTP_TX_QUEUE_MASK); /* SBSW_CANTP_TXQUEUE_WRITE_ENTRY */
                                                ...
                                                CanTp_ResetControlType ResetRequest)
{
  ...
  connId = pTpRxState->ConnId; /* Copy ConnId so it is available after RxState has been reset. */
...
# if (CANTP_TRANSMIT_QUEUE == STD_ON)
    CanTp_TxQueueCancel(connId, CANTP_DIR_RX);                                                                         /* SBSW_CANTP_TXQUEUCANCEL_CALL_RX */
# else
    /* no FC transmission request pending, only waiting for confirmation; release semaphore and cancel transmission */
    CanTp_Data_TxSemaphore( CanTp_RxCfg_GetTxFcConfPduId(connId), connId.PartIdx).Handle = CANTP_INVALID_HDL;          /* SBSW_CANTP_RXINIT_WRITE_TXSEMAPHORE */
    CanTp_Data_TxSemaphore( CanTp_RxCfg_GetTxFcConfPduId(connId), connId.PartIdx).Direction = CANTP_DIR_NONE;          /* SBSW_CANTP_RXINIT_WRITE_TXSEMAPHORE */
    CanTp_CanIfCancelTransmit(CANTP_DIR_RX, connId, CanTp_RxCfg_GetLoLayerTxFcPduId(connId));
# endif
  }
...
# endif
  ...
  if (ResetRequest == CANTP_RESET_BUT_KEEP_MAPPING)
  {
    ...
    pTpRxState->ConnId = connId;                                                                                       /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
  }
# if (CANTP_DYN_CHANNEL_ASSIGNMENT == STD_ON)
  else
  {
    /* channel no longer needed, completely remove the mapping */
    CanTp_Data_RxChannelMap(connId) = CANTP_INVALID_HDL;                                                               /* SBSW_CANTP_RXINIT_WRITE_CHANNELMAP */
    CanTp_NumRxChannelsAvail++;
  }
# endif

  /* --- Notify other BSW modules --- */
  /* #50 Notify PduR and application, if needed. */
  if (lIndicationPduR != 0u)
  {
    CanTp_ApplRxIndication(CanTp_RxCfg_GetPduRRxSduId(connId), NotifyCode)
    CanTp_PduRRxIndication(CanTp_RxCfg_GetPduRRxSduId(connId), NotifyCode);
  }
...
CANTP_LOCAL FUNC(void, CANTP_CODE) CanTp_TxInit(CanTp_TxStatePtrType pTpTxState,                                       /* PRQA S 3673 */ /* MD_CanTp_3673 */
                                                ...
                                                uint8 DetError)
{
  /* Create a copy of ConnId so it is available after TxState has been reset. */
  CanTp_ConnIdType connId;

  connId.PartIdx = pTpTxState->ConnId.PartIdx;                                                                         /* PRQA S 2982, 2983 */ /* MD_CanTp_RedundantAssignment */
  connId.Hdl = pTpTxState->ConnId.Hdl;                                                                                 /* PRQA S 2982, 2983 */ /* MD_CanTp_RedundantAssignment */
...
# if (CANTP_TRANSMIT_QUEUE == STD_ON)
    ...
    CanTp_TxQueueCancel(connId, CANTP_DIR_TX);                                                                         /* SBSW_CANTP_TXQUEUCANCEL_CALL_TX */
# else
    /* waiting for confirmation; release semaphore and cancel transmission */
    CanTp_Data_TxSemaphore( CanTp_TxCfg_GetTxConfPduId(connId), connId.PartIdx ).Handle = CANTP_INVALID_HDL;           /* SBSW_CANTP_TXINIT_WRITE_TXSEMAPHORE */
    CanTp_Data_TxSemaphore( CanTp_TxCfg_GetTxConfPduId(connId), connId.PartIdx ).Direction = CANTP_DIR_NONE;           /* SBSW_CANTP_TXINIT_WRITE_TXSEMAPHORE */
    CanTp_CanIfCancelTransmit(CANTP_DIR_TX, connId, CanTp_TxCfg_GetLoLayerTxPduId(connId));
# endif
  }

  /* #30 Reset Tx channel data (including SDU-to-channel mapping, if dynamic channel assignment is used). */
  CanTp_TxInitState(pTpTxState);                                                                                       /* SBSW_CANTP_INTERNAL_CALL_TXSTATE */
# if (CANTP_DYN_CHANNEL_ASSIGNMENT == STD_ON)
  CanTp_Data_TxChannelMap(connId) = CANTP_INVALID_HDL;                                                                 /* SBSW_CANTP_TXINIT_WRITE_CHANNELMAP */
  CanTp_NumTxChannelsAvail++;
# endif

  /* #40 If required, notify PduR and application of the transmission result. */
# if (CANTP_TX_HAS_CUSTOM_ADDRESSING == STD_ON)
  if (NotifyCode != CANTP_NOTIFY_NORESULT)
# endif
  {
    CanTp_ApplTxConfirmation(CanTp_TxCfg_GetPduRTxSduId(connId), NotifyCode)                                           /* SBSW_CANTP_APPLNOTIFICATION_CALL_PDUINFO */   
    CanTp_PduRTxConfirmation(CanTp_TxCfg_GetPduRTxSduId(connId), NotifyCode);
  }
...
{
  {
    ...
    CanTp_InitConnId(connId, 0u, PartIdx);                                                                             /* PRQA S 2982, 2983 */ /* MD_CanTp_RedundantAssignment */

    /* #10 Iterate through all configured Rx N-SDUs; for each do the following: */
    for (lSduHdl = 0; lSduHdl < CanTp_Cfg_GetNumRxHdls(PartIdx); lSduHdl++)
    {
#if (CANTP_DYN_CHANNEL_ASSIGNMENT == STD_ON) || (CANTP_ENABLE_CHANGE_PARAM == STD_ON)
      connId.Hdl = lSduHdl;
#endif
#if (CANTP_DYN_CHANNEL_ASSIGNMENT == STD_ON) 
      /* #20 In case of dynamic channel assignment: reset Rx SDU-to-channel mapping. */
      CanTp_Data_RxChannelMap(connId) = CANTP_INVALID_HDL;                                                             /* SBSW_CANTP_RESETCHANNELS_WRITE_RXHDL */
#endif

#if (CANTP_ENABLE_CHANGE_PARAM == STD_ON)
      /* #40 Reset dynamic FC parameters, if available. */
      CanTp_Data_DynFcParam(connId).BS = CanTp_RxCfg_GetBS(connId);                                                    /* SBSW_CANTP_RESETCHANNELS_WRITE_RXHDL */
      CanTp_Data_DynFcParam(connId).STmin = CanTp_RxCfg_GetSTmin(connId);                                              /* SBSW_CANTP_RESETCHANNELS_WRITE_RXHDL */
#endif
    }

#if (CANTP_DYN_CHANNEL_ASSIGNMENT == STD_ON)
    /* #50 Iterate through all configured Tx N-SDUs; for each do the following: */
    for (lSduHdl = 0; lSduHdl < CanTp_Cfg_GetNumTxHdls(PartIdx); lSduHdl++)
    {
      connId.Hdl = lSduHdl;
      /* #60 In case of dynamic channel assignment: reset Tx SDU-to-channel mapping. */
      CanTp_Data_TxChannelMap(connId) = CANTP_INVALID_HDL;                                                             /* SBSW_CANTP_RESETCHANNELS_WRITE_TXHDL */
    }
#endif
  }
  ...
  /* --- Reset channel specific data --- */
  {
    ...
    for (lChannelHdl = 0; lChannelHdl < CanTp_Cfg_GetNumRxChannels(PartIdx); lChannelHdl++)
    {
      CanTp_RxInitState(&CanTp_Data_RxState(lChannelHdl, PartIdx));                                                    /* SBSW_CANTP_RXINITSTATE_CALL_CHANNELHDL */
    }

    /* #120 Always initialize all the Tx channels. */
    for (lChannelHdl = 0; lChannelHdl < CanTp_Cfg_GetNumTxChannels(PartIdx); lChannelHdl++)
    {
      CanTp_TxInitState(&CanTp_Data_TxState(lChannelHdl, PartIdx));                                                    /* SBSW_CANTP_TXINITSTATE_CALL_CHANNELHDL */
    }
  }

#if(CANTP_DYN_CHANNEL_ASSIGNMENT == STD_ON)
  /* #130 In case of dynamic channel assignment: Reset the number of available channels. */
  CanTp_NumRxChannelsAvail = CanTp_Cfg_GetNumRxChannels(PartIdx);
  CanTp_NumTxChannelsAvail = CanTp_Cfg_GetNumTxChannels(PartIdx);
#endif
  ...
  /* #200 Iterate through all configured Tx N-PDUs; for each do the following: */
  {
    ...
    for (lTxConfHdl = 0; lTxConfHdl < CanTp_Cfg_GetNumTxConfPdus(PartIdx); lTxConfHdl++)
    {
      /* #210 Reset the transmit semaphore of the N-PDU. */
      CanTp_Data_TxSemaphore(lTxConfHdl, PartIdx).Handle     = CANTP_INVALID_HDL;                                      /* SBSW_CANTP_RESETCHANNELS_WRITE_TXSEMAPHORE */
      CanTp_Data_TxSemaphore(lTxConfHdl, PartIdx).Direction  = CANTP_DIR_NONE;                                         /* SBSW_CANTP_RESETCHANNELS_WRITE_TXSEMAPHORE */
...
#if (CANTP_TRANSMIT_QUEUE == STD_ON)
      {
        ...
        CanTp_Data_TxQueue(lTxConfHdl, PartIdx).ReadPos = 0;                                                           /* SBSW_CANTP_RESETCHANNELS_WRITE_TXSEMAPHORE */
        CanTp_Data_TxQueue(lTxConfHdl, PartIdx).WritePos = 0;                                                          /* SBSW_CANTP_RESETCHANNELS_WRITE_TXSEMAPHORE */

        for (lQueueCntr = 0; lQueueCntr < CANTP_TX_QUEUE_SIZE; lQueueCntr++)
        {
          CanTp_Data_TxQueue(lTxConfHdl, PartIdx).Entries[lQueueCntr].SduHandle = CANTP_INVALID_HDL;                   /* SBSW_CANTP_RESETCHANNELS_WRITE_TXSEMAPHORE */ /* SBSW_CANTP_RESETCHANNELS_WRITE_QUEUEENTRY */
        }
      }
#endif
    }
...
# if (CANTP_SUPPORT_LONG_FF == STD_ON)
    if (pTpTxState->DataLength > CANTP_FF_DL_12BIT)
    {
      ...
      pTpTxState->PayloadLength = CanTp_TxCfg_GetMaxPayloadLFF(pTpTxState->ConnId);                                    /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
    }
    else
# endif
    {
      pTpTxState->PayloadLength = CanTp_TxCfg_GetMaxPayloadFF(pTpTxState->ConnId);                                     /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
    }
  ...
  case CANTP_TXSTATE_WAIT_BUFFER_CF:
    ...
    pTpTxState->PayloadLength = CanTp_TxCfg_GetMaxPayloadCF(pTpTxState->ConnId);                                       /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
  ...
  if (lIsChannelStateValid == TRUE)
  {
    ...
    pTpTxState->BufferState = CanTp_PduRCopyTxData(CanTp_TxCfg_GetPduRTxSduId(pTpTxState->ConnId),                     /* SBSW_CANTP_PDURCOPYTXDATA_CALL_PDUINFO */ /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
      &lPduInfo,
    ...
    {
      pTpTxState->Timer = CanTp_TxCfg_GetNAs(pTpTxState->ConnId);                                                      /* SBSW_CANTP_INTERNAL_WRITE_TXSTATE */
    ...
    else
    {
      ...
      {
        if (pTpTxState->BufferState > CANTP_BUFREQ_E_OVFL)
        {
          /* For invalid buffer state, stop transfer and issue DET error */
          CanTp_TxInit(pTpTxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_INVALID_TX_BUFFER);                                 /* SBSW_CANTP_INTERNAL_CALL_TXSTATE */
        }
        else
        {
          /* solely stop transfer */
          CanTp_TxInit(pTpTxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_NO_ERROR);                                          /* SBSW_CANTP_INTERNAL_CALL_TXSTATE */
        }
      }
...
# if (CANTP_RX_CHECK_ADDRESSING_FORMAT == STD_ON)
  if (CanTp_Util_GetAdrOffset(CanTp_RxCfg_GetAddressingFormat(pTpRxState->ConnId)) != 0u)
# endif
  {
  ...
  lBlockSizeValue = CanTp_RxGetCurrentBS(pTpRxState->ConnId);
  ...
  switch(pTpRxState->ChannelState)
  {
    ...
    case CANTP_RXSTATE_TRANSMIT_FC_CTS:
      ...
      if (lBlockSizeValue != 0u)                                                                                       /* PRQA S 2991, 2995 */ /* MD_CanTp_ExpressionAlwaysTrue, MD_CanTp_OperationAlwaysTrue */
      {
# if (CANTP_CONSTANT_BS == STD_ON)
        if (CanTp_Data_CalcBS(pTpRxState->ConnId) != 0u)
        {
          /* use the initially calculated BS */
          lBlockSizeValue = CanTp_Data_CalcBS(pTpRxState->ConnId);
        }
        else
# endif
        {
          ...
          lBytesPerBlock = ((uint16)lBlockSizeValue) * CanTp_RxCfg_GetMaxPayloadCF(pTpRxState->ConnId);
               ...
               (pTpRxState->ProvidedBufferSize < (pTpRxState->DataLength - pTpRxState->DataIndex)) ) /* buffer too small for last block */
          {
            ...
            lBlockSizeValue = (uint8)(pTpRxState->ProvidedBufferSize / CanTp_RxCfg_GetMaxPayloadCF(pTpRxState->ConnId));
...
# if (CANTP_CONSTANT_BS == STD_ON)
          ...
          CanTp_Data_CalcBS(pTpRxState->ConnId) = lBlockSizeValue;                                                     /* SBSW_CANTP_INTERNAL_WRITE_CALCBS */
# endif
        } /* Calculate BS */
      } /* BS != 0 */
  ...
  if (lTransmitFC == TRUE)
  {
    ...
    lFCFrameData.SduDataPtr[lFCFrameData.SduLength + CANTP_FRAME_OFFSET_FC_STMIN] = CanTp_RxGetCurrentSTmin(pTpRxState->ConnId); /* STmin */ /* SBSW_CANTP_RXTRANSMITFRAME_WRITE_SDUDATAPTR */
    lFCFrameData.SduLength += CANTP_PCI_LENGTH_FC;

    /* --- Transmit FC --- */
    lPduTxInfo.ConnId = pTpRxState->ConnId;
    lPduTxInfo.SduDirection = CANTP_DIR_RX;
    lPduTxInfo.TxPduId = CanTp_RxCfg_GetLoLayerTxFcPduId(pTpRxState->ConnId);
    lPduTxInfo.TxConfPduId = CanTp_RxCfg_GetTxFcConfPduId(pTpRxState->ConnId);

# if (CANTP_PADDING_SUPPORT_REQUIRED == STD_ON)
    lPduTxInfo.PaddingLength = CanTp_Util_GetPaddingLength((uint8)lFCFrameData.SduLength, CanTp_RxCfg_HasPaddingActivation(pTpRxState->ConnId), CanTp_RxCfg_GetCanType(pTpRxState->ConnId));
# endif

    /* #120 If required, append the FC-PDU metadata information. */
# if (CANTP_MAX_PDU_METADATA_LENGTH != 0)
    /* In case of custom addressing, the metadata is modified immediately after the related UL callout */
    lPduTxInfo.PduMetadata = pTpRxState->FcPduMetadata;
    lPduTxInfo.PduMetadataLength = CanTp_RxCfg_GetPduMetadataLength(pTpRxState->ConnId);
# endif
...
# if (CANTP_TX_CHECK_ADDRESSING_FORMAT == STD_ON)
  if (CanTp_Util_GetAdrOffset(CanTp_TxCfg_GetAddressingFormat(pTpTxState->ConnId)) != 0u)
# endif
  {
...
# endif
  ...
  if (lIsChannelStateValid == TRUE)
  {
    ...
    /* --- Transmit TP Frame --- */
    lPduTxInfo.ConnId = pTpTxState->ConnId;
    lPduTxInfo.SduDirection = CANTP_DIR_TX;
    lPduTxInfo.TxPduId = CanTp_TxCfg_GetLoLayerTxPduId(pTpTxState->ConnId);
    lPduTxInfo.TxConfPduId = CanTp_TxCfg_GetTxConfPduId(pTpTxState->ConnId);

#if (CANTP_PADDING_SUPPORT_REQUIRED == STD_ON)
    lPduTxInfo.PaddingLength = CanTp_Util_GetPaddingLength((uint8)lTxFrameData.SduLength,
      CanTp_TxCfg_HasPaddingActivation(pTpTxState->ConnId),
      CanTp_TxCfg_GetCanType(pTpTxState->ConnId));
#endif
    ...
    lPduTxInfo.PduMetadataLength = CanTp_TxCfg_GetPduMetadataLength(pTpTxState->ConnId);
#endif
...
# if (CANTP_TXSDUCFGUSEDOFRXSDUCFG == STD_ON)
  CanTp_ConnIdType txConnId;
  
  txConnId.PartIdx = ConnId.PartIdx;                                                                                   /* PRQA S 2983 */ /* MD_CanTp_RedundantAssignment, MD_CanTp_RedundantAssignment */
  txConnId.Hdl = CanTp_RxCfg_GetTxSduHdl(ConnId);
# endif
  if (!CanTp_RxSdu2TxSduRefIsAvailable(ConnId))                                                                        /* PRQA S 2741, 2742 */ /* MD_CanTp_ExpressionAlwaysTrue, MD_CanTp_ExpressionAlwaysFalse */ /* COV_CANTP_POSTBUILD_DELETED */
  {
    /* no Tx connection defined for this channel */
    lResult = E_OK;                                                                                                    /* PRQA S 2880 */ /* MD_MSR_Unreachable */
  }
# if (CANTP_RX_CHECK_HALF_DUPLEX == STD_ON)
  else if (CANTP_MODE_FULL_DUPLEX == CanTp_RxCfg_GetChannelMode(ConnId))                                               /* PRQA S 2880 */ /* MD_MSR_Unreachable */ /* COV_CANTP_POSTBUILD_DELETED */
  {
    /* full duplex: both directions are allowed */
    lResult = E_OK;
  }
# endif
  /* only simplex allowed: */
# if (CANTP_TXSDUCFGUSEDOFRXSDUCFG == STD_ON)
  else if (CanTp_TxChannelIsIdle(txConnId) == TRUE)         /* COV_CANTP_POSTBUILD_DELETED */
  {
...
# endif
  else
  {
    ...
    lResult = E_NOT_OK;                                                                                                /* PRQA S 2880 */ /* MD_MSR_Unreachable */
  }
...
# if (CANTP_RXSDUCFGUSEDOFTXSDUCFG == STD_ON)
  CanTp_ConnIdType rxConnId;
  
  rxConnId.PartIdx = ConnId.PartIdx;                                                                                   /* PRQA S 2983 */ /* MD_CanTp_RedundantAssignment */
  rxConnId.Hdl = CanTp_TxCfg_GetRxSduHdl(ConnId);
# endif
  if (!CanTp_TxSdu2RxSduRefIsAvailable(ConnId))                                                                        /* PRQA S 2741, 2742 */ /* MD_CanTp_ExpressionAlwaysTrue, MD_CanTp_ExpressionAlwaysFalse */ /* COV_CANTP_POSTBUILD_DELETED */
  {
    /* no Tx connection defined for this channel */
    lResult = E_OK;                                                                                                    /* PRQA S 2880 */ /* MD_MSR_Unreachable */
  }
# if (CANTP_TX_CHECK_HALF_DUPLEX == STD_ON)
  else if (CANTP_MODE_FULL_DUPLEX == CanTp_TxCfg_GetChannelMode(ConnId))                                               /* COV_CANTP_POSTBUILD_DELETED */
  {
    /* full duplex: both directions are allowed */
    lResult = E_OK;
  }
# endif
  /* only simplex allowed: */
# if (CANTP_RXSDUCFGUSEDOFTXSDUCFG == STD_ON)
  else if (CanTp_RxChannelIsIdle(rxConnId) == TRUE)         /* COV_CANTP_POSTBUILD_DELETED */
  {
    /* Rx channel is idle, half duplex communication can be started */
    lResult = E_OK;
  }
# endif
  else
                                                        ...
                                                        CanTp_PartIdxType PartIdx)
{
  ...
  if (CanTp_TxQueueEntryReadPos(TxConfPduId, PartIdx).SduHandle != CANTP_INVALID_HDL)
  {
    if (CanTp_Data_TxSemaphore(TxConfPduId, PartIdx).Handle == CANTP_INVALID_HDL)
    {
      PduInfoType lPduInfo;
      PduIdType lQueuedPduId;
      CanTp_ConnIdType lQueuedConnId;
      CanTp_TxResultType lCanIfResult;
      CanTp_DirectionType lQueuedDirection;

      /* #20 Transmit queued N-PDU and remove entry from queue. */
      lPduInfo.SduLength = CanTp_TxQueueEntryReadPos(TxConfPduId, PartIdx).SduLength;
      lPduInfo.SduDataPtr = CanTp_TxQueueEntryReadPos(TxConfPduId, PartIdx).SduData;

      lQueuedPduId = CanTp_TxQueueEntryReadPos(TxConfPduId, PartIdx).PduId;

      CanTp_InitConnId(lQueuedConnId, CanTp_TxQueueEntryReadPos(TxConfPduId, PartIdx).SduHandle, PartIdx);             /* PRQA S 2983 */ /* MD_CanTp_RedundantAssignment */

      lQueuedDirection = CanTp_TxQueueEntryReadPos(TxConfPduId, PartIdx).SduDirection;

      lCanIfResult = CanTp_CanIfTransmit(lQueuedDirection, lQueuedConnId, lQueuedPduId, &lPduInfo);                    /* SBSW_CANTP_CANIFTRANSMIT_CALL_TXQUEUE */
                                           
      CanTp_TxQueueEntryReadPos(TxConfPduId, PartIdx).SduHandle = CANTP_INVALID_HDL;                                   /* SBSW_CANTP_TXQUEUE_WRITE_ENTRY */
      CanTp_Data_TxQueue(TxConfPduId, PartIdx).ReadPos = (uint8)((CanTp_Data_TxQueue(TxConfPduId, PartIdx).ReadPos + 1u) & CANTP_TX_QUEUE_MASK); /* SBSW_CANTP_TXQUEUE_WRITE_ENTRY */

      if (lCanIfResult == CANTP_TX_OK)
      {
        /* #30 If N-PDU has been successfully transmitted, lock its semaphore an call debugging call-out. */
        CanTp_Data_TxSemaphore(TxConfPduId, PartIdx).Handle = lQueuedConnId.Hdl;                                       /* SBSW_CANTP_TXQUEUEPROCESS_WRITE_TXSEMAPHORE */
        CanTp_Data_TxSemaphore(TxConfPduId, PartIdx).Direction = lQueuedDirection;                                     /* SBSW_CANTP_TXQUEUEPROCESS_WRITE_TXSEMAPHORE */
      ...
      else
      {
        ...
        {
          CanTp_TxInit(CanTp_TxGetChannelPtr(lQueuedConnId), CANTP_NOTIFY_NOT_OK, CANTP_E_TX_TRANSMIT_ERROR); /* SBSW_CANTP_RXTXINIT_CALL_QUEUEDHDL */
        }
        else
        {
          CanTp_RxInit(CanTp_RxGetChannelPtr(lQueuedConnId), CANTP_NOTIFY_NOT_OK, CANTP_E_RX_TRANSMIT_ERROR, CANTP_RESET_ALL); /* SBSW_CANTP_RXTXINIT_CALL_QUEUEDHDL */
        }
      }
    }
                                                       ...
                                                       CanTp_DirectionType Direction)
{
  ...
  if (Direction == CANTP_DIR_TX)
  {
    lTxConfPduId = CanTp_TxCfg_GetTxConfPduId(ConnId);
  }
  else
  {
    lTxConfPduId = CanTp_RxCfg_GetTxFcConfPduId(ConnId);
  }


  /* --- if N-PDU is processed by the CanIf: release semaphore and cancel transmission --- */
  if ((CanTp_Data_TxSemaphore(lTxConfPduId, ConnId.PartIdx).Handle == ConnId.Hdl) &&
      (CanTp_Data_TxSemaphore(lTxConfPduId, ConnId.PartIdx).Direction == Direction))
  {
    /* #30 Release semaphore and cancel N-PDU transmission. */
    CanTp_Data_TxSemaphore(lTxConfPduId, ConnId.PartIdx).Handle = CANTP_INVALID_HDL;                                          /* SBSW_CANTP_TXQUEUECANCEL_WRITE_TXSEMAPHORE */
    CanTp_Data_TxSemaphore(lTxConfPduId, ConnId.PartIdx).Direction = CANTP_DIR_NONE;                                          /* SBSW_CANTP_TXQUEUECANCEL_WRITE_TXSEMAPHORE */
...
# if (CANTP_LOLAYER_TC == STD_ON)
  ...
  else
  {
    ...
    lQueueCntr = CanTp_Data_TxQueue(lTxConfPduId, ConnId.PartIdx).ReadPos;
    while (lQueueCntr != CanTp_Data_TxQueue(lTxConfPduId, ConnId.PartIdx).WritePos)
    {
      if ((CanTp_Data_TxQueue(lTxConfPduId, ConnId.PartIdx).Entries[lQueueCntr].SduHandle == ConnId.Hdl) &&
          (CanTp_Data_TxQueue(lTxConfPduId, ConnId.PartIdx).Entries[lQueueCntr].SduDirection == Direction))
      {
        CanTp_Data_TxQueue(lTxConfPduId, ConnId.PartIdx).Entries[lQueueCntr].SduHandle = CANTP_INVALID_HDL;                   /* SBSW_CANTP_TXQUEUECANCEL_WRITE_TXSEMAPHORE */ /* SBSW_CANTP_TXQUEUE_WRITE_ENTRY */
        break;
      }
 ...
 *
 *********************************************************************************************************************/
FUNC(void, CANTP_CODE) CanTp_Init(P2CONST(CanTp_PBConfigType, AUTOMATIC, CANTP_INIT_DATA) ConfigPtr)
{
/* ----- Local Variables ---------------------------------------------- */
  uint8 lErrorId = CANTP_E_NO_ERROR;                                                                                   /* PRQA S 2981 */ /* MD_MSR_RetVal */
  CanTp_PartIdxType partIdx = CanTp_Util_GetPartIdx(&lErrorId);                                                        /* SBSW_CANTP_INTERNAL_CALL_ERROR_ID_PTR */

  /* ----- Development Error Checks ------------------------------------- */
  /* #10 Check global, partition-independent flag. */
  if (!CanTp_IsPreInitialized())
  {
    /* No CfgPtr available, else: CfgPtr and Initialized-flag are valid */
    lErrorId = CANTP_E_NO_PREINIT;
  }
  /* PRQA S 2991, 2995, 2004 4 */ /* MD_CanTp_ExpressionAlwaysTrue, MD_CanTp_OperationAlwaysTrue, MD_MSR_EmptyClause */
  else if (lErrorId == CANTP_E_NO_ERROR)                                                                               /* COV_CANTP_ERROR_ID_VALID */
  {
    /* #25 Check validity of the current partitions configuration. */  
    if (CanTp_Util_CheckConfigValidity(partIdx) == TRUE)                                                               /* COV_CANTP_SILENT_RUNTIME_CHECK */                                                     
    {
      /* #30 If all checks are passed, reset all channels and set module state to "initialized". */
      CanTp_ResetChannels(partIdx);
      CanTp_SetInitialized(0, TRUE, partIdx);                                                                          /* SBSW_CANTP_INIT_WRITE_PARTITION_CONFIG */
    }
  }
  ...
  CANTP_DUMMY_STATEMENT(ConfigPtr);                                                                                    /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */
} /* CanTp_Init() */
 ...
 **********************************************************************************************************************/
/* PRQA S 6080 1 */ /* MD_MSR_STMIF */
FUNC(void, CANTP_CODE) CanTp_PreInit(P2CONST(CanTp_PBConfigType, AUTOMATIC, CANTP_INIT_DATA) ConfigPtr)
{
  /* ----- Local Variables --------------------------------------------------------------------- */
  CanTp_PartIdxType partIdx = 0u;                                                                                      /* PRQA S 2981 */ /* MD_CanTp_RedundantAssignment */
  ...
  CanTp_Data_CfgPtr() = ConfigPtr;
  CanTp_SetPreInitialized(FALSE);
  ...
  {
    partIdx = CanTp_Util_GetPartIdx(&lErrorId);                                                                        /* SBSW_CANTP_INTERNAL_CALL_ERROR_ID_PTR */
  }
  if(lErrorId != CANTP_E_NO_ERROR)
  {
    lErrorId = lErrorId; /* CANTP_E_OSAPP2MEMMAPPING */
  }
# if (CANTP_CONFIGURATION_VARIANT == CANTP_CONFIGURATION_VARIANT_POSTBUILD_LOADABLE)
  /* Check generator version compatibility: SPEC00035482 */
  else if (CANTP_GENERATOR_COMPATIBILITY_VERSION != CanTp_GetCompatibilityVersion(partIdx))
  {
    CanTp_EcuMReportBswError(ECUM_BSWERROR_COMPATIBILITYVERSION)
  }
  /* Check magic number: SPEC00035483 */
  else if (CANTP_FINAL_MAGIC_NUMBER != ConfigPtr->FinalMagicNumberOfPBConfig)
  {
...
# endif
  ...
  {
    /* Note: In case of CANTP_USE_INIT_POINTER == STD_ON this is the 'error-free' else-path, independent of
             the value of CANTP_CONFIGURATION_VARIANT !
             In case of CANTP_USE_INIT_POINTER == STD_OFF this block is executed, always, but prior this
             block there is no possibility that lErrorId gets a value != CANTP_E_NOT_ERROR.
             Thus no further check of lErrorId is required within this block !!! */
    /* Set pre-init-flag only after ConfigPtr-Check */
    CanTp_SetPreInitialized(TRUE);

#if (CANTP_SUPPORT_MULTI_PARTITION == STD_ON)
    /* Only access config pointer in case it is "valid", and use case is multi-partition. */
    /* #10  Set partition-specific module state as well as pre and post-initialization-flag to 'not initialized'. */
    for (partIdx = 0; partIdx < CanTp_GetSizeOfPartitionIdentifiers(); partIdx++)
    {
      CanTp_SetInitialized(0, FALSE, partIdx);                                                                         /* SBSW_CANTP_INIT_WRITE_PARTITION_CONFIG */
    }
#endif
  }
...
# if (CANTP_DEV_ERROR_REPORT == STD_ON)
  if (lErrorId != CANTP_E_NO_ERROR)
  {
    CanTp_DetReportError(CANTP_SID_PREINIT, lErrorId)
  }
# endif
#endif /* CANTP_USE_INIT_POINTER == STD_ON */
  /* Errors may occur, but are not reported. */
  CANTP_DUMMY_STATEMENT(ConfigPtr);                                                                                    /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */
  CANTP_DUMMY_STATEMENT(lErrorId);                                                                                     /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */
  CANTP_DUMMY_STATEMENT(partIdx);                                                                                      /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */
} /* CanTp_PreInit() */
...
FUNC(void, CANTP_CODE) CanTp_InitMemory(void)
{
  ...
  CanTp_SetPreInitialized(FALSE);
#if (CANTP_SUPPORT_MULTI_PARTITION == STD_OFF)
  /* Init-flag only is "plain link-time" for NON-MultiPartition: */
  CanTp_SetInitialized(0, FALSE, 0u);                                                                                  /* SBSW_CANTP_INIT_WRITE_PARTITION_CONFIG */
/* #else: For MultiPartition a valid CfgPtr is required to reset it which is done in CanTp_PreInit() */
#endif
#if(CANTP_USE_INIT_POINTER == STD_ON)
  CanTp_ConfigDataPtr = NULL_PTR;
#endif
} /* CanTp_InitMemory() */
...
FUNC(void, CANTP_CODE) CanTp_Shutdown(void)
{
  ...
  /* ----- Development Error Checks ------------------------------------------------------------ */
  /* #10 Always ensure that the CanTp is pre-initialized (even with disabled DET). */
  if (!CanTp_IsPreInitialized())
  {
    /* No CfgPtr available, else: CfgPtr and Initialized-flag are valid */
    lErrorId = CANTP_E_NO_PREINIT;
  }
  else if (lErrorId != CANTP_E_NO_ERROR)                                                                               /* PRQA S 2992, 2996 */ /* MD_CanTp_CheckCfgExpressionAlwaysFalse, MD_CanTp_CheckCfgOperationAlwaysFalse */ /* COV_CANTP_ERROR_ID_VALID */
  {
    lErrorId = lErrorId; /* CANTP_E_OSAPP2MEMMAPPING */
  }
  /* #15 Always ensure that the CanTp is initialized (even with disabled DET). */
  else if (!CanTp_IsInitialized(0, partIdx))
  {
...
FUNC(void, CANTP_CODE) CanTp_MainFunctionRx(void)
{
  ...
  /* #10 Ensure that the CanTp is pre-initialized, but do not report an error (SWS_BSW_00037). */
  if(CanTp_IsPreInitialized())
  {
    ...
    if(
       ...
       (lErrorId == CANTP_E_NO_ERROR) &&
#endif
       CanTp_IsInitialized(0, partIdx))
    {
      /* ----- Implementation -------------------------------------------------------------------- */
      /* #20 For each active Rx channel: */
      for (lRxChannelHdl = 0; lRxChannelHdl < CanTp_Cfg_GetNumRxChannels(partIdx); lRxChannelHdl++)
      {
        pTpRxState = &CanTp_Data_RxState(lRxChannelHdl, partIdx);
        /* is channel active (i.e.: occupied by a connection)? */
        if (pTpRxState->ConnId.Hdl != CANTP_INVALID_HDL)
        {
            ...
            if (pTpRxState->Timer == 0u)
            {
              ...
              switch (pTpRxState->ChannelState)
              {
                ...
                case CANTP_RXSTATE_WAIT_TXCONFFC_OVFL:
                  ...
                  CanTp_RxInit(pTpRxState, CANTP_NOTIFY_NO_BUFFER, CANTP_E_NO_ERROR, CANTP_RESET_ALL);                 /* SBSW_CANTP_TASK_CALL_RXSTATE */
                  break;
                ...
                case CANTP_RXSTATE_WAIT_NEXT_FCWAIT:
                  ...
                  pTpRxState->Timer = CanTp_RxCfg_GetNAr(pTpRxState->ConnId);                                          /* SBSW_CANTP_TASK_WRITE_RXSTATE */
  ...
  # if (CANTP_TRANSMIT_QUEUE == STD_ON)
              ...
              /* Check this one first to be sure the channel is still active and pTpRxState->ConnId.Hdl is valid. If it would */
              ...
              if ( (CanTp_RxCfg_GetTxFcConfPduId(pTpRxState->ConnId) != CANTP_INVALID_HDL) &&
                   (!CanTp_TxQueueIsEmpty(CanTp_RxCfg_GetTxFcConfPduId(pTpRxState->ConnId), partIdx)) )
              {
                CanTp_TxQueueProcess(CanTp_RxCfg_GetTxFcConfPduId(pTpRxState->ConnId), partIdx);                       /* SBSW_CANTP_TXQUEUEPROCESS_CALL_RX */
              }
  # endif
    ...
    } /* if((lErrorId == CANTP_E_NO_ERROR) && CanTp_IsInitialized(0, partIdx)) */
#if ((CANTP_DEV_ERROR_REPORT == STD_ON) && (CANTP_SUPPORT_MULTI_PARTITION == STD_ON))
    if(lErrorId != CANTP_E_NO_ERROR)
    {    
      CanTp_DetReportError(CANTP_SID_MAINFUNCTIONRX, lErrorId);
    }
#endif
  } /* if( partIdx was looked up successfully and CanTp is initialized ) */
#endif
} /* CanTp_MainFunctionRx() */
...
FUNC(void, CANTP_CODE) CanTp_MainFunctionTx(void)
{
  ...
  if(CanTp_IsPreInitialized())
  {
       ...
       CanTp_IsInitialized(0, partIdx))
    {
      ...
      for (lTxChannelHdl = 0; lTxChannelHdl < CanTp_Cfg_GetNumTxChannels(partIdx); lTxChannelHdl++)
      {
        pTpTxState = &CanTp_Data_TxState(lTxChannelHdl, partIdx);
        /* is channel active (i.e.: occupied by a connection)? */
        if (pTpTxState->ConnId.Hdl != CANTP_INVALID_HDL)
        {
          CanTp_EnterCritical(); /* CanTp_MainFunctionTx ~~~~~~~~~~~~~~~~~~~~ */

          /* is it still active? */
          if (pTpTxState->ConnId.Hdl != CANTP_INVALID_HDL) /* check will only fail if state changed before critical section have been entered */
          {
  ...
  # if (CANTP_TRANSMIT_QUEUE == STD_ON)
              ...
              /* Check this one first to be sure the channel is still active and pTpTxState->ConnId.Hdl is valid.
                 If it would be done later, CanTp_TxGetBuffer() and CanTp_TxTransmitFrame() may have reset the channel.
                 However, it doesn't matter if the channel is reset in CanTp_TxQueueProcess(), because before the other
                 calls below the channel state is checked anyway. */
              if (!CanTp_TxQueueIsEmpty(CanTp_TxCfg_GetTxConfPduId(pTpTxState->ConnId), partIdx))
              {
                CanTp_TxQueueProcess(CanTp_TxCfg_GetTxConfPduId(pTpTxState->ConnId), partIdx);                         /* SBSW_CANTP_TXQUEUEPROCESS_CALL_TX */
              }
  # endif
    ...
    if(lErrorId != CANTP_E_NO_ERROR)
    {    
      CanTp_DetReportError(CANTP_SID_MAINFUNCTIONTX, lErrorId);
    }
#endif
  } /* if( partIdx was looked up successfully and CanTp is initialized ) */
#endif
...
FUNC(Std_ReturnType, CANTP_CODE) CanTp_Transmit(PduIdType TxSduId,
                                                P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) CanTpTxInfoPtr)
{
  /* ----- Local Variables --------------------------------------------------------------------- */
  uint8 lErrorId = CANTP_E_NO_ERROR;                                                                                   /* PRQA S 2981 */ /* MD_CanTp_RedundantAssignment */
  ...
  CANTP_DUMMY_STATEMENT(TxSduId);                                                                                      /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e{438} */

  /* ----- Development Error Checks ------------------------------------------------------------ */
  /* #10 Always ensure that the CanTp is pre-initialized to avoid start of communication (even with disabled DET). */
  if(!CanTp_IsPreInitialized())
  {
    /* No CfgPtr available, else: CfgPtr and Initialized-flag are valid */
    lErrorId = CANTP_E_NO_PREINIT;
...
#if (CANTP_DEV_ERROR_DETECT == STD_ON)                                                                                 /* COV_CANTP_DET_CHECK TX */
  /* #15  Check that all parameters are in range and valid. */
  else if (    (TxSduId >= CanTp_Cfg_GetNumTxSduIds())                                                                 /* PRQA S 2991, 2995 */ /* MD_CanTp_ExpressionAlwaysTrue, MD_CanTp_OperationAlwaysTrue */
# if (CANTP_INVALIDHNDOFTXSDUSNV2HDL == STD_ON)
            || (CanTp_IsInvalidHndOfTxSduSnv2Hdl(TxSduId))
# endif
          )                                                                                                            /* COV_CANTP_POSTBUILD_DELETED */
  {
...
# endif
#endif
  /* #20 Always ensure that the CanTp is initialized to avoid start of communication (even with disabled DET). */
  else if (!CanTp_IsInitialized(0, CanTp_GetPartIdxOfTxNSduSnv(TxSduId)))
  {
    lErrorId = CANTP_E_UNINIT;
  }
  else
  {
#if (CANTP_TX_OPTIMIZATION != CANTP_OPT_ZERO_SDU)                                                                      /* COV_CANTP_ROBUSTNESS_CASE TX */
    /* ----- Implementation -------------------------------------------------------------------- */
    CanTp_PartIdxType partIdx = CanTp_GetPartIdxOfTxNSduSnv(TxSduId);
    /* #30  If all checks are passed: get internal handle for requested Tx N-SDU. */
    CONST(PduIdType, AUTOMATIC) lTxSduHdl = CanTp_TxGetSduHdlBySduId(TxSduId);
    ...
    if (CanTpTxInfoPtr->SduLength > 0u)
    {
      ...
      CanTp_InitConnId(connId, lTxSduHdl, partIdx);
...
# if (CANTP_TX_HAS_HALF_DUPLEX == STD_ON)
      ...
      if (CanTp_TxCheckChannelMode(connId) == E_OK)                                                                    /* PRQA S 2991, 2995 */ /* MD_CanTp_ExpressionAlwaysTrue, MD_CanTp_OperationAlwaysTrue */ /* COV_CANTP_POSTBUILD_DELETED */
# endif
      {
        /* --- Start Transmission --- */
        /* #60 If half duplex check is passed, setup a new Tx connection for the requested N-SDU. */
        lReturnValue = CanTp_TxStartConnection(TxSduId, connId, CanTpTxInfoPtr->SduLength, CanTpTxInfoPtr);            /* SBSW_CANTP_TXSTARTCONNECTION_CALL_PDUINFO */
      }
...
FUNC(void, CANTP_CODE) CanTp_RxIndication(PduIdType RxPduId, P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) PduInfoPtr)
{
  /* ----- Local Variables --------------------------------------------------------------------- */
  uint8 lErrorId = CANTP_E_NO_ERROR;
  CanTp_PartIdxType partIdx = CanTp_Util_GetPartIdxOfRxPduId(RxPduId, &lErrorId);                                      /* SBSW_CANTP_INTERNAL_CALL_ERROR_ID_PTR */
...
#if (CANTP_DEV_ERROR_DETECT == STD_ON)                                                                                 /* COV_CANTP_DET_CHECK TX */
  /* #15 Check validity of parameters RxPduId, PduInfoPtr and PduInfoPtr->SduDataPtr. */
  else if ((RxPduId >= CanTp_Cfg_GetNumRxPdus()) || ((lErrorId != CANTP_E_NO_ERROR)))                                  /* PRQA S 2996 */ /* MD_CanTp_OperationAlwaysFalse */ /* COV_CANTP_ERROR_ID_VALID */
  {
...
#endif
  /* #20 Always ensure that the CanTp is pre-initialized to avoid start of communication (even with disabled DET). */
  else if ((lErrorId == CANTP_E_NO_ERROR) && (!CanTp_IsInitialized(0, partIdx)))                                       /* PRQA S 2995 */ /* MD_CanTp_OperationAlwaysTrue */ /* COV_CANTP_ERROR_ID_VALID */
  {
    lErrorId = CANTP_E_UNINIT;
  }
  else
  {
    ...
    if ((PduInfoPtr->SduLength - CanTp_Cfg_GetPduMetadataLengthByRxPdu(RxPduId)) > 1u)                                 /* PRQA S 2985 */ /* MD_CanTp_RedundantOperation */
    {
      ...
      /* #26  Separate the N-PDU's payload and metadata */
#if (CANTP_MAX_PDU_METADATA_LENGTH != 0)
      ...
      /* #27  Determine the type of the received N-PDU */
      lReceivedFrameType = CanTp_Util_GetFrameType(CanTp_Cfg_GetAddressingTypeByRxPdu(RxPduId), &lPayloadInfo);        /* SBSW_CANTP_CALL_INTERNAL_RCHECK */

      /* #30  If the received N-PDU is associated to generic connections:
       *        Get an N-SDU handle, based on the N-PDU id, frame type and metadata.
       *      Otherwise:
       *        Continue depending on the addressing associated to the N-PDU:
       *          Standard or Normal Fixed addressing:
       *            Get an N-SDU handle, based on the N-PDU id and frame type.
       *          Extended or Mixed11 or Mixed 29 addressing:
       *            Get an N-SDU handle, based on the N-PDU id, frame type and in-payload addressing information.
       *          Other:
       *            Ignore the N-PDU. */
#if (CANTP_GENERIC_CONNECTIONS == STD_ON)
      if (CanTp_Cfg_IsGenericConnectionByRxPdu(RxPduId))
      {
        lSduHdl = CanTp_Cfg_GetSduHdlByRxPduAndMetadata(RxPduId, partIdx, lReceivedFrameType, &lPayloadInfo, &lMetadataInfo); /* SBSW_CANTP_CALL_INTERNAL_RCHECK */
      }
      else
#endif
      {
...
# endif
          ...
          default:                                                                                                     /* COV_CANTP_DEFAULT_BRANCH */
          /* No DET is reported here because in theory, this may happen when the PduMap entry has been removed at post-build time. */
...
# if (CANTP_SUPPORT_MULTIPLE_ADDRESSING == STD_ON)
      if (    (CanTp_Cfg_GetAddressingTypeByRxPdu(RxPduId) == CANTP_ADDRESS_FORMAT_MIXED11)
           && ((lReceivedFrameType == CANTP_FRAME_SF) || (lReceivedFrameType == CANTP_FRAME_FF)))
# else
      if ((lReceivedFrameType == CANTP_FRAME_SF) || (lReceivedFrameType == CANTP_FRAME_FF))
# endif
      {
      ...
      /* #110  If an N-SDU handle could be retrieved, call CanTp_CanIfRxIndication(). */
      if (lSduHdl != CANTP_INVALID_HDL)                                                                                /* PRQA S 2991, 2995 */ /* MD_CanTp_ExpressionAlwaysTrue, MD_CanTp_OperationAlwaysTrue */ /* COV_CANTP_SINGLE_CHANNEL_PDU */
      {
        ...
        CanTp_InitConnId(connId, lSduHdl, partIdx);
        CanTp_CanIfRxIndication(connId, lReceivedFrameType, &lPayloadInfo, &lMetadataInfo);                            /* SBSW_CANTP_CANIFRXINDICATION_CALL_PDUINFO */
      }
      ...
      else if ((!CanTp_RxPduHasTxSduRef(RxPduId)) && (!CanTp_RxPduHasRxSduRef(RxPduId)))                               /* COV_CANTP_PDU_REF */
      {
        lErrorId = CANTP_E_INVALID_RX_ID;
      }
      else
...
#endif
    } /* if(passed CAN_DL is greater than 1) */
  } /* if(Further DET-checks) else: Impl. */
...
FUNC(void, CANTP_CODE) CanTp_TxConfirmation(PduIdType TxPduId)
{
  /* ----- Local Variables --------------------------------------------------------------------- */
  uint8 lErrorId = CANTP_E_NO_ERROR;  
  CanTp_PartIdxType partIdx = CanTp_Util_GetPartIdxOfTxPduId(TxPduId, &lErrorId);                                      /* SBSW_CANTP_INTERNAL_CALL_ERROR_ID_PTR */
  ...
  else if ((TxPduId >= CanTp_GetSizeOfTxPduMap()) || ((lErrorId != CANTP_E_NO_ERROR)))                                 /* PRQA S 2996 */ /* MD_CanTp_OperationAlwaysFalse */ /* COV_CANTP_ERROR_ID_VALID */
  {
    lErrorId = CANTP_E_INVALID_TX_ID;
  }
  else if ((lErrorId == CANTP_E_NO_ERROR) && (!CanTp_IsInitialized(0, partIdx)))                                       /* PRQA S 2995 */ /* MD_CanTp_OperationAlwaysTrue */ /* COV_CANTP_ERROR_ID_VALID */
  {
    lErrorId = CANTP_E_UNINIT;
    /* Note: Currently each array of partition-dependent TxSemaphore-C-structs is as large as the range of all
             partition-independent Tx-Pdus. Thus there is no indirection to a partition-specific TxSemaphore-C-struct-
             array-index and thus, there is no further check or mapping of TxPduId, here! */
  }
  else
#endif
  {
    ...
    CanTp_InitConnId(connId, CanTp_Data_TxSemaphore(TxPduId, partIdx).Handle, partIdx)
    lSduDir = CanTp_Data_TxSemaphore(TxPduId, partIdx).Direction;

    /* release semaphore */
    CanTp_Data_TxSemaphore(TxPduId, partIdx).Handle    = CANTP_INVALID_HDL;                                            /* SBSW_CANTP_TXCONFIRMATION_WRITE_TXSEMAPHORE */
    CanTp_Data_TxSemaphore(TxPduId, partIdx).Direction = CANTP_DIR_NONE;                                               /* SBSW_CANTP_TXCONFIRMATION_WRITE_TXSEMAPHORE */

#if (CANTP_TRANSMIT_QUEUE == STD_ON)
    /* ... and process next queue entry, if there is one */
    if (!CanTp_TxQueueIsEmpty(TxPduId, partIdx))
    {
      CanTp_TxQueueProcess(TxPduId, partIdx);
    }
#endif
    ...
    if (connId.Hdl != CANTP_INVALID_HDL)
    {
      if (lSduDir == CANTP_DIR_TX)
      {
#if (CANTP_TX_OPTIMIZATION != CANTP_OPT_ZERO_SDU)                                                                      /* COV_CANTP_ROBUSTNESS_CASE TX */
        /* TX direction */
        if (CanTp_TxCfg_GetTxConfPduId(connId) == TxPduId)                                                             /* PRQA S 2991, 2995 */ /* MD_CanTp_ExpressionAlwaysTrue, MD_CanTp_OperationAlwaysTrue */ /* COV_CANTP_INVALID_TXCONFPDU */
        {
          CanTp_TxStatePtrType pTpTxState;
          ...
          if (pTpTxState != NULL_PTR)
          {
            ...
            CanTp_ApplFrameTxConfirmation(CanTp_TxCfg_GetLoLayerTxPduId(connId))                                       /* SBSW_CANTP_APPLNOTIFICATION_CALL_PDUINFO */
...
#if (CANTP_RX_OPTIMIZATION != CANTP_OPT_ZERO_SDU)                                                                      /* COV_CANTP_ROBUSTNESS_CASE TX */
        /* RX direction */
        if (CanTp_RxCfg_GetTxFcConfPduId(connId) == TxPduId)                                                           /* PRQA S 2991, 2995 */ /* MD_CanTp_ExpressionAlwaysTrue, MD_CanTp_OperationAlwaysTrue */ /* COV_CANTP_INVALID_TXCONFPDU */
        {
          CanTp_RxStatePtrType pTpRxState;
          ...
          if (pTpRxState != NULL_PTR)
          {
            ...
            CanTp_ApplFrameTxConfirmation(CanTp_RxCfg_GetLoLayerTxFcPduId(connId))                                     /* SBSW_CANTP_APPLNOTIFICATION_CALL_PDUINFO */
...
# if (CANTP_INVALIDHNDOFRXSDUSNV2HDL == STD_ON)
            || (CanTp_IsInvalidHndOfRxSduSnv2Hdl(id))
# endif
          )                                                                                                            /* COV_CANTP_POSTBUILD_DELETED */
  {
    lErrorId = CANTP_E_INVALID_RX_ID;
  }  
  else if (!CanTp_IsInitialized(0, CanTp_GetPartIdxOfRxNSduSnv(id)))
  {
    lErrorId = CANTP_E_UNINIT;
  }
...
# endif
  {
    ...
    /* --- Get SDU Handle --- */
    CanTp_InitConnId(connId, CanTp_RxGetSduHdlBySduId(id), CanTp_GetPartIdxOfRxNSduSnv(id));                           /* PRQA S 2983 */ /* MD_CanTp_RedundantAssignment */
    ...
    if (CanTp_RxChannelIsIdle(connId) == TRUE)
    {
      ...
      if (parameter == TP_STMIN)
      {
        ...
        {
          CanTp_Data_DynFcParam(connId).STmin = (uint8)value;                                                          /* SBSW_CANTP_CHANGEPARAMETER_WRITE_DYNFCPARAM_INDEX */ /* SBSW_CANTP_CHANGEPARAMETER_WRITE_DYNFCPARAM_SIZE */
          lReturnValue = E_OK;
        }
      }
      ...
      {
        CanTp_Data_DynFcParam(connId).BS = (uint8)value;                                                               /* SBSW_CANTP_CHANGEPARAMETER_WRITE_DYNFCPARAM_INDEX */ /* SBSW_CANTP_CHANGEPARAMETER_WRITE_DYNFCPARAM_SIZE */
        lReturnValue = E_OK;
      }
    }
    /* else: busy reception */
...
# endif
  {
    ...
    if (parameter == TP_STMIN)
    {
      *value = CanTp_RxGetCurrentSTmin(connId);                                                                        /* SBSW_CANTP_READPARAMETER_WRITE_VALUE */
    }
    else /* (parameter == TP_BS) */
    {
      *value = CanTp_RxGetCurrentBS(connId);                                                                           /* SBSW_CANTP_READPARAMETER_WRITE_VALUE */
    }
  }
 ...
 **********************************************************************************************************************/
/* PRQA S 6030, 6080 1 */ /* MD_MSR_STCYC, MD_MSR_STMIF */
FUNC(Std_ReturnType, CANTP_CODE) CanTp_CancelTransmit(PduIdType TxSduId)
{
  /* ----- Local Variables --------------------------------------------------------------------- */
  Std_ReturnType lReturnValue = E_NOT_OK;
  uint8 lErrorId = CANTP_E_NO_ERROR;  
...
# if (CANTP_DEV_ERROR_DETECT == STD_ON)                                                                                /* COV_CANTP_DET_CHECK TX */
  ...
  else if (    (TxSduId >= CanTp_Cfg_GetNumTxSduIds())                                                                 
# if (CANTP_INVALIDHNDOFTXSDUSNV2HDL == STD_ON)
           || (CanTp_IsInvalidHndOfTxSduSnv2Hdl(TxSduId))
# endif
          )                                                                                                            /* COV_CANTP_POSTBUILD_DELETED */
...
# endif
  {
    ...
    /* --- Get SDU Handle --- */
    CONST(PduIdType, AUTOMATIC) lTxSduHdl = CanTp_TxGetSduHdlBySduId(TxSduId);                                         /* PRQA S 2983 */ /* MD_CanTp_RedundantAssignment */

    CanTp_InitConnId(connId, lTxSduHdl, CanTp_GetPartIdxOfTxNSduSnv(TxSduId));                                         /* PRQA S 2983 */ /* MD_CanTp_RedundantAssignment */
    ...
    if (CanTp_TxCfg_HasTxCancel(connId))                                                                               /* PRQA S 2741, 2742 */ /* MD_CanTp_ExpressionAlwaysTrue, MD_CanTp_ExpressionAlwaysFalse */ /* COV_CANTP_TC_ACTIVATION */
    {
      ...
      if (pTpTxState != NULL_PTR)                                                                                      /* COV_CANTP_TC_ACTIVATION */
      {
        ...
        if (pTpTxState->DataLength > CanTp_TxCfg_GetMaxSFDataLength(connId))                                           /* COV_CANTP_TC_ACTIVATION */
        {
...
#  if (CANTP_INVALIDHNDOFRXSDUSNV2HDL == STD_ON)
            || (CanTp_IsInvalidHndOfRxSduSnv2Hdl(CanTpRxSduId))
#  endif
          )                                                                                                            /* COV_CANTP_POSTBUILD_DELETED */
  {
    lErrorId = CANTP_E_INVALID_RX_ID;
  }
  else if (!CanTp_IsInitialized(0, CanTp_GetPartIdxOfRxNSduSnv(CanTpRxSduId)))
  {
    lErrorId = CANTP_E_UNINIT;
  }
  else
# endif
  {
    ...
    /* --- Get SDU Handle --- */
    CanTp_InitConnId(connId, CanTp_RxGetSduHdlBySduId(CanTpRxSduId), CanTp_GetPartIdxOfRxNSduSnv(CanTpRxSduId));
...
# if (CANTP_DEV_ERROR_DETECT == STD_ON)                                                                                /* COV_CANTP_DET_CHECK TX */
  ...
  else if (    (CanTpTxSduId >= CanTp_Cfg_GetNumTxSduIds())                                                            /* PRQA S 2004 */ /* MD_MSR_EmptyClause */
#  if (CANTP_INVALIDHNDOFTXSDUSNV2HDL == STD_ON)
            || (CanTp_IsInvalidHndOfTxSduSnv2Hdl(CanTpTxSduId))
#  endif
          )                                                                                                            /* COV_CANTP_POSTBUILD_DELETED */
  {
    lErrorId = CANTP_E_INVALID_TX_ID;
  }
  else if (!CanTp_IsInitialized(0, CanTp_GetPartIdxOfTxNSduSnv(CanTpTxSduId)))
  {
    lErrorId = CANTP_E_UNINIT;
  }
  else
# endif
  {
    /* ----- Implementation -------------------------------------------------------------------- */
    CanTp_PartIdxType partIdx = CanTp_GetPartIdxOfTxNSduSnv(CanTpTxSduId);
    CanTp_TxStatePtrType pTpTxState;
    CanTp_ConnIdType connId;

    /* --- Get SDU Handle --- */
    CanTp_InitConnId(connId, CanTp_TxGetSduHdlBySduId(CanTpTxSduId), partIdx);
...
/* Justification for module-specific MISRA deviations:
  ...
  MD_CanTp_RedundantAssignment:
    Reason:     Maintain readability and/or avoid warnings from some compilers.
...
\ID COV_CANTP_RESERVED_STMIN
  \ACCEPT TX tx tx
  \REASON Usage of reserved STmin can be disabled by precompile switch, so in this case, not all conditions will be fullfilled.
...
\ID COV_CANTP_POSTBUILD_DELETED
  ...
  \ACCEPT TF tx tf
  \ACCEPT TF tf xf
  \REASON This runtime check requires that CanTp[Rx|Tx]Sdus have been deleted at post-build time, which is not part of all post-build configurations
...
\ID COV_CANTP_INVALID_TXCONFPDU
  \ACCEPT TX
  \REASON In some configurations, the received TxConfPdu will always match the configured TxConfPdu here, because validity is already ensured by previous checks. This happens especially when only one Sdu per direction is configured.

\ID COV_CANTP_ERROR_ID_VALID
  \ACCEPT TX
  \ACCEPT XF
  \ACCEPT XX
  \ACCEPT TF tf xf
  \ACCEPT TF tx tf
  \REASON In some configurations, CanTp_Util_GetPartIdx(), CanTp_Util_GetPartIdxOfRxPduId() or CanTp_Util_GetPartIdxOfTxPduId() will never return CANTP_E_INVALID_TX_ID. This happens especially when only one Sdu per direction or no partition is configured.
...
\ID SBSW_CANTP_INTERNAL_WRITE_CALCBS
  \DESCRIPTION    Write access to the CanTp_CalcBS array, indexed by the ConnId.Hdl.
                  ConnId.Hdl is part of the struct referenced by the pTpRxState pointer,
                  which is a function parameter here and expected to refer to a valid and initialized channel state.
                  This implies that ConnId.Hdl is a valid TxHdl < CanTp_Cfg_GetNumRxHdls(), which is mapped to
...
\ID SBSW_CANTP_TASK_CALL_RXSTATE
  \DESCRIPTION    In CanTp_MainFunctionRx, the pTpRxState pointer is used as function parameter to other internal functions.
                  ...
                  state is initialized (ConnId.Hdl != invalid).
  \COUNTERMEASURE \N -
...
\ID SBSW_CANTP_RXINIT_WRITE_TXSEMAPHORE
  \DESCRIPTION    Write access to the CanTp_TxSemaphores array.
                  The index is read from CanTp_RxCfg_GetTxFcConfPduId(ConnId.Hdl).
                  ConnId.Hdl is part of the struct referenced by the pTpRxState pointer, 
                  which is expected to refer to a valid Rx state.
...
\ID SBSW_CANTP_RXINIT_WRITE_CHANNELMAP
  \DESCRIPTION    With dynamic channel assignment,
                  the ConnId.Hdl from the Rx state struct is used as index for CanTp_RxChannelMap.
                  CanTp_RxInit expects the state struct pointer to be valid and initialized, 
                  which implies that ConnId.Hdl is a valid RxHdl < CanTp_Cfg_GetNumRxHdls().
...
\ID SBSW_CANTP_TXINIT_WRITE_TXSEMAPHORE
  \DESCRIPTION    Write access to the CanTp_TxSemaphores array. The index is read by CanTp_TxCfg_GetTxConfPduId(ConnId).
                  The TxHdl in ConnId.Hdl is part of the struct referenced by the pTpTxState pointer, 
                  which is expected to refer to a valid Tx state.
                  The value read from the configuration must always be < CanTp_GetSizeOfTxSemaphores.
  \COUNTERMEASURE \R [CM_CANTP_TXCFG_TXCONFPDUIDS]

\ID SBSW_CANTP_TXINIT_WRITE_CHANNELMAP
  \DESCRIPTION    With dynamic channel assignment, the ConnId.Hdl of the Tx state struct is used as index for CanTp_TxChannelMap.
...
\ID SBSW_CANTP_TXSTART_CALL_TXSTATE
  \DESCRIPTION    In CanTp_TxStartConnection an internal function is called, where the TpTxState parameter is retrieved
                  ...
                  CanTp_Transmit. There it is derived from the TxSduId parameter via the CanTp_TxSduSnv2Hdl indirection
                  table (macro CanTp_TxGetSduHdlBySduId) and CanTp_TxGetChannelPtr.
                  The range and the availability (in case of post build) of TxSduId is verified by DET.
  \COUNTERMEASURE \N Qualified use-case CSL03 of ComStackLib
...
\ID SBSW_CANTP_TXINIT_CALL_TXSTATE
  \DESCRIPTION    pTpTxState is used as function parameter to CanTp_TxInit in order to reset the connection. The pointer is
                  ...
                  build) of TxSduId is verified by DET.
  \COUNTERMEASURE \N Qualified use-case CSL03 of ComStackLib
...
\ID SBSW_CANTP_TXQUEUCANCEL_CALL_RX
  \DESCRIPTION    A Call to CanTp_TxQueueCancel expects a valid Rx or Tx handle, depending on the direction. Here the
                  function is called with direction = Rx, and ConnId.Hdl is part of the struct referenced by the pTpRxState
                  pointer, which is a function parameter here and expected to refer to a valid Rx state.
  \COUNTERMEASURE \N -

\ID SBSW_CANTP_TXQUEUCANCEL_CALL_TX
  \DESCRIPTION    A Call to CanTp_TxQueueCancel expects a valid Rx or Tx handle, depending on the direction. Here the
                  function is called with direction = Tx, and ConnId.Hdl is part of the struct referenced by the pTpTxState
                  pointer, which is a function parameter here and expected to refer to a valid Rx state.
  \COUNTERMEASURE \N -
...
\ID SBSW_CANTP_TXTRANSMITFRAME_WRITE_SDUDATAPTR
  \DESCRIPTION    Write access to SduDataPtr, either with constant or calculated index. The pointer is also used in
                  ...
                  - the max pointer offset for write accesses and usage with MemCopy is
                    ...
                    ----------+-----------+---------------+----------------------++-----------------------------------------------
                    .. SF ... | ... 1 ... | ..... 0 ..... | MaxPayloadLength ... || (MaxPayloadLength + 1) <= CANTP_MAX_FRAME_LENGTH
                    .. SF ... | ... 1 ... | ..... 1 ..... | MaxPayloadLength ... || (MaxPayloadLength + 2) <= CANTP_MAX_FRAME_LENGTH
                    .. LSF... | ... 2 ... | ..... 0 ..... | MaxPayloadLength - 1 || (MaxPayloadLength + 1) <= CANTP_MAX_FRAME_LENGTH
                    .. LSF... | ... 2 ... | ..... 1 ..... | MaxPayloadLength - 1 || (MaxPayloadLength + 2) <= CANTP_MAX_FRAME_LENGTH
                    .. FF.... | ... 2 ... | ..... 0 ..... | MaxPayloadLength - 1 || (MaxPayloadLength + 1) <= CANTP_MAX_FRAME_LENGTH
                    .. FF.... | ... 2 ... | ..... 1 ..... | MaxPayloadLength - 1 || (MaxPayloadLength + 2) <= CANTP_MAX_FRAME_LENGTH
                    .. LFF... | ... 6 ... | ..... 0 ..... | MaxPayloadLength - 5 || (MaxPayloadLength + 1) <= CANTP_MAX_FRAME_LENGTH
                    .. LFF... | ... 6 ... | ..... 1 ..... | MaxPayloadLength - 5 || (MaxPayloadLength + 2) <= CANTP_MAX_FRAME_LENGTH
                    .. CF.... | ... 1 ... | ..... 0 ..... | MaxPayloadLength ... || (MaxPayloadLength + 1) <= CANTP_MAX_FRAME_LENGTH
                    .. CF.... | ... 1 ... | ..... 1 ..... | MaxPayloadLength ... || (MaxPayloadLength + 2) <= CANTP_MAX_FRAME_LENGTH
...
\ID SBSW_CANTP_STOPSEPARATIONTIME_WRITE_TXSTATE
  \DESCRIPTION    The TpTxState pointer is used as function parameter and for write access. The pointer is retrieved by
                  ...
                  (in case of post build) of TxSduId is verified by DET.
  \COUNTERMEASURE \N Qualified use-case CSL03 of ComStackLib

\ID SBSW_CANTP_RXGETFREECHANNEL_WRITE_CHANNELMAP
  \DESCRIPTION    Write access to the CanTp_RxChannelMap array, indexed by the ConnId.Hdl parameter.
                  CanTp_RxGetFreechannelPtr expects ConnId.Hdl to be a valid RxHdl < CanTp_Cfg_GetNumRxHdls(),
                  which is mapped to CanTp_GetSizeOfRxSduCfg() in CanTp_Priv.h.
...
\ID SBSW_CANTP_RXGETFREECHANNEL_WRITE_STATE
  \DESCRIPTION    Write access to pTpRxState pointer, which is initialized before, depending on the channel assignment:
                  ...
                  - static: pointer is set to CanTp_RxState array, indexed by ConnId.Hdl parameter.
                    ConnId.Hdl is expected to be a valid RxHdl < CanTp_Cfg_GetNumRxHdls(),
                    which is mapped to CanTp_GetSizeOfRxSduCfg() in CanTp_Priv.h.
...
\ID SBSW_CANTP_TXGETFREECHANNEL_WRITE_CHANNELMAP
  \DESCRIPTION    Write access to the CanTp_TxChannelMap array, indexed by the ConnId.Hdl parameter.
                  CanTp_TxGetFreechannelPtr expects ConnId.Hdl to be a valid TxHdl < CanTp_Cfg_GetNumTxHdls(),
                  which is mapped to CanTp_GetSizeOfTxSduCfg() in CanTp_Priv.h.
...
\ID SBSW_CANTP_TXGETFREECHANNEL_WRITE_STATE
  \DESCRIPTION    Write access to pTpTxState pointer, which is initialized before, depending on the channel assignment:
                  ...
                  - static: pointer is set to CanTp_TxState array, indexed by ConnId.Hdl parameter.
                    ConnId.Hdl is expected to be a valid TxHdl < CanTp_Cfg_GetNumTxHdls(),
                    which is mapped to CanTp_GetSizeOfTxSduCfg() in CanTp_Priv.h.
...
\ID SBSW_CANTP_CANIFRXINDICATIONCF_WRITE_TXSEMAPHORE
  \DESCRIPTION    Write access to CanTp_TxSemaphores array.
                  The index is read by CanTp_RxCfg_GetTxFcConfPduId(ConnId.Hdl).
                  ConnId.Hdl is a parameter and expected to be a valid RxHdl < CanTp_Cfg_GetNumRxHdls(),
                  which is mapped to CanTp_GetSizeOfRxSduCfg() in CanTp_Priv.h and, thus, can be assumed to be in range.
                  The write access only occurs for flow controls, i.e. the TA-Type of the connection must be 'physical'.
                  The value read from the configuration must always be < CanTp_GetSizeOfTxSemaphores.
  \COUNTERMEASURE \R [CM_CANTP_RXCFG_TXFCCONFPDUIDS]
...
\ID SBSW_CANTP_CANIFRXINDICATION_CALL_PDUINFO
  \DESCRIPTION    The passed pointers are valid, since they both point to local PduInfoType structs.
                  ...
                  from CanTp_RxPduMap to CanTp_RxSduCfg/CanTp_TxSduCfg:
                   - CanTp_GetRxPdu2RxSduInd
                   - CanTp_GetRxPdu2RxSduStartIdxOfRxPduMap
                   - CanTp_GetRxPdu2RxSduEndIdxOfRxPduMap
                   - CanTp_GetRxPdu2TxSduInd
                   - CanTp_GetRxPdu2TxSduStartIdxOfRxPduMap
                   - CanTp_GetRxPdu2TxSduEndIdxOfRxPduMap
...
\ID SBSW_CANTP_CUSTOMADDRCALLOUT_FCTPTR
  \DESCRIPTION    An custom addressing callout is called by dereferencing a pointer from the component's generated
                  data. SDUs configured with custom addressing are also expected to be configured with a valid callout.
  \COUNTERMEASURE \R [CM_CANTP_CUSTOMADDRCALLOUTFCT]
...
\ID SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PSDUMETADATA
  \DESCRIPTION    The parameter pSduMetadata passed to CanTp_Util_RxAssembleSduMetadataAndFcPduMetadata must be a
                  pointer to an array of length CANTP_MAX_SDU_METADATA_LENGTH. CANTP_MAX_SDU_METADATA_LENGTH is
                  generated based on the available addressing modes.
  \COUNTERMEASURE \R [CM_CANTP_MAXSDUMETADATALENGTH]

\ID SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PSDUMETADATALENGTH
  \DESCRIPTION    The parameter pSduMetadataLength passed to CanTp_Util_RxAssembleSduMetadataAndFcPduMetadata is
                  expected to be a valid pointer.
  \COUNTERMEASURE \N -

\ID SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PFCPDUMETADATA
  \DESCRIPTION    The parameter pFcPduMetadata passed to CanTp_Util_RxAssembleSduMetadataAndFcPduMetadata must be a
                  pointer to an array of length CANTP_MAX_PDU_METADATA_LENGTH. CANTP_MAX_PDU_METADATA_LENGTH is
                  generated based on the available addressing modes.
  \COUNTERMEASURE \R [CM_CANTP_MAXPDUMETADATALENGTH]
...
\ID SBSW_CANTP_TXGETFREECHANNEL_CALL_PTR
 ...
 \ID SBSW_CANTP_TXSDUMETADATAMATCH_CALL_PTRS
  \DESCRIPTION    The pointers pPayloadInfo and pMetadataInfo are assumed to be valid and forwarded unchanged to the
                  CanTp_Util_TxSduMetadataFromFcPduMatch function.
  \COUNTERMEASURE \R The pTpTxState pointer is checked for validity before the function call takes place.

 \ID SBSW_CANTP_RXSDUMETADATAMATCH_CALL_PTRS
  \DESCRIPTION    The pointers pPayloadInfo and pMetadataInfo are assumed to be valid and forwarded unchanged to the
                  CanTp_Util_RxSduMetadataFromDataPduMatch function.
  \COUNTERMEASURE \R The pTpRxState pointer is checked for validity before the function call takes place.
...
\ID SBSW_CANTP_RXASSEMBLESDUMETADATA_CALL_PTRS
  \DESCRIPTION    The pointers pPayloadInfo and pMetadataInfo are assumed to be valid and forwarded unchanged to the
                  CanTp_Util_RxAssembleSduMetadataAndFcPduMetadata function. The passed pSduMetadata points to an array
                  of length CANTP_MAX_SDU_METADATA_LENGTH and the pLength is also a valid pointer, since it points
                  either to a local variable or a member of a valid RxState struct.
  \COUNTERMEASURE \N -

\ID SBSW_CANTP_MEMCMP_CALL_PTRS
  \DESCRIPTION    The passed pointers are either assumed to be valid and forwarded unchanged, or point to
                  local variables.
  \COUNTERMEASURE \N -

\ID SBSW_CANTP_CALL_INTERNAL_RCHECK
...
\ID SBSW_CANTP_INIT_WRITE_PARTITION_CONFIG
  \DESCRIPTION    Write access to partition-dependent 1-element array 'Initialized' which is referenced by
                  partition-independent array 'OsApplication2PartitionIdx'. The index is '0', always.
                  The partIdx used to access the correct PartitionConfig structure is determined using function
                  CanTp_GetInitializedPartitionIdxOfOsApplication2PartitionIdx().
                  Thus, this is a qualified use case CSL05 of the ComStackLib.
  \COUNTERMEASURE \N Qualified use-case CSL05 of ComStackLib

\ID SBSW_CANTP_INTERNAL_CALL_ERROR_ID_PTR
  \DESCRIPTION    The ErrorIdPtr pointer is used in an internal function as parameter to another function. It is passed to
                  this function as a parameter and expected to valid. Pointer validity is analyzed at the function call.
  \COUNTERMEASURE \N -
...
\CM CM_CANTP_TXMAXFRAMELENGTH     For all NormalAddressing or NormalFixed or Custom (0-byte address offset) Tx N-SDUs, (CanTp_TxCfg_GetMaxPayload + 1) must be less or equal CANTP_MAX_FRAME_LENGTH
...
\CM CM_CANTP_CUSTOMADDRCALLOUTFCT All Rx and Tx SDUs configured with custom addressing shall be configured with a custom addressing callout function.
