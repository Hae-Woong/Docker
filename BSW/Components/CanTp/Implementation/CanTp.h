 ...
 *                        visath  ESCAN00097790: Compiler warning:  'TxSduId' : unreferenced formal parameter
 ...
 *  05.00.00  2021-07-01  viswnk  SWAT-986       The CanTp implements custom meta data interpretation for use-specific addressing
 *                        visath  SWAT-981       Simultaneous generic connections over the same N-PDU
 *  05.01.00  2022-08-26  visore  SWAT-1802      Support non-strict length checks
 *  05.02.00  2022-12-01  visore  SWAT-2097      Support CanTp failure indication callouts
 *  06.00.00  2022-05-02  viswnk  SWAT-1739      Implementation of MultiPartition Support
 *            2022-09-28  viswnk  ESCAN00112544  Compiler errors: Unresolved external symbols _CanTp_TxInit, _CanTp_TxGetActiveChannelPtr
 *            2022-10-17  viswnk  ESCAN00092107  Stop reporting a DET error if PduR_CanTpCopyRxData or PduR_CanTpStartOfReception returns CANTP_BUFREQ_E_NOT_OK
 *            2023-04-06  viswnk  ESCAN00099959  Compiler error: undefined reference to `CanTp_IsTxSduCfgIndUsedOfRxPduMap'
 *                        viswnk  ESCAN00105569  Compiler error: Undefined CanTp_Cfg_GetSduHdlByRxPduAddress
 *            2023-05-30  viswnk  ESCAN00114739  MISRA analysis on CanTp issues warning "Functions are indirectly recursive" for function CanTp_TxQueueProcess()
 *********************************************************************************************************************/
#if !defined (CANTP_H)
#define CANTP_H
...
#define CANTP_SID_PREINIT                        (0x37u)
...
#define CANTP_E_TX_TRANSMIT_ERROR                (0xD7u)   /* !< Error code: transmission of at least one frame failed   (Vector specific) */
#define CANTP_E_NO_PREINIT                       (0xE0u)   /* !< Error code: Pre-initialization was not executed         (Vector specific) */
#define CANTP_E_OSAPP2MEMMAPPING                 (0xE3u)   /* !< Error code: Failed to look up partition of OsApp.       (Vector specific) */
...
#if (!defined (CANTP_APPL_RX_SDU_INDICATION))
# define CANTP_APPL_RX_SDU_INDICATION            STD_OFF
#endif

#if (!defined (CANTP_APPL_TX_SDU_CONFIRMATION))
# define CANTP_APPL_TX_SDU_CONFIRMATION          STD_OFF
#endif
 ...
 *                 The ConfigPtr is ignored in case of multi partition and stored using CanTp_PreInit(), otherwise.
 *  \param[in]     ConfigPtr              Pointer to CanTp configuration data
 *  \context       TASK|ISR2
 *  \reentrant     FALSE
 *  \synchronous   TRUE
 *  \pre           Module is uninitialized.
 *  \pre           CanTp_InitMemory has been called unless CanTp_SetInitialized(FALSE) is called by start-up code.
 *  \trace         CREQ-102794
 **********************************************************************************************************************/
 ...
 * CanTp_PreInit()
 **********************************************************************************************************************/
/*! \brief         Initializes the CanTp in case multi partition is configured
 *  \details       This function stores the ConfigPtr and sets the module state to pre-initialized.
 *  \param[in]     ConfigPtr              Pointer to CanTp configuration data
 *  \context       TASK|ISR2
 *  \reentrant     FALSE
 *  \synchronous   TRUE
 ...
 *  \param[in]     TxSduId           Tx N-SDU Id of the connection, which shall be used for transmission
 ...
 *  \param[in]     TxSduId           Tx N-SDU Id of the connection, which shall be canceled
