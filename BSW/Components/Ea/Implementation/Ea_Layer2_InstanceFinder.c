...
typedef struct
{
  ...
  EaIf_StateType EaIf_StateMachine;
...
EA_LOCAL VAR(EaIf_ComponentParameterType, EA_FAST_DATA) EaIf_ComponentParameter =
        ...
        EaIh_InstanceVarPointerType PreviousInstance)
{
  ...
  switch (EaIh_GetInstanceStatus(Instance)) /* SBSW_EA_46 */
  {
    ...
    case INSTANCE_STATUS_INCONSISTENT:
      ...
      /* #25 No else necessary. Special case with inconsistent header and trailer value. Instance is returned as last job failed
         and it can not be checked if instance is more up to date */
      break;

    default :/* #30 INSTANCE_STATUS_OK and INSTANCE_STATUS_INVALID */
      /* #31 CycleCounter of both instances have to be compared to each other */
      if (EaIh_CompareTo(PreviousInstance, Instance) == TRUE) /* SBSW_EA_46 */
      {
        /* #32 Both instances have same age. Search has to be continued */
        *PreviousInstance = *Instance; /* SBSW_EA_42 */
        retVal = FALSE;
      }
      else
      {
        /* #33 Search finished. Previous instance is more up to date: previousInstance will be returned */
        *Instance = *PreviousInstance; /* SBSW_EA_41 */
      }
      break;
  }
