...
typedef struct
{
  ...
  EaWr_InstStateType EaWrInst_StateMachine;
...
EA_LOCAL VAR(EaWrInst_ComponentParameterType, EA_FAST_DATA) EaWrInst_ComponentParameter =
 ...
 * \trace DSGN-Ea22975
 */
EA_LOCAL FUNC(void, EA_PRIVATE_CODE) EaWrInst_InitiateVerifyState(EaWr_InstStateType nextState);
 ...
 * \reentrant  FALSE
 */
EA_LOCAL FUNC(void, EA_PRIVATE_CODE) EaWrInst_ProcessStateWriteTrailerAndEntire(EaWr_InstStateType nextState);
...
EA_LOCAL FUNC(void, EA_PRIVATE_CODE) EaWrInst_InitiateNextWriteState(void)
{
  ...
  if (EaIh_GetInstanceLength(&EaWrInst_Instance) > (2u * EaPh_GetPartitionWriteAlignment())) /* SBSW_EA_64 */
  {
