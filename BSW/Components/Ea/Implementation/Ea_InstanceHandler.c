...
/* Own memory section for the internal read buffer which will be forwarded to underlying EEPROM driver.
 * To handle possible alignment requirements from the underlying module */
#define EA_START_SEC_VAR_NO_INIT_INTERNAL_READ_BUFFER
#include "Ea_MemMap.h"     /* PRQA S 5087 */ /* MD_MSR_MemMap */

EA_LOCAL  VAR(uint8, EA_APPL_DATA) EaIh_ReadBuffer[EA_MAX_READ_ALIGNMENT];

#define EA_STOP_SEC_VAR_NO_INIT_INTERNAL_READ_BUFFER
#include "Ea_MemMap.h"     /* PRQA S 5087 */ /* MD_MSR_MemMap */

/* Own memory section for the internal write buffer which will be forwarded to underlying EEPROM driver.
 * To handle possible alignment requirements from the underlying module */
#define EA_START_SEC_VAR_NO_INIT_INTERNAL_WRITE_BUFFER
#include "Ea_MemMap.h"     /* PRQA S 5087 */ /* MD_MSR_MemMap */

EA_LOCAL  VAR(uint8, EA_APPL_DATA) EaIh_WriteBuffer[EA_MAX_WRITE_ALIGNMENT];

#define EA_STOP_SEC_VAR_NO_INIT_INTERNAL_WRITE_BUFFER
#include "Ea_MemMap.h"     /* PRQA S 5087 */ /* MD_MSR_MemMap */

#define EA_START_SEC_CONST_8
#include "Ea_MemMap.h"     /* PRQA S 5087 */ /* MD_MSR_MemMap */

EA_LOCAL CONST(uint8, EA_PRIVATE_CONST) EaIh_ValidManagementValues[4u] = {0xF0u, 0x69u, 0x5Au, 0xC3u};
EA_LOCAL CONST(uint8, EA_PRIVATE_CONST) EaIh_InvalidManagementValues[4u] = {0x3Cu, 0xA5u, 0x96u, 0x0Fu};

#define EA_STOP_SEC_CONST_8
#include "Ea_MemMap.h"     /* PRQA S 5087 */ /* MD_MSR_MemMap */
        ...
        Ea_AddressType InstanceStartAddress, uint16 DataLength, uint8 CycleCounter)
{
  ...
  Instance->InstanceLength = DataLength + (EA_MANAGEMENT_SIZE * 2u); /* SBSW_EA_21 */
        ...
        Ea_UserJobParameterType UserJobParameter, EaIh_CompareParameterPointerType CompareParameter)
{
  ...
  /* EaIh_WriteBuffer can not be overrunned.
   * The related buffer is as big as the biggest configured write alignment.
   * As the LoopCounter variable will always stop before reaching the Write Alignment value (Write Alignment minus 1),
   * there will always be accessed only valid elements of the buffer (which is as big as Write Alignment, or even bigger).
   */
        ...
        Ea_UserJobParameterType UserJobParameter, EaIh_CompareParameterPointerType CompareParameter)
{
  ...
  uint16 middlePartLength = Ea_AlignValue(EaIh_GetInstanceLength(Instance), EaPh_GetPartitionWriteAlignment()) -
          ( 2u * EaPh_GetPartitionWriteAlignment() ); /* SBSW_EA_203 */
        ...
        Ea_UserJobParameterType UserJobParameter, EaIh_CompareParameterPointerType CompareParameter)
{
   ...
   * This buffer is as big as the biggest configured write alignment.
   * This function will be only called if InstanceLength <= Write Alignment of the current partition (see Function EaWrInst_ProcessStateIdle()).
   * The InstanceLength  equals DataLength + 2 (as there are always 2 Management Bytes for an Instance,
   * see function EaIh_CreateInstance())
   * The for-loop will be always go through N time of the Data Length.
   * Conclusion: As before it was ensured that the buffer is bigger than DataLength+2, there will be no overrun, as even with the offset (EA_MANAGEMENT_SIZE = 1)
   *             write access will stop before reaching the size of the buffer. */
