...
EA_LOCAL VAR(EaTm_FctPtrType, EA_APPL_DATA) EaTm_ExecuteServiceStack[EA_NUMBER_OF_LAYERS];

/* Variable which represents cancel stack */
EA_LOCAL VAR(EaTm_FctPtrType, EA_APPL_DATA) EaTm_CancelServiceStack[EA_NUMBER_OF_LAYERS];
...
EA_LOCAL FUNC (EaTm_FctPtrType, EA_PRIVATE_CODE) EaTm_GetActiveTask (void)
{
  ...
  for (LoopCounter = ((sint8) EA_NUMBER_OF_LAYERS - (sint8) 1); LoopCounter >= (sint8) 0; LoopCounter--)
  {
...
FUNC (void, EA_PUBLIC_CODE) EaTm_ExecuteTask (void)
{
  ...
  do
  {
    ...
    /* #20 Check for NULL_PTR which is returned in case no sub-component is registered to TaskManager. Therefore
     * either execute the next Task or terminate the do while loop. */
  ...
  /* #30 Execute Tasks as long as EEPROM driver is not BUSY (no asynchronous EEPROM driver job was yet set up). */
  } while ((Ea_Eep_GetStatus() == MEMIF_IDLE));
  /* Here can NOT be an endless loop (e.g. EEPROM driver is always IDLE right after accepting job -> synchronous behavior)
   * as the state machine will then always continue its processing. As EEPROM driver jobs (Read, Write, Erase, Compare)
   * are defined as asynchronous by AUTOSAR it is also not expected to happen.
   * If the last EEPROM driver job is done, there will NOT be any Task inside the queue, therefore then the do while loop will
   * be terminated cause of the else branch above ("break") and Ea job will be finished. */
}
