...
EA_LOCAL VAR(Ea_StateType, EA_FAST_DATA) Ea_StateMachine = EA_STATE_UNINIT;
...
EA_LOCAL FUNC(MemIf_StatusType, EA_PRIVATE_CODE) Ea_GetLayer1Status(void)
{
  ...
  switch (Ea_UserJobParameter.JobType)
  {
    ...
    case (EA_INVALIDATE_JOB):
      ...
      break;
    default: /* Always EA_ERASE_JOB */
      retVal = EaEr_GetStatus();
      break;
  } /* PRQA S 2002 */ /* MD_MSR_EmptyClause */
  return retVal;
}
...
EA_LOCAL FUNC(void, EA_PRIVATE_CODE) Ea_ProcessStateMachine(void)
{
  /* #10 State Machine is IDLE after a new job was set up */
  /**---------- STATE IDLE ----------------------------------------------------*/
  if (Ea_StateMachine == EA_STATE_IDLE)
  {
    if (Ea_InternalFlags.CancelRequested == TRUE)
    {
      ...
      {
        /* #11 If a new job was cancelled immediately and no new job was accepted, MainFunctionBusy flag has to be cleared */
    ...
    {
      /* #12 Otherwise the requested job has to be started. State Machine is set to BUSY */
      Ea_StartJob();
      Ea_SetFlagJobRequested(FALSE);
    }
  }

  /* #20 State Machine is in state BUSY till job is finished */
  /**---------- STATE BUSY ----------------------------------------------------*/
  if (Ea_StateMachine == EA_STATE_BUSY)
  {
    if (Ea_InternalFlags.CancelRequested == TRUE)
    {
      /* #21 If a cancel event occurs during job processing, cancel all Tasks */
      EaTm_CancelTasks();

      if (Ea_InternalFlags.JobRequested == FALSE)
      {
        /* #22 If no new job was requested during cancellation, MainFunctionBusy flag has to be cleared */
        Ea_SetFlagMainFunctionBusy(FALSE);
      }
    ...
    {
      /* #23 Otherwise check if the last EEPROM driver job has been finished (IDLE) and if so, execute the next Task */
      if (Ea_Eep_GetStatus() == MEMIF_IDLE)
      {

        EaTm_ExecuteTask();

        /* #24 If there is no Task registered (Layer 1 is IDLE) and there was no cancel event during job processing, finish
         * the EA job */
...
#else
  EA_DUMMY_STATEMENT(errorId); /* PRQA S 1338, 2983, 3122 */ /* MD_MSR_DummyStmt */
#endif
...
/* ----- UNINIT CHECK --------------------------------------------------------------------------------*/
  ...
  return retVal;
} /* PRQA S 6050 */ /* MD_MSR_STCAL, MD_MSR_STMIF */
/* Ea_Write */
...
FUNC(void, EA_PUBLIC_CODE) Ea_GetVersionInfo(P2VAR(Std_VersionInfoType, AUTOMATIC, EA_APPL_DATA) VersionInfoPtr)
{
  ...
  /* ----- Development Error Checks --------------------------------------------------------------------*/
# if (EA_DEV_ERROR_DETECT == STD_ON) /* COV_EA_DET_NEVER_OFF */
...
# endif /* EA_DEV_ERROR_DETECT */
  ...
  /* ----- Development Error Report --------------------------------------------------------------------*/
  /* #20 Report development error if any occurred */
#if (EA_DEV_ERROR_REPORT == STD_ON) /* COV_EA_DET_NEVER_OFF */
  if (errorId != EA_E_NO_ERROR)
  {
...
FUNC(void, EA_PUBLIC_CODE) Ea_SetMode(MemIf_ModeType Mode)
{
  ...
  EA_DUMMY_STATEMENT(Mode); /* PRQA S 1338, 2983, 3122 */ /* MD_MSR_DummyStmt */
}
