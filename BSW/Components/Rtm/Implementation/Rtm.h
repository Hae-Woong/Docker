 ...
 *  9.00.00   2022-09-27  viszda      SWAT-2298     Persist maximum measurement results via NvM
 *            2022-10-07  viszda      SWAT-2235     Tidy up (Optimize description of enum Rtm_ItemType values)
 *  9.01.00   2022-11-15  viszda      SWAT-2378     Support for Logical Execution Time (LET)
 *  9.01.01   2023-03-15  viszda      ESCAN00114160 Unexpected high MaxRuntimeInUs reported after task response time histogram results were cleared
 *            2023-05-12  mgourabathi ESCAN00114587 MaxRunTimeInUs reported is unexpectedly lower than the percentiles calculated in the task response time histogram.
 *  10.00.00  2023-06-27  mgourabathi SWAT-2749     Improving the handling of multiple NET and FET MPs in a Task.
 *  11.00.00  2023-09-07  mgourabathi SWAT-2851     Support Flat and Net MPs for Multi-Core.
 *            2023-09-07  mgourabathi ESCAN00115299 Unexpected high MaxRuntimeInUs reported in Task Response time histogram for extended Tasks during ECU Startup.
 *            2023-09-26  mgourabathi SWAT-2887     Rtm: deprecated Rtm_Enter/ Leave, TaskFct(),IsrFct() functionality.
 *            2023-09-29  viszda      ESCAN00115783 VCA reports possible out-of-bounds accesses
 *********************************************************************************************************************/
...
#if (RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_ON) || (RTM_TASK_STACK_USAGE == STD_ON) || (RTM_NUMBER_OF_TASKRESPONSETIME_PERCENTILES > 0) /* COV_RTM_MSR_UNSUPPORTED */
# include "Os.h"
...
#define RTM_SID_STARTFLATMP                         ((uint8)0x18u)
#define RTM_SID_STOPFLATMP                          ((uint8)0x19u)
...
#define RTM_RESP_TRIGGER_READING                    (0x00000008uL)
...
#define RTM_MP_TYPE_TASK_EXTENDED                   (0x03u)
...
# define Rtm_StartGrossMP(id)
# define Rtm_StopGrossMP(id)
...
extern VAR(uint32, RTM_VAR_INIT)                    Rtm_NetActiveMeasurementId[RTM_NUMBER_OF_CORES];
# endif
# if (RTM_FLAT_EXECUTION_TIME_AVAILABLE== STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
extern VAR(uint32, RTM_VAR_INIT)                    Rtm_FlatActiveMeasurementId[RTM_NUMBER_OF_CORES];
#  if (RTM_MULTICORE_SUPPORT == STD_ON)
extern VAR(Rtm_OverheadCorrectionType, RTM_VAR_CLEARED) Rtm_OverheadCorrection[RTM_NUMBER_OF_CORES]; /* PRQA S 3408 */  /* MD_Rtm_3408 */ /*lint -e552 */ /* Suppress ID552 */
#  else
extern VAR(Rtm_OverheadCorrectionType, RTM_VAR_CLEARED) Rtm_OverheadCorrection; /* PRQA S 3408 */  /* MD_Rtm_3408 */ /*lint -e552 */ /* Suppress ID552 */
#  endif
# endif

extern VAR(uint32, RTM_VAR_INIT)                        Rtm_ActiveThreadId[RTM_NUMBER_OF_CORES];
 ...
 *  \ingroup     runtime
 *********************************************************************************************************************/
extern FUNC(void, RTM_CODE) Rtm_StartGrossMP(
 ...
 *  \ingroup     runtime
 *********************************************************************************************************************/
extern FUNC(void, RTM_CODE) Rtm_StopGrossMP(
  uint32 measurementId); /* ESCAN00076464 */
 ...
 *  \spec        requires measurementId < RTM_NUMBER_OF_ACTIVATED_MPS;
 *  \endspec
 *  \config      Available only if RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_ON && RTM_NET_EXECUTION_TIME_AVAILABLE == STD_ON && RTM_TIME_MEASUREMENT == STD_ON.
 *  \pre         Runtime measurement was started globally and this measurement section is active.
 *  \ingroup     netRuntime
 *********************************************************************************************************************/
 ...
 * Rtm_StartFlatMP()
 *********************************************************************************************************************/
/*! \brief       Starts flat runtime measurement for the given measurement section.
 *  \details     -
 *  \param[in]   measurementId           ID of the measurement section.
 *  \context     TASK|ISR
 *  \reentrant   FALSE
 ...
 *  \spec        requires measurementId < RTM_NUMBER_OF_ACTIVATED_MPS;
 *  \endspec
 *  \config      Available only if RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_ON && RTM_FLAT_EXECUTION_TIME_AVAILABLE == STD_ON && RTM_TIME_MEASUREMENT == STD_ON.
 *  \pre         Runtime measurement was started globally and this measurement section is active.
 *  \ingroup     netRuntime
 *********************************************************************************************************************/
extern FUNC(void, RTM_CODE) Rtm_StartFlatMP(
  uint32 measurementId);

/**********************************************************************************************************************
 * Rtm_StopFlatMP()
 *********************************************************************************************************************/
/*! \brief       Stops Flat runtime measurement for the given measurement section.
 *  \details     This function calculates the flat runtime for the given measurement section.
 *  \param[in]   measurementId           ID of the measurement section.
 *  \context     TASK|ISR
 *  \reentrant   FALSE
 *  \synchronous TRUE
 *  \spec        requires measurementId < RTM_NUMBER_OF_ACTIVATED_MPS;
 *  \endspec
 *  \config      Available only if RTM_FLAT_RUNTIME_SUPPORT == STD_ON && RTM_FLAT_EXECUTION_TIME_AVAILABLE == STD_ON && RTM_TIME_MEASUREMENT == STD_ON.
 *  \pre         Runtime measurement was started globally and this measurement section is active.
 *  \ingroup     netRuntime
 *********************************************************************************************************************/
extern FUNC(void, RTM_CODE) Rtm_StopFlatMP(
  uint32 measurementId);
# endif
 ...
 *  \reentrant   FALSE
 *  \synchronous TRUE
 *  \config      Available only if RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_ON && RTM_TIME_MEASUREMENT == STD_ON.
 *  \pre         -
 *  \note        1. Should be called by the TimingHook OS_VTH_SCHEDULE or in alternative APIs which have the similar functionality.
 *               2. In an OS context, DET checks are prohibited as it would result in an endless loop if a DET error occurs.
 *                  Therefore, parameters check is performed, but DET errors are not reported.
 *  \ingroup     netRuntime
 *********************************************************************************************************************/
 ...
 * Rtm_TriggerReading()
 *********************************************************************************************************************/
/*! \brief       Triggers reading of measurement results via XCP.
 *  \details     -
 *  \context     TASK|ISR
 *  \reentrant   TRUE
 *  \synchronous TRUE
 *  \pre         -
 *  \note        Only required for XCP.
 *  \ingroup     general
 *********************************************************************************************************************/
extern FUNC(void, RTM_CODE) Rtm_TriggerReading(void);
