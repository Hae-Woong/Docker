...
# if (RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
VAR(uint32, RTM_VAR_INIT)                          Rtm_ActiveThreadId[RTM_NUMBER_OF_CORES];
# endif
...
# if ((RTM_MULTICORE_SUPPORT == STD_OFF) && (RTM_FLAT_EXECUTION_TIME_AVAILABLE == STD_ON)) /* COV_RTM_MSR_UNSUPPORTED */
VAR(Rtm_OverheadCorrectionType, RTM_VAR_CLEARED) Rtm_OverheadCorrection = { 0u, 0u }; /* PRQA S 3408 */  /* MD_Rtm_3408 */ /*lint -e552 */ /* Suppress ID552 */
# endif

# if (RTM_TIME_MEASUREMENT == STD_ON)
RTM_LOCAL VAR(boolean, RTM_VAR_CLEARED)            Rtm_IsReadingRequested = FALSE; /* PRQA S 3408 */  /* MD_Rtm_3408 */
# endif
...
#if (RTM_NVM_IS_USED_FOR_RESULTS == STD_ON)
/**********************************************************************************************************************
 * Rtm_Init_PersistencyDataOfResults()
 *********************************************************************************************************************/
/*! \brief       Initializes persisted data of Rtm_Results[] if it was not recovered from flash.
 *  \details     -
 *  \context     TASK|ISR
 *  \reentrant   FALSE
 *  \synchronous TRUE
 *  \config      Available only if RTM_NVM_IS_USED_FOR_RESULTS == STD_ON.
 *  \pre         -
 *  \ingroup     init
 *********************************************************************************************************************/
RTM_LOCAL_INLINE FUNC(void, RTM_CODE) Rtm_Init_PersistencyDataOfResults(void);
#endif
...
# if (RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
/**********************************************************************************************************************
 * Rtm_NetFlat_LeaveThread()
 *********************************************************************************************************************/
/*! \brief       Stores data of a preempted net/flat MP.
 *  \details     If a net/flat MP is active on now preempted/stopped thread, this MP is also preempted.
 ...
 *  \reentrant   FALSE
 *  \synchronous TRUE
 *  \config      Available only if RTM_TIME_MEASUREMENT == STD_ON and RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_ON.
 *  \pre         -
 *  \note        Should be called by Rtm_Schedule or an API with equal behavior.
 *  \ingroup     netAndFlatExecutionTime
 *********************************************************************************************************************/
RTM_LOCAL_INLINE FUNC(void, RTM_CODE) Rtm_NetFlat_LeaveThread(
  uint32 LeaveThread,
  uint16 CoreId,
  Rtm_MeasurementTimestampType CurrentTime);

/**********************************************************************************************************************
 * Rtm_NetFlat_EnterThread()
 *********************************************************************************************************************/
/*! \brief       Resumes preempted MPs.
 *  \details     If a net/flat MP was active on the started/resumed thread, this MP is also resumed.
 *  \param[in]   EnterThread             The thread which is started/resumed.
 *  \param[in]   CoreId                  The core on which the scheduling is performed.
 *  \param[in]   CurrentTime             The timestamp when the thread was resumed.
 *  \context     TASK|ISR
 *  \reentrant   FALSE
 *  \synchronous TRUE
 *  \config      Available only if RTM_TIME_MEASUREMENT == STD_ON and RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_ON.
 ...
 *  \ingroup     netAndFlatExecutionTime
 *********************************************************************************************************************/
RTM_LOCAL_INLINE FUNC(void, RTM_CODE) Rtm_NetFlat_EnterThread(
  uint32 EnterThread,
  uint16 CoreId,
  Rtm_MeasurementTimestampType CurrentTime);

/**********************************************************************************************************************
 * Rtm_NetFlat_CheckDetError_StartMP()
 *********************************************************************************************************************/
/*! \brief       Check whether the MPs(start) have Det errors.
 *  \details     -
 *  \param[in]   MeasurementId                  The activated MP id to which shall DET error check to be performed.
 *  \return      RTM_E_NO_ERROR                 No develepment Error.
 *  \return      RTM_E_WRONG_PARAMETERS         Active Measurement Id is out of range.
 *  \return      RTM_E_INVALID_CONFIGURATION    Active Measurement Idis not in valid thread.
 *  \return      RTM_E_UNBALANCED               Nesting counter Error.
 *  \context     TASK|ISR
 *  \reentrant   FALSE
 *  \synchronous TRUE
 *  \config      Available only if RTM_TIME_MEASUREMENT == STD_ON and
 *               RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_OFF.
 *  \pre         -
 *  \ingroup     netAndFlatExecutionTime
 *********************************************************************************************************************/
RTM_LOCAL_INLINE FUNC(uint8, RTM_CODE) Rtm_NetFlat_CheckDetError_StartMP(uint32 MeasurementId);

/**********************************************************************************************************************
 * Rtm_NetFlat_CheckDetError_StartMP()
 *********************************************************************************************************************/
/*! \brief       Check whether the MPs(stop) have Det errors.
 *  \details     -
 *  \param[in]   MeasurementId                  The activated MP id to which shall DET error check to be performed.
 *  \return      RTM_E_NO_ERROR                 No develepment Error.
 *  \return      RTM_E_WRONG_PARAMETERS         Active Measurement Id is out of range.
 ...
 *  \synchronous TRUE
 *  \config      Available only if RTM_TIME_MEASUREMENT == STD_ON and
 *               RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_ON.
 *  \pre         -
 *  \ingroup     netAndFlatExecutionTime
 *********************************************************************************************************************/
RTM_LOCAL_INLINE FUNC(uint8, RTM_CODE) Rtm_NetFlat_CheckDetError_StopMP(uint32 MeasurementId);

# endif /* RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_ON */
...
/**********************************************************************************************************************
 * Rtm_TaskStackUsage_Shutdown()
 *********************************************************************************************************************/
/*! \brief       Checks if any task stack usage has changed.
 *  \details     If changed, requests to persist the new values.
 *  \context     TASK|ISR
 *  \reentrant   FALSE
 *  \synchronous TRUE
 *  \pre         -
 *  \ingroup     taskStackUsage
 *********************************************************************************************************************/
RTM_LOCAL_INLINE FUNC(void, RTM_CODE) Rtm_TaskStackUsage_Shutdown(void);
# endif

#if (RTM_TIME_MEASUREMENT == STD_ON)
#if (RTM_NVM_IS_USED_FOR_RESULTS == STD_ON)
/**********************************************************************************************************************
 *   RRRRRRRR    EEEEEEEEEE    SSSSSS    UU      UU  LL          TTTTTTTTTT    SSSSSS
 *   RR      RR  EE          SS      SS  UU      UU  LL              TT      SS      SS
 *   RR      RR  EE          SS          UU      UU  LL              TT      SS
 *   RRRRRRRR    EEEEEE        SSSSSS    UU      UU  LL              TT        SSSSSS
 *   RR    RRR   EE                  SS  UU      UU  LL              TT              SS
 *   RR      RR  EE          SS      SS  UU      UU  LL              TT      SS      SS
 *   RR      RR  EEEEEEEEEE    SSSSSS      UUUUUU    LLLLLLLLLL      TT        SSSSSS
 *********************************************************************************************************************/

/**********************************************************************************************************************
 * Rtm_Results_ClearResults()
 *********************************************************************************************************************/
/*! \brief       Clears the Rtm_Results[] and requests to persist the cleared values.
 *  \details     -
 *  \context     TASK|ISR
 *  \reentrant   FALSE
 *  \synchronous TRUE
 ...
 *  \ingroup     results
 *********************************************************************************************************************/
RTM_LOCAL_INLINE FUNC(void, RTM_CODE) Rtm_Results_ClearResults(void);

/**********************************************************************************************************************
 * Rtm_Results_Shutdown()
 *********************************************************************************************************************/
/*! \brief       Checks if any Rtm_Results[] value has changed.
 *  \details     If changed, requests to persist the new values.
 *  \context     TASK|ISR
 *  \reentrant   FALSE
 *  \synchronous TRUE
 ...
 *  \ingroup     results
 *********************************************************************************************************************/
RTM_LOCAL_INLINE FUNC(void, RTM_CODE) Rtm_Results_Shutdown(void);
# endif
...
RTM_LOCAL_INLINE FUNC(void, RTM_CODE) Rtm_Init_PersistencyDataOfResults(void)
{
  ...
  /* ----- Implementation -------------------------------------------------------------------- */
  retVal = NvM_GetErrorStatus(Rtm_GetResultsNvMSnv(), &requestResult);
...
# if (RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    ...
    Rtm_MeasurementPointInfo[measurementIndex].nextActive = RTM_INACTIVE;
...
# if (RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
  for (coreIndex = 0u; coreIndex < RTM_NUMBER_OF_CORES; coreIndex++)
  {
#  if (RTM_NET_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    Rtm_NetActiveMeasurementId[coreIndex] = RTM_INACTIVE;
#  endif
#  if (RTM_FLAT_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    Rtm_FlatActiveMeasurementId[coreIndex] = RTM_INACTIVE;
#  endif
  }
# endif
  ...
  uint8 Command)
{
  ...
  /* ----- Implementation -------------------------------------------------------------------- */
# if (RTM_XCP_IS_PRESENT == STD_ON)
  /* #10 If reading via XCP is requested and no command is set:
   *       Set trigger reading flag in response, set stop command, and reset the trigger request. */
  if ((Rtm_IsReadingRequested == TRUE)
   && (Rtm_Cmd == RTM_NULL))
  {
    Rtm_Resp |= ((uint32) RTM_RESP_TRIGGER_READING << 28u);
    command = (uint32)(RTM_CMD_STOP_MEASUREMENT & 0x000000FFu);
    Rtm_IsReadingRequested = FALSE;
  }
# endif

  /* #20 If a valid command is passed, execute it accordingly. */
  switch(command)
  {
...
# endif
  ...
  /* #30 Otherwise: */
  default :
    /* #40 If a measurement is currently running, proceed it. */
# if (RTM_MULTICORE_SUPPORT == STD_OFF)
...
# endif
       )
    {
      ...
      Rtm_StopGrossMP(activatedCpuLoadMpId);
      ...
      if ((Rtm_ThresholdCbkFctArr[activatedCpuLoadMpId] != NULL_PTR)
       && (Rtm_ThresholdTimes[activatedCpuLoadMpId] < collectedTime)) /* COV_RTM_CPU_LOAD_THRESHOLD_ALWAYS_ACTIVE_OR_INACTIVE */
      {
        (Rtm_ThresholdCbkFctArr[activatedCpuLoadMpId]) (collectedTime);
      }
    }
...
# endif
    ...
    {
      Rtm_StartGrossMP(activatedCpuLoadMpId);
    }
  ...
  Rtm_MeasurementTimestampType CurrentTime)
{
  ...
  if ((Rtm_MeasurementPointConfigInfo[MeasurementId].mpType == RTM_MP_TYPE_TASK) /* COV_RTM_TASKRESPONSE_TIME_EXTENDED_TASKS*/
   || (Rtm_MeasurementPointConfigInfo[MeasurementId].mpType == RTM_MP_TYPE_TASK_EXTENDED)) /* COV_RTM_TASKRESPONSE_TIME_EXTENDED_TASKS*/
  {
    uint32 percentileIndex;
...
#   if (RTM_MULTICORE_SUPPORT == STD_ON)
  if ((Rtm_MeasurementPointConfigInfo[MeasurementId].coreId < RTM_NUMBER_OF_CORES)
    && (coreId != Rtm_MeasurementPointConfigInfo[MeasurementId].coreId)
...
#  endif
    {
      /* #40 If the active MP started, is not in valid thread, indicate an error. */
      if (Rtm_ActiveThreadId[coreId] >= RTM_NUMBER_OF_THREADS)
      {
        errorCode = RTM_E_INVALID_CONFIGURATION;
      }
    }
...
/**********************************************************************************************************************
 * Rtm_NetFlat_CheckDetError_StopMP()
 *********************************************************************************************************************/
 /*!
  *
  * Internal comment removed.
...
#  endif
  /* #20 Otherwise: */
  {
    /* ----- Implementation -------------------------------------------------------------------- */
#  if (RTM_NESTING_COUNTER == STD_ON)
    /* #30 If the MP is stopped the first time and running on a valid thread. */
    Rtm_MeasurementNestingCtr[MeasurementId]--;
#  else
#   if (RTM_DEV_ERROR_DETECT == STD_ON)
    Rtm_MeasurementNestingCtr[MeasurementId]--;
#   endif
#  endif
    /* #40 If the active MP stopped, is not in valid thread, indicate an error. */
    if (Rtm_ActiveThreadId[coreId] >= RTM_NUMBER_OF_THREADS)
    {
      errorCode = RTM_E_INVALID_CONFIGURATION;
    }
    /* #50 If nesting counter threshold underflows, report DET error. */
#  if (RTM_DEV_ERROR_REPORT == STD_ON)
  ...
  Rtm_MeasurementTimestampType CurrentTime)
{
   ...
   *       Remember both flat and net MP which preempted and when this happened. */
  if (LeaveThread < RTM_NUMBER_OF_THREADS)
  {
#  if (RTM_NET_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    Rtm_ThreadContext[LeaveThread].prevActiveNetMP = Rtm_NetActiveMeasurementId[CoreId];
#  endif

#  if (RTM_FLAT_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    Rtm_ThreadContext[LeaveThread].prevActiveFlatMP = Rtm_FlatActiveMeasurementId[CoreId];
#  endif
...
# if (RTM_NET_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
  uint32_least i;
  uint32 locPrevActiveNet;
# endif

  /* ----- Implementation -------------------------------------------------------------------- */
  /* #10 If a Thread is started,
   *       and the Thread has interrupted a MP: */
  if (EnterThread < RTM_NUMBER_OF_THREADS)
  {
    Rtm_MeasurementTimestampType deltaTime;
    Rtm_ActiveThreadId[CoreId] = EnterThread;

#  if (RTM_FLAT_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    Rtm_FlatActiveMeasurementId[CoreId] = Rtm_ThreadContext[EnterThread].prevActiveFlatMP;
#  endif
    /* #20 Calculate the preemption time. */
#  if (RTM_NET_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    Rtm_NetActiveMeasurementId[CoreId] = Rtm_ThreadContext[EnterThread].prevActiveNetMP;

    if (Rtm_NetActiveMeasurementId[CoreId] < RTM_NUMBER_OF_ACTIVATED_MPS)
    {
      Rtm_Time_GetCounterDelta(Rtm_ThreadContext[EnterThread].preemptionTime, CurrentTime, &deltaTime, Rtm_NetActiveMeasurementId[CoreId]);
      /* #30 If all MPs are of net type, add preemption time to all interrupted NET MPs. */
      locPrevActiveNet = Rtm_NetActiveMeasurementId[CoreId];
      for (i = 0u;
        (i < RTM_NUMBER_OF_ACTIVATED_MPS) &&
        (locPrevActiveNet < RTM_NUMBER_OF_ACTIVATED_MPS) &&
        (Rtm_MeasurementPointInfo[locPrevActiveNet].threadContext == (uint32)EnterThread);
        i++)
      {
        Rtm_MeasurementPointInfo[locPrevActiveNet].intermediateTime += deltaTime;
        locPrevActiveNet = Rtm_MeasurementPointInfo[locPrevActiveNet].prevActive;
      }
    }
#  endif
#  if (RTM_FLAT_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    if (Rtm_FlatActiveMeasurementId[CoreId] < RTM_NUMBER_OF_ACTIVATED_MPS)
    {
      /* #40 If all MPs are of flat type, add preemption time to the last active flat MP. */
      Rtm_Time_GetCounterDelta(Rtm_ThreadContext[EnterThread].preemptionTime, CurrentTime, &deltaTime, Rtm_FlatActiveMeasurementId[CoreId]);
      Rtm_MeasurementPointInfo[Rtm_FlatActiveMeasurementId[CoreId]].intermediateTime += deltaTime;
    }
#  endif
  }
  /* #50 Otherwise, no new context started, therefore no MP is currently active. */
  else
  {
#  if (RTM_NET_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    Rtm_NetActiveMeasurementId[CoreId] = RTM_INACTIVE;
#  endif
#  if (RTM_FLAT_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    Rtm_FlatActiveMeasurementId[CoreId] = RTM_INACTIVE;
#  endif
    Rtm_ActiveThreadId[CoreId] = RTM_NUMBER_OF_THREADS;
  }
  RTM_DUMMY_STATEMENT(CurrentTime); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e438 */
  RTM_DUMMY_STATEMENT(CoreId); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e438 */
} /* Rtm_NetFlat_EnterThread */
  ...
  uint16 CoreIndex)
{
  ...
  for (taskIndex = 0u; taskIndex < Rtm_GetOsTaskInfoLengthOfCommonConst(CoreIndex); taskIndex++)
  {
    ...
    Rtm_GetTaskResponseTimeDataPtrOfCommonConst(CoreIndex)[taskIndex].MaxRuntimeInTicks = 0u; /* VCA_RTM_VALID_LENGTH_OF_CORE_SPECIFIC_TASKS */
    Rtm_GetTaskResponseTimeDataPtrOfCommonConst(CoreIndex)[taskIndex].OverallPercentiles = 0u; /* VCA_RTM_VALID_LENGTH_OF_CORE_SPECIFIC_TASKS */
  ...
  /* #20 Enter exclusive area 0. */
  SchM_Enter_Rtm_RTM_EXCLUSIVE_AREA_0();

  /* #30 Reset the measurement results of all task MPs on the given core. */
  for (activeMpIndex = 0u; activeMpIndex < RTM_NUMBER_OF_ACTIVATED_MPS; activeMpIndex++) /* ESCAN00114160 */
  {
    if (((Rtm_MeasurementPointConfigInfo[activeMpIndex].mpType == RTM_MP_TYPE_TASK)          /* COV_RTM_TASKRESPONSE_TIME_EXTENDED_TASKS*/
     || (Rtm_MeasurementPointConfigInfo[activeMpIndex].mpType == RTM_MP_TYPE_TASK_EXTENDED)) /* COV_RTM_TASKRESPONSE_TIME_EXTENDED_TASKS*/
#  if (RTM_MULTICORE_SUPPORT == STD_ON)
     && (Rtm_MeasurementPointConfigInfo[activeMpIndex].coreId == CoreIndex)
#  endif
       )
    {
      Rtm_Results[activeMpIndex].time = 0u;
      Rtm_Results[activeMpIndex].count = 0u;
      Rtm_Results[activeMpIndex].max = 0u;
      Rtm_Results[activeMpIndex].min = 0xFFFFFFFFu;
    }
  }
  /* #40 Leave exclusive area 0. */
  SchM_Exit_Rtm_RTM_EXCLUSIVE_AREA_0();

  /* #50 Request to persist the task response time results. */
#  if (RTM_NVM_IS_USED_FOR_TASK_RESPONSE_TIME == STD_ON)
  if (Rtm_IsTaskResponseTimeNvMHandleUsedOfCommonConst(CoreIndex)) /* COV_RTM_TASKRESPONSETIME_NVMHANDLE */
  {
    (void)NvM_SetRamBlockStatus((NvM_BlockIdType)Rtm_GetNvMTaskResponseTimeBlockSnvOfCommonConst(CoreIndex), TRUE);
...
RTM_LOCAL_INLINE FUNC(void, RTM_CODE) Rtm_TaskStackUsage_Shutdown(void)
{
  ...
  /* ----- Implementation ---------------------------------------------------------------------- */
  /* #10 Iterate over all available tasks. */
  for (osTaskIndex = 0u; osTaskIndex < RTM_NUMBER_OF_TASKS; osTaskIndex++)
  {
    ...
    /* #20 Check for each task that if the current max stack usage is higher than the cached value.
  ...
  /* #30 If any max stack usage value changed, request to persist all values. */
...
RTM_LOCAL_INLINE FUNC(void, RTM_CODE) Rtm_Results_ClearResults(void)
{
  ...
  /* ----- Implementation ---------------------------------------------------------------------- */
  /* #10 Reset all values of Rtm_Results[] and request to persist it. */
  for (mpIndex = 0u; mpIndex < RTM_NUMBER_OF_ACTIVATED_MPS; mpIndex++)
  {
    Rtm_Results[mpIndex].time = 0u;
    Rtm_Results[mpIndex].count = 0u;
    Rtm_Results[mpIndex].max = 0u;
    Rtm_Results[mpIndex].min = 0xFFFFFFFFu;
  }
...
RTM_LOCAL_INLINE FUNC(void, RTM_CODE) Rtm_Results_Shutdown(void)
{
  ...
  /* ----- Implementation ---------------------------------------------------------------------- */
  /* #10 Iterate over all activated mps.
   *       If any Rtm_Results[] value changed, request to persist all values and leave the loop. */
  for (mpIndex = 0u; mpIndex < RTM_NUMBER_OF_ACTIVATED_MPS; mpIndex++)
  {
    if (Rtm_Results[mpIndex].count > 0u)
    {
      (void)NvM_SetRamBlockStatus((NvM_BlockIdType)Rtm_GetResultsNvMSnv(), TRUE);
      break;
    }
  ...
  P2VAR(uint32, AUTOMATIC, RTM_APPL_VAR) ItemValuePtr)
{
  ...
  if (Rtm_Results[ActivatedMPId].count > 0u)
  {
    switch(ItemType)
    {
      ...
      /* #70 Otherwise, if the raltive maximum runtime is requested:
       *       If target runtime is configured for this MP:
       *         Calculate the relative maximum and return E_OK.
       *       Otherwise, return RTM_RETVAL_ITEM_NOT_AVAILABLE_FOR_MP. */
      case(RTM_ITEM_RELATIVE_MAX):
        if (Rtm_ThresholdTimes[ActivatedMPId] > 0u) /* COV_RTM_TARGETRUNTIME_FOR_ALL_MPS_ACTIVE_OR_INACTIVE */
        {
          *ItemValuePtr = (uint32)(((Rtm_Results[ActivatedMPId].max) * 100u) / (Rtm_ThresholdTimes[ActivatedMPId]));
          retVal = E_OK;
        }
        else
        {
      ...
      /* #75 Otherwise, if the relative remaining runtime is requested:
       *       If target runtime is configured for this MP:
       *         If the remaing time is smaller than 0, then report 0.
       *         Otherwise, calculate the relative remaining time and report it.
       *         Return E_OK.
       *       Otherwise, return RTM_RETVAL_ITEM_NOT_AVAILABLE_FOR_MP. */
      case(RTM_ITEM_RELATIVE_REMAINING):
        if (Rtm_ThresholdTimes[ActivatedMPId] > 0u) /* COV_RTM_TARGETRUNTIME_FOR_ALL_MPS_ACTIVE_OR_INACTIVE */
        {
          uint32 relativeMax = (uint32)(((Rtm_Results[ActivatedMPId].max) * 100u) / (Rtm_ThresholdTimes[ActivatedMPId]));
      ...
      /* #80 Otherwise, request is invalid. */
  ...
  /* #90 Otherwise, set result to 0 and return RTM_RETVAL_MP_NOT_EXECUTED_YET. */
  else
  {
    *ItemValuePtr = 0u;
  ...
  /* #100 If the result is invalidly high, set it to highest valid value 100. */
  if ((*ItemValuePtr > 100u)
   && (ItemType != RTM_ITEM_RELATIVE_MAX))
  {
    *ItemValuePtr = 100u;
  }
  ...
  P2VAR(uint32, AUTOMATIC, RTM_APPL_VAR) ItemValuePtr)
{
  ...
  if (Rtm_Results[ActivatedMPId].count > 0u)
  {
    switch(ItemType)
    {
      ...
      /* #60 Otherwise, if the relative maximum runtime is requested:
       *       If target runtime is configured for this MP:
       *         Calculate the relative maximum and return E_OK.
       *       Otherwise, return RTM_RETVAL_ITEM_NOT_AVAILABLE_FOR_MP. */
      case(RTM_ITEM_RELATIVE_MAX):
      ...
      /* #65 Otherwise, if the relative remaining runtime is requested:
       *       If target runtime is configured for this MP:
       *         If the remaing time is smaller than 0, then report 0.
       *         Otherwise, calculate the relative remaining time and report it.
       *         Return E_OK.
      ...
      /* #70 Otherwise, request is invalid, set result to 0 and return RTM_RETVAL_ITEM_NOT_AVAILABLE_FOR_MP. */
      default: /* PRQA S 2018 */ /* MD_Rtm_2018 */
        *ItemValuePtr = 0u;
        retVal = RTM_RETVAL_ITEM_NOT_AVAILABLE_FOR_MP;
      break;
...
FUNC(void, RTM_CODE) Rtm_StartGrossMP(
...
/* PRQA S 6030 3 */ /* MD_MSR_STCYC */
/* PRQA S 6010 2 */ /* MD_MSR_STPTH */
FUNC(void, RTM_CODE) Rtm_StopGrossMP(
  uint32 measurementId) /* ESCAN00076464 */
{
  /* ----- Local Variables --------------------------------------------------------------------- */
  Rtm_MeasurementTimestampType currentTime;
...
# endif
      {
        ...
        if ((Rtm_ThresholdCbkFctArr[measurementId] != NULL_PTR) && (Rtm_ThresholdTimes[measurementId] < currentTime))
        {
...
/* PRQA S 6030 3 */ /* MD_MSR_STCYC */
/* PRQA S 6010 2 */ /* MD_MSR_STPTH */
FUNC(void, RTM_CODE) Rtm_StartFlatMP(
  uint32 measurementId)
{
  /* ----- Development Error Checks ------------------------------------------------------------ */
  /* #10 With the information of measurement Id, check the DET errors,
   *      If there is an error present, indicate an DET error.*/
  uint16 coreId = Rtm_MeasurementPointConfigInfo[measurementId].coreId;
  uint8 errorCode = Rtm_NetFlat_CheckDetError_StartMP(measurementId);
...
#  endif
    /* #20 Otherwise: */
  {
    ...
    /* #30 If the MP is started the first time and running on a valid thread: Store current time. */
#  if (RTM_NESTING_COUNTER == STD_ON)
    if (Rtm_MeasurementNestingCtr[measurementId] == (sint8)1)
#  endif
    {
      /* #40 If there is an active MP of type flat execution time:
       *       Cache start time and active thread.
       *       Set the currently active MP to previously active.
       *       Set the new MP to active. */
      Rtm_MeasurementPointInfo[measurementId].threadContext = Rtm_ActiveThreadId[coreId];
      /* only update when there is flat Measurement point type*/
      Rtm_MeasurementPointInfo[measurementId].prevActive = Rtm_FlatActiveMeasurementId[coreId];
      Rtm_FlatActiveMeasurementId[coreId] = measurementId;
      Rtm_Time_GetCurrentTime(&currentTime);
      Rtm_StartTimeStamps[measurementId] = currentTime;
#  if (RTM_TIMER_OVERRUN_SUPPORT == STD_ON)
      Rtm_PrevOverrunCounterValues[measurementId] = Rtm_TimerOverrunCnt;
...
#  if (RTM_DEV_ERROR_REPORT == STD_ON)
  if (errorCode != RTM_E_NO_ERROR)
  {
    Rtm_DetReportError(RTM_SID_STARTFLATMP, errorCode);
  }
#  else
  RTM_DUMMY_STATEMENT(errorCode); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e438 */
#  endif
...
/* PRQA S 6030 3 */ /* MD_MSR_STCYC */
/* PRQA S 6010 2 */ /* MD_MSR_STPTH */
FUNC(void, RTM_CODE) Rtm_StopFlatMP(
  uint32 measurementId)
{
  /* ----- Development Error Checks ------------------------------------------------------------ */
  /* #10 With the information of measurement Id, check the DET errors,
   *     If there is an error present, indicate an DET error. */
  uint16 coreId = Rtm_MeasurementPointConfigInfo[measurementId].coreId;
  uint8 errorCode = Rtm_NetFlat_CheckDetError_StopMP(measurementId);
...
#  endif
    {
      ...
      /* #40 If the active MP is stopped. */
      if (Rtm_FlatActiveMeasurementId[coreId] == measurementId)
      {
        ...
        /* #60 Update the interrupted MP. */
        interruptedMeasId = Rtm_MeasurementPointInfo[measurementId].prevActive;

        /* #70 If the interrupted MP is of type flat execution time:
...
#  else
          if (Rtm_MeasurementPointConfigInfo[interruptedMeasId].interruptsDisabled == TRUE)
          {
            Rtm_MeasurementPointInfo[interruptedMeasId].intermediateTime = Rtm_MeasurementPointInfo[interruptedMeasId].intermediateTime + Rtm_OverheadCorrection[coreId].execTime_Flat_DisableISRs;
          }
          else
          {
            Rtm_MeasurementPointInfo[interruptedMeasId].intermediateTime = Rtm_MeasurementPointInfo[interruptedMeasId].intermediateTime + Rtm_OverheadCorrection[coreId].execTime_Flat_EnableISRs;
          }
#  endif
        }

        /* #80 Activate the interrupted MP. */
        Rtm_MeasurementPointInfo[measurementId].threadContext = RTM_NUMBER_OF_THREADS;
        Rtm_FlatActiveMeasurementId[coreId] = interruptedMeasId;
        Rtm_MeasurementPointInfo[measurementId].intermediateTime = 0u;

        /* #90 Subtract intermediateTime from current result. */
        ...
        /* #100 Check for new minimum and maximum value. */
        if (currentTime > Rtm_Results[measurementId].max)
        {
          Rtm_Results[measurementId].max = currentTime;
        }
        ...
        /* #110 Cumulate the overall runtime, reset results if overflow detected and indicate an error. */
        Rtm_Results[measurementId].time = Rtm_Results[measurementId].time + currentTime;
        Rtm_Results[measurementId].count = Rtm_Results[measurementId].count + 1u;
...
#  endif
        ...
        /* #120 Call callback if threshold exceeded. */
        if ((Rtm_ThresholdCbkFctArr[measurementId] != NULL_PTR) && (Rtm_ThresholdTimes[measurementId] < currentTime))
        {
          (Rtm_ThresholdCbkFctArr[measurementId]) (currentTime);
        }
      }
      /* #130 Otherwise, inconsistent Start/Stop sequence detected; Only the active MP is allowed to be stopped. */
      else
      {
        errorCode = RTM_E_UNBALANCED;
      }
...
#  if (RTM_DEV_ERROR_REPORT == STD_ON)
  if (errorCode != RTM_E_NO_ERROR)
  {
    Rtm_DetReportError(RTM_SID_STOPFLATMP, errorCode);
  }
#  else
  RTM_DUMMY_STATEMENT(errorCode); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */ /*lint -e438 */
#  endif
...
#  endif
    {
      Rtm_MeasurementTimestampType currentTime;
      uint32 locPreActiveId;

      /* #40 If there is an active MP of type net execution time:
       *    Cache start time and active thread.
       *    Set the currently active MP to previously active.
       *    Set the new MP to active. */
      Rtm_MeasurementPointInfo[measurementId].threadContext = Rtm_ActiveThreadId[coreId];
      Rtm_MeasurementPointInfo[measurementId].prevActive = Rtm_NetActiveMeasurementId[coreId];
      Rtm_NetActiveMeasurementId[coreId] = measurementId;
      /* Storing  active Id to the previous preempt Measurement Point as a next Id information,
       * so that every MP has pre and next Id information. */
      if (Rtm_MeasurementPointInfo[measurementId].prevActive != RTM_INACTIVE)
      {
        locPreActiveId = Rtm_MeasurementPointInfo[measurementId].prevActive;
        Rtm_MeasurementPointInfo[locPreActiveId].nextActive = measurementId;
      }
      Rtm_Time_GetCurrentTime(&currentTime);
      Rtm_StartTimeStamps[measurementId] = currentTime;
...
#  endif
    {
      ...
      uint32 interTime;
      uint32 interruptedMeasId;
      uint32 locNxtActv = RTM_INACTIVE;
      uint32 locPreActv;

      locNxtActv = Rtm_MeasurementPointInfo[measurementId].nextActive;
      locPreActv = Rtm_MeasurementPointInfo[measurementId].prevActive;

      /* #40 Calculate time difference since measurement start. */
      Rtm_Time_GetCurrentTime(&currentTime);
      interTime = Rtm_MeasurementPointInfo[measurementId].intermediateTime;
      Rtm_Time_GetCounterDelta(Rtm_StartTimeStamps[measurementId], currentTime, &currentTime, measurementId);

      /* #50 If the active MP is stopped:
       *       Update the interrupt MP and also the preActive and nextActive accordingly, such that
       *       the connection between the MPs still present. */
      if (locNxtActv != RTM_INACTIVE)
      {
        if (locPreActv != RTM_INACTIVE)
        {
          Rtm_MeasurementPointInfo[locNxtActv].prevActive = Rtm_MeasurementPointInfo[measurementId].prevActive;
          Rtm_MeasurementPointInfo[locPreActv].nextActive = Rtm_MeasurementPointInfo[measurementId].nextActive;
          interruptedMeasId = Rtm_MeasurementPointInfo[measurementId].nextActive;
        }
        else
        {
          Rtm_MeasurementPointInfo[locNxtActv].prevActive = Rtm_MeasurementPointInfo[measurementId].prevActive;
      ...
      else
      {
        if (locPreActv != RTM_INACTIVE)
        {
          Rtm_MeasurementPointInfo[locPreActv].nextActive = RTM_INACTIVE;
          interruptedMeasId = Rtm_MeasurementPointInfo[measurementId].prevActive;
        }
        else
        {
          interruptedMeasId = RTM_INACTIVE;
        }
      }

      /* Clear pre and next active info of respective MP. */
      Rtm_MeasurementPointInfo[measurementId].prevActive = RTM_INACTIVE;
      Rtm_MeasurementPointInfo[measurementId].nextActive = RTM_INACTIVE;

      /* #60 Activate the interrupted MP. */
      Rtm_MeasurementPointInfo[measurementId].threadContext = RTM_NUMBER_OF_THREADS;
      Rtm_NetActiveMeasurementId[coreId] = interruptedMeasId;
      Rtm_MeasurementPointInfo[measurementId].intermediateTime = 0u;

      /* #70 Subtract intermediateTime from current result. */
      if (interTime < currentTime)
      {
        currentTime = currentTime - interTime;
      }
      ...
      /* #80 Check for new minimum and maximum value. */
      if (currentTime > Rtm_Results[measurementId].max)
      {
        Rtm_Results[measurementId].max = currentTime;
      }
      ...
      /* #90 Cumulate the overall runtime, reset results if overflow detected and indicate an error. */
      Rtm_Results[measurementId].time = Rtm_Results[measurementId].time + currentTime;
      Rtm_Results[measurementId].count = Rtm_Results[measurementId].count + 1u;
...
#  endif
      ...
      /* #100 Call callback if threshold exceeded. */
      if ((Rtm_ThresholdCbkFctArr[measurementId] != NULL_PTR) && (Rtm_ThresholdTimes[measurementId] < currentTime))
      {
        (Rtm_ThresholdCbkFctArr[measurementId]) (currentTime);
      }
...
{
  /* ----- Implementation -------------------------------------------------------------------- */
  boolean locActiveMeasurement = FALSE;
  boolean locActiveThreadContext = FALSE;
...
#  endif
  {
    ...
    if ((CoreId < RTM_NUMBER_OF_CORES)
     ...
     && (FromThreadId < RTM_NUMBER_OF_THREADS))
    {
      /* ----- Implementation -------------------------------------------------------------------- */
#  if (RTM_NET_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
      locActiveMeasurement = ((Rtm_NetActiveMeasurementId[CoreId] != RTM_INACTIVE) ? TRUE : FALSE);
      locActiveThreadContext = ((Rtm_ThreadContext[ToThreadId].prevActiveNetMP < RTM_NUMBER_OF_ACTIVATED_MPS) ? TRUE : FALSE);
#  endif
#  if (RTM_FLAT_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
#   if (RTM_NET_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
      locActiveMeasurement = (((locActiveMeasurement == TRUE) || (Rtm_FlatActiveMeasurementId[CoreId] != RTM_INACTIVE)) ? TRUE : FALSE);
      locActiveThreadContext = (((locActiveThreadContext == TRUE) || (Rtm_ThreadContext[ToThreadId].prevActiveFlatMP < RTM_NUMBER_OF_ACTIVATED_MPS)) ? TRUE : FALSE);
#   else
      locActiveMeasurement = ((Rtm_FlatActiveMeasurementId[CoreId] != RTM_INACTIVE) ? TRUE : FALSE);
      locActiveThreadContext = ((Rtm_ThreadContext[ToThreadId].prevActiveFlatMP < RTM_NUMBER_OF_ACTIVATED_MPS) ? TRUE : FALSE);
#   endif
#  endif
      /* #15 If the active MP is interrupted or if a interrupted MP is resumed, get the current time. */
      if ((locActiveMeasurement == TRUE)
       || (locActiveThreadContext == TRUE))
      {
        Rtm_Time_GetCurrentTime(&currentTime);
      }
      /* #20 If a Thread is suspended/preempted:
       *       Store the preempted MP and the current time. */
      Rtm_NetFlat_LeaveThread(FromThreadId, CoreId, currentTime);
      ...
      Rtm_NetFlat_EnterThread(ToThreadId, CoreId, currentTime);
    }
  }
#  if (RTM_CONTROL == STD_ON)
  else
...
# if (RTM_NUMBER_OF_TASKRESPONSETIME_PERCENTILES > 0u)
       ...
       && (taskIndex < Rtm_GetOsTaskInfoLengthOfCommonConst(coreIndex))) /* COV_RTM_TASKRESPONSETIME_COREANDTASKINDEX_ALWAYS_VALID_WHEN_DET_CHECKS_ENABLED */
      {
        ...
        if (overallPercentileCount > 0u)
        {
          TaskResponseTimeHistogram->MaxRuntimeInUs = (uint32)(((uint64)(Rtm_GetTaskResponseTimeDataPtrOfCommonConst(coreIndex)[taskIndex].MaxRuntimeInTicks) * 1000u) / RTM_TICKS_PER_MILISECOND); /* VCA_RTM_STRUCT_WRT_UNCHANGED */
 ...
 *
 */
FUNC(void, RTM_CODE) Rtm_TriggerReading(void)
{
  /* #10 If Rtm's functionality is enabled: */
#if (RTM_CONTROL == STD_ON)
  if (Rtm_ControlState == RTM_CONTROL_STATE_ENABLED)
#endif
  {
    ...
    /* #30 If endless measurement is active, remember that reading is requested. */
    if ((Rtm_EndlessMeasurement == TRUE)
# if (RTM_MULTICORE_SUPPORT == STD_OFF)
     && (Rtm_LiveMeasurement == FALSE)
# endif
    ...
    {
      Rtm_IsReadingRequested = TRUE;
    }

    /* #40 Leave exclusive area 2. */
    SchM_Exit_Rtm_RTM_EXCLUSIVE_AREA_2();
#endif
  }
#if (RTM_CONTROL == STD_ON)
...
# if (RTM_NET_OR_FLAT_RUNTIME_SUPPORT == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    ...
    Rtm_MeasurementPointInfo[i].nextActive = RTM_INACTIVE;
...
#  if (RTM_NET_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    Rtm_ThreadContext[i].prevActiveNetMP = Rtm_ThreadContextInit[i].prevActiveNetMP;
#  endif
#  if (RTM_FLAT_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    Rtm_ThreadContext[i].prevActiveFlatMP = Rtm_ThreadContextInit[i].prevActiveFlatMP;
#  endif
  }
  ...
  {
#  if (RTM_NET_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    Rtm_NetActiveMeasurementId[i] = RTM_INACTIVE;
#  endif
#  if (RTM_FLAT_EXECUTION_TIME_AVAILABLE == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    Rtm_FlatActiveMeasurementId[i] = RTM_INACTIVE;
#   if (RTM_MULTICORE_SUPPORT == STD_ON) /* COV_RTM_MSR_UNSUPPORTED */
    Rtm_OverheadCorrection[i].execTime_Flat_EnableISRs = 0u;
    Rtm_OverheadCorrection[i].execTime_Flat_DisableISRs = 0u;
#   else
    Rtm_OverheadCorrection.execTime_Flat_EnableISRs = 0u;
    Rtm_OverheadCorrection.execTime_Flat_DisableISRs = 0u;
#   endif
#  endif
    Rtm_ActiveThreadId[i] = RTM_NUMBER_OF_THREADS;
  }
# endif
...
# if (RTM_IS_ANY_CPU_LOAD_MP_SET_TO_AUTOSTART == STD_ON) && (RTM_NVM_IS_USED_FOR_RESULTS == STD_ON)
      Rtm_Init_PersistencyDataOfResults();
# endif
    }
#endif /* RTM_TIME_MEASUREMENT == STD_ON */
...
/* COV_JUSTIFICATION_BEGIN
  ...
  \ACCEPT TF tx tf
  \REASON The check (Rtm_MeasurementPointConfigInfo[measurementId].coreId < RTM_NUMBER_OF_CORES) can only become false
          if at least one cross core mp is configured (not mapped to a specific core).

  \ID COV_RTM_TARGETRUNTIME_FOR_ALL_MPS_ACTIVE_OR_INACTIVE
  \ACCEPT TX
  \ACCEPT XF
  \REASON The check Rtm_ThresholdTimes[] > 0 is always true or always false depending on configuration of
          /MICROSAR/Rtm/RtmMeasurementPoint/RtmTargetRuntime.

  \ID COV_RTM_CPU_LOAD_THRESHOLD_ALWAYS_ACTIVE_OR_INACTIVE
  \ACCEPT XF xf xx
  \ACCEPT TF tx tf
  \REASON If /MICROSAR/Rtm/RtmMeasurementPoint/RtmThresholdCallback of all CPU load mps are enabled, the check
          Rtm_ThresholdCbkFctArr[activatedCpuLoadMpId] != NULL_PTR cannot be true and false.
  ...
  \ID COV_RTM_TASKRESPONSE_TIME_EXTENDED_TASKS
  \ACCEPT TF tf xf
  \ACCEPT TF xf tf
  \REASON The checks of extended tasks is always false when there are no extended type tasks included and for basic tasks is always
          false when there are no basic type tasks included in the configured configuration.
...
/* VCA_JUSTIFICATION_BEGIN
 ...
 \ID             VCA_RTM_VALID_LENGTH_OF_CORE_SPECIFIC_TASKS
 \DESCRIPTION    The Rtm_CommonConst[<CoreIndex>].OsTaskInfoLengthOfCommonConst must be equal the core specific RTM_NUMBER_OF_TASKS_ON_CORE_<CoreIndex>.
 \COUNTERMEASURE \S SMI-1242904
