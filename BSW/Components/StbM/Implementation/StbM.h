 ...
 *  09.00.00  2021-10-22  visjwe  MSR4-31533    ASR memmap include structure ALL SLP
 *            2021-10-29  visssf  MSR4-30277    [MC] Advanced MC Distribution for CAN - Beta
 *            2022-03-08  vissi   TSYNC-3042    StbM: Finite Execution Time Analysis (FETA)
 *            2022-04-08  vissin  TSYNC-2756    StbM: New API to provide MainTimeTuple, Status and Rate
 *  09.00.01  2022-06-02  visssf  ESCAN00111994 TimeRecording and TimeValidation callback leads to OS API called with disabled interrupts
 *            2022-06-14  visssf  ESCAN00111801 Gpt timer not started by StbM
 *  09.01.00  2022-06-22  visjwe  MSR4-31414    Extend use-case for time synchronization and safety (non EthTsyn part)
 *            2022-06-30  vrietz  TSYNC-3471    Implement missing ErrorChecks
 *  09.02.00  2022-06-22  vissin  MSR4-31412    Clone time from one time domain (slave) to another (master)
 *            2022-09-01  visjwe  TSYNC-3686    Add unit infix for all non-Module APIs
 *            2022-09-02  visjwe  ESCAN00112537 Wrong casting of OffsetJumpCorrectionThreshold leads to incorrect behaviour
 *            2022-09-07  visjwe  ESCAN00112721 StbM_BusGetCurrentTime() not available but required for TimeValidation EthTSyn
 *            2022-09-20  vissin  TSYNC-3756    Refactoring of conditional return statements
 *  09.03.00  2022-10-20  vissin  TSYNC-2979    StbM/EthTSyn Rate correction of sync reception delay
 *            2022-11-03  visssf  MSR4-33988    Allow application to modify local clock in StbM
 *  09.04.00  2022-11-30  vrietz  TSYNC-3968    StbM: Analyze VCA warnings and possible fixes
 *            2022-12-09  vissi   TSYNC-4037    StbM: QAC 2022.2 support
 *            2023-01-20  visssf  TSYNC-4014    StbM local time correlation critical section
 *            2023-01-26  visssf  TSYNC-4064    StbM LocalClockSyncCallback add EthIfCtrlIdx
 *            2023-01-27  visssf  ESCAN00113556 Time leap check is not deactivated when threshold is configured to 0
 *            2023-01-30  visssf  TSYNC-4073    Allow to get the Global Time for Ethernet Slave use case
 *  09.05.00  2023-02-15  vrietz  TSYNC-4110    Fix possible overflow of elapsed virtual local time
 *  09.05.01  2023-04-03  visssf  ESCAN00114318 Cloning of time base value not correct for time bases using the same local clock
 *  09.05.02  2023-04-18  vrietz  ESCAN00114147 Conversion of microseconds to OS ticks is inaccurate for Triggered Customers
 *            2023-07-05  vrietz  ESCAN00114952 OS schedule tables cannot be synchronized cross-core by Triggered Customers
 *            2023-08-23  visjwe  TSYNC-4484    Add unit annotations for header files and remove NO_UNIT switches for prototypes
 *            2023-09-14  visjwe  TSYNC-4432    Improve LocalClockSync on time leap and backwards adjustment
 *  09.05.03  2023-10-11  fmommer TSYNC-4239    StbM: Optimize preprocessor switches for [Get|Set]ElapsedClockValue
 *            2023-11-13  visjwe  ESCAN00116113 Partially updated time tuple leading to current time being stuck
 *  09.06.00  2023-12-01  visjwe  MSR4-35922    Remove float64 based calculations
 *            2024-01-11  fmommer TSYNC-4804    Fix Vector Coding Rules Findings
 *            2024-01-24  fmommer ESCAN00116573 Time Base Cloning might copy invalid rate value to destination time base
 *            2024-01-31  fmommer ESCAN00116384 Compiler error: Configuring StbMAllowSystemWideGlobalTimeMaster in a post-build configuration without master time bases leads to undefined macro
 *********************************************************************************************************************/
...
# define STBM_AR_RELEASE_MINOR_VERSION                 (0x07U)
# define STBM_AR_RELEASE_REVISION_VERSION              (0x00U)

/* ----- Component version information (decimal version of ALM implementation package) ----- */
# define STBM_SW_MAJOR_VERSION                         (9U)
# define STBM_SW_MINOR_VERSION                         (6U)
...
# define STBM_SID_CLONE_TIME_BASE                      (0x2BU)  /* !< Service ID: StbM_CloneTimeBase() */

# define STBM_SID_PRE_INIT                             (0xC0U)  /* !< Service ID: StbM_PreInit() */
# define STBM_SID_POST_INIT                            (0xC1U)  /* !< Service ID: StbM_PostInit() */
# define STBM_SID_GET_MAIN_TIME                        (0xC2U)  /* !< Service ID: StbM_GetMainTime() */
...
# define STBM_E_PARAM_TIMESTAMP                        (0x25U)  /* !< Error code: API service called with invalid timestamp */
# define STBM_E_PARAM_USERDATA                         (0x26U)  /* !< Error code: API service called with invalid user data */
...
extern P2CONST(StbM_ConfigType, STBM_VAR_CLEARED, STBM_INIT_DATA) StbM_ConfigDataPtr;
# endif
 ...
 *  \details     This function initializes the module StbM. It initializes all variables of the partition context this
 *               function is called in and sets the partition specific module state to initialized.
 *  \param[in]   ConfigPtr    Component configuration structure
 *  \pre         Interrupts are disabled and StbM_PreInit has been called.
 *  \context     TASK
 *  \reentrant   FALSE
 *  \synchronous TRUE
 ...
 *  \details     Sets the module state to initialized if each partition context has been initialized before.
 *  \pre         Interrupts are disabled and StbM_Init has been called for each partition context.
 *  \context     TASK
 *  \reentrant   FALSE
 *  \synchronous TRUE
 *  \note        Normally called by EcuM.
 ...
 *  StbM_GetMainTime
 *********************************************************************************************************************/
/*! \brief       Returns TGsync, TVsync, status and rate deviation
 *  \details     This function allows the customers to get the main time tuple, status and current rate deviation.
 *  \param[in]   timeBaseId              The synchronized time-base, whose time, status and rate are of interest.
 *  \param[out]  globalTimeSyncPtr       Global time sync.
 *  \param[out]  localTimeSyncPtr        Virtual local time sync.
 *  \param[out]  rateDeviation           Current rate deviation.
 *  \return      E_OK     - Time stamps and rate deviation are valid
 *  \return      E_NOT_OK - Time stamps and rate deviation are not valid
 *  \pre         -
 *  \context     TASK|ISR2
 *  \reentrant   FALSE
 *  \synchronous TRUE
 ...
 *  StbM_CloneTimeBase
 *********************************************************************************************************************/
/*! \brief       Triggers cloning of time base
 *  \details     This function allows the customers to get the main time tuple, status and current rate deviation.
 ...
 *  \return      E_OK     - Cloning request successful
 *  \return      E_NOT_OK - Cloning request failed
 *  \pre         -
 *  \context     TASK|ISR2
 *  \reentrant   FALSE
 *  \synchronous TRUE|FALSE
 *  \unit        CloneTime
 *  \config      STBM_CONFIG_SOURCE_TIMEBASE
 *********************************************************************************************************************/
FUNC(Std_ReturnType, STBM_CODE) StbM_CloneTimeBase( /* PRQA S 3449, 3451 */ /* MD_StbM_Rule8.5_3449, MD_StbM_Rule8.5_3451 */
        StbM_SynchronizedTimeBaseType timeBaseId,
...
* Ensures consistency of global RAM variables locally on one core.
* \protects All affected global RAM variables.
* \usedin StbM_GetSyncTimeRecordHead, StbM_GetOffsetTimeRecordHead, StbM_StatusNotification_NotifyCustomers,
*         StbM_TimeRecording_NotifyCustomers, StbM_TimeValidation_NotifyEthMasterCustomers, StbM_TimeValidation_NotifyEthSlaveCustomers,
...
* \endexclusivearea
*
* \exclusivearea STBM_EXCLUSIVE_AREA_1
* Ensures consistency of global RAM variables across all cores on which a time base is located.
* \protects All affected global RAM variables.
* \usedin StbM_GetCurrentTime, StbM_GetCurrentTimeExtended, StbM_GetCurrentVirtualLocalTime, StbM_BusGetCurrentTime, StbM_GetTimeLeap, StbM_GetTimeBaseUpdateCounter, StbM_GetTimeBaseStatus,
*         StbM_SetGlobalTime, StbM_UpdateGlobalTime, StbM_SetUserData, StbM_TriggerTimeTransmission, StbM_SetOffset, StbM_GetOffset, StbM_BusSetGlobalTime, StbM_SetRateCorrection, StbM_GetRateDeviation,
*         StbM_NotificationFunction, StbM_TimerCallback, StbM_StartTimer, StbM_MainFunction, StbM_Timeout_Update, StbM_GetMainTime, StbM_CloneTimeBase
* \exclude All functions provided by StbM.
* \length SHORT depending on the API and the call graph.
* \endexclusivearea
*/
