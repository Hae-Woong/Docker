...
#if ( CANNM_DEV_ERROR_REPORT == STD_ON ) || ( CANNM_RUNTIME_ERROR_REPORT == STD_ON )
...
#if (  (CANNM_SW_MAJOR_VERSION != (0x11u)) \
...
/* vendor specific version information check */
#if (  (CANNM_CBK_MAJOR_VERSION != (0x11u)) \
...
#define CANNM_PNSR_BIT_MASK                         ((uint8)0x02u)
#define CANNM_PNSR_BIT_INV_MASK                     ((uint8)0xFDu)
...
/*! Macro for CanNm_TriggerTransmission context */
#define CANNM_TRIGGER_TRANSMISSION_REGULAR          ((uint8)0x01u)
#define CANNM_TRIGGER_TRANSMISSION_PNSR             ((uint8)0x02u)
#define CANNM_TRIGGER_TRANSMISSION_REQ_BUS_SYNC     ((uint8)0x03u)
 ...
 *********************************************************************************************************************/
CANNM_LOCAL_INLINE FUNC( CanNm_SizeOfPnInfoByteConfigType, CANNM_CODE ) CanNm_DetermineDynamicPnInfoLength( P2CONST( PduInfoType, AUTOMATIC, CANNM_APPL_VAR )PduInfoPtr );
#endif

#if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
/**********************************************************************************************************************
  CanNm_PnsrMessageHandling
**********************************************************************************************************************/
 ...
 *********************************************************************************************************************/
CANNM_LOCAL_INLINE FUNC( void, CANNM_CODE ) CanNm_PnsrMessageHandling( CANNM_CHANNEL_TYPE_ONLY );
#endif
...
/**********************************************************************************************************************
  CanNm_PnEraCalculation_NewDataHandling
**********************************************************************************************************************/
/*! \brief       Process new received Partial Networking ERA information
 *  \details     Subfunction of the CanNm_PnEraCalculation
 *  \pre         -
 ...
 *  \context     TASK
 *  \note        Called by CanNm_PnEraCalculation (CanNm_MainFunction)
 *********************************************************************************************************************/
# if ( CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG == STD_ON )
CANNM_LOCAL_INLINE FUNC( void, CANNM_CODE ) CanNm_PnEraCalculation_NewDataHandling( CONST( CanNm_PbPncClusterConfigExtIterType, AUTOMATIC ) currentPnByteIndex,
                                                                                    P2VAR( uint8, AUTOMATIC, CANNM_APPL_VAR ) pnClusterRequestExtNewLocal,  P2VAR( boolean, AUTOMATIC, CANNM_APPL_VAR ) pnClusterRequestChanged,
                                                                                    P2VAR( uint8, AUTOMATIC, CANNM_APPL_VAR ) syncPncShutdownReqExtLocal, P2VAR( boolean, AUTOMATIC, CANNM_APPL_VAR ) pnSyncClusterReqChanged );
# else
CANNM_LOCAL_INLINE FUNC( void, CANNM_CODE ) CanNm_PnEraCalculation_NewDataHandling( CONST( CanNm_PbPncClusterConfigExtIterType, AUTOMATIC ) currentPnByteIndex,
                                                                                    P2VAR( uint8, AUTOMATIC, CANNM_APPL_VAR ) pnClusterRequestExtNewLocal, P2VAR( boolean, AUTOMATIC, CANNM_APPL_VAR ) pnClusterRequestChanged );
# endif
...
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
/**********************************************************************************************************************
  CanNm_TriggerTransmission_PnsrHandling
**********************************************************************************************************************/
 ...
 *  \param[in]   channel              - Local channel handle
 *  \context     TASK
 *  \note        Called by CanNm_TriggerTransmission (CanNm_MainFunction)
 *  \return      TRUE             - PNSR message shall be skipped
 *               FALSE            - PNSR message shall be transmitted
 *********************************************************************************************************************/
CANNM_LOCAL FUNC( boolean, CANNM_CODE ) CanNm_TriggerTransmission_PnsrHandling( CANNM_CHANNEL_TYPE_ONLY );
# endif
 ...
 *********************************************************************************************************************/
CANNM_LOCAL FUNC( Std_ReturnType, CANNM_CODE ) CanNm_TriggerTransmission( CANNM_CHANNEL_TYPE_FIRST uint8 context );
#endif
...
{
  /* ----- Local Variables ---------------------------------------------- */
  CanNm_SizeOfPnInfoByteConfigType adjustedInfoLength;
# if defined ( CANNM_CANIF_RANGE_CONFIG_DLC_CHECK )
  /* No need to check RX PDU Length, fixed length assumed */
  adjustedInfoLength = CanNm_GetSizeOfPnInfoByteConfig();
...
# else
   ...
   */
  if( (PduInfoPtr->SduLength ) < (PduLengthType)(CanNm_GetPnInfoOffset() + CanNm_GetSizeOfPnInfoByteConfig()) ) /* COV_CANNM_PN_INFO_LENGTH */
  {
    adjustedInfoLength = (CanNm_SizeOfPnInfoByteConfigType)((PduLengthType)(PduInfoPtr->SduLength - CanNm_GetPnInfoOffset()));
  }
  else
  {
    adjustedInfoLength = CanNm_GetSizeOfPnInfoByteConfig();
...
CANNM_LOCAL_INLINE FUNC(void, CANNM_CODE) CanNm_PnsrMessageHandling(CANNM_CHANNEL_TYPE_ONLY)
{
  ...
  if ( CanNm_IsSynchronizedPncShutdownEnabledOfChannelConfig(CANNM_CHANNEL_IDX) && CanNm_IsActivePncCoordinatorChannelOfChannelConfig(CANNM_CHANNEL_IDX) ) /* COV_SYNCHRONIZED_PNC_SHUTDOWN TX tx tx */
  {
# if ( CANNM_COMCONTROLENABLEDOFCHANNELCONFIG == STD_ON )
    if ( CanNm_GetTxControlState(CANNM_CHANNEL_IDX) == MSG_TX_ENABLED )
# endif
    {
      /* - #100 Check if a MsgTransmission is currently pending */
# if ( CANNM_IMMEDIATE_TXCONF_ENABLED == STD_OFF )
      if ( CanNm_GetMsgTimeoutTimer(CANNM_CHANNEL_IDX) == 0u )
# endif
      {
        CanNm_PbPncClusterConfigExtStartIdxOfPbChannelConfigType syncMessageRequestIterator;
        ...
        CanNm_SetPnsrMessageTransmissionOngoing(CANNM_CHANNEL_IDX, FALSE); /* SBSW_CANNM_CSL_CHANNEL_INDEX_ACCESS */

        /* - #1000 If no MsgTransmission pending -> Check if there is a PNC that was requested to be shutdown synchronously. */
        for ( syncMessageRequestIterator = CanNm_GetPbPncClusterConfigExtStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX); /* FETA_CANNM_CSL_USE_CASE */
              syncMessageRequestIterator < CanNm_GetPbPncClusterConfigExtEndIdxOfPbChannelConfig(CANNM_CHANNEL_IDX);
              syncMessageRequestIterator++ )
        {
          if ( (CanNm_GetSyncPncShutdownReq(syncMessageRequestIterator) | CanNm_GetSyncPncShutdownReqNew(syncMessageRequestIterator)) != 0u )
          {
            /* - #10000 Trigger the PNSR message. */
            CanNm_SetTxMessageData( CanNm_GetTxMessageData_CBVIdxOfPbChannelConfig(CANNM_CHANNEL_IDX), /* SBSW_CANNM_CSL03 */
              (CanNm_TxMessageDataType)(CanNm_GetTxMessageData(CanNm_GetTxMessageData_CBVIdxOfPbChannelConfig(CANNM_CHANNEL_IDX)) | CANNM_PNSR_BIT_MASK) );

            if ( CanNm_TriggerTransmission(CANNM_CHANNEL_PARA_FIRST CANNM_TRIGGER_TRANSMISSION_PNSR) == E_NOT_OK )
            {
# if ( CANNM_PNSYNCSHUTDOWNRETRANSMISSIONDURATION == STD_ON )
              /* If the PNSR message could not be transmitted due to an E_NOT_OK by the CanIf, it shall be retried next main function. No regular NM message shall interfere. */
              CanNm_SetPnsrMessageTransmissionOngoing(CANNM_CHANNEL_IDX, TRUE); /* SBSW_CANNM_CSL_CHANNEL_INDEX_ACCESS */
# else
              /* - #20000 If the return value is E_NOT_OK and RetransmissionDuration is disabled
               *           Call Det_ReportRuntimeError and clear all shutdown requests.
               */
              CanNm_PbPncClusterConfigExtStartIdxOfPbChannelConfigType syncPncCounter;
#  if ( CANNM_RUNTIME_ERROR_REPORT == STD_ON )
              /* If no Retransmission Duration is enabled, a Det shall be thrown immediately. */
              (void)Det_ReportRuntimeError( CANNM_MODULE_ID, CANNM_INSTANCE_ID, CANNM_SID_MAINFUNCTION, CANNM_E_TRANSMISSION_OF_PN_SHUTDOWN_MESSAGE_FAILED );
#  endif
              /* All transmitted requests shall be removed immediately. */
              for ( syncPncCounter = CanNm_GetPbPncClusterConfigExtStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX); /* FETA_CANNM_CSL_USE_CASE */
                    syncPncCounter < CanNm_GetPbPncClusterConfigExtEndIdxOfPbChannelConfig(CANNM_CHANNEL_IDX);
                    syncPncCounter++ )
              {
                CanNm_SetSyncPncShutdownReq( syncPncCounter, 0u ); /* SBSW_CANNM_CSL02 */
              }
# endif
            }
            ...
            /* Clear the PNSR bit */
            CanNm_SetTxMessageData( CanNm_GetTxMessageData_CBVIdxOfPbChannelConfig(CANNM_CHANNEL_IDX), /* SBSW_CANNM_CSL03 */
              (CanNm_TxMessageDataType)(CanNm_GetTxMessageData(CanNm_GetTxMessageData_CBVIdxOfPbChannelConfig(CANNM_CHANNEL_IDX)) & CANNM_PNSR_BIT_INV_MASK) );
            break;
          }
        }
      }
...
{
  /* ----- Local Variables ---------------------------------------------- */
  CanNm_SizeOfPnInfoByteConfigType pnClusterRequestByte;
...
# endif
  ...
  {
    /* #100 Iterate over all Partial Networking Cluster Bytes
    ...
    for ( pnClusterRequestByte = 0u; pnClusterRequestByte < CanNm_GetSizeOfPnInfoByteConfig(); pnClusterRequestByte++ )
    {
      uint8_least                                             pnClusterRequestBit = 0u;
      CanNm_PnClusterReqNewType                               pnClusterRequestNew = 0u;
      CanNm_PnClusterResetTimerStartIdxOfPnInfoByteConfigType pnClusterResetTimerIndex;
      ...
      for ( pnClusterResetTimerIndex = CanNm_GetPnClusterResetTimerStartIdxOfPnInfoByteConfig(pnClusterRequestByte); /* FETA_CANNM_CSL_USE_CASE */
            pnClusterResetTimerIndex < CanNm_GetPnClusterResetTimerEndIdxOfPnInfoByteConfig(pnClusterRequestByte);
            pnClusterResetTimerIndex++ )
      {
        if ( (pnClusterRequestNew & (CanNm_PnClusterReqNewType)(1u << pnClusterRequestBit)) != 0u )
        {
          CanNm_SetPnClusterResetTimer( pnClusterResetTimerIndex, CanNm_GetPnResetTime() ); /* SBSW_CANNM_CSL03 */
        }
        if ( CanNm_GetPnClusterResetTimer(pnClusterResetTimerIndex) != 0u )
        {
          CanNm_DecPnClusterResetTimer(pnClusterResetTimerIndex); /* SBSW_CANNM_CSL03 */
          if ( CanNm_GetPnClusterResetTimer(pnClusterResetTimerIndex) == 0u )
          {
            pnClusterRequestChanged = TRUE;
            CanNm_SetPnClusterReq( pnClusterRequestByte, (CanNm_PnClusterReqType)  /* SBSW_CANNM_CSL02 */
              (CanNm_GetPnClusterReq(pnClusterRequestByte) & (CanNm_PnClusterReqType)(~(1u << pnClusterRequestBit))) );
          }
        }
        pnClusterRequestBit++;
      }
    ...
    /* #200 If an PNC EIRA request has changed, indicate this to the ComM (via PduR -> Com <- ComM)
 ...
 */
# if ( CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG == STD_ON )
/* PRQA S 3673 3 */ /* MD_CANNM_Rule8.3 */
CANNM_LOCAL_INLINE FUNC( void, CANNM_CODE ) CanNm_PnEraCalculation_NewDataHandling( CONST( CanNm_PbPncClusterConfigExtIterType, AUTOMATIC ) currentPnByteIndex,
                                                                                    P2VAR( uint8, AUTOMATIC, CANNM_APPL_VAR ) pnClusterRequestExtNewLocal,  P2VAR( boolean, AUTOMATIC, CANNM_APPL_VAR ) pnClusterRequestChanged,
                                                                                    P2VAR( uint8, AUTOMATIC, CANNM_APPL_VAR ) syncPncShutdownReqExtLocal, P2VAR( boolean, AUTOMATIC, CANNM_APPL_VAR ) pnSyncClusterReqChanged )
# else
...
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED == STD_ON )
  /* - #10 Copy the ShutdownRequests received via the bus to a local variable.
   *       Clear the ShutdownRequests.
   */
  *syncPncShutdownReqExtLocal = CanNm_GetSyncPncShutdownReqExt(currentPnByteIndex); /* SBSW_CANNM_LOCAL_FUNCTION_POINTER_ACCESS */
  CanNm_SetSyncPncShutdownReqExt( currentPnByteIndex, 0u ); /* SBSW_CANNM_CSL02 */
# else
#  if ( CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG == STD_ON )
  CANNM_DUMMY_STATEMENT(syncPncShutdownReqExtLocal); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
#  endif
# endif
   ...
   */
  if ( CanNm_GetPnClusterReqExtNew(currentPnByteIndex) != 0u )
  {
# if ( CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG == STD_ON )
    uint8 bitsToFilter;
# endif
    *pnClusterRequestExtNewLocal = CanNm_GetPnClusterReqExtNew(currentPnByteIndex); /* SBSW_CANNM_LOCAL_FUNCTION_POINTER_ACCESS */
    CanNm_SetPnClusterReqExtNew( currentPnByteIndex, 0u ); /* SBSW_CANNM_CSL02 */

    pnClusterRequestBits = (uint8)(CanNm_GetPnClusterReqExt(currentPnByteIndex) | *pnClusterRequestExtNewLocal);
    if ( pnClusterRequestBits != CanNm_GetPnClusterReqExt(currentPnByteIndex) )
    {
      *pnClusterRequestChanged = TRUE; /* SBSW_CANNM_LOCAL_FUNCTION_POINTER_ACCESS */
      CanNm_SetPnClusterReqExt( currentPnByteIndex, (CanNm_PnClusterReqExtType)pnClusterRequestBits ); /* SBSW_CANNM_CSL02 */
    }
...
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED == STD_ON )
    ...
    if ( bitsToFilter != 0u )
    {
      ...
      *syncPncShutdownReqExtLocal &= ~bitsToFilter; /* SBSW_CANNM_LOCAL_FUNCTION_POINTER_ACCESS */
    }
# endif
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
...
#  if ( CANNM_IMMEDIATE_TXCONF_ENABLED == STD_OFF )
    /* Note: This is irrelevant in case ImmediateTxConf is enabled. The TxConfirmation handling is executed prior to this in the MainFunction.
             The SyncPncShutdownReq array will there always be set to zero. In case CanIf returns E_NOT_OK the TxConfirmation handling is not done.
             The RetransmissionDuration must be off (if ImmediateTxConf on) and if CanIf_Transmit returns E_NOT_OK, it will result in a DET. */
    bitsToFilter = CanNm_GetSyncPncShutdownReq(currentPnByteIndex) & *pnClusterRequestExtNewLocal;

    if ( bitsToFilter != 0u )
    {
      /* Remove the bits from the request array, that have been internally requested via CanNm_RequestSynchronizedPncShutdown. */
      CanNm_GetSyncPncShutdownReq(currentPnByteIndex) &= ~bitsToFilter; /* SBSW_CANNM_CSL02 */
      *pnSyncClusterReqChanged = TRUE; /* SBSW_CANNM_LOCAL_FUNCTION_POINTER_ACCESS */
    }
#  endif
    ...
    bitsToFilter = CanNm_GetSyncPncShutdownReqNew(currentPnByteIndex) & *pnClusterRequestExtNewLocal;

    if (bitsToFilter != 0u)
    {
      /* Remove the bits from the request array, that have been internally requested via CanNm_RequestSynchronizedPncShutdown. */
      CanNm_GetSyncPncShutdownReqNew(currentPnByteIndex) &= ~bitsToFilter; /* SBSW_CANNM_CSL02 */
      *pnSyncClusterReqChanged = TRUE; /* SBSW_CANNM_LOCAL_FUNCTION_POINTER_ACCESS */
    }
# else
#  if ( CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG == STD_ON )
    CANNM_DUMMY_STATEMENT(pnSyncClusterReqChanged); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
#  endif
# endif
  }
  SchM_Exit_CanNm_CANNM_EXCLUSIVE_AREA_0();
...
{
  /* ----- Local Variables ---------------------------------------------- */
  CanNm_PbPncClusterConfigExtStartIdxOfPbChannelConfigType pnClusterRequestByte;
  boolean                                                  pnClusterRequestChanged;
# if ( CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG == STD_ON )
...
# endif
    ...
    for ( pnClusterRequestByte = CanNm_GetPbPncClusterConfigExtStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX); /* FETA_CANNM_CSL_USE_CASE */
          pnClusterRequestByte < CanNm_GetPbPncClusterConfigExtEndIdxOfPbChannelConfig(CANNM_CHANNEL_IDX);
          pnClusterRequestByte++ )
    {
      uint8                                                           pnClusterRequestExtNewLocal = 0u;
      uint8_least                                                     pnClusterRequestBit = 0u;
      CanNm_PnClusterResetTimerExtStartIdxOfPbPncClusterConfigExtType pnClusterResetTimerExtIndex = 0u;

      /* #1000 Handle new Era data. */
# if ( CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG == STD_ON )
      CanNm_PnEraCalculation_NewDataHandling( pnClusterRequestByte, &pnClusterRequestExtNewLocal, &pnClusterRequestChanged, &syncPncShutdownReqExtLocal, &pnSyncClusterReqChanged ); /* SBSW_CANNM_LOCAL_FUNCTION_CALL_WITH_LOCAL_VARS */
# else
      CanNm_PnEraCalculation_NewDataHandling( pnClusterRequestByte, &pnClusterRequestExtNewLocal, &pnClusterRequestChanged ); /* SBSW_CANNM_LOCAL_FUNCTION_CALL_WITH_LOCAL_VARS */
# endif

      /* #2000 Iterate over all PNC Bits in the current PNC Byte (and corresponding PnResetTimer)
       *        If a PNC was requested for synchronized PNC shutdown externally
       *         Clear the Era bit and clear the Era timer.
       *        If PNC Bit is set in last received message (Era)
      ...
      for ( pnClusterResetTimerExtIndex = CanNm_GetPnClusterResetTimerExtStartIdxOfPbPncClusterConfigExt(pnClusterRequestByte); /* FETA_CANNM_CSL_USE_CASE */
            pnClusterResetTimerExtIndex < CanNm_GetPnClusterResetTimerExtEndIdxOfPbPncClusterConfigExt(pnClusterRequestByte);
...
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED == STD_ON )
        ...
        if ( !CanNm_IsActivePncCoordinatorChannelOfChannelConfig(CANNM_CHANNEL_IDX)
          && (((syncPncShutdownReqExtLocal) & (1u << pnClusterRequestBit)) != 0u)
          && ((CanNm_GetPnClusterReqExt(pnClusterRequestByte) & (1u << pnClusterRequestBit)) != 0u) ) /* COV_CANNM_ERA_CALC_SYNC_SHUTDOWN_RX TF tx tf tf */
        {
          /* Clear the Era bit for the PNC that has been received. Clear the timer. */
          pnClusterRequestChanged = TRUE;
          CanNm_SetPnClusterReqExt( pnClusterRequestByte, (CanNm_PnClusterReqExtType)(CanNm_GetPnClusterReqExt(pnClusterRequestByte) & (uint8)(~(1u << pnClusterRequestBit))) ); /* SBSW_CANNM_CSL02 */
          CanNm_SetPnClusterResetTimerExt( pnClusterResetTimerExtIndex, 0u ); /* SBSW_CANNM_CSL03 */
        }
# endif

        if ( (pnClusterRequestExtNewLocal & (CanNm_PnClusterReqExtNewType)(1u << pnClusterRequestBit)) != 0u )
        {
          CanNm_SetPnClusterResetTimerExt( pnClusterResetTimerExtIndex, CanNm_GetPnResetTime() ); /* SBSW_CANNM_CSL03 */
        }

        if ( CanNm_GetPnClusterResetTimerExt(pnClusterResetTimerExtIndex) != 0u )
        {
          CanNm_DecPnClusterResetTimerExt(pnClusterResetTimerExtIndex); /* SBSW_CANNM_CSL03 */
          if ( CanNm_GetPnClusterResetTimerExt(pnClusterResetTimerExtIndex) == 0u )
          {
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON ) && ( CANNM_IMMEDIATE_TXCONF_ENABLED == STD_OFF )
            /* If the timer was started due to a synchronized pnc shutdown request. */
            if ( CanNm_IsActivePncCoordinatorChannelOfChannelConfig(CANNM_CHANNEL_IDX) &&
               ( ((CanNm_GetSyncPncShutdownReq(pnClusterRequestByte) & (1u << pnClusterRequestBit)) != 0u )
              || ((CanNm_GetSyncPncShutdownReqNew(pnClusterRequestByte) & (1u << pnClusterRequestBit)) != 0u) ) ) /* COV_CANNM_ERA_CALC_SYNC_SHUTDOWN_TX */
            {
               ...
               */
              CanNm_SetSyncPncShutdownReq( pnClusterRequestByte, CanNm_GetSyncPncShutdownReq(pnClusterRequestByte) & ~(1u << pnClusterRequestBit) ); /* SBSW_CANNM_CSL02 */
              CanNm_SetSyncPncShutdownReqNew( pnClusterRequestByte, CanNm_GetSyncPncShutdownReqNew(pnClusterRequestByte) & ~(1u << pnClusterRequestBit) ); /* SBSW_CANNM_CSL02 */
#  if ( CANNM_RUNTIME_ERROR_REPORT == STD_ON )
              (void)Det_ReportRuntimeError( CANNM_MODULE_ID, CANNM_INSTANCE_ID, CANNM_SID_MAINFUNCTION, CANNM_E_TRANSMISSION_OF_PN_SHUTDOWN_MESSAGE_FAILED );
#  endif
            }
...
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON ) && ( CANNM_IMMEDIATE_TXCONF_ENABLED == STD_OFF )
        ...
        {
          /* If the Retransmission Duration is enabled, check the SyncShutdownReq and the SyncShutdownReqNew variable. The timer must be started immediately when the CanNm_SynchronizedPncShutdown has been called.
...
#  if ( CANNM_PNSYNCSHUTDOWNRETRANSMISSIONDURATION == STD_ON )
          if ( ((CanNm_GetSyncPncShutdownReq(pnClusterRequestByte) & (1u << pnClusterRequestBit)) != 0u) || ((CanNm_GetSyncPncShutdownReqNew(pnClusterRequestByte) & (1u << pnClusterRequestBit)) != 0u) )
          {
            CanNm_SetPnClusterResetTimerExt( pnClusterResetTimerExtIndex, CanNm_GetPnSyncShutdownRetransmissionDuration() ); /* SBSW_CANNM_CSL03 */
          }
#  else
          if ( ((CanNm_GetSyncPncShutdownReq(pnClusterRequestByte) & (1u << pnClusterRequestBit)) != 0u) )
          {
#   if ( CANNM_PNSR_MSG_TIMEOUT_TICK_EQ_ONE == STD_ON )
            CanNm_SetSyncPncShutdownReq( pnClusterRequestByte, CanNm_GetSyncPncShutdownReq(pnClusterRequestByte) & ~(1u << pnClusterRequestBit) ); /* SBSW_CANNM_CSL02 */
#    if ( CANNM_RUNTIME_ERROR_REPORT == STD_ON )
            (void)Det_ReportRuntimeError( CANNM_MODULE_ID, CANNM_INSTANCE_ID, CANNM_SID_MAINFUNCTION, CANNM_E_TRANSMISSION_OF_PN_SHUTDOWN_MESSAGE_FAILED );
#    endif
#   else
            ...
            CanNm_SetPnClusterResetTimerExt( pnClusterResetTimerExtIndex, CanNm_GetPnSyncShutdownMessageTimeoutTime() - 1u ); /* SBSW_CANNM_CSL03 */
#   endif
          }
#  endif
        }
...
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
    /*
     * #200 Check if a SyncShutdownReqest was erased due to an Era message.
    ...
    if ( pnSyncClusterReqChanged == TRUE )
    {
      ...
      for ( pnClusterRequestByte = CanNm_GetPbPncClusterConfigExtStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX); /* FETA_CANNM_CSL_USE_CASE */
            ...
            pnClusterRequestByte++ )
      {
        if ( CanNm_GetSyncPncShutdownReq(pnClusterRequestByte) != 0u ) /* COV_CANNM_PNSR_MSG_TIMEOUT_TIME_EQ_ONE_NO_ADDITIONAL_PNSR_MESSAGE XF */
        {
          stopPnsrMessage = FALSE;
          break;
        }
      }
      if ( stopPnsrMessage == TRUE ) /* COV_CANNM_PNSR_MSG_TIMEOUT_TIME_EQ_ONE_NO_ADDITIONAL_PNSR_MESSAGE TX */
      {
        CanNm_SetPnsrMessageTransmissionOngoing( CANNM_CHANNEL_IDX, FALSE ); /* SBSW_CANNM_CSL_CHANNEL_INDEX_ACCESS */
#  if ( CANNM_IMMEDIATE_TXCONF_ENABLED == STD_OFF )
...
# endif
     ...
     *       Pass the changed PnClusterRequestExt buffer to the Com by calling PduR_CanNmRxIndication
     */
    if( pnClusterRequestChanged == TRUE )
    {
      PduInfoType  nmPnInfo;
      /* ESCAN00064309 */
      nmPnInfo.SduDataPtr = (P2VAR(uint8, AUTOMATIC, CANNM_VAR_NOINIT)) (CanNm_GetAddrPnClusterReqExt(CanNm_GetPbPncClusterConfigExtStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX))); /* Assign nmPnInfo.SduDataPtr pointer to address of PnClusterReqExt array (with channel offset) */
      nmPnInfo.SduLength = CanNm_GetPnInfoLength(); /* Set nmPnInfo.SduLength to length of PnInfo container */
...
CANNM_LOCAL_INLINE FUNC( boolean, CANNM_CODE ) CanNm_PnFilterHandling( CANNM_CHANNEL_TYPE_FIRST P2CONST( PduInfoType, AUTOMATIC, CANNM_APPL_VAR ) PduInfoPtr )
{
   ...
   *      If CRI bit is set in received CBV
...
# endif
    ...
    {
      if( (((uint8)(PduInfoPtr->SduDataPtr[CanNm_GetPduCbvPositionOfChannelConfig(CANNM_CHANNEL_IDX)]) & CANNM_CRI_BIT_MASK) != 0u )
# if defined ( CANNM_CANIF_RANGE_CONFIG_DLC_CHECK )
          /* No need to check RX PDU Length, fixed length assumed */
# else
...
# endif
      ...
      {
        CanNm_SizeOfPnInfoByteConfigType   rxMsgBytePos;
        uint8                              reqCluster;
# if ( CANNM_PN_ERA_CALC_FEATURE_ENABLED == STD_ON )
        CanNm_SizeOfPnClusterReqExtNewType pnClusterRequestIndex;
# endif
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED == STD_ON )
        boolean                            pnsrMessage = FALSE;
        boolean                            forwardSynchronizedPncShutdown = FALSE;
        uint8                              filteredPnInfoVector[CANNM_MAX_PN_INFO_LENGTH] = { 0u };
# endif
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED == STD_ON ) || ( ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON ) &&  ( ( CANNM_PNSYNCSHUTDOWNERRORREACTIONENABLED == STD_ON ) || ( CANNM_RUNTIME_ERROR_REPORT == STD_ON ) ) )
        /*
         ...
         */
        if ( CanNm_IsSynchronizedPncShutdownEnabledOfChannelConfig(CANNM_CHANNEL_IDX) ) /* COV_SYNCHRONIZED_PNC_SHUTDOWN TX */
        {
          if ( ((uint8)(PduInfoPtr->SduDataPtr[CanNm_GetPduCbvPositionOfChannelConfig(CANNM_CHANNEL_IDX)]) & CANNM_PNSR_BIT_MASK) != 0u )
          {
#  if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
            /* If the channel is actively coordinated, this message should never be received. */
            if ( CanNm_IsActivePncCoordinatorChannelOfChannelConfig(CANNM_CHANNEL_IDX) ) /* COV_SYNCHRONIZED_PNC_SHUTDOWN */
            {
              /* Error use case. The message should not be received here. */
#   if ( CANNM_RUNTIME_ERROR_REPORT == STD_ON )
              (void)Det_ReportRuntimeError( CANNM_MODULE_ID, CANNM_INSTANCE_ID, CANNM_SID_RXINDICATION, CANNM_E_INVALID_PN_SYNC_SHUTDOWN_REQUEST );
#   endif
#   if ( CANNM_PNSYNCSHUTDOWNERRORREACTIONENABLED == STD_ON )
...
# endif
        ...
        /* #300 Calculate an adjusted / truncated Pn Info Length value in case <CanIf Range Config DLC Check> is configured and the received DLC does not match the expected DLC
         ...
         *          Store the filtered shutdown request for further processing in the CanNm_MainFunction
         *          Store a flag to forward the synchronized shutdown message
         *         Else: Store the filtered ERA request for further processing in CanNm_MainFunction
         ...
         * Due to the data type of the returned value (CanNm_SizeOfPnInfoByteConfigType) of this sub-function,
         * it is guaranateed that the returned value serves as a valid and defined upper limit for the for-loop.
         */
        for ( rxMsgBytePos = 0u; rxMsgBytePos < CanNm_DetermineDynamicPnInfoLength( PduInfoPtr ); rxMsgBytePos++ ) /* FETA_CANNM_DYNAMIC_PN_INFO_LENGTH */ /* SBSW_CANNM_LOCALFUNCTION_CALL */
        {
#   if( CANNM_DEV_ERROR_DETECT == STD_ON )
          /* Runtime check that the returned value of the local function CanNm_DetermineDynamicPnInfoLength is smaller than the size of the PnInfoByteConfig array. */
          if ( rxMsgBytePos < CanNm_GetSizeOfPnInfoByteConfig() ) /* COV_CANNM_RUNTIME_CHECK TX */
#   endif
          {
            reqCluster = (uint8)(PduInfoPtr->SduDataPtr[rxMsgBytePos + CanNm_GetPnInfoOffset()] & CanNm_GetPnFilterMaskOfPnInfoByteConfig(rxMsgBytePos));
            if (reqCluster != 0u)
            {
              msgRelevant = TRUE;
...
#   endif
              {
                CanNm_SetPnClusterReqNew( rxMsgBytePos, (CanNm_PnClusterReqNewType)(CanNm_GetPnClusterReqNew(rxMsgBytePos) | reqCluster) ); /* SBSW_CANNM_DYN_PN_INFO_LENGTH */
              }
#  endif
#  if ( CANNM_PN_ERA_CALC_FEATURE_ENABLED == STD_ON )
              ...
              { /* ESCAN00064309 */
                pnClusterRequestIndex = (CanNm_PnClusterReqExtNewType)(CanNm_GetPbPncClusterConfigExtStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX) + rxMsgBytePos);
#   if( CANNM_DEV_ERROR_DETECT == STD_ON )
                /* Runtime Check that CanNm_SetPnClusterReqExtNew array is always written within its defined bounds. */
                if ( pnClusterRequestIndex < CanNm_GetSizeOfPnClusterReqExtNew() ) /* COV_CANNM_RUNTIME_CHECK TX */
#   endif
                {
#   if ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED == STD_ON )
                  if ( pnsrMessage == TRUE )
                  {
                    filteredPnInfoVector[rxMsgBytePos] = reqCluster; /* SBSW_CANNM_FILTERED_PN_INFO_ACCESS */
                    forwardSynchronizedPncShutdown = TRUE;
                    CanNm_SetSyncPncShutdownReqExt( pnClusterRequestIndex, (CanNm_SyncPncShutdownReqType)((CanNm_GetSyncPncShutdownReqExt(pnClusterRequestIndex)) | reqCluster)); /* SBSW_CANNM_SYNCPNCSHUTDOWNREQEXT */
                  }
                  else
#  endif
                  {
...
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED == STD_ON )
        ...
        {
          Nm_ForwardSynchronizedPncShutdown( CANNM_NMTOSYSTEM_CHANNEL_IND, filteredPnInfoVector ); /* SBSW_CANNM_POINTER_PARAM */
        }
# endif
      }
    } /* if ( (CanNm_GetPduCbvPositionOfChannelConfig(CANNM_CHANNEL_IDX) != CANNM_PDU_OFF) && ... ) */
...
CANNM_LOCAL_INLINE FUNC( void, CANNM_CODE ) CanNm_CopyRxMessageData( CANNM_CHANNEL_TYPE_FIRST P2CONST( PduInfoType, AUTOMATIC, CANNM_APPL_VAR ) PduInfoPtr )
{
  CanNm_RxMessageDataStartIdxOfPbChannelConfigType rxMsgByte;

  /* #10 Copy received NM message data to the Rx Message Data buffer */
  SchM_Enter_CanNm_CANNM_EXCLUSIVE_AREA_4();
  for( rxMsgByte = 0u; rxMsgByte < CanNm_GetRxMessageDataLengthOfPbChannelConfig(CANNM_CHANNEL_IDX); rxMsgByte++ ) /* FETA_CANNM_CSL_USE_CASE */ /* ESCAN00064309 */
  {
# if defined ( CANNM_CANIF_RANGE_CONFIG_DLC_CHECK )
    /* No need to check RX PDU Length, fixed length assumed */
# else
...
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
     ...
     */
    if ( CanNm_IsPnsrMessageTransmissionOngoing(CANNM_CHANNEL_IDX) )
    {
      CanNm_PbPncClusterConfigExtStartIdxOfPbChannelConfigType pnClusterRequestByte;
...
# endif
        ...
        CanNm_SetSyncPncShutdownReq( pnClusterRequestByte, 0u ); /* SBSW_CANNM_CSL02 */
      }
    }
# endif
  }
...
# endif
      {
        ...
        Std_ReturnType success = CanNm_TriggerTransmission( CANNM_CHANNEL_PARA_FIRST CANNM_TRIGGER_TRANSMISSION_REGULAR );
...
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
      else
      {
        ...
        {
          /* Try transmission in next main function cycle
           * If an asynchronous message request is triggered, the variable is just not cleared and retried next main function again.
           */
          CanNm_SetMsgTimer( CANNM_CHANNEL_IDX, 1u ); /* SBSW_CANNM_CSL_CHANNEL_INDEX_ACCESS */
        }
      }
...
#  if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
      if (CanNm_IsPnsrMessageTransmissionOngoing(CANNM_CHANNEL_IDX))
      {
         ...
         * Exception: If CanNm_RequestBusSynchronization is called.
         */
        CanNm_SetPnsrMessageTransmissionOngoing( CANNM_CHANNEL_IDX, FALSE ); /* SBSW_CANNM_CSL_CHANNEL_INDEX_ACCESS */
      }
      else
...
# endif
# if ( CANNM_PN_ERA_CALC_FEATURE_ENABLED == STD_ON )
  CanNm_PnClusterResetTimerExtStartIdxOfPbPncClusterConfigExtType pnClusterResetTimerExtIndex;
  CanNm_PbPncClusterConfigExtStartIdxOfPbChannelConfigType        pnClusterRequestByteExt;
# endif
  ...
  {
# if ( CANNM_PN_ERA_CALC_FEATURE_ENABLED == STD_ON )
    for( pnClusterRequestByteExt = CanNm_GetPbPncClusterConfigExtStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX); /* FETA_CANNM_CSL_USE_CASE */
         pnClusterRequestByteExt < CanNm_GetPbPncClusterConfigExtEndIdxOfPbChannelConfig(CANNM_CHANNEL_IDX);
         pnClusterRequestByteExt++ )
    {
      CanNm_SetPnClusterReqExt( pnClusterRequestByteExt, 0x00u ); /* SBSW_CANNM_CSL02 */
      CanNm_SetPnClusterReqExtNew( pnClusterRequestByteExt, 0x00u ); /* SBSW_CANNM_CSL02 */

#  if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
      CanNm_SetSyncPncShutdownReq( pnClusterRequestByteExt, 0u ); /* SBSW_CANNM_CSL02 */
      CanNm_SetSyncPncShutdownReqNew( pnClusterRequestByteExt, 0u ); /* SBSW_CANNM_CSL02 */
#  endif
#  if ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED == STD_ON )
      CanNm_SetSyncPncShutdownReqExt( pnClusterRequestByteExt, 0u ); /* SBSW_CANNM_CSL02 */
#  endif
      for ( pnClusterResetTimerExtIndex = CanNm_GetPnClusterResetTimerExtStartIdxOfPbPncClusterConfigExt(pnClusterRequestByteExt); /* FETA_CANNM_CSL_USE_CASE */
            pnClusterResetTimerExtIndex < CanNm_GetPnClusterResetTimerExtEndIdxOfPbPncClusterConfigExt(pnClusterRequestByteExt);
            pnClusterResetTimerExtIndex++ )
      {
        CanNm_SetPnClusterResetTimerExt( pnClusterResetTimerExtIndex, 0u ); /* SBSW_CANNM_CSL03 */
      }
...
#  endif
      for( pnClusterRequestByte = 0u; pnClusterRequestByte < CanNm_GetSizeOfPnInfoByteConfig(); pnClusterRequestByte++ ) /* FETA_CANNM_CSL_USE_CASE */
      {
        CanNm_SetPnClusterReq( pnClusterRequestByte, 0x00u ); /* SBSW_CANNM_CSL02 */
        CanNm_SetPnClusterReqNew( pnClusterRequestByte, 0x00u ); /* SBSW_CANNM_CSL02 */

        for ( pnClusterResetTimerIndex = CanNm_GetPnClusterResetTimerStartIdxOfPnInfoByteConfig(pnClusterRequestByte); /* FETA_CANNM_CSL_USE_CASE */
              pnClusterResetTimerIndex < CanNm_GetPnClusterResetTimerEndIdxOfPnInfoByteConfig(pnClusterRequestByte);
              pnClusterResetTimerIndex++ )
        {
          CanNm_SetPnClusterResetTimer( pnClusterResetTimerIndex, 0u ); /* SBSW_CANNM_CSL03 */
        }
      }
    }
# endif
  ...
  /*
   * #30 Execute runtime gen data checks
   */
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED == STD_ON )
#  if (CANNM_DEV_ERROR_REPORT == STD_ON )
  {
    CanNm_SizeOfPnInfoByteConfigType sizeOfPnFilterMask = CanNm_GetSizeOfPnInfoByteConfig();
    ...
    if ( CANNM_MAX_PN_INFO_LENGTH < sizeOfPnFilterMask ) /* PRQA S 2992, 2996 */ /* MD_CANNM_Rule14.3, MD_CANNM_Rule2.2 */ /* COV_CANNM_INVALID_GENDATA XF */
    {
      (void)Det_ReportError( CANNM_MODULE_ID, CANNM_INSTANCE_ID, CANNM_SID_INIT, CANNM_E_INVALID_GENDATA ); /* PRQA S 2880 */ /* MD_CANNM_Rule2.1 */
    }
  }
#  endif
# endif
...
CANNM_LOCAL FUNC(boolean, CANNM_CODE) CanNm_TriggerTransmission_PnsrHandling( CANNM_CHANNEL_TYPE_ONLY )
{
  CanNm_TxMessageDataStartIdxOfPbChannelConfigType         txMessageCounter = 0u;
  CanNm_PbPncClusterConfigExtStartIdxOfPbChannelConfigType syncRequestCounter;
   ...
   */
  SchM_Enter_CanNm_CANNM_EXCLUSIVE_AREA_7();
  for ( syncRequestCounter = CanNm_GetPbPncClusterConfigExtStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX); /* FETA_CANNM_CSL_USE_CASE */
        syncRequestCounter < CanNm_GetPbPncClusterConfigExtEndIdxOfPbChannelConfig(CANNM_CHANNEL_IDX);
        syncRequestCounter++ )
  {
    CanNm_SetSyncPncShutdownReq( syncRequestCounter, (CanNm_SyncPncShutdownReqType)(CanNm_GetSyncPncShutdownReq(syncRequestCounter) | CanNm_GetSyncPncShutdownReqNew(syncRequestCounter)) ); /* SBSW_CANNM_CSL02 */
    CanNm_SetSyncPncShutdownReqNew( syncRequestCounter, 0u ); /* SBSW_CANNM_CSL02 */
  }
  SchM_Exit_CanNm_CANNM_EXCLUSIVE_AREA_7();
  ...
  for ( txMessageCounter = CanNm_GetTxMessageData_PnFilterMaskStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX); /* FETA_CANNM_CSL_USE_CASE */
        txMessageCounter < CanNm_GetTxMessageData_PnFilterMaskEndIdxOfPbChannelConfig(CANNM_CHANNEL_IDX);
        txMessageCounter++ )
  {
    syncRequestCounter = (CanNm_PbPncClusterConfigExtStartIdxOfPbChannelConfigType)(CanNm_GetPbPncClusterConfigExtStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX)) + pncByteCounter;

    /* Check if the current Pn byte has a PNC that needs to be shutdown */
    if ( CanNm_GetSyncPncShutdownReq(syncRequestCounter) != 0u )
    {
      uint8 bitsToFilter = CanNm_GetTxMessageData(txMessageCounter) & CanNm_GetSyncPncShutdownReq(syncRequestCounter);

      /* If the Eira Tx is set to 1 for a PNC that needs to be shutdown, cancel the shutdown for this Pnc. */
      if ( bitsToFilter != 0u )
      {
        if ( syncRequestCounter < CanNm_GetPbPncClusterConfigExtEndIdxOfPbChannelConfig(CANNM_CHANNEL_IDX) ) /* COV_CANNM_RUNTIME_CHECK TX */
        {
#  if ( CANNM_IMMEDIATE_TXCONF_ENABLED == STD_OFF )
          CanNm_PnClusterResetTimerExtStartIdxOfPbPncClusterConfigExtType pnClusterResetTimerExtIndex = 0u;
          uint8_least                                                     pnClusterRequestBit = 0u;

          for ( pnClusterResetTimerExtIndex = CanNm_GetPnClusterResetTimerExtStartIdxOfPbPncClusterConfigExt(syncRequestCounter); /* FETA_CANNM_CSL_USE_CASE */
                pnClusterResetTimerExtIndex < CanNm_GetPnClusterResetTimerExtEndIdxOfPbPncClusterConfigExt(syncRequestCounter);
                pnClusterResetTimerExtIndex++ )
          {
...
#  endif
          ...
          CanNm_GetSyncPncShutdownReq(syncRequestCounter) &= ~bitsToFilter; /* SBSW_CANNM_SYNCPNCSHUTDOWNREQ */
        }
      }
    }
    ...
    CanNm_SetTxMessageData( txMessageCounter, CanNm_GetSyncPncShutdownReq(syncRequestCounter) ); /* SBSW_CANNM_CSL03 */
...
# if ( CANNM_PN_EIRA_CALC_FEATURE_ENABLED == STD_ON )
  CanNm_PnClusterReqNewIterType  pnClusterRequestByte;
# endif
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
  boolean                        skipMessage = FALSE;
# endif
...
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
   ...
   *        Call CanNm_TriggerTransmission_PnsrHandling
   */
  if ( context == CANNM_TRIGGER_TRANSMISSION_PNSR )
  {
    skipMessage = CanNm_TriggerTransmission_PnsrHandling( CANNM_CHANNEL_PARA_ONLY );
  }
# endif
...
# endif
  {
    /* #30 If Partial Networking is enabled and PnEiraCalcEnabled is enabled on this channel
      ...
      ) /* COV_CANNM_PN_ENABLED */
    {
      ...
      for ( pnClusterRequestByte = 0u; pnClusterRequestByte < CanNm_GetSizeOfPnClusterReqNew(); pnClusterRequestByte++ )
      {
        ...
        CanNm_SetPnClusterReqNew( pnClusterRequestByte, /* SBSW_CANNM_CSL01 */
          ...
          ((CanNm_GetTxMessageData((CanNm_GetTxMessageDataStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX)) + (pnClusterRequestByte + CanNm_GetPnInfoOffset()))) & CanNm_GetPnFilterMaskOfPnInfoByteConfig(pnClusterRequestByte))) );
      }
      SchM_Exit_CanNm_CANNM_EXCLUSIVE_AREA_0();
    }
# endif
...
# endif
    ...
    /* #70 If CanIf_Transmit returns E_OK
...
#  if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
      if ( context == CANNM_TRIGGER_TRANSMISSION_PNSR )
      {
        CanNm_SetMsgTimeoutTimer(CANNM_CHANNEL_IDX, (CanNm_MsgTimeoutTimerType)CanNm_GetPnSyncShutdownMessageTimeoutTime()); /* SBSW_CANNM_CSL_CHANNEL_INDEX_ACCESS */
      }
      else
#  endif
      {
        /* The MsgTimeoutTimer handling is done prior to MsgTimerHandling. That means the timeout value can be exactly as the generated value for the regular use cases.
           ...
           This is due to the fact that the CanNm_MainFunction can be executed shortly after the asynchronous TriggerTransmission was called. That means
        ...
        CanNm_MsgTimeoutTimerType msgTimeoutTimerValue = (CanNm_MsgTimeoutTimerType)CanNm_GetMsgTimeoutTimeOfChannelConfig(CANNM_CHANNEL_IDX);
#  if ( CANNM_BUS_SYNCHRONIZATION_ENABLED == STD_ON )
        if ( context == CANNM_TRIGGER_TRANSMISSION_REQ_BUS_SYNC )
        {
          msgTimeoutTimerValue++;
        }
#  endif
        CanNm_SetMsgTimeoutTimer( CANNM_CHANNEL_IDX, msgTimeoutTimerValue ); /* SBSW_CANNM_CSL_CHANNEL_INDEX_ACCESS */
      }
# endif
    }
  }
...
# endif
  CANNM_DUMMY_STATEMENT(context);  /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
  return retVal;
}
#endif /* ( CANNM_PASSIVE_MODE_ENABLED == STD_OFF ) */
...
CANNM_LOCAL_INLINE FUNC(void, CANNM_CODE) CanNm_LocalInit(CANNM_CHANNEL_TYPE_ONLY)
{
  ...
  CanNm_RxMessageDataStartIdxOfPbChannelConfigType rxUserDataByte;
#endif
#if ( CANNM_TXMESSAGEDATA == STD_ON )
  CanNm_TxMessageDataStartIdxOfPbChannelConfigType txUserDataByte;
...
#if ( CANNM_TXMESSAGEDATA == STD_ON )
  for( txUserDataByte = CanNm_GetTxMessageDataStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX); /* FETA_CANNM_CSL_USE_CASE */
...
#if ( CANNM_RXMESSAGEDATA == STD_ON )
  for( rxUserDataByte = CanNm_GetRxMessageDataStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX); /* FETA_CANNM_CSL_USE_CASE */
...
# if ( CANNM_IMMEDIATE_TXCONF_ENABLED == STD_OFF )
        /* If ImmediateTxConf is enabled, the PNSR array can be cleared without condition. */
        if ( (CanNm_GetSyncPncShutdownReq(syncPncCounter) != 0u) || (CanNm_GetSyncPncShutdownReqNew(syncPncCounter) != 0u) )
# endif
        {
# if ( CANNM_IMMEDIATE_TXCONF_ENABLED == STD_OFF )
          /* If ImmediateTxConf is enabled, the timers will never be started due to a PNSR. */
          uint8_least                                                     pnClusterRequestBit = 0u;
          CanNm_PnClusterResetTimerExtStartIdxOfPbPncClusterConfigExtType pnClusterResetTimerExtIndex = 0u;

          /* The timers must also be stopped. */
          for ( pnClusterResetTimerExtIndex = CanNm_GetPnClusterResetTimerExtStartIdxOfPbPncClusterConfigExt(syncPncCounter); /* FETA_CANNM_CSL_USE_CASE */
                pnClusterResetTimerExtIndex < CanNm_GetPnClusterResetTimerExtEndIdxOfPbPncClusterConfigExt(syncPncCounter);
                pnClusterResetTimerExtIndex++ )
          {
            ...
            if ( (CanNm_GetSyncPncShutdownReq(syncPncCounter) & (1u << pnClusterRequestBit)) != 0u )
            {
              CanNm_SetPnClusterResetTimerExt( pnClusterResetTimerExtIndex, 0u ); /* SBSW_CANNM_CSL03 */
            }
...
# endif
          ...
          CanNm_SetSyncPncShutdownReq( syncPncCounter, 0u ); /* SBSW_CANNM_CSL02 */
          CanNm_SetSyncPncShutdownReqNew( syncPncCounter, 0u ); /* SBSW_CANNM_CSL02 */
        }
      }
    }
#endif
...
# endif
  ...
  if( additionalTransmissionRequired == TRUE )
  {
    ...
    retVal = CanNm_TriggerTransmission( CANNM_CHANNEL_PARA_FIRST CANNM_TRIGGER_TRANSMISSION_REGULAR );
                                                      ...
                                                      CONSTP2CONST( uint8, AUTOMATIC, CANNM_APPL_VAR ) nmUserDataPtr )
{
  ...
  Std_ReturnType                                            retVal = E_NOT_OK; /* PRQA S 2981 */ /* MD_MSR_RetVal */
  uint8                                                     errorId = CANNM_E_NO_ERROR;
  CanNm_TxMessageData_UserDataStartIdxOfPbChannelConfigType txMesssageDataByte;
...
#  endif
  /* #20 If module is initialized and parameter checks are passed */
  {
    CanNm_TxMessageDataIterType userDataByte = 0u;
# if !defined ( CANNM_OPTIMIZE_CHANNEL_ENABLED )
    channel = CanNm_GetSysToNmChInd(nmChannelHandle);
# endif
     ...
     */
    for( txMesssageDataByte = CanNm_GetTxMessageData_UserDataStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX); /* FETA_CANNM_CSL_USE_CASE */
                                                      ...
                                                      CONSTP2VAR( uint8, AUTOMATIC, CANNM_APPL_VAR ) nmUserDataPtr )
{
  ...
  Std_ReturnType                                            retVal = E_NOT_OK; /* PRQA S 2981 */ /* MD_MSR_RetVal */
  uint8                                                     errorId = CANNM_E_NO_ERROR;
  CanNm_RxMessageData_UserDataStartIdxOfPbChannelConfigType userDataByte;
...
# endif
     ...
     */
    for( userDataByte = 0u; userDataByte < CanNm_GetRxMessageData_UserDataLengthOfPbChannelConfig(CANNM_CHANNEL_IDX); userDataByte++ ) /* FETA_CANNM_CSL_USE_CASE */
    {
                                                     ...
                                                     CONSTP2VAR( uint8, AUTOMATIC, CANNM_APPL_VAR ) nmPduDataPtr )
{
  ...
  Std_ReturnType                                   retVal = E_NOT_OK; /* PRQA S 2981 */ /* MD_MSR_RetVal */
  uint8                                            errorId = CANNM_E_NO_ERROR;
  CanNm_RxMessageDataStartIdxOfPbChannelConfigType msgDataByte;
...
# endif
    {
      ...
      for( msgDataByte = 0u; msgDataByte < CanNm_GetRxMessageDataLengthOfPbChannelConfig(CANNM_CHANNEL_IDX); msgDataByte++ ) /* FETA_CANNM_CSL_USE_CASE */
      {
        /* ESCAN00064309 */
...
# endif
    ...
    {
      retVal = CanNm_TriggerTransmission( CANNM_CHANNEL_PARA_FIRST CANNM_TRIGGER_TRANSMISSION_REQ_BUS_SYNC );
    }
  }
  /* ----- Development Error Report --------------------------------------- */
# if ( CANNM_DEV_ERROR_REPORT == STD_ON )
 ...
 *
  */
/* PRQA S 6080 1 */ /* MD_MSR_STMIF */
FUNC( Std_ReturnType, CANNM_CODE ) CanNm_RequestSynchronizedPncShutdown( CONST( NetworkHandleType, AUTOMATIC ) nmChannelHandle, CONST( PNCHandleType, AUTOMATIC ) pncId )
{
  /* ----- Local Variables ---------------------------------------------- */
  Std_ReturnType    retVal = E_NOT_OK;
  uint8             errorId = CANNM_E_NO_ERROR;
...
# endif
  ...
  CanNm_Rtm_Start( RtmMeasurementPoint_CANNM_SID_REQUESTSYNCHRONIZEDPNCSHUTDOWN )
...
# if ( CANNM_DEV_ERROR_DETECT == STD_ON ) /* COV_CANNM_DEV_ERROR_DETECT_ON */
      ...
      if ( CanNm_GetNmState(CANNM_CHANNEL_IDX) < NM_STATE_READY_SLEEP )
      {
        errorId = CANNM_E_REQUEST_PN_SHUTDOWN_MESSAGE_INVALID_STATE;
      }
      else
# endif
      {
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
        ...
        if ( CanNm_IsSynchronizedPncShutdownEnabledOfChannelConfig(CANNM_CHANNEL_IDX) ) /* COV_SYNCHRONIZED_PNC_SHUTDOWN */
        {
#if ( CANNM_PNC_ID_COUNTING_START_FROM_PNC_VECTOR == STD_ON )
          PNCHandleType byteIndex = pncId / 8u;
#else
          PNCHandleType byteIndex = (pncId / 8u) - CanNm_GetPnInfoOffset();
#endif          
          uint8 bitPosition = (uint8)(pncId % 8u);

          /* Only proceed with the request if the channel is actively coordinated. */
          if ( ( ( CanNm_GetPbPncClusterConfigExtStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX) + byteIndex) < CANNM_MAX_SYNCPNCSHUTDOWNREQNEW ) && CanNm_IsActivePncCoordinatorChannelOfChannelConfig(CANNM_CHANNEL_IDX) ) /* COV_SYNCHRONIZED_PNC_SHUTDOWN */
          {
            CanNm_SizeOfSyncPncShutdownReqNewType calculatedByteIndex = (CanNm_SizeOfSyncPncShutdownReqNewType)(CanNm_GetPbPncClusterConfigExtStartIdxOfPbChannelConfig(CANNM_CHANNEL_IDX) + byteIndex);

            if ( calculatedByteIndex < CanNm_GetSizeOfSyncPncShutdownReqNew() ) /* COV_CANNM_RUNTIME_CHECK TX */
            {
              /* No filter mask used here. Same as for Tx Eira. */
              /* #2000 Store the request for the Pnc. */
              CanNm_SetSyncPncShutdownReqNew( calculatedByteIndex, CanNm_GetSyncPncShutdownReqNew(calculatedByteIndex) | (1u << bitPosition) ); /* SBSW_CANNM_REQUEST_SYNC_PNC_SHUTDOWN */
...
# else
        CANNM_DUMMY_STATEMENT_CONST(pncId); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
# endif
      }
    }
...
# if ( CANNM_DEV_ERROR_REPORT == STD_ON )
  ...
  if ( errorId != CANNM_E_NO_ERROR )
  {
    (void)Det_ReportError( CANNM_MODULE_ID, CANNM_INSTANCE_ID, CANNM_SID_REQUESTSYNCHRONIZEDPNCSHUTDOWN, errorId );
  }
# else
  CANNM_DUMMY_STATEMENT(errorId); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
# endif
  CanNm_Rtm_Stop( RtmMeasurementPoint_CANNM_SID_REQUESTSYNCHRONIZEDPNCSHUTDOWN )
  return retVal;
}
#endif
...
#if ( CANNM_PN_ERA_CALC_FEATURE_ENABLED == STD_ON )
      /* #300 Calculate Pn Era (if PnEraCalcEnabled enabled) */
...
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
      /* #320 Process PNSR message handling (if PNSR enabled) */
      CanNm_PnsrMessageHandling(CANNM_CHANNEL_PARA_ONLY);
# endif

      /* #330 Process Tx Message Timer */
      CanNm_MessageTimerHandling( CANNM_CHANNEL_PARA_ONLY );

#endif /* ( CANNM_PASSIVE_MODE_ENABLED == STD_OFF ) */

#if ( CANNM_PN_EIRA_CALC_FEATURE_ENABLED == STD_ON )
      /* #340 Calculate Pn Eira (if PnEiraCalcEnabled enabled) */
...
#if !( defined ( CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG ) && \
      ( ( CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG == STD_OFF ) || ( CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG == STD_ON ) ) )
# error: "CAN NM configuration issue: Synchronized PNC Shutdown has to be defined STD_OFF or STD_ON"
#endif

#if !( defined ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED ) && \
      ( ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_OFF ) || ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON ) ) )
# error: "CAN NM configuration issue: Synchronized PNC Shutdown TX Feature has to be defined STD_OFF or STD_ON"
#endif

#if !( defined ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED ) && \
      ( ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED == STD_OFF ) || ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED == STD_ON ) ) )
# error: "CAN NM configuration issue: Synchronized PNC Shutdown RX Feature has to be defined STD_OFF or STD_ON"
#endif

#if !( defined ( CANNM_PNSR_MSG_TIMEOUT_TICK_EQ_ONE ) && \
      ( ( CANNM_PNSR_MSG_TIMEOUT_TICK_EQ_ONE == STD_OFF ) || ( CANNM_PNSR_MSG_TIMEOUT_TICK_EQ_ONE == STD_ON ) ) )
# error: "CAN NM configuration issue: Synchronized PNC Shutdown Msg Timeout Time Eq One has to be defined STD_OFF or STD_ON"
#endif

#if !( defined ( CANNM_PNC_ID_COUNTING_START_FROM_PNC_VECTOR ) && \
      ( ( CANNM_PNC_ID_COUNTING_START_FROM_PNC_VECTOR == STD_OFF ) || ( CANNM_PNC_ID_COUNTING_START_FROM_PNC_VECTOR == STD_ON ) ) )
# error: "CAN NM configuration issue: The PNC Id counting starting from PNC Vector has to be defined STD_OFF or STD_ON"
#endif
...
#if ( CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG == STD_ON )
# if ( ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_OFF ) && ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED == STD_OFF ) )
#  error: "CAN NM configuration issue: CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED and CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED cannot both be deactivated if CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG is STD_ON."
# endif
#else
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED == STD_ON )
#  error: "CAN NM configuration issue: CANNM_SYNC_SHUTDOWN_FEATURE_TX_ENABLED cannot be activated if CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG is STD_OFF."
# endif
# if ( CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED == STD_ON )
#  error: "CAN NM configuration issue: CANNM_SYNC_SHUTDOWN_FEATURE_RX_ENABLED cannot be activated if CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG is STD_OFF."
# endif
# if ( CANNM_PNSR_MSG_TIMEOUT_TICK_EQ_ONE == STD_ON )
#  error: "CAN NM configuration issue: CANNM_PNSR_MSG_TIMEOUT_TICK_EQ_ONE cannot be activated if CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG is STD_OFF."
# endif
# if ( CANNM_PNSYNCSHUTDOWNRETRANSMISSIONDURATION == STD_ON )
#  error: "CAN NM configuration issue: CANNM_PNSYNCSHUTDOWNRETRANSMISSIONDURATION cannot be activated if CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG is STD_OFF."
# endif
# if ( CANNM_PNSYNCSHUTDOWNERRORREACTIONENABLED == STD_ON )
#  error: "CAN NM configuration issue: CANNM_PNSYNCSHUTDOWNERRORREACTIONENABLED cannot be activated if CANNM_SYNCHRONIZEDPNCSHUTDOWNENABLEDOFCHANNELCONFIG is STD_OFF."
# endif
#endif
...
\ID                SBSW_CANNM_CSL01
\DESCRIPTION       Access of CanNm_PnClusterReqNew using ComStackLib
\COUNTERMEASURE \N Qualified use-case CSL01 of ComStackLib

\ID                SBSW_CANNM_CSL02
\DESCRIPTION       Access of CanNm_PnClusterReq, CanNm_PnClusterReqNew via size relation to CanNm_PnInfoByteConfig structure and
                   access of CanNm_PnClusterReqExt, CanNm_PnClusterReqExtNew, CanNm_SyncPncShutdownReq, CanNm_SyncPncShutdownReqNew, CanNm_SyncPncShutdownReqExt via size relation to CanNm_PbPncClusterConfigExt structure
                   with same index using ComStackLib.
\COUNTERMEASURE \N Qualified use-case CSL02 of ComStackLib

\ID                SBSW_CANNM_CSL03
\DESCRIPTION       Access of CanNm_PnClusterResetTimer, CanNm_PnClusterResetTimerExt, CanNm_RxMessageData, CanNm_TxMessageData using indirections created via the ComStackLib.
...
\DESCRIPTION       Access of CanNm_TxControlState, CanNm_TxControlStateReq, CanNm_TimeoutTimer, CanNm_RemoteSleepTimer, CanNm_MsgTimer, CanNm_MsgConfirmationFlag,
                             ...
                             CanNm_CommState, CanNm_NetworkRestartFlag, CanNm_RepeatMsgReqFlag, CanNm_PnsrMessageTransmissionOngoing
...
\ID                SBSW_CANNM_REQUEST_SYNC_PNC_SHUTDOWN
\DESCRIPTION       The array CanNm_SyncPncShutdownReqNew is accessed with a calculated index.
\COUNTERMEASURE \R The size of the calculated index is checked before accessing the array.

\ID                SBSW_CANNM_FILTERED_PN_INFO_ACCESS
\DESCRIPTION       The write access index of the local array that is created with the define CANNM_MAX_PN_INFO_LENGTH can be at maximum the return value of the function CanNm_DetermineDynamicPnInfoLength.
                   It is guaranteed that the value is never greater than the CanNm_PnInfoByteConfig via a runtime check.
\COUNTERMEASURE \R The runtime check in the Init function guarantees that no out of bounds access can occur.

\ID                SBSW_CANNM_DYN_PN_INFO_LENGTH
\DESCRIPTION       The write access index of array CanNm_PnClusterReqNew is calculated at runtime by the local function CanNm_DetermineDynamicPnInfoLength().
                   The function CanNm_DetermineDynamicPnInfoLength() shall always return an index value smaller or equal to the size of CanNm_PnInfoByteConfig.
                   CanNm_PnClusterReqNew has a size relation to CanNm_PnInfoByteConfig.
...
\DESCRIPTION       The array CanNm_PnClusterReqExtNew is accessed with a calculated index. The size of the index is checked via runtime check.
\COUNTERMEASURE \R A runtime check guarantees that the array is not accessed out of bounds.

\ID                SBSW_CANNM_SYNCPNCSHUTDOWNREQ
\DESCRIPTION       The array CanNm_SyncPncShutdownReq is accessed with a calculated index. The size of the index is checked via runtime check. The checked size is for the end index of the PnInfoByteConfig.
                   Since the creation is done with the same const size relation, the CSL02 guarantees no out of bounds access.
\COUNTERMEASURE \R A runtime check guarantees that the array is not accessed out of bounds. Qualified use-case CSL02 of ComStackLib.

\ID                SBSW_CANNM_SYNCPNCSHUTDOWNREQEXT
\DESCRIPTION       The array CanNm_SyncPncShutdownReqExt is accessed with a calculated index. The size of the index is checked via runtime check. The checked size is for the array CanNm_PnClusterReqExtNew.
                   Since the creation is done with the same const size relation, the CSL02 guarantees no out of bounds access.
\COUNTERMEASURE \R A runtime check guarantees that the array is not accessed out of bounds. Qualified use-case CSL02 of ComStackLib.

\ID                SBSW_CANNM_LOCAL_FUNCTION_POINTER_ACCESS
\DESCRIPTION       The pointer parameters pnClusterRequestExtNew, pnClusterRequestChanged, syncPncShutdownReqExtLocal and pnSyncClusterReqChanged are written in the function CanNm_PnEraCalculation_NewDataHandling. The variables are created locally in the calling function CanNm_PnEraCalculation.
\COUNTERMEASURE \N Local variables are always instantiated when the function is called.

\ID                SBSW_CANNM_LOCAL_FUNCTION_CALL_WITH_LOCAL_VARS
\DESCRIPTION       The function call CanNm_PnEraCalculation_NewDataHandling is only done with local variables. Therefore it is ensured that the passed pointer is not NULL.
\COUNTERMEASURE \N Local variables are always instantiated when the function is called.

SBSW_JUSTIFICATION_END */

/* FETA_JUSTIFICATION_BEGIN
\ID                FETA_CANNM_CSL_USE_CASE
...
\ID                FETA_CANNM_DYNAMIC_PN_INFO_LENGTH
\DESCRIPTION       The local function CanNm_DetermineDynamicPnInfoLength returns the maximum number of PnInfo Bytes depending on current PDU Length.
                   The return value of this function is of the data type CanNm_SizeOfPnInfoByteConfigType. Therefore the return value of this function guarantees a defined upper limit of the loop.
                   Besides the stepwise loop increments, the loop variable is not modified elsewhere.
...
\ID COV_CANNM_ERA_CALC_SYNC_SHUTDOWN_TX
   \ACCEPT TF tf tf tx
   \ACCEPT TF tx tf xf
   \ACCEPT TF tf tf xf
   \ACCEPT XF tf xf xf
   \REASON The configuration has only actively coordinated channels. Therefore the first decision coverage cannot be fulfilled other than tx.
           ...
           If the RetransmissionDuration is not enabled and the MsgTimeoutTime tick equals one, the timer is never started due to the PNSR. The condition is therefore XF tf xf xf.

\ID COV_CANNM_ERA_CALC_SYNC_SHUTDOWN_RX
...
\ID COV_SYNCHRONIZED_PNC_SHUTDOWN
   \ACCEPT TX
   \ACCEPT XF
   \ACCEPT TF tf tx
...
\ID COV_CANNM_INVALID_GENDATA
   \ACCEPT XF
   \ACCEPT TF tf tx
   \REASON This statements checks if the configuration is correctly generated.
...
\ID COV_CANNM_PNSR_MSG_TIMEOUT_TIME_EQ_ONE_NO_ADDITIONAL_PNSR_MESSAGE
   \REASON If the PnsrMsgTimeouTime tick equals one and the Retransmission Duration is disabled, it is not possible to still have a pending PNSR message. The Det is thrown before the condition can be evaluated to true.
           Therefore also the SyncPncClusterReq array is set to zero for all pending Pncs.
