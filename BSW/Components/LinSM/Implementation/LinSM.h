 ...
 *  06.00.00  2018-08-02  vismpf  STORYC-5677:   MISRA-C:2012 Compliance
 *  07.00.00  2019-05-13  vismfi  STORYC-8321    Node Type SLAVE support
 *  07.00.01  2020-05-13  vismfi  ESCAN00106366  LIN (slave) does not reach NO_COMMUNICATION state
 *  07.00.02  2020-07-07  vismfi  ESCAN00106690  LIN (slave) NO_COM not reached
                                  ESCAN00106689  LIN (slave) Wakeup repeated endless if silence timer is zero
                                  ESCAN00106871  LINSM EXCLUSIVE_AREA_3 is not ended correctly
 *  07.00.03  2022-09-06  vispps  ESCAN00112535  LinSM channels without SleepSupport: ComM NO_COM is only indicated once during first startup and shutdown of the channel
 *********************************************************************************************************************/
...
#define LINSM_SW_MAJOR_VERSION                    (0x07u)
#define LINSM_SW_MINOR_VERSION                    (0x00u)
#define LINSM_SW_PATCH_VERSION                    (0x03u)
...
#define LINSM_SID_GOTO_SLEEP_INDICATION           (0x03u)
...
#define LINSM_E_NODE_TYPE                         (0x6Au) /*!< Error code: API service called at a channel where the wrong node type (Master/Slave) is configured  */
 ...
 *  \reentrant   FALSE
 *  \synchronous FALSE
 *  \config      LINSM_MASTERNODE_POSSIBLE == STD_ON, i.e. LinSMNodeType has been set to MASTER at least at one cluster
 ...
 * \protects LinSM_ComState, LinSM_RequestedComMode, LinSM_WakeUpRetryCounter, LinSM_ConfirmationTimer, LinSM_DesiredConfirmation, LinSM_SilenceAfterWakeupTimer
 * \usedin LinSM_RequestComMode()
 * \exclude LinSM_MainFunction(), LinSM_GotoSleepIndication(), LinIf_MainFunction() and BswMLinScheduleSwitch action (call context depends on configuration)
 ...
 * \protects LinSM_ComState, LinSM_RequestedComMode, LinSM_ConfirmationTimer, LinSM_WakeUpRetryCounter, LinSM_NegativeConfirmation, LinSM_DesiredConfirmation, LinSM_SilenceAfterWakeupTimer
 * \usedin LinSM_WakeupConfirmation(), LinSM_GotoSleepIndication(), LinSM_GotoSleepConfirmation() and LinSM_ScheduleRequestConfirmation()
 * \exclude ComM_MainFunction(), LinSM_GotoSleepIndication(), LinSM_MainFunction() and BswMLinScheduleSwitch action (call context depends on configuration)
 ...
 * \exclude ComM_MainFunction(), LinSM_GotoSleepIndication(), LinIf_MainFunction() and BswMLinScheduleSwitch action (call context depends on configuration)
 ...
 * \exclude ComM_MainFunction(), LinSM_GotoSleepIndication(), LinIf_MainFunction() and LinSM_MainFunction()
 ...
 * \endexclusivearea
 *
 * \exclusivearea LINSM_EXCLUSIVE_AREA_4
 * Protects LinSM_ModeManager related variables from being modified while a go to sleep indication  is processed.
 * \protects LinSM_ComState, LinSM_RequestedComMode, LinSM_ConfirmationTimer, LinSM_NegativeConfirmation, LinSM_DesiredConfirmation
 * \usedin LinSM_GotoSleepIndication()
 * \exclude LinSM_MainFunction(), LinSM_RequestComMode() and LinSM_WakeupConfirmation() action (call context depends on configuration)
 * \length LONG This exclusive area calls to other BSW modules (LinIf, ComM and BswM).
 * \endexclusivearea
 *
 */
