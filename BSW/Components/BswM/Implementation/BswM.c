...
#else
# define BswM_GetInternalPartitionIdx(index) (BswM_GetPartitionIdentifiersOfPCConfig()[0u].PCPartitionConfigIdxOfPartitionIdentifiers)
#endif
...
{
  /* ----- Local Variables ---------------------------------------------- */
  BswM_ModeRequestQueueIterType immediateUserIndex, iterationIdx;
  ...
  BswM_EnterCriticalArea_0();
  for( iterationIdx = 0u; iterationIdx < BswM_GetSizeOfModeRequestQueue(partitionIdx); iterationIdx++ ) /* COV_BSWM_FINITE_EXECUTION_TIME */
  {
    if((executeActionLists == TRUE) && (BswM_IsQueueWritten(partitionIdx)))
    {
      ...
      for(immediateUserIndex = 0u; immediateUserIndex < BswM_GetSizeOfModeRequestQueue(partitionIdx); immediateUserIndex++)
      {
        ...
        {
          for(ruleIndex = BswM_GetRulesIndStartIdxOfImmediateUser(immediateUserIndex, partitionIdx); ruleIndex < BswM_GetRulesIndEndIdxOfImmediateUser(immediateUserIndex, partitionIdx); ruleIndex++) /* FETA_BSWM_CSL_USE_CASE */
          {
            {
...
# endif
    ...
    {
      break; /* no action list has been executed in previous iteration or no more request is queued */
    }
  }
  BswM_LeaveCriticalArea_0();
...
# endif
      )
  {
    ...
    for (immediateUserIndex = start; immediateUserIndex < end; immediateUserIndex++) /* FETA_BSWM_IMMEDIATE_MODE_REQUEST_USERS */
    {
# if ( BSWM_FORCEDOFIMMEDIATEUSER == STD_ON )
...
# endif
  {
     ...
     *            +--------+              +--------+
     */
    while ((child > 0u) && (BswM_GetActionListPriorityQueue(parent, partitionIdx) > actionListIndex)) /* FETA_BSWM_MIN_HEAP_INSERT */
    {
...
BSWM_LOCAL_INLINE FUNC(BswM_SizeOfActionListPriorityQueueType, BSWM_CODE) BswM_DetermineMinimalQueueIdx(uint8 executionLevel, BswM_PCPartitionConfigIdxOfPartitionIdentifiersType partitionIdx)
{
  ...
  for( tempPriorityQueueIdx = 0u; tempPriorityQueueIdx < BswM_GetLengthOfActionListPriorityQueue(partitionIdx); tempPriorityQueueIdx++ ) /* FETA_BSWM_QUEUELENGTH */
  {
...
BSWM_LOCAL_INLINE FUNC(void, BSWM_CODE) BswM_RemoveIdxFromQueue(BswM_SizeOfActionListPriorityQueueType index, BswM_PCPartitionConfigIdxOfPartitionIdentifiersType partitionIdx)
{
  ...
  BswM_SizeOfActionListPriorityQueueType iterationIdx;
  ...
  /* #20 If one of the children of the element on the new position has a smaller index compared to the parent, the
         ...
         repeated as long as the heap property is not fulfilled. The maximum number of iterations is limited by the size of
         BswM_ActionListPriorityQueueIterType datatype. If the min-heap property is not restored after these iterations, there
         is something wrong with our PriorityQueue. */
  for( iterationIdx = 0u; iterationIdx < ( sizeof(BswM_ActionListPriorityQueueIterType) * BSWM_BYTE_TO_BIT ); iterationIdx++ ) /* COV_BSWM_FINITE_EXECUTION_TIME */
  {
...
#if ((BSWM_FUNCTION_BASED == STD_OFF) && (BSWM_ACTIONS == STD_ON))
/**********************************************************************************************************************
...
FUNC(Std_ReturnType, BSWM_CODE) BswM_Action_ActionListHandler(BswM_HandleType handleId, BswM_PCPartitionConfigIdxOfPartitionIdentifiersType partitionIdx)
{
  ...
  /* ----- Implementation ----------------------------------------------- */
  for(itemId = BswM_GetActionItemsStartIdxOfActionLists(handleId, partitionIdx); itemId < BswM_GetActionItemsEndIdxOfActionLists(handleId, partitionIdx); itemId++) /* FETA_BSWM_CSL_USE_CASE */
  {
...
BSWM_LOCAL_INLINE FUNC(void, BSWM_CODE) BswM_DecreaseTimer(BswM_PCPartitionConfigIdxOfPartitionIdentifiersType partitionIdx)
{
  ...
  /* ----- Implementation ----------------------------------------------- */
  for(timerIndex = 0u; timerIndex < BswM_GetSizeOfTimerValue(partitionIdx); timerIndex++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
  {
...
BSWM_LOCAL_INLINE FUNC(void, BSWM_CODE) BswM_ExecuteActionLists(uint8 executionLevel, BswM_PCPartitionConfigIdxOfPartitionIdentifiersType partitionIdx)
{
  ...
  /* ----- Implementation ----------------------------------------------- */
# if (BSWM_ACTIONLISTPRIORITYQUEUE == STD_ON)
  for (actionListIndex = BswM_PopFromActionListQueue(executionLevel, partitionIdx); /* FETA_BSWM_EXECUTE_ACTION_LISTS */
...
BSWM_LOCAL_INLINE FUNC(void, BSWM_CODE) BswM_ExecuteDeferredRules(BswM_PCPartitionConfigIdxOfPartitionIdentifiersType partitionIdx)
{
  ...
  /* ----- Implementation ----------------------------------------------- */
  for (ruleIndex = 0u; ruleIndex < BswM_GetSizeOfDeferredRules(partitionIdx); ruleIndex++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
  {
# if (BSWM_FUNCTION_BASED == STD_ON)
...
# endif
...
#endif
  {
    ...
    for (partitionCounter = 0u; partitionCounter < BswM_GetSizeOfPartitionIdentifiers(); partitionCounter++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
# endif
    ...
    /* ----- Implementation ----------------------------------------------- */
  {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfActionListQueue(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
# endif
    ...
    for (idx = 0u; idx < BswM_GetSizeOfRules(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfCanSMChannelState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfComMChannelState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
# endif
    ...
    for (idx = 0u; idx < BswM_GetSizeOfComMPncState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#if (BSWM_DOIPSETCHANNELREADYSTATE == STD_ON)
# if((BSWM_POSTBUILD_VARIANT_SUPPORT == STD_ON && BSWM_USE_INIT_POINTER == STD_ON) || (BSWM_MULTIPARTITION == STD_ON))
    for (idx = 0u; idx < BswM_GetSizeOfDoIPSetChannelReadyMapping(partitionIdx); idx++) /* Size can only be 0 or 1. */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
# endif
    {
      BswM_SetDoIPSetChannelReadyState(BswM_GetInitValueOfDoIPSetChannelReadyMapping(0u, partitionIdx), partitionIdx); /* SBSW_BSWM_PARTITIONINDEXACCESS */
    }
#endif

#if (BSWM_DOIPINTSETCHANNELREADYSTATE == STD_ON)
# if((BSWM_POSTBUILD_VARIANT_SUPPORT == STD_ON && BSWM_USE_INIT_POINTER == STD_ON) || (BSWM_MULTIPARTITION == STD_ON))
    for (idx = 0u; idx < BswM_GetSizeOfDoIPIntSetChannelReadyMapping(partitionIdx); idx++) /* Size can only be 0 or 1. */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
# endif
    {
      BswM_SetDoIPIntSetChannelReadyState(BswM_GetInitValueOfDoIPIntSetChannelReadyMapping(0u, partitionIdx), partitionIdx); /* SBSW_BSWM_PARTITIONINDEXACCESS */
    }
#endif

#if (BSWM_DCMCOMSTATE == STD_ON)
    for (idx = 0u; idx < BswM_GetSizeOfDcmComState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
# endif
    ...
    for (idx = 0u; idx < BswM_GetSizeOfEcuMRunRequestState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfEcuMWakeupState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfEthIfPortState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfEthSMState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfFrSMState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfGenericState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
# endif
    ...
    for (idx = 0u; idx < BswM_GetSizeOfJ1939NmState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfLinScheduleState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfLinScheduleEndState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfLinSMState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfLinTPState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfNmState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfNvMJobState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfNvMBlockState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfPduRPreTransmitState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfPduRRxIndicationState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfPduRTpRxIndicationState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfPduRTpStartOfReceptionState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfPduRTpTxConfirmationState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfPduRTransmitState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfPduRTxConfirmationState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfSdClientServiceState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfSdConsumedEventState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfSdEventHandlerState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#if (BSWM_TIMERSTATE == STD_ON)
    for (idx = 0u; idx < BswM_GetSizeOfTimerState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
    ...
    for (idx = 0u; idx < BswM_GetSizeOfWdgMState(partitionIdx); idx++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
# endif
  ...
  {
#if(BSWM_GENERICSTATE == STD_ON)
    for(userHandle = 0u; userHandle < BswM_GetSizeOfGenericState(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
# if(BSWM_IMMEDIATEUSERUSEDOFGENERICMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfGenericMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfGenericMapping(userHandle, partitionIdx), BSWM_REQUESTMODE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
# endif
...
# endif
  ...
  {
# if (BSWM_COMMCHANNELSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfComMChannelState(partitionIdx); userHandle++) /* COV_BSWM_ALLNETWORKPORTS */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFCOMMCHANNELMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfComMChannelMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfComMChannelMapping(userHandle, partitionIdx), BSWM_COMM_CURRENTMODE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
#  endif
    {
      ...
      BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfComMInitiateResetMapping(0u, partitionIdx), BswM_GetImmediateUserEndIdxOfComMInitiateResetMapping(0u, partitionIdx), BSWM_COMM_INITIATERESET_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
...
# endif
  ...
  {
# if (BSWM_COMMPNCSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfComMPncState(partitionIdx); userHandle++) /* COV_BSWM_ALLNETWORKPORTS */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFCOMMPNCMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfComMPncMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfComMPncMapping(userHandle, partitionIdx), BSWM_COMM_PNC_CURRENTMODE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
#  endif
    {
      ...
      BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfDcmApplUpdateMapping(0u, partitionIdx), BswM_GetImmediateUserEndIdxOfDcmApplUpdateMapping(0u, partitionIdx), BSWM_DCM_APPLICATION_UPDATED_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
...
FUNC(void, BSWM_CODE) BswM_DoIP_SetChannelReady(NetworkHandleType Channel)
{
  /* ----- Local Variables ---------------------------------------------- */
  uint8 errorId = BSWM_E_NO_ERROR;
# if (BSWM_IMMEDIATEUSERUSEDOFDOIPSETCHANNELREADYMAPPING == STD_ON) && ((BSWM_POSTBUILD_VARIANT_SUPPORT == STD_ON && BSWM_USE_INIT_POINTER == STD_ON) || BSWM_MULTIPARTITION == STD_ON)
  BswM_DoIPSetChannelReadyMappingIterType userHandle;
# endif
  BswM_PCPartitionConfigIdxOfPartitionIdentifiersType partitionIdx = BswM_GetInternalPartitionIdx(GetApplicationID());
...
#  endif
  if (!BswM_IsInitialized(partitionIdx)) /* COV_BSWM_INITIALIZED */
  {
    errorId = BSWM_E_NO_INIT;
  }
  else
# endif
  /* ----- Implementation ----------------------------------------------- */
  {
# if(BSWM_IMMEDIATEUSERUSEDOFDOIPSETCHANNELREADYMAPPING == STD_ON)
#  if((BSWM_POSTBUILD_VARIANT_SUPPORT == STD_ON && BSWM_USE_INIT_POINTER == STD_ON) || (BSWM_MULTIPARTITION == STD_ON))
    for (userHandle = 0u; userHandle < BswM_GetSizeOfDoIPSetChannelReadyMapping(partitionIdx); userHandle++) /* Size can only be 0 or 1. */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
#  endif
    {
#  if (BSWM_DEV_ERROR_DETECT == STD_ON) /* COV_BSWM_DEV_ERROR_DETECT TX */
#   if (BSWM_IMMEDIATEUSERUSEDOFDOIPINTSETCHANNELREADYMAPPING == STD_ON)
      if ((BswM_GetDoIPIntSetChannelReadyState(partitionIdx) == TRUE) || (BswM_GetDoIPSetChannelReadyState(partitionIdx) == TRUE)) /* COV_BSWM_CONCURRENT_REQUEST */
#   else
      if (BswM_GetDoIPSetChannelReadyState(partitionIdx) == TRUE) /* COV_BSWM_CONCURRENT_REQUEST */
#   endif
      {
        errorId = BSWM_E_ALREADY_QUEUED;
      }
      else
#  endif
      {
        BswM_SetDoIPSetChannelReadyState(TRUE, partitionIdx); /* SBSW_BSWM_PARTITIONINDEXACCESS */
        BswM_SetDcmSetChannelReadyChannel(Channel, partitionIdx); /* SBSW_BSWM_PARTITIONINDEXACCESS */
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfDoIPSetChannelReadyMapping(0u, partitionIdx), BswM_GetImmediateUserEndIdxOfDoIPSetChannelReadyMapping(0u, partitionIdx), BSWM_DOIP_SET_CHANNEL_READY_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
        BswM_SetDoIPSetChannelReadyState(FALSE, partitionIdx); /* SBSW_BSWM_PARTITIONINDEXACCESS */
      }
    }
# endif
  }
  BSWM_DUMMY_STATEMENT(partitionIdx); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
  BSWM_DUMMY_STATEMENT(Channel); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
  /* ----- Development Error Report --------------------------------------- */
# if (BSWM_DEV_ERROR_REPORT == STD_ON)
  if (errorId != BSWM_E_NO_ERROR)
  {
    (void)Det_ReportError(BSWM_MODULE_ID, BSWM_INSTANCE_ID_DET, BSWM_DOIP_SET_CHANNEL_READY_ID, errorId);
  }
# else
  BSWM_DUMMY_STATEMENT(errorId); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
# endif
...
FUNC(void, BSWM_CODE) BswM_DoIPInt_SetChannelReady(NetworkHandleType Channel)
{
  /* ----- Local Variables ---------------------------------------------- */
  uint8 errorId = BSWM_E_NO_ERROR;
# if (BSWM_IMMEDIATEUSERUSEDOFDOIPINTSETCHANNELREADYMAPPING == STD_ON) && ((BSWM_POSTBUILD_VARIANT_SUPPORT == STD_ON && BSWM_USE_INIT_POINTER == STD_ON) || BSWM_MULTIPARTITION == STD_ON)
  BswM_DoIPIntSetChannelReadyMappingIterType userHandle;
# endif
  BswM_PCPartitionConfigIdxOfPartitionIdentifiersType partitionIdx = BswM_GetInternalPartitionIdx(GetApplicationID());
...
# if(BSWM_IMMEDIATEUSERUSEDOFDOIPINTSETCHANNELREADYMAPPING == STD_ON)
#  if((BSWM_POSTBUILD_VARIANT_SUPPORT == STD_ON && BSWM_USE_INIT_POINTER == STD_ON) || (BSWM_MULTIPARTITION == STD_ON))
    for (userHandle = 0u; userHandle < BswM_GetSizeOfDoIPIntSetChannelReadyMapping(partitionIdx); userHandle++) /* Size can only be 0 or 1. */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
#  endif
    {
#  if (BSWM_DEV_ERROR_DETECT == STD_ON) /* COV_BSWM_DEV_ERROR_DETECT TX */
#   if (BSWM_IMMEDIATEUSERUSEDOFDOIPSETCHANNELREADYMAPPING == STD_ON)
      if ((BswM_GetDoIPSetChannelReadyState(partitionIdx) == TRUE) || (BswM_GetDoIPIntSetChannelReadyState(partitionIdx) == TRUE)) /* COV_BSWM_CONCURRENT_REQUEST */
#   else
      if (BswM_GetDoIPIntSetChannelReadyState(partitionIdx) == TRUE) /* COV_BSWM_CONCURRENT_REQUEST */
#   endif
      {
        errorId = BSWM_E_ALREADY_QUEUED;
      }
...
#  endif
      {
        BswM_SetDoIPIntSetChannelReadyState(TRUE, partitionIdx); /* SBSW_BSWM_PARTITIONINDEXACCESS */
        BswM_SetDcmSetChannelReadyChannel(Channel, partitionIdx); /* SBSW_BSWM_PARTITIONINDEXACCESS */
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfDoIPIntSetChannelReadyMapping(0u, partitionIdx), BswM_GetImmediateUserEndIdxOfDoIPIntSetChannelReadyMapping(0u, partitionIdx), BSWM_DOIPINT_SET_CHANNEL_READY_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
        BswM_SetDoIPIntSetChannelReadyState(FALSE, partitionIdx); /* SBSW_BSWM_PARTITIONINDEXACCESS */
      }
    }
# endif
  }
...
# if (BSWM_DEV_ERROR_REPORT == STD_ON)
  if (errorId != BSWM_E_NO_ERROR)
  {
    (void)Det_ReportError(BSWM_MODULE_ID, BSWM_INSTANCE_ID_DET, BSWM_DOIPINT_SET_CHANNEL_READY_ID, errorId);
  }
# else
  BSWM_DUMMY_STATEMENT(errorId); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
# endif
...
# endif
  ...
  {
# if (BSWM_DCMCOMSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfDcmComState(partitionIdx); userHandle++) /* COV_BSWM_ALLNETWORKPORTS */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFDCMCOMMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfDcmComMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfDcmComMapping(userHandle, partitionIdx), BSWM_DCM_COMMUNICATION_STATE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
# endif
  ...
  {
# if (BSWM_CANSMCHANNELSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfCanSMChannelState(partitionIdx); userHandle++) /* COV_BSWM_ALLNETWORKPORTS */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFCANSMCHANNELMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfCanSMChannelMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfCanSMChannelMapping(userHandle, partitionIdx), BSWM_CANSM_CURRENTSTATE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
# endif
  ...
  {
# if (BSWM_ETHIFPORTSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfEthIfPortState(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFETHIFPORTMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfEthIfPortMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfEthIfPortMapping(userHandle, partitionIdx), BSWM_ETHIF_PORTGROUPLINKSTATECHANGE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
# endif
  ...
  {
# if (BSWM_ETHSMSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfEthSMState(partitionIdx); userHandle++) /* COV_BSWM_ALLNETWORKPORTS */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFETHSMMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfEthSMMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfEthSMMapping(userHandle, partitionIdx), BSWM_ETHSM_CURRENTMODE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
# endif
  ...
  {
# if (BSWM_FRSMSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfFrSMState(partitionIdx); userHandle++) /* COV_BSWM_ALLNETWORKPORTS */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFFRSMMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfFrSMMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfFrSMMapping(userHandle, partitionIdx), BSWM_FRSM_CURRENTSTATE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
#   endif
    {
      BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfJ1939DcmMapping(0u, partitionIdx), BswM_GetImmediateUserEndIdxOfJ1939DcmMapping(0u, partitionIdx), BSWM_J1939DCM_BROADCASTSTATUS_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
    }
#  endif
# else
...
# endif
  ...
  {
# if (BSWM_J1939NMSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfJ1939NmState(partitionIdx); userHandle++) /* COV_BSWM_ALLNETWORKPORTS */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFJ1939NMMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfJ1939NmMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfJ1939NmMapping(userHandle, partitionIdx), BSWM_J1939NM_STATE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
# endif
  ...
  {
# if (BSWM_LINSCHEDULESTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfLinScheduleState(partitionIdx); userHandle++) /* COV_BSWM_ALLNETWORKPORTS */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFLINSCHEDULEMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfLinScheduleMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfLinScheduleMapping(userHandle, partitionIdx), BSWM_LINSM_CURRENTSCHEDULE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
# endif
  ...
  {
# if (BSWM_LINSMSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfLinSMState(partitionIdx); userHandle++) /* COV_BSWM_ALLNETWORKPORTS */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFLINSMMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfLinSMMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfLinSMMapping(userHandle, partitionIdx), BSWM_LINSM_CURRENTSTATE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
 ...
 */
/* PRQA S 6080 1 */ /* MD_MSR_STMIF */
FUNC(void, BSWM_CODE) BswM_LinSM_ScheduleEndNotification(NetworkHandleType Network, LinIf_SchHandleType Schedule) /* PRQA S 3334 */ /* MD_BSWM_3334 */
{
  /* ----- Local Variables ---------------------------------------------- */
  uint8 errorId = BSWM_E_NO_ERROR;
# if (BSWM_LINSCHEDULEENDSTATE == STD_ON)
...
# endif
  ...
  {
# if (BSWM_LINSCHEDULEENDSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfLinScheduleEndState(partitionIdx); userHandle++) /* COV_BSWM_ALLNETWORKPORTS */ /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFLINSCHEDULEENDMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfLinScheduleEndMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfLinScheduleEndMapping(userHandle, partitionIdx), BSWM_LINSM_SCHEDULEENDNOTIFICATION_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
# endif
  ...
  {
# if (BSWM_LINTPSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfLinTPState(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFLINTPMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfLinTPMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfLinTPMapping(userHandle, partitionIdx), BSWM_LINTP_REQUESTMODE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
#   endif
    {
      BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfEcuMModeMapping(0u, partitionIdx), BswM_GetImmediateUserEndIdxOfEcuMModeMapping(0u, partitionIdx), BSWM_ECUM_CURRENTSTATE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
    }
#  endif
# else
    BSWM_DUMMY_STATEMENT_MODE_CHECK_OFF(CurrentState); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
...
# endif
  ...
  {
# if(BSWM_ECUMWAKEUPSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfEcuMWakeupState(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if(BSWM_IMMEDIATEUSERUSEDOFECUMWAKEUPMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfEcuMWakeupMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfEcuMWakeupMapping(userHandle, partitionIdx), BSWM_ECUM_CURRENTWAKEUP_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
      }
    }
# else
...
# endif
  ...
  {
# if (BSWM_ECUMRUNREQUESTSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfEcuMRunRequestState(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFECUMRUNREQUESTMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfEcuMRunRequestMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfEcuMRunRequestMapping(userHandle, partitionIdx), BSWM_ECUM_REQUESTEDSTATE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
# endif
  ...
  {
# if (BSWM_NMSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfNmState(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFNMMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfNmMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfNmMapping(userHandle, partitionIdx), BSWM_NM_STATE_CHANGE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
# endif
  ...
  {
# if (BSWM_NVMBLOCKSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfNvMBlockState(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFNVMBLOCKMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfNvMBlockMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfNvMBlockMapping(userHandle, partitionIdx), BSWM_NVM_CURRENTBLOCKMODE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
#  endif
  ...
  else if (!((ServiceId == NVM_WRITE_ALL) || (ServiceId == NVM_READ_ALL)))
  {
    errorId = BSWM_E_REQ_USER_OUT_OF_RANGE;
  }
#  if (BSWM_MODE_CHECK == STD_ON)
...
# endif
  ...
  {
# if (BSWM_NVMJOBSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfNvMJobState(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFNVMJOBMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfNvMJobMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfNvMJobMapping(userHandle, partitionIdx), BSWM_NVM_CURRENTJOBMODE_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
# if (BSWM_IMMEDIATEUSERUSEDOFPDURPRETRANSMITMAPPING == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfPduRPreTransmitMapping(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_PDURPRETRANSMITSTATEIDXOFPDURPRETRANSMITMAPPING == STD_ON)
        ...
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfPduRPreTransmitMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfPduRPreTransmitMapping(userHandle, partitionIdx), BSWM_PDUR_PRETRANSMIT_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
...
# if (BSWM_IMMEDIATEUSERUSEDOFPDURRXINDICATIONMAPPING == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfPduRRxIndicationMapping(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_PDURRXINDICATIONSTATEIDXOFPDURRXINDICATIONMAPPING == STD_ON)
        ...
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfPduRRxIndicationMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfPduRRxIndicationMapping(userHandle, partitionIdx), BSWM_PDUR_RXINDICATION_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
...
# if (BSWM_IMMEDIATEUSERUSEDOFPDURTPRXINDICATIONMAPPING == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfPduRTpRxIndicationMapping(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_PDURTPRXINDICATIONSTATEIDXOFPDURTPRXINDICATIONMAPPING == STD_ON)
        ...
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfPduRTpRxIndicationMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfPduRTpRxIndicationMapping(userHandle, partitionIdx), BSWM_PDUR_TPRXINDICATION_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
...
# if (BSWM_IMMEDIATEUSERUSEDOFPDURTPSTARTOFRECEPTIONMAPPING == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfPduRTpStartOfReceptionMapping(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_PDURTPSTARTOFRECEPTIONSTATEIDXOFPDURTPSTARTOFRECEPTIONMAPPING == STD_ON)
        ...
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfPduRTpStartOfReceptionMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfPduRTpStartOfReceptionMapping(userHandle, partitionIdx), BSWM_PDUR_TPSTARTOFRECEPTION_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
...
# if (BSWM_IMMEDIATEUSERUSEDOFPDURTPTXCONFIRMATIONMAPPING == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfPduRTpTxConfirmationMapping(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_PDURTPTXCONFIRMATIONSTATEIDXOFPDURTPTXCONFIRMATIONMAPPING == STD_ON)
        ...
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfPduRTpTxConfirmationMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfPduRTpTxConfirmationMapping(userHandle, partitionIdx), BSWM_PDUR_TPTXCONFIRMATION_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
...
# if (BSWM_IMMEDIATEUSERUSEDOFPDURTRANSMITMAPPING == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfPduRTransmitMapping(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_PDURTRANSMITSTATEIDXOFPDURTRANSMITMAPPING == STD_ON)
        ...
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfPduRTransmitMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfPduRTransmitMapping(userHandle, partitionIdx), BSWM_PDUR_TRANSMIT_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
...
# if (BSWM_IMMEDIATEUSERUSEDOFPDURTXCONFIRMATIONMAPPING == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfPduRTxConfirmationMapping(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_PDURTXCONFIRMATIONSTATEIDXOFPDURTXCONFIRMATIONMAPPING == STD_ON)
        ...
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfPduRTxConfirmationMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfPduRTxConfirmationMapping(userHandle, partitionIdx), BSWM_PDUR_TXCONFIRMATION_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
...
# endif
  ...
  {
# if (BSWM_SDCLIENTSERVICESTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfSdClientServiceState(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFSDCLIENTSERVICEMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfSdClientServiceMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfSdClientServiceMapping(userHandle, partitionIdx), BSWM_SD_CLIENTSERVICE_CURRENT_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
# endif
  ...
  {
# if (BSWM_SDCONSUMEDEVENTSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfSdConsumedEventState(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFSDCONSUMEDEVENTMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfSdConsumedEventMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfSdConsumedEventMapping(userHandle, partitionIdx), BSWM_SD_CONSUMEDEVENTGROUP_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
# endif
  ...
  {
# if (BSWM_SDEVENTHANDLERSTATE == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfSdEventHandlerState(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
...
#  if (BSWM_IMMEDIATEUSERUSEDOFSDEVENTHANDLERMAPPING == STD_ON)
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfSdEventHandlerMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfSdEventHandlerMapping(userHandle, partitionIdx), BSWM_SD_EVENTHANDLER_CURRENT_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
#  endif
        break;
      }
    }
...
# if (BSWM_IMMEDIATEUSERUSEDOFWDGMMAPPING == STD_ON)
    for (userHandle = 0u; userHandle < BswM_GetSizeOfWdgMState(partitionIdx); userHandle++) /* PRQA S 2877 */ /* MD_BSWM_2877 */
    {
      if (BswM_GetExternalIdOfWdgMMapping(userHandle, partitionIdx) == Application)
      {
        ...
        BSWM_CALL_IMMEDIATE(BswM_GetImmediateUserStartIdxOfWdgMMapping(userHandle, partitionIdx), BswM_GetImmediateUserEndIdxOfWdgMMapping(userHandle, partitionIdx), BSWM_WDGM_REQUESTPARTITIONRESET_ID, partitionIdx); /* FETA_BSWM_CSL_USE_CASE */
...
\DESCRIPTION       Iteration from a defined lower limit to a defined upper limit. The upper limit is provided as a constant by the CSL.
                   The qualified CSL APIs return the positive constant values from CSL tables. The positive values in the table depends
                   on the configuration.
                   The stop index is a defined upper border of the loop. The CSL guarantees that the type of the loop variable is equal
                   to the type of the upper limit.
...
\ID                FETA_BSWM_IMMEDIATE_MODE_REQUEST_USERS
\DESCRIPTION       Iteration over immediate mode request users.
\COUNTERMEASURE \N Caller ensures correctness of start and end parameters.

\ID                FETA_BSWM_MIN_HEAP_INSERT
\DESCRIPTION       The loop is used to put the new element (child) to the correct position in the queue. The queue is using the Min-Heap
                   concept to sort its elements by priority. The new element is put into the queue at the end (highest index), then it is
                   compared to its parent element (which always has a lower index in the queue). As long as the (current) parent has lower
                   priority compared to the child, both are swapped, i.e. child is assigned to lower queue index which eventually leads to
                   loop termination (first loop condition). If the parent has higher or same priority as the child, the loop is terminated
                   immediately (second loop condition).
\COUNTERMEASURE \D see CDD #20

\ID                FETA_BSWM_QUEUELENGTH
\DESCRIPTION       [tempPriorityQueueIdx] iterates over all elements of ActionListPriorityQueue (from 0 to BswM_GetLengthOfActionListPriorityQueue).
                   The length of the priority queue is initialized with 0 in BswM_Init() and incremented only in function
                   BswM_PushIntoActionListQueue(). In BswM_PushIntoActionListQueue() a DET error is thrown if the queue is already full, i.e. if
                   LengthOfActionListPriorityQueue exceeds BswM_GetSizeOfActionListPriorityQueue which is constant.
\COUNTERMEASURE \R [tempPriorityQueueIdx] boundaries are guaranteed by DetCheck.

\ID                FETA_BSWM_EXECUTE_ACTION_LISTS
\DESCRIPTION       The loop iterates over the ActionListPriorityQueue to find all ActionLists that need to be executed for current executionLevel.
                   The CriticalArea_0 and BSWM_ACTIONLIST_BLOCKED ensure that the queue is not modified in between.
\COUNTERMEASURE \D see CDD #32 and CDD #34

\ID                FETA_BSWM_WAIT_FOR_BARRIER_FREE
\DESCRIPTION       The loop iterates until the barrier of current partition is free. Code is needed in our testsuite to be able to block one partition
                   in order to test the snychronization.
\COUNTERMEASURE \N Code is only needed for testing purposes. This code is not generated into productive code.

\ID                FETA_BSWM_CORESYNC
\DESCRIPTION       The loop iterates until all cores are synchronized. This code is executed during initialization phase only.
\COUNTERMEASURE \N Availability requirement is not applicable during initialization phase.
...
\ID COV_BSWM_FINITE_EXECUTION_TIME
  \ACCEPT TX
  \REASON The upper limit makes sure to not end in a endless loop. Upper limit can only be reached in error case. E.g. corrupt data.
...
\ID COV_BSWM_SYNCHRONIZATION
  \ACCEPT XF
  \REASON Depending on the configuration, the processing of partition initialization takes varying amount of time. Therefore this line is not fully covered in all configurations.
          This code is only added for testing purposes. It is not generated into productive code.

\ID COV_BSWM_INITIALIZED
  \ACCEPT XF
  \REASON Due to the configurations, this cannot be covered because DET is produced by action list callout.

\ID COV_BSWM_CONCURRENT_REQUEST
  \ACCEPT XF
  \ACCEPT XF xf xf
  \ACCEPT TF tf xf
  \REASON This cannot be covered in all configurations because DET is not produceable in all configurations.
...
MD_BSWM_2877:
      Reason:     The number of loop iterations is configuration dependent.
      Risk:       No functional risk.
      Prevention: Covered by code review.

 MD_BSWM_2889
      Reason:     According to the configuration, ActionLists have the option to abort/return after a failed call to a single action.
                  It's much easier and more readable to have multiple return paths instead of increasing the path count with additional checks.
      Risk:       Due to multiple return paths the control flow might not be as expected
      Prevention: Covered by code review.
 ...
 MD_BSWM_3334:
      Reason:     The function parameter 'Schedule' has the same name as the public OS API 'Schedule'.
      Risk:       No functional risk.
      Prevention: Covered by code review.
