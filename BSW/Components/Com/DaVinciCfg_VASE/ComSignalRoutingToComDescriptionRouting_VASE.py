...
#                                              Support for AR4.3  
#   1.03.01  2020-04-06 visrai                 fix 1:Find the gateway package 
#                                              fix 2:method which analyses the byteOrder BigEndian in layout of an ISignalIPdu
#                                              improvement: report in the logfile if both groupSignal and SignalGroup are being routed through ISignalMappings 
#                                              Support for several targets for the same source 
#######################################################################################################################
...
#----------------------------------------------------------------------------------------------------------------
#-----------------------------------FindAutosar------------------------------------------------------------------
def AnalyseDescriptionRoutings(logFile, GatewayCollections):
    ...
    try:
        ...
        for gateway in GatewayCollections:                                                           
            for ISignalMapping in gateway.SignalMappingList:
        ...
        Log(e.message)
        stringToPrint = "An error occurred" + format(e)
        logFile.write("{0}\n".format(stringToPrint))     
...
def Start_Find_elements_Source_and_TargetSignal(ISignalMapping, ISignalIPduRxCollectorArray, RxTxPropertiesCollector, logFile):
    ...
    try:
                   ...
                   for ISignalRx in ARISignalPackageListRx:            
                    ...
                    for findPduTriggering in ISignalTriggeringRx.ReferencedFromList:
                        if type(findPduTriggering.Owner.Owner) is VPduTriggering:
                            ...
                            if IPduTriggeringrefRx.IPduSpecified == True:
                   ...
                   for ISignalTx in ARISignalPackageListTx:
                    ...
                    for findPduTriggering in ISignalTriggeringTx.ReferencedFromList:
                        if type(findPduTriggering.Owner.Owner) is VPduTriggering:
                            ...
                            if IPduTriggeringrefTx.IPduSpecified == True:
        ...
        if  ISignalIPduTx is not None and ISignalIPduRx is not None:
            ...
            saveISignalIPduRxCollection(ISignalIPduRx , ISignalIPduRxCollectorArray, RxTxPropertiesCollector[RxTxPropertiesCollectorIndex], logFile)
        ...
        Log(e.message)
        logFile.write("\n an error has occurred while analysing ISignalMappings: " + str(e.message))
...
def AnalyseSignalGroupMappings(ISignalMapping, ISignalIPduRxCollectorArray, RxTxPropertiesCollector, logFile):
    ...
    try:
        ...
        Log(e.message)
        logFile.write("\n an error has occurred while analysing ISignalMappings of ISignalGroups (AnalyseSignalGroupMappings): " + str(e.message))        
...
def Create_ComDescription_Routings(ISignalIPduRxCollectorArray, GatewayCollections,logFile):
    ...
    try:
        ...
        counterPduPairsAnalysed = 0
        for SortISignalIPduTxRx in ISignalIPduRxCollectorArray:    
            
            #find out how many targets (different Pdus as targets SortISignalIPduTxRx[1])
            ##############################################################################
            PduDifferentTargets = []
            counterElements = 0
            for elements in SortISignalIPduTxRx:
                counterElements +=1
                if counterElements >1:            
                    found = False
                    for pdus in PduDifferentTargets:
                        if pdus == elements[1].AsrPath:
                            found = True
                            break

                    if found == False:
                       PduDifferentTargets.append(elements[1].AsrPath)

            #report relations one source many targets
            if PduDifferentTargets.Count >0:
                logFile.write("\n ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------") 
                logFile.write("\n SignalRoutings(SignalMappings) between ISignals of the following ISignalIPdu: " +  SortISignalIPduTxRx[0].AsrPath + " and " +  str(PduDifferentTargets.Count) + " other Pdus were found")                
                for target in PduDifferentTargets:
                    logFile.write("\n : ISignalIPdu: " + target)  
                logFile.write("\n ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------") 

            ISignalIPduRxCollectorOrderedByTarget = []                     
            indexForList = 0
            for foundPduTarget in PduDifferentTargets:
                iterationIndexSortISignalIPduTxRx = 0
                for elements in SortISignalIPduTxRx:
                    if iterationIndexSortISignalIPduTxRx == 0:
                       ISignalIPduRxCollectorOrderedByTarget.append([])
                       ISignalIPduRxCollectorOrderedByTarget[indexForList].append(elements)                       
                    else:
                        if elements[1].AsrPath == foundPduTarget:
                           ISignalIPduRxCollectorOrderedByTarget[indexForList].append(elements)
                    iterationIndexSortISignalIPduTxRx +=1
                indexForList +=1
            ##############################################################################
            
            for sortedISignalIPduByTarget in  ISignalIPduRxCollectorOrderedByTarget:
                counterPduPairsAnalysed +=1
                logFile.write("\n Analysis of ISignalIPdu Pair " +  SortISignalIPduTxRx[0].AsrPath + " and " + sortedISignalIPduByTarget[1][1].AsrPath + "  " +  str(counterPduPairsAnalysed))        
                CounterRxElements = sortedISignalIPduByTarget.Count

                # ##order TxRx information
                if (format(sortedISignalIPduByTarget[1][10])) == "Intel":
                    for bsort in range(len(sortedISignalIPduByTarget) - 1, 0, -1):
                        for i in range(bsort):
                            if  i > 0 :
                               if sortedISignalIPduByTarget[i][17] > sortedISignalIPduByTarget[i + 1][17]:
                                  temp = sortedISignalIPduByTarget[i]
                                  sortedISignalIPduByTarget[i] = sortedISignalIPduByTarget[i + 1]
                                  sortedISignalIPduByTarget[i + 1] = temp
                ...
                for j in range(CounterRxElements):
                    ...
                    if j > 0:
                        ...
                        Vbitlength = int(sortedISignalIPduByTarget[j][5])
                        Vtype = format(sortedISignalIPduByTarget[j][10])
                        VstartPosition = int(sortedISignalIPduByTarget[j][17])

                        arraybitpositions.append([])
                        arraybitpositions[j - 1].append(sortedISignalIPduByTarget[j][8])


                        if (format(sortedISignalIPduByTarget[j][10])) == "Intel":
                            IntelCounter += 1
                        ...
                        elif (format(sortedISignalIPduByTarget[j][10])) == "Motorola":
                             MotorolaCounter += 1
        ...
        #*******************************************************************************************************************************************
                ...
                create_AdminData(IntelCounter, MotorolaCounter, arraybitpositions, sortedISignalIPduByTarget, GatewayCollections, frame, logFile )

    except Exception, e:
        Log(e.message)
        logFile.write("\n an error has occurred while creating the ComDescriptionRoutings: " + str(e.message))        


def orderBigEndian(positionCollector, numberToFind, start, incrementor, maxValueToAnalyse, SortISignalIPduTxRx, logFile, counter):
    try:
      while numberToFind < maxValueToAnalyse:
        found = False
        currentValue = start 
        ...
        if found == False: 
           ...
           if mod0 > 0:                 
              ...
              numberToFind = numberToFind + 15 
                 
           counter +=1
           #orderBigEndian(positionCollector,numberToFind, start, incrementor, maxValueToAnalyse, SortISignalIPduTxRx, logFile,counter) 
        #else: return
    except Exception, e:
      Log(e.message)   
      logFile.write("\n an error has occurred while analysing the bit order of type bigEndian of the ISignalIpdu " + positionCollector[i][0] + " , exception:  " + str(e.message))                 
      return 
...
def create_AdminData(IntelCounter, MotorolaCounter, arraybitpositions, SortISignalIPduTxRx, GatewayCollections, frame, logFile):
    ...
    try:
        ...
        if MotorolaCounter >0 and SortISignalIPduTxRx.Count > 2:
           ...
           #reorder
           counter = 0
           orderBigEndian(positionCollector, 7, 0, 0, maxValueToAnalyse, SortISignalIPduTxRx, logFile, counter)
        ...
        if CountElements == 1:  # #Just one Signal "Not possible to search for "Gaps"
            ...
            CreateOneGatewayDescription(SortISignalIPduTxRx[1], GatewayCollections,logFile)
                ...
                # order signals reversed? not important anymore
                    ...
                    # Descriptions (source/destination)
                        ...
                        if (sameSourcePdu == True) and (sameDestinationPdu == True) and (sameTransferProperty == True) and (noGaps == True) and (sameLayout == True):
                            ...
                            logFile.write("\n ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------") 
                            logFile.write("\n merge NOT Possible, affected SignalMappings:")
                            logFile.write("\n SignalMapping 1 source: " +  SortISignalIPduTxRx[i][8].AsrPath + "target: " +  SortISignalIPduTxRx[i][9].AsrPath)
                            logFile.write("\n SignalMapping 2 source: " +  SortISignalIPduTxRx[i+1][8].AsrPath + "target: " +  SortISignalIPduTxRx[i+1][9].AsrPath)
                    ...
                    #######################################################################
                            if ArrayCreateAdminData.Count > 1:
                               logFile.write("\n ") 
                               logFile.write("\n ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------") 
                               logFile.write("\n for the ISignalIPdu {0} and the ISignalIPdu {1} {2} signals will be merged".format(ArrayCreateAdminData[0][0].AsrPath,ArrayCreateAdminData[0][1].AsrPath,str(ArrayCreateAdminData.Count)))
            ...
            if ArrayNotMergeableData.Count > 0:
               for notmergable in ArrayNotMergeableData:
                   ...
                   CreateOneGatewayDescription(notmergable, GatewayCollections,logFile)
            ...
            if ArrayCreateAdminData.Count > 1:
               ...
               CreateMultipleSignalRoutingDescriptions(ArrayCreateAdminData, GatewayCollections)
                ...
                CreateOneGatewayDescription(ArrayCreateAdminData, GatewayCollections,logFile)        

    except Exception, e:
      Log(e.message)   
      logFile.write("\n an error has occurred while creating the AdminData of a new ComDescriptionRouting,  exception:  " + str(e.message))                 
...
def CreateOneGatewayDescription(SortISignalIPduTxRx, GatewayCollections,logFile):

    logFile.write("\n only one ComDescriptionRouting will be created for:")
    logFile.write("\n SignalMapping 1 source: " +  SortISignalIPduTxRx[8].AsrPath + "target: " +  SortISignalIPduTxRx[9].AsrPath)
    logFile.write("\n---------------------------------------------------------------------")
    logFile.write("\n")
    for Gateway in GatewayCollections:
        NewSDG = Gateway.AdminData.AddNewSdg()
...
def saveISignalIPduRxCollection(ISignalIPduRx , ISignalIPduRxCollectorArray, RxPropertiesCollectorRxPropertiesCollectorIndex, logFile):
    ...
    for PacketRx in ISignalIPduRxCollectorArray:
        ...
        if  PacketRx[0] == ISignalIPduRx:
            ...
            alreadyExists = False
            for alreadySavedElement in ISignalIPduRxCollectorArray[index]:
                if not type(alreadySavedElement) is VISignalIPdu:
                   if alreadySavedElement[4].AsrPath == RxPropertiesCollectorRxPropertiesCollectorIndex[4].AsrPath and alreadySavedElement[6].AsrPath == RxPropertiesCollectorRxPropertiesCollectorIndex[6].AsrPath:
                      logFile.write("\n it seems that the ISignal ---> " + str(RxPropertiesCollectorRxPropertiesCollectorIndex[4].AsrPath) + " and the ISignal --> " + str(RxPropertiesCollectorRxPropertiesCollectorIndex[6].AsrPath) + " are being routed more than once trough ISignalMappings(GatewayRoutings).This might occur if for example both SignalGroup and GroupSignal are mapped through ISignalMappings. Only one of them will be considered to create the ComDescriptionRoutings")                      
                      alreadyExists = True
                      break
...
def Execute():
    try: 
        ...
        allArObjectsInModel = root.AllObjects

        for findReferrable in allArObjectsInModel:

            if type(findReferrable) is VGateway:  
               gateway = findReferrable
               GatewayCollections.Add(gateway)                                           
              

        AnalyseDescriptionRoutings(logFile, GatewayCollections)
        
        if GatewayCollections.Count >0:
           collectedAdminData = 0
           for foundGateway in GatewayCollections:
               if foundGateway.AdminDataSpecified == True:
                   collectedAdminData = collectedAdminData + foundGateway.AdminData.SdgList.Count           
           logFile.write("\n ComDescriptionRoutings created ---> " + str(collectedAdminData))
           logFile.write("\n Script execution completed successfully")
           Log("Script execution completed successfully")
        else:
           logFile.write("\n no Gateway with ISignalMapping was found")
           Log("Script execution completed successfully, no changes were done in the ARXML File")
