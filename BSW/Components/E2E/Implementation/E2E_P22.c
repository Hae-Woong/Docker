...
FUNC(E2E_P22CheckStatusType, E2E_CODE) E2E_P22DoChecks( /* PRQA S 6080 */ /* MD_MSR_STMIF */
  CONST(uint8, AUTOMATIC) ReceivedCRC,
  CONST(uint8, AUTOMATIC) ComputedCRC,
  P2VAR(E2E_P22CheckStateType, AUTOMATIC, E2E_APPL_DATA) StatePtr,
  CONST(uint8, AUTOMATIC) ReceivedCounter,
...
)
{
  ...
  if(NewDataAvailable == TRUE)
  {
    if(ReceivedCRC == ComputedCRC)
    {
      ...
      {
        deltaCounter = (uint8) (ReceivedCounter + (E2E_P22_MAX_COUNTER_VALUE - stateCounter) + 1u);
      }
...
)
{
  ...
  if(returnValue == E2E_E_OK)
  {
    ...
    /* Write counter */
    DataPtr[counterOffset] = (uint8) ((DataPtr[counterOffset] & (uint8)0xF0u) | (StatePtr->Counter & ((uint8)0x0Fu))); /* SBSW_E2E_05 */
