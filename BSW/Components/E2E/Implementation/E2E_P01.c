...
#if !defined (E2EP01_LOCAL) /* COV_E2E_COMPATIBILITY */
# define E2EP01_LOCAL static
#endif

#if !defined (E2EP01_LOCAL_INLINE) /* COV_E2E_COMPATIBILITY */
# define E2EP01_LOCAL_INLINE LOCAL_INLINE
#endif
 ...
 * \synchronous TRUE
 */
E2EP01_LOCAL FUNC (Std_ReturnType, E2E_CODE) E2E_P01ProtectVerifyInputs( P2CONST (E2E_P01ConfigType, AUTOMATIC, E2E_APPL_CONST) ConfigPtr,
 ...
 * \synchronous TRUE
 */
E2EP01_LOCAL FUNC (Std_ReturnType, E2E_CODE) E2E_P01CheckVerifyInputs(P2CONST (E2E_P01ConfigType, AUTOMATIC, E2E_APPL_CONST) ConfigPtr,
 ...
 * \synchronous TRUE
 */
E2EP01_LOCAL FUNC (Std_ReturnType, E2E_CODE) E2E_P01CheckDataIDMode(P2CONST (E2E_P01ConfigType, AUTOMATIC, E2E_APPL_CONST) ConfigPtr);
 ...
 * \synchronous TRUE
 */
E2EP01_LOCAL FUNC (uint8, E2E_CODE) E2E_P01ComputeCRC(P2CONST (uint8, AUTOMATIC, E2E_APPL_DATA) DataPtr,
 ...
 * \synchronous TRUE
 */
E2EP01_LOCAL FUNC (void, E2E_CODE) E2E_CheckDeltaCounter(P2CONST (E2E_P01ConfigType, AUTOMATIC, E2E_APPL_CONST) ConfigPtr,
 ...
 * \synchronous    TRUE
 */
E2EP01_LOCAL FUNC (void, E2E_CODE) E2E_CheckSync(P2CONST (E2E_P01ConfigType, AUTOMATIC, E2E_APPL_CONST) ConfigPtr,
                                              P2VAR (E2E_P01CheckStateType, AUTOMATIC, E2E_APPL_DATA) StatePtr);
                                                                      ...
                                                                      P2CONST (uint8, AUTOMATIC, E2E_APPL_DATA ) DataPtr)
{
    ...
    if ( (ConfigPtr != NULL_PTR) && (StatePtr != NULL_PTR) && (DataPtr != NULL_PTR) )
    {
        ...
        else if (E2E_P01CheckDataIDMode(ConfigPtr) != E_OK) /* SBSW_E2E_03 */
        {
            ret = E2E_E_INPUTERR_WRONG;
        }
        else
                                                      ...
                                                      VAR (uint8, AUTOMATIC) ReceivedCounter)
{
    ...
    uint8 DeltaCounter = (uint8)(ReceivedCounter - StatePtr->LastValidCounter) & 0x0Fu; /* PRQA S 2897 */ /* MD_MSR_Dir1.1 */
    ...
    else
    {
        ...
        else
        {
            ...
            if (DeltaCounter <= StatePtr->MaxDeltaCounter)
            {
                StatePtr->MaxDeltaCounter = ConfigPtr->MaxDeltaCounterInit; /* SBSW_E2E_03 */
                StatePtr->LastValidCounter = ReceivedCounter; /* SBSW_E2E_03 */
                StatePtr->LostData = (uint8) (DeltaCounter - 1u); /* SBSW_E2E_03 */
                                                ...
                                                E2E_BytePtrType DataPtr)
{
    ...
    if (ret == E2E_E_OK)
    {
        ...
        {
            DataPtr[ComputedOffset] = (uint8) ((DataPtr[ComputedOffset] & 0xF0u) /* SBSW_E2E_03 */
                                    | StatePtr->Counter);
        }
        /* #22 Write value of Counter to high nibble in DataPtr, ComputedOffset is pointing to */
        else
        {
            DataPtr[ComputedOffset] = (uint8) ((DataPtr[ComputedOffset] & 0x0Fu) /* SBSW_E2E_03 */
                                    | (StatePtr->Counter << 4u));
        }
        ...
        if (ConfigPtr->DataIDMode == E2E_P01_DATAID_NIBBLE)
        {
            ...
            if ((ConfigPtr->DataIDNibbleOffset & 0x07u) == 0u)
            {
                DataPtr[ComputedOffset] =  (uint8) ( (DataPtr[ComputedOffset] & 0xF0u)   /* SBSW_E2E_03 */
        ...
        StatePtr->Counter =  (uint8) ((StatePtr->Counter + 1u) % (E2E_P01_MAX_COUNTER_VALUE + 1u)); /* SBSW_E2E_03 */
    }
    return ret;
}
 ...
 *
 */
FUNC (Std_ReturnType, E2E_CODE) E2E_P01Check /* PRQA S 6030, 6080 */ /* MD_MSR_STCYC, MD_MSR_STMIF */
     (P2CONST(E2E_P01ConfigType, AUTOMATIC, E2E_APPL_CONST) ConfigPtr,
      P2VAR(E2E_P01CheckStateType, AUTOMATIC, E2E_APPL_DATA) StatePtr,
      P2CONST (uint8, AUTOMATIC, E2E_APPL_DATA) DataPtr)
{
    ...
    if (ret == E2E_E_OK)
    {
        ...
        if (StatePtr->NewDataAvailable == TRUE)
        {
            ...
            /* #50 Check if ReceivedCounter is within range */
            if (ReceivedCounter <= E2E_P01_MAX_COUNTER_VALUE)
            {
                uint8 ReceivedDataIDNibble = 0u; /* PRQA S 2981 */ /* MD_EtoE_2.2 */

                /* #60 CRC is read from Data */
                ComputedOffset  = ConfigPtr->CRCOffset >> 3u;
                ReceivedCRC     = DataPtr[ComputedOffset];

                /* #70 Read low nibble of high byte of DataID from Data in case DATAID_NIBBLE is configured */
                if (ConfigPtr->DataIDMode == E2E_P01_DATAID_NIBBLE)
                {
                    ComputedOffset = ConfigPtr->DataIDNibbleOffset >> 3u;
                ...
                /* #90 Check if received nibble is identical to the one in DataID */
                if (  (ConfigPtr->DataIDMode == E2E_P01_DATAID_NIBBLE)
                                                            ...
                                                            VAR (boolean, AUTOMATIC) profileBehavior)
{
    ...
    if (CheckReturn == E2E_E_OK)
    {
        ...
        switch (Status)
        {
            ...
            case E2E_P01STATUS_NONEWDATA    : ret = E2E_P_NONEWDATA;     break;
            ...
            /* Intentional fall through - same behavior */
