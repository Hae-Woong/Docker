 ...
 *  VERSION CHECK
 *********************************************************************************************************************/
#if(   (E2E_SW_MAJOR_VERSION != (7u) ) \
    || (E2E_SW_MINOR_VERSION != (0u) ))
...
/* Justification for module-specific MISRA deviations:
 ...
 MD_EtoE_2.2:
      Reason:     Redundant initialization is kept because some compilers (and linters) are criticizing the follow up
                  code to be 'potentially uninitialized' even this is not true.
      Risk:       None.
      Prevention: Program flow has been verified by component test and review.

 MD_EtoE_17.8:
      Reason:     Target function requires the start address to the buffer that shall be calculated.
      Risk:       The used function parameter is an object with automatic storage duration. When the address would be
                  stored and accessed later it may not be valid anymore.
      Prevention: Program flow has been verified by component test and review.
...
/* FETA_JUSTIFICATION_BEGIN
  ...
  \ID FETA_E2E_SM_ADD_STATUS
  \DESCRIPTION Count OK and ERROR messages in the current window.
  \COUNTERMEASURE \S SMI-494148

  \ID FETA_E2E_SM_CLEAR_STATUS
  \DESCRIPTION Clear the profile status window.
  \COUNTERMEASURE \S SMI-494148

  \ID FETA_E2E_SM_CLEAR_REMAINING
  \DESCRIPTION Clear all elements from the profile state window which are new available in the next state.
  \COUNTERMEASURE \S SMI-494148

  \ID FETA_E2E_SM_CHECK_INIT
  \DESCRIPTION Initialize the profile status window.
  \COUNTERMEASURE \S SMI-494148
