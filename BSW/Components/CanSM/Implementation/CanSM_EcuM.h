...
# define CCL_ASR4SMCAN_ECUM_HEADER_VERSION                                        0x0600u   /* BCD coded version number */
 ...
 *  \details     The function sets the transceiver and controller in the same state like in case of a normal full communication request. The sequence may be continued in the mainfunction.
 *  \param[in]   CanSM_NetworkHandle     network handle
 *  \return      E_OK                    Request accepted, wake-up source will be started
 *  \return      E_NOT_OK                Execution not possible.
 *  \context     ANY
 *  \reentrant   TRUE for different handles
 *  \synchronous FALSE
 *  \config      -
 ...
 *  \details     The function sets the transceiver and controller in the same state like in case of a normal no communication request. The sequence may be continued in the mainfunction. It this case the function can be used to poll the current status.
 *  \param[in]   CanSM_NetworkHandle                network handle
 *  \param[in]   WakeupSource                       not used, obsolete
 *  \return      E_OK                               Request accepted wake-up source are stopped i.e. CanSM has reached CANSM_BSM_S_NOCOM.
 *  \return      CANSM_E_STOP_WAKEUPSOURCES_RUNNING Shutdown is active. Current state is CANSM_BSM_S_PRE_NOCOM (independent if caused by NoComRequest or this API). 
 *  \return      CANSM_E_REQUESTED_COMM_MODE        Action skipped due to internal communication request is FullCommunication or SilentCommunication (not NoCom). 
 *  \return      E_NOT_OK                           Execution not possible
 *  \context     ANY
 *  \reentrant   TRUE for different handles
 *  \synchronous FALSE
