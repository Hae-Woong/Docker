 ...
 * \ingroup      RX
 *********************************************************************************************************************/
FRIF_LOCAL_INLINE FUNC(Std_ReturnType, FRIF_CODE) FrIf_Receive(FRIF_VCLST_ONLY, /* PRQA S 0784 */ /* MD_FrIf_IdentifierAsMacro */
 ...
 * \ingroup      RX
**********************************************************************************************************************/
FRIF_LOCAL_INLINE FUNC(void, FRIF_CODE) FrIf_HandleRedundantFrame(FRIF_VCLST_ONLY, /* PRQA S 0784 */ /* MD_FrIf_IdentifierAsMacro */
                                            FRIF_P2CONSTCFG(FrIf_FrameDescriptorType) FrameDescriptorPtr,
 ...
 * \ingroup      RX
 *********************************************************************************************************************/
FRIF_LOCAL_INLINE FUNC(void, FRIF_CODE) FrIf_FillRedundantRxPduInfo(FRIF_VCLST_ONLY, /* PRQA S 0784 */ /* MD_FrIf_IdentifierAsMacro */
                                        ...
                                        boolean HandleIsReceived)
{
  ...
  boolean RedundantHandleIsReceived;
  if ((Fr_RedundantLPduStatus != FR_NOT_RECEIVED) && (redundantResult == E_OK))
  {
    RedundantHandleIsReceived = TRUE;
  }
  else
  {
    RedundantHandleIsReceived = FALSE;
  }

  /* #20 If any of the two frames was received: */
  if((HandleIsReceived == TRUE) || (RedundantHandleIsReceived == TRUE)) /* PRQA S 2995, 2996 */ /* MD_MSR_ConstantCondition */
  {
    ...
    for(; PduNumber >= (sint16_least) 0; PduNumber--)
    {
       ...
       */
      if (FrIf_RxVotingFunction((P2CONST(P2VAR(PduInfoType, AUTOMATIC, FRIF_VAR_NOINIT), AUTOMATIC, FRIF_APPL_DATA))&FrIf_P2RedundantRxPduInfo[FRIF_VCLST_IDX], FRIF_MAX_NUMBER_OF_REDUNDANT_PDUS, &selectedPduIndex) == E_OK) /* PRQA S 0310 */ /* MD_FrIf_VotingFct */ /* SBSW_FRIF_FCT_CALL_REDPDUINFO */
      {
...
#  if (FRIF_DIRECT_BUFFER_ACCESS_ENABLE == STD_ON)
    if (HandleIsReceived == TRUE) /* PRQA S 2995, 2996 */ /* MD_MSR_ConstantCondition */
    {
      (void)Fr_UnlockRxLPdu_DBA(FRIF_VCLST_IDX, Handle);
    }
    if (RedundantHandleIsReceived == TRUE) /* PRQA S 2995, 2996 */ /* MD_MSR_ConstantCondition */
    {
      (void)Fr_UnlockRxLPdu_DBA(FRIF_VCLST_IDX, RedundantHandle);
    }
#  else
    FRIF_DUMMY_STATEMENT(Handle); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
                                        ...
                                        boolean HandleIsReceived)
{
   ...
   */
  if((HandleIsReceived == TRUE) && (FrameLayoutElementPtr->UpdateBitMask != 0u)) /* PRQA S 2995, 2996 */ /* MD_MSR_ConstantCondition */
  {
      PduIsValid = (boolean)(receiveBuffer[UpdateBitBytePos] & FrameLayoutElementPtr->UpdateBitMask); /* PRQA S 4304, 4340, 4394 */ /* MD_MSR_AutosarBoolean */ 
  }
   ...
   */
  if((PduIsValid != FALSE) && (Fr_LPduLength > FrameLayoutElementPtr->ByteOffsetInFrame)) /* PRQA S 2995, 2996 */ /* MD_MSR_ConstantCondition */
  {
    if (     (PduNumber != 0)
                                          ...
                                          uint16 Handle)
{
  ...
  P2VAR(uint8, AUTOMATIC, FRIF_VAR_NOINIT) receiveBuffer = (uint8*)FRIF_GET_FRAMEBUFFER(FRIF_VCLST_IDX); /* PRQA S 0310 */ /* MD_FrIf_0310 */
  ...
  {
    boolean HandleIsReceived;
    if ((Fr_LPduStatus != FR_NOT_RECEIVED) && (Result == E_OK))
    {
      HandleIsReceived = TRUE;
    }
    else
    {
      HandleIsReceived = FALSE;
    }
  ...
  if(Result == E_OK)
  {
...

    {
      ...
      for(; PduNumber >= (sint16_least) 0; PduNumber--)
      {
         ...
         */
        boolean PduIsValid = TRUE; /* PRQA S 2995, 2996 */ /* MD_MSR_ConstantCondition */
        if(FrameLayoutElementPtr->UpdateBitMask != (uint8_least) 0)
        {
          PduIsValid = (boolean)(receiveBuffer[FrIf_GetCurrentUpdateBitBytePos()] & FrameLayoutElementPtr->UpdateBitMask); /* PRQA S 4304, 4340, 4394 */ /* MD_MSR_AutosarBoolean */ 
        }
         ...
         */
        if ((PduIsValid != FALSE) && (FrameLayoutElementPtr->ByteOffsetInFrame < Fr_LPduLength)) /* PRQA S 2995, 2996 */ /* MD_MSR_ConstantCondition */
        {
  ...
  return Result;
} /* FrIf_Receive() */ /* PRQA S 6030, 6080 */ /* MD_MSR_STCYC */ /* MD_MSR_STMIF */
