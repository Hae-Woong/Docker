...
struct Fee_tagBufferSizes { /* PRQA S 3213 */ /* MD_FEE_2.4_CompileChecks */
    Fee_BitFieldType ChunkAlloc : ((sizeof(Fee_Globals_t.FsmLocalVars_t.ChunkAllocContext_t.buffer_au32) >= FEE_CHUNK_HEADER_SIZE) ? 1 : -1); /* PRQA S 3493 */ /* MD_FEE_14.3_CompileChecks */
    Fee_BitFieldType ChunkSearch : ((sizeof(Fee_Globals_t.FsmLocalVars_t.chunkSearchContext_t.super_t.headerBuffer_au32) >= FEE_CHUNK_HEADER_SIZE) ? 1 : -1); /* PRQA S 3493 */ /* MD_FEE_14.3_CompileChecks */
    Fee_BitFieldType InstanceWrite : ((sizeof(Fee_Globals_t.FsmLocalVars_t.instanceWriteContext_t.BufferBytes) >= FEE_CHUNK_HEADER_SIZE) ? 1 : -1); /* PRQA S 3493 */ /* MD_FEE_14.3_CompileChecks */
    Fee_BitFieldType InstanceCopy : ((sizeof(Fee_Globals_t.FsmLocalVars_t.instanceCopyContext_t.BufferBytes) >= FEE_CHUNK_HEADER_SIZE) ? 1 : -1); /* PRQA S 3493 */ /* MD_FEE_14.3_CompileChecks */
    Fee_BitFieldType InstanceSearch : ((sizeof(Fee_Globals_t.FsmLocalVars_t.instanceSearchContext_t.Buffer_au32) >= FEE_CHUNK_HEADER_SIZE) ? 1 : -1); /* PRQA S 3493 */ /* MD_FEE_14.3_CompileChecks */
    Fee_BitFieldType PartitionInit : ((sizeof(Fee_Globals_t.FsmLocalVars_t.PartitionInitContext_t.sectorHeaderData) >= FEE_CHUNK_HEADER_SIZE) ? 1 : -1); /* PRQA S 3493 */ /* MD_FEE_14.3_CompileChecks */
    Fee_BitFieldType SectorFormat : ((sizeof(Fee_Globals_t.FsmLocalVars_t.SectorFormatContext_t.sectorHeader_au32) >= FEE_CHUNK_HEADER_SIZE) ? 1 : -1); /* PRQA S 3493 */ /* MD_FEE_14.3_CompileChecks */
};
 ...
 *  ACTIONS
 *******************************************************************/
enum Fee_StateActionEnumType /* PRQA S 3213 */ /* MD_FEE_2.4_Parameter_Used_For_Other_Configs */ /*<! Enumerate all possible actions; function to be directly called by FSM processing */
{
    ...
    FEE_ACT_ID_INIT_SETUP_SECTORS_FSM,                 /* PRQA S 3205 */ /* MD_FEE_2.4_Parameter_Used_For_Other_Configs */
    ...
    FEE_ACT_ID_PREPARE_BLOCK_COPY_INSTANCE_TRAILER_WRITE,
    ...
    FEE_ACT_ID_PREPARE_WRITE_STREAM_THIRD,
...
#if (FEE_LOOKUPTABLE_MODE == STD_ON)
    FEE_ACT_ID_LUT_CHECK_CHUNK_HEADER,
    FEE_ACT_ID_LUT_HANDLE_INVALID_HEADER,
    FEE_ACT_ID_LUT_READ_CHUNK_LINK,
    FEE_ACT_ID_LUT_VALIDATE_CHUNK_LINK,
    FEE_ACT_ID_LUT_HANDLE_INVALID_LINK,
    FEE_ACT_ID_LUT_FINALIZE_CHUNK_SEARCH,
    FEE_ACT_ID_LUT_RESET_LUT_ENTRY,
    FEE_ACT_ID_LUT_READ_LUT_BLOCK,
    FEE_ACT_ID_LUT_INITIALIZATION,
    FEE_ACT_ID_LUT_VALIDATE_BLOCK_CONTENT,
    FEE_ACT_ID_LUT_FINISH_BLOCK_SYNCHRONIZATION,
#endif
    /*The following "lut"-actions are always needed, because they are part of regular statemachines*/
    /*In case the feature is disabled, they do just nothing*/
    FEE_ACT_ID_FINISH_SUCCESSFUL_WRITING,
    FEE_ACT_ID_FINISH_UNSUCCESSFUL_WRITING,
    FEE_ACT_ID_FINISH_SUCCESSFUL_COPYING,
    FEE_ACT_ID_FINISH_UNSUCCESSFUL_COPYING,
    FEE_ACT_ID_UPDATE_LUT,
    FEE_ACT_ID_FINALIZE_UPWARDS_SEARCH_EMPTY_LINK,
    FEE_ACT_ID_EVALUATE_SECTOR_HEADER,
#if (FEE_USE_RELIABLE_ERASE_PROCEDURE == STD_ON)
    FEE_ACT_ID_READ_ERASE_START_MARKER,
    FEE_ACT_ID_EVALUATE_ERASE_START_MARKER,
#endif
    FEE_ACT_ID_FINALIZE_SECTOR_HEADER_EVALUATION,
    FEE_ACT_ID_READ_NEXT_SECTOR_HEADER,
    FEE_ACT_ID_SETUP_SECTORS_ERASE_SECTOR,                 /* PRQA S 3205 */ /* MD_FEE_2.4_Parameter_Used_For_Other_Configs */
 ...
 *  QUERIES
 *******************************************************************/
enum Fee_StateQueryEnumType /* PRQA S 3213 */ /* MD_FEE_2.4_Parameter_Used_For_Other_Configs */ /*<! Enumerate all possible queries; function to be directly called by FSM processing */
{
...
#if (FEE_LOOKUPTABLE_MODE == STD_ON)
    FEE_QRY_ID_LUT_IS_LUT_ENABLED,
    FEE_QRY_ID_LUT_MORE_BLOCKS_TO_READ,
#endif
#if (FEE_USE_RELIABLE_ERASE_PROCEDURE == STD_ON)
    FEE_QRY_ID_IS_SECTOR_HEADER_VALID,
#endif
    FEE_QRY_ID_IS_SECTOR_HEADER_LEFT_TO_READ,
...
#define FEE_INTERNAL_ERROR_SECTOROVERFLOW 0x1u
#define FEE_INTERNAL_ERROR_FORMAT_FAILED  0x2u
#define FEE_INTERNAL_ERROR_BLOCK_COPY_READ_FAILED  0x4u
#define FEE_INTERNAL_ERROR_BLOCK_COPY_WRITE_FAILED  0x8
#define FEE_INTERNAL_ERROR_BLOCK_CORRUPTED_CHUNKHEADER  0x10
#define FEE_INTERNAL_ERROR_BLOCK_INCONSISTENT_CHUNKHEADER  0x20
...
/*!
 *
 * \brief      Checks if given chunk's payload size equals the ceiled payload size
 * \details    Header size plus plus payload shall be write aligned
 * \param[in]  chunk
 * \param[in]  config_pt (not NULL)
 * \return     TRUE: Chunk's payload size is ceiled; FALSE otherwise
  * \pre       chunk not null
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_INLINE_FUNC(Fee_Bool) Fee_Proc_isPayloadSizeCeiled(Fee_ChunkInfoConstRefType chunk, Fee_BlockConfigRefType config_pt);
...
/**********************************************************************************************************************
 * Fee_Proc_CeilPayloadSize
 *********************************************************************************************************************/
/*!
 * \brief      Computes the ceiled payload size for a given blockSize in a given sector
 * \details    Computes the ceiled payload size for a given blockSize in a given sector
 * \param[in]  sector
 * \param[in]  blockSize
 * \return     ceiled payload size
 * \pre        sector not null
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(uint16) Fee_Proc_CeilPayloadSize(Fee_SectorConstRefType sector, uint16 blockSize);
 ...
 *********************************************************************************************************************/
/*!
 * \brief      Prepares chunk search context for continuing the upwards chunk search where it has stopped.
 * \details    Prepares chunk search context for continuing the upwards chunk search where it has stopped.
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
...
/**********************************************************************************************************************
 * Fee_ActPrepareBlockCopyInstanceTrailerWrite
 *********************************************************************************************************************/
/*!
 * \brief      Writes the instance trailer page as a last step of the block copy operation.
 * \details    Also writes the instance trailer debug info into it.
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActPrepareBlockCopyInstanceTrailerWrite(void);
...
/**********************************************************************************************************************
 * Fee_ActPrepareWriteStreamThird
 *********************************************************************************************************************/
/*!
 * \brief      Forwards to write third write stream
 * \details    Writes one page payload data optionally mixed with ceiling (in case ceiling is necessary
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActPrepareWriteStreamThird(void);

/**********************************************************************************************************************
 * Fee_ActPrepareWriteStreamSecond
 *********************************************************************************************************************/
/*!
 * \brief      Requests Flash to write middle part of the data
 * \details    Writes directly via user buffer. Only necessary if 4 write streams are needed
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
...
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
/**********************************************************************************************************************
 * Fee_ActLutCheckChunkHeader
 *********************************************************************************************************************/
/*!
 * \brief      Creates chunk info out of read in data (hopefully chunk header) and sets local job result according to success.
 * \details    Checks if chunk header is formally correct and if its block id fits so search block's block id
 * \config     FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre        Alleged chunk header was read into buffer
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActLutCheckChunkHeader(void);



/**********************************************************************************************************************
 * Fee_ActLutHandleInvalidHeader
 *********************************************************************************************************************/
/*!
 * \brief      Sets local job result to failed.
 * \details    Sets local job result to failed.
 * \config     FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActLutHandleInvalidHeader(void);


/**********************************************************************************************************************
 * Fee_ActLutReadChunkLink
 *********************************************************************************************************************/
/*!
 * \brief      Sets up a flash read request in order to read chunk link
 * \details    Issues to read chunk link that belongs to previously read chunk header
 * \config     FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActLutReadChunkLink(void);


/**********************************************************************************************************************
 * Fee_ActLutValidateChunkLink
 *********************************************************************************************************************/
/*!
 * \brief      Creates chunk info out of read in data (hopefully chunk link) and sets local job result according to success
 * \details    Issues to read chunk link that belongs to previously read chunk header
 * \config     FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre        Alleged chunk link was read into buffer
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActLutValidateChunkLink(void);


/**********************************************************************************************************************
 * Fee_ActLutHandleInvalidLink
 *********************************************************************************************************************/
/*!
 * \brief      Prepares chunk search context in order to continue with normal chunk search and sets local job result to
 *             failed.
 * \details    Copies latest chunk info into context (whose link was not empty). Chunk search will be continued there.
 * \config     FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActLutHandleInvalidLink(void);


/**********************************************************************************************************************
 * Fee_ActLutFinalizeLutChunkSearch
 *********************************************************************************************************************/
/*!
 * \brief      Copies latest found chunk from lut context to chunk search context and sets local job result to ok
 * \details    Copies latest found chunk from lut context to chunk search context and sets local job result to ok
 * \config     FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActLutFinalizeLutChunkSearch(void);

/**********************************************************************************************************************
 * Fee_ActLutResetLutEntry
 *********************************************************************************************************************/
/*!
 * \brief      Resets LUT entry for given lookup table context
 * \details    Resets LUT entry for given lookup table context
 * \config     FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActLutResetLutEntry(void);


/**********************************************************************************************************************
 * Fee_ActLutInitialization
 *********************************************************************************************************************/
/*!
 * \brief      Initializes lut sync context in order to start with reading the first lut block from nv
 * \details    Also starts with reading the first lut block
 * \config     FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActLutInitialization(void);


/**********************************************************************************************************************
 * Fee_ActLutReadLutBlock
 *********************************************************************************************************************/
/*!
 * \brief      Reads next lut block (according to LookUpTableSyncBlockIndex) from flash
 * \details    Starts consistent chunk search fsm with corresponding parameters
 * \config     FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActLutReadLutBlock(void);


/**********************************************************************************************************************
 * Fee_ActLutValidateBlockContent
 *********************************************************************************************************************/
/*!
 * \brief      Checks every entry within recently read in lut block whether it "points to the future"
 * \details    Checks whether lut block from old sector points to new sector. Such entries will be reset. Increments sync block index
 *             afterwards
 * \config     FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActLutValidateBlockContent(void);

/**********************************************************************************************************************
 * Fee_ActLutFinishBlockSynchronization
 *********************************************************************************************************************/
/*!
 * \brief         Finishes sync of latest block by either handling the deferred job or by resetting blocks modfied flag
 * \details       In case the job is deferred, sync block index is reset in order to start sync from scratch
 * \config        FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre           -
 * \context       TASK
 * \reentrant     FALSE
 * \synchronous   TRUE
 */
FEE_LOCAL_FUNC(void) Fee_ActLutFinishBlockSynchronization(void);

/**********************************************************************************************************************
 * Fee_Internal_LutCopyChunkSearchContext
 *********************************************************************************************************************/
/*!
 * \brief      Copies chunk search relevant data from given lut context to another chunk search context
 * \details    Copies chunk search relevant data from one given context to another given context
 * \param[out] target
 * \param[in]  self
 * \config     FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre        -
 ...
 * \synchronous TRUE
 */
FEE_LOCAL_FUNC(void) Fee_Internal_LutCopyChunkSearchContext(Fee_ChunkSearchContextRefType target,Fee_LookUpTableContextConstRefType self);



/**********************************************************************************************************************
 * Fee_Internal_LutInitChunkSearchContext
 *********************************************************************************************************************/
/*!
 * \brief         Initializes lut chunk search context
 * \details       Initializes lut chunk search context
 * \param[in,out] target
 * \param[in]     searchSector
 * \param[in]     absoluteChunkAddress
 * \config        FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre           Lut entry is plausible and belongs to search sector
 * \context       TASK
 * \reentrant     FALSE
 * \synchronous   TRUE
 */
FEE_LOCAL_FUNC(void) Fee_Internal_LutInitChunkSearchContext(Fee_LookUpTableContextRefType lutContext,
                                                             Fee_SectorRefType searchSector,Fee_LengthType absoluteChunkAddress);



/**********************************************************************************************************************
 * Fee_Internal_LutInitSyncChunkSearchContext
 *********************************************************************************************************************/
/*!
 * \brief         Initializes chunk search context in order to read/write lut block
 * \details       Initializes chunk search context in order to read/write lut block
 ...
 * \synchronous   TRUE
 */
FEE_LOCAL_FUNC(void) Fee_Internal_LutInitSyncChunkSearchContext(Fee_PartitionRefType jobPartition_pt, uint16 blockIndex, Fee_Bool isWriteJob);

/**********************************************************************************************************************
 * Fee_Internal_LutTryToStartLutSearch
 *********************************************************************************************************************/
/*!
 * \brief       Gets entry from lut, evaluates it and starts lut search if everything is plausible
 * \details     Returns whether lut search was actually started or not
 * \param[in]   searchSector: sector FEE is currently searching in
 * \config      FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre         -
 * \return      TRUE: Lut search was started
 * \return      FALSE: Lut search was not started
 * \context     TASK
 * \reentrant   FALSE
 * \synchronous TRUE
 */
FEE_LOCAL_FUNC(Fee_Bool) Fee_Internal_LutTryToStartLutSearch(Fee_SectorRefType searchSector);


/**********************************************************************************************************************
 * Fee_Internal_LutInitSynchronization
 *********************************************************************************************************************/
/*!
 * \brief       Checks if lut synchronization might be needed and prepares sync context for first block
 * \details     Checks if there is any partition that is initialized and has lut feature enabled.
 * \param[in]   globals
 * \config      FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre         -
 * \return      TRUE: synchronization might be necessary
 * \return      FALSE: synchronization is not necessary
 * \context     TASK
 * \reentrant   FALSE
 * \synchronous TRUE
 */
FEE_LOCAL_FUNC(Fee_Bool) Fee_Internal_LutInitSynchronization(Fee_GlobalsPtrType globals);



/**********************************************************************************************************************
 * Fee_Internal_LutInitSyncFsm
 *********************************************************************************************************************/
/*!
 * \brief       Starts the lut sync sub fsm if synchronization is necessary
 * \details     Synchronization continues in descending order (partition wise): higher partitions first, higher block indexes first.
 * \param[in]   globals
 * \config      FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \pre         -
 * \return      TRUE: lut sync sub fsm was started
 * \return      FALSE: lut sync sub fsm was not started
 * \context     TASK
 * \reentrant   FALSE
 * \synchronous TRUE
 */
FEE_LOCAL_FUNC(Fee_Bool) Fee_Internal_LutInitSyncFsm(Fee_GlobalsPtrType globals);

#endif

/**********************************************************************************************************************
 * Fee_ActFinishSuccessfulWriting
 *********************************************************************************************************************/
/*!
 * \brief      Sets local job result to ok and updates lut entry if feature is enabled
 * \details    Uses current dataset in order to update the entry
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActFinishSuccessfulWriting(void);


/**********************************************************************************************************************
 * Fee_ActFinishUnsuccessfulWriting
 *********************************************************************************************************************/
/*!
 * \brief      Sets local job result to failed and resets lut entry if feature is enabled
 * \details    Sets local job result to failed and resets lut entry
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActFinishUnsuccessfulWriting(void);


/**********************************************************************************************************************
 * Fee_ActFinishSuccessfulCopying
 *********************************************************************************************************************/
/*!
 * \brief      Sets local job result to ok and updates lut entry if feature is enabled
 * \details    Uses instance copy context in order to update the entry
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActFinishSuccessfulCopying(void);


/**********************************************************************************************************************
 * Fee_ActFinishUnsuccessfulCopying
 *********************************************************************************************************************/
/*!
 * \brief      Sets local job result to failed and resets lut entry if feature is enabled
 * \details    Sets local job result to failed and resets lut entry if feature is enabled
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActFinishUnsuccessfulCopying(void);

/**********************************************************************************************************************
 * Fee_ActUpdateLut
 *********************************************************************************************************************/
/*!
 * \brief      Updates lut entry if feature is enabled
 * \details    Uses current dataset in order to update the entry
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActUpdateLut(void);

/**********************************************************************************************************************
 * Fee_ActFinalizeUpwardsSearchEmptyLink
 *********************************************************************************************************************/
/*!
 * \brief     Set local job result to failed and save previous and most recent chunk info.
 * \details   Set local job result to failed and save previous and most recent chunk info.
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActFinalizeUpwardsSearchEmptyLink(void);

/**********************************************************************************************************************
 * Fee_ActEvaluateSectorHeader
 *********************************************************************************************************************/
/*!
 * \brief     Evaluates the recently read sector header and stores the validity information in the sector info object.
 * \details   Also extracts the sector ID and stores it in the context.
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActEvaluateSectorHeader(void);

#if (FEE_USE_RELIABLE_ERASE_PROCEDURE == STD_ON)
/**********************************************************************************************************************
 * Fee_ActReadEraseStartMarker
 *********************************************************************************************************************/
/*!
 * \brief     Requests to read the erase start marker of the currently processed sector.
 * \details   -
 * \config     FEE_USE_RELIABLE_ERASE_PROCEDURE is enabled
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActReadEraseStartMarker(void);

/**********************************************************************************************************************
 * Fee_ActEvaluateEraseStartMarker
 *********************************************************************************************************************/
/*!
 * \brief     Evaluate the erase start marker of the just read sector and stores the validity information in the sector info object.
 * \details   -
 * \config    FEE_USE_RELIABLE_ERASE_PROCEDURE is enabled
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActEvaluateEraseStartMarker(void);
#endif

/**********************************************************************************************************************
 * Fee_ActFinalizeSectorHeaderEvaluation
 *********************************************************************************************************************/
/*!
 * \brief     Takes both evaluated sectors and determines which one is the current sector.
 * \details   -
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActFinalizeSectorHeaderEvaluation(void);

/**********************************************************************************************************************
 * Fee_ActReadNextSectorHeader
 *********************************************************************************************************************/
/*!
 * \brief     Requests to read the next sector header.
 * \details   The next sector header is determined by the partition init context
 * \pre        There is another sector header left to read
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActReadNextSectorHeader(void);

/**********************************************************************************************************************
 * Fee_ActSetupSectorsEraseSector
 *********************************************************************************************************************/
/*!
 * \brief     Requests to erase the sector.
 * \details   -
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_ActSetupSectorsEraseSector(void);

/**********************************************************************************************************************
 * Fee_Internal_ValidateChunkLinkInContext
 *********************************************************************************************************************/
/*!
 * \brief      Validates ChunkLink in input search context
 * \details    Validates ChunkLink in input search context
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_Internal_ValidateChunkLinkInContext(const Fee_ChunkSearchContextRefType context_pt);
...
#if (FEE_LOOKUPTABLE_MODE == STD_ON)
/**********************************************************************************************************************
 * Fee_QryLutIsLutEnabled
 *********************************************************************************************************************/
/*!
 * \brief      Checks if lut feature is enabled for current partition
 * \details    Checks if lut feature is enabled for current partition
 * \config     FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \return     TRUE: Lut feature is enabled
 * \return     FALSE: Lut feature is not enabled
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryLutIsLutEnabled(void);


/**********************************************************************************************************************
 * Fee_QryLutMoreBlocksToRead
 *********************************************************************************************************************/
/*!
 * \brief      Checks if another lut block must be read
 * \details    Checks if another lut block must be read
 * \config     FEE_LOOKUPTABLE_MODE is enabled via pre-compile switch
 * \return     TRUE: At least one lut block must still be read
 * \return     FALSE: All lut blocks have already been read
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryLutMoreBlocksToRead(void);
#endif

#if (FEE_USE_RELIABLE_ERASE_PROCEDURE == STD_ON)
/**********************************************************************************************************************
 * Fee_QryIsSectorHeaderValid
 *********************************************************************************************************************/
/*!
 * \brief      Checks if the recently evaluated sector header was valid
 * \details    -
 * \return     TRUE: Sector header was valid
 * \return     FALSE: Sector header was not valid
 * \config     FEE_USE_RELIABLE_ERASE_PROCEDURE is enabled
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryIsSectorHeaderValid(void);
#endif

/**********************************************************************************************************************
 * Fee_QryIsSectorHeaderLeftToRead
 *********************************************************************************************************************/
/*!
 * \brief      Checks if another sector header is left to be read and evaluated
 * \details    -
 * \return     TRUE: Another sector header is left to be read
 * \return     FALSE: No sector header is left to be read
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryIsSectorHeaderLeftToRead(void);
...
/**********************************************************************************************************************
 * Fee_InternalFillInstanceBufferWithValue
 *********************************************************************************************************************/
/*!
 * \brief      Copies given byte in given number into given buffer
 * \details    CCopies given byte in given number into given buffer
 * \param[in,out]  instBuf
 * \param[in]  value
 * \param[in]  length
 * \pre        -
 ...
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_InternalFillInstanceBufferWithValue(const Fee_IntBuffer_u8PtrType instBuf,uint8 value, uint32 length);


/**********************************************************************************************************************
 * Fee_InternalFillInstanceBufferTrailerPage
 *********************************************************************************************************************/
/*!
 * \brief      Fills the instance trailer page with valid instance status and some log information
 * \details    Fills the instance trailer page with valid instance status and some log information
 * \param[in,out]  instBuf
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_InternalFillInstanceBufferTrailerPage(const Fee_IntBuffer_u8PtrType instBuf);
 ...
 *********************************************************************************************************************/
/*!
 * \brief      Gets instance's status out of instance's header/trailer within the given buffer
 * \details    Gets instance's status out of instance's header/trailer within the given buffer
 * \param[in]  sector_pt
 * \param[in]  intBuf_au8
 * \param[in]  flsResult
 ...
 *********************************************************************************************************************/
/*!
 * \brief      Initializes the partition and forwards to read the first sector's header
 * \details    Initializes the partition an forwards to read the first sector's header
 * \param[in,out]  g
 * \param[in,out]  part
 ...
 * \param[in]  apiRetVal: Value returned by Fee flash operation
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
 ...
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_Proc_SetupWriteSuspendHandling(Fee_GlobalsPtrType globals, Fee_DeferredFuncType deferredRequest);
 ...
 *********************************************************************************************************************/
/*!
 * \brief      Starts write processing.
 * \details    Forwards to write first write stream if at least 3 write streams are necessary
 * \param[in,out]  g
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_InternalDoWriteStreamFirst(Fee_GlobalsPtrType g);
...
/**********************************************************************************************************************
 * Fee_InternalDoWriteStreamThird
 *********************************************************************************************************************/
/*!
 * \brief      Forwards to write third write stream to Flash
 * \details    Forwards to write third write stream to Flash
 * \param[in,out]  g
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_InternalDoWriteStreamThird(Fee_GlobalsPtrType g);
...
/**********************************************************************************************************************
 * Fee_InternalDoBlockCopyInstanceTrailerWrite
 *********************************************************************************************************************/
/*!
 * \brief      Forwards to write the instance trailer to flash
 * \details    Fills internal buffer with trailer byte and debug information and writes it.
 * \param[in,out]  g
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_InternalDoBlockCopyInstanceTrailerWrite(Fee_GlobalsPtrType g);
...
#if (FEE_USE_RELIABLE_ERASE_PROCEDURE == STD_ON)
/**********************************************************************************************************************
 * Fee_InternalDoEraseStartMarkerWrite
 *********************************************************************************************************************/
/*!
 *
 * \brief      Setup a erase start marker write request
 * \details    Forward to Fee_Sector and evaluate return value.
 * \param[in,out]  g
 * \config     FEE_USE_RELIABLE_ERASE_PROCEDURE == STD_ON
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_InternalDoEraseStartMarkerWrite(Fee_GlobalsPtrType g);
#endif
...
/**********************************************************************************************************************
 * Fee_InternalLogError
 *********************************************************************************************************************/
/*!
 * \brief      Logs the error flag for the given error into the global error flags variable.
 * \details    -
 * \param[in]  error: the error that shall be logged
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_InternalLogError(Fee_InternalErrorType error);


/**********************************************************************************************************************
 * Fee_InternalGetSectorSwitchLogInformation
 *********************************************************************************************************************/
/*!
 * \brief      Returns log information about the sector switch.
 * \details    Return value contains information: Is SS onngoing, which kind of SS is ongoing, are bss and fss allowed?
 * \return     log information
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(uint8) Fee_InternalGetSectorSwitchLogInformation(void);


/**********************************************************************************************************************
 * Fee_InternalGetSectorGeneralLogInformation
 *********************************************************************************************************************/
/*!
 * \brief      Returns log information that did not fit in any other category.
 * \details    Return value contains information: is a broken link detected and is fee in fbl mode?
 * \return     log information
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(uint8) Fee_InternalGetGeneralLogInformation(void);


/**********************************************************************************************************************
 * Fee_InternalStartupUpdateSectorId
 *********************************************************************************************************************/
/*!
 * \brief      Updates the sector id in the partition init context with the corresponding sector id.
 * \details    Function uses the partition init context to decide whether to update lower or upper sector.
 * \param[in]  sectorId: id that shall be set
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_InternalStartupUpdateSectorId(uint8 sectorId);
...
#if (FEE_DATA_CONVERSION_API == STD_ON || FEE_LOOKUPTABLE_MODE == STD_ON)
...
/**********************************************************************************************************************
 * Fee_InternalConfirmExplicitlyMarkedInstance
 *********************************************************************************************************************/
/*!
 * \brief      Determines, if the instance has really the status it pretends or if it is write aborted
 * \details    Checks that also the following three bytes equal the instance status
 * \param[in]  intBuf_au8 buffer containing at least the four first bytes of the instance
 * \param[in]  pretendedStatus the status that was revealed by the instance header byte
 * \return     TRUE: pretended status is correct, FALSE: pretended status is not correct, instance is write aborted
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_INLINE_FUNC(Fee_Bool) Fee_InternalConfirmExplicitlyMarkedInstance(Fee_IntBuffer_u8ConstPtrType intBuf_au8, Fee_InstanceStatusType pretendedStatus);

/**********************************************************************************************************************
 * Fee_InternalPrepareSectorFormatContext
 *********************************************************************************************************************/
/*!
 * \brief      Sets all necessary variables in the sector format context structure.
 * \details    -
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_InternalPrepareSectorFormatContext(void);

/**********************************************************************************************************************
 * Fee_InternalComputeNewLinkTableSize
 *********************************************************************************************************************/
/*!
 * \brief      Sets all necessary variables in the sector format context structure.
 * \details    -
 * \return     Size of the link table that shall be used for the new sector header.
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(uint16) Fee_InternalComputeNewLinkTableSize(void);
...
static CONST(sint8, FEE_PRIVATE_CONST) FEE_INVALID_SECTOR_NUMBER = -1; /* PRQA S 3218 */ /* MD_FEE_8.9_invalid_block_number */
static CONST(sint8, FEE_PRIVATE_CONST) FEE_LOWER_SECTOR_NUMBER = 0;
static CONST(sint8, FEE_PRIVATE_CONST) FEE_UPPER_SECTOR_NUMBER = 1;
...
static CONST(struct Fee_StateDescrStruct, FEE_PRIVATE_CONST) Fee_StateDescrTable_at[] =
{
    ...
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_LOCAL_JOB_RESULT_OK, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN { FEE_ACT_ID_UPDATE_LUT, FEE_ACT_ID_PREPARE_PAYLOAD_DATA_PROCESSING},
                FEE_NEXT_STATE(FEE_STATE_BUSY_PROCESS_PAYLOAD_DATA)
            }
        },
        {
            FEE_STATE_ELSE { FEE_ACT_ID_UPDATE_LUT, FEE_ACT_ID_RETURN_FROM_SUB_FSM},
            FEE_NEXT_STATE(FEE_STATE_IDLE)
        }
    },
    ...
    {
        /* Chunk found, continue following chunk-links, if allowed*/
        {
    ...
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_FLS_JOB_RESULT_OK, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_FINISH_SUCCESSFUL_WRITING, FEE_ACT_ID_RETURN_FROM_SUB_FSM},
                FEE_NEXT_STATE(FEE_STATE_IDLE)
            }
        },
        {
            FEE_STATE_ELSE {FEE_ACT_ID_FINISH_UNSUCCESSFUL_WRITING, FEE_ACT_ID_RETURN_FROM_SUB_FSM},
            FEE_NEXT_STATE(FEE_STATE_IDLE)
        }
    },
    ...
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_FLS_JOB_RESULT_OK, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_PREPARE_WRITE_STREAM_THIRD, FEE_ACT_ID_NOP},
    ...
    /* FEE_STATE_CREATE_INSTANCE_4_REMAINING_WRITE_STREAMS */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_FLS_JOB_RESULT_OK, FEE_QRY_ID_TRUE},
            {
    ...
    /* FEE_STATE_CREATE_INSTANCE_BLOCK_COPY_READ */
    {
        ...
        {
            FEE_STATE_ELSE {FEE_ACT_ID_FINISH_UNSUCCESSFUL_COPYING, FEE_ACT_ID_RETURN_FROM_SUB_FSM},
            FEE_NEXT_STATE(FEE_STATE_IDLE)
        }
    },
    ...
    /* FEE_STATE_CREATE_INSTANCE_BLOCK_COPY_WRITE_2 */
    {
        ...
        {
            FEE_STATE_ELSE {FEE_ACT_ID_PREPARE_BLOCK_COPY_INSTANCE_TRAILER_WRITE, FEE_ACT_ID_NOP},
            FEE_NEXT_STATE(FEE_STATE_CREATE_INSTANCE_BLOCK_COPY_INSTANCE_TRAILER_WRITE)
        }
    },
     /* FEE_STATE_CREATE_INSTANCE_BLOCK_COPY_INSTANCE_TRAILER_WRITE */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_FLS_JOB_RESULT_OK, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_FINISH_SUCCESSFUL_COPYING, FEE_ACT_ID_RETURN_FROM_SUB_FSM},
                FEE_NEXT_STATE(FEE_STATE_IDLE)
            }
        },
        {
    ...
    /* FEE_STATE_SEARCH_UPWARDS_INSTANCE_EVALUATION_2 */
    {
        ...
        {
            FEE_STATE_ELSE {FEE_ACT_ID_INCR_TO_NEXT_INSTANCE_INDEX, FEE_ACT_ID_NOP},
            FEE_NEXT_STATE(FEE_STATE_SEARCH_UPWARDS_CONTINUE_CHUNK_SEARCH)
        }
    },

    /* FEE_STATE_SEARCH_UPWARDS_CONTINUE_CHUNK_SEARCH */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_MAY_FOLLOW_LINKS,FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_CONTINUE_UPWARDS_CHUNK_SEARCH,FEE_ACT_ID_PREPARE_CHUNK_LINK_READ_OF_CURRENT_CHUNK},
                FEE_NEXT_STATE(FEE_STATE_SEARCH_UPWARDS_FOLLOW_CHUNK_LINK_1)
            }
        },
        {
            FEE_STATE_ELSE {FEE_ACT_ID_CONTINUE_UPWARDS_CHUNK_SEARCH, FEE_ACT_ID_INIT_CHUNK_HOPPING_FSM},
            FEE_NEXT_STATE(FEE_STATE_SEARCH_UPWARDS_CHUNK_HOPPING)
        }
    },
    ...
    {
        {
            ...
            {
                FEE_STATE_THEN {FEE_ACT_ID_FINALIZE_UPWARDS_SEARCH_EMPTY_LINK, FEE_ACT_ID_RETURN_FROM_SUB_FSM},
                FEE_NEXT_STATE(FEE_STATE_IDLE)
            }
        },
        {
...
#if (FEE_USE_RELIABLE_ERASE_PROCEDURE == STD_ON)
     /* FEE_STATE_SETUP_SECTORS_WRITE_ERASE_START_MARKER */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_FLS_JOB_RESULT_OK, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_SETUP_SECTORS_ERASE_SECTOR, FEE_ACT_ID_NOP},
...
#if (FEE_USE_RELIABLE_ERASE_PROCEDURE == STD_ON)
    /* FEE_STATE_STARTUP_READ_SECTOR_HEADER */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_TRUE, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_EVALUATE_SECTOR_HEADER, FEE_ACT_ID_NOP},
                FEE_NEXT_STATE(FEE_STATE_STARTUP_EVALUATE_SECTOR_HEADER_CHECK_VALIDITY)
            }
        },
        {
            FEE_STATE_ELSE {FEE_ACT_ID_EVALUATE_SECTOR_HEADER, FEE_ACT_ID_NOP},
            FEE_NEXT_STATE(FEE_STATE_STARTUP_EVALUATE_SECTOR_HEADER_CHECK_VALIDITY)
        }
    },
        /* FEE_STATE_STARTUP_EVALUATE_SECTOR_HEADER_CHECK_VALIDITY */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_IS_SECTOR_HEADER_VALID, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_READ_ERASE_START_MARKER, FEE_ACT_ID_NOP},
                FEE_NEXT_STATE(FEE_STATE_STARTUP_READ_ERASE_START_MARKER)
            }
        },
        {
            FEE_STATE_ELSE {FEE_ACT_ID_NOP, FEE_ACT_ID_NOP},
            FEE_NEXT_STATE(FEE_STATE_STARTUP_SWITCH_TO_NEXT_SECTOR)
        }
    },
#else
    /* FEE_STATE_STARTUP_READ_SECTOR_HEADER */
...
#endif
    /* FEE_STATE_STARTUP_SWITCH_TO_NEXT_SECTOR */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_IS_SECTOR_HEADER_LEFT_TO_READ, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_READ_NEXT_SECTOR_HEADER, FEE_ACT_ID_NOP},
                FEE_NEXT_STATE(FEE_STATE_STARTUP_READ_SECTOR_HEADER)
            }
        },
        {
            FEE_STATE_ELSE {FEE_ACT_ID_FINALIZE_SECTOR_HEADER_EVALUATION, FEE_ACT_ID_NOP},
            FEE_NEXT_STATE(FEE_STATE_STARTUP_FINISH_SECTOR_CHECKING)
        }
    },
#if (FEE_USE_RELIABLE_ERASE_PROCEDURE == STD_ON)
    /* FEE_STATE_STARTUP_READ_ERASE_START_MARKER */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_TRUE, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_EVALUATE_ERASE_START_MARKER,FEE_ACT_ID_NOP },
                FEE_NEXT_STATE(FEE_STATE_STARTUP_SWITCH_TO_NEXT_SECTOR)
            }
        },
        {
            FEE_STATE_ELSE {FEE_ACT_ID_EVALUATE_ERASE_START_MARKER,FEE_ACT_ID_NOP},
            FEE_NEXT_STATE(FEE_STATE_STARTUP_SWITCH_TO_NEXT_SECTOR)
        }
    },
#endif
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
    /* FEE_STATE_STARTUP_FINISH_SECTOR_CHECKING */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_LUT_IS_LUT_ENABLED, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_LUT_INITIALIZATION, FEE_ACT_ID_NOP},
                FEE_NEXT_STATE(FEE_STATE_STARTUP_READ_LUT_BLOCK)
            }
        },
        {
            FEE_STATE_ELSE {FEE_ACT_ID_RETURN_FROM_SUB_FSM, FEE_ACT_ID_NOP},
    ...
    /* FEE_STATE_LUT_CHUNK_SEARCH */
    {
        ...
        /* Exit 2 */
        {
            FEE_STATE_ELSE {FEE_ACT_ID_LUT_RESET_LUT_ENTRY,FEE_ACT_ID_PREPARE_CHUNK_LINK_READ_OF_CURRENT_CHUNK},
            FEE_NEXT_STATE(FEE_STATE_CHUNK_SEARCH_FOLLOW_CHUNK_LINK)
        }
    },


    /* FEE_STATE_LUT_CHUNK_SEARCH_READ_CHUNK_HEADER */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_TRUE, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_LUT_CHECK_CHUNK_HEADER, FEE_ACT_ID_NOP},
                FEE_NEXT_STATE(FEE_STATE_LUT_CHUNK_SEARCH_CHECK_CHUNK_HEADER)
            }
        },
        /* Exit 2 */
        {
    ...
    /* FEE_STATE_LUT_CHUNK_SEARCH_CHECK_CHUNK_HEADER */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_LOCAL_JOB_RESULT_OK, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_LUT_READ_CHUNK_LINK, FEE_ACT_ID_NOP},
                FEE_NEXT_STATE(FEE_STATE_LUT_CHUNK_SEARCH_READ_CHUNK_LINK)
            }
        },
        {
            FEE_STATE_ELSE {FEE_ACT_ID_LUT_HANDLE_INVALID_HEADER, FEE_ACT_ID_RETURN_FROM_SUB_FSM},
            FEE_NEXT_STATE(FEE_STATE_IDLE)
        }
    },

    /* FEE_STATE_LUT_CHUNK_SEARCH_READ_CHUNK_LINK */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_TRUE, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_LUT_VALIDATE_CHUNK_LINK, FEE_ACT_ID_NOP},
                FEE_NEXT_STATE(FEE_STATE_LUT_CHUNK_SEARCH_CHECK_CHUNK_LINK)
            }
        },
        {
            FEE_STATE_ELSE {FEE_ACT_ID_NOP, FEE_ACT_ID_NOP},
    ...
    /* FEE_STATE_LUT_CHUNK_SEARCH_CHECK_CHUNK_LINK */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_LOCAL_JOB_RESULT_OK, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_LUT_FINALIZE_CHUNK_SEARCH, FEE_ACT_ID_RETURN_FROM_SUB_FSM},
                FEE_NEXT_STATE(FEE_STATE_IDLE)
            }
        },
        {
            FEE_STATE_ELSE {FEE_ACT_ID_LUT_HANDLE_INVALID_LINK, FEE_ACT_ID_RETURN_FROM_SUB_FSM},
            FEE_NEXT_STATE(FEE_STATE_IDLE)
        }
    },

    /* FEE_STATE_STARTUP_READ_LUT_BLOCK */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_LUT_MORE_BLOCKS_TO_READ, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_LUT_VALIDATE_BLOCK_CONTENT, FEE_ACT_ID_LUT_READ_LUT_BLOCK},
                FEE_NEXT_STATE(FEE_STATE_STARTUP_READ_LUT_BLOCK)
            }
        },
        {
            FEE_STATE_ELSE {FEE_ACT_ID_LUT_VALIDATE_BLOCK_CONTENT, FEE_ACT_ID_RETURN_FROM_SUB_FSM},
            FEE_NEXT_STATE(FEE_STATE_IDLE)
        }
    },

    /* FEE_STATE_MAIN_SYNC_LOOKUPTABLE */
    {
        {
            FEE_STATE_IF {FEE_QRY_ID_TRUE, FEE_QRY_ID_TRUE},
            {
                FEE_STATE_THEN {FEE_ACT_ID_LUT_FINISH_BLOCK_SYNCHRONIZATION, FEE_ACT_ID_NOP},
                FEE_NEXT_STATE(FEE_STATE_IDLE)
            }
        },
        {
...
/* Just eases development; some kind of compile-time assertion: do ActionTable, QueryTable and StateTable
...
struct Fee_tagFsmTableSizesCheck { /* PRQA S 3213 */ /* MD_FEE_2.4_CompileChecks */
    Fee_BitFieldType ActionTable : ((sizeof(Fee_ActionTable_ap) == (((uint16)(FEE_ACT_ID_WAIT + 1)) * sizeof(Fee_ActFctPtrType))) ? 1 : -1); /* PRQA S 3493, 4521 */ /* MD_FEE_14.3_CompileChecks, MD_FEE_10.1_Action_Query_State_Compile_Checks */
    Fee_BitFieldType QueryTable  : ((sizeof(Fee_QueryTable_ap) == (((uint16)(FEE_QRY_ID_TRUE + 1)) * sizeof(Fee_QryFctPtrType))) ? 1 : -1); /* PRQA S 3493, 4521 */ /* MD_FEE_14.3_CompileChecks, MD_FEE_10.1_Action_Query_State_Compile_Checks */
    Fee_BitFieldType StateTable  : ((sizeof(Fee_StateDescrTable_at) == (FEE_STATE_NEVER_ENTERED * sizeof(struct Fee_StateDescrStruct))) ? 1 : -1); /* PRQA S 3493, 4521 */ /* MD_FEE_14.3_CompileChecks, MD_FEE_10.1_Action_Query_State_Compile_Checks */
};
...
{
    return g->FsmStack_t.Stack_at[0] == FEE_STATE_MAIN_SECTOR_SWITCH; /* PRQA S 4403, 1881 */ /* MD_FEE_10.3_essentially_boolean_type,  MD_FEE_10.4_compare_enums */
}
...
FEE_INLINE_FUNC(Fee_Bool) Fee_FsmIsIdle(Fee_FsmStackConstRefType fsmStack)
{
    return fsmStack->Stack_at[0] == FEE_STATE_IDLE; /* PRQA S 4403, 1881 */ /* MD_FEE_10.3_essentially_boolean_type,  MD_FEE_10.4_compare_enums */
}
...
FEE_INTERN_FUNC(void) Fee_DoProcessing(Fee_GlobalsPtrType g)
{
    ...
    if(Fee_FsmIsIdle(&g->FsmStack_t) == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_Func_passParamByReference */
    {
     if (Fee_InternalIsFlsBusy(g) == FEE_FALSE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_PointerWrite_refParam */
     {
    ...
    while(!g->InternalFlags_t.leaveMain) /* PRQA S 4558, 2872, 4116 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_4.1_Loop_Does_Terminate, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
    {
...
FEE_LOCAL_FUNC(void) Fee_InternalHandleCancelRequest(Fee_GlobalsPtrType g)
{
    ...
    if(Fee_InternalIsFlsBusy(g) == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_PointerWrite_refParam */ /* COV_FEE_FLS_CANCEL */
    {
        Fee_FlsCancel();
    }
...
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
    /*Prevents from continuing with look up table sync*/
    g->InternalFlags_t.syncLookUpTable = FALSE; /* SBSW_FEE_PointerWrite_refParam */
#endif
...
FEE_LOCAL_FUNC(void) Fee_InternalForceSectorSwitchSetup(Fee_ConfigRefTypeInternal cfg)
{
    ...
    for(; partIdx < cfg->Fee_NumberOfPartitions_u8; partIdx++) {
...
FEE_LOCAL_FUNC(void) Fee_InternalSetupToplevelFsm(Fee_GlobalsPtrType g)
{
    ...
    {
        g->SectorSwitchData_t.partitionId_s8 = ((sint8)g->Config_pt->Fee_NumberOfPartitions_u8 - (sint8)1); /* SBSW_FEE_PointerWrite_refParam */
...
#endif
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
  if (g->ApiFlags.syncLookUpTable == TRUE)
  {
    if (Fee_Internal_LutInitSynchronization(g) == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_PointerWrite_refParam */
    {
      Fee_GlobalSuspend();
      g->InternalFlags_t.syncLookUpTable = TRUE; /* SBSW_FEE_PointerWrite_refParam */
      g->ApiFlags.syncLookUpTable = FALSE; /* SBSW_FEE_PointerWrite_refParam */
      Fee_GlobalRestore();
    }
    else
    {
...
#endif
    if(Fee_InternalScheduleProcess(g) == FEE_FALSE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_PointerWrite_refParam */
    {
        Fee_Internal_SignalWait();
...
#if(FEE_FORCE_SECTOR_SWITCH_API == STD_ON)
                && (!g->ApiFlags.forceSectorSwitch) /* PRQA S 4558, 4116 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
#endif
        ) /* COV_FEE_PREEMPTION */
        {
...
FEE_INLINE_FUNC(void) Fee_FsmProcess(Fee_FsmStackRefType fsmStack)
{
    ...
    if ((Fee_FsmQuery(CurrentExit_ptloc->ifChange_t.Queries_at) == FEE_TRUE)) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_FunctionPointerCall_FsmTable */
    {
        transition = CurrentExit_ptloc->ifChange_t.Next_t;
    }
    else
    {
        transition = CurrentExit_ptloc->elseChange_t;
    }
...
FEE_INLINE_FUNC(Fee_Bool) Fee_FsmQuery(Fee_StateQueryPtrType Queries_at)
{
    ...
    /* PRQA S 3344,3415 2 */ /* MD_FEE_13.2_boolean, MD_FEE_13.5_queries */
    return (((Fee_QueryTable_ap[Queries_at[0u]]()) && (Fee_QueryTable_ap[Queries_at[1u]]()))); /* PRQA S 4403, 4558, 4115 */ /* MD_FEE_10.3_essentially_boolean_type, MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_FunctionPointerCall_FsmTable */ /* SBSW_FEE_FunctionPointerCall_FsmTable */
}
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_InstSearch_BinarySplitRange(Fee_InstanceSearchContextRefType context_pt, Fee_InstanceStatusType status)
{
    Fee_Bool retVal = FEE_FALSE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
    ...
    if(context_pt->binary.step_t > 0u)
    {
        retVal = FEE_TRUE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_InstSearch_LinearDown(Fee_InstanceSearchContextRefType context_pt, Fee_InstanceStatusType status)
{
    ...
    if((context_pt->currentInstance.index_t > 0u) && (status == FEE_INSTANCE_STATUS_EMPTY))
    {
        retVal = FEE_TRUE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
...
FEE_LOCAL_FUNC(void) Fee_ActDecrementToPrevInstanceIndex(void)
{
    ...
    if(Fee_InstSearch_LinearDown(context_pt,   /* SBSW_FEE_Func_passParamByReference */
            Fee_InternalEvalInstHdrStatus(context_pt, Fee_Globals_t.FlsJobResult_t)) == FEE_FALSE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_Func_passParamByReference */
    {
...
FEE_LOCAL_FUNC(void) Fee_ActEvaluateChunkHopping(void)
{
    ...
    Fee_Bool continueSearch = FEE_FALSE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
    ...
    enum Fee_MngmtStatus headerStatus = Fee_ChunkInfo_EvaluateHeaderData(
        ...
        (Fee_Globals_t.FlsJobResult_t == MEMIF_JOB_OK)); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
    ...
    switch (headerStatus) 
    {
        ...
        case FEE_MNGMT_CONSISTENT:
            ...
            /* PRQA S 3344,3415 2 */ /* MD_FEE_13.2_boolean, MD_FEE_13.5_queries */
            if(((((context->searchBlockId_t.Tag) ^ (blockId.Tag)) & (context->blockIdMask_t.Tag)) != 0u)
                    || ((Fee_ChunkInfo_isDataChunk(chunk)) /* SBSW_FEE_Func_passParamByReference */
                       && ((((context->searchBlockId_t.DataIndex) ^ (blockId.DataIndex)) & (context->blockIdMask_t.DataIndex)) != 0u))) /* PRQA S 4558, 4115 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
            {

                continueSearch = FEE_TRUE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
        ...
        default:
            ...
            continueSearch = FEE_TRUE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
            Fee_InternalLogError(FEE_INTERNAL_ERROR_BLOCK_CORRUPTED_CHUNKHEADER);
            break;
    }

    if(continueSearch == FEE_FALSE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */
    {
 ...
 */
    ...
    FEE_LOCAL_FUNC(void) Fee_ActEvaluateWriteCycle(void)
    {
        ...
        /* PRQA S 3415, 4403 3 */ /* MD_FEE_13.5_queries, MD_FEE_10.3_essentially_boolean_type */
        const Fee_Bool wccErased = ((!Fee_ChunkInfo_isDataChunk(&instDescr->chunk_t)) /* PRQA S 4558, 4116 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_ConstPointer_refParam */
        ...
        else
        {
            context_pt->buffer_au32[FEE_CHUNK_HEADER_WCC_WORD_OFFSET] += /* SBSW_FEE_InternalDataBuffer_writeAccess */
                instDescr->instance_t.index_t + ((instDescr->instance_t.status_t != FEE_INSTANCE_STATUS_EMPTY) ? 1uL : 0uL);
        }
    }
...
FEE_LOCAL_FUNC(void) Fee_ActEvaluateInstanceTrailer(void)
{
    Fee_InstanceSearchContextConstRefType context_pt = &Fee_Globals_t.FsmLocalVars_t.instanceSearchContext_t;
...
FEE_LOCAL_FUNC(void) Fee_ActFinalizeGetNextBlockFsm(void)
{
    Fee_ChunkHoppingContextConstRefType context_pt = &Fee_Globals_t.FsmLocalVars_t.chunkSearchContext_t.super_t;
...
FEE_LOCAL_FUNC(void) Fee_ActFinalizeSetupSectors(void)
{
    ...
    { 
        ...
        Fee_InternalLogError(FEE_INTERNAL_ERROR_FORMAT_FAILED);
    }
}
...
FEE_LOCAL_FUNC(void) Fee_ActFinalizeJob(void)
{
    ...
    if((Fee_Globals_t.FeeLocalJobResult_t != FEE_LOCAL_JOB_RESULT_OK) && /* PRQA S 1881 */ /* MD_FEE_10.4_compare_enums */
        (Fee_Globals_t.InternalFlags_t.jobDeferred == FEE_TRUE)) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */
    {
 ...
 */
    ...
    FEE_LOCAL_FUNC(void) Fee_ActFinalizeWriteCycleJob(void)
    {
        Fee_ChunkAllocContextConstRefType context_pt = &Fee_Globals_t.FsmLocalVars_t.ChunkAllocContext_t;
...
FEE_LOCAL_FUNC(void) Fee_ActInitChunkSearchFsm(void)
{
    ...
    Fee_SectorRefType searchSector;  /* PRQA S 3679 */ /* MD_FEE_8.13_not_const_in_other_configs */
    context_pt->chunkHoppingForced = FALSE; /* SBSW_FEE_Func_writeObjectByReference */
    searchSector = Fee_ChunkInfo_GetSector(&context_pt->recentChunk_pt->chunk_t); /* SBSW_FEE_Chunks_Sector_ref */ /* SBSW_FEE_ChunkSearchContext */
    /* PRQA S 3344 1 */ /* MD_FEE_13.2_boolean */
    if(Fee_Sector_isUsable(searchSector))  /* SBSW_FEE_Func_passParamByReference */
    {
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
      if(!Fee_Internal_LutTryToStartLutSearch(searchSector)) /* PRQA S 4558, 4116 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_Func_passParamByReference */
#endif
      {
...
FEE_LOCAL_FUNC(void) Fee_ActInitCreateInstanceFsm(void)
{
    ...
    Fee_InstanceDescriptorConstRefType target_pt = globals->FsmLocalVars_t.ChunkAllocContext_t.previousChunk_pt;
    Fee_SectorConstRefType targetSector = Fee_ChunkInfo_GetSector(&target_pt->chunk_t); /* SBSW_FEE_ChunkAllocContext */

    /* PRQA S 3415,3344 3 */ /* MD_FEE_13.5_queries, MD_FEE_13.2_boolean */
    if((!globals->FsmLocalVars_t.ChunkAllocContext_t.config_pt->CriticalData_bl) || /* PRQA S 4558, 4116 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
       (Fee_Proc_isNewestSector(partition, targetSector) == FEE_FALSE) || /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_InitializedProcessData */ /* SBSW_FEE_Chunks_Sector_ref */
    ...
    {  
        ...
        if((globals->JobParams.requestedJob == FEE_JOB_WRITE) /* PRQA S 1881 */ /* MD_FEE_10.4_compare_enums */
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
            || (globals->InternalFlags_t.syncLookUpTable == TRUE)
#endif
            )
        {
            uint16 ceiledPayload = Fee_Proc_CeilPayloadSize(target_pt->chunk_t.Sector_pt, /* SBSW_FEE_ConstPointer_refParam */
            ...
            /*Use job params from current process data: Will be either a usual write job or a write job of a lut block. Nevertheless
             *we don't have to care here. current process data is initialized before with correct information*/
            context_pt->userData_pt = Fee_Globals_t.CurrentProcessData.jobParams.add.write.writeBuffer; /* SBSW_FEE_Func_writeObjectByReference */

            context_pt->ByteCount_u32 = Fee_Sector_alignWrite(target_pt->chunk_t.Sector_pt,(uint32)ceiledPayload + FEE_INSTANCE_HEADER_SIZE + /* SBSW_FEE_Func_writeObjectByReference */ /* SBSW_FEE_ConstPointer_refParam */
                                                              FEE_INSTANCE_TRAILER_SIZE );
            context_pt->numberOfCeilingBytes = (uint16)(ceiledPayload - Fee_Globals_t.CurrentProcessData.BlockConfig_pt->BlockPayloadSize_u16); /* SBSW_FEE_Func_writeObjectByReference */


            /*Depending on the number of bytes FEE needs up to three write streams*/
            Fee_FsmInitSubFsm(&globals->FsmStack_t, FEE_STATE_CREATE_INSTANCE_4_REMAINING_WRITE_STREAMS); /* SBSW_FEE_Func_passParamByReference */
...
/**********************************************************************************************************************
 * Fee_Internal_GetBlockNumberOfNextBlockToCopyInPartition
 *********************************************************************************************************************/
/*!
 * \brief      Returns the index of the next block in the given partition that shall be checked if it must be copied.
 * \details    Loops over Fee_BlockConfig_at until it finds the next block of the same partition. We here live with
 *             the loop implementation. Better than waiting for the next main function cycle all the time.
 * \param[in]  currentIndex: Index from where this function should start to find the next block
 * \param[in]  partitionId: Id of the partition where the block shall belong to
 * \param[in]  cfg: pointer to configuration
 * \return     No further valid block: return NumberOfBlocks (signals out of range) Further valid block: return index of the block
 *             (guaranteed to be less or equal to NumberOfBlocks - 1)
 * \pre        FEE is not in FBL mode
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(uint16) Fee_Internal_GetBlockNumberOfNextBlockToCopyInPartition(uint16 currentIndex, uint8 partitionId,Fee_ConfigRefTypeInternal cfg);
...
FEE_LOCAL_FUNC(void) Fee_Internal_FblGetNextBlockToCopy(Fee_GlobalsPtrType g)
{
    ...
    if(Fee_ChunkInfo_GetBaseAddress(fblChunk) == 0uL) /* SBSW_FEE_Func_passParamByReference */
    {
    ...
    g->chunkHoppingContext_pt = context_pt; /* PRQA S 2916 */ /* MD_FEE_18.6_CopyingToPointerWithGreaterLifetime */ /* SBSW_FEE_PointerWrite_refParam */
...
FEE_LOCAL_FUNC(void) Fee_Internal_ApplGetNextBlockToCopy(Fee_GlobalsPtrType g)
{
    ...
    uint16 currBlockIndex = g->SectorSwitchData_t.mode.appl.BlockConfigIndex_u16;
    ...
    partitionId = Fee_Partition_getId(g->CurrentProcessData.Partition_pt); /* SBSW_FEE_PartitionConfig */
    currBlockIndex = Fee_Internal_GetBlockNumberOfNextBlockToCopyInPartition(currBlockIndex,partitionId,g->Config_pt); /* SBSW_FEE_GlobalConfigPointer */
    if(currBlockIndex < g->Config_pt->Fee_NumberOfBlocks_u16)
    {
        blockConfig = &blockConfig[currBlockIndex];

        if(DataIndex_s16loc < 0)
        {
            DataIndex_s16loc = ((sint16)blockConfig->NumberOfDatasets_u8 - (sint16)1);
        }
    ...
    {
        currBlockIndex = 0xFFFFu;
        DataIndex_s16loc = -1;
        g->FeeLocalJobResult_t = FEE_LOCAL_JOB_RESULT_FAILED; /* SBSW_FEE_Func_writeObjectByReference */
    }

    g->SectorSwitchData_t.mode.appl.BlockConfigIndex_u16 = currBlockIndex; /* SBSW_FEE_Func_writeObjectByReference */
 ...
 *
 */
FEE_LOCAL_FUNC(uint16) Fee_Internal_GetBlockNumberOfNextBlockToCopyInPartition(uint16 currentIndex, uint8 partitionId,
                                                                               Fee_ConfigRefTypeInternal cfg)
{
    uint16 index;
    for(index = currentIndex; index < cfg->Fee_NumberOfBlocks_u16; index++ )
    {

        if(((cfg->BlockConfig_at[index].PartitionIndex) == partitionId)
#if (FEE_LOOKUPTABLE_MODE == STD_ON)
         && 
            (cfg->BlockConfig_at[index].LookUpTableBlock_bl == FALSE)
#endif
        ) /* COV_FEE_PART_CONFIG */
        {
            break;
...
FEE_LOCAL_FUNC(void) Fee_InternalInitLinkTableAllocationFsm(Fee_GlobalsPtrType g)
{
    ...
    const uint8 datasets =  (uint8)((g->CurrentProcessData.BlockConfig_pt == NULL_PTR)
                                    ? (g->CurrentProcessData.BlockId.DataIndex + 1u)
                                    : g->CurrentProcessData.BlockConfig_pt->NumberOfDatasets_u8);

    /* PRQA S 3344,3415 1 */ /* MD_FEE_13.2_boolean, MD_FEE_13.5_queries */
    if((datasets > 1u) && (Fee_Proc_isLinkTableChunkAllowed(&target->chunk_t))) /* PRQA S 4558, 4115 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_Func_passParamByReference */
    {
...
FEE_LOCAL_FUNC(uint16) Fee_Proc_CeilPayloadSize(Fee_SectorConstRefType sector, uint16 blockSize)
{
    return (uint16)Fee_Sector_alignWrite(sector, (uint32)FEE_INSTANCE_HEADER_SIZE + blockSize) - FEE_INSTANCE_HEADER_SIZE; /* SBSW_FEE_ConstPointer_refParam */
}
...
FEE_LOCAL_FUNC(void) Fee_ActInitReverseChunkSearch(void)
{
  /*#10 Reset lut entry if lookup table mode is configured*/
#if (FEE_LOOKUPTABLE_MODE == STD_ON)
    Fee_Lut_ResetLut(&Fee_Globals_t.LookUpTableContext_t); /* SBSW_FEE_Func_passParamByReference */
#endif
    /*#20 If previous chunk info is pointing to the linktable (i.e. a pseudo chunk), there is nothing to search more in this sector other*/
    /*#30 If previous chunk info is not pointing to the linktable, search in same sector again*/
...
FEE_LOCAL_FUNC(void) Fee_ActContinueUpwardsChunkSearch(void)
{
    ...
    Fee_SectorConstRefType newSector = Fee_ChunkInfo_GetSector(&target->chunk_t); /* SBSW_FEE_Func_passParamByReference */
...
FEE_LOCAL_FUNC(void) Fee_ActInitSetupSectorsFsm(void)
{
    Fee_InternalPrepareSectorFormatContext();
#if (FEE_USE_RELIABLE_ERASE_PROCEDURE == STD_ON)
    if(Fee_Globals_t.FsmLocalVars_t.SectorFormatContext_t.sector_pt->eraseStartMarkerWritten == FALSE)
    {
        Fee_FsmInitSubFsm(&Fee_Globals_t.FsmStack_t, FEE_STATE_SETUP_SECTORS_WRITE_ERASE_START_MARKER); /* SBSW_FEE_Func_passParamByReference */
        Fee_InternalDoEraseStartMarkerWrite(&Fee_Globals_t); /* SBSW_FEE_Func_passParamByReference */
    }
    else
    {
 ...
 *
 */
/* PRQA S 3673 1 */ /* MD_FEE_8.13_Pointer2ConstParams */
FEE_LOCAL_FUNC(void) Fee_InternalDoSectorErase(Fee_GlobalsPtrType g)
{
...
FEE_LOCAL_FUNC(void) Fee_InternalDoEraseStartMarkerWrite(Fee_GlobalsPtrType g)
{
    const Fee_SectorFormatContextRefType context_pt = &g->FsmLocalVars_t.SectorFormatContext_t;
     /* SectorFormatContext_t was initialized in Fee_ActInitSetupSectorsFsm */
    Fee_InternalEvalFlsRetVal(Fee_Sector_writeEraseStartMarker(context_pt->sector_pt,context_pt->sectorHeader_au32)); /* SBSW_FEE_Partitions_OldestNewestSectorRef */ /* SBSW_FEE_SectorFormatBuffer */
}
#endif
...
FEE_LOCAL_FUNC(void) Fee_Proc_Write_SetupChunkAlloc(Fee_GlobalsPtrType g, Fee_ChunkAllocContextRefType ctx)
{
    if((Fee_Proc_isLinkTableChunkAllowed(&ctx->previousChunk_pt->chunk_t)) /* SBSW_FEE_ChunkAllocContext */
            && (ctx->config_pt->NumberOfDatasets_u8 > 1u)) /* PRQA S 4558, 4115 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */                 /* SBSW_FEE_ChunkAllocContext */
    {
        struct Fee_ChunkInfoStruct c;
...
FEE_INLINE_FUNC(void) Fee_Proc_Write_SetupAllocation(Fee_GlobalsPtrType g, Fee_ChunkAllocContextRefType ctx)
{
    Fee_ChunkInfoConstRefType currChunk = &ctx->previousChunk_pt->chunk_t;


    /* Chunk allocation may be interrupted by a sector switch Deferred job will be executed after sector switch then*/
    /* PRQA S 3344,3415 4 */ /* MD_FEE_13.2_boolean, MD_FEE_13.5_queries */
    g->InternalFlags_t.jobDeferred = TRUE; /* SBSW_FEE_Func_writeObjectByReference */


    /* PRQA S 3415 4 */ /* MD_FEE_13.5_queries */
    if((Fee_ChunkInfo_isDataChunk(currChunk)) /* SBSW_FEE_ChunkAllocContext */
         && (ctx->previousChunk_pt->instance_t.index_t < Fee_ChunkInfo_GetInstanceCount(currChunk)) /* PRQA S 4558, 4115 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */  /* SBSW_FEE_ChunkAllocContext */
         && (Fee_Proc_isPayloadSizeCeiled(currChunk, ctx->config_pt)) /* PRQA S 4558, 4115 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */  /* SBSW_FEE_ChunkAllocContext */
...
FEE_LOCAL_FUNC(void) Fee_ActInitWriteProcessingFsm(void)
{
    ...
    if((criticality >= FEE_PART_FILL_LEVEL_FSS) ||
       (((Fee_Globals_t.InternalFlags_t.brokenLink == TRUE)) && (isNewSector == FEE_FALSE))) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */
    {
        Fee_Globals_t.FeeLocalJobResult_t = FEE_LOCAL_JOB_RESULT_FAILED;

        Fee_Globals_t.InternalFlags_t.jobDeferred = (Fee_Globals_t.ApiFlags.enableFss == TRUE); /* PRQA S 4404 */ /* MD_FEE_10.3_essentially_boolean_type */
    }
    else
    {
                                                                    ...
                                                                    Fee_SectorRefType sector_pt)
{
    ...
    if(Fee_Sector_isFillLevelKnown(sector_pt) == FEE_FALSE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_PointerWrite_refParam */
    {
...
FEE_LOCAL_FUNC(void) Fee_ActFinalizeSearchForNfa_WriteFsm(void)
{
    Fee_ChunkInfoConstRefType currChunk = &Fee_Globals_t.CurrentDataset_t.chunk_t;
...
FEE_LOCAL_FUNC(void) Fee_ActInitDataChunkAllocWriteFsm(void)
{
    ...
    uint16 ceiledPayloadSize = Fee_Proc_CeilPayloadSize(Fee_ChunkInfo_GetSector(&context_pt->previousChunk_pt->chunk_t), blockConfig->BlockPayloadSize_u16); /* SBSW_FEE_ConstPointer_refParam */ /* SBSW_FEE_ConstPointer_refParam */
...
FEE_LOCAL_FUNC(void) Fee_ActInitDataChunkAllocCopyFsm(void)
{
    ...
    ceiledPayload = Fee_Proc_CeilPayloadSize(source->chunk_t.Sector_pt,payload); /* SBSW_FEE_ConstPointer_refParam */
    if(blockConfig != NULL_PTR)
    {
        
        if((blockConfig->BlockPayloadSize_u16 != payload) && (ceiledPayload != payload)) /* SBSW_FEE_Func_passParamByReference */
        {
    ...
    if(Fee_ChunkInfo_getPayload(&target->chunk_t) != ceiledPayload) /* SBSW_FEE_Func_passParamByReference */
    {
        target->instance_t.index_t = targetInstances; /* SBSW_FEE_Func_writeObjectByReference */
    }

    /* PRQA S 3415, 4403 3 */ /* MD_FEE_13.5_queries, MD_FEE_10.3_essentially_boolean_type */
    chunkNecessary = ((Fee_ChunkInfo_isDataChunk(&target->chunk_t) == FEE_FALSE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_Func_passParamByReference */
                                || (target->instance_t.index_t >= targetInstances));


#if(FEE_GET_WRITE_CYCLE_API == STD_ON)
    /* PRQA S 4403, 4538, 4558, 4115, 4116 1 */ /* MD_FEE_10.3_essentially_boolean_type, MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.1_Essentially_boolean_in_logical_expressions,MD_FEE_10.1_Essentially_boolean_in_logical_expressions,MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
    ...
    if(chunkNecessary)   /* PRQA S 3344 */ /* MD_FEE_13.2_boolean */
    {
        ...
        Fee_ChunkInfo_CreateNewDataChunk(&c, ceiledPayload, instanceExponent); /* SBSW_FEE_Func_passParamByReference */
...
FEE_LOCAL_FUNC(void) Fee_ActAllocationInCurrentSector(void)
{
    /* PRQA S 3415 2 */ /* MD_FEE_13.5_queries */
    if((Fee_Globals_t.FeeLocalJobResult_t != FEE_LOCAL_JOB_RESULT_OK) && /* PRQA S 1881 */ /* MD_FEE_10.4_compare_enums */
       (Fee_Proc_isNewestSector(Fee_Globals_t.CurrentProcessData.Partition_pt, /* SBSW_FEE_InitializedProcessData */
                                      Fee_ChunkInfo_GetSector(&Fee_Globals_t.CurrentDataset_t.chunk_t)) == FEE_FALSE)) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_Func_passParamByReference */
    {
...
FEE_INLINE_FUNC(void) Fee_InternalDoCopyInit(Fee_GlobalsPtrType g)
{
    Fee_InstanceDescriptorConstRefType source = &g->CurrentDataset_t;
...
FEE_LOCAL_FUNC(void) Fee_ActInitializeBlockCopy(void)
{
    Fee_InstanceDescriptorConstRefType target = &Fee_Globals_t.SectorSwitchTarget_t;
    Fee_InstanceDescriptorConstRefType source = &Fee_Globals_t.CurrentDataset_t;
    const Fee_InstanceCopyContextRefType context_pt = &Fee_Globals_t.FsmLocalVars_t.instanceCopyContext_t;
    uint16 payload = Fee_ChunkInfo_getPayload(&source->chunk_t); /* SBSW_FEE_ConstPointer_refParam */
    ...
    context_pt->ByteCount_t = (uint32)Fee_Proc_CeilPayloadSize(source->chunk_t.Sector_pt, payload) + 1u;  /* SBSW_FEE_Func_writeObjectByReference */ /* SBSW_FEE_ConstPointer_refParam */
...
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
            Fee_ChunkInfo_Clone(&context_pt->targetChunk_t,&target->chunk_t);  /* SBSW_FEE_Func_passParamByReference */
#endif
            Fee_ActPrepareBlockCopyRead();
            break;
        case FEE_INSTANCE_STATUS_INCONSISTENT:
            ...
            /*Do not update lut entry here. This is done while creating the empty instance*/
            break;
        default:
            Fee_Globals_t.FeeLocalJobResult_t = FEE_LOCAL_JOB_RESULT_OK;
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
...
FEE_LOCAL_FUNC(void) Fee_InternalDoWriteStreamFirst(Fee_GlobalsPtrType g)
{
    ...
    /* Penultimate page (last payload bytes mixed up with ceiling bytes) and trailer page will be written in 3rd and 4th write stream*/
    uint32 twoPagesSize = Fee_Sector_alignTo(1u,context->sector_pt->cfg_pt->alignments_t.write_u8) * 2u;
    /* Remaining pages will be written in 1st and second write stream, depending on the size*/
    uint32 remainingSize = context->ByteCount_u32 - twoPagesSize;

    if(context->ByteCount_u32 == twoPagesSize)
    {
    /* 3rd and 4th write stream are sufficient, nothing more to do here*/
       context->numberOfWritesNecessary = 2u; /* SBSW_FEE_Func_writeObjectByReference */
    }
    else
    {
       uint32 payloadBytesToCopy;
       if(remainingSize > FEE_INTERNAL_BUFFER_SIZE)
       {
       /* All four write streams are necessary.*/
          context->numberOfWritesNecessary = 4u; /* SBSW_FEE_Func_writeObjectByReference */
          payloadBytesToCopy = FEE_INTERNAL_BUFFER_SIZE - 1u;
          context->ByteCount_u32 -= FEE_INTERNAL_BUFFER_SIZE; /* SBSW_FEE_Func_writeObjectByReference */
       }
       else
       {
          context->numberOfWritesNecessary = 3u; /* SBSW_FEE_Func_writeObjectByReference */
          payloadBytesToCopy = remainingSize - 1u;
          context->ByteCount_u32 -= remainingSize; /* SBSW_FEE_Func_writeObjectByReference */
       }
       DataBuffer_pu8loc[0] = (uint8)FEE_INSTANCE_STATUS_OK; /* SBSW_FEE_Func_writeObjectByReference */ /* Note: Accessing [0] cannot go out of bounds! */
       ...
       Fee_InternalEvalFlsRetVal(Fee_Sector_writeData(context->sector_pt, writeAddr, DataBuffer_pu8loc, payloadBytesToCopy + 1u)); /* SBSW_FEE_ConstPointer_refParam */
       context->targetOffset_t += payloadBytesToCopy + 1u;  /* SBSW_FEE_Func_writeObjectByReference */
    }
}
...
FEE_LOCAL_FUNC(void) Fee_ActPrepareWriteStreamSecond(void)
{
    Fee_InstanceWriteContextConstRefType context = &Fee_Globals_t.FsmLocalVars_t.instanceWriteContext_t;

    if(context->numberOfWritesNecessary == 4u)
    {
...
FEE_LOCAL_FUNC(void) Fee_InternalDoWriteStreamSecond(Fee_GlobalsPtrType g)
{
    ...
    Fee_InternalEvalFlsRetVal(Fee_Sector_writeData(context->sector_pt, context->targetOffset_t, context->userData_pt, remainingSize)); /* SBSW_FEE_ConstPointer_refParam */
    context->userData_pt = &context->userData_pt[remainingSize]; /* SBSW_FEE_Func_writeObjectByReference */
    context->targetOffset_t += remainingSize; /* SBSW_FEE_Func_writeObjectByReference */
    context->ByteCount_u32 = twoPagesSize; /* SBSW_FEE_Func_writeObjectByReference */
}
...
{
    const Fee_InstanceWriteContextRefType context = &g->FsmLocalVars_t.instanceWriteContext_t;
    uint32 pageSize = Fee_Sector_alignTo(1u,context->sector_pt->cfg_pt->alignments_t.write_u8);
    uint32 remainingSize = pageSize - context->numberOfCeilingBytes;

    Fee_InternalFillInstanceBufferWithValue(context->BufferBytes,0x0u,pageSize); /* SBSW_FEE_InternalDataBuffer_passedTo */
    if(context->numberOfWritesNecessary == 2u)
    {
       context->BufferBytes[0] = (uint8)FEE_INSTANCE_STATUS_OK; /* SBSW_FEE_Func_writeObjectByReference */
       Fee_InternalFillInstanceBuffer(&context->BufferBytes[1], context->userData_pt, remainingSize - 1u); /* SBSW_FEE_InternalDataBuffer_passedTo */ /* SBSW_FEE_ConstPointer_refParam */
    }
    else
    {
       Fee_InternalFillInstanceBuffer(context->BufferBytes, context->userData_pt, remainingSize); /* SBSW_FEE_InternalDataBuffer_passedTo */ /* SBSW_FEE_ConstPointer_refParam */
    }

    Fee_InternalEvalFlsRetVal(Fee_Sector_writeData(context->sector_pt, context->targetOffset_t, context->BufferBytes, pageSize)); /* SBSW_FEE_ConstPointer_refParam */
    context->targetOffset_t += pageSize; /* SBSW_FEE_Func_writeObjectByReference */
    context->ByteCount_u32 = pageSize; /* SBSW_FEE_Func_writeObjectByReference */
}
...
FEE_LOCAL_FUNC(void) Fee_ActPrepareBlockCopyRead(void)
{
    ...
    Fee_LengthType readLength;
    context_pt->currentStep = FEE_SECTOR_SWITCH_STEP_BLOCK_COPY_READ; /* SBSW_FEE_Func_writeObjectByReference */
   
    if(context_pt->ByteCount_t <= FEE_INTERNAL_BUFFER_SIZE)
    {
        readLength = context_pt->ByteCount_t;
    }
    else
    {
        readLength = FEE_INTERNAL_BUFFER_SIZE;
    }

    Fee_Sector_ClearBuffer(oldSector, context_pt->BufferBytes, FEE_INTERNAL_BUFFER_SIZE);

    Fee_InternalEvalFlsRetVal(
        Fee_Sector_readDataWithoutBlankCheck(oldSector, context_pt->sourceOffset_t, context_pt->BufferBytes, readLength)); /* SBSW_FEE_Partitions_OldestNewestSectorRef */
...
FEE_LOCAL_FUNC(void) Fee_ActPrepareBlockCopyInstanceTrailerWrite(void)
{
    Fee_Proc_SetupWriteSuspendHandling(&Fee_Globals_t, Fee_InternalDoBlockCopyInstanceTrailerWrite); /* SBSW_FEE_Func_passParamByReference */
}
...
FEE_LOCAL_FUNC(void) Fee_InternalDoBlockCopyWrite(Fee_GlobalsPtrType g)
{
    ...
    context_pt->currentStep = FEE_SECTOR_SWITCH_STEP_BLOCK_COPY_WRITE; /* SBSW_FEE_Func_writeObjectByReference */

    writeLength = (context_pt->ByteCount_t > FEE_INTERNAL_BUFFER_SIZE) ?  FEE_INTERNAL_BUFFER_SIZE : context_pt->ByteCount_t;
...
FEE_LOCAL_FUNC(void) Fee_InternalDoBlockCopyInstanceTrailerWrite(Fee_GlobalsPtrType g)
{
    const Fee_InstanceCopyContextRefType context = &g->FsmLocalVars_t.instanceCopyContext_t;
    const Fee_SectorConstRefType newSector_pt = Fee_Partition_getNewestSector(Fee_Globals_t.CurrentProcessData.Partition_pt); /* SBSW_FEE_InitializedProcessData */
    const uint32 pageSize = Fee_Sector_alignTo(1u,newSector_pt->cfg_pt->alignments_t.write_u8);
    Fee_InternalFillInstanceBufferTrailerPage(context->BufferBytes); /* SBSW_FEE_InternalDataBuffer_passedTo */ /* SBSW_FEE_ConstPointer_refParam */
    Fee_InternalEvalFlsRetVal(Fee_Sector_writeData(newSector_pt, context->targetOffset_t, context->BufferBytes, pageSize)); /* SBSW_FEE_ConstPointer_refParam */
}
...
FEE_LOCAL_FUNC(void) Fee_ActPrepareWccRead(void)
{
  ...
  if (Fee_ChunkInfo_isDataChunk(chunk)) /* SBSW_FEE_ChunkAllocContext */
  {
    ...
    const Fee_LengthType addr = Fee_ChunkInfo_GetBaseAddress(chunk) + FEE_CHUNK_HEADER_SIZE_READ; /* SBSW_FEE_ChunkAllocContext */

#if (FEE_LOOKUPTABLE_MODE == STD_ON)
    if (Fee_Globals_t.JobParams.requestedJob == FEE_JOB_GET_WRITE_CYCLE) /* PRQA S 1881 */ /* MD_FEE_10.4_compare_enums */
    {
      Fee_Lut_UpdateLut(&Fee_Globals_t.LookUpTableContext_t, chunk);  /* SBSW_FEE_Func_passParamByReference */
    }
#endif
    ...
    addr, &context_pt->buffer_au32[FEE_CHUNK_HEADER_WCC_WORD_OFFSET], /* SBSW_FEE_InternalDataBuffer_passedTo */
    FEE_WCC_BYTE_COUNT));
  }
  else
  {
...
FEE_LOCAL_FUNC(void) Fee_ActPrepareChunkLinkReadOfCurrentChunk(void)
{
    ...
    context_pt->chunkLinkPresent = (boolean)(ret != FEE_REQ_SYNC_USELESS); /* PRQA S 4304 1 */ /* MD_FEE_10.5_cast essentially_boolean */ /* SBSW_FEE_Func_writeObjectByReference */
...
FEE_LOCAL_FUNC(void) Fee_ActPrepareWriteStreamThird(void)
{
    Fee_Proc_SetupWriteSuspendHandling(&Fee_Globals_t, Fee_InternalDoWriteStreamThird); /* SBSW_FEE_Func_passParamByReference */
}
...
FEE_LOCAL_FUNC(void) Fee_InternalPrepareEmptyInstanceWrite(Fee_SectorConstRefType sector, Fee_LengthType addr, Fee_InstanceStatusType status)
{
    ...
    bufPtr[0] = (uint8)status;       /* SBSW_FEE_Func_writeObjectByReference */ /* Note: Accessing [0] is always safe */
    bufPtr[1] = (uint8)status;/* SBSW_FEE_InternalDataBuffer_writeAccess */
    bufPtr[2] = (uint8)status; /* SBSW_FEE_InternalDataBuffer_writeAccess */
    bufPtr[3] = (uint8)status; /* SBSW_FEE_InternalDataBuffer_writeAccess */
...
FEE_LOCAL_FUNC(void) Fee_ActPreparePayloadDataProcessing(void)
{
    Fee_InstanceDescriptorConstRefType instDescr_ptloc = &Fee_Globals_t.CurrentDataset_t;
    /*Use job params from current process data: Will be either a usual read job or a read job of a lut block. Nevertheless
     *we don't have to care here. current process data is initialized before with correct information*/
    Fee_InternalEvalFlsRetVal(Fee_ChunkInfo_ReadInstancePayload(
                         ...
                         Fee_Globals_t.CurrentProcessData.jobParams.add.read.readBuffer, /* SBSW_FEE_UserDataBuffer */
                         Fee_Globals_t.CurrentProcessData.jobParams.add.read.offset,
                         Fee_Globals_t.CurrentProcessData.jobParams.add.read.length));
}
...
FEE_LOCAL_FUNC(void) Fee_ActReportSectorOverflow(void)
{
    if(Fee_Globals_t.FsmLocalVars_t.ChunkAllocContext_t.ChunkAllocResult_t == FEE_CHUNK_ALLOC_SECTOR_FULL)
    {
        ...
        Fee_InternalLogError(FEE_INTERNAL_ERROR_SECTOROVERFLOW);
    }
}
...
FEE_LOCAL_FUNC(void) Fee_ActGetCurrentChunkInfo(void)
{
    ...
    if(Fee_ChunkInfo_isLinkConsistent(&context_pt->recentChunk_pt->chunk_t) == FEE_FALSE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_ChunkSearchContext */
    {
...
FEE_LOCAL_FUNC(void) Fee_ActValidateChunkLink(void)
{
    /*#10 Validate chunk link using local chunk search context*/
    const Fee_ChunkSearchContextRefType context_pt = &Fee_Globals_t.FsmLocalVars_t.chunkSearchContext_t;
    Fee_Internal_ValidateChunkLinkInContext(context_pt);  /* SBSW_FEE_Func_passParamByReference */
}
...
FEE_LOCAL_FUNC(void) Fee_ActCheckChunkConsistency(void)
{
    ...
    Fee_Bool flsReadOk = (Fee_Globals_t.FlsJobResult_t == MEMIF_JOB_OK); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
    ...
    else
    {
      ...
      Fee_InternalLogError(FEE_INTERNAL_ERROR_BLOCK_INCONSISTENT_CHUNKHEADER);
    }
...
FEE_LOCAL_FUNC(void) Fee_ActFinalizeChunkSearch(void)
{
    Fee_ChunkSearchContextConstRefType context_pt = &Fee_Globals_t.FsmLocalVars_t.chunkSearchContext_t;
 ...
 */
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryWriteSuspended(void) {
    return (Fee_Globals_t.WritesSuspended_bl == TRUE); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
}
...
FEE_LOCAL_FUNC(void) Fee_ActEvaluateSectorHeader(void)
{
    Fee_PartitionConstRefType partition = Fee_Globals_t.CurrentProcessData.Partition_pt; /* SBSW_FEE_InitializedProcessData */
    const Fee_PartitionInitContextConstRefType context = &Fee_Globals_t.FsmLocalVars_t.PartitionInitContext_t;
    
    const Fee_Bool readOk = (Fee_Globals_t.FlsJobResult_t == MEMIF_JOB_OK); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */

    uint8 sectorId = Fee_Sector_evaluateHeaderData(
                       Fee_Partition_getSectorByPos(partition, (uint8)context->recentlyCheckedSectorIndex), /* SBSW_FEE_PartitionConfig */ 
                       Fee_Globals_t.FsmLocalVars_t.PartitionInitContext_t.sectorHeaderData, readOk); /* SBSW_FEE_InitializedProcessData */

    Fee_InternalStartupUpdateSectorId(sectorId);
}
...
{
    Fee_PartitionConstRefType partition = Fee_Globals_t.CurrentProcessData.Partition_pt; /* SBSW_FEE_InitializedProcessData */
    Fee_PartitionInitContextRefType context = &Fee_Globals_t.FsmLocalVars_t.PartitionInitContext_t;
    const Fee_SectorConstRefType sectorInfo =  Fee_Partition_getSectorByPos(partition, (uint8)context->recentlyCheckedSectorIndex); /* SBSW_FEE_PartitionConfig */ 
    Fee_InternalEvalFlsRetVal(Fee_Sector_readEraseStartMarker(sectorInfo,context->sectorHeaderData)); /* SBSW_FEE_Partitions_GetSectorByPosition */ /* SBSW_FEE_SectorFormatBuffer */
}
...
FEE_LOCAL_FUNC(void) Fee_ActEvaluateEraseStartMarker(void)
{
    Fee_PartitionConstRefType partition = Fee_Globals_t.CurrentProcessData.Partition_pt; /* SBSW_FEE_InitializedProcessData */
    const Fee_PartitionInitContextConstRefType context = &Fee_Globals_t.FsmLocalVars_t.PartitionInitContext_t;
    Fee_SectorRefType sectorInfo =  Fee_Partition_getSectorByPos(partition, (uint8)context->recentlyCheckedSectorIndex); /* SBSW_FEE_PartitionConfig */ 
    const Fee_Bool readOk = (Fee_Globals_t.FlsJobResult_t == MEMIF_JOB_OK); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
    Fee_Sector_evaluateEraseStartMarker(sectorInfo, context->sectorHeaderData,readOk); /* SBSW_FEE_Partitions_GetSectorByPosition */ /* SBSW_FEE_SectorFormatBuffer */
}
...
FEE_LOCAL_FUNC(void) Fee_ActFinalizeSectorHeaderEvaluation(void)
{
    ...
    Fee_Partition_initSector(partition, Fee_Partition_getSectorByPos(partition, (uint8)FEE_LOWER_SECTOR_NUMBER), /* SBSW_FEE_InitializedProcessData */ /* SBSW_FEE_PartitionConfig */ /* SBSW_FEE_InitializedProcessData */
            Fee_Globals_t.FsmLocalVars_t.PartitionInitContext_t.lowerSectorId);

    Fee_Partition_initSector(partition, Fee_Partition_getSectorByPos(partition, (uint8)FEE_UPPER_SECTOR_NUMBER), /* SBSW_FEE_InitializedProcessData */ /* SBSW_FEE_PartitionConfig */ /* SBSW_FEE_InitializedProcessData */
            Fee_Globals_t.FsmLocalVars_t.PartitionInitContext_t.upperSectorId);
...
FEE_LOCAL_FUNC(void) Fee_ActReadNextSectorHeader(void)
{
    const Fee_PartitionConstRefType partition = Fee_Globals_t.CurrentProcessData.Partition_pt;
    Fee_PartitionInitContextRefType context = &Fee_Globals_t.FsmLocalVars_t.PartitionInitContext_t;
    Fee_SectorConstRefType sectorInfo;
    /* it is assured by query before that we only get here when incrementing the index is allowed*/
    context->recentlyCheckedSectorIndex++; /* SBSW_FEE_Func_writeObjectByReference */
    sectorInfo = Fee_Partition_getSectorByPos(partition, (uint8)context->recentlyCheckedSectorIndex); /* SBSW_FEE_PartitionConfig */
    Fee_InternalEvalFlsRetVal(Fee_Proc_readSectorHeader(sectorInfo,context->sectorHeaderData)); /* SBSW_FEE_Partitions_GetSectorByPosition */ /* SBSW_FEE_SectorFormatBuffer */
}
...
FEE_LOCAL_FUNC(void) Fee_ActSetupSectorsEraseSector(void)
{
    FEE_NFY_SECTOR_ERASE(&Fee_Globals_t); /* SBSW_FEE_Func_passParamByReference */
    Fee_Proc_SetupWriteSuspendHandling(&Fee_Globals_t, Fee_InternalDoSectorErase); /* SBSW_FEE_Func_passParamByReference */
}
...
FEE_LOCAL_FUNC(void) Fee_Internal_ValidateChunkLinkInContext(const Fee_ChunkSearchContextRefType context_pt)
{
  ...
  enum Fee_MngmtStatus linkStatus = Fee_ChunkInfo_InitFromLinkData(newChunk_pt, /* SBSW_FEE_Func_passParamByReference */
                                                                   currentChunk_pt, /* SBSW_FEE_Func_passParamByReference */
                                      context_pt->super_t.headerBuffer_au32, (Fee_Globals_t.FlsJobResult_t == MEMIF_JOB_OK)); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
  ...
  default: /*FEE_MNGMT_CONSISTENT*/
    ...
    /* PRQA S 3344,3415 3 */ /* MD_FEE_13.2_boolean, MD_FEE_13.5_queries */
    if (((context_pt->super_t.searchBlockId_t.DataIndex) != 0u) && /* PRQA S 4558, 4115 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
       (Fee_ChunkInfo_isInitialLinkTable(currentChunk_pt)) && /* PRQA S 4558, 4115 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_Func_passParamByReference */
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryBlockCopyBytesRemaining(void)
{
    return (Fee_Globals_t.FsmLocalVars_t.instanceCopyContext_t.ByteCount_t > 0uL); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
}
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryChunkNotFull(void)
{
    ...
    return ((Fee_Globals_t.SectorSwitchTarget_t.instance_t.index_t + 1u) < /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryInstHeaderErasedValue(void)
{
    ...
    return (FEE_INSTANCE_STATUS_EMPTY == status); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
}
...
FEE_INLINE_FUNC(Fee_Bool) Fee_InternalIsFlsBusy(Fee_GlobalsPtrType g)
{
    Fee_Bool isBusy = FEE_FALSE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
    ...
    if (g->FlsJobResult_t == MEMIF_JOB_PENDING)
    {
        ...
        isBusy = (Fee_FlsGetStatus() == MEMIF_BUSY); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
        if (isBusy == FEE_FALSE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */
        {
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryFlsJobResultOk(void)
{
    return (MEMIF_JOB_OK == Fee_Globals_t.FlsJobResult_t); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
}
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryValidInstanceWritten(void)
{
    return ((Fee_Globals_t.FeeLocalJobResult_t != FEE_LOCAL_JOB_RESULT_EMPTY_CHUNKS) && /* PRQA S 4403, 1881 */ /* MD_FEE_10.3_essentially_boolean_type,  MD_FEE_10.4_compare_enums */
                     (Fee_Globals_t.FeeLocalJobResult_t != FEE_LOCAL_JOB_RESULT_FAILED)); /* PRQA S 1881 */ /* MD_FEE_10.4_compare_enums */
}  
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryBlockPayloadUpdated(void)
{
    ...
    const Fee_BlockConfigRefType blockCfg_pt = Fee_Globals_t.CurrentProcessData.BlockConfig_pt;
    Fee_Bool retVal = FEE_FALSE;  /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */


    if((Fee_ChunkInfo_isDataChunk(chunk_pt)) && (blockCfg_pt != NULL_PTR)) /* PRQA S 4558, 4115 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_ChunkSearchContext */
    {
        uint16 ceiledPayloadSize = Fee_Proc_CeilPayloadSize(chunk_pt->Sector_pt, blockCfg_pt->BlockPayloadSize_u16); /* SBSW_FEE_ConstPointer_refParam */
        retVal = !((Fee_ChunkInfo_getPayload(chunk_pt) == blockCfg_pt->BlockPayloadSize_u16) || /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */ /* SBSW_FEE_ChunkSearchContext */
                  (Fee_ChunkInfo_getPayload(chunk_pt) == ceiledPayloadSize)); /* PRQA S 3415 */ /*  MD_FEE_13.5_queries */ /* SBSW_FEE_ConstPointer_refParam */
    }
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryLocalJobResultEmptyChunks(void)
{
    return ( /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryLocalJobResultInvalid(void)
{
    return ((Fee_LocalJobResultType)FEE_LOCAL_JOB_RESULT_INVALID == Fee_Globals_t.FeeLocalJobResult_t); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
}
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryLocalJobResultOk(void)
{
    return ((Fee_LocalJobResultType)FEE_LOCAL_JOB_RESULT_OK == Fee_Globals_t.FeeLocalJobResult_t); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
}
 ...
 */
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryMayFollowLinks(void) {
    return (Fee_Globals_t.FsmLocalVars_t.chunkSearchContext_t.chunkHoppingForced == FALSE); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
}
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryPreviousInstanceAvailable(void)
{
    return (Fee_Globals_t.FsmLocalVars_t.instanceSearchContext_t.instDescr_pt->instance_t.index_t > 0u); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
}
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QrySearchingInCurrentSector(void)
{
    Fee_SectorConstRefType sector_ptloc = Fee_ChunkInfo_GetSector(&Fee_Globals_t.CurrentDataset_t.chunk_t); /* SBSW_FEE_Func_passParamByReference */
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryTrue(void)
{
    return (FEE_TRUE);  /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
}
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryLutIsLutEnabled(void)
{
  /* #10 Get reference to current partition and check whether lut feature is enabled*/
  Fee_PartitionConstRefType jobPartition_pt = Fee_Globals_t.CurrentProcessData.Partition_pt;
  return Fee_Lut_IsLutEnabled(jobPartition_pt); /* SBSW_FEE_ConstPointer_refParam */
}
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryLutMoreBlocksToRead(void)
{
  uint8 lutCfgIndex = Fee_Globals_t.CurrentProcessData.Partition_pt->cfg_pt->lutCfgIdx;
  uint16 numberOfLutBlocks = Fee_LookUpTableConfig[lutCfgIndex].NrBlocks;

  /* #10 We counted ascending starting with 0 so last index that should have been read is (no of lut blocks) -1*/
  /*index "points" to lut block we just read, therefore we have to check that a increment of it is still within number of lut blocks*/
  return ((Fee_Globals_t.LookUpTableSyncData_t.LookUpTableSyncBlockIndex + 1u) < numberOfLutBlocks); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
}
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryIsSectorHeaderValid(void)
{
    ...
    const Fee_PartitionInitContextConstRefType context = &Fee_Globals_t.FsmLocalVars_t.PartitionInitContext_t;
    const Fee_SectorConstRefType sectorInfo = Fee_Partition_getSectorByPos(partition, (uint8)context->recentlyCheckedSectorIndex); /* SBSW_FEE_PartitionConfig */ 
    return Fee_Sector_isUsable(sectorInfo) == FEE_TRUE; /* PRQA S 4403, 1881 */ /* MD_FEE_10.3_essentially_boolean_type, MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_Partitions_GetSectorByPosition */
}
#endif
...
{
    const Fee_PartitionInitContextConstRefType context = &Fee_Globals_t.FsmLocalVars_t.PartitionInitContext_t;
    return (context->recentlyCheckedSectorIndex == FEE_UPPER_SECTOR_NUMBER) ? FEE_FALSE : FEE_TRUE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
}
...
FEE_LOCAL_FUNC(void) Fee_InternalFillInstanceBufferWithValue(const Fee_IntBuffer_u8PtrType instBuf,uint8 value, uint32 length)
{
    ...
    for(; loopCount < length; loopCount++)
    {
        instBuf[loopCount] = value; /* SBSW_FEE_InternalDataBuffer_writeAccess */
    }
}
...
FEE_LOCAL_FUNC(void) Fee_InternalFillInstanceBufferTrailerPage(const Fee_IntBuffer_u8PtrType instBuf)
{
    instBuf[0] = (uint8)FEE_INSTANCE_STATUS_OK;                      /* SBSW_FEE_InternalDataBuffer_writeAccess */
    instBuf[1] = Fee_Globals_t.Fee_InternalErrorFlags;        /* SBSW_FEE_InternalDataBuffer_writeAccess */
    instBuf[2] = Fee_InternalGetSectorSwitchLogInformation(); /* SBSW_FEE_InternalDataBuffer_writeAccess */
    instBuf[3] = Fee_InternalGetGeneralLogInformation();        /* SBSW_FEE_InternalDataBuffer_writeAccess */
    instBuf[4] = FEE_SW_MAJOR_VERSION;                        /* SBSW_FEE_InternalDataBuffer_writeAccess */
    instBuf[5] = FEE_SW_MINOR_VERSION;                        /* SBSW_FEE_InternalDataBuffer_writeAccess */
    instBuf[6] = FEE_SW_PATCH_VERSION;                        /* SBSW_FEE_InternalDataBuffer_writeAccess */
...
FEE_LOCAL_FUNC(uint8) Fee_InternalGetSectorSwitchLogInformation(void)
{
    uint8 logInformation;
    boolean sectorSwitchOngoing = (Fee_Globals_t.FsmStack_t.Stack_at[0u] == FEE_STATE_MAIN_SECTOR_SWITCH); /* PRQA S 4404 */ /* MD_FEE_10.3_essentially_boolean_type */
    boolean isBss = ((sectorSwitchOngoing == TRUE) && (Fee_Globals_t.FeeModuleStatus_t == MEMIF_BUSY_INTERNAL));     /* PRQA S 4404 */ /* MD_FEE_10.3_essentially_boolean_type */
    boolean isBssEnabled = (Fee_Globals_t.ApiFlags.enableBss == 1u);                                       /* PRQA S 4404 */ /* MD_FEE_10.3_essentially_boolean_type */
    boolean isFssEnabled = (Fee_Globals_t.ApiFlags.enableFss == 1u);                                       /* PRQA S 4404 */ /* MD_FEE_10.3_essentially_boolean_type */   
    logInformation = sectorSwitchOngoing | (uint8)(isBss << 1u) | (uint8)(isBssEnabled << 2u) | (uint8)(isFssEnabled << 3u);
...
FEE_LOCAL_FUNC(uint8) Fee_InternalGetGeneralLogInformation(void)
{
    uint8 logInformation = (uint8)Fee_Globals_t.InternalFlags_t.brokenLink;
    logInformation |= (uint8)(Fee_Globals_t.Config_pt->Fee_ConfigurationIncomplete << 1u);
...
{
    Fee_PartitionInitContextRefType context = &Fee_Globals_t.FsmLocalVars_t.PartitionInitContext_t;
    if(context->recentlyCheckedSectorIndex == FEE_LOWER_SECTOR_NUMBER)
    {
        context->lowerSectorId = sectorId; /* SBSW_FEE_Func_writeObjectByReference */
    }
    else
    {
        context->upperSectorId = sectorId; /* SBSW_FEE_Func_writeObjectByReference */
    }
}
...
FEE_LOCAL_FUNC(Fee_InstanceStatusType) Fee_InternalEvalInstHdrStatus(Fee_InstanceSearchContextConstRefType context_pt, MemIf_JobResultType flsResult)
{
    ...
    {
        if(Fee_Sector_isWholePageErased(sector,context_pt->Buffer_au32) == FEE_FALSE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_Chunks_Sector_ref */ /* SBSW_FEE_ConstPointer_refParam */
        {
                                                ...
                                                Fee_IntBuffer_u8ConstPtrType intBuf_au8, MemIf_JobResultType flsResult)
{
    ...
    if(MEMIF_JOB_FAILED != flsResult)
    {
        ...
        else
        {
            switch(intBuf_au8[0])
            {
                case FEE_INSTANCE_STATUS_INVALID:  /* PRQA S 570 */ /* MD_FEE_10.3_instance_status_type */
                    if(Fee_InternalConfirmExplicitlyMarkedInstance(intBuf_au8, FEE_INSTANCE_STATUS_INVALID) == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */  /* SBSW_FEE_ConstPointer_refParam */
                    {
                        retVal = FEE_INSTANCE_STATUS_INVALID;
                    }
                    break;
                case FEE_INSTANCE_STATUS_OK:  /* PRQA S 570 */ /* MD_FEE_10.3_instance_status_type */
                    retVal = FEE_INSTANCE_STATUS_OK;
                    break;
                case FEE_INSTANCE_STATUS_INCONSISTENT: /* explicitly marked as INCONSISTENT */  /* PRQA S 570 */ /* MD_FEE_10.3_instance_status_type */
                   if(Fee_InternalConfirmExplicitlyMarkedInstance(intBuf_au8, FEE_INSTANCE_STATUS_INCONSISTENT) == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */  /* SBSW_FEE_ConstPointer_refParam */
                    {
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_InternalScheduleProcess(Fee_GlobalsPtrType g)
{
    Fee_Bool processStarted = FEE_FALSE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
    ...
    if((reqJob != FEE_JOB_NONE) /* PRQA S 1881 */ /* MD_FEE_10.4_compare_enums */
#if(FEE_FORCE_SECTOR_SWITCH_API == STD_ON) /* ECO_IGNORE_BLOCK_BEGIN */
            &&        (!g->InternalFlags_t.forceSectorSwitch) /* PRQA S 4558, 4116 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
#endif /* ECO_IGNORE_BLOCK_END */
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
    /* An ongoing LookUpTable synchronization shall not be interrupted by a "normal" job */
            &&        (!g->InternalFlags_t.syncLookUpTable) /* PRQA S 4558, 4116 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
#endif
      )
    {
        const uint8 jobPartIndex = (uint8)g->JobParams.partitionId;
        ...
        if((reqJob <= FEE_JOB_WRITE) && /* PRQA S 4558, 1881, 1880, 4115 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.4_compare_enums, MD_FEE_10.4_compare_enums, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
        ...
        if(processStarted == FEE_FALSE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */
        {
    ...
    {
        const Fee_PartitionRefType ssPart_pt = Fee_Partition_getFromConfigById(g->Config_pt, (uint8)ssPartIndex); /* SBSW_FEE_GlobalConfigPointer */

        processStarted = FEE_TRUE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */

        /* PRQA S 3344 1 */ /* MD_FEE_13.2_boolean */
        if(Fee_InternalInitSectorSwitchFsm(g, ssPart_pt) == FEE_FALSE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_PointerWrite_refParam */
        {
            Fee_InternalResetSectorSwitch(&Fee_Globals_t.SectorSwitchData_t, Fee_InternalIsInFblMode(&Fee_Globals_t));  /* SBSW_FEE_Func_passParamByReference */ /* SBSW_FEE_Func_passParamByReference */
...
#if (FEE_LOOKUPTABLE_MODE == STD_ON)
  /* No user job; Check if LookUpTable blocks need to be synchronized/written to flash memory */
  else if (g->InternalFlags_t.syncLookUpTable == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */
  {

    /* Start a sector switch when synchronization is marked as deferred (due to previous processing)*/
    if (g->InternalFlags_t.jobDeferred) /* PRQA S 3344 1 */ /* MD_FEE_13.2_boolean */
    {
      const Fee_PartitionRefType syncPart_pt = Fee_Partition_getFromConfigById(g->Config_pt, /* SBSW_FEE_GlobalConfigPointer */
          g->LookUpTableSyncData_t.LookUpTablePartitionIndex);
      g->LookUpTableContext_t.partitionIndex = (uint8) g->LookUpTableSyncData_t.LookUpTablePartitionIndex; /* SBSW_FEE_PointerWrite_refParam */

      /* Job has already been started before (otherwise it could have never been deferred*/
      /* Hence Partition does not have to be initialized*/
      processStarted = Fee_InternalInitSectorSwitchFsm(g, syncPart_pt); /* SBSW_FEE_PointerWrite_refParam */ /* SBSW_FEE_PartitionConfig */
    }
    else
    {
      /* Start state machine to synchronize all LookUpTable blocks of all partitions if it's not deferred */
      processStarted = Fee_Internal_LutInitSyncFsm(g); /* SBSW_FEE_PointerWrite_refParam */
      if (!processStarted) /* PRQA S 4538, 4116 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
      {
        g->InternalFlags_t.syncLookUpTable = FEE_FALSE; /* PRQA S 4424 */ /* MD_FEE_10.3_essentially_enum_type */ /* SBSW_FEE_PointerWrite_refParam */
        g->InternalFlags_t.jobDeferred = FALSE; /* SBSW_FEE_PointerWrite_refParam */
      }
    }
  }
                                                                                  ...
                                                                                  Fee_PartitionRefType part)
{
    ...
    context_pt->recentlyCheckedSectorIndex = FEE_INVALID_SECTOR_NUMBER; /* SBSW_FEE_Func_writeObjectByReference */


    g->CurrentProcessData.Partition_pt = part; /* SBSW_FEE_PointerWrite_refParam */

    Fee_FsmInitSubFsm(&g->FsmStack_t, FEE_STATE_STARTUP_READ_SECTOR_HEADER); /* SBSW_FEE_Func_passParamByReference */
...
{
    self->BlockConfig_pt = block.config;                   /* PRQA S 2916 */ /* MD_FEE_18.6_CopyingToPointerWithGreaterLifetime */ /* SBSW_FEE_PointerWrite_refParam */
...
FEE_LOCAL_FUNC(void) Fee_ActSetupJobProcessing(void) {
    ...
    /* Copy content of JobParams to CurrentProcessData which will be used in further read/write processing */
    globals->CurrentProcessData.jobParams = globals->JobParams.params.blockJob; /* SBSW_FEE_Func_writeObjectByReference */
    Fee_Initialize_ProcessData(&globals->CurrentProcessData, jobPartition_pt, globals->JobParams.params.blockJob.blockRef);/* SBSW_FEE_Func_writeObjectByReference */ /* SBSW_FEE_PartitionConfig */
    switch(globals->JobParams.requestedJob)
    {
        ...
        case FEE_JOB_GET_WRITE_CYCLE: 
            ...
            if(Fee_Partition_isReadable(jobPartition_pt) == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_InitializedProcessData */
#endif
            {
                Fee_ChunkSearchContext_Init(&globals->FsmLocalVars_t.chunkSearchContext_t, /* SBSW_FEE_Func_passParamByReference */
                                            &globals->CurrentProcessData.BlockId,          /* SBSW_FEE_Func_passParamByReference */
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_InternalInitSectorSwitchFsm(Fee_GlobalsPtrType globals, Fee_PartitionRefType partition)
{
    ...
    const Fee_Bool ssBlocked = ((fillLevel >= FEE_PART_FILL_LEVEL_FSS) && (globals->ApiFlags.enableFss == FALSE)); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
    Fee_Bool processStarted = FEE_FALSE;  /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
    Fee_Bool ssAllowed = (Fee_Partition_isWriteable(partition)) && (ssBlocked == FEE_FALSE); /* PRQA S 4403, 1881, 4558, 4115 */ /* MD_FEE_10.3_essentially_boolean_type, MD_FEE_10.4_compare_FeeBool_to_enum, MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_ConstPointer_refParam */
    ...
    globals->CurrentProcessData.Partition_pt = partition; /* PRQA S 2916 */ /* MD_FEE_18.6_CopyingToPointerWithGreaterLifetime */ /* SBSW_FEE_PointerWrite_refParam */

    if(fillLevel > FEE_PART_FILL_LEVEL_UNKNOWN) /* COV_FEE_PART_CONFIG */
    {
        if(ssAllowed == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */
        {
            ...
            if((Fee_Sector_isUsable(olderSector) == FEE_FALSE)) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_ConstPointer_refParam */
            {
                processStarted = FEE_TRUE;  /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
            ...
            else if((Fee_Partition_requiresSectorSwitch(partition) == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_ConstPointer_refParam */
    ...
    else if(globals->InternalFlags_t.forceSectorSwitch == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* COV_FEE_PART_CONFIG */
    {
...
#if (FEE_LOOKUPTABLE_MODE == STD_ON && FEE_FSS_CONTROL_API == STD_ON)
    if((globals->InternalFlags_t.syncLookUpTable == TRUE) && (!ssAllowed)) /* PRQA S 4538, 4116 */ /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
    {
      globals->InternalFlags_t.syncLookUpTable = FALSE; /* SBSW_FEE_PointerWrite_refParam */
    }
#endif
    return processStarted;
} /* PRQA S 6050, 6080 */ /* MD_MSR_STCAL */
 ...
 */
    FEE_LOCAL_FUNC(void) Fee_ActInitDCWriteProcSubFsm(void)
    {
        ...
        {
            newPayload = Fee_Proc_CeilPayloadSize(Fee_Globals_t.CurrentDataset_t.chunk_t.Sector_pt,cfg->BlockPayloadSize_u16); /* SBSW_FEE_ConstPointer_refParam */
        }
 ...
 */
    FEE_LOCAL_FUNC(void) Fee_ActInitInstanceWriteDcFsm(void)
    {
        ...
        Fee_InstanceDescriptorConstRefType instDescr_ptloc = &Fee_Globals_t.SectorSwitchTarget_t;
        uint16 ceiledPayload ;
        ...
        ceiledPayload = Fee_Proc_CeilPayloadSize(context_pt->sector_pt, instDescr_ptloc->chunk_t.PayloadSize_b16); /* SBSW_FEE_ConstPointer_refParam */
        context_pt->ByteCount_u32 = Fee_Sector_alignWrite(context_pt->sector_pt,(uint32)ceiledPayload + FEE_INSTANCE_HEADER_SIZE + /* SBSW_FEE_Func_writeObjectByReference */ /* SBSW_FEE_ConstPointer_refParam */
                                                              FEE_INSTANCE_TRAILER_SIZE );
        context_pt->numberOfCeilingBytes = (uint16)(ceiledPayload - instDescr_ptloc->chunk_t.PayloadSize_b16); /* SBSW_FEE_Func_writeObjectByReference */

        Fee_FsmInitSubFsm(&Fee_Globals_t.FsmStack_t, FEE_STATE_CREATE_INSTANCE_4_REMAINING_WRITE_STREAMS); /* SBSW_FEE_Func_passParamByReference */
 ...
 */
    FEE_LOCAL_FUNC(void) Fee_ActFinalizeDataConversion(void)
    {
        ...
        if(pIndex == 0u) /* COV_FEE_PART_CONFIG */
        {
            Fee_GlobalSuspend();
        ...
        else
        {
            Fee_Globals_t.CurrentProcessData.Partition_pt = /* SBSW_FEE_GlobalConfigPointer */
                    Fee_Partition_getFromConfigById(Fee_Globals_t.Config_pt, (pIndex - 1u)); /*  */
 ...
 */
    FEE_LOCAL_FUNC(void) Fee_InternalInitDataConversionFsm(Fee_GlobalsPtrType globals)
    {
        ...
        globals->CurrentProcessData.Partition_pt = /* SBSW_FEE_PointerWrite_refParam */
                    Fee_Partition_getFromConfigById(globals->Config_pt, (globals->Config_pt->Fee_NumberOfPartitions_u8 - 1u)); /* SBSW_FEE_GlobalConfigPointer */
...
FEE_LOCAL_FUNC(void) Fee_ActLutCheckChunkHeader(void)
{
  const Fee_ChunkSearchContextRefType context_pt = &Fee_Globals_t.LookUpTableContext_t.chunkSearchContext_t;
  Fee_Bool flsReadOk = (Fee_Globals_t.FlsJobResult_t == MEMIF_JOB_OK); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
  const Fee_ChunkInfoRefType chunk = &context_pt->mostRecentChunk_t;
  struct Fee_BlockIdStruct blockId;
  /*#10 Set default result to failed*/
  Fee_LocalJobResultType result = FEE_LOCAL_JOB_RESULT_FAILED;

  /* #20 Get necessary information from recently read chunk header */
  enum Fee_MngmtStatus headerStatus = Fee_ChunkInfo_EvaluateHeaderData(chunk,  /* SBSW_FEE_Func_passParamByReference */
                                                                       &blockId,  /* SBSW_FEE_Func_passParamByReference */
                                                                       context_pt->super_t.headerBuffer_au32,  /* SBSW_FEE_ConstPointer_refParam */
                                                                       flsReadOk);
  /* #30 Check if recently read data was a chunk header */
  if (headerStatus == FEE_MNGMT_CONSISTENT)
  {
    /* #40 Check if BlockId in chunk header matches the expected block */
    if ((Fee_Globals_t.CurrentProcessData.BlockId.DataIndex == blockId.DataIndex)
        && (Fee_Globals_t.CurrentProcessData.BlockId.Tag == blockId.Tag))
    {
      /*#41 Set result to ok*/
      result = FEE_LOCAL_JOB_RESULT_OK;
    }
  }
  Fee_Globals_t.FeeLocalJobResult_t = result;
}
...
FEE_LOCAL_FUNC(void) Fee_ActLutHandleInvalidHeader(void)
{
  /*#10 Chunk search context is expected to be initialized, do not change it or assign it*/
  /*#20 Set local job result to failed*/
  Fee_Globals_t.FeeLocalJobResult_t = FEE_LOCAL_JOB_RESULT_FAILED;
}
...
FEE_LOCAL_FUNC(void) Fee_ActLutReadChunkLink(void)
{
  /* #10 Read ChunkLink which relates to current block's LUT entry */
  Fee_ChunkSearchContextRefType context_pt = &Fee_Globals_t.LookUpTableContext_t.chunkSearchContext_t;

  Fee_InternalEvalFlsRetVal(
      Fee_ChunkInfo_ReadChunkLink(&context_pt->mostRecentChunk_t, /* SBSW_FEE_Func_passParamByReference */
                                  &context_pt->super_t.searchBlockId_t, /* SBSW_FEE_Func_passParamByReference */
                                   context_pt->super_t.headerBuffer_au32));
}
...
{
  const Fee_ChunkSearchContextRefType context_pt = &Fee_Globals_t.LookUpTableContext_t.chunkSearchContext_t;
  /* #10 Validate Chunk Link in LUT search context */
  Fee_Internal_ValidateChunkLinkInContext(context_pt) /* SBSW_FEE_Func_passParamByReference */;
}
...
FEE_LOCAL_FUNC(void) Fee_ActLutHandleInvalidLink(void)
{
  /*#10 Copy lut chunk search context into standard chunk search context*/
  Fee_ChunkSearchContextRefType stdSearchContext_pt = &Fee_Globals_t.FsmLocalVars_t.chunkSearchContext_t;
  Fee_Internal_LutCopyChunkSearchContext(stdSearchContext_pt,&Fee_Globals_t.LookUpTableContext_t); /* SBSW_FEE_Func_passParamByReference */
  /*#20 Set local job result to failed*/
  Fee_Globals_t.FeeLocalJobResult_t = FEE_LOCAL_JOB_RESULT_FAILED;
}
...
FEE_LOCAL_FUNC(void) Fee_ActLutInitialization(void)
{
  Fee_LookUpTableSyncDataRefType LookUpTableContextPtr = &Fee_Globals_t.LookUpTableSyncData_t;
  /* #10 Initialize reading of all LookUpTable blocks related to current partition */
  /* #20 Start with lut block at index 0 in ascending order for reading (writing is vice versa)*/
  LookUpTableContextPtr->LookUpTableSyncBlockIndex = 0; /* SBSW_FEE_Func_writeObjectByReference */
  /*we don't need to set partition index here. It is not needed for reading lut blocks*/

  /* #30 Perform read of first LookUpTable block */
  Fee_ActLutReadLutBlock();
}
...
{
  const Fee_GlobalsPtrType globals = &Fee_Globals_t;
  Fee_LookUpTableSyncDataConstRefType LookUpTableSyncContextPtr = &globals->LookUpTableSyncData_t;
  const Fee_PartitionRefType jobPartition_pt = globals->CurrentProcessData.Partition_pt;

  /* #10 Prepare all necessary parameters for upcoming read job */
  Fee_Internal_LutInitSyncChunkSearchContext(jobPartition_pt, LookUpTableSyncContextPtr->LookUpTableSyncBlockIndex, /* SBSW_FEE_Func_writeObjectByReference */
                                             FEE_FALSE);  /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */

  /* #20 Initiate sub state machines to find and read prepared LookUpTable block */
  /* #21 Content of LookUpTable block will be stored into corresponding RAM block */
  Fee_FsmInitSubFsm(&globals->FsmStack_t, FEE_STATE_BUSY_SEARCH_CONSISTENT); /* SBSW_FEE_Func_passParamByReference */
  Fee_FsmInitSubFsm(&globals->FsmStack_t, FEE_STATE_SEARCH_CONSISTENT_CHUNK_SEARCH_1); /* SBSW_FEE_Func_passParamByReference */
  Fee_ActInitChunkSearchFsm();
}
...
FEE_LOCAL_FUNC(void) Fee_ActLutValidateBlockContent(void)
{
  /*#10 Start validation only if a block was found at all. This means current chunk is no pseudo chunk*/
  Fee_ChunkInfoConstRefType chunk = &Fee_Globals_t.CurrentDataset_t.chunk_t;
  Fee_LookUpTableSyncDataRefType syncContext = &Fee_Globals_t.LookUpTableSyncData_t;
  if (!chunk->isPseudoChunk_bl) /* PRQA S 4558, 4116 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
  {
    Fee_PartitionConstRefType jobPartition_pt = Fee_Globals_t.CurrentProcessData.Partition_pt;
    Fee_Bool validInstanceRead = (Fee_Globals_t.FlsJobResult_t == MEMIF_JOB_OK); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */

    Fee_Lut_ValidateBlockContent(jobPartition_pt, chunk->Sector_pt, syncContext, validInstanceRead); /* SBSW_FEE_Func_writeObjectByReference */ /* SBSW_FEE_PartitionConfig */
  }
  /*#20 Increment blockIndex in syncContext*/
  syncContext->LookUpTableSyncBlockIndex++; /* SBSW_FEE_Func_writeObjectByReference */
}
...
{
  Fee_LookUpTableSyncDataRefType syncContext = &Fee_Globals_t.LookUpTableSyncData_t;
  Fee_Bool sectorSwitchNecessary = FEE_FALSE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
  const enum Fee_Partition_FillLevelEnum fillLevel = Fee_Partition_getFillLevel(Fee_Globals_t.CurrentProcessData.Partition_pt); /* SBSW_FEE_InitializedProcessData */

  /*#10 If background sector switch is necessary due to fill level and allowed, request background sector switch*/
  if ((fillLevel == FEE_PART_FILL_LEVEL_BSS) && (Fee_Globals_t.ApiFlags.enableBss == TRUE))
  {
    Fee_InternalRequestSectorSwitch(&Fee_Globals_t, Fee_Globals_t.CurrentProcessData.Partition_pt); /* SBSW_FEE_Func_passParamByReference */ /* SBSW_FEE_InitializedProcessData */
    sectorSwitchNecessary = FEE_TRUE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
  }
  /*#20 If foreground sector switch is necessary due to fill level, request foreground sector switch*/
  if(fillLevel > FEE_PART_FILL_LEVEL_BSS)
  {
    Fee_InternalRequestSectorSwitch(&Fee_Globals_t, Fee_Globals_t.CurrentProcessData.Partition_pt); /* SBSW_FEE_Func_passParamByReference */ /* SBSW_FEE_InitializedProcessData */
    sectorSwitchNecessary = FEE_TRUE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
  }

  /*#30 If a job failed, a sector switch is also necessary*/
  if (Fee_Globals_t.FeeLocalJobResult_t != FEE_LOCAL_JOB_RESULT_OK) /* PRQA S 1881 */ /* MD_FEE_10.4_compare_enums */
  {
    /*#31 Only request sector switch if fill level is normal. For other fill levels sector switch is already requested.*/
    if (fillLevel == FEE_PART_FILL_LEVEL_NORMAL)
    {
      Fee_Partition_setBlockCopyRequired(Fee_Globals_t.CurrentProcessData.Partition_pt); /* SBSW_FEE_InitializedProcessData */
      sectorSwitchNecessary = FEE_TRUE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
  ...
  /*#40 If any sector switch was requested reset sync block index in order to start over again*/
  /*#41 Otherwise reset modified flag for current lut block*/
  if (sectorSwitchNecessary == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */
  {
    uint8 lutCfgIndex = Fee_Globals_t.CurrentProcessData.Partition_pt->cfg_pt->lutCfgIdx;
    syncContext->LookUpTableSyncBlockIndex = Fee_LookUpTableConfig[lutCfgIndex].NrBlocks; /* SBSW_FEE_Func_writeObjectByReference */
  }
  else
  {
    Fee_PartitionConstRefType part_pt = Fee_Partition_getFromConfigById(Fee_Globals_t.Config_pt,syncContext->LookUpTablePartitionIndex); /* SBSW_FEE_PartitionConfig */
    Fee_Lut_ResetModifiedFlag(part_pt->cfg_pt,syncContext); /* SBSW_FEE_PartitionConfig */ /*SBSW_FEE_ConstPointer_refParam*/
  }
}
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_Internal_LutTryToStartLutSearch(Fee_SectorRefType searchSector)
{
  ...
  /*#10 Set default return value to FEE_FALSE*/
  Fee_Bool retValue = FEE_FALSE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
  /*#20 Initialize LookUpTableContext*/
  Fee_ProcessDataConstRefType processData_pt = &Fee_Globals_t.CurrentProcessData;
  Fee_BlockConfigRefType blockConfigRef = Fee_Globals_t.CurrentProcessData.BlockConfig_pt;

  Fee_Lut_InitContextStruct(&Fee_Globals_t.LookUpTableContext_t, processData_pt->Partition_pt, processData_pt, blockConfigRef); /* SBSW_FEE_Func_passParamByReference */

  /*#30 Continue only if lut is allowed to be used*/
  if (Fee_Lut_IsAccessAllowed(&Fee_Globals_t.LookUpTableContext_t) == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_Func_passParamByReference */
  {
    /*#40 Get lut entry and its sector*/
    chunkHeaderAddress = Fee_Lut_GetChunkAddressFromLut(&Fee_Globals_t.LookUpTableContext_t); /* SBSW_FEE_Func_passParamByReference */

    /*#50 If lut entry is not reliable, reset it*/
    if (Fee_Lut_IsChunkAddressPlausible(&Fee_Globals_t.LookUpTableContext_t, chunkHeaderAddress) == FEE_FALSE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_Func_passParamByReference */
    {
      Fee_Lut_ResetLut(&Fee_Globals_t.LookUpTableContext_t); /* SBSW_FEE_Func_writeObjectByReference */
    }
    /*#51 Otherwise, if entry is reliable, check if its sector matches sector FEE is currently searching in*/
    else
    {
      /*#52 If sector matches, set return value to true, init lut chunk search context and start lut chunk search fsm*/
      if(Fee_Sector_containsAbsAddr(searchSector,chunkHeaderAddress) == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_ConstPointer_refParam */
      {
        Fee_Internal_LutInitChunkSearchContext(&Fee_Globals_t.LookUpTableContext_t,searchSector,chunkHeaderAddress); /* SBSW_FEE_Func_writeObjectByReference */ /*SBSW_FEE_ConstPointer_refParam*/
        Fee_FsmInitSubFsm(&Fee_Globals_t.FsmStack_t, FEE_STATE_LUT_CHUNK_SEARCH); /* SBSW_FEE_Func_passParamByReference */
        Fee_FsmInitSubFsm(&Fee_Globals_t.FsmStack_t, FEE_STATE_LUT_CHUNK_SEARCH_READ_CHUNK_HEADER); /* SBSW_FEE_Func_passParamByReference */
        Fee_InternalEvalFlsRetVal(Fee_ChunkInfo_ReadChunkHeader(&Fee_Globals_t.LookUpTableContext_t.chunkSearchContext_t.mostRecentChunk_t, /* SBSW_FEE_Func_passParamByReference */
            Fee_Globals_t.LookUpTableContext_t.chunkSearchContext_t.super_t.headerBuffer_au32)); /* SBSW_FEE_Func_passParamByReference */
        retValue = FEE_TRUE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
      }
...
FEE_LOCAL_FUNC(void) Fee_Internal_LutCopyChunkSearchContext(Fee_ChunkSearchContextRefType target,Fee_LookUpTableContextConstRefType self)
{
  Fee_ChunkSearchContextConstRefType lutSearchContext_pt = &self->chunkSearchContext_t;

  /* #15 Store latest information from mostRecentChunk to InstanceDescriptorStruct recentChunk_pt,
   * because this information is stored to standard search context in next step */
  Fee_ChunkInfo_Clone(&lutSearchContext_pt->recentChunk_pt->chunk_t, &lutSearchContext_pt->mostRecentChunk_t); /* SBSW_FEE_LutChunkSearchContext */

  /* #20 Initialize target search context with latest information from self for current block*/
  Fee_ChunkSearchContext_Init(target, &lutSearchContext_pt->super_t.searchBlockId_t, /* SBSW_FEE_Func_passParamByReference */
                               lutSearchContext_pt->prevChunk_pt,  /* SBSW_FEE_Func_passParamByReference */
                               lutSearchContext_pt->recentChunk_pt, lutSearchContext_pt->super_t.upperSearchLimit_t); /* SBSW_FEE_Func_passParamByReference */
}
...
FEE_LOCAL_FUNC(void) Fee_Internal_LutInitChunkSearchContext(Fee_LookUpTableContextRefType lutContext,
    Fee_SectorRefType searchSector,Fee_LengthType absoluteChunkAddress)
{
  Fee_ChunkSearchContextRefType searchContext_pt = &lutContext->chunkSearchContext_t;
  Fee_GlobalsPtrType globals = &Fee_Globals_t;

  /*#10 Init chunk search context*/
  Fee_ChunkSearchContext_Init(searchContext_pt, &globals->CurrentProcessData.BlockId, /* SBSW_FEE_Func_writeObjectByReference */
                              &globals->PreviousChunk_t, &globals->CurrentDataset_t, Fee_Proc_getSectorEndOffset(searchSector)); /* SBSW_FEE_Func_passParamByReference */

  /*#20 Init context's chunk info with entry's data*/
  searchContext_pt->mostRecentChunk_t.isPseudoChunk_bl = FALSE; /* SBSW_FEE_LutChunkSearchContext */

  searchContext_pt->mostRecentChunk_t.ChunkBaseOffset_t = absoluteChunkAddress - searchSector->cfg_pt->logicalRange_t.baseAddress_t; /* SBSW_FEE_LutChunkSearchContext */
  searchContext_pt->mostRecentChunk_t.Sector_pt = searchSector; /* SBSW_FEE_LutChunkSearchContext */
}
...
{
  const Fee_GlobalsPtrType globals = &Fee_Globals_t;
  const Fee_SectorRefType newSector = Fee_Partition_getNewestSector(jobPartition_pt); /* SBSW_FEE_ConstPointer_refParam */

  /* #10 Prepare job parameters */
  struct Fee_tagBlockSpecificJob job; /* PRQA S 0759 */ /* MD_MSR_Union */
  Fee_Lut_PrepareProcessParameter(jobPartition_pt, blockIndex, isWriteJob, &job);/* SBSW_FEE_PartitionConfig  */ /* SBSW_FEE_Func_writeObjectByReference */

  /* #20 Set current process data and chunk context for search and read/write */
  globals->CurrentProcessData.jobParams = job; /* SBSW_FEE_Func_writeObjectByReference */
  Fee_Initialize_ProcessData(&globals->CurrentProcessData, jobPartition_pt, job.blockRef); /* SBSW_FEE_PointerWrite_refParam */
  Fee_ChunkInfo_Clear(&globals->CurrentDataset_t.chunk_t);  /* SBSW_FEE_Func_passParamByReference */
  Fee_ChunkInfo_Clear(&globals->PreviousChunk_t); /* SBSW_FEE_Func_passParamByReference */
  Fee_ChunkInfo_Construct(&Fee_Globals_t.CurrentDataset_t.chunk_t, newSector);  /* SBSW_FEE_Func_passParamByReference */

  /* #30 Initialize chunk search context, which is used in both jobs, read and write */
  Fee_ChunkSearchContext_Init(&globals->FsmLocalVars_t.chunkSearchContext_t, /* SBSW_FEE_Func_passParamByReference */
                              &globals->CurrentProcessData.BlockId,         /* SBSW_FEE_Func_passParamByReference */
                              &globals->PreviousChunk_t,                    /* SBSW_FEE_Func_passParamByReference */
                              &globals->CurrentDataset_t,                   /* SBSW_FEE_Func_passParamByReference */
                              Fee_Proc_getSectorEndOffset(newSector));      /* SBSW_FEE_ConstPointer_refParam*/
}
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_Internal_LutInitSynchronization(Fee_GlobalsPtrType globals)
{
  ...
  Fee_LookUpTableSyncDataRefType syncContext = &globals->LookUpTableSyncData_t;
  /*#10 Prepare sync context set partition index to number of partitions*/
  syncContext->LookUpTablePartitionIndex = globals->Config_pt->Fee_NumberOfPartitions_u8; /* SBSW_FEE_Func_writeObjectByReference */

  /*#20 Compute next partition index which might need a sync by using sync context*/
  syncContext->LookUpTablePartitionIndex = Fee_Lut_GetNextSyncPartitionIndex(syncContext,globals->Config_pt); /* SBSW_FEE_Func_writeObjectByReference */ /* SBSW_FEE_Func_passParamByReference */ /* SBSW_FEE_ConstPointer_refParam*/

  /*#30 If there is nothing to sync, return false*/
  if(syncContext->LookUpTablePartitionIndex == FEE_LUT_PARTITION_INVALID_INDEX)
  {
    syncContext->LookUpTablePartitionIndex = FEE_LUT_PARTITION_INVALID_INDEX; /* SBSW_FEE_Func_writeObjectByReference */
    syncContext->LookUpTableSyncBlockIndex = FEE_LUT_BLOCK_INVALID_INDEX; /* SBSW_FEE_Func_writeObjectByReference */
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_Internal_LutInitSyncFsm(Fee_GlobalsPtrType globals)
{
  ...
  /*#30 If there is nothing to sync, return false*/
  if (Fee_Lut_PrepareSyncContextForNextBlock(syncContext, globals->Config_pt) == FEE_FALSE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_PointerWrite_refParam */ /* SBSW_FEE_GlobalConfigPointer */
  {
    retValue = FEE_FALSE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
  }
  else
  {
    /*#40 Otherwise syncContext holds valid information for the sync process. Start the corresponding FSM*/
    Fee_FsmSetupTopLevel(&globals->FsmStack_t, FEE_STATE_MAIN_SYNC_LOOKUPTABLE); /* SBSW_FEE_Func_passParamByReference */

    /*#50 Init chunk search context and job parameters*/
    Fee_Internal_LutInitSyncChunkSearchContext(
        Fee_Partition_getFromConfigById(globals->Config_pt, syncContext->LookUpTablePartitionIndex), /* SBSW_FEE_GlobalConfigPointer */ /* SBSW_FEE_Func_passParamByReference */
        syncContext->LookUpTableSyncBlockIndex, FEE_TRUE); /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */ /* SBSW_FEE_Func_passParamByReference */

    /*#60 Start writing the prepared block */
    Fee_FsmInitSubFsm(&globals->FsmStack_t, FEE_STATE_BUSY_SEARCH_WRITTEN); /* SBSW_FEE_Func_passParamByReference */
    Fee_ActInitSearchWrittenFsm();
  }
...
{
#if (FEE_LOOKUPTABLE_MODE == STD_ON)
  /*#10 Update entry in lut if lut configured, use current data set*/
  Fee_InstanceDescriptorConstRefType instDescr_ptloc = &Fee_Globals_t.CurrentDataset_t;
  Fee_LookUpTableContextRefType lutContext_pt = &Fee_Globals_t.LookUpTableContext_t;

  Fee_Lut_UpdateLut(lutContext_pt, &instDescr_ptloc->chunk_t);  /* SBSW_FEE_Func_passParamByReference */
#endif
  /*#20 Set local job result to ok */
  Fee_Globals_t.FeeLocalJobResult_t = FEE_LOCAL_JOB_RESULT_OK;
}
...
{
#if (FEE_LOOKUPTABLE_MODE == STD_ON)
  /*#10 Reset lut entry, if lut is configured*/
  Fee_Lut_ResetLut(&Fee_Globals_t.LookUpTableContext_t);  /* SBSW_FEE_Func_passParamByReference */
#endif
  /*#20 Set local job result to failed*/
  Fee_Globals_t.FeeLocalJobResult_t = FEE_LOCAL_JOB_RESULT_FAILED;
...
{
#if (FEE_LOOKUPTABLE_MODE == STD_ON)
  /*#10 Update entry in lut if lut is configured, use instance copy context*/
  Fee_ChunkInfoConstRefType chunk = &Fee_Globals_t.FsmLocalVars_t.instanceCopyContext_t.targetChunk_t;
  Fee_LookUpTableContextRefType lutContext_pt = &Fee_Globals_t.LookUpTableContext_t;

  Fee_Lut_UpdateLut(lutContext_pt, chunk);  /* SBSW_FEE_Func_passParamByReference */
#endif
  /*#20 Set local job result to ok */
  Fee_Globals_t.FeeLocalJobResult_t = FEE_LOCAL_JOB_RESULT_OK;
}
...
#endif
  Fee_Globals_t.FeeLocalJobResult_t = FEE_LOCAL_JOB_RESULT_FAILED;
  if(Fee_Globals_t.FsmLocalVars_t.instanceCopyContext_t.currentStep == FEE_SECTOR_SWITCH_STEP_BLOCK_COPY_READ)
  {
      Fee_InternalLogError(FEE_INTERNAL_ERROR_BLOCK_COPY_READ_FAILED);
  }
  else
  {
      Fee_InternalLogError(FEE_INTERNAL_ERROR_BLOCK_COPY_WRITE_FAILED);
  }
}
...
{
#if (FEE_LOOKUPTABLE_MODE == STD_ON)
  /*#10 Check if chunk in current data set is not a pseudo chunk. Pseudo chunk means that nothing was found and therefore nothing
   * shall be updated*/
  Fee_LookUpTableContextRefType lutContext_pt= &Fee_Globals_t.LookUpTableContext_t;
  Fee_ChunkInfoConstRefType chunk = &Fee_Globals_t.CurrentDataset_t.chunk_t;
  if (!chunk->isPseudoChunk_bl) /* PRQA S 4558, 4116 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
  {
    /*#20 Update lut entry, use current dataset*/
    Fee_Lut_UpdateLut(lutContext_pt, chunk); /* SBSW_FEE_Func_passParamByReference */
  }
  else
  {
    /*#21 Otherwise reset lut entry*/
    Fee_Lut_ResetLut(lutContext_pt);  /* SBSW_FEE_Func_passParamByReference */
  }
#endif
}
                                                                 ...
                                                                 Fee_LengthType upperLimit_t)
{
    ...
    self->super_t.searchBlockId_t = *blockId_pt; /* SBSW_FEE_PointerWrite_refParam */
    
    self->super_t.blockIdMask_t.Tag = 0xFFFFu; /* SBSW_FEE_PointerWrite_refParam */
    self->super_t.blockIdMask_t.DataIndex = 0xFFu; /* SBSW_FEE_PointerWrite_refParam */
    
    self->super_t.resultBlockId_pt = NULL_PTR; /* SBSW_FEE_PointerWrite_refParam */

    self->super_t.upperSearchLimit_t = upperLimit_t; /* SBSW_FEE_PointerWrite_refParam */

    self->recentChunk_pt = recentChunk_pt; /* SBSW_FEE_PointerWrite_refParam */
    self->prevChunk_pt = prevChunk_pt; /* SBSW_FEE_PointerWrite_refParam */
    self->chunkLinkPresent = 0u; /* SBSW_FEE_PointerWrite_refParam */
    self->chunkHoppingForced = 0u; /* SBSW_FEE_PointerWrite_refParam */

    Fee_ChunkInfo_Clone(&self->mostRecentChunk_t, &recentChunk_pt->chunk_t); /* SBSW_FEE_Func_passParamByReference */
}
...
{
#  if (FEE_DATA_CONVERSION_API == STD_ON)
    return ((g->Config_pt->Fee_ConfigurationIncomplete == FEE_TRUE) || (g->ApiFlags.forceFblMode == FEE_TRUE)); /* PRQA S 4403, 1881 */ /* MD_FEE_10.3_essentially_boolean_type, MD_FEE_10.4_compare_FeeBool_to_enum */
#  else
    return (g->Config_pt->Fee_ConfigurationIncomplete == FEE_TRUE); /* PRQA S 4403, 1881 */ /* MD_FEE_10.3_essentially_boolean_type, MD_FEE_10.4_compare_FeeBool_to_enum */
#  endif
}
...
FEE_INLINE_FUNC(Fee_Bool) Fee_Proc_isNewestSector(Fee_PartitionConstRefType part, Fee_SectorConstRefType sector)
{
    return (Fee_Partition_getNewestSector(part) == sector); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */ /* SBSW_FEE_PointerWrite_refParam */
}


/**********************************************************************************************************************
 * Fee_Proc_isPayloadSizeCeiled
 *********************************************************************************************************************/
/*!
 * Internal comment removed.
 *
...
FEE_INLINE_FUNC(Fee_Bool) Fee_Proc_isPayloadSizeCeiled(Fee_ChunkInfoConstRefType chunk, Fee_BlockConfigRefType config_pt)
{
    uint16 ceiledPayloadSize = Fee_Proc_CeilPayloadSize(chunk->Sector_pt,config_pt->BlockPayloadSize_u16); /* SBSW_FEE_ConstPointer_refParam */
    return (Fee_ChunkInfo_getPayload(chunk) == ceiledPayloadSize); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */ /* SBSW_FEE_PointerWrite_refParam */
}
...
FEE_INLINE_FUNC(Fee_Bool) Fee_Proc_isLinkTableChunkAllowed(Fee_ChunkInfoConstRefType chunk)
{
    /* PRQA S 3344,3415,4403 1 */ /* MD_FEE_13.2_boolean, MD_FEE_13.5_queries, MD_FEE_10.3_essentially_boolean_type */
    return ((Fee_ChunkInfo_isInitialLinkTable(chunk)) /* SBSW_FEE_PointerWrite_refParam */
            && (Fee_ChunkInfo_isLinkAllowed(chunk))); /* PRQA S 4558, 3415, 4115 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_13.5_queries, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_PointerWrite_refParam */
}
...
FEE_LOCAL_FUNC(void) Fee_InternalLogError(Fee_InternalErrorType error)
{
    Fee_Globals_t.Fee_InternalErrorFlags |= error;
}
...
{
    uint8_least index;
    Fee_Bool retVal = FEE_TRUE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
    /* Confirmation is needed when the instance header value points to an invalid or inconsistent instance status.
     * - When reading out the instance header it is ensured that at least 4 Bytes were read out into the buffer.
     * Theoretically (but very unlikely) the instance trailer value can also point to an invalid or inconsistent instance status.
     * - The confirmation result is not relevant here as for the trailer part it is only evaluated if trailer is the same as header (therefore OK). */
    for(index = 0; index < 4u; index++)
    {
        if(intBuf_au8[index] != (uint8)pretendedStatus)
        {
            retVal = FEE_FALSE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
        }
    }
    return retVal;
}
...
{
    const Fee_SectorFormatContextRefType context_pt = &Fee_Globals_t.FsmLocalVars_t.SectorFormatContext_t;
    context_pt->sector_pt = Fee_Partition_getOldestSector(Fee_Globals_t.CurrentProcessData.Partition_pt); /* SBSW_FEE_Func_writeObjectByReference */  /* SBSW_FEE_InitializedProcessData */
    /*If the sector is not usable, the link table size is not set, update it with the configured value*/
    if(context_pt->sector_pt->linkTableSize_t == 0u)
    {
        context_pt->sector_pt->linkTableSize_t = Fee_Partition_getConfiguredLinktableSize(Fee_Globals_t.CurrentProcessData.Partition_pt); /* SBSW_FEE_Partitions_OldestNewestSectorRef */ /* SBSW_FEE_InitializedProcessData */
    }
    context_pt->newLinkTableSize = Fee_InternalComputeNewLinkTableSize(); /* SBSW_FEE_Func_writeObjectByReference */
    context_pt->sectorId_u8 = Fee_Partition_getNextSectorId(Fee_Globals_t.CurrentProcessData.Partition_pt); /* SBSW_FEE_Func_writeObjectByReference */ /* SBSW_FEE_InitializedProcessData */
}
...
FEE_LOCAL_FUNC(uint16) Fee_InternalComputeNewLinkTableSize(void)
{
    const Fee_GlobalsConstPtrType globals = &Fee_Globals_t;
    Fee_SectorConstRefType newSector = Fee_Partition_getNewestSector(globals->CurrentProcessData.Partition_pt); /* SBSW_FEE_InitializedProcessData */
    uint16 configuredLinkTableSize = Fee_Partition_getConfiguredLinktableSize(globals->CurrentProcessData.Partition_pt);   /* SBSW_FEE_InitializedProcessData */
    uint16 resultLinkTableSize = configuredLinkTableSize;
    /*Motivation: The bootloader shall not shrink the link table size (it does not know application blocks). Therefore, it shall
      fallback to the size it found in flash.*/
    if(Fee_InternalIsInFblMode(globals)) /* PRQA S 3344 */ /* MD_FEE_13.2_boolean */ /* SBSW_FEE_Func_passParamByReference */
    {
        const uint16 actualLinkTableSize = Fee_Sector_currentLinktableEntryCount(newSector); /* SBSW_FEE_Partitions_OldestNewestSectorRef */
        if(configuredLinkTableSize < actualLinkTableSize)
        {
            resultLinkTableSize = actualLinkTableSize;
        }
    }
    return resultLinkTableSize;
}
