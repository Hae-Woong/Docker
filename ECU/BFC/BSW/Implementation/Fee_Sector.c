...
#define FEE_ERASE_START_MARKER_WORD                                   (0xAAAAAAAAu)
...
/**********************************************************************************************************************
 * Fee_Sector_getOffsetOfEraseStartMarker
 *********************************************************************************************************************/
/*!
 * \brief      Computes the offset of the erase start marker within the sector.
 * \details    Address of the erase start marker ist one page after the end of the link table.
 * \config    FEE_USE_RELIABLE_ERASE_PROCEDURE is enabled
 * \param[in]  self: Sector, for which the offset shall be computed.
 * \return     offset of the erase start marker within the sector.
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(Fee_LengthType) Fee_Sector_getOffsetOfEraseStartMarker(Fee_SectorConstRefType self);

/**********************************************************************************************************************
 * Fee_Sector_copyEraseStartMarkerToBuffer
 *********************************************************************************************************************/
/*!
 * \brief      Copies the erase start marker to the given buffer. Its length is one page.
 * \details    -
 * \config    FEE_USE_RELIABLE_ERASE_PROCEDURE is enabled
 * \param[in]  self: target sector
 * \param[in,out]  targetBuffer buffer that contains the erase start marker
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_Sector_copyEraseStartMarkerToBuffer(Fee_SectorConstRefType self, Fee_IntBuffer_u32PtrType target);
#endif
...
FEE_LOCAL_FUNC(Fee_LengthType) Fee_Sector_getOffsetOfEraseStartMarker(Fee_SectorConstRefType self)
{
    Fee_LengthType eraseMarkerOffset = Fee_Sector_calcFirstFreeAddress(self); /* SBSW_FEE_ConstPointer_refParam */
    /* Validator assures that the chunk header has exactly two pages, otherwise there would be no space for the erase start marker*/
    eraseMarkerOffset+= Fee_Sector_alignWrite(self, 1u); /* SBSW_FEE_ConstPointer_refParam */
    return eraseMarkerOffset;
}
...
FEE_LOCAL_FUNC(void) Fee_Sector_copyEraseStartMarkerToBuffer(Fee_SectorConstRefType self,Fee_IntBuffer_u32PtrType target)
{
    /* Since the page size is forced to be a multiple of 4, it is assured that we do not truncate here*/
    Fee_LengthType numberOfWords = (Fee_LengthType) (Fee_Sector_alignWrite(self, 1u) / 4u); /* SBSW_FEE_ConstPointer_refParam */
    Fee_LengthType currentWord;
    for(currentWord = 0u; currentWord < numberOfWords; currentWord++)
    {
        target[currentWord] = FEE_ERASE_START_MARKER_WORD; /* SBSW_FEE_SectorFormatBuffer */
    }
}
#endif
    ...
    Fee_Bool ReadOk)
{
...
#endif
    /* ESCAN00111054: Assume here that the erase start marker is written. Only if we have checked it later and proved that it is not written
        we set it back to not written. */
    self->eraseStartMarkerWritten = TRUE;  /* SBSW_FEE_PointerWrite_refParam */
    ...
    if (ReadOk)
    {
        ...
        else if(((sectorId != 0xFFu) && (sectorId != 0u) && ((*intBuf & reservedMask) == 0u))
#if (FEE_EXTENDED_SECTOR_HEADER_CHECK == STD_ON)
        && (intBuf[0] == (~(intBuf[FEE_ERASE_CYCLE_OFFSET_ADDRESS])))
#endif
        )
        {
            ...
            #if(CPU_BYTE_ORDER == LOW_BYTE_FIRST)  /* ECO_DISABLE_GENERATION : only variable definitions */
                (uint16)(((*intBuf >> 4) & 0xFF0uL) | ((*intBuf >> 16) & 0x0FuL));
            #else  /* ECO_DISABLE_GENERATION : only variable definitions */
                (uint16)(((*intBuf >> 12) & 0xFF0u) | ((*intBuf >> 8) & 0x0FuL));
            #endif
                                                              ...
                                                              uint16 linkTableSize)
{
    ...
    firstWord |= ((uint32)linkTableSize & 0x0FuL) << 16;
#else
    firstWord = (uint32)sectorId << 24;
    firstWord |= ((uint32)linkTableSize & 0x0FF0u) << 12;
    firstWord |= ((uint32)linkTableSize & 0x0Fu) << 8;
#endif
...
#else 
   headerBuffer_au32[FEE_ERASE_CYCLE_OFFSET_ADDRESS] = ~(firstWord); /* SBSW_FEE_InternalDataBuffer_writeAccess */
#endif
}
...
/**********************************************************************************************************************
 * Fee_Sector_readDataWithBlankCheck
 *********************************************************************************************************************/
/*!
 * Internal comment removed.
 *
 *
 */
FEE_INTERN_FUNC(enum Fee_FlsRequestStatus) Fee_Sector_readDataWithBlankCheck(Fee_SectorConstRefType self,
                                                                      Fee_LengthType offset,
                                                                      Fee_IntBuffer_u8PtrType data,
                                                                      Fee_LengthType byteCount)
{
    return (Fee_FlsReadBlank(self->cfg_pt->logicalRange_t.baseAddress_t + offset, data, byteCount) == E_OK) /* SBSW_FEE_PointerWrite_refParam */
                        ? FEE_REQ_ACCEPTED : FEE_REQ_REJECTED;
}

/**********************************************************************************************************************
 * Fee_Sector_readDataWithoutBlankCheck
 *********************************************************************************************************************/
/*!
 * Internal comment removed.
 *
 *
 */
FEE_INTERN_FUNC(enum Fee_FlsRequestStatus) Fee_Sector_readDataWithoutBlankCheck(Fee_SectorConstRefType self,
                                                                      Fee_LengthType offset,
                                                                      Fee_IntBuffer_u8PtrType data,
                                                                      Fee_LengthType byteCount)
{
                                                                              ...
                                                                              Fee_LengthType byteCount)
{
    ...
    return Fee_Sector_readDataWithBlankCheck(self, offset, (Fee_UserDataPtrType)data, byteCount); /* SBSW_FEE_PointerWrite_refParam */
}
...
{
    return (!!self->isEmpty) ? 0uL : self->NextFreeAddress_t; /* PRQA S 4558, 4116 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
}
...
{
    return !((self->isEmpty) && (self->isFull)); /* PRQA S 4403, 4558, 4115 */ /* MD_FEE_10.3_essentially_boolean_type, MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
}
...
{
    return (Fee_Sector_isUsable(self) == FEE_FALSE) /* PRQA S 4403, 1881 */ /* MD_FEE_10.3_essentially_boolean_type, MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_ConstPointer_refParam */
            && (!self->isNfaConfirmed_bl); /* PRQA S 4558, 4116 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
}
...
FEE_INTERN_FUNC(Fee_Bool) Fee_Sector_isFillLevelKnown(Fee_SectorConstRefType self)
{
    ...
    return ((Fee_Sector_isUsable(self) == FEE_FALSE) /* PRQA S 4403, 1881 */ /* MD_FEE_10.3_essentially_boolean_type, MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_ConstPointer_refParam */
            || (self->isNfaConfirmed_bl)); /* PRQA S 4558, 4115 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
}
...
FEE_INLINE_FUNC(Fee_Bool) Fee_Sector_isWriteable(Fee_SectorConstRefType self)
{
    return (Fee_Sector_isUsable(self)) /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */ /* SBSW_FEE_ConstPointer_refParam */
            && (!self->isFull); /* PRQA S 4558, 4115, 4116 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
}
...
FEE_INTERN_FUNC(Fee_Bool) Fee_Sector_containsAbsAddr(Fee_SectorConstRefType self, Fee_SectorAddressType addr)
{
    ...
    return ((base <= addr) && (addr < end)); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */
}


#if(FEE_LOOKUPTABLE_MODE == STD_ON)
/**********************************************************************************************************************
 * Fee_Sector_isValidLutAddress
 *********************************************************************************************************************/
/*!
 * Internal comment removed.
 *
 *
 */
FEE_INTERN_FUNC(Fee_Bool) Fee_Sector_isValidLutAddress(Fee_SectorConstRefType self, Fee_SectorAddressType addr)
{
  const Fee_SectorAddressType base = self->cfg_pt->logicalRange_t.baseAddress_t;
  const Fee_SectorAddressType end = base + Fee_Sector_getSize(self); /* SBSW_FEE_ConstPointer_refParam */
  return (((base + Fee_Sector_calcFirstFreeAddress(self)) <= addr) && (addr < end)); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */ /* SBSW_FEE_ConstPointer_refParam */
}
#endif
...
FEE_INTERN_FUNC(void) Fee_Sector_markFull(Fee_SectorRefType self)
{
    ...
    s.isEmpty = FEE_FALSE; /* PRQA S 4424 */ /* MD_FEE_10.3_essentially_enum_type */
    s.isFull = FEE_TRUE; /* PRQA S 4424 */ /* MD_FEE_10.3_essentially_enum_type */
 ...
 *
 */
FEE_INTERN_FUNC(uint32) Fee_Sector_alignTo(uint32 val, uint8 alignBits)
{
    uint32 temp = val;
    temp += ~Fee_Sector_getAlignmentMask(alignBits);
    return (temp  & Fee_Sector_getAlignmentMask(alignBits));
}

/**********************************************************************************************************************
 * Fee_Sector_isWholePageErased
 *********************************************************************************************************************/
/*!
 * Internal comment removed.
 *
 ...
 *
 */
FEE_INTERN_FUNC(Fee_Bool) Fee_Sector_isWholePageErased(Fee_SectorConstRefType self, Fee_IntBuffer_u32ConstPtrType pageToCheck)
{
    Fee_Bool retVal = FEE_TRUE; /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
    /*We need a zero based index to navigate through an array with 32 bit entries. Divide by 4 to get it*/
    uint32 u_index = ((Fee_Sector_alignWrite(self, 1u) - 1u) >> 2u); /* SBSW_FEE_Chunks_Sector_ref */
    /*Avoid misra warnings: execute bit operation with unsigned and cast it to signed. Value fits into sint32 */
    sint32 index = (sint32)u_index;
    uint32 eraseValue = Fee_Sector_getEraseValueU32(self); /* SBSW_FEE_Chunks_Sector_ref */
    do
    {
        if(pageToCheck[index] != eraseValue) 
        {
...
#if (FEE_USE_RELIABLE_ERASE_PROCEDURE == STD_ON)
/**********************************************************************************************************************
 * Fee_Sector_writeEraseStartMarker
 *********************************************************************************************************************/
/*!
 * Internal comment removed.
 *
 ...
 *
 */
FEE_INTERN_FUNC(enum Fee_FlsRequestStatus) Fee_Sector_writeEraseStartMarker(Fee_SectorRefType self,
                                                                            Fee_IntBuffer_u32PtrType bufferForRequest)
{
    Fee_LengthType eraseStartMarkerOffset = Fee_Sector_getOffsetOfEraseStartMarker(self); /* SBSW_FEE_ConstPointer_refParam */
    Fee_LengthType eraseStartMarkerLength = Fee_Sector_alignWrite(self, 1u); /* SBSW_FEE_ConstPointer_refParam */
    Fee_Sector_copyEraseStartMarkerToBuffer(self,bufferForRequest); /* SBSW_FEE_SectorFormatBuffer */
    self->eraseStartMarkerWritten = TRUE; /* SBSW_FEE_PointerWrite_refParam */
    return Fee_Sector_writeMngmtData(self,eraseStartMarkerOffset,bufferForRequest,eraseStartMarkerLength); /* SBSW_FEE_ConstPointer_refParam */
}


/**********************************************************************************************************************
 * Fee_Sector_readEraseStartMarker
 *********************************************************************************************************************/
/*!
 * Internal comment removed.
 *
 ...
 *
 */
FEE_INTERN_FUNC(enum Fee_FlsRequestStatus) Fee_Sector_readEraseStartMarker(Fee_SectorConstRefType self,
                                                                            Fee_IntBuffer_u32PtrType bufferForRequest)
{
    Fee_LengthType eraseStartMarkerOffset = Fee_Sector_getOffsetOfEraseStartMarker(self); /* SBSW_FEE_ConstPointer_refParam */
    Fee_LengthType eraseStartMarkerLength = Fee_Sector_alignWrite(self, 1u); /* SBSW_FEE_ConstPointer_refParam */
    return Fee_Sector_readMngmtData(self,eraseStartMarkerOffset,bufferForRequest,eraseStartMarkerLength); /* SBSW_FEE_ConstPointer_refParam */ /* SBSW_FEE_SectorFormatBuffer */
}


/**********************************************************************************************************************
 * Fee_Sector_evaluateEraseStartMarker
 *********************************************************************************************************************/
/*!
 * Internal comment removed.
 *
 ...
 *
 */
FEE_INTERN_FUNC(void) Fee_Sector_evaluateEraseStartMarker(Fee_SectorRefType self,
                                                                           Fee_IntBuffer_u32ConstPtrType buffer,
                                                                           Fee_Bool readOk)
{
    Fee_Bool eraseStartMarkerErased = Fee_Sector_isWholePageErased(self,buffer); /* SBSW_FEE_ConstPointer_refParam */
    self->eraseStartMarkerWritten = FALSE; /* SBSW_FEE_PointerWrite_refParam */
    if((readOk == FEE_FALSE) ||(eraseStartMarkerErased == FEE_FALSE)) /* PRQA S 1881 */ /*  MD_FEE_10.4_compare_FeeBool_to_enum */ 
    {
        Fee_Sector_free(self); /* SBSW_FEE_PointerWrite_refParam */
        self->eraseStartMarkerWritten = TRUE; /* SBSW_FEE_PointerWrite_refParam */
    }
}
#endif
