 ...
 *   1. Main FSM
 *   2. SectorSwitch FSM
 *   3. Force_SS_Part_Init
 *   4. StartUp_Read_Lut_Block
 *   5. SearchConsistent FSM
 *   6. ChunkSearch FSM
 *   7. ChunkHopping FSM
 */
#  define FEE_FSM_STACK_SIZE                                           (7u)
#else
/* Define the depth of the FSM stack. 
 * Determined from design: largest depth is reached when the Fee performs ChunkHopping during a read/write job:
 *   1. Main FSM
 ...
 */
#  define FEE_FSM_STACK_SIZE                                           (6u)
# endif /*FEE_FORCE_SECTOR_SWITCH_API == STD_ON*/
...
#define STATETRACER_NO_QUERY 255
#define STATETRACER_NO_ACTION 255
...
typedef enum
{
    FEE_SECTOR_SWITCH_STEP_BLOCK_COPY_READ,
    FEE_SECTOR_SWITCH_STEP_BLOCK_COPY_WRITE
} Fee_SectorSwitchStepType;
...
typedef P2FUNC(void, FEE_PRIVATE_CODE, Fee_DeferredFuncType) (Fee_GlobalsPtrType g);
...
typedef P2CONST(struct Fee_ChunkHoppingContextStruct, AUTOMATIC, FEE_VAR_NOINIT) Fee_ChunkHoppingContextConstRefType;
...
typedef P2CONST(struct Fee_InstanceWriteContextStruct, AUTOMATIC, FEE_VAR_NOINIT) Fee_InstanceWriteContextConstRefType;
...
typedef P2CONST(struct Fee_ChunkSearchContextStruct, AUTOMATIC, FEE_VAR_NOINIT) Fee_ChunkSearchContextConstRefType;
typedef P2VAR(struct Fee_ChunkAllocContextStruct, AUTOMATIC, FEE_VAR_NOINIT) Fee_ChunkAllocContextRefType;
typedef P2CONST(struct Fee_ChunkAllocContextStruct, AUTOMATIC, FEE_VAR_NOINIT) Fee_ChunkAllocContextConstRefType;
typedef P2VAR(struct Fee_PartitionInitContextStruct, AUTOMATIC, FEE_VAR_NOINIT) Fee_PartitionInitContextRefType;
typedef P2CONST(struct Fee_PartitionInitContextStruct, AUTOMATIC, FEE_VAR_NOINIT) Fee_PartitionInitContextConstRefType;
...
typedef P2VAR(struct Fee_LookUpTableSyncData, AUTOMATIC, FEE_VAR_NOINIT) Fee_LookUpTableSyncDataRefType;
typedef P2CONST(struct Fee_LookUpTableSyncData, AUTOMATIC, FEE_VAR_NOINIT) Fee_LookUpTableSyncDataConstRefType;
typedef P2VAR(struct Fee_LookUpTableContextStruct, AUTOMATIC, FEE_VAR_NOINIT) Fee_LookUpTableContextRefType;
typedef P2CONST(struct Fee_LookUpTableContextStruct, AUTOMATIC, FEE_VAR_NOINIT) Fee_LookUpTableContextConstRefType;

typedef uint8 Fee_InternalErrorType;
...
struct Fee_ChunkHoppingContextStruct   /*<! Context, i.e. parameters and run-time variables, of Chunk Hopping operations */
{
    ...
    uint32 headerBuffer_au32[FEE_INTERNAL_BUFFER_SIZE / 4u]; /*<! Buffer to read a chenk header from flash into.
...
struct Fee_InstanceWriteContextStruct /*<! Context, i.e. parameters and run-time variables, of an instance write operation */
{
    ...
    uint16 numberOfCeilingBytes;          /*<! amount of ceiling bytes */
    uint8 numberOfWritesNecessary;        /* <! number of writes that must be issued to fls*/
    uint8 BufferBytes[FEE_INTERNAL_BUFFER_SIZE]; /*<! internal buffer; used for 1st and 3rd and 4th (last) write streams */
};
...
struct Fee_InstanceSearchContextStruct /*<! Context, i.e. parameters and run-time variables, of an instance search operation */
{
    ...
    uint32 Buffer_au32[FEE_INTERNAL_BUFFER_SIZE / 4u]; /*<! buffer to read an instance header or trailer into. Must be at least one single page, not just a byte or word! */
};
...
struct Fee_InstanceCopyContextStruct /*<! Context, i.e. parameters and run-time variables, of an instance copy operation */
{
    ...
    Fee_SectorSwitchStepType currentStep; /* <! Step that is currently processed, either read or write, necessary for error logging later*/ 
    struct Fee_ChunkInfoStruct targetChunk_t; /*<! Chunk that acts as traget for instance copy process */
...
struct Fee_ChunkAllocContextStruct  /*<! Context, i.e. parameters and run-time variables, of an chunk allocation operation */
{
    ...
    uint32 buffer_au32[FEE_INTERNAL_BUFFER_SIZE / 4u]; /*<! Buffer holding the chunk's byte-stream representation to be written to flash */
};
...
{
    uint32 sectorHeader_au32[FEE_INTERNAL_BUFFER_SIZE / 4u];   /*<! Buffer to write the sector header and the erase start marker */
    Fee_SectorRefType sector_pt;                               /*<! Pointer to the sector that shall be formated */
    uint16 newLinkTableSize;                                   /*<! Link table size that shall be used within the sector header that will be allocated
                                                                    Attention: This might be different to the actual link table size that is stored
                                                                    in the sector object (in case of config update) */
    uint8 sectorId_u8;                                         /*<! ID within the sector header that shall be allocated */
};
                                           ...
                                           Note: Partition it self, as well as its sectors are globally known */
{
    ...
    uint8 upperSectorId;              /*<! temporary hold upper sector's Id */
    sint8 recentlyCheckedSectorIndex; /*<! determines which sector was the latest one that was checked by the startup state machine*/
    uint32 sectorHeaderData[FEE_INTERNAL_BUFFER_SIZE / 4u]; /*<! sector header read buffer */
};

/* PRQA S 0750 2 */ /* MD_MSR_Union */
...
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
struct Fee_LookUpTableSyncData
{
    uint8 LookUpTablePartitionIndex; /*<! index of partition that shall be synchronized*/
    uint16 LookUpTableSyncBlockIndex; /*<! index of lut block that shall be read/written next. Read works with ascending index, write with descending index*/
};


struct Fee_LookUpTableContextStruct
{
...
struct Fee_tagProcessDataStruct /*<! Central data of currently running processing */
{
    ...
    struct Fee_tagBlockSpecificJob jobParams; /*<! currently executed job*/
};

typedef P2VAR(struct Fee_tagProcessDataStruct, AUTOMATIC, AUTOMATIC) Fee_ProcessDataRefType;
typedef P2CONST(struct Fee_tagProcessDataStruct, AUTOMATIC, AUTOMATIC) Fee_ProcessDataConstRefType;
...
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
    struct Fee_LookUpTableSyncData LookUpTableSyncData_t; /*!< parameters which are used for LUT initialization and synchronization */
    struct Fee_LookUpTableContextStruct LookUpTableContext_t; /*!< parameters which are used in LUT chunk search context */
#endif /* (FEE_LOOKUPTABLE_MODE == STD_ON) */
...
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
        Fee_BitFieldType syncLookUpTable:1; /*!< LUT synchronization was requested */
#endif /* (FEE_LOOKUPTABLE_MODE == STD_ON) */
...
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
        Fee_BitFieldType syncLookUpTable:1; /*!< LUT synchronization was started / is running */
#endif /*FEE_LOOKUPTABLE_MODE == STD_ON */
    } InternalFlags_t; /*<! flags only written by MainFunctuion; no sync necessary */
    Fee_InternalErrorType Fee_InternalErrorFlags;
